...
pub enum AuthMode {
    /// OpenAI API key provided by the caller and stored by Agere.
    ApiKey,
    /// ChatGPT OAuth managed by Agere (tokens persisted and refreshed by Agere).
    Chatgpt,
    ...
    ChatgptAuthTokens,
    /// Programmatic Agere auth backed by a registered Agent Identity.
...
client_request_definitions! {
    ...
    /// Execute a standalone command (argv vector) using the server's execution environment.
...
/// Superset of [`agere_file_search::FileMatch`]
#[derive(Serialize, Deserialize, Debug, Clone, PartialEq, JsonSchema, TS)]
...
server_notification_definitions! {
    ...
    /// This event is internal-only. Used by Agere Cloud.
    ...
    /// Notifies the user of world-writable directories on Windows, which Agere cannot reliably protect from writes.
...
mod tests {
    ...
    fn serialize_mcp_server_elicitation_request() -> Result<()> {
        ...
        let params = v2::McpServerElicitationRequestParams {
            ...
            server_name: "agere_apps".to_string(),
    ...
    fn serialize_client_response() -> Result<()> {
        ...
        let response = ClientResponse::ThreadStart {
            ...
            response: v2::ThreadStartResponse {
                ...
                access_policy: v2::AccessPolicy::DangerFullAccess,
                permission_profile: Some(
                    agere_protocol::models::PermissionProfile::from_legacy_access_policy(
                        &v2::AccessPolicy::DangerFullAccess.to_core(),
                    )
                    .into(),
                ),
