...
//!
//! If the keyring is not available or fails, we fall back to AGERE_HOME/.credentials.json which is consistent with other coding CLI agents.
...
use agere_keyring_store::DefaultKeyringStore;
use agere_keyring_store::KeyringStore;
use rmcp::transport::auth::AuthorizationManager;
use tokio::sync::Mutex;

use agere_utils_fs::find_agere_home;

const KEYRING_SERVICE: &str = "Agere MCP Credentials";
...
fn fallback_file_path() -> Result<PathBuf> {
    Ok(find_agere_home()?.join(FALLBACK_FILENAME).to_path_buf())
}
...
mod tests {
    ...
    use agere_keyring_store::tests::MockKeyringStore;

    struct TempAgereHome {
    ...
    impl TempAgereHome {
        fn new() -> Self {
            ...
            let dir = tempdir().expect("create AGERE_HOME temp dir");
            unsafe {
                std::env::set_var("AGERE_HOME", dir.path());
            }
            Self {
                _guard: guard,
                _dir: dir,
