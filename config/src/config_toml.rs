//! Schema-heavy configuration TOML types used by Agere.
...
use crate::types::AccessWorkspaceWrite;
...
use agere_app_server_protocol::Tools;
use agere_app_server_protocol::UserSavedConfig;
use agere_features::FeaturesToml;
use agere_model_provider_info::AMAZON_BEDROCK_PROVIDER_ID;
...
use agere_model_provider_info::OPENAI_PROVIDER_ID;
...
use agere_utils_fs::normalize_for_path_comparison;
...
/// Minimal model entry in `[[models]]`. Full `ModelInfo` is constructed at runtime.
...
pub struct ConfigToml {
    ...
    /// Access mode configuration to apply if `access_mode` is `WorkspaceWrite`.
    ...
    /// sanctioned by Agere will likely degrade model performance.
    ...
    /// keyring: Use an OS-specific keyring service.
    /// file: Use a file in the Agere home directory.
    ...
    /// Defaults to `$AGERE_SQLITE_HOME` when set. Otherwise uses `$AGERE_HOME`.
    pub sqlite_home: Option<AbsolutePathBuf>,

    /// Directory where Agere writes log files, for example `openagere.log`.
    /// Defaults to `$AGERE_HOME/log`.
    ...
    /// User-defined model entries for the /model picker.
    #[serde(default)]
    ...
    /// directories for `.openagere` folders. Defaults to [".git"] when unset.
    #[serde(default)]
...
impl From<ConfigToml> for UserSavedConfig {
    fn from(config_toml: ConfigToml) -> Self {
        ...
        Self {
            ...
            access_mode: config_toml.access_mode,
            access_settings: config_toml.access_workspace_write.map(From::from),
...
pub use agere_protocol::protocol::RealtimeConversationVersion as RealtimeWsVersion;
pub use agere_protocol::protocol::RealtimeVoice;
...
impl ConfigToml {
    ...
    pub async fn derive_permission_profile(
        ...
        let access_mode_was_explicit = access_mode_override.is_some()
            || profile_access_mode.is_some()
            || self.access_mode.is_some();
        let resolved_access_mode = access_mode_override
            .or(profile_access_mode)
            .or(self.access_mode)
            .or(if access_mode_was_explicit {
                None
            } else {
                // If no access_mode is set but this directory has a trust decision,
                // default to workspace-write except on Windows without execution restrictions where we
            ...
            .unwrap_or_default();
        let effective_access_mode = if cfg!(target_os = "windows")
            // If Windows execution restrictions are enabled, do not force a downgrade.
            && windows_execution_restriction_level == WindowsExecutionRestrictionLevel::Disabled
            && matches!(resolved_access_mode, AccessMode::WorkspaceWrite)
        {
            AccessMode::ReadOnly
        } else {
        ...
        let permission_profile = match effective_access_mode {
            AccessMode::ReadOnly => PermissionProfile::read_only(),
            AccessMode::WorkspaceWrite => match self.access_workspace_write.as_ref() {
                Some(AccessWorkspaceWrite {
                    writable_roots,
                    network_access,
            ...
            AccessMode::DangerFullAccess => PermissionProfile::Disabled,
        };
        if !access_mode_was_explicit
            && let Some(constraint) = permission_profile_constraint
            && let Err(err) = constraint.can_set(&permission_profile)
        {
            tracing::warn!(
                error = %err,
                "default permission profile is disallowed by requirements; falling back to required default"
            );
...
pub fn validate_oss_provider(provider: &str) -> std::io::Result<()> {
    ...
    Err(std::io::Error::new(
        std::io::ErrorKind::InvalidInput,
        format!(
            "OSS provider '{provider}' is no longer supported. Configure a custom provider in config.toml instead."
        ),
    ))
}
