...
async fn apply_role_preserves_unspecified_keys() {
    ...
    .await;
    config.agere_linux_exe = Some(PathBuf::from("/tmp/agere-linux-helper"));
    config.main_execve_wrapper_exe = Some(PathBuf::from("/tmp/agere-execve-wrapper"));
    let role_path = write_role_config(
        &home,
        "effort-only.toml",
    ...
    assert_eq!(config.model_reasoning_effort, Some(ReasoningEffort::High));
    assert_eq!(
        config.agere_linux_exe,
        Some(PathBuf::from("/tmp/agere-linux-helper"))
    );
    assert_eq!(
        config.main_execve_wrapper_exe,
        Some(PathBuf::from("/tmp/agere-execve-wrapper"))
    );
}
...
#[tokio::test]
#[cfg(not(windows))]
async fn apply_role_does_not_materialize_default_access_workspace_write_fields() {
    let (home, mut config) = test_config_with_cli_overrides(vec![
        (
        ...
        (
            "access_workspace_write.network_access".to_string(),
...
"#,
    ...
    let access_workspace_write = role_layer
        .config
        .get("access_workspace_write")
        .and_then(TomlValue::as_table)
        .expect("role layer should include access_workspace_write");
    assert_eq!(access_workspace_write.contains_key("network_access"), false);
    assert_eq!(
        access_workspace_write.contains_key("exclude_tmpdir_env_var"),
        false
    );
    assert_eq!(
        access_workspace_write.contains_key("exclude_slash_tmp"),
        false
    );
    ...
    match config.legacy_access_policy().as_str() {
        ...
        other => panic!("expected workspace-write access policy, got {other:?}"),
    }
}
...
"#,
    ...
    let outcome = skills_manager
        .skills_for_config(
            &skills_input,
            Some(Arc::clone(&agere_exec_server::LOCAL_FS)),
        )
        .await;
    let skill = outcome
        .skills
