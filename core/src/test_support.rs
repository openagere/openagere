...
use agere_models_manager::collaboration_mode_presets;
...
pub fn auth_manager_from_auth(auth: AgereAuth) -> Arc<AuthManager> {
    AuthManager::from_auth_for_testing(auth)
}

pub fn auth_manager_from_auth_with_home(auth: AgereAuth, agere_home: PathBuf) -> Arc<AuthManager> {
    AuthManager::from_auth_for_testing_with_home(auth, agere_home)
}
...
) -> agere_protocol::error::Result<crate::NewThread> {
    thread_manager
...
pub fn models_manager_with_provider(
    ...
    let provider = create_model_provider(provider, Some(auth_manager), vec![]);
