...
impl App {
            ...
            .await
        {
            Ok(thread) => {
                self.upsert_agent_picker_thread(
                    ...
                    matches!(
                        thread.status,
                        agere_app_server_protocol::ThreadStatus::NotLoaded
                    ),
                );
                true
            }
    ...
    pub(super) async fn select_agent_thread(
        ...
        if let Some(session) = snapshot.session.as_ref() {
            Self::sync_config_to_resumed_session(&mut config, session);
        }
        let previous_model_provider_id = self.config.model_provider_id.clone();
        self.config = config.clone();
        let model_catalog = Self::model_catalog_for_config_or_existing(
            &self.config,
    ...
    pub(super) async fn replace_chat_widget_with_app_server_thread(
        ...
        let init = self.chatwidget_init_for_forked_or_resumed_thread(
            ...
            self.config.clone(),
            initial_user_message,
            Some(started.session.model.clone()),
        );
        self.replace_chat_widget(ChatWidget::new_with_app_event(init));
    ...
    pub(super) async fn resume_target_session(
            ...
            .await
        {
            Ok(resumed) => {
                self.shutdown_current_thread(app_server).await;
                let previous_model_provider_id = self.config.model_provider_id.clone();
                Self::sync_config_to_resumed_session(&mut resume_config, &resumed.session);
    ...
    pub(super) fn sync_config_to_resumed_session(
        config: &mut Config,
        session: &crate::app_server_session::ThreadSessionState,
    ) {
        if let Some(provider) = config.model_providers.get(&session.model_provider_id) {
            config.model_provider_id = session.model_provider_id.clone();
            config.model_provider = provider.clone();
            if let Some(model) = Self::resumed_session_model_override(session) {
                config.model = Some(model);
            }
        } else {
            tracing::warn!(
                model_provider_id = %session.model_provider_id,
                "resumed session provider is missing from TUI config; keeping existing provider config"
            );
        }
    }

    pub(super) fn resumed_session_model_override(
        session: &crate::app_server_session::ThreadSessionState,
    ) -> Option<String> {
    ...
    pub(super) fn model_catalog_from_config(config: &Config) -> Arc<ModelCatalog> {
        let model = config
            .model
            .clone()
            .or_else(|| config.models.first().map(|model| model.name.clone()))
            .unwrap_or_default();
    ...
    pub(super) fn model_catalog_from_presets(
        config: &Config,
    ...
    pub(super) async fn model_catalog_for_config_or_existing(
        config: &Config,
        existing: Arc<ModelCatalog>,
    ...
    pub(super) async fn model_catalog_from_provider_config(config: &Config) -> Arc<ModelCatalog> {
    ...
    pub(super) fn is_builtin_model_provider(model_provider_id: &str) -> bool {
        agere_model_provider_info::BUILT_IN_PROVIDERS.contains(&model_provider_id)
    }

    pub(super) fn should_refresh_builtin_model_catalog(
        config: &Config,
        previous_model_provider_id: &str,
    ) -> bool {
        Self::is_builtin_model_provider(config.model_provider_id.as_str())
            && config.model_provider_id != previous_model_provider_id
    }
}
