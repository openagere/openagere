...
use agere_config::types::KeybindingsSpec;
    ...
    fn built_in_defaults() -> Self {
        Self {
            ...
            list: ListKeymap {
                ...
                move_left: default_bindings![plain(KeyCode::Left)],
                move_right: default_bindings![plain(KeyCode::Right)],
...
Set unique keys in `~/.openagere/config.toml` and retry. \
...
fn validate_no_reserved<const N: usize>(
    ...
    for (action, bindings) in pairs {
        for binding in bindings {
            ...
            {
                if action == "chat.interrupt_turn"
                    && *reserved_action == "fixed.return_from_side_or_backtrack"
                    && key == key_hint::plain(KeyCode::Esc).parts()
                {
                    continue;
                }
                return Err(format!(
                    "Ambiguous `tui.keymap.{context}` bindings: `{action}` uses a key reserved by `{reserved_action}`. \
Set a different key in `~/.openagere/config.toml` and retry. \
See the Agere keymap documentation for supported actions and examples."
                ));
            }
        }
...
mod tests {
    use super::*;
    use agere_config::types::KeybindingSpec;
    ...
    #[test]
    fn interrupt_turn_defaults_to_escape_and_can_be_remapped_or_unbound() {
        let mut keymap = TuiKeymap::default();
        ...
        keymap.chat.interrupt_turn = Some(KeybindingsSpec::Many(Vec::new()));
        let runtime = RuntimeKeymap::from_config(&keymap).expect("unbound keymap should parse");
        assert_eq!(runtime.chat.interrupt_turn, Vec::new());
    }
