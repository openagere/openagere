...
fn write_curated_plugin_sha(agere_home: &Path) {
    write_file(
...
fn has_plugins_clone_dirs(agere_home: &Path) -> bool {
    let Ok(entries) = std::fs::read_dir(agere_home.join(".tmp")) else {
        return false;
    };
...
async fn run_http_sync(
    ...
    let api_base_url = api_base_url.into();
    tokio::task::spawn_blocking(move || {
        sync_openai_plugins_repo_via_http(agere_home.as_path(), &api_base_url)
    })
    .await
    .expect("sync task should join")
}
...
if [ "$1" = "clone" ]; then
  dest="$5"
  mkdir -p "$dest/.git" "$dest/.agents/plugins" "$dest/plugins/gmail/.agere-plugin"
  cat > "$dest/.agents/plugins/marketplace.json" <<'EOF'
{{"name":"openagere-curated","plugins":[{{"name":"gmail","source":{{"source":"local","path":"./plugins/gmail"}}}}]}}
EOF
  printf '%s\n' '{{"name":"gmail"}}' > "$dest/plugins/gmail/.agere-plugin/plugin.json"
  exit 0
fi
...
fn sync_openai_plugins_repo_via_git_succeeds_with_local_rewritten_remote() {
    ...
    std::fs::create_dir_all(work_repo.join("plugins/gmail/.agere-plugin"))
    ...
    std::fs::write(
        work_repo.join("plugins/gmail/.agere-plugin/plugin.json"),
        r#"{"name":"gmail"}"#,
    )
    ...
    let commit_status = Command::new("git")
        ...
        .arg(&work_repo)
        .arg("-c")
        .arg("user.name=Agere Test")
        .arg("-c")
        .arg("user.email=agere@example.com")
        .arg("commit")
        .arg("-m")
        .arg("init")
        .status()
...
async fn sync_openai_plugins_repo_skips_export_archive_when_snapshot_exists() {
    ...
    let plugin_manifest_path = curated_root.join("plugins/linear/.agere-plugin/plugin.json");
...
fn curated_repo_zipball_bytes(sha: &str) -> Vec<u8> {
  ...
  "plugins": [
    ...
    writer
        .start_file(
            format!("{root}/plugins/gmail/.agere-plugin/plugin.json"),
            options,
        )
  ...
  "plugins": [
    {
      ...
      "source": {
        ...
        .expect("write marketplace");
    writer
        .start_file("plugins/plugins/gmail/.agere-plugin/plugin.json", options)
        .expect("start plugin manifest entry");
    writer
        .write_all(br#"{"name":"gmail"}"#)
        .expect("write plugin manifest");
