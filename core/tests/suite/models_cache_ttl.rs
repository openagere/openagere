...
use agere_models_manager::client_version_to_whole;
...
const REMOTE_MODEL: &str = "agere-test-ttl";
const VERSIONED_MODEL: &str = "agere-test-versioned";
const MISSING_VERSION_MODEL: &str = "agere-test-missing-version";
const DIFFERENT_VERSION_MODEL: &str = "agere-test-different-version";
...
async fn renews_cache_ttl_on_matching_models_etag() -> Result<()> {
    ...
    let agere = Arc::clone(&test.agere);
    ...
    let cache_path = config.agere_home.join(CACHE_FILE);
    ...
    agere
        .submit(Op::UserTurn {
            ...
            approval_policy: agere_protocol::protocol::AskForApproval::Never,
    ...
    let _ = wait_for_event(&agere, |event| matches!(event, EventMsg::TurnComplete(_))).await;
