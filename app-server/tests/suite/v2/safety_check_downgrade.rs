...
const SERVER_MODEL: &str = "gpt-5.3";
...
#[tokio::test]
#[ignore]
async fn provider_model_header_mismatch_emits_model_rerouted_notification_v2() -> Result<()> {
    ...
    assert_eq!(
        ...
        ModelReroutedNotification {
            ...
            reason: ModelRerouteReason::ProviderSafetyRouting,
        }
    );
...
async fn cyber_policy_response_emits_typed_error_notification_v2() -> Result<()> {
    ...
    assert_eq!(
        error,
        ErrorNotification {
            error: agere_app_server_protocol::TurnError {
    ...
    loop {
        ...
        match notification.method.as_str() {
            "error" => {
                ...
                if payload.error.agere_error_info == Some(AgereErrorInfo::CyberPolicy) {
                    error = Some(payload);
                }
            }
            "model/rerouted" => {
