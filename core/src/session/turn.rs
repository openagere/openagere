...
use agere_analytics::AppInvocation;
use agere_analytics::CompactionPhase;
use agere_analytics::CompactionReason;
use agere_analytics::InvocationType;
use agere_analytics::TurnResolvedConfigFact;
use agere_analytics::build_track_events_context;
use agere_async_utils::OrCancelExt;
use agere_features::Feature;
use agere_hooks::HookEvent;
use agere_hooks::HookEventAfterAgent;
use agere_hooks::HookPayload;
use agere_hooks::HookResult;
use agere_protocol::config_types::ModeKind;
use agere_protocol::error::AgereErr;
use agere_protocol::error::Result as AgereResult;
use agere_protocol::items::PlanItem;
...
use agere_protocol::protocol::AgentReasoningSectionBreakEvent;
...
use agere_tools::filter_tool_suggest_discoverable_tools_for_client;
use agere_utils_common::AssistantTextChunk;
use agere_utils_common::AssistantTextStreamParser;
use agere_utils_common::ProposedPlanSegment;
use agere_utils_common::extract_proposed_plan_text;
use agere_utils_common::strip_citations;
...
pub(crate) async fn run_turn(
    ...
    let available_connectors = if turn_context.apps_enabled() {
        let connectors = agere_connectors::merge::merge_plugin_connectors_with_accessible(
            loaded_plugins
                .effective_apps()
                .into_iter()
    ...
    // Although from the perspective of agere.rs, TurnDiffTracker has the lifecycle of a Task which contains
        ...
        .await
        {
            Ok(sampling_request_output) => {
                ...
                if !needs_follow_up {
                    ...
                    .to_string();
                    let stop_request = agere_hooks::StopRequest {
                    ...
                    for run in sess.hooks().preview_stop(&stop_request) {
                        sess.send_event(
                            &turn_context,
                            EventMsg::HookStarted(agere_protocol::protocol::HookStartedEvent {
            ...
            Err(AgereErr::TurnAborted) => {
                // Aborted turn is reported via a different event.
                break;
            }
            Err(AgereErr::InvalidImageRequest()) => {
                {
...
/// Pure trigger decision for pre-sampling downshift compaction.
///
/// Fires when usage exceeds the new model's auto-compact limit, the model or provider changed,
/// and the new context window is strictly smaller than the old one. An empty `old_provider_id`
/// means "unknown" (e.g. after rollout reconstruction) and never counts as a provider change.
#[allow(clippy::too_many_arguments)]
pub(super) fn should_run_downshift_compaction(
    ...
    // Empty old_provider_id == "unknown" (e.g. after rollout reconstruction); do not
    // treat it as a provider change. Only a known, differing provider id counts.
    let provider_changed = !old_provider_id.is_empty() && old_provider_id != new_provider_id;
    let changed = old_model != new_model || provider_changed;
...
async fn maybe_run_previous_model_inline_compact(
    ...
    // `previous_turn_settings.model` is moved into `with_model`, so capture the
    // provider/window fields we still need afterwards first.
    let previous_provider_id = previous_turn_settings.provider_id.clone();
    let previous_context_window = previous_turn_settings.context_window;
    ...
    // Prefer the window recorded for the previous turn. When it's unknown (e.g. after
    // rollout reconstruction), fall back to resolving the old model on the current manager.
    let old_context_window = match previous_context_window {
        Some(w) => w,
...
pub(super) fn collect_explicit_app_ids_from_skill_items(
    ...
    for connector in connectors {
        let slug = agere_connectors::metadata::connector_mention_slug(connector);
...
fn connector_inserted_in_messages(
    ...
    let mention_slug = agere_connectors::metadata::connector_mention_slug(connector);
...
async fn run_sampling_request(
    ...
    loop {
        ...
        if retries < max_retries {
            retries += 1;
            let delay = match &err {
                AgereErr::Stream(_, requested_delay) => {
...
impl ProposedPlanItemState {
    ...
    async fn complete_with_text(
        ...
        sess.emit_turn_item_completed(turn_context, item, is_interim)
            .await;
    }
}
...
fn agent_message_text(item: &agere_protocol::items::AgentMessageItem) -> String {
    item.content
        .iter()
        .map(|entry| match entry {
            agere_protocol::items::AgentMessageContent::Text { text } => text.as_str(),
        })
        .collect()
}
...
async fn maybe_complete_plan_item_from_message(
        ...
        && role == "assistant"
    {
        ...
        if let Some(plan_text) = extract_proposed_plan_text(&text) {
            ...
            state
                .plan_item_state
                .complete_with_text(sess, turn_context, plan_text, is_interim)
                .await;
        }
    }
}
...
async fn emit_agent_message_in_plan_mode(
    sess: &Session,
    turn_context: &TurnContext,
    agent_message: agere_protocol::items::AgentMessageItem,
    state: &mut PlanModeStreamState,
    is_interim: bool,
) {
        ...
        .contains(&agent_message_id)
    {
        let start_item = state
            ...
            .unwrap_or_else(|| {
                TurnItem::AgentMessage(agere_protocol::items::AgentMessageItem {
...
async fn emit_turn_item_in_plan_mode(
    ...
    match turn_item {
        TurnItem::AgentMessage(agent_message) => {
            emit_agent_message_in_plan_mode(sess, turn_context, agent_message, state, is_interim)
                .await;
        }
        _ => {
            ...
            sess.emit_turn_item_completed(turn_context, turn_item, is_interim)
                .await;
        }
    }
}
...
async fn handle_assistant_item_done_in_plan_mode(
    ...
    if let ResponseItem::Message { role, .. } = item
        && role == "assistant"
    {
        maybe_complete_plan_item_from_message(sess, turn_context, state, item, is_interim).await;
...
async fn try_run_sampling_request(
    ...
    feedback_tags!(
        model = turn_context.model_info.slug.clone(),
        approval_policy = turn_context.approval_policy.value(),
        access_policy = &turn_context.access_policy(),
    ...
    let inference_trace = sess.services.rollout_thread_trace.inference_trace_context(
        ...
        turn_context.model_info.slug.as_str(),
        turn_context.provider.info().wire_api.to_string(),
    );
    let mut stream = client_session
        .stream(
            ...
            turn_metadata_header,
            turn_context.provider_changed_for_turn,
            &inference_trace,
        )
    ...
    let mut pending_provisional_completions: VecDeque<TurnItem> = VecDeque::new();
    let receiving_span = trace_span!("receiving_stream");
    let outcome: AgereResult<SamplingRequestResult> = loop {
        let handle_responses = trace_span!(
            ...
            agere.usage.reasoning_output_tokens = field::Empty,
            agere.usage.total_tokens = field::Empty,
        );
        ...
        {
            Ok(event) => event,
            Err(agere_async_utils::CancelErr::Cancelled) => break Err(AgereErr::TurnAborted),
        };

        let event = match event {
            Some(Ok(event)) => event,
            Some(Err(err)) => {
                flush_provisional_completions(
                    &sess,
                    &turn_context,
                    &mut pending_provisional_completions,
                    true,
                )
                .await;
                break Err(err);
        ...
        match event {
            ...
            ResponseEvent::OutputItemDone { item, completion } => {
                ...
                // When a tool call consumes a message's active_item (Anthropic
                // buffered-message ordering), restore it so the deferred
                // OutputItemDone(Message) can find its ItemStarted context.
                let previously_active_was_agent_message =
                    matches!(&previously_active_item, Some(TurnItem::AgentMessage(_)));
                let previously_active_clone = previously_active_item.clone();
                let plan_is_interim = !in_flight.is_empty();
                ...
                let (output_result, completion) = match handle_output_item_done(
                    &mut ctx,
                    item,
                    previously_active_item,
                    completion,
                ...
                .await
                {
                    Ok((output_result, completion)) => (output_result, completion),
                    Err(err) => break Err(err),
                };
                if let Some(tool_future) = output_result.tool_future {
                    in_flight.push_back(tool_future);
                    // Restore a message active_item that was consumed by this
                    // tool call, so the buffered OutputItemDone(Message) can
                    // find its ItemStarted context (Anthropic ordering).
                    if previously_active_was_agent_message && active_item.is_none() {
                        active_item = previously_active_clone;
                    }
                }
                ...
                if let Some(item) = output_result.completed_item {
                    match completion {
                        OutputItemCompletion::Final => {
                            // Use `in_flight` as the interim check: if there are pending tool
                            // futures, the message is interim (more content is coming). Unlike
                            // `needs_follow_up`, `in_flight` reflects real-time queue state and
                            // goes back to empty once tools complete.
                            let is_interim = !in_flight.is_empty();
                            sess.emit_turn_item_completed(&turn_context, item, is_interim)
                                .await;
                        }
                        OutputItemCompletion::Provisional => {
                            pending_provisional_completions.push_back(item);
                        }
                        _ => {
                            // Future-proof: treat unknown completion states as Final.
                            let is_interim = !in_flight.is_empty();
                            sess.emit_turn_item_completed(&turn_context, item, is_interim)
                                .await;
                        }
            ...
            ResponseEvent::OutputItemAdded(item) => {
                ...
                } else if let ResponseItem::FunctionCall { call_id, name, .. } = &item {
                    let tool_name = ToolName::plain(name.as_str());
                    active_tool_argument_diff_consumer = tool_runtime
                        .create_diff_consumer(&tool_name)
                        .map(|consumer| (call_id.clone(), consumer));
                        ...
                        && let Some(raw_text) = raw_assistant_output_text_from_item(&item)
                    {
                        ...
                        if let TurnItem::AgentMessage(agent_message) = &mut turn_item {
                            agent_message.content =
                                vec![agere_protocol::items::AgentMessageContent::Text {
                                    text: if plan_mode {
                                        String::new()
                                    } else {
            ...
            ResponseEvent::Completed {
                ...
                // Flush remaining provisional completions on turn completion.
                let is_interim = end_turn != Some(true);
                flush_provisional_completions(
                    &sess,
                    &turn_context,
                    &mut pending_provisional_completions,
...
mod downshift_tests {
    ...
    fn triggers_on_provider_change_same_model() {
        assert!(should_run_downshift_compaction(
            /*total*/ 300_000, /*new_auto_compact_limit*/ 230_000,
            /*old_model*/ "gpt-5", /*new_model*/ "gpt-5", /*old_provider*/ "openai",
            /*new_provider*/ "small", /*old_window*/ 1_000_000,
            /*new_window*/ 256_000,
        ));
    }
    ...
    // CRITICAL: empty old_provider_id means "unknown" (after rollout reconstruction).
    // It must NOT count as a provider change.
    #[test]
    fn empty_old_provider_id_is_not_a_change() {
        // Over limit, window IS smaller, same model — the only reason this must NOT
        // trigger is that an empty old_provider_id means "unknown", not a provider change.
        // (If the empty-provider guard were removed, "" != "openai" would make this true.)
        assert!(!should_run_downshift_compaction(
            /*total*/ 300_000, /*new_auto_compact_limit*/ 230_000,
