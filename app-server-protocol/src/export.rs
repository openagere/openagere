...
pub fn generate_json_with_experimental(out_dir: &Path, experimental_api: bool) -> Result<()> {
    ...
    write_pretty_json(
        out_dir.join("agere_app_server_protocol.schemas.json"),
        &bundle,
    )?;
    let flat_v2_bundle = build_flat_v2_schema(&bundle)?;
    write_pretty_json(
        out_dir.join("agere_app_server_protocol.v2.schemas.json"),
        &flat_v2_bundle,
    )?;
...
fn build_schema_bundle(schemas: Vec<GeneratedSchema>) -> Result<Value> {
    ...
    root.insert(
        "title".to_string(),
        Value::String("AgereAppServerProtocol".into()),
    );
...
fn build_flat_v2_schema(bundle: &Value) -> Result<Value> {
    ...
    let title = root
        ...
        .unwrap_or("AgereAppServerProtocol");
...
mod tests {
    ...
    fn stable_schema_filter_removes_mock_thread_start_field() -> Result<()> {
        let output_dir = std::env::temp_dir().join(format!("agere_schema_{}", Uuid::now_v7()));
    ...
    fn build_flat_v2_schema_keeps_shared_root_schemas_and_dependencies() -> Result<()> {
        ...
        assert_eq!(
            flat_bundle["title"],
            serde_json::json!("AgereAppServerProtocolV2")
        );
    ...
    fn experimental_type_fields_ts_filter_handles_interface_shape() -> Result<()> {
        let output_dir = std::env::temp_dir().join(format!("agere_ts_filter_{}", Uuid::now_v7()));
 ...
 *
 * Uses the same shape as thread/turn execution access configuration and
...
"#;
        ...
        assert_eq!(filtered.contains("accessPolicy?: AccessPolicy"), true);
        assert_eq!(
            filtered.contains(r#"import type { AccessPolicy } from "./AccessPolicy";"#),
            true
        );
        Ok(())
    }
    ...
    fn generate_json_filters_experimental_fields_and_methods() -> Result<()> {
        ...
        assert_eq!(
            flat_v2_bundle_json.contains("\"title\": \"AgereAppServerProtocolV2\""),
            true
        );
        let flat_v2_bundle =
            read_json_value(&output_dir.join("agere_app_server_protocol.v2.schemas.json"))?;
