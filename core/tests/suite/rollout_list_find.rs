...
use agere_core::EventPersistenceMode;
use agere_core::RolloutRecorder;
use agere_core::RolloutRecorderParams;
...
fn write_minimal_rollout_with_id_in_subdir(agere_home: &Path, subdir: &str, id: Uuid) -> PathBuf {
    let sessions = agere_home.join(subdir).join("2024/01/01");
...
/// provided conversation id in the SessionMeta line. Returns the absolute path.
fn write_minimal_rollout_with_id(agere_home: &Path, id: Uuid) -> PathBuf {
    write_minimal_rollout_with_id_in_subdir(agere_home, "sessions", id)
}

async fn upsert_thread_metadata(agere_home: &Path, thread_id: ThreadId, rollout_path: PathBuf) {
    let runtime = StateRuntime::init(agere_home.to_path_buf(), "test-provider".to_string())
        .await
        .unwrap();
    runtime
    ...
    builder.cwd = agere_home.to_path_buf();
...
#[tokio::test]
async fn find_handles_gitignore_covering_agere_home_directory() {
    let repo = TempDir::new().unwrap();
    let agere_home = repo.path().join(".openagere");
    std::fs::create_dir_all(&agere_home).unwrap();
    std::fs::write(repo.path().join(".gitignore"), ".openagere/**\n").unwrap();
    let id = Uuid::new_v4();
    let expected = write_minimal_rollout_with_id(&agere_home, id);

    let found = find_thread_path_by_id_str(&agere_home, &id.to_string())
        .await
        .unwrap();
