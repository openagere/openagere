...
use agere_app_server_protocol::Config as ApiConfig;
use agere_app_server_protocol::ConfigBatchWriteParams;
use agere_app_server_protocol::ConfigLayerMetadata;
...
use agere_app_server_protocol::OverriddenMetadata;
...
use agere_config::merge_toml_values;
use agere_core::config::deserialize_config_toml_with_base;
use agere_core::config::edit::ConfigEdit;
use agere_core::config::edit::ConfigEditsBuilder;
use agere_core::config::validate_feature_requirements_for_config_toml;
use agere_core::path_utils;
use agere_core::path_utils::SymlinkWritePaths;
use agere_core::path_utils::resolve_symlink_write_paths;
use agere_core::path_utils::write_atomically;
...
impl ConfigManager {
    ...
    async fn apply_edits(
        ...
        let allowed_path =
            AbsolutePathBuf::resolve_path_against_base(CONFIG_TOML_FILE, self.agere_home());
        ...
        let user_config_toml =
            deserialize_config_toml_with_base(user_config.clone(), self.agere_home()).map_err(
                |err| {
                    ConfigManagerError::write(
                        ConfigWriteErrorCode::ConfigValidationError,
                        format!("Invalid configuration: {err}"),
    ...
    /// include any in-repo .openagere/ folders because there is no cwd/project root
...
fn override_message(layer: &ConfigLayerSource) -> String {
    match layer {
        ...
        ConfigLayerSource::Project { dot_agere_folder } => format!(
            "Overridden by project config: {}/{CONFIG_TOML_FILE}",
            dot_agere_folder.display(),
        ),
