...
/// Special argv[1] flag used when the Agere executable self-invokes to run the
/// internal `apply_patch` path.
///
/// Although this constant lives in `agere-apply-patch` (to avoid forcing
/// `agere-arg0` to depend on `agere-core`), it remains part of the "agere core"
/// process-invocation contract for the standalone `apply_patch` command
/// surface.
pub const AGERE_CORE_APPLY_PATCH_ARG1: &str = "--agere-run-as-apply-patch";
    ...
    for hunk in hunks {
        ...
        match hunk {
            Hunk::AddFile { contents, .. } => {
                write_file_with_missing_parent_retry(fs, &path_abs, contents.clone().into_bytes())
                    .await?;
            ...
            Hunk::UpdateFile {
                ...
                if let Some(dest) = move_path {
                    ...
                    write_file_with_missing_parent_retry(fs, &dest_abs, new_contents.into_bytes())
                        .await?;
                    let result: io::Result<()> = async {
                        let metadata = fs.get_metadata(&path_abs).await?;
                        if metadata.is_directory {
...
async fn write_file_with_missing_parent_retry(
    ...
    match fs.write_file(path_abs, contents.clone()).await {
        Ok(()) => Ok(()),
