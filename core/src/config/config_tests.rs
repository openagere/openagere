...
use agere_config::config_toml::AgentRoleToml;
use agere_config::config_toml::AgentsToml;
use agere_config::config_toml::AutoReviewToml;
...
use agere_config::config_toml::RealtimeToml;
use agere_config::config_toml::RealtimeTransport;
use agere_config::config_toml::RealtimeWsMode;
use agere_config::config_toml::RealtimeWsVersion;
use agere_config::config_toml::ToolsToml;
use agere_config::loader::project_trust_key;
use agere_config::permissions_toml::FilesystemPermissionToml;
use agere_config::permissions_toml::FilesystemPermissionsToml;
use agere_config::permissions_toml::NetworkDomainPermissionToml;
use agere_config::permissions_toml::NetworkDomainPermissionsToml;
use agere_config::permissions_toml::NetworkToml;
use agere_config::permissions_toml::PermissionProfileToml;
use agere_config::permissions_toml::PermissionsToml;
use agere_config::profile_toml::ConfigProfile;
use agere_config::types::AccessWorkspaceWrite;
use agere_config::types::AppToolApproval;
use agere_config::types::ApprovalsReviewer;
use agere_config::types::BundledSkillsConfig;
use agere_config::types::FeedbackConfigToml;
use agere_config::types::HistoryPersistence;
use agere_config::types::McpServerEnvVar;
use agere_config::types::McpServerToolConfig;
use agere_config::types::McpServerTransportConfig;
use agere_config::types::MemoriesConfig;
use agere_config::types::MemoriesToml;
...
use agere_config::types::WindowsToml;
...
async fn derive_legacy_access_policy_for_test(
    cfg: &ConfigToml,
    access_mode_override: Option<AccessMode>,
    profile_access_mode: Option<AccessMode>,
    windows_execution_restriction_level: WindowsExecutionRestrictionLevel,
    ...
    let permission_profile = cfg
        ...
        .await;
    permission_profile
        .to_legacy_access_policy(Path::new("/"))
        .unwrap_or_else(|err| {
            tracing::warn!(
                error = %err,
                "derived permission profile cannot be represented as a legacy access-policy label; falling back to read-only"
            );
            "read-only".to_string()
        })
}
...
async fn load_config_loads_global_agents_instructions() -> std::io::Result<()> {
    let agere_home = tempdir()?;
    std::fs::write(
        agere_home.path().join(DEFAULT_AGENTS_MD_FILENAME),
        "\n  global instructions  \n",
    )?;
...
async fn load_config_prefers_global_agents_override_instructions() -> std::io::Result<()> {
    ...
    let global_agents_override_path = agere_home.path().join(LOCAL_AGENTS_MD_FILENAME);
...
-> std::io::Result<()> {
    ...
    .await?;
    assert_eq!(
        config.permissions.network_access_policy(),
...
#[tokio::test]
async fn default_permissions_profile_populates_runtime_access_policy() -> std::io::Result<()> {
    let agere_home = TempDir::new()?;
    let cwd = TempDir::new()?;
    ...
    let memories_root = agere_home.path().join("memories").abs();
    assert_eq!(
        config.permissions.file_system_access_policy(),
...
-> std::io::Result<()> {
    ...
    requirements.network = Some(Sourced::new(
        agere_config::NetworkConstraints {
            enabled: Some(true),
            ..Default::default()
        },
...
async fn project_root_glob_none_compiles_to_filesystem_pattern_entry() -> std::io::Result<()> {
    ...
    assert!(
        config
            ...
            .file_system_access_policy()
            .entries
            .contains(&FileSystemAccessEntry {
...
-> std::io::Result<()> {
    ...
    let policy = config.permissions.file_system_access_policy();
    assert!(
...
async fn empty_config_defaults_to_builtin_profile_for_trusted_project() -> std::io::Result<()> {
    ...
    if cfg!(target_os = "windows") {
        assert!(
            !policy.can_write_path_with_cwd(cwd.path(), cwd.path()),
            "expected trusted project fallback to stay read-only without Windows execution restrictions, policy: {policy:?}"
        );
    } else {
        assert!(
            policy.can_write_path_with_cwd(cwd.path(), cwd.path()),
        ...
        assert!(
            !policy.can_write_path_with_cwd(&cwd.path().join(".openagere"), cwd.path()),
...
#[tokio::test]
async fn implicit_builtin_workspace_profile_preserves_access_workspace_write_settings()
-> std::io::Result<()> {
    let agere_home = TempDir::new()?;
    let cwd = TempDir::new()?;
    ...
    let config = Config::load_from_base_config_with_overrides(
        ConfigToml {
            ...
            access_workspace_write: Some(AccessWorkspaceWrite {
            ...
            windows: Some(WindowsToml {
                access_mode: Some(WindowsAccessModeToml::Elevated),
    ...
    assert!(
        ...
        "expected implicit :workspace to preserve access_workspace_write.writable_roots, policy: {policy:?}"
    );
    assert_eq!(
        config.permissions.network_access_policy(),
        NetworkAccessPolicy::Enabled
    ...
    if cfg!(not(target_os = "windows")) {
        assert_eq!(config.legacy_access_policy(), "workspace-write");
    }
    Ok(())
}
...
-> std::io::Result<()> {
    ...
    let cwd = TempDir::new()?;
    let extra_root = TempDir::new()?;
    for subpath in [".git", ".agents", ".openagere"] {
...
#[tokio::test]
async fn default_permissions_can_select_builtin_danger_full_access_profile() -> std::io::Result<()>
{
    let agere_home = TempDir::new()?;
    let cwd = TempDir::new()?;
    ...
    assert_eq!(
        permission_profile.file_system_access_policy().kind,
        FileSystemAccessLevel::Unrestricted
    );
    Ok(())
}
...
-> std::io::Result<()> {
    ...
    let _memories_root = AbsolutePathBuf::from_absolute_path(std::fs::canonicalize(
        agere_home.path().join("memories"),
    )?)?;
    assert!(
        config
            .permissions
...
async fn permissions_profiles_allow_unknown_special_paths() -> std::io::Result<()> {
    ...
    assert_eq!(&config.legacy_access_policy(), &"read-only".to_string());
    assert!(
        config.startup_warnings.iter().any(|warning| warning.contains(
            "Configured filesystem path `:future_special_path` is not recognized by this version of Agere and will be ignored."
        )),
        "{:?}",
...
-> std::io::Result<()> {
    ...
    assert!(
        config.startup_warnings.iter().any(|warning| warning.contains(
            "Configured filesystem path `:future_special_path` with nested entry `docs` is not recognized by this version of Agere and will be ignored."
        )),
        "{:?}",
        config.startup_warnings
    );
...
async fn permissions_profiles_allow_missing_filesystem_with_warning() -> std::io::Result<()> {
    ...
    assert!(
        config.startup_warnings.iter().any(|warning| warning.contains(
            "Permissions profile `workspace` does not define any recognized filesystem entries for this version of Agere."
        )),
        "{:?}",
        config.startup_warnings
    );
...
async fn permissions_profiles_allow_network_enablement() -> std::io::Result<()> {
    ...
    assert!(
        config.permissions.network_access_policy().is_enabled(),
        "expected network access policy to be enabled",
    );
    Ok(())
}
...
#[tokio::test]
async fn test_legacy_access_mode_config_parsing() {
    let legacy_full_access_toml = r#"
access_mode = "danger-full-access"

[access_workspace_write]
network_access = false  # This should be ignored.
"#;
    let legacy_full_access_cfg = toml::from_str::<ConfigToml>(legacy_full_access_toml)
        .expect("TOML deserialization should succeed");
    let access_mode_override = None;
    let resolution = derive_legacy_access_policy_for_test(
        &legacy_full_access_cfg,
        access_mode_override,
        /*profile_access_mode*/ None,
        WindowsExecutionRestrictionLevel::Disabled,
        /*active_project*/ None,
        /*permission_profile_constraint*/ None,
    )
    .await;
    assert_eq!(resolution, "danger-full-access".to_string());

    let legacy_read_only_toml = r#"
...
"#;
    ...
    let legacy_read_only_cfg = toml::from_str::<ConfigToml>(legacy_read_only_toml)
        .expect("TOML deserialization should succeed");
    let access_mode_override = None;
    let resolution = derive_legacy_access_policy_for_test(
        &legacy_read_only_cfg,
        access_mode_override,
        /*profile_access_mode*/ None,
        WindowsExecutionRestrictionLevel::Disabled,
        /*active_project*/ None,
    ...
    .await;
    assert_eq!(resolution, "read-only".to_string());

    let writable_root = test_absolute_path("/my/workspace");
    let access_workspace_write = format!(
        r#"
...
"#,
    ...
    let access_workspace_write_cfg = toml::from_str::<ConfigToml>(&access_workspace_write)
        .expect("TOML deserialization should succeed");
    let access_mode_override = None;
    let resolution = derive_legacy_access_policy_for_test(
        &access_workspace_write_cfg,
        access_mode_override,
        /*profile_access_mode*/ None,
        WindowsExecutionRestrictionLevel::Disabled,
        /*active_project*/ None,
    ...
    if cfg!(target_os = "windows") {
        ...
        assert_eq!(resolution, "workspace-write".to_string());
    }
...
#[tokio::test]
async fn legacy_access_mode_builds_profiles_with_compatible_projection() -> std::io::Result<()> {
    let agere_home = TempDir::new()?;
    let cwd = TempDir::new()?;
    ...
    let cases = vec![
        (
            "danger-full-access".to_string(),
            r#"access_mode = "danger-full-access"
"#
            .to_string(),
        ),
        (
...
"#,
    ...
    for (name, config_toml) in cases {
        ...
        let access_policy = config.legacy_access_policy();
        let file_system_policy = config.permissions.file_system_access_policy();
        let network_policy = config.permissions.network_access_policy();
        ...
        let profile_round_trip = config
            .permissions
            .permission_profile()
            .to_legacy_access_policy(cwd.path())
            .unwrap_or_else(|err| panic!("case `{name}` should round-trip: {err}"));
        assert_eq!(
            profile_round_trip, access_policy,
        ...
        match name.as_str() {
            ...
            "workspace-write" => {
                if cfg!(target_os = "windows") {
                    assert_eq!(
                        ...
                        "legacy workspace-write should keep the existing Windows downgrade when \
                         Windows execution restrictions are disabled"
                    );
                    assert_eq!(
                        file_system_policy,
                        FileSystemAccessPolicy::from_legacy_access_policy_for_cwd(
                ...
                assert!(file_system_policy.entries.contains(&FileSystemAccessEntry {
                    path: FileSystemPath::Special {
                        value: FileSystemSpecialPath::project_roots(/*subpath*/ None),
                    },
                    access: FileSystemAccessMode::Write,
                ...
                for subpath in [".git", ".agents", ".openagere"] {
                    assert!(
                        file_system_policy.entries.contains(&FileSystemAccessEntry {
                            path: FileSystemPath::Special {
...
async fn add_dir_override_extends_workspace_writable_roots() -> std::io::Result<()> {
    ...
    let overrides = ConfigOverrides {
        cwd: Some(frontend),
        access_mode: Some(AccessMode::WorkspaceWrite),
    ...
    if cfg!(target_os = "windows") {
        match config.legacy_access_policy().as_str() {
            "read-only" => {}
        ...
        assert!(
            config
                .permissions
                .file_system_access_policy()
                .can_write_path_with_cwd(expected_backend.as_path(), config.cwd.as_path()),
            "expected backend path writable under workspace-write policy"
        );
    }
...
#[tokio::test]
async fn sqlite_home_defaults_to_agere_home_for_workspace_write() -> std::io::Result<()> {
    let agere_home = TempDir::new()?;
    let config = Config::load_from_base_config_with_overrides(
        ConfigToml::default(),
        ConfigOverrides {
    ...
    assert_eq!(config.sqlite_home, agere_home.path().to_path_buf());
...
async fn workspace_write_always_includes_memories_root_once() -> std::io::Result<()> {
    let agere_home = TempDir::new()?;
    let memories_root = agere_home.path().join("memories");
    let config = Config::load_from_base_config_with_overrides(
        ConfigToml {
            access_workspace_write: Some(AccessWorkspaceWrite {
    ...
    if cfg!(target_os = "windows") {
        ...
        assert!(
            config
                .permissions
                .file_system_access_policy()
                .can_write_path_with_cwd(expected_memories_root.as_path(), config.cwd.as_path()),
            "expected memories root writable under workspace-write policy"
        );
    }
...
fn web_search_mode_for_turn_uses_preference_for_read_only() {
    ...
    let permission_profile = PermissionProfile::from_legacy_access_policy("read-only");
...
async fn project_profile_overrides_user_profile() -> std::io::Result<()> {
    ...
    std::fs::write(
        agere_home.path().join(CONFIG_TOML_FILE),
        format!(
            r#"
profile = "global"
...
"#,
    ...
    let project_config_dir = workspace.path().join(".openagere");
...
#[tokio::test]
async fn profile_access_mode_overrides_base() -> std::io::Result<()> {
    let agere_home = TempDir::new()?;
    let mut profiles = HashMap::new();
    profiles.insert(
        "work".to_string(),
        ConfigProfile {
            access_mode: Some(AccessMode::DangerFullAccess),
            ..Default::default()
        },
    );
    let cfg = ConfigToml {
    ...
    assert!(matches!(
        config.legacy_access_policy().as_str(),
        "danger-full-access"
    ));
...
#[tokio::test]
async fn cli_override_takes_precedence_over_profile_access_mode() -> std::io::Result<()> {
    let agere_home = TempDir::new()?;
    let mut profiles = HashMap::new();
    profiles.insert(
        "work".to_string(),
    ...
    let config =
        Config::load_from_base_config_with_overrides(cfg, overrides, agere_home.abs()).await?;
...
async fn managed_config_overrides_oauth_store_mode() -> anyhow::Result<()> {
    let agere_home = TempDir::new()?;
    let managed_path = agere_home.path().join("managed_config.toml");
    let config_path = agere_home.path().join(CONFIG_TOML_FILE);
    ...
    .await?;
    let cfg =
        deserialize_config_toml_with_base(config_layer_stack.effective_config(), agere_home.path())
            .map_err(|e| {
...
async fn load_global_mcp_servers_returns_empty_if_missing() -> anyhow::Result<()> {
    ...
    let servers = load_global_mcp_servers(agere_home.path()).await?;
...
async fn replace_mcp_servers_round_trips_entries() -> anyhow::Result<()> {
    ...
    let loaded = load_global_mcp_servers(agere_home.path()).await?;
...
async fn to_mcp_config_preserves_apps_feature_from_config() -> std::io::Result<()> {
    ...
    .await?;
    let plugins_manager = PluginsManager::new(agere_home.path().to_path_buf());
...
"#,
    ...
    let err = load_global_mcp_servers(agere_home.path())
        .await
...
async fn replace_mcp_servers_serializes_cwd() -> anyhow::Result<()> {
    ...
    let cwd_path = PathBuf::from("/tmp/agere-mcp");
    let servers = BTreeMap::from([(
        "docs".to_string(),
        McpServerConfig {
            transport: McpServerTransportConfig::Stdio {
    ...
    let serialized = std::fs::read_to_string(&config_path)?;
    assert!(
        serialized.contains(r#"cwd = "/tmp/agere-mcp""#),
    ...
    match &docs.transport {
        McpServerTransportConfig::Stdio { cwd, .. } => {
            assert_eq!(cwd.as_deref(), Some(Path::new("/tmp/agere-mcp")));
        }
        other => panic!("unexpected transport {other:?}"),
    }
...
async fn set_model_updates_defaults() -> anyhow::Result<()> {
    ...
    ConfigEditsBuilder::new(agere_home.path())
        .set_model(Some("gpt-5.4"), Some(ReasoningEffort::High), None)
        .apply()
        .await?;

    let serialized = tokio::fs::read_to_string(agere_home.path().join(CONFIG_TOML_FILE)).await?;
...
impl PrecedenceTestFixture {
    ...
    fn agere_home(&self) -> AbsolutePathBuf {
        self.agere_home.abs()
    }
}
...
async fn loads_compact_prompt_from_file() -> std::io::Result<()> {
    let agere_home = TempDir::new()?;
    let workspace = agere_home.path().join("workspace");
...
async fn load_config_uses_requirements_guardian_policy_config() -> std::io::Result<()> {
    ...
    let config_layer_stack = ConfigLayerStack::new(
        Vec::new(),
        Default::default(),
        agere_config::ConfigRequirementsToml {
    ...
    let config = Config::load_config_with_layer_stack(
        ...
        ConfigToml::default(),
        ConfigOverrides {
            cwd: Some(agere_home.path().to_path_buf()),
            ..Default::default()
        },
        agere_home.abs(),
        config_layer_stack,
...
async fn load_config_rejects_missing_agent_role_config_file() -> std::io::Result<()> {
    let agere_home = TempDir::new()?;
    let missing_path = agere_home.path().join("agents").join("researcher.toml");
    let cfg = ConfigToml {
...
async fn agent_role_relative_config_file_resolves_against_config_toml() -> std::io::Result<()> {
    let agere_home = TempDir::new()?;
    let role_config_path = agere_home.path().join("agents").join("researcher.toml");
...
"#,
    ...
    let config = ConfigBuilder::without_managed_config_for_tests()
        .agere_home(agere_home.path().to_path_buf())
        .fallback_cwd(Some(agere_home.path().to_path_buf()))
        .build()
        .await?;
    assert_eq!(
        config
...
"#,
    ...
    let config_layer_stack = agere_config::ConfigLayerStack::new(
        vec![agere_config::ConfigLayerEntry::new(
            agere_app_server_protocol::ConfigLayerSource::User {
                file: agere_home.path().join(CONFIG_TOML_FILE).abs(),
            },
            layer_config,
        )],
        Default::default(),
        agere_config::ConfigRequirementsToml::default(),
    )
    .map_err(std::io::Error::other)?;
...
"#
    ...
    let standalone_agents_dir = repo_root.path().join(".openagere").join("agents");
...
async fn loads_legacy_split_agent_roles_from_config_toml() -> std::io::Result<()> {
    let agere_home = TempDir::new()?;
    let researcher_path = agere_home.path().join("agents").join("researcher.toml");
    let reviewer_path = agere_home.path().join("agents").join("reviewer.toml");
    tokio::fs::create_dir_all(
        researcher_path
            .parent()
            .expect("role config should have a parent directory"),
...
"#
    ...
    let home_agents_dir = agere_home.path().join("agents");
...
async fn model_catalog_json_loads_from_path() -> std::io::Result<()> {
    let agere_home = TempDir::new()?;
    let catalog_path = agere_home.path().join("catalog.json");
...
"#;
    ...
    let agere_home_temp_dir = TempDir::new().unwrap();
    ...
    Ok(PrecedenceTestFixture {
        cwd: cwd_temp_dir,
        agere_home: agere_home_temp_dir,
        cfg,
        model_provider_map,
        openai_provider,
    ...
    assert_eq!(
        Config {
            ...
            permissions: Permissions {
                ...
                windows_access_mode: None,
                windows_execution_private_desktop: true,
            },
            ...
            sqlite_home: fixture.agere_home().to_path_buf(),
            log_dir: fixture.agere_home().join("log").to_path_buf(),
...
async fn test_requirements_web_search_mode_allowlist_does_not_warn_when_unset() -> anyhow::Result<()>
{
    ...
    let requirements_toml = agere_config::ConfigRequirementsToml {
        allowed_approval_policies: None,
        allowed_approvals_reviewers: None,
        allowed_access_modes: None,
        remote_access_config: None,
        allowed_web_search_modes: Some(vec![agere_config::WebSearchModeRequirement::Cached]),
    ...
    let requirement_source = agere_config::RequirementSource::Unknown;
    ...
    let requirements = agere_config::ConfigRequirements {
        web_search_mode: agere_config::ConstrainedWithSource::new(
            constrained,
    ...
    let config_layer_stack =
        agere_config::ConfigLayerStack::new(Vec::new(), requirements, requirements_toml)
...
projects = { "/Users/mbolin/code/agere4" = { trust_level = "trusted", foo = "bar" } , "/Users/mbolin/code/agere3" = { trust_level = "trusted" } }
model = "foo""#;
    let mut doc = initial.parse::<DocumentMut>()?;

    // Approve a new directory
    let new_project = Path::new("/Users/mbolin/code/agere2");
...
[projects."/Users/mbolin/code/agere4"]
trust_level = "trusted"
foo = "bar"

[projects."/Users/mbolin/code/agere3"]
trust_level = "trusted"
...
#[test]
fn test_set_default_oss_provider_rejects_all_providers() -> std::io::Result<()> {
    let temp_dir = TempDir::new()?;
    let agere_home = temp_dir.path();

    let result = set_default_oss_provider(agere_home, LEGACY_OLLAMA_CHAT_PROVIDER_ID);
...
#[tokio::test]
async fn test_untrusted_project_gets_workspace_write_profile() -> anyhow::Result<()> {
...
"#;
    ...
    if cfg!(target_os = "windows") {
        assert!(
            matches!(resolution.as_str(), "read-only"),
            "Expected ReadOnly on Windows, got {resolution:?}"
        );
    } else {
        assert!(
            matches!(resolution.as_str(), "workspace-write"),
...
#[tokio::test]
async fn derive_access_policy_falls_back_to_read_only_for_implicit_defaults() -> anyhow::Result<()>
{
...
#[tokio::test]
async fn derive_access_policy_preserves_windows_downgrade_for_unsupported_fallback()
-> anyhow::Result<()> {
    let project_dir = TempDir::new()?;
    let project_path = project_dir.path().to_path_buf();
    let project_key = project_path.to_string_lossy().to_string();
    ...
    let constrained = Constrained::new(
        PermissionProfile::from_legacy_access_policy("workspace-write"),
        |candidate| {
            if matches!(
                candidate,
...
async fn test_untrusted_project_gets_unless_trusted_approval_policy() -> anyhow::Result<()> {
    ...
    // Verify that untrusted projects still get workspace-write access (or ReadOnly on Windows)
    if cfg!(target_os = "windows") {
        assert!(
            matches!(config.legacy_access_policy().as_str(), "read-only"),
            "Expected ReadOnly on Windows"
        );
    } else {
        assert!(
            matches!(config.legacy_access_policy().as_str(), "workspace-write"),
            "Expected workspace-write access profile for untrusted project"
        );
    }
...
#[tokio::test]
async fn requirements_disallowing_default_permission_profile_falls_back_to_required_default()
-> std::io::Result<()> {
    let agere_home = TempDir::new()?;
    ...
    let config = ConfigBuilder::without_managed_config_for_tests()
        ...
        .cloud_requirements(CloudRequirementsLoader::new(async {
            Ok(Some(agere_config::ConfigRequirementsToml {
                allowed_access_modes: Some(vec![agere_config::AccessModeRequirement::ReadOnly]),
                ..Default::default()
            }))
        }))
        .build()
...
#[tokio::test]
async fn explicit_access_mode_falls_back_when_disallowed_by_requirements() -> std::io::Result<()> {
    let agere_home = TempDir::new()?;
    std::fs::write(
        agere_home.path().join(CONFIG_TOML_FILE),
        r#"access_mode = "danger-full-access"
"#,
    ...
    let requirements = agere_config::ConfigRequirementsToml {
        allowed_approval_policies: None,
        allowed_approvals_reviewers: None,
        allowed_access_modes: Some(vec![agere_config::AccessModeRequirement::ReadOnly]),
        remote_access_config: None,
...
-> std::io::Result<()> {
    ...
    let expected_access_policy = "read-only".to_string();
    assert_eq!(config.legacy_access_policy(), expected_access_policy);
    assert_eq!(
...
async fn permission_profile_override_preserves_split_write_roots() -> std::io::Result<()> {
    let agere_home = TempDir::new()?;
    let cwd = agere_home.path().join("workspace");
    let outside_root = agere_home.path().join("outside-write");
    ...
    let permission_profile = PermissionProfile::from_runtime_permissions_with_filesystem_isolation(
        ...
        .cloud_requirements(CloudRequirementsLoader::new(async {
            Ok(Some(agere_config::ConfigRequirementsToml {
                allowed_web_search_modes: Some(vec![
                    agere_config::WebSearchModeRequirement::Cached,
                ]),
                ..Default::default()
            }))
        }))
...
async fn feature_requirements_normalize_effective_feature_values() -> std::io::Result<()> {
    ...
    let config = ConfigBuilder::without_managed_config_for_tests()
        ...
        .cloud_requirements(CloudRequirementsLoader::new(async {
            Ok(Some(agere_config::ConfigRequirementsToml {
                feature_requirements: Some(agere_config::FeatureRequirementsToml {
                    entries: BTreeMap::from([
...
discoverables = [
  ...
  { type = "plugin", id = "plugin_alpha@openagere-curated" },
  { type = "connector", id = "   " }
]
"#,
    )
        ...
        Some(ToolSuggestConfig {
            discoverables: vec![
                ...
                ToolSuggestDiscoverable {
                    ...
                    id: "plugin_alpha@openagere-curated".to_string(),
                },
                ToolSuggestDiscoverable {
                    kind: ToolSuggestDiscoverableType::Connector,
                    id: "   ".to_string(),
