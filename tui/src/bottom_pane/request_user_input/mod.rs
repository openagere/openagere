...
impl RequestUserInputOverlay {
    ...
    pub(crate) fn new(
        ...
        Self::new_with_interrupt_keys(
            request,
            app_event_tx,
            has_input_focus,
            enhanced_keys_supported,
            disable_paste_burst,
            RuntimeKeymap::defaults().chat.interrupt_turn,
        )
    }

    pub(crate) fn new_with_interrupt_keys(
        request: RequestUserInputEvent,
        app_event_tx: AppEventSender,
        has_input_focus: bool,
        enhanced_keys_supported: bool,
    ...
    fn current_question(
        &self,
    ) -> Option<&agere_protocol::request_user_input::RequestUserInputQuestion> {
    ...
    fn footer_tips(&self) -> Vec<FooterTip> {
        ...
        {
            let interrupt_label = ratatui::text::Span::from(interrupt_key).content.to_string();
            tips.push(FooterTip::new(format!("{interrupt_label} to interrupt")));
        }
        tips
    }
...
mod tests {
    ...
    fn expect_interrupt_only(rx: &mut tokio::sync::mpsc::UnboundedReceiver<AppEvent>) {
        ...
        let AppEvent::AgereOp(op) = event else {
            panic!("expected AgereOp");
        };
    ...
    #[test]
    fn remapped_interrupt_key_interrupts_while_notes_are_visible() {
        let (tx, mut rx) = test_sender();
