...
#[test]
#[ignore = "requires tmux and a locally built agere binary; run with --ignored for manual resize smoke"]
fn tmux_split_preserves_fresh_session_composer_row_after_resize_reflow() -> Result<()> {
    ...
    let repo_root = agere_utils_cargo_bin::repo_root()?;
    let agere = agere_binary(&repo_root)?;
    ...
    write_auth(agere_home.path())?;

    let session_name = format!("agere-resize-reflow-smoke-{}", std::process::id());
    ...
    let start_output = checked_output(
        Command::new("tmux")
            ...
            .arg("--")
            .arg("env")
            .arg(format!("AGERE_HOME={}", agere_home.path().display()))
            .arg("OPENAI_API_KEY=dummy")
            .arg(format!("AGERE_RS_SSE_FIXTURE={}", fixture.display()))
            .arg(agere)
            .arg("-c")
    ...
    let agere_pane = stdout_text(&start_output).trim().to_string();
    anyhow::ensure!(!agere_pane.is_empty(), "tmux did not report a pane id");
    ...
    let baseline_capture =
        wait_for_capture_contains(&agere_pane, draft, Duration::from_secs(/*secs*/ 15))?;
    ...
    let final_capture = capture_pane(&agere_pane)?;
    let final_row =
...
fn tmux_width_resize_restore_keeps_visible_content_anchored() -> Result<()> {
    ...
    let session_name = format!("agere-resize-width-{}", std::process::id());
    let _session = TmuxSession {
        name: session_name.clone(),
    };
    ...
    sleep(Duration::from_millis(/*millis*/ 1_000));
    let restored_capture = capture_pane(&agere_pane)?;
    let restored_row =
...
fn run_repeated_resize_smoke(terminal_resize_reflow_enabled: bool) -> Result<()> {
    ...
    let session_name = format!("agere-resize-repeat-{suffix}-{}", std::process::id());
    let _session = TmuxSession {
        name: session_name.clone(),
    };
...
fn agere_binary(repo_root: &Path) -> Result<PathBuf> {
    if let Ok(path) = agere_utils_cargo_bin::cargo_bin("agere") {
        return Ok(path);
    }

    let fallback = repo_root.join("target/debug/agere");
    anyhow::ensure!(
        fallback.is_file(),
        "agere binary is unavailable; run `cargo build -p agere-cli` first"
    );
    Ok(fallback)
}
...
"#
    ...
    std::fs::write(agere_home.join("config.toml"), config)?;
    Ok(())
}

fn write_auth(agere_home: &Path) -> Result<()> {
    std::fs::write(
