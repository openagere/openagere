...
use agere_config::types::OtelHttpProtocol;
...
fn set_metrics_exporter(config: &mut agere_core::config::Config) {
...
async fn app_server_default_analytics_disabled_without_flag() -> Result<()> {
    ...
    let provider = agere_core::otel_init::build_provider(
        &config,
        SERVICE_VERSION,
...
pub(crate) async fn mount_analytics_capture(server: &MockServer, agere_home: &Path) -> Result<()> {
    Mock::given(method("POST"))
        .and(path("/agere/analytics-events/events"))
        ...
        loop {
            ...
            for request in &requests {
                if request.method != "POST"
                    || request.url.path() != "/agere/analytics-events/events"
                {
                    continue;
                }
...
pub(crate) fn thread_initialized_event(payload: &Value) -> Result<&Value> {
    ...
    events
        .iter()
        .find(|event| event["event_type"] == "agere_thread_initialized")
        .ok_or_else(|| anyhow::anyhow!("agere_thread_initialized event should be present"))
}
