//! Persist Agere session rollouts (.jsonl) so sessions can be replayed or inspected later.
...
use agere_protocol::protocol::GitInfo as ProtocolGitInfo;
...
///
/// ```ignore
/// $ jq -C . ~/.openagere/sessions/rollout-2025-05-07T17-24-21-5973b6c0-94b8-487b-a530-2aeb6098ae0e.jsonl
/// $ fx ~/.openagere/sessions/rollout-2025-05-07T17-24-21-5973b6c0-94b8-487b-a530-2aeb6098ae0e.jsonl
/// ```
#[derive(Clone)]
...
impl RolloutRecorder {
    /// List threads (rollout files) under the provided Agere home directory.
    ...
    async fn list_threads_with_db_fallback(
        ...
        let agere_home = config.agere_home();
...
async fn list_threads_from_files_desc(
    ...
    if let Some(search_term) = search_term {
        ...
        loop {
            ...
            filter_thread_items_by_search_term(agere_home, &mut page.items, Some(search_term))
                .await?;
...
async fn list_threads_from_files_desc_unfiltered(
    ...
    if archived {
        let root = agere_home.join(ARCHIVED_SESSIONS_SUBDIR);
        get_threads_in_root(
            root,
            page_size,
            cursor,
...
async fn list_threads_from_files_asc(
    ...
    filter_thread_items_by_search_term(agere_home, &mut all_items, search_term).await?;
...
fn precompute_log_file_info(
    ...
    // Resolve ~/.openagere/sessions/YYYY/MM/DD path.
    let timestamp = OffsetDateTime::now_local()
        .map_err(|e| IoError::other(format!("failed to get local time: {e}")))?;
    let mut dir = config.agere_home().to_path_buf();
...
async fn sync_thread_state_after_write(
    ...
    if new_thread_memory_mode.is_some()
        || items
            .iter()
            .any(agere_state::rollout_item_affects_thread_metadata)
    {
...
impl From<agere_state::ThreadsPage> for ThreadsPage {
    fn from(db_page: agere_state::ThreadsPage) -> Self {
        let items = db_page
            .items
            .into_iter()
