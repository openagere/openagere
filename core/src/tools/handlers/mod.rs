...
pub(crate) fn normalize_and_validate_additional_permissions(
    ...
    let uses_additional_permissions = matches!(
        permission_level,
        ExecutionPermissionLevel::WithAdditionalPermissions
    );
    ...
    if additional_permissions.is_some() {
        Err(
            "`additional_permissions` requires `permission_level` set to `with_additional_permissions`"
                .to_string(),
        )
    } else {
        Ok(None)
...
pub(super) fn implicit_granted_permissions(
    ...
    if !permission_level.uses_additional_permissions()
        && !matches!(permission_level, ExecutionPermissionLevel::RequireEscalated)
...
pub(super) async fn apply_granted_turn_permissions(
    ...
    if matches!(permission_level, ExecutionPermissionLevel::RequireEscalated) {
    ...
    let permission_level =
        if effective_permissions.is_some() && !permission_level.uses_additional_permissions() {
            ExecutionPermissionLevel::WithAdditionalPermissions
        } else {
            permission_level
        };
