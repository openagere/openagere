...
ON CONFLICT(id) DO NOTHING
        ...
        .bind(preview)
        .bind(metadata.access_policy.as_str())
...
fn thread_spawn_parent_thread_id_from_source_str(source: &str) -> Option<ThreadId> {
    ...
    match parsed_source.ok() {
        Some(SessionSource::SubAgent(agere_protocol::protocol::SubAgentSource::ThreadSpawn {
            parent_thread_id,
            ..
...
mod tests {
    ...
    use agere_protocol::protocol::GitInfo;
    ...
    async fn upsert_thread_keeps_creation_memory_mode_for_existing_rows() {
        let agere_home = unique_temp_dir();
        let runtime = StateRuntime::init(agere_home.clone(), "test-provider".to_string())
            .await
            .expect("state db should initialize");
        let thread_id =
            ThreadId::from_string("00000000-0000-0000-0000-000000000123").expect("valid thread id");
        let mut metadata = test_thread_metadata(&agere_home, thread_id, agere_home.clone());
    ...
    async fn list_threads_filters_by_cwd() {
        ...
        let first_cwd = agere_home.join("first");
        let second_cwd = agere_home.join("second");
        let other_cwd = agere_home.join("other");
        ...
        for (thread_id, cwd, updated_at) in [
            ...
            let mut metadata = test_thread_metadata(&agere_home, thread_id, cwd);
            metadata.updated_at =
    ...
    async fn apply_rollout_items_restores_memory_mode_from_session_meta() {
        ...
        let metadata = test_thread_metadata(&agere_home, thread_id, agere_home.clone());
    ...
    async fn apply_rollout_items_preserves_existing_git_branch_and_fills_missing_git_fields() {
        ...
        let items = vec![RolloutItem::SessionMeta(SessionMetaLine {
            ...
            git: Some(GitInfo {
                commit_hash: Some(agere_git_utils::GitSha::new("rollout-sha")),
                branch: Some("rollout-branch".to_string()),
                repository_url: Some("git@example.com:openai/agere.git".to_string()),
            }),
        })];
        ...
        assert_eq!(
            ...
            Some("git@example.com:openai/agere.git")
        );
    }
    ...
    async fn update_thread_git_info_preserves_newer_non_git_metadata() {
        ...
        let updated = runtime
            .update_thread_git_info(
                ...
                Some(Some("git@example.com:openai/agere.git")),
            )
            .await
    ...
    async fn insert_thread_if_absent_preserves_existing_metadata() {
        ...
        let mut existing = test_thread_metadata(&agere_home, thread_id, agere_home.clone());
        ...
        let mut fallback = test_thread_metadata(&agere_home, thread_id, agere_home.clone());
    ...
    async fn update_thread_git_info_can_clear_fields() {
        ...
        metadata.git_origin_url = Some("git@example.com:openai/agere.git".to_string());
    ...
    async fn set_thread_preview_if_empty_only_fills_blank_preview() {
        ...
        let changed = runtime
            ...
            .await
            .expect("blank preview set should succeed");
        assert!(!changed);
        let changed = runtime
            .set_thread_preview_if_empty(thread_id, "  goal preview  ")
            .await
            .expect("preview set should succeed");
        assert!(changed);
        let changed = runtime
            .set_thread_preview_if_empty(thread_id, "new preview")
            .await
            .expect("second preview set should succeed");
        assert!(!changed);
        ...
        assert_eq!(persisted.first_user_message, None);
    }
    ...
    async fn thread_updated_at_uses_unique_epoch_millis_and_reads_legacy_seconds() {
        ...
        let mut first = test_thread_metadata(&agere_home, first_id, agere_home.clone());
        first.updated_at = updated_at;
        let mut second = test_thread_metadata(&agere_home, second_id, agere_home.clone());
        ...
        let mut older = test_thread_metadata(&agere_home, older_id, agere_home.clone());
    ...
    async fn apply_rollout_items_uses_override_updated_at_when_provided() {
        ...
        let items = vec![RolloutItem::EventMsg(EventMsg::TokenCount(
            agere_protocol::protocol::TokenCountEvent {
                info: Some(agere_protocol::protocol::TokenUsageInfo {
                    total_token_usage: agere_protocol::protocol::TokenUsage {
                        input_tokens: 0,
                    ...
                    last_token_usage: agere_protocol::protocol::TokenUsage::default(),
    ...
    async fn thread_spawn_edges_track_directional_status() {
        let agere_home = unique_temp_dir();
        let runtime = StateRuntime::init(agere_home, "test-provider".to_string())
            .await
            .expect("state db should initialize");
