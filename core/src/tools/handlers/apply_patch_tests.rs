use super::*;
use agere_apply_patch::MaybeApplyPatchVerified;
...
async fn invocation_for_payload(payload: ToolPayload) -> ToolInvocation {
    ...
    ToolInvocation {
        ...
        tool_name: agere_tools::ToolName::plain("apply_patch"),
...
*** End Patch"#;
    ...
    let action =
        match agere_apply_patch::maybe_parse_apply_patch_verified(&argv, &cwd, LOCAL_FS.as_ref())
            .await
        {
...
fn write_permissions_for_paths_skip_dirs_already_writable_under_workspace_root() {
    ...
    let access_policy =
        FileSystemAccessPolicy::from_legacy_access_policy_for_cwd("workspace-write", cwd_path);

    let permissions = write_permissions_for_paths(&[file_path], &access_policy, &cwd);
...
fn write_permissions_for_paths_keep_dirs_outside_workspace_root() {
    ...
    let cwd_abs = cwd.abs();
    let access_policy = FileSystemAccessPolicy::from_legacy_access_policy_for_cwd(
        "workspace-write",
        cwd_abs.as_path(),
    );

    let permissions = write_permissions_for_paths(&[file_path], &access_policy, &cwd_abs);
