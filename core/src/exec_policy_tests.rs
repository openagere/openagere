...
use agere_config::ConfigLayerEntry;
...
use agere_config::RequirementsExecPolicy;
...
fn config_stack_for_dot_agere_folder(dot_agere_folder: &Path) -> ConfigLayerStack {
    let dot_agere_folder =
        AbsolutePathBuf::from_absolute_path(dot_agere_folder).expect("absolute dot_agere_folder");
...
fn read_only_file_system_access_policy() -> FileSystemAccessPolicy {
...
fn workspace_write_file_system_access_policy() -> FileSystemAccessPolicy {
    FileSystemAccessPolicy::workspace_write(
        &[],
...
fn unrestricted_file_system_access_policy() -> FileSystemAccessPolicy {
    FileSystemAccessPolicy::unrestricted()
}

fn external_file_system_access_policy() -> FileSystemAccessPolicy {
    FileSystemAccessPolicy::external_filesystem_policy()
}

fn permission_profile_from_access_policy(access_policy: &str) -> PermissionProfile {
...
async fn returns_empty_policy_when_no_policy_files_exist() {
    ...
    let config_stack = config_stack_for_dot_agere_folder(temp_dir.path());
...
async fn merges_requirements_exec_policy_network_rules() -> anyhow::Result<()> {
    ...
    requirements_exec_policy.add_network_rule(
        "blocked.example.com",
        agere_execpolicy::NetworkRuleProtocol::Https,
        Decision::Forbidden,
        /*justification*/ None,
    )?;

    let requirements = ConfigRequirements {
        exec_policy: Some(agere_config::Sourced::new(
            agere_config::RequirementsExecPolicy::new(requirements_exec_policy),
            agere_config::RequirementSource::Unknown,
        )),
        ..ConfigRequirements::default()
    };
    let dot_agere_folder = AbsolutePathBuf::from_absolute_path(temp_dir.path())?;
    let layer = ConfigLayerEntry::new(
        ConfigLayerSource::Project { dot_agere_folder },
        TomlValue::Table(Default::default()),
    );
...
async fn ignore_user_project_rules_keeps_system_policy_files() {
    ...
    .expect("write policy file");
    let config_file =
        AbsolutePathBuf::from_absolute_path(config_dir.join(agere_config::CONFIG_TOML_FILE))
...
async fn ignores_rules_from_untrusted_project_layers() -> anyhow::Result<()> {
    ...
    let project_dot_agere_folder = AbsolutePathBuf::from_absolute_path(project_dir.path())?;
    let layers = vec![ConfigLayerEntry::new_disabled(
        ConfigLayerSource::Project {
            dot_agere_folder: project_dot_agere_folder,
        },
        TomlValue::Table(Default::default()),
        "marked untrusted",
    )];
...
async fn evaluates_bash_lc_inner_commands() {
    assert_exec_approval_requirement_for_command(
        ExecApprovalRequirementScenario {
            ...
            access_policy: "danger-full-access".to_string(),
            file_system_access_policy: unrestricted_file_system_access_policy(),
...
async fn ignore_user_config_keeps_user_policy_files() -> std::io::Result<()> {
    let temp = tempdir()?;
    let agere_home = temp.path().join("home_ignore_user_config");
    let rules_dir = agere_home.join(RULES_DIR_NAME);
    fs::create_dir_all(&rules_dir)?;
    fs::write(
        agere_home.join(CONFIG_TOML_FILE),
...
async fn evaluates_heredoc_script_against_prefix_rules() {
    ...
    assert_exec_approval_requirement_for_command(
        ExecApprovalRequirementScenario {
            ...
            file_system_access_policy: read_only_file_system_access_policy(),
            permission_level: ExecutionPermissionLevel::UseDefault,
            prefix_rule: None,
        },
...
#[test]
fn unmatched_granular_policy_still_prompts_for_restricted_access_escalation() {
    ...
    assert_eq!(
        ...
        render_decision_for_unmatched_command(
            ...
            &permission_profile_from_access_policy("read-only"),
            &read_only_file_system_access_policy(),
            Path::new("/tmp"),
            &command,
            ExecutionPermissionLevel::RequireEscalated,
...
fn unmatched_on_request_uses_split_filesystem_policy_for_escalation_prompts() {
    let command = vec!["madeup-cmd".to_string()];
    let restricted_file_system_policy = FileSystemAccessPolicy::restricted(vec![]);
...
async fn exec_approval_requirement_prompts_for_inline_additional_permissions_under_on_request() {
    assert_exec_approval_requirement_for_command(
        ExecApprovalRequirementScenario {
            ...
            access_policy: "read-only".to_string(),
            file_system_access_policy: read_only_file_system_access_policy(),
            permission_level: ExecutionPermissionLevel::WithAdditionalPermissions,
            prefix_rule: None,
        },
        ExecApprovalRequirement::NeedsApproval {
            reason: None,
...
#[tokio::test]
async fn exec_approval_requirement_rejects_unmatched_access_escalation_when_granular_access_escalation_is_disabled()
 {
    assert_exec_approval_requirement_for_command(
        ExecApprovalRequirementScenario {
            policy_src: None,
        ...
        ExecApprovalRequirement::Forbidden {
            reason: REJECT_ESCALATION_APPROVAL_REASON.to_string(),
        },
    )
    .await;
}
...
#[tokio::test]
async fn mixed_rule_and_access_escalation_prompt_prioritizes_rule_for_rejection_decision() {
    ...
    let requirement = manager
        .create_exec_approval_requirement_for_command(ExecApprovalRequest {
            ...
            permission_profile: permission_profile_from_access_policy("read-only"),
            file_system_access_policy: &read_only_file_system_access_policy(),
            access_cwd: Path::new("/tmp"),
            permission_level: ExecutionPermissionLevel::RequireEscalated,
            prefix_rule: None,
        })
        .await;
...
#[tokio::test]
async fn mixed_rule_and_access_escalation_prompt_rejects_when_granular_rules_are_disabled() {
    let policy_src = r#"prefix_rule(pattern=["git"], decision="prompt")"#;
    let mut parser = PolicyParser::new();
    parser
        .parse("test.rules", policy_src)
...
async fn request_rule_falls_back_when_prefix_rule_does_not_approve_all_commands() {
    let command = vec![
        "bash".to_string(),
        "-lc".to_string(),
        "cargo install cargo-insta && rm -rf /tmp/agere".to_string(),
    ];
    let manager = ExecPolicyManager::default();
...
async fn append_execpolicy_amendment_updates_policy_and_file() {
    let agere_home = tempdir().expect("create temp dir");
    let prefix = vec!["echo".to_string(), "hello".to_string()];
    let manager = ExecPolicyManager::default();

    manager
        .append_amendment_and_update(agere_home.path(), &ExecPolicyAmendment::from(prefix))
        .await
    ...
    let contents = fs::read_to_string(default_policy_path(agere_home.path()))
...
async fn append_execpolicy_amendment_rejects_empty_prefix() {
    ...
    let result = manager
        .append_amendment_and_update(agere_home.path(), &ExecPolicyAmendment::from(vec![]))
        .await;
...
#[tokio::test]
async fn multi_segment_shell_requires_policy_allow_for_every_segment_to_bypass_managed_restriction()
{
    let policy_src = r#"
    ...
    for approval_policy in [AskForApproval::OnRequest, AskForApproval::Never] {
        assert_exec_approval_requirement_for_command(
            ExecApprovalRequirementScenario {
                ...
                command: command.clone(),
                approval_policy,
                access_policy: "workspace-write".to_string(),
                file_system_access_policy: workspace_write_file_system_access_policy(),
                permission_level: ExecutionPermissionLevel::UseDefault,
                prefix_rule: None,
            },
            ExecApprovalRequirement::Skip {
...
#[tokio::test]
async fn multi_segment_shell_bypasses_managed_restriction_when_every_segment_matches_policy_allow()
{
    let policy_src = r#"
prefix_rule(pattern=["cat"], decision="allow")
...
async fn verify_approval_requirement_for_unsafe_powershell_command() {
    ...
    let permissions = ExecutionPermissionLevel::UseDefault;

    // This command should not be run without user approval unless managed access
    // enforcement makes that safe.
    ...
    let (pwsh_approval_reason, expected_req) = if cfg!(windows) {
        (
            r#"On Windows, read-only managed access policy should be assumed to mean
                that no Agere filesystem envelope is present, so anything that is not "provably
        ...
        (
            "On non-Windows, rely on read-only filesystem policy to prevent harm.",
            ExecApprovalRequirement::Skip {
                proposed_execpolicy_amendment: expected_amendment.clone(),
            },
        )
...
#[tokio::test]
async fn dangerous_command_allowed_when_access_restriction_explicitly_disabled() {
    let command = vec_str(&["rm", "-rf", "/tmp/nonexistent"]);
    assert_exec_approval_requirement_for_command(
        ExecApprovalRequirementScenario {
            policy_src: None,
            command,
            approval_policy: AskForApproval::Never,
            access_policy: "external-access".to_string(),
            file_system_access_policy: external_file_system_access_policy(),
            permission_level: ExecutionPermissionLevel::UseDefault,
            prefix_rule: None,
        },
        ExecApprovalRequirement::Skip {
...
#[tokio::test]
async fn dangerous_command_forbidden_in_external_filesystem_policy_when_policy_matches() {
    let command = vec_str(&["rm", "-rf", "/tmp/nonexistent"]);
    assert_exec_approval_requirement_for_command(
        ExecApprovalRequirementScenario {
...
async fn assert_exec_approval_requirement_for_command(
    ...
    let permission_profile = permission_profile_from_access_policy(&access_policy);
...
async fn exec_policies_only_load_from_trusted_project_layers() -> std::io::Result<()> {
    ...
    let agere_home = temp.path().join("home_execpolicy_nested");
    let project_root = temp.path().join("project_execpolicy_nested");
    let nested = project_root.join("nested");
    let root_rules = project_root.join(".openagere").join(RULES_DIR_NAME);
    let nested_rules = nested.join(".openagere").join(RULES_DIR_NAME);

    fs::create_dir_all(&agere_home)?;
...
async fn exec_policies_require_project_trust_without_config_toml() -> std::io::Result<()> {
    ...
    let nested = project_root.join("nested");
    let rules_dir = project_root.join(".openagere").join(RULES_DIR_NAME);
    ...
    for (name, trust_entries, expected_decision) in cases {
        let agere_home = temp.path().join(format!("home_execpolicy_{name}"));
        fs::create_dir_all(&agere_home)?;
...
-> std::io::Result<()> {
    ...
    for (name, trust_entries, expect_warning) in cases {
        let agere_home = temp.path().join(format!("home_execpolicy_warning_{name}"));
        fs::create_dir_all(&agere_home)?;
        write_project_trust_config(&agere_home, &trust_entries).await?;
