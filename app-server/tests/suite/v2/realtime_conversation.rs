...
use agere_app_server_protocol::ThreadRealtimeAudioChunk;
...
#[derive(Debug, Clone, Copy)]
enum RealtimeTestAccessMode {
    ReadOnly,
    DangerFullAccess,
}

impl RealtimeTestAccessMode {
    fn config_value(self) -> &'static str {
        match self {
            RealtimeTestAccessMode::ReadOnly => "read-only",
            RealtimeTestAccessMode::DangerFullAccess => "danger-full-access",
        }
    }
}
...
impl RealtimeE2eHarness {
    ...
    async fn new(
        ...
        Self::new_with_main_loop_responses_server_and_access_mode(
            realtime_version,
            main_loop_responses_server,
            realtime_sideband,
            RealtimeTestAccessMode::ReadOnly,
        )
        .await
    }

    async fn new_with_access_mode(
        realtime_version: RealtimeTestVersion,
        main_loop: MainLoopResponsesScript,
        realtime_sideband: RealtimeSidebandScript,
        access_mode: RealtimeTestAccessMode,
    ) -> Result<Self> {
        let main_loop_responses_server =
            create_mock_responses_server_sequence_unchecked(main_loop.responses).await;
        Self::new_with_main_loop_responses_server_and_access_mode(
    ...
    async fn new_with_main_loop_responses_server_and_access_mode(
        realtime_version: RealtimeTestVersion,
        main_loop_responses_server: MockServer,
        realtime_sideband: RealtimeSidebandScript,
        access_mode: RealtimeTestAccessMode,
...
async fn webrtc_v2_background_agent_steering_ack_requests_response_create() -> Result<()> {
    ...
    // Phase 4: release the gated delegated turn. Agere should then continue
...
async fn webrtc_v2_tool_call_delegated_turn_can_execute_shell_tool() -> Result<()> {
    ...
    let mut harness = RealtimeE2eHarness::new_with_access_mode(
        RealtimeTestVersion::V2,
        main_loop,
        realtime,
        RealtimeTestAccessMode::DangerFullAccess,
    )
    .await?;
...
fn create_config_toml_with_realtime_version(
    ...
    let access_mode = access_mode.config_value();
