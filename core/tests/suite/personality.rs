...
async fn personality_does_not_mutate_base_instructions_without_template() {
    ...
    let model_info = agere_core::test_support::construct_model_info_offline("gpt-5.4", &config);
    assert_eq!(
...
async fn user_turn_personality_some_adds_update_message() -> anyhow::Result<()> {
    ...
    let mut builder = test_agere()
        .with_model("exp-agere-personality")
        .with_config(|config| {
            config
                .features
                .enable(Feature::Personality)
...
async fn remote_model_friendly_personality_instructions_with_feature() -> anyhow::Result<()> {
    ...
    let remote_slug = "agere-remote-default-personality";
    ...
    let mut builder = test_agere()
        .with_auth(agere_login::AgereAuth::create_dummy_chatgpt_auth_for_testing())
        .with_config(|config| {
            config
                .features
                .enable(Feature::Personality)
...
async fn user_turn_personality_remote_model_template_includes_update_message() -> anyhow::Result<()>
{
    ...
    let remote_slug = "agere-remote-personality";
