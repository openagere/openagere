...
use agere_state::BackfillStatus;
...
fn test_config(agere_home: PathBuf) -> RolloutConfig {
    RolloutConfig {
        sqlite_home: agere_home.clone(),
        cwd: agere_home.clone(),
        agere_home,
...
async fn backfill_sessions_resumes_from_watermark_and_marks_complete() {
    let dir = tempdir().expect("tempdir");
    let agere_home = dir.path().to_path_buf();
    ...
    let runtime = agere_state::StateRuntime::init(agere_home.clone(), "test-provider".to_string())
        .await
        .expect("initialize runtime");
    let first_watermark = backfill_watermark_for_path(agere_home.as_path(), first_path.as_path());
    ...
    let config = test_config(agere_home.clone());
...
async fn backfill_sessions_normalizes_cwd_before_upsert() {
    ...
    let agere_home = dir.path().to_path_buf();
    let thread_uuid = Uuid::new_v4();
    let session_cwd = agere_home.join(".");
...
fn write_rollout_in_sessions_with_cwd(
    ...
    let sessions_dir = agere_home.join("sessions");
