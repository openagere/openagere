...
async fn plugin_manifest_name(
    ...
    let contents = fs.read_file_text(&manifest_path).await.ok()?;
...
/// plugin manifest (same `name` rules as full manifest loading in agere-core).
...
mod tests {
    ...
    async fn uses_manifest_name() {
        ...
        fs::create_dir_all(plugin_root.join(".agere-plugin")).expect("mkdir manifest");
        fs::write(
