...
fn exec_repo_root() -> anyhow::Result<std::path::PathBuf> {
    Ok(agere_utils_cargo_bin::repo_root()?)
}
...
fn exec_resume_preserves_cli_configuration_overrides() -> anyhow::Result<()> {
    ...
    if cfg!(target_os = "windows") {
        assert!(
            stderr.contains("permission_profile: read-only"),
            "stderr missing downgraded permission profile note: {stderr}"
        );
    } else {
        assert!(
            stderr.contains("permission_profile: workspace-write"),
            "stderr missing permission profile override: {stderr}"
        );
    }
