...
#[test]
fn turn_metadata_state_uses_access_mode_tag() {
    let temp_dir = TempDir::new().expect("temp dir");
    ...
    let access_mode_name = json.get("access_mode").and_then(Value::as_str);
    let session_id = json.get("session_id").and_then(Value::as_str);
    let thread_source = json.get("thread_source").and_then(Value::as_str);

    let expected_access_tag = super::permission_profile_access_mode_tag(
        &permission_profile,
        WindowsExecutionRestrictionLevel::Disabled,
        false,
    );
    assert_eq!(access_mode_name, Some(expected_access_tag.as_str()));
