...
//!
//! The same permission- and feature-gating rules are used by both the composer
//! and the command popup. Centralizing them here keeps those call sites small
//! and ensures they stay in sync.
use std::str::FromStr;

use agere_utils_common::fuzzy_match::fuzzy_match;
...
pub(crate) struct BuiltinCommandFlags {
    ...
    pub(crate) allow_elevate_restriction: bool,
...
pub(crate) fn builtins_for_input(flags: BuiltinCommandFlags) -> Vec<(&'static str, SlashCommand)> {
    built_in_slash_commands()
        .into_iter()
        .filter(|(_, cmd)| {
            flags.allow_elevate_restriction || *cmd != SlashCommand::SetupDefaultRestriction
        })
        .filter(|(_, cmd)| {
...
mod tests {
    ...
    fn all_enabled_flags() -> BuiltinCommandFlags {
        BuiltinCommandFlags {
            ...
            allow_elevate_restriction: true,
            side_conversation_active: false,
        }
    }
