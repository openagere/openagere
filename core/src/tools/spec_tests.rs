...
use agere_models_manager::model_info::with_config_overrides;
...
use agere_tools::AdditionalProperties;
use agere_tools::ConfiguredToolSpec;
use agere_tools::DiscoverableTool;
use agere_tools::JsonSchema;
use agere_tools::JsonSchemaPrimitiveType;
use agere_tools::JsonSchemaType;
use agere_tools::LoadableToolSpec;
use agere_tools::ResponsesApiNamespaceTool;
use agere_tools::ResponsesApiTool;
use agere_tools::ShellCommandBackendConfig;
use agere_tools::TOOL_SEARCH_TOOL_NAME;
use agere_tools::TOOL_SUGGEST_TOOL_NAME;
...
use agere_tools::mcp_call_tool_result_output_schema;
use agere_tools::mcp_tool_to_deferred_responses_api_tool;
...
fn deferred_responses_api_tool_serializes_with_defer_loading() {
    ...
    let serialized = serde_json::to_value(ToolSpec::Function(
        mcp_tool_to_deferred_responses_api_tool(
            &ToolName::namespaced("mcp__agere_apps__", "lookup_order"),
            &tool,
        )
...
fn function_tool<'a>(tools: &'a [ConfiguredToolSpec], expected_name: &str) -> &'a ResponsesApiTool {
...
fn command_schema_type(tool: &ResponsesApiTool) -> JsonSchemaPrimitiveType {
    let command = tool
        .parameters
        .properties
        .as_ref()
        .and_then(|properties| properties.get("command"))
        .unwrap_or_else(|| panic!("{} should have command property", tool.name));
    match command.schema_type.as_ref() {
        Some(JsonSchemaType::Single(schema_type)) => *schema_type,
        other => panic!("unexpected command schema type: {other:?}"),
    }
}
...
#[tokio::test]
async fn model_provided_unified_exec_is_downgraded_for_windows_restricted_access() {
    ...
    let config = ToolsConfig::new(&ToolsConfigParams {
        ...
        windows_execution_restriction_level: WindowsExecutionRestrictionLevel::RestrictedToken,
    });
...
#[tokio::test]
async fn model_shell_type_controls_shell_tool_command_schema() {
    let config = test_config().await;
    let mut model_info = construct_model_info_offline("gpt-5.4", &config);
    let features = Features::with_defaults();
    let available_models = Vec::new();

    model_info.shell_type = ConfigShellToolType::Default;
    ...
    .build();
    let shell = function_tool(&tools, "shell");
    assert_eq!(command_schema_type(shell), JsonSchemaPrimitiveType::Array);
    assert!(tools.iter().all(|tool| tool.name() != "shell_command"));
    ...
    .build();
    let shell_command = function_tool(&tools, "shell_command");
    assert_eq!(
        command_schema_type(shell_command),
        JsonSchemaPrimitiveType::String
    );
    assert!(tools.iter().all(|tool| tool.name() != "shell"));
}
...
#[tokio::test]
async fn test_build_specs_gpt54_default() {
    let features = Features::with_defaults();
...
#[tokio::test]
async fn test_build_specs_gpt54_cached_default() {
    let features = Features::with_defaults();
    assert_default_model_tools(
        "gpt-5.4",
        &features,
...
#[tokio::test]
async fn test_build_specs_gpt54_unified_exec_web_search() {
    let mut features = Features::with_defaults();
...
#[tokio::test]
async fn test_build_specs_gpt54_cached_unified_exec_web_search() {
    let mut features = Features::with_defaults();
    features.enable(Feature::UnifiedExec);
    assert_model_tools(
        "gpt-5.4",
...
#[tokio::test]
async fn test_gpt_5_1_agere_max_defaults() {
    let features = Features::with_defaults();
    assert_default_model_tools(
        "gpt-5.4",
        &features,
...
#[tokio::test]
async fn test_agere_5_1_mini_defaults() {
    let features = Features::with_defaults();
    assert_default_model_tools(
        "gpt-5.4-mini",
        &features,
...
#[tokio::test]
async fn test_gpt_5_1_agere_max_unified_exec_web_search() {
    let mut features = Features::with_defaults();
    features.enable(Feature::UnifiedExec);
    assert_model_tools(
        "gpt-5.4",
    ...
    assert_eq!(
        tools_config
            .with_unified_exec_shell_mode_for_session(
                ...
                Some(&PathBuf::from(if cfg!(windows) {
                    r"C:\opt\agere\agere-execve-wrapper"
                } else {
                    "/opt/agere/agere-execve-wrapper"
                })),
            )
            .unified_exec_shell_mode,
        if cfg!(unix) {
            UnifiedExecShellMode::ZshFork(ZshForkConfig {
                shell_zsh_path: AbsolutePathBuf::from_absolute_path("/opt/agere/zsh").unwrap(),
...
async fn search_tool_description_falls_back_to_connector_name_without_description() {
    ...
    let (tools, _) = build_specs(
        ...
        /*mcp_tools*/ None,
        Some(HashMap::from([(
            "mcp__agere_apps__calendar_create_event".to_string(),
            ToolInfo {
                server_name: AGERE_APPS_MCP_SERVER_NAME.to_string(),
                callable_name: "_create_event".to_string(),
                callable_namespace: "mcp__agere_apps__calendar".to_string(),
                server_instructions: None,
                tool: mcp_tool(
...
async fn search_tool_registers_namespaced_mcp_tool_aliases() {
    ...
    let (_, registry) = build_specs(
        ...
        Some(HashMap::from([
            ...
            (
                "mcp__agere_apps__calendar_list_events".to_string(),
                ToolInfo {
                    server_name: AGERE_APPS_MCP_SERVER_NAME.to_string(),
    ...
    let app_alias = ToolName::namespaced("mcp__agere_apps__calendar", "_create_event");
...
async fn unavailable_mcp_tools_are_exposed_as_dummy_function_tools() {
    ...
    let unavailable_tool = ToolName::namespaced("mcp__agere_apps__calendar", "_create_event");
    ...
    let tool = find_tool(&tools, "mcp__agere_apps__calendar_create_event");
    ...
    assert!(registry.has_handler(&ToolName::namespaced(
        "mcp__agere_apps__calendar",
        "_create_event"
    )));
    assert!(!registry.has_handler(&ToolName::plain("mcp__agere_apps__calendar_create_event")));
}
