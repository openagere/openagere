...
pub const AGERE_EXEC_SERVER_URL_ENV_VAR: &str = "AGERE_EXEC_SERVER_URL";

/// Owns the execution/filesystem environments available to the Agere runtime.
///
...
/// `AGERE_EXEC_SERVER_URL` is set to a websocket URL, it also creates a remote
...
///
/// Setting `AGERE_EXEC_SERVER_URL=none` disables environment access by leaving
...
impl EnvironmentManagerArgs {
    ...
    pub fn from_env(local_runtime_paths: ExecServerRuntimePaths) -> Self {
        Self {
            exec_server_url: std::env::var(AGERE_EXEC_SERVER_URL_ENV_VAR).ok(),
            local_runtime_paths,
        }
    }
}
...
impl EnvironmentManager {
    /// Builds a test-only manager without configured filesystem helper paths.
    ...
    /// Builds a manager from the raw `AGERE_EXEC_SERVER_URL` value and local
...
impl Environment {
    /// Builds a test-only local environment without configured filesystem helper paths.
    pub fn default_for_tests() -> Self {
        Self {
            exec_server_url: None,
...
impl Environment {
    /// Builds an environment from the raw `AGERE_EXEC_SERVER_URL` value.
    pub fn create(
        exec_server_url: Option<String>,
    ...
    /// Builds a test-only environment without configured filesystem helper paths.
    ...
    /// Builds an environment from the raw `AGERE_EXEC_SERVER_URL` value and
