...
async fn resume_startup_does_not_consume_model_availability_nux_count() -> Result<()> {
    // run_agere_cli() does not work on Windows due to PTY limitations.
    if cfg!(windows) {
        return Ok(());
    }
    ...
    let custom_catalog_path = agere_home.path().join("catalog.json");
    std::fs::write(
        &custom_catalog_path,
...
"#
    ...
    std::fs::write(agere_home.path().join("config.toml"), config_contents)?;

    let fixture_path =
        agere_utils_cargo_bin::find_resource!("../core/tests/cli_responses_fixture.sse")?;
    let agere = if let Ok(path) = agere_utils_cargo_bin::cargo_bin("agere") {
        path
    } else {
        ...
        if fallback.is_file() {
            ...
            eprintln!("skipping integration test because agere binary is unavailable");
            return Ok(());
        }
    };

    let exec_output = std::process::Command::new(&agere)
        .arg("exec")
        .arg("--skip-git-repo-check")
        .arg("-C")
        .arg(&repo_root)
        .arg("seed session for resume")
        .env("AGERE_HOME", agere_home.path())
        .env("OPENAI_API_KEY", "dummy")
        .env("AGERE_RS_SSE_FIXTURE", fixture_path)
        .output()
        .context("failed to execute agere exec")?;
    anyhow::ensure!(
    ...
    env.insert(
        "AGERE_HOME".to_string(),
        agere_home.path().display().to_string(),
    );
    ...
    let spawned = agere_utils_pty::spawn_pty_process(
        agere.to_string_lossy().as_ref(),
        &args,
        &repo_root,
        &env,
        &None,
        agere_utils_pty::TerminalSize::default(),
    )
    .await?;

    let mut output = Vec::new();
    let agere_utils_pty::SpawnedProcess {
        session,
        stdout_rx,
        stderr_rx,
        exit_rx,
    } = spawned;
    let mut output_rx = agere_utils_pty::combine_output_receivers(stdout_rx, stderr_rx);
    ...
    let exit_code = match exit_code_result {
        ...
        Err(_) => {
            session.terminate();
            anyhow::bail!("timed out waiting for openagere resume to exit");
        }
    };
    ...
    anyhow::ensure!(
        ...
        "unexpected exit code from openagere resume: {exit_code}; output: {output_text}",
    );

    let config_contents = std::fs::read_to_string(agere_home.path().join("config.toml"))?;
