...
fn test_tracing_subscriber() -> impl tracing::Subscriber + Send + Sync {
    ...
    let tracer = provider.tracer("agere-exec-tests");
...
fn exec_root_span_can_be_parented_from_trace_context() {
    ...
    let parent = agere_protocol::protocol::W3cTraceContext {
...
async fn resume_lookup_model_providers_filters_only_last_lookup() {
    let agere_home = tempdir().expect("create temp agere home");
...
fn turn_items_for_thread_returns_matching_turn_items() {
    let thread = AppServerThread {
        ...
        source: agere_app_server_protocol::SessionSource::Exec,
        agent_nickname: None,
        agent_role: None,
        git_info: None,
        name: None,
        turns: vec![
            agere_app_server_protocol::Turn {
                id: "turn-1".to_string(),
...
async fn thread_start_params_include_review_policy_when_review_policy_is_manual_only() {
    ...
    assert_eq!(
        ...
        Some(agere_app_server_protocol::ApprovalsReviewer::User)
    );
...
async fn thread_start_params_include_review_policy_when_auto_review_is_enabled() {
    ...
    assert_eq!(
        params.approvals_reviewer,
        Some(agere_app_server_protocol::ApprovalsReviewer::AutoReview)
    );
}
...
fn session_configured_from_thread_response_uses_review_policy_from_response() {
    let response = ThreadStartResponse {
        ...
        approval_policy: agere_app_server_protocol::AskForApproval::OnRequest,
        approvals_reviewer: agere_app_server_protocol::ApprovalsReviewer::AutoReview,
        access_policy: agere_app_server_protocol::AccessPolicy::WorkspaceWrite {
            writable_roots: vec![],
        ...
        permission_profile: Some(
            agere_protocol::models::PermissionProfile::from_legacy_access_policy("workspace-write")
                .into(),
        ),
