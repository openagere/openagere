...
#[command(
    version,
    override_usage = "agere exec [OPTIONS] [PROMPT]\n       agere exec [OPTIONS] <COMMAND> [ARGS]"
)]
pub struct Cli {
    ...
    /// Allow running Agere outside a Git repository.
    ...
    /// Do not load `$AGERE_HOME/config.toml`; auth still uses `AGERE_HOME`.
...
struct ResumeArgsRaw {
    ...
    // so "openagere resume --last <prompt>" treats the positional as a prompt, not a session id.
