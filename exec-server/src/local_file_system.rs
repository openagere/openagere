...
pub(crate) fn resolve_existing_path(path: &std::path::Path) -> io::Result<std::path::PathBuf> {
...
fn copy_symlink(source: &std::path::Path, target: &std::path::Path) -> io::Result<()> {
...
#[cfg(windows)]
fn symlink_points_to_directory(source: &std::path::Path) -> io::Result<bool> {
