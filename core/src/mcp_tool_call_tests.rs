...
use agere_config::types::AppConfig;
use agere_config::types::AppToolConfig;
use agere_config::types::AppToolsConfig;
use agere_config::types::ApprovalsReviewer;
use agere_config::types::AppsConfigToml;
...
fn install_mcp_permission_request_hook(
    ...
    std::fs::create_dir_all(&turn_context.config.agere_home)
        .expect("create agere home for MCP permission hook");
    let script = format!(
        r#"import json
...
"#,
    ...
    std::fs::write(
        turn_context.config.agere_home.join("hooks.json"),
        serde_json::json!({
            "hooks": {
                "PermissionRequest": [{
                    "matcher": matcher,
...
#[test]
fn openai_file_params_are_only_honored_for_agere_apps() {
    ...
    assert_eq!(
        openai_file_input_params_for_server(AGERE_APPS_MCP_SERVER_NAME, meta),
...
async fn mcp_result_telemetry_records_allowlisted_span_fields() {
    ...
    assert!(
        logs.contains("agere.mcp.target.id=\"com.apple.reminders\"")
            && logs.contains("agere.mcp.server_user_flow.triggered=false"),
...
async fn mcp_result_telemetry_ignores_invalid_and_missing_values() {
    ...
    .await;
    assert!(
        !invalid_logs.contains("agere.mcp.target.id=")
            && !invalid_logs.contains("agere.mcp.server_user_flow.triggered="),
    ...
    .await;
    assert!(
        !missing_logs.contains("agere.mcp.target.id=")
            && !missing_logs.contains("agere.mcp.server_user_flow.triggered="),
    ...
    assert!(
        !no_meta_logs.contains("agere.mcp.target.id=")
            && !no_meta_logs.contains("agere.mcp.server_user_flow.triggered="),
...
async fn mcp_result_telemetry_truncates_long_target_id() {
    ...
    assert!(
        logs.contains(&format!("agere.mcp.target.id=\"{truncated}\"")) && !logs.contains("tail"),
...
async fn approval_elicitation_request_uses_message_override_and_preserves_tool_params_keys() {
    ...
    assert_eq!(
        ...
        McpServerElicitationRequestParams {
            ...
            server_name: AGERE_APPS_MCP_SERVER_NAME.to_string(),
...
#[test]
fn agere_apps_tool_question_uses_fallback_app_label() {
    let question = build_mcp_tool_approval_question(
        "q".to_string(),
        AGERE_APPS_MCP_SERVER_NAME,
        "run_action",
...
#[test]
fn trusted_agere_apps_tool_question_offers_always_allow() {
    let question = build_mcp_tool_approval_question(
        "q".to_string(),
        AGERE_APPS_MCP_SERVER_NAME,
        "run_action",
...
#[test]
fn agere_apps_tool_question_without_elicitation_omits_always_allow() {
    let session_key = McpToolApprovalKey {
        server: AGERE_APPS_MCP_SERVER_NAME.to_string(),
...
#[test]
fn agere_apps_connectors_support_persistent_approval() {
    let invocation = McpInvocation {
        server: AGERE_APPS_MCP_SERVER_NAME.to_string(),
...
async fn mcp_tool_call_request_meta_includes_turn_metadata_for_custom_server() {
    ...
    assert_eq!(
        meta,
        serde_json::json!({
            crate::X_AGERE_TURN_METADATA_HEADER: expected_turn_metadata,
        })
    );
}
...
async fn mcp_tool_call_request_meta_includes_turn_started_at_unix_ms() {
    ...
    let turn_metadata = meta
        .get(crate::X_AGERE_TURN_METADATA_HEADER)
...
#[tokio::test]
async fn agere_apps_tool_call_request_meta_includes_turn_metadata_and_agere_apps_meta() {
    let (_, turn_context) = make_session_and_context().await;
    let expected_turn_metadata = serde_json::from_str::<serde_json::Value>(
        &turn_context
            .turn_metadata_state
    ...
    let metadata = McpToolApprovalMetadata {
        ...
        agere_apps_meta: Some(
            ...
            .as_object()
            .cloned()
            .expect("_agere_apps metadata should be an object"),
        ),
        openai_file_input_params: None,
    };
...
#[tokio::test]
async fn agere_apps_tool_call_request_meta_includes_call_id_without_existing_agere_apps_meta() {
    let (_, turn_context) = make_session_and_context().await;
    let expected_turn_metadata = serde_json::from_str::<serde_json::Value>(
        &turn_context
            .turn_metadata_state
...
async fn guardian_review_decision_maps_to_mcp_tool_decision() {
    ...
    session.services.guardian_rejections.lock().await.insert(
        ...
        crate::guardian::GuardianRejection {
            ...
            source: agere_protocol::protocol::GuardianAssessmentDecisionSource::Agent,
        },
    );
...
#[test]
fn approval_elicitation_meta_includes_connector_source_for_agere_apps() {
    assert_eq!(
        build_mcp_tool_approval_elicitation_meta(
            AGERE_APPS_MCP_SERVER_NAME,
            Some(&approval_metadata(
...
#[tokio::test]
async fn persist_agere_app_tool_approval_writes_tool_override() {
    let tmp = tempdir().expect("tempdir");

    persist_agere_app_tool_approval(&tmp.path().abs(), "calendar", "calendar/list_events")
        .await
...
async fn maybe_persist_mcp_tool_approval_writes_project_config_for_project_server() {
    ...
    let project_agere_dir = project_dir.path().join(".openagere");
    std::fs::create_dir_all(&project_agere_dir).expect("create project .agere dir");
    std::fs::write(
        project_agere_dir.join(CONFIG_TOML_FILE),
        "[mcp_servers.docs]\ncommand = \"docs-server\"\n",
    )
    ...
    ConfigEditsBuilder::new(&agere_home)
        .set_project_trust_level(
            project_dir.path(),
            agere_protocol::config_types::TrustLevel::Trusted,
        )
        .apply()
        .await
    ...
    let contents = std::fs::read_to_string(project_agere_dir.join(CONFIG_TOML_FILE))
...
async fn approve_mode_blocks_when_arc_returns_interrupt_for_model() {
    ...
    turn_context.auth_manager = Some(crate::test_support::auth_manager_from_auth(
        agere_login::AgereAuth::create_dummy_chatgpt_auth_for_testing(),
    ));
    let mut config = (*turn_context.config).clone();
