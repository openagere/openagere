...
use agere_apply_patch::APPLY_PATCH_TOOL_INSTRUCTIONS;
...
-> anyhow::Result<()> {
    ...
    let TestAgere { agere, config, .. } = test_agere()
        .with_config(|config| {
            config.user_instructions = Some("be consistent and helpful".to_string());
            config
                .features
...
async fn overrides_turn_context_but_keeps_cached_prefix_and_key_constant() -> anyhow::Result<()> {
    ...
    agere
        .submit(Op::OverrideTurnContext {
            ...
            summary: Some(Some(ReasoningSummary::Detailed)),
            service_tier: None,
...
async fn per_turn_overrides_keep_cached_prefix_and_key_constant() -> anyhow::Result<()> {
    ...
    let permission_profile = turn_permission_fields(permission_profile, new_cwd.path());
    agere
        .submit(Op::UserTurn {
            environments: None,
            items: vec![UserInput::Text {
...
async fn send_user_turn_with_no_changes_does_not_send_environment_context() -> anyhow::Result<()> {
    ...
    let default_access_policy = config.legacy_access_policy();
    ...
    agere
        .submit(Op::UserTurn {
            ...
            permission_profile: Some(PermissionProfile::from_legacy_access_policy(
                default_access_policy.as_str(),
            )),
            model: default_model.clone(),
            effort: Some(default_effort),
            summary: Some(Some(default_summary.unwrap_or(ReasoningSummary::Auto))),
            service_tier: None,
            collaboration_mode: None,
            final_output_json_schema: None,
            personality: None,
