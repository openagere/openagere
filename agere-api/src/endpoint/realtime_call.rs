...
use agere_client::RequestTelemetry;
use bytes::Bytes;
use http::HeaderMap;
...
const MULTIPART_BOUNDARY: &str = "agere-realtime-call-boundary";
const MULTIPART_CONTENT_TYPE: &str = "multipart/form-data; boundary=agere-realtime-call-boundary";
...
impl<T: HttpTransport> RealtimeCallClient<T> {
    ...
    pub async fn create_with_session_and_headers(
        ...
        trace!(target: "agere_api::realtime_websocket::wire", "realtime call request SDP: {sdp}");
...
mod tests {
    ...
    use agere_client::Response;
    use agere_client::StreamResponse;
    ...
    async fn extracts_call_id_from_forwarded_backend_location() {
        ...
        let client = RealtimeCallClient::new(
            transport.clone(),
            provider("https://chatgpt.com/backend-api/agere"),
            Arc::new(DummyAuth),
        );
        ...
        assert_eq!(
            request.url,
            "https://chatgpt.com/backend-api/agere/realtime/calls"
        );
        assert_eq!(
            request.body,
            Some(RequestBody::Raw(Bytes::from_static(b"v=offer\r\n")))
