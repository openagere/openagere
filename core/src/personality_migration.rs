...
use agere_thread_store::ListThreadsParams;
use agere_thread_store::LocalThreadStore;
use agere_thread_store::ThreadSortKey;
...
pub async fn maybe_migrate_personality(
    ...
    let marker_path = agere_home.join(PERSONALITY_MIGRATION_FILENAME);
    ...
    if !has_recorded_sessions(agere_home, model_provider_id.as_str()).await? {
        create_marker(&marker_path).await?;
...
async fn has_recorded_sessions(agere_home: &Path, default_provider: &str) -> io::Result<bool> {
    let store = LocalThreadStore::new(agere_rollout::RolloutConfig {
        agere_home: agere_home.to_path_buf(),
        sqlite_home: agere_home.to_path_buf(),
...
async fn has_threads(store: &LocalThreadStore, archived: bool) -> io::Result<bool> {
    store
        .list_threads(ListThreadsParams {
            ...
            sort_direction: agere_thread_store::SortDirection::Desc,
