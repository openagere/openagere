...
pub fn get_upgrade_version(config: &Config) -> Option<String> {
    if !config.check_for_update_on_startup || is_source_build_version(AGERE_CLI_VERSION) {
        return None;
    }
    ...
    info.and_then(|info| {
        if is_newer(&info.latest_version, AGERE_CLI_VERSION).unwrap_or(false) {
...
const HOMEBREW_CASK_API_URL: &str = "https://formulae.brew.sh/api/cask/agere.json";
const LATEST_RELEASE_URL: &str = "https://api.github.com/repos/openagere/openagere/releases/latest";
...
fn version_filepath(config: &Config) -> PathBuf {
    config.agere_home.join(VERSION_FILENAME).into_path_buf()
}
    ...
    let latest_version = match action {
        ...
        Some(UpdateAction::NpmGlobalLatest) | Some(UpdateAction::BunGlobalLatest) => {
            let package_info = create_client()
                ...
                .json::<NpmPackageInfo>()
                .await?;
            match package_info.dist_tag("latest") {
        ...
        Some(UpdateAction::StandaloneUnix) | Some(UpdateAction::StandaloneWindows) | None => {
            async {
                match fetch_latest_github_release_version().await {
                    Ok(v) => Ok(v),
                    Err(e) => {
                        tracing::warn!(
                            "GitHub Releases API failed: {e}, falling back to npm registry"
                        );
                        fetch_latest_npm_version().await
                    }
                }
            }
            .await?
...
/// Fallback: fetch the latest version from npm registry dist-tags.
/// Used when GitHub Releases API is rate-limited or unreachable.
async fn fetch_latest_npm_version() -> anyhow::Result<String> {
    let package_info = create_client()
        .get(npm_registry::PACKAGE_URL)
        .send()
        .await?
