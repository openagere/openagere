...
use crate::access_policy_transforms::intersect_permission_profiles;
...
use agere_analytics::SubAgentThreadStartedInput;
use agere_app_server_protocol::McpServerElicitationRequest;
...
use agere_features::unstable_features_warning_event;
use agere_hooks::Hooks;
use agere_hooks::HooksConfig;
use agere_login::AgereAuth;
use agere_login::AuthManager;
use agere_login::auth_env_telemetry::collect_auth_env_telemetry;
...
use agere_mcp::agere_apps_tools_cache_key;
...
use agere_network_proxy::NetworkProxyAuditMetadata;
use agere_network_proxy::normalize_host;
use agere_otel::current_span_trace_id;
use agere_otel::current_span_w3c_trace_context;
use agere_otel::set_parent_from_w3c_trace_context;
use agere_protocol::ThreadId;
use agere_protocol::ToolName;
...
use agere_protocol::approvals::NetworkPolicyAmendment;
use agere_protocol::approvals::NetworkPolicyRuleAction;
...
use agere_protocol::protocol::TurnContextNetworkItem;
...
use agere_protocol::request_permissions::RequestPermissionsArgs;
use agere_protocol::request_permissions::RequestPermissionsEvent;
use agere_protocol::request_permissions::RequestPermissionsResponse;
use agere_protocol::request_user_input::RequestUserInputArgs;
use agere_protocol::request_user_input::RequestUserInputResponse;
use agere_rmcp_client::ElicitationResponse;
use agere_rollout::state_db;
use agere_rollout_trace::AgentResultTracePayload;
use agere_rollout_trace::ThreadStartedTraceMetadata;
use agere_rollout_trace::ThreadTraceContext;
use agere_shell_command::parse_command::parse_command;
use agere_thread_store::CreateThreadParams;
use agere_thread_store::LiveThread;
use agere_thread_store::LiveThreadInitGuard;
use agere_thread_store::LocalThreadStore;
use agere_thread_store::ResumeThreadParams;
use agere_thread_store::ThreadEventPersistenceMode;
use agere_thread_store::ThreadStore;
use agere_utils_output_truncation::TruncationPolicy;
...
use crate::agere_thread::ThreadConfigSnapshot;
...
use agere_mcp::compute_auth_statuses;
use agere_mcp::with_agere_apps_mcp;
use agere_otel::SessionTelemetry;
use agere_otel::THREAD_STARTED_METRIC;
...
use agere_protocol::protocol::AgereErrorInfo;
...
use agere_protocol::protocol::ModelRerouteEvent;
use agere_protocol::protocol::ModelRerouteReason;
use agere_protocol::protocol::ModelVerification;
use agere_protocol::protocol::ModelVerificationEvent;
...
use agere_protocol::protocol::SkillDependencies as ProtocolSkillDependencies;
use agere_protocol::protocol::SkillErrorInfo;
use agere_protocol::protocol::SkillInterface as ProtocolSkillInterface;
use agere_protocol::protocol::SkillMetadata as ProtocolSkillMetadata;
use agere_protocol::protocol::SkillToolDependency as ProtocolSkillToolDependency;
...
use agere_tools::ToolsConfigParams;
#[cfg(test)]
use agere_utils_common::ProposedPlanSegment;
use agere_utils_fs::AbsolutePathBuf;
use agere_utils_readiness::ReadinessFlag;

/// The high-level interface to the Agere system.
...
/// Wrapper returned by [`Agere::spawn`] containing the spawned [`Agere`] and
/// the unique session id.
pub struct AgereSpawnOk {
    pub agere: Agere,
...
fn model_reroute_warning_message(
    uses_openai_provider: bool,
    requested_model: &str,
    server_model: &str,
) -> String {
    if uses_openai_provider {
        return format!(
            "OpenAI routed this request from {requested_model} to {server_model} after a safety check. If you need authorization for security work, join the Trusted Access for Cyber program: {CYBER_VERIFY_URL}"
        );
    }

    format!(
        "A provider-side safety check routed this request from {requested_model} to {server_model}. If this seems wrong, check this model provider's safety, access, or model routing policy."
    )
}

fn model_reroute_reason(uses_openai_provider: bool) -> ModelRerouteReason {
    if uses_openai_provider {
        ModelRerouteReason::HighRiskCyberActivity
    } else {
        ModelRerouteReason::ProviderSafetyRouting
    }
}

impl Agere {
    /// Spawn a new [`Agere`] and initialize the session.
    pub(crate) async fn spawn(args: AgereSpawnArgs) -> AgereResult<AgereSpawnOk> {
        let parent_trace = match args.parent_trace {
            Some(trace) => {
                if agere_otel::context_from_w3c_trace_context(&trace).is_some() {
                    Some(trace)
                } else {
        ...
        Self::spawn_internal(AgereSpawnArgs {
            parent_trace,
            ..args
        })
    ...
    async fn spawn_internal(args: AgereSpawnArgs) -> AgereResult<AgereSpawnOk> {
        let AgereSpawnArgs {
            mut config,
            auth_manager,
            models_manager,
        ...
        let exec_policy = if crate::guardian::is_guardian_reviewer_source(&session_source) {
            ...
            Arc::new(
                ExecPolicyManager::load(&config.config_layer_stack)
                    .await
                    .map_err(|err| AgereErr::Fatal(format!("failed to load rules: {err}")))?,
            )
        };

        let config = Arc::new(config);
        let refresh_strategy = if session_source.is_non_root_agent() {
            agere_models_manager::manager::RefreshStrategy::Offline
        } else {
            agere_models_manager::manager::RefreshStrategy::OnlineIfUncached
        };
        ...
        let persisted_tools = if dynamic_tools.is_empty() {
            ...
            match thread_id {
                Some(thread_id) => {
                    ...
                    state_db::get_dynamic_tools(state_db_ctx.as_deref(), thread_id, "agere_spawn")
                        .await
                }
                None => None,
            }
        ...
        // Generate a unique ID for the lifetime of this Agere session.
        ...
        .map_err(|e| {
            ...
            map_session_init_error(&e, &config.agere_home)
        })?;
        ...
        Ok(AgereSpawnOk { agere, thread_id })
    }
    ...
    /// Use sparingly: prefer `submit()` so Agere is responsible for generating
    /// unique IDs for each submission.
    pub async fn submit_with_id(&self, mut sub: Submission) -> AgereResult<()> {
        ...
        self.tx_sub
            .send(sub)
            .await
            .map_err(|_| AgereErr::InternalAgentDied)?;
        Ok(())
    }
    ...
    pub async fn set_thread_memory_mode(
        &self,
        mode: agere_protocol::protocol::ThreadMemoryMode,
    ) -> anyhow::Result<()> {
    ...
    pub async fn shutdown_and_wait(&self) -> AgereResult<()> {
        ...
        match self.submit(Op::Shutdown).await {
            Ok(_) => {}
            Err(AgereErr::InternalAgentDied) => {}
...
async fn thread_title_from_state_db(
    ...
    find_thread_name_by_id(agere_home, &conversation_id)
        .await
        .ok()
        .flatten()
}
...
impl Session {
    ...
    /// Builds the `x-agere-beta-features` header value for this session.
    ///
    ...
    async fn start_managed_network_proxy(
        ...
        exec_policy: &agere_execpolicy::Policy,
        permission_profile: &PermissionProfile,
        network_policy_decider: Option<Arc<dyn agere_network_proxy::NetworkPolicyDecider>>,
        blocked_request_observer: Option<Arc<dyn agere_network_proxy::BlockedRequestObserver>>,
    ...
    async fn refresh_managed_network_proxy_for_current_permission_profile(&self) {
            ...
            .recompute_for_permission_profile(&session_configuration.permission_profile())
        {
            ...
            Err(err) => {
                warn!(
                    "failed to rebuild managed network proxy policy for permission profile change: {err}"
                );
                return;
            }
        };
        ...
        if let Err(err) = spec.apply_to_started_proxy(started_proxy).await {
            warn!("failed to refresh managed network proxy for permission profile change: {err}");
        }
    }

    pub(crate) async fn agere_home(&self) -> AbsolutePathBuf {
        let state = self.state.lock().await;
        state.session_configuration.agere_home().clone()
    }
                    ...
                    .filter(|model| *model != curr)
                {
                    ...
                    self.send_event(
                        ...
                        EventMsg::Warning(WarningEvent {
                         ...
                         Consider switching back to `{prev}` as it may affect Agere performance."
                            ),
                        }),
                    )
                    .await;
    ...
    pub(crate) async fn update_settings(
        ...
        let provider_config = updates.provider_config.clone();
        let (previous_cwd, permission_profile_changed, next_cwd, agere_home, session_source) = {
            let mut state = self.state.lock().await;
            ...
            let agere_home = updated.agere_home.clone();
        ...
        if let Some(provider) = provider_update {
            ...
            self.apply_provider_to_runtime(provider, provider_config)
                .await;
        }
    ...
    pub(crate) async fn send_event(&self, turn_context: &TurnContext, msg: EventMsg) {
        ...
        self.services
            ...
            .record_agere_turn_event(&turn_context.sub_id, &legacy_source);
        self.services
            .rollout_thread_trace
    ...
    async fn forward_child_completion_to_parent(
        ...
        child_agent_path: &agere_protocol::AgentPath,
        status: AgentStatus,
    ) {
        let Some(parent_agent_path) = child_agent_path
            .as_str()
            .rsplit_once('/')
            .and_then(|(parent, _)| agere_protocol::AgentPath::try_from(parent).ok())
        else {
            return;
        };
    ...
    pub(crate) async fn persist_execpolicy_amendment(
        ...
        self.services
            .exec_policy
            .append_amendment_and_update(&agere_home, amendment)
            .await?;
    ...
    pub(crate) async fn request_permissions_for_cwd(
        ...
        if crate::guardian::routes_approval_to_guardian(turn_context.as_ref()) {
            ...
            let review_rx = crate::guardian::spawn_approval_request_review(
                ...
                request,
                /*retry_reason*/ None,
                agere_analytics::GuardianApprovalRequestSource::MainTurn,
    ...
    pub(crate) async fn record_model_warning(&self, message: impl Into<String>, ctx: &TurnContext) {
        self.services
            .session_telemetry
            .counter("agere.model_warning", /*inc*/ 1, &[]);
    ...
    async fn maybe_warn_on_server_model_mismatch(
        ...
        let uses_openai_provider = turn_context.config.model_provider_id == "openai"
            || turn_context.provider.info().is_openai();
        let warning_message =
            model_reroute_warning_message(uses_openai_provider, &requested_model, &server_model);
        ...
        self.send_event(
            ...
            EventMsg::ModelReroute(ModelRerouteEvent {
                ...
                reason: model_reroute_reason(uses_openai_provider),
            }),
        )
        .await;
    ...
    pub(crate) async fn build_initial_context(
        ...
        if turn_context.features.enabled(Feature::MemoryTool)
            ...
            && let Some(memory_prompt) =
                build_memory_tool_developer_instructions(&turn_context.config.agere_home).await
        {
        ...
        if turn_context.features.enabled(Feature::AgereGitCommit)
    ...
    pub(crate) async fn record_response_item_and_emit_turn_item(
        ...
        if let Some(item) = parse_turn_item(&response_item) {
            ...
            self.emit_turn_item_completed(turn_context, item, false)
                .await;
        }
    }
    ...
    pub(crate) async fn record_user_prompt_and_emit_turn_item(
        ...
        self.emit_turn_item_completed(turn_context, turn_item, false)
            .await;
        self.ensure_rollout_materialized().await;
    }
    ...
    pub(crate) async fn notify_stream_error(
        ...
        turn_context: &TurnContext,
        message: impl Into<String>,
        agere_error: AgereErr,
    ) {
        let additional_details = agere_error.to_string();
        let agere_error_info = AgereErrorInfo::ResponseStreamDisconnected {
            http_status_code: agere_error.http_status_code_value(),
        };
        let event = EventMsg::StreamError(StreamErrorEvent {
            message: message.into(),
            agere_error_info: Some(agere_error_info),
    ...
    pub(crate) async fn take_pending_session_start_source(
        &self,
    ) -> Option<agere_hooks::SessionStartSource> {
        let mut state = self.state.lock().await;
...
use agere_memories_read::build_memory_tool_developer_instructions;
