...
use agere_config::ConfigRequirementsWithSources;
use agere_config::FilesystemDenyReadPattern;
...
use agere_config::config_error_from_toml;
...
async fn returns_config_error_for_schema_error_in_user_config() {
    ...
    let config_error = config_error_from_io(&err);
    let _guard = agere_utils_fs::AbsolutePathBufGuard::new(tmp.path());
    let expected_config_error =
...
fn schema_error_points_to_feature_value() {
    ...
    let _guard = agere_utils_fs::AbsolutePathBufGuard::new(tmp.path());
    let error = agere_config::config_error_from_typed_toml::<ConfigToml>(&config_path, contents)
        .expect("schema error");
...
async fn returns_empty_when_all_layers_missing() {
    ...
    let user_layer = layers
        .get_user_layer()
        .expect("expected a user layer even when AGERE_HOME/config.toml does not exist");
    assert_eq!(
        &ConfigLayerEntry {
...
allowed_access_modes = ["read-only"]
"#
            .as_bytes(),
        ),
    );
    ...
    assert!(
        state
            .requirements()
            .permission_profile
            .can_set(&PermissionProfile::from_legacy_access_policy(
...
"#,
    ...
    assert_eq!(
        config_requirements_toml
            ...
            .as_deref()
            .cloned(),
        Some(vec![agere_config::WebSearchModeRequirement::Cached])
    );
    assert_eq!(
        config_requirements_toml
        ...
        Some(agere_config::FeatureRequirementsToml {
    ...
    assert_eq!(
        ...
        Some(agere_config::ResidencyRequirement::Us)
    );
    assert_eq!(
        config_requirements
            .feature_requirements
...
#[tokio::test(flavor = "current_thread")]
async fn system_remote_access_config_keeps_cloud_access_modes() -> anyhow::Result<()> {
    let tmp = tempdir()?;
    let requirements_file = tmp.path().join("requirements.toml");
    tokio::fs::write(
        &requirements_file,
...
hostname_patterns = ["*"]
allowed_access_modes = ["read-only", "workspace-write"]
"#,
    )
    .await?;
...
async fn load_config_layers_includes_cloud_requirements() -> anyhow::Result<()> {
    let tmp = tempdir()?;
    let agere_home = tmp.path().join("home");
    tokio::fs::create_dir_all(&agere_home).await?;
...
async fn load_config_layers_includes_cloud_hook_requirements() -> anyhow::Result<()> {
    ...
    let requirements = ConfigRequirementsToml {
        hooks: Some(agere_config::ManagedHooksRequirementsToml {
            managed_dir: Some(managed_dir.clone()),
            windows_managed_dir: None,
            hooks: agere_config::HookEventsToml {
                pre_tool_use: vec![agere_config::MatcherGroup {
                    matcher: Some("^Bash$".to_string()),
                    hooks: vec![agere_config::HookHandlerConfig::Command {
...
#[tokio::test]
async fn load_config_layers_applies_matching_remote_access_config() -> anyhow::Result<()> {
    let tmp = tempdir()?;
    let agere_home = tmp.path().join("home");
    tokio::fs::create_dir_all(&agere_home).await?;
    let cwd = AbsolutePathBuf::from_absolute_path(tmp.path())?;
    ...
    assert_eq!(
        layers.requirements_toml().allowed_access_modes,
        Some(vec![
            agere_config::AccessModeRequirement::ReadOnly,
            agere_config::AccessModeRequirement::WorkspaceWrite,
        ])
    );
    assert!(
        layers
...
async fn project_layers_prefer_closest_cwd() -> std::io::Result<()> {
    ...
    tokio::fs::create_dir_all(nested.join(".openagere")).await?;
    tokio::fs::create_dir_all(project_root.join(".openagere")).await?;
    tokio::fs::write(project_root.join(".git"), "gitdir: here").await?;

    tokio::fs::write(
        project_root.join(".openagere").join(CONFIG_TOML_FILE),
        "foo = \"root\"\n",
    )
    .await?;
    tokio::fs::write(
        nested.join(".openagere").join(CONFIG_TOML_FILE),
        "foo = \"child\"\n",
    )
    .await?;
    ...
    let project_layers: Vec<_> = layers
        ...
        .filter_map(|layer| match &layer.name {
            ConfigLayerSource::Project { dot_agere_folder } => Some(dot_agere_folder),
            _ => None,
        })
        .collect();
    ...
    assert_eq!(
        ...
        nested.join(".openagere").as_path()
    );
    assert_eq!(
        project_layers[1].as_path(),
        project_root.join(".openagere").as_path()
    );
...
#[tokio::test]
async fn project_paths_resolve_relative_to_dot_agere_and_override_in_order() -> std::io::Result<()>
{
    let tmp = tempdir()?;
    let project_root = tmp.path().join("project");
    let nested = project_root.join("child");
...
"#;
    ...
    .await?;
    tokio::fs::write(nested.join(".openagere").join(CONFIG_TOML_FILE), nested_cfg).await?;
    tokio::fs::write(
        project_root.join(".openagere").join("root.txt"),
        "root instructions",
    )
    .await?;
    tokio::fs::write(
        nested.join(".openagere").join("child.txt"),
        "child instructions",
    )
    .await?;
...
async fn cli_override_model_instructions_file_sets_base_instructions() -> std::io::Result<()> {
    ...
    let agere_home = tmp.path().join("home");
    tokio::fs::create_dir_all(&agere_home).await?;
    tokio::fs::write(agere_home.join(CONFIG_TOML_FILE), "").await?;
...
#[tokio::test]
async fn project_layer_is_added_when_dot_agere_exists_without_config_toml() -> std::io::Result<()> {
    let tmp = tempdir()?;
    let project_root = tmp.path().join("project");
    let nested = project_root.join("child");
    ...
    assert_eq!(
        vec![&ConfigLayerEntry {
            name: ConfigLayerSource::Project {
                dot_agere_folder: AbsolutePathBuf::from_absolute_path(
                    project_root.join(".openagere")
                )?,
            },
            config: TomlValue::Table(toml::map::Map::new()),
            raw_toml: None,
...
#[tokio::test]
async fn agere_home_is_not_loaded_as_project_layer_from_home_dir() -> std::io::Result<()> {
    let tmp = tempdir()?;
    let home_dir = tmp.path().join("home");
    let agere_home = home_dir.join(".openagere");
    tokio::fs::create_dir_all(&agere_home).await?;
    tokio::fs::write(agere_home.join(CONFIG_TOML_FILE), "foo = \"user\"\n").await?;
...
#[tokio::test]
async fn agere_home_within_project_tree_is_not_double_loaded() -> std::io::Result<()> {
    let tmp = tempdir()?;
    let project_root = tmp.path().join("project");
    let nested = project_root.join("child");
    let project_dot_agere = project_root.join(".openagere");
    let nested_dot_agere = nested.join(".openagere");

    tokio::fs::create_dir_all(&nested_dot_agere).await?;
    tokio::fs::create_dir_all(project_root.join(".git")).await?;
    tokio::fs::write(nested_dot_agere.join(CONFIG_TOML_FILE), "foo = \"child\"\n").await?;

    tokio::fs::create_dir_all(&project_dot_agere).await?;
    make_config_for_test(
        &project_dot_agere,
        &project_root,
        TrustLevel::Trusted,
    ...
    .await?;
    let user_config_path = project_dot_agere.join(CONFIG_TOML_FILE);
    ...
    assert_eq!(
        vec![&ConfigLayerEntry {
            name: ConfigLayerSource::Project {
                dot_agere_folder: AbsolutePathBuf::from_absolute_path(&nested_dot_agere)?,
            },
...
async fn project_layers_disabled_when_untrusted_or_unknown() -> std::io::Result<()> {
    ...
    let agere_home_untrusted = tmp.path().join("home_untrusted");
    tokio::fs::create_dir_all(&agere_home_untrusted).await?;
    make_config_for_test(
        &agere_home_untrusted,
        &project_root,
        TrustLevel::Untrusted,
        /*project_root_markers*/ None,
    )
    .await?;
    let untrusted_config_path = agere_home_untrusted.join(CONFIG_TOML_FILE);
    ...
    let agere_home_unknown = tmp.path().join("home_unknown");
    tokio::fs::create_dir_all(&agere_home_unknown).await?;
    tokio::fs::write(
        agere_home_unknown.join(CONFIG_TOML_FILE),
        "foo = \"user\"\n",
    )
    .await?;
...
-> std::io::Result<()> {
    ...
    let agere_home = tmp.path().join("home");
    tokio::fs::create_dir_all(&nested).await?;
    tokio::fs::create_dir_all(&dot_agere).await?;
    tokio::fs::create_dir_all(&agere_home).await?;
    tokio::fs::write(project_root.join(".git"), "gitdir: here").await?;
    tokio::fs::write(
        dot_agere.join(CONFIG_TOML_FILE),
        r#"
[mcp_servers.sentry]
...
async fn invalid_project_config_ignored_when_untrusted_or_unknown() -> std::io::Result<()> {
    ...
    tokio::fs::create_dir_all(nested.join(".openagere")).await?;
    tokio::fs::write(project_root.join(".git"), "gitdir: here").await?;
    tokio::fs::write(nested.join(".openagere").join(CONFIG_TOML_FILE), "foo =").await?;
    ...
    for (name, trust_level) in cases {
        let agere_home = tmp.path().join(format!("home_{name}"));
        tokio::fs::create_dir_all(&agere_home).await?;
        let config_path = agere_home.join(CONFIG_TOML_FILE);
...
-> std::io::Result<()> {
    ...
    for (name, trust_level, expect_disabled) in cases {
        let agere_home = tmp.path().join(format!("home_no_config_{name}"));
        tokio::fs::create_dir_all(&agere_home).await?;
        if let Some(trust_level) = trust_level {
            make_config_for_test(
                &agere_home,
...
mod requirements_exec_policy_tests {
    ...
    use agere_config::ConfigRequirementsWithSources;
    use agere_config::RequirementSource;
    use agere_config::RequirementsExecPolicyDecisionToml;
    use agere_config::RequirementsExecPolicyParseError;
    use agere_config::RequirementsExecPolicyPatternTokenToml;
    use agere_config::RequirementsExecPolicyPrefixRuleToml;
    use agere_config::RequirementsExecPolicyToml;
    use agere_execpolicy::Decision;
    use agere_execpolicy::Evaluation;
    use agere_execpolicy::RuleMatch;
    use agere_utils_fs::AbsolutePathBuf;
    use pretty_assertions::assert_eq;
    use std::path::Path;
    use tempfile::tempdir;
    ...
    fn config_stack_for_dot_agere_folder_with_requirements(
        dot_agere_folder: &Path,
        requirements: ConfigRequirements,
    ) -> ConfigLayerStack {
        let dot_agere_folder = AbsolutePathBuf::from_absolute_path(dot_agere_folder)
            .expect("absolute dot_agere_folder");
...
"#;
    ...
    async fn loads_requirements_exec_policy_without_rules_files() -> anyhow::Result<()> {
        ...
        let config_stack =
            config_stack_for_dot_agere_folder_with_requirements(temp_dir.path(), requirements);
