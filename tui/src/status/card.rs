...
// Inline permission summary for status display (replaces former dedicated summary helper crate).
fn summarize_permission_profile(profile: &PermissionProfile, _cwd: &Path) -> String {
    match profile {
        PermissionProfile::Disabled => "danger-full-access".to_string(),
        PermissionProfile::Managed { network, .. } => {
            if network.is_enabled() {
                "workspace-write (network access enabled)".to_string()
            } else {
        ...
        PermissionProfile::External { network } => {
            if network.is_enabled() {
                "external-access (network access enabled)".to_string()
            } else {
...
impl StatusHistoryCell {
    ...
    fn new(
        ...
        // Show reasoning effort based on wire_api type with appropriate defaults.
        let effort_value = reasoning_effort_override
            .unwrap_or(config.model_reasoning_effort)
            .map(|effort| effort.to_string())
            .unwrap_or_else(|| {
                // Default based on wire_api type.
                match config.model_provider.wire_api {
                    WireApi::Responses => "none".to_string(),
                    WireApi::Anthropic | WireApi::Chat => "medium".to_string(),
                }
            });
        ...
        let access_summary = status_permission_summary(&permission_profile, config.cwd.as_path());
            ...
            && permission_profile == PermissionProfile::Disabled
        {
            ...
            format!("Custom ({access_summary}, {approval})")
        };
...
impl HistoryCell for StatusHistoryCell {
    fn display_lines(&self, width: u16) -> Vec<Line<'static>> {
        ...
        lines.push(Line::from(vec![
            Span::from(FieldFormatter::INDENT.to_string()).dim(),
            Span::from("Open Agere").bold(),
            Span::from(" ").dim(),
            Span::from(format!("(v{AGERE_CLI_VERSION})")).dim(),
        ]));
        ...
        let account_value = self.account.as_ref().map(|account| match account {
            StatusAccountDisplay::ChatGpt { email, plan } => match (email, plan) {
                ...
                (None, None) => "Account login".to_string(),
            },
            StatusAccountDisplay::ApiKey => {
                "API key configured (run agere login to use account login)".to_string()
            }
        });
        ...
        lines.push(Line::from(Vec::<Span<'static>>::new()));
        // Hide token usage only for account-plan subscribers
...
fn format_model_provider(config: &Config) -> Option<String> {
    ...
    let provider_name = config.model_provider_id.as_str();
