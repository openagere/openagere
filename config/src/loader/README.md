...
This module is the canonical place to **load and describe Agere configuration layers** (user config, CLI/session overrides, managed config, and MDM-managed preferences) and to produce:
...
Exported from `agere_config::loader`:

- `load_config_layers_state(fs, agere_home, cwd_opt, cli_overrides, overrides, cloud_requirements, thread_config_loader) -> ConfigLayerStack`
- `ConfigLayerStack`
...
use agere_utils_absolute_path::AbsolutePathBuf;
