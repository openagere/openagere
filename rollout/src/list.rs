...
use crate::state_db;
use agere_file_search as file_search;
...
impl From<agere_state::Anchor> for Cursor {
    fn from(anchor: agere_state::Anchor) -> Self {
        let ts = anchor
            .ts
            .timestamp_nanos_opt()
...
pub async fn get_threads(
    ...
    let root = agere_home.join(SESSIONS_SUBDIR);
    get_threads_in_root(
        root,
        page_size,
        cursor,
...
///
/// Directory layout: `~/.openagere/sessions/YYYY/MM/DD/rollout-YYYY-MM-DDThh-mm-ss-<uuid>.jsonl`
...
async fn find_thread_path_by_id_str_in_subdir(
    ...
    let state_db_ctx = state_db::open_if_present(agere_home, "").await;
    ...
    let mut root = agere_home.to_path_buf();
    root.push(subdir);
    if !root.exists() {
        return Ok(None);
    }
...
pub async fn find_thread_path_by_id_str(
    ...
    find_thread_path_by_id_str_in_subdir(agere_home, SESSIONS_SUBDIR, id_str).await
}
...
pub async fn find_archived_thread_path_by_id_str(
    ...
    find_thread_path_by_id_str_in_subdir(agere_home, ARCHIVED_SESSIONS_SUBDIR, id_str).await
}
