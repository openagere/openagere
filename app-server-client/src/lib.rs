...
//!
//! This crate wraps [`agere_app_server::in_process`] behind a single async API
...
//! [`InProcessClientHandle`](agere_app_server::in_process::InProcessClientHandle),
...
pub use agere_app_server::in_process::DEFAULT_IN_PROCESS_CHANNEL_CAPACITY;
...
use agere_app_server_protocol::Result as JsonRpcResult;
...
use agere_config::NoopThreadConfigLoader;
use agere_config::RemoteThreadConfigLoader;
use agere_config::ThreadConfigLoader;
use agere_core::config::Config;
pub use agere_exec_server::EnvironmentManager;
pub use agere_exec_server::EnvironmentManagerArgs;
pub use agere_exec_server::ExecServerRuntimePaths;
...
/// module exists so clients can remove a direct `agere-core` dependency
/// while legacy startup/config paths are migrated to RPCs.
pub mod legacy_core {
    pub use agere_core::DEFAULT_AGENTS_MD_FILENAME;
    pub use agere_core::LOCAL_AGENTS_MD_FILENAME;
    pub use agere_core::McpManager;
    pub use agere_core::append_message_history_entry;
    pub use agere_core::check_execpolicy_for_warnings;
    pub use agere_core::format_exec_policy_error_with_source;
    // Stub for deleted function
    pub fn grant_read_root_non_elevated(
        _policy: &str,
        _policy_cwd: &std::path::Path,
        _command_cwd: &std::path::Path,
        _path: &std::path::Path,
    ) -> Result<std::path::PathBuf, String> {
        Err("Not available".to_string())
    }
    pub use agere_core::lookup_message_history_entry;
    pub use agere_core::message_history_metadata;
    pub use agere_core::web_search_detail;

    pub mod config {
        pub use agere_core::config::*;

        // Stub for deleted function
        pub fn system_bwrap_warning(
            _profile: &agere_protocol::models::PermissionProfile,
        ) -> Option<String> {
            None
        }

        pub mod edit {
            pub use agere_core::config::edit::*;
        }
    }

    pub mod connectors {
        pub use agere_core::connectors::*;
    }

    pub mod otel_init {
        pub use agere_core::otel_init::*;
    }

    pub mod personality_migration {
        pub use agere_core::personality_migration::*;
    }

    pub mod plugins {
        pub use agere_core::plugins::PluginsManager;
    }

    pub mod review_format {
        pub use agere_core::review_format::*;
    }

    pub mod review_prompts {
        pub use agere_core::review_prompts::*;
    }

    pub mod test_support {
        pub use agere_core::test_support::*;
    }

    pub mod util {
        pub use agere_core::util::*;
    }
}
...
pub struct InProcessClientStartArgs {
    ...
    /// Whether auth loading should honor the `AGERE_API_KEY` environment variable.
    pub enable_agere_api_key_env: bool,
...
impl InProcessClientStartArgs {
    ...
    fn into_runtime_start_args(self) -> InProcessStartArgs {
        ...
        InProcessStartArgs {
            ...
            enable_agere_api_key_env: self.enable_agere_api_key_env,
            initialize,
...
/// - [`agere_app_server::in_process::InProcessClientHandle`], which speaks to
...
impl InProcessAppServerClient {
    ...
    pub async fn start(args: InProcessClientStartArgs) -> IoResult<Self> {
        ...
        let mut handle =
            agere_app_server::in_process::start(args.into_runtime_start_args()).await?;
...
mod tests {
    ...
    async fn start_test_client_with_capacity(
        ...
        InProcessAppServerClient::start(InProcessClientStartArgs {
            ...
            client_name: "agere-app-server-client-test".to_string(),
    ...
    fn command_execution_output_delta_notification(delta: &str) -> ServerNotification {
        ServerNotification::CommandExecutionOutputDelta(
            agere_app_server_protocol::CommandExecutionOutputDeltaNotification {
    ...
    fn item_completed_notification(text: &str) -> ServerNotification {
        ServerNotification::ItemCompleted(agere_app_server_protocol::ItemCompletedNotification {
            thread_id: "thread".to_string(),
            turn_id: "turn".to_string(),
            item: agere_app_server_protocol::ThreadItem::AgentMessage {
                id: "item".to_string(),
                text: text.to_string(),
                phase: None,
    ...
    fn turn_completed_notification() -> ServerNotification {
        ServerNotification::TurnCompleted(agere_app_server_protocol::TurnCompletedNotification {
            thread_id: "thread".to_string(),
            turn: agere_app_server_protocol::Turn {
                id: "turn".to_string(),
                items: Vec::new(),
                status: agere_app_server_protocol::TurnStatus::Completed,
                error: None,
                started_at: None,
                completed_at: Some(0),
                duration_ms: Some(1),
    ...
    async fn typed_request_reports_json_rpc_errors() {
        ...
        let err = client
            .request_typed::<ConfigRequirementsReadResponse>(ClientRequest::ThreadRead {
                ...
                params: agere_app_server_protocol::ThreadReadParams {
    ...
    async fn threads_started_via_app_server_are_visible_through_typed_requests() {
        ...
        let response: ThreadStartResponse = client
            ...
            .expect("thread/start should succeed");
        let read = client
            .request_typed::<agere_app_server_protocol::ThreadReadResponse>(
    ...
    async fn forward_in_process_event_preserves_transcript_notifications_under_backpressure() {
        ...
        assert!(matches!(
            ...
            InProcessServerEvent::ServerNotification(ServerNotification::ItemCompleted(
                ...
                &notification.item,
                agere_app_server_protocol::ThreadItem::AgentMessage { text, .. } if text == "hello"
            )
        ));
        assert!(matches!(
            &events[4],
            InProcessServerEvent::ServerNotification(ServerNotification::TurnCompleted(
                notification
            )) if notification.turn.status == agere_app_server_protocol::TurnStatus::Completed
        ));
    }
    ...
    async fn remote_typed_request_roundtrip_works() {
        ...
        let response: GetAccountResponse = client
            .request_typed(ClientRequest::GetAccount {
                request_id: RequestId::Integer(1),
                params: agere_app_server_protocol::GetAccountParams {
                    refresh_token: false,
                },
            })
            .await
        ...
        for event in &remaining_events {
            match event {
                ...
                AppServerEvent::ServerNotification(ServerNotification::TurnCompleted(
                    ...
                    == agere_app_server_protocol::TurnStatus::Completed =>
                {
    ...
    fn event_requires_delivery_marks_transcript_and_terminal_events() {
        assert!(event_requires_delivery(
            &InProcessServerEvent::ServerNotification(
                agere_app_server_protocol::ServerNotification::TurnCompleted(
                    agere_app_server_protocol::TurnCompletedNotification {
                        thread_id: "thread".to_string(),
                        turn: agere_app_server_protocol::Turn {
                            id: "turn".to_string(),
                            items: Vec::new(),
        ...
        assert!(event_requires_delivery(
            &InProcessServerEvent::ServerNotification(
                agere_app_server_protocol::ServerNotification::AgentMessageDelta(
                    agere_app_server_protocol::AgentMessageDeltaNotification {
                        thread_id: "thread".to_string(),
                        turn_id: "turn".to_string(),
                        item_id: "item".to_string(),
        ...
        assert!(event_requires_delivery(
            &InProcessServerEvent::ServerNotification(
                agere_app_server_protocol::ServerNotification::ItemCompleted(
                    agere_app_server_protocol::ItemCompletedNotification {
                        thread_id: "thread".to_string(),
                        turn_id: "turn".to_string(),
                        item: agere_app_server_protocol::ThreadItem::AgentMessage {
                            id: "item".to_string(),
        ...
        assert!(!event_requires_delivery(
            &InProcessServerEvent::ServerNotification(
                agere_app_server_protocol::ServerNotification::CommandExecutionOutputDelta(
                    agere_app_server_protocol::CommandExecutionOutputDeltaNotification {
                        thread_id: "thread".to_string(),
                        turn_id: "turn".to_string(),
                        item_id: "item".to_string(),
    ...
    async fn runtime_start_args_use_remote_thread_config_loader_when_configured() {
        ...
        assert_eq!(
            err.code(),
            agere_config::ThreadConfigLoadErrorCode::RequestFailed
        );
    }
