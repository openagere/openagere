...
use agere_app_server_protocol::TextPosition as AppTextPosition;
use agere_app_server_protocol::TextRange as AppTextRange;
use agere_config::ConfigLoadError;
use agere_config::TextRange as CoreTextRange;
use agere_core::ExecPolicyError;
...
fn project_config_warning(config: &Config) -> Option<ConfigWarningNotification> {
    ...
    for layer in config.config_layer_stack.get_layers(
        ...
        let ConfigLayerSource::Project { dot_agere_folder } = &layer.name else {
            continue;
        };
        ...
        disabled_folders.push((
            dot_agere_folder.as_path().display().to_string(),
...
pub async fn run_main_with_transport_options(
        ...
        .await
    {
        Ok(config) => {
            ...
            match effective_toml.try_into() {
                Ok(config_toml) => {
                    if let Err(err) = agere_core::personality_migration::maybe_migrate_personality(
                        &config.agere_home,
                        &config_toml,
                    )
                    .await
    ...
    let feedback = AgereFeedback::new();

    let otel = agere_core::otel_init::build_provider(
        &config,
        env!("CARGO_PKG_VERSION"),
        Some("agere-app-server"),
        default_analytics_enabled,
    )
    .map_err(|e| {
        std::io::Error::new(
    ...
    let state_db_result = agere_state::StateRuntime::init(
