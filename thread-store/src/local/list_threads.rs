...
use agere_rollout::parse_cursor;
...
pub(super) async fn list_threads(
    ...
    let sort_key = match params.sort_key {
        ThreadSortKey::CreatedAt => agere_rollout::ThreadSortKey::CreatedAt,
        ThreadSortKey::UpdatedAt => agere_rollout::ThreadSortKey::UpdatedAt,
    };
    let sort_direction = match params.sort_direction {
        SortDirection::Asc => agere_rollout::SortDirection::Asc,
        SortDirection::Desc => agere_rollout::SortDirection::Desc,
    };
...
async fn list_rollout_threads(
    ...
    params: &ListThreadsParams,
    cursor: Option<&agere_rollout::Cursor>,
    sort_key: agere_rollout::ThreadSortKey,
    sort_direction: agere_rollout::SortDirection,
) -> ThreadStoreResult<agere_rollout::ThreadsPage> {
...
mod tests {
    ...
    #[tokio::test]
    async fn list_threads_state_db_only_preserves_goal_preview_without_first_user_message() {
        let home = TempDir::new().expect("temp dir");
        let config = test_config(home.path());
        let store = LocalThreadStore::new(config.clone());
        let uuid = Uuid::from_u128(104);
        let thread_id = ThreadId::from_string(&uuid.to_string()).expect("valid thread id");
        let rollout_path = home.path().join("rollout-goal-preview.jsonl");
        fs::write(&rollout_path, "").expect("placeholder rollout file");
        ...
        assert_eq!(page.items[0].preview, "goal preview");
