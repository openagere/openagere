...
pub(crate) enum WindowsRestrictionEnableMode {
    Elevated,
    Legacy,
}
...
pub(crate) enum AppEvent {
    ...
    /// Switch the active model provider to `name`. The handler reads the entry
    /// from `provider.toml`, writes the mirrored fields into `config.toml`, and
    /// reloads the in-memory config.
    SwitchProvider {
        name: String,
    },
    ...
    /// Triggered when the user presses # in the chat composer.
    StartGrepSearch(String),
    /// Results from an external ripgrep search.
    GrepSearchResult {
        query: String,
        matches: Vec<RgMatch>,
    },
    ...
    /// Open the current thread goal editor.
    ...
    /// Remote provider templates loaded.
    ProviderTemplatesLoaded {
    ...
    /// approval/access configuration on Continue; if `None`, it performs no
    ...
    /// Prompt to configure Windows filesystem/network access before using Agent mode.
    #[cfg_attr(not(target_os = "windows"), allow(dead_code))]
    OpenWindowsRestrictionEnablePrompt {
        preset: ApprovalPreset,
    },

    /// Open the Windows access fallback prompt after declining or failing elevation.
    #[cfg_attr(not(target_os = "windows"), allow(dead_code))]
    OpenWindowsRestrictionFallbackPrompt {
        preset: ApprovalPreset,
    },

    /// Begin the elevated Windows access setup flow.
    #[cfg_attr(not(target_os = "windows"), allow(dead_code))]
    BeginWindowsRestrictionElevatedSetup {
        preset: ApprovalPreset,
    },

    /// Begin the non-elevated Windows access setup flow.
    #[cfg_attr(not(target_os = "windows"), allow(dead_code))]
    BeginWindowsRestrictionLegacySetup {
        preset: ApprovalPreset,
    },

    /// Begin a non-elevated grant of read access for an additional directory.
    #[cfg_attr(not(target_os = "windows"), allow(dead_code))]
    BeginWindowsRestrictionGrantReadRoot {
        path: String,
    },

    /// Result of attempting to grant read access for an additional directory.
    #[cfg_attr(not(target_os = "windows"), allow(dead_code))]
    WindowsRestrictionGrantReadRootCompleted {
        path: PathBuf,
        error: Option<String>,
    },

    /// Enable the Windows access layer and switch to Agent mode.
    #[cfg_attr(not(target_os = "windows"), allow(dead_code))]
    EnableWindowsRestrictionForAgentMode {
        preset: ApprovalPreset,
        mode: WindowsRestrictionEnableMode,
    },
