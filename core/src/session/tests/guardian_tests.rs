...
use agere_protocol::models::AdditionalPermissionProfile as PermissionProfile;
use agere_protocol::models::ContentItem;
use agere_protocol::models::NetworkPermissions;
use agere_protocol::models::ResponseItem;
use agere_protocol::models::function_call_output_content_items_to_text;
...
use core_test_support::agere_linux_exe_or_skip;
    ...
    *session.active_turn.lock().await = Some(ActiveTurn::default());
    ...
    timeout(Duration::from_secs(5), async {
        loop {
            ...
            if matches!(
                event.msg,
                agere_protocol::protocol::EventMsg::GuardianAssessment(_)
            ) {
                break;
            }
        }
...
async fn guardian_allows_shell_additional_permissions_requests_past_policy_validation() {
    ...
    let (mut session, mut turn_context_raw) = make_session_and_context().await;
    turn_context_raw.agere_linux_exe = agere_linux_exe_or_skip!();
    turn_context_raw
        .approval_policy
        .set(AskForApproval::OnRequest)
        .expect("test setup should allow updating approval policy");
    ...
    turn_context_raw.permission_profile = agere_protocol::models::PermissionProfile::Disabled;
    let mut config = (*turn_context_raw.config).clone();
    config.model_provider.base_url = Some(format!("{}/v1", server.uri()));
    let config = Arc::new(config);
    let models_manager = models_manager_with_provider(
    ...
    let resp = handler
        .handle(ToolInvocation {
            ...
            tool_name: agere_tools::ToolName::plain("shell"),
        ...
        .handle(ToolInvocation {
            ...
            payload: ToolPayload::Function {
                arguments: serde_json::json!({
                    "cmd": "echo hi",
                    "permission_level": ExecutionPermissionLevel::WithAdditionalPermissions,
                    "justification": "need additional scoped filesystem permissions",
                })
                .to_string(),
            },
        })
...
async fn guardian_subagent_does_not_inherit_parent_exec_policy_rules() {
    ...
    config.config_layer_stack = ConfigLayerStack::new(
        vec![ConfigLayerEntry::new(
            ConfigLayerSource::Project {
                dot_agere_folder: project_dir.path().abs(),
            },
    ...
    let thread_store = Arc::new(agere_thread_store::LocalThreadStore::new(
        agere_rollout::RolloutConfig::from_view(&config),
    ));

    let AgereSpawnOk { agere, .. } = Agere::spawn(AgereSpawnArgs {
        config,
        auth_manager,
        models_manager,
