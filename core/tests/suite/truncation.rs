...
async fn tool_call_output_exceeds_limit_truncated_chars_limit() -> Result<()> {
    ...
    // Use a model that exposes the shell_command tool.
    let mut builder = test_agere().with_model("gpt-5.2");
    ...
    let mut builder = test_agere().with_config(move |config| {
        ...
        servers.insert(
            ...
            agere_config::types::McpServerConfig {
                transport: agere_config::types::McpServerTransportConfig::Stdio {
...
async fn mcp_image_output_preserves_image_and_no_text_summary() -> Result<()> {
    ...
    let access_policy = permission_profile.to_legacy_access_policy(fixture.cwd.path())?;
