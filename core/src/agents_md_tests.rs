...
async fn project_root_markers_are_honored_for_agents_discovery() {
    ...
    fs::write(root.path().join(".agere-root"), "").unwrap();
...
async fn instruction_sources_include_global_before_agents_md_docs() {
    ...
    let global_agents = cfg.agere_home.join(DEFAULT_AGENTS_MD_FILENAME);
    fs::create_dir_all(&cfg.agere_home).unwrap();
...
fn create_skill(agere_home: PathBuf, name: &str, description: &str) {
    let skill_dir = agere_home.join(format!("skills/{name}"));
