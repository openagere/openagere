...
Set a stable path to the skill CLI (default `AGERE_HOME` is `~/.openagere`):

```
export AGERE_HOME="${AGERE_HOME:-$HOME/.openagere}"
export IMAGE_GEN="$AGERE_HOME/skills/.system/imagegen/scripts/image_gen.py"
```
...
- Network and managed-access notes for fallback CLI mode: `references/agere-network.md`
- Built-in-first transparent image workflow: `SKILL.md` and `$AGERE_HOME/skills/.system/imagegen/scripts/remove_chroma_key.py`
