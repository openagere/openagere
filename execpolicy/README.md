...
- The legacy rule matcher lives in `agere-execpolicy-legacy`.
...
- From the Agere CLI, run `agere execpolicy check` subcommand with one or more policy files (for example `src/default.rules`) to check a command:

```bash
agere execpolicy check --rules path/to/policy.rules git status
```

- To opt into basename fallback for absolute program paths, pass `--resolve-host-executables`:

```bash
agere execpolicy check \
...
```bash
cargo run -p agere-execpolicy -- check --rules path/to/policy.rules git status
```
