...
pub(crate) fn render_menu_surface(area: Rect, buf: &mut Buffer) -> Rect {
    render_menu_surface_for_style(area, buf, popup_surface_style())
}

fn render_menu_surface_for_style(area: Rect, buf: &mut Buffer, surface_style: Style) -> Rect {
    if area.is_empty() {
        return area;
    }
...
mod surface_tests {
    ...
    #[test]
    fn menu_surface_paints_background_without_border_glyphs() {
        let area = Rect::new(0, 0, 8, 3);
        let mut buf = Buffer::empty(area);

        let content =
            render_menu_surface_for_style(area, &mut buf, Style::default().bg(Color::DarkGray));
    ...
    #[test]
    fn menu_surface_fallback_renders_border_without_background() {
        let area = Rect::new(0, 0, 8, 3);
        let mut buf = Buffer::empty(area);
