...
async fn build_memory_tool_developer_instructions_renders_embedded_template() {
    ...
    let agere_home = AbsolutePathBuf::from_absolute_path(temp.path()).unwrap();
    let memories_dir = agere_home.join("memories");
    ...
    let instructions = build_memory_tool_developer_instructions(&agere_home)
        .await
        .unwrap();
