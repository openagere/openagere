    ...
    if config.code_mode_enabled {
        let namespace_descriptions = params
            ...
            .map(|(namespace, detail)| {
                (
                    namespace.clone(),
                    agere_code_mode::ToolNamespaceDescription {
        ...
        plan.register_handler(
            agere_code_mode::WAIT_TOOL_NAME,
        ...
        && (deferred_mcp_tools_for_search.is_some() || !deferred_dynamic_tools.is_empty())
    {
        ...
        if !deferred_dynamic_tools.is_empty() {
            search_source_infos.push(ToolSearchSourceInfo {
                ...
                description: Some("Tools provided by the current Agere thread.".to_string()),
            });
        }
...
fn compare_code_mode_tools(
    left: &agere_code_mode::ToolDefinition,
    right: &agere_code_mode::ToolDefinition,
...
fn code_mode_namespace_name<'a>(
    tool: &agere_code_mode::ToolDefinition,
    namespace_descriptions: &'a BTreeMap<String, agere_code_mode::ToolNamespaceDescription>,
) -> Option<&'a str> {
    tool.tool_name
        .namespace
        .as_ref()
