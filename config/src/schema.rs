...
use agere_features::legacy_feature_keys;
...
pub fn features_schema(schema_gen: &mut SchemaGenerator) -> Schema {
    ...
    for feature in FEATURES {
        if feature.id == agere_features::Feature::Artifact {
            continue;
        }
        if feature.id == agere_features::Feature::MultiAgentV2 {
            validation.properties.insert(
                feature.key.to_string(),
                schema_gen.subschema_for::<agere_features::FeatureToml<
                    agere_features::MultiAgentV2ConfigToml,
                >>(),
            );
            continue;
        }
