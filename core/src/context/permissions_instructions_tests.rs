...
#[test]
fn renders_access_mode_text() {
    assert_eq!(
        access_mode_text(AccessMode::WorkspaceWrite, NetworkAccess::Restricted),
        "Filesystem access defines which files can be read or written. `access_mode` is `workspace-write`: Reading files is allowed, and editing files in `cwd` and `writable_roots` is permitted. Editing files in other directories requires approval. Network access is restricted."
    );

    assert_eq!(
        access_mode_text(AccessMode::ReadOnly, NetworkAccess::Restricted),
        "Filesystem access defines which files can be read or written. `access_mode` is `read-only`: Only reading files is allowed. Network access is restricted."
    );

    assert_eq!(
        access_mode_text(AccessMode::DangerFullAccess, NetworkAccess::Enabled),
        "Filesystem access defines which files can be read or written. `access_mode` is `danger-full-access`: No filesystem restriction — all commands are permitted. Network access is enabled."
    );
}
...
fn builds_permissions_from_profile() {
    ...
    let text = instructions.body();
    assert!(text.contains("`access_mode` is `workspace-write`"));
...
#[test]
fn granular_policy_includes_command_permission_instructions_when_access_approval_can_prompt() {
    let text = approval_text(
        AskForApproval::Granular(GranularApprovalConfig {
