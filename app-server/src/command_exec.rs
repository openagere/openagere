...
use agere_app_server_protocol::CommandExecResizeResponse;
...
use agere_app_server_protocol::CommandExecTerminateResponse;
use agere_app_server_protocol::CommandExecWriteParams;
use agere_app_server_protocol::CommandExecWriteResponse;
use agere_app_server_protocol::JSONRPCErrorError;
use agere_app_server_protocol::ServerNotification;
use agere_core::config::StartedNetworkProxy;
use agere_core::exec::DEFAULT_EXEC_COMMAND_TIMEOUT_MS;
use agere_core::exec::ExecExpiration;
use agere_core::exec::ExecRequest;
use agere_core::exec::IO_DRAIN_TIMEOUT_MS;
use agere_protocol::exec_output::bytes_to_string_smart;
use agere_utils_pty::ProcessHandle;
use agere_utils_pty::SpawnedProcess;
use agere_utils_pty::TerminalSize;
use base64::Engine;
...
impl CommandExecManager {
    pub(crate) async fn start(
        ...
        let spawned = if tty {
            agere_utils_pty::spawn_pty_process(
                program,
                args,
                cwd.as_path(),
                &env,
            ...
            agere_utils_pty::spawn_pipe_process(program, args, cwd.as_path(), &env, &arg0).await
        } else {
            agere_utils_pty::spawn_pipe_process_no_stdin(program, args, cwd.as_path(), &env, &arg0)
                .await
        };
...
mod tests {
    ...
    async fn dropped_control_request_is_reported_as_not_running() {
        ...
        let request_id = ConnectionRequestId {
            ...
            request_id: agere_app_server_protocol::RequestId::Integer(3),
        };
