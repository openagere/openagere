...
pub fn emit_feedback_request_tags_with_auth_env(
    ...
    tracing::info!(
        ...
        auth_env_agere_api_key_present = tracing::field::debug(auth_env.agere_api_key_env_present),
        auth_env_agere_api_key_enabled = tracing::field::debug(auth_env.agere_api_key_env_enabled),
...
#[derive(Clone)]
pub struct AgereFeedback {
    inner: Arc<FeedbackInner>,
}

impl Default for AgereFeedback {
    fn default() -> Self {
        Self::new()
    }
}
...
impl FeedbackSnapshot {
    ...
    pub fn save_to_temp_file(&self) -> io::Result<PathBuf> {
        ...
        let filename = format!("agere-feedback-{}.log", self.thread_id);
    ...
    fn feedback_attachments(
        ...
        if include_logs {
            attachments.push(Attachment {
                ...
                filename: String::from("agere-logs.log"),
...
mod tests {
    ...
    fn ring_buffer_drops_front_when_full() {
        let fb = AgereFeedback::with_capacity(/*max_bytes*/ 8);
        {
    ...
    fn feedback_attachments_gate_connectivity_diagnostics() {
        ...
        let attachments_without_diagnostics = AgereFeedback::new()
            .snapshot(/*session_id*/ None)
