...
#[cfg(unix)]
const AGERE_MANAGED_CONFIG_SYSTEM_PATH: &str = "/etc/agere/managed_config.toml";
...
pub(super) struct LoadedConfigLayers {
    /// If present, data read from a file such as `/etc/agere/managed_config.toml`.
...
pub(super) async fn load_config_layers_internal(
    ...
    let managed_config_path = AbsolutePathBuf::from_absolute_path(
        managed_config_path.unwrap_or_else(|| managed_config_default_path(agere_home)),
    )?;
...
pub(super) async fn read_config_from_path(
    ...
    match fs.read_file_text(path).await {
...
pub(super) fn managed_config_default_path(agere_home: &Path) -> PathBuf {
    #[cfg(unix)]
    {
        let _ = agere_home;
        PathBuf::from(AGERE_MANAGED_CONFIG_SYSTEM_PATH)
    }
