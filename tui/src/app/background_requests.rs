...
impl App {
    ...
    pub(super) fn fetch_plugin_install(
        ...
        tokio::spawn(async move {
            ...
            let result = fetch_plugin_install(request_handle, marketplace_path, plugin_name)
                .await
                .map_err(|err| format!("Failed to install plugin: {}", format_error_chain(err)));
    ...
    pub(super) fn refresh_plugin_mentions(&mut self) {
        ...
        tokio::spawn(async move {
            let plugins = PluginsManager::new(config.agere_home.to_path_buf())
...
pub(super) async fn send_add_credits_nudge_email(
    request_handle: AppServerRequestHandle,
    credit_type: AddCreditsNudgeCreditType,
) -> Result<agere_app_server_protocol::AddCreditsNudgeEmailStatus> {
    let request_id = RequestId::String(format!("add-credits-nudge-{}", Uuid::new_v4()));
    let response: agere_app_server_protocol::SendAddCreditsNudgeEmailResponse = request_handle
...
pub(super) async fn fetch_plugins_list(
    ...
    let mut response = fetch_plugins_list_once(&request_handle, cwd).await?;
    hide_cli_only_plugin_marketplaces(&mut response);
    Ok(response)
}

async fn fetch_plugins_list_once(
    request_handle: &AppServerRequestHandle,
    cwd: AbsolutePathBuf,
) -> Result<PluginListResponse> {
...
const CLI_HIDDEN_PLUGIN_MARKETPLACES: &[&str] = &["openagere-bundled"];
...
pub(super) type McpInventoryMaps = (
    HashMap<String, agere_protocol::mcp::Tool>,
    HashMap<String, Vec<agere_protocol::mcp::Resource>>,
    HashMap<String, Vec<agere_protocol::mcp::ResourceTemplate>>,
...
mod tests {
    ...
    #[test]
    fn hide_cli_only_plugin_marketplaces_removes_openagere_bundled() {
        let mut response = PluginListResponse {
            marketplaces: vec![
                PluginMarketplaceEntry {
                    name: "openagere-bundled".to_string(),
                    path: Some(test_absolute_path("/marketplaces/openagere-bundled")),
                    interface: None,
                    plugins: Vec::new(),
                },
                PluginMarketplaceEntry {
                    name: "openagere-curated".to_string(),
                    path: Some(test_absolute_path("/marketplaces/openagere-curated")),
                    interface: None,
                    plugins: Vec::new(),
                },
            ],
