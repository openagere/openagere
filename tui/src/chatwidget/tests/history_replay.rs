...
async fn resumed_initial_messages_render_history() {
    ...
    let configured = agere_protocol::protocol::SessionConfiguredEvent {
        ...
        permission_profile: agere_protocol::models::PermissionProfile::read_only(),
...
async fn thread_snapshot_replay_does_not_duplicate_agent_message_history() {
    ...
    chat.handle_agere_event_replay(Event {
        id: "turn-1".into(),
...
async fn session_configured_syncs_widget_config_permissions_and_cwd() {
    ...
    let expected_file_system_policy = FileSystemAccessPolicy::restricted(vec![
    ...
    let expected_access_policy = expected_permission_profile
        .to_legacy_access_policy(expected_cwd.as_path())
        .expect("permission profile should project to legacy access policy");
    let configured = agere_protocol::protocol::SessionConfiguredEvent {
    ...
    assert_eq!(
        &chat.config_ref().legacy_access_policy(),
...
#[tokio::test]
async fn session_configured_external_profile_keeps_external_runtime_policy() {
    let (mut chat, _rx, _ops) = make_chatwidget_manual(/*model_override*/ None).await;
    ...
    let expected_access_policy = expected_permission_profile
        .to_legacy_access_policy(test_path_buf("/home/user/external").as_path())
        .expect("external profile should project to legacy access policy");
    let configured = agere_protocol::protocol::SessionConfiguredEvent {
        session_id: ThreadId::new(),
        forked_from_id: None,
        thread_name: None,
    ...
    assert_eq!(
        chat.config_ref()
            ...
            .kind,
        FileSystemAccessLevel::External,
    );
    assert_eq!(
        chat.config_ref().permissions.network_access_policy(),
        NetworkAccessPolicy::Restricted,
    );
}
...
async fn forked_thread_history_line_without_name_shows_id_once_snapshot() {
    ...
    chat.config.agere_home = agere_utils_fs::AbsolutePathBuf::from_absolute_path(temp.path())
...
async fn app_server_forked_thread_history_line_uses_app_server_title_snapshot() {
    ...
    assert!(
        ...
        "app-server fork title lookup should not read local AGERE_HOME"
    );
