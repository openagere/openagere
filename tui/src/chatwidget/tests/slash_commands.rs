...
async fn slash_init_skips_when_project_doc_exists() {
    ...
    match op_rx.try_recv() {
        ...
        other => panic!("expected no Agere op to be sent, got {other:?}"),
    }
...
async fn goal_slash_command_preserves_attached_images() {
    ...
    assert_eq!(draft.local_images[0].path, local_image);
...
async fn queued_goal_slash_command_preserves_large_paste() {
    let (mut chat, mut rx, mut op_rx) = make_chatwidget_manual(/*model_override*/ None).await;
    chat.set_feature_enabled(Feature::Goals, /*enabled*/ true);
    let paste = "x".repeat(agere_protocol::user_input::MAX_USER_INPUT_TEXT_CHARS + 1);
...
async fn copy_shortcut_can_be_remapped() {
    ...
    keymap_config.global.copy = Some(agere_config::types::KeybindingsSpec::One(
        agere_config::types::KeybindingSpec("ctrl-x".to_string()),
    ));
    let runtime_keymap =
        ...
        ServerNotification::ThreadGoalUpdated(
            agere_app_server_protocol::ThreadGoalUpdatedNotification {
                ...
                goal: agere_app_server_protocol::ThreadGoal {
                    ...
                    status: agere_app_server_protocol::ThreadGoalStatus::Active,
                    token_budget: None,
                    tokens_used: 0,
                    time_used_seconds: 0,
                    created_at: 1,
...
async fn slash_rollout_displays_current_path() {
    let (mut chat, mut rx, _op_rx) = make_chatwidget_manual(/*model_override*/ None).await;
    let rollout_path = PathBuf::from("/tmp/agere-test-rollout.jsonl");
