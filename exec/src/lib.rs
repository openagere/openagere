...
use agere_app_server_client::EnvironmentManager;
use agere_app_server_client::EnvironmentManagerArgs;
use agere_app_server_client::ExecServerRuntimePaths;
use agere_app_server_client::InProcessAppServerClient;
use agere_app_server_client::InProcessClientStartArgs;
use agere_app_server_client::InProcessServerEvent;
...
use agere_app_server_protocol::ThreadItem as AppServerThreadItem;
...
use agere_core::config::load_config_as_toml_with_cli_and_loader_overrides;
use agere_core::find_thread_meta_by_name_str;
use agere_core::format_exec_policy_error_with_source;
...
use agere_login::default_client::set_default_originator;
use agere_login::enforce_login_restrictions;
use agere_otel::set_parent_from_context;
use agere_otel::traceparent_context_from_env;
...
use agere_utils_cli::SharedCliOptions;
...
pub use event_processor_with_jsonl_output::AgereStatus;
...
enum StdinPromptBehavior {
    ...
    /// `agere exec` behavior for `agere exec` with piped input.
    RequiredIfPiped,
    /// Always treat stdin as the prompt, used for the explicit `agere exec -`
...
pub async fn run_main(cli: Cli, arg0_paths: Arg0DispatchPaths) -> anyhow::Result<()> {
    if let Err(err) = set_default_originator("agere_exec".to_string()) {
        tracing::warn!(?err, "Failed to set agere exec originator override {err:?}");
    }
    ...
    let SharedCliOptions {
        ...
        model: model_cli_arg,
        config_profile,
        access_mode: access_mode_cli_arg,
        full_auto,
        dangerously_bypass_approvals: _,
        cwd,
        add_dir,
    } = shared;
    ...
    let _config_toml = match load_config_as_toml_with_cli_and_loader_overrides(
        &agere_home,
        Some(&config_cwd),
    ...
    let otel = match std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| {
        agere_core::otel_init::build_provider(
            &config,
    ...
    let in_process_start_args = InProcessClientStartArgs {
        ...
        enable_agere_api_key_env: true,
        client_name: "agere_exec".to_string(),
...
async fn run_exec_session(args: ExecRunArgs) -> anyhow::Result<()> {
    ...
    let (initial_operation, prompt_summary) = match (command.as_ref(), prompt, images) {
        (Some(ExecCommand::Review(review_cli)), _, _) => {
            ...
            let summary = agere_core::review_prompts::user_facing_hint(&review_request.target);
    ...
    // Print the effective configuration and initial request so users can see what Agere
    // is using.
    event_processor.print_config_summary(&config, &prompt_summary, &session_configured);
    // system_bwrap_warning removed (legacy bubblewrap isolation path deleted)

    info!("Agere initialized with event: {session_configured:?}");
        ...
        match server_event {
            ...
            InProcessServerEvent::ServerNotification(mut notification) => {
                if let ServerNotification::Error(payload) = &notification {
                    ...
                    && matches!(
                        payload.turn.status,
                        agere_app_server_protocol::TurnStatus::Failed
                            | agere_app_server_protocol::TurnStatus::Interrupted
                    )
                {
                    error_seen = true;
                }
...
fn session_configured_from_thread_response(
    ...
    let session_id = agere_protocol::ThreadId::from_string(thread_id)
    ...
    Ok(SessionConfiguredEvent {
        ...
        permission_profile: permission_profile
            .unwrap_or_else(|| PermissionProfile::from_legacy_access_policy(&access_policy)),
        cwd,
        reasoning_effort,
        history_log_id: 0,
...
async fn resolve_resume_thread_id(
    ...
    if let Some(state_db) = agere_core::get_state_db(config).await {
        ...
        if let Some((_, session_meta)) =
            find_thread_meta_by_name_str(&config.agere_home, session_id).await?
...
/// Shared CLI entry used by `agere-exec` and `agere-linux-helper`.
pub fn run_cli_package_main() -> anyhow::Result<()> {
    use agere_arg0::Arg0DispatchPaths;
...
/// Top-level CLI parser shared between the binary entry-point and tests.
#[derive(Parser, Debug)]
pub struct TopCli {
    #[clap(flatten)]
