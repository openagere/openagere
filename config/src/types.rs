//! Types used to define loaded and effective Agere configuration values.
...
pub use agere_protocol::config_types::AltScreenMode;
pub use agere_protocol::config_types::ApprovalsReviewer;
use agere_protocol::config_types::EnvironmentVariablePattern;
pub use agere_protocol::config_types::ModeKind;
pub use agere_protocol::config_types::Personality;
pub use agere_protocol::config_types::ServiceTier;
use agere_protocol::config_types::ShellEnvironmentPolicy;
use agere_protocol::config_types::ShellEnvironmentPolicyInherit;
pub use agere_protocol::config_types::WebSearchMode;
...
/// Determine where Agere should store CLI auth credentials.
#[derive(Debug, Default, Copy, Clone, PartialEq, Eq, Serialize, Deserialize, JsonSchema)]
#[serde(rename_all = "lowercase")]
pub enum AuthCredentialsStoreMode {
    #[default]
    /// Persist credentials in AGERE_HOME/auth.json.
    File,
    /// Persist credentials in the keyring. Fail if unavailable.
    Keyring,
    /// Use keyring when available; otherwise, fall back to a file in AGERE_HOME.
    Auto,
    /// Store credentials in memory only for the current process.
    Ephemeral,
}

/// Determine where Agere should store and read MCP credentials.
#[derive(Debug, Default, Copy, Clone, PartialEq, Eq, Serialize, Deserialize, JsonSchema)]
#[serde(rename_all = "lowercase")]
...
pub enum WindowsAccessModeToml {
    Elevated,
    Unelevated,
}
...
pub struct WindowsToml {
    pub access_mode: Option<WindowsAccessModeToml>,
    /// Defaults to `true`. Set to `false` to launch restricted child processes on
    /// `Winsta0\\Default` instead of a private desktop.
    pub private_desktop: Option<bool>,
}
...
pub struct AnalyticsConfigToml {
    /// When `false`, disables analytics across Agere product surfaces in this profile.
...
pub struct FeedbackConfigToml {
    /// When `false`, disables the feedback flow across Agere product surfaces.
    pub enabled: Option<bool>,
}
...
pub struct MemoriesToml {
    ...
    /// Minimum remaining percentage required in Agere rate-limit windows before memory startup runs.
...
pub struct AppConfig {
    /// When `false`, Agere does not surface this app.
    #[serde(default = "default_enabled")]
    pub enabled: bool,
...
pub struct Tui {
    ...
    /// When unset, the TUI defaults to: `model-with-reasoning`, `current-dir`,
    /// `used-tokens`, `context-remaining`, `context-used`, and `context-window-size`.
    #[serde(default)]
    ...
    /// Use `/theme` in the TUI or see `$AGERE_HOME/themes` for custom themes.
    #[serde(default)]
    ...
    /// transcript exceeds this cap. Omit to use Agere's terminal-specific default. Set to `0` to
...
/// (primarily the Agere IDE extension). NOTE: these are different from
...
pub struct Notice {
    ...
    /// Tracks whether the user opted out of Agere-managed fast defaults.
    pub fast_default_opt_out: Option<bool>,
    /// Tracks whether the user opted out of the rate limit model switch reminder.
    pub hide_rate_limit_model_nudge: Option<bool>,
    /// Tracks whether the user has seen the model migration prompt
    pub hide_gpt5_1_migration_prompt: Option<bool>,
    /// Legacy compatibility key for the old gpt-5.1-agere-max migration prompt.
    #[serde(rename = "hide_gpt-5.1-agere-max_migration_prompt")]
    pub hide_gpt_5_1_agere_max_migration_prompt: Option<bool>,
...
pub struct MarketplaceConfig {
    /// Last time Agere successfully added or refreshed this marketplace.
    #[serde(default)]
    pub last_updated: Option<String>,
    /// Git revision Agere last successfully activated for this marketplace.
    #[serde(default)]
...
impl From<AccessWorkspaceWrite> for agere_app_server_protocol::AccessSettings {
    fn from(access_workspace_write: AccessWorkspaceWrite) -> Self {
        Self {
            writable_roots: access_workspace_write.writable_roots,
            network_access: Some(access_workspace_write.network_access),
            exclude_tmpdir_env_var: Some(access_workspace_write.exclude_tmpdir_env_var),
            exclude_slash_tmp: Some(access_workspace_write.exclude_slash_tmp),
        }
    }
}
