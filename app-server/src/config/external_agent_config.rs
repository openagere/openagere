...
use agere_core::plugins::PluginsManager;
use agere_core_plugins::marketplace::MarketplacePluginInstallPolicy;
use agere_core_plugins::marketplace::find_marketplace_manifest_path;
use agere_core_plugins::marketplace_add::MarketplaceAddRequest;
use agere_core_plugins::marketplace_add::add_marketplace;
use agere_core_plugins::marketplace_add::is_local_marketplace_source;
...
const EXTERNAL_AGENT_CONFIG_DETECT_METRIC: &str = "agere.external_agent_config.detect";
const EXTERNAL_AGENT_CONFIG_IMPORT_METRIC: &str = "agere.external_agent_config.import";
...
impl ExternalAgentConfigService {
    ...
    #[cfg(test)]
    fn new_for_test(agere_home: PathBuf, external_agent_home: PathBuf) -> Self {
        Self {
            agere_home,
            external_agent_home,
        }
    ...
    async fn detect_migrations(
        ...
        let target_config = repo_root.map_or_else(
            || self.agere_home.join("config.toml"),
            |repo_root| repo_root.join(".openagere").join("config.toml"),
        );
        ...
        let target_hooks = repo_root.map_or_else(
            || self.agere_home.join("hooks.json"),
            |repo_root| repo_root.join(".openagere").join("hooks.json"),
        );
        ...
        let target_subagents = repo_root.map_or_else(
            || self.agere_home.join("agents"),
            |repo_root| repo_root.join(".openagere").join("agents"),
        );
        ...
        let target_agents_md = repo_root.map_or_else(
            || self.agere_home.join("AGENTS.md"),
        ...
        if let Some(settings) = settings.as_ref() {
            match ConfigBuilder::default()
                ...
                .fallback_cwd(Some(self.agere_home.clone()))
                .build()
                .await
            {
                Ok(config) => {
                    ...
                    let configured_marketplace_plugins = configured_marketplace_plugins(
                        &config,
                        &PluginsManager::new(self.agere_home.clone()),
                    )?;
    ...
    pub(crate) async fn import_plugins(
        ...
        let plugins_manager = PluginsManager::new(self.agere_home.clone());
        for plugin_group in plugins {
            ...
            let add_marketplace_outcome = add_marketplace(self.agere_home.clone(), request).await;
    ...
    fn import_subagents(&self, cwd: Option<&Path>) -> io::Result<usize> {
        let (source_agents, target_agents) = if let Some(repo_root) = find_repo_root(cwd)? {
            (
                ...
                repo_root.join(".openagere").join("agents"),
            )
        } else if cwd.is_some_and(|cwd| !cwd.as_os_str().is_empty()) {
            return Ok(0);
        } else {
        ...
        let (source_external_agent_dir, target_hooks) =
            if let Some(repo_root) = find_repo_root(cwd)? {
                (
                    ...
                    repo_root.join(".openagere").join("hooks.json"),
                )
            } else if cwd.is_some_and(|cwd| !cwd.as_os_str().is_empty()) {
                return Ok(());
            } else {
        ...
        let plugins = marketplace
            ...
            .filter(|plugin| {
                plugin
                    ...
                    .products
                    .as_deref()
                    .is_none_or(|products| Product::Agere.matches_product_restriction(products))
            })
...
fn rewrite_external_agent_terms(content: &str) -> String {
    ...
    for from in [
        ...
        rewritten = replace_case_insensitive_with_boundaries(&rewritten, from, "Agere");
    }
    rewritten
}
...
fn emit_migration_metric(
    ...
    let Some(metrics) = agere_otel::global() else {
        return;
    };
...
// Stub implementations for functions previously provided by deleted crates.

fn build_mcp_config_from_external(
    _source_root: &Path,
    _external_agent_home: Option<&Path>,
    _mcp_settings: Option<&JsonValue>,
) -> io::Result<TomlValue> {
    Ok(TomlValue::Table(Default::default()))
}

fn hook_migration_event_names(
    _source_external_agent_dir: &Path,
    _target_hooks: &Path,
) -> io::Result<Vec<String>> {
    Ok(Vec::new())
}

fn import_hooks(_source_external_agent_dir: &Path, _target_hooks: &Path) -> io::Result<()> {
    Ok(())
}

fn count_missing_commands(_source_commands: &Path, _target_skills: &Path) -> io::Result<usize> {
    Ok(0)
}
...
fn import_commands(_source_commands: &Path, _target_skills: &Path) -> io::Result<usize> {
    Ok(0)
}

fn count_missing_subagents(
    _source_subagents: &Path,
    _target_subagents: &Path,
) -> io::Result<usize> {
    Ok(0)
}
...
fn import_subagents(_source_agents: &Path, _target_agents: &Path) -> io::Result<usize> {
    Ok(0)
}
