...
"#,
    ...
    std::fs::write(agere_home.join("config.toml"), config_contents)?;

    let AgereCliOutput { exit_code, output } = run_agere_cli(agere_home, cwd).await?;
    assert_ne!(0, exit_code, "Agere CLI should exit nonzero.");
    assert!(
        output.contains("ERROR: Failed to initialize agere:"),
...
struct AgereCliOutput {
    exit_code: i32,
    output: String,
}

async fn run_agere_cli(
    agere_home: impl AsRef<Path>,
    cwd: impl AsRef<Path>,
) -> anyhow::Result<AgereCliOutput> {
    let agere_cli = agere_utils_cargo_bin::cargo_bin("agere")?;
    let mut env = HashMap::new();
    env.insert(
        "AGERE_HOME".to_string(),
        agere_home.as_ref().display().to_string(),
    );

    let args = vec!["-c".to_string(), "analytics.enabled=false".to_string()];
    let spawned = agere_utils_pty::spawn_pty_process(
        agere_cli.to_string_lossy().as_ref(),
        &args,
        cwd.as_ref(),
        &env,
        &None,
    ...
    let exit_code = match exit_code_result {
        ...
        Err(_) => {
            session.terminate();
            anyhow::bail!("timed out waiting for agere CLI to exit");
        }
    };
