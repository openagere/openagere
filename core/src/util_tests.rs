...
use agere_feedback::emit_feedback_request_tags;
...
fn emit_feedback_request_tags_records_sentry_feedback_fields() {
    ...
    assert_eq!(
        tags.get("auth_env_agere_api_key_present")
            .map(String::as_str),
        Some("false")
    );
    assert_eq!(
        tags.get("auth_env_agere_api_key_enabled")
            .map(String::as_str),
        Some("true")
    );
    assert_eq!(
...
fn emit_feedback_request_tags_preserves_auth_env_fields_for_legacy_emitters() {
    ...
    let auth_env = AuthEnvTelemetry {
        openai_api_key_env_present: true,
        agere_api_key_env_present: true,
        agere_api_key_env_enabled: true,
        provider_env_key_name: Some("configured".to_string()),
        provider_env_key_present: Some(true),
    };
...
fn resume_command_prefers_name_over_id() {
    ...
    assert_eq!(command, Some("openagere resume my-thread".to_string()));
}
...
fn resume_command_quotes_thread_name_when_needed() {
    ...
    assert_eq!(
        command,
        Some("openagere resume -- -starts-with-dash".to_string())
    );

    let command = resume_command(Some("two words"), /*thread_id*/ None);
    assert_eq!(command, Some("openagere resume 'two words'".to_string()));

    let command = resume_command(Some("quote'case"), /*thread_id*/ None);
    assert_eq!(command, Some("openagere resume \"quote'case\"".to_string()));
}
