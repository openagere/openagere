...
pub(crate) fn maybe_init(config: &Config) {
    let enabled = std::env::var("AGERE_TUI_RECORD_SESSION")
    ...
    let path = if let Ok(path) = std::env::var("AGERE_TUI_SESSION_LOG_PATH") {
        PathBuf::from(path)
    } else {
    ...
    let header = json!({
        ...
        "model_provider_wire_api": config.model_provider.wire_api.to_string(),
    });
