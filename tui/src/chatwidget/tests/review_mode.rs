...
#[tokio::test]
async fn review_without_token_update_restores_context_window_indicator() {
    let (mut chat, mut rx, _ops) = make_chatwidget_manual(/*model_override*/ None).await;
...
#[tokio::test]
async fn review_token_update_survives_review_exit() {
    let (mut chat, mut rx, _ops) = make_chatwidget_manual(/*model_override*/ None).await;
...
async fn custom_prompt_enter_empty_does_not_send() {
    ...
    // No AppEvent::AgereOp should be sent
...
async fn interrupted_turn_after_goal_budget_limited_uses_budget_message_snapshot() {
    ...
    chat.handle_server_notification(
        agere_app_server_protocol::ServerNotification::TurnStarted(
            agere_app_server_protocol::TurnStartedNotification {
                thread_id: "thread-1".to_string(),
                turn: agere_app_server_protocol::Turn {
                    id: "turn-1".to_string(),
                    items: Vec::new(),
                    status: agere_app_server_protocol::TurnStatus::InProgress,
                    error: None,
                    started_at: None,
                    completed_at: None,
                    duration_ms: None,
    ...
    chat.handle_server_notification(
        agere_app_server_protocol::ServerNotification::ThreadGoalUpdated(
            agere_app_server_protocol::ThreadGoalUpdatedNotification {
                thread_id: "thread-1".to_string(),
                turn_id: Some("turn-1".to_string()),
                goal: agere_app_server_protocol::ThreadGoal {
                    thread_id: "thread-1".to_string(),
                    objective: "Run until the token budget is limited".to_string(),
                    status: agere_app_server_protocol::ThreadGoalStatus::BudgetLimited,
        ...
        agere_app_server_protocol::ServerNotification::TurnCompleted(
            agere_app_server_protocol::TurnCompletedNotification {
                ...
                turn: agere_app_server_protocol::Turn {
                    id: "turn-1".to_string(),
                    items: Vec::new(),
                    status: agere_app_server_protocol::TurnStatus::Interrupted,
                    error: None,
                    started_at: None,
                    completed_at: None,
                    duration_ms: None,
