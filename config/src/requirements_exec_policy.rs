...
use agere_execpolicy::RuleRef;
use agere_execpolicy::rule::PatternToken;
use agere_execpolicy::rule::PrefixPattern;
use agere_execpolicy::rule::PrefixRule;
...
pub enum RequirementsExecPolicyParseError {
    ...
    #[error(
        "rules prefix_rule at index {rule_index} has decision 'allow', which is not permitted in requirements.toml: Agere merges these rules with other config and uses the most restrictive result (use 'prompt' or 'forbidden')"
    )]
...
impl RequirementsExecPolicyToml {
    ...
    /// representation used by `agere-execpolicy`.
