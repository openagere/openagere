...
use agere_core::config::set_project_trust_level;
...
fn write_plugins_enabled_config(agere_home: &std::path::Path) -> std::io::Result<()> {
    std::fs::write(
...
async fn plugin_list_skips_invalid_marketplace_file_and_reports_error() -> Result<()> {
    ...
    write_plugins_enabled_config(agere_home.path())?;
    let marketplace_path =
        AbsolutePathBuf::try_from(repo_root.path().join(".agents/plugins/marketplace.json"))?;
    std::fs::write(marketplace_path.as_path(), "{not json")?;

    let home = agere_home.path().to_string_lossy().into_owned();
...
async fn plugin_list_keeps_valid_marketplaces_when_another_marketplace_fails_to_load() -> Result<()>
{
    ...
    std::fs::create_dir_all(
        valid_repo_root
            .path()
            .join("plugins/valid-plugin/.agere-plugin"),
    )?;
  ...
  "plugins": [
    ...
    std::fs::write(
        valid_repo_root
            .path()
            .join("plugins/valid-plugin/.agere-plugin/plugin.json"),
        r#"{"name":"valid-plugin"}"#,
    )?;
...
#[tokio::test]
#[ignore]
async fn plugin_list_returns_empty_when_workspace_agere_plugins_disabled() -> Result<()> {
    let agere_home = TempDir::new()?;
    let repo_root = TempDir::new()?;
...
#[tokio::test]
#[ignore]
async fn plugin_list_reuses_cached_workspace_agere_plugins_setting() -> Result<()> {
    let agere_home = TempDir::new()?;
    let repo_root = TempDir::new()?;
    let server = MockServer::start().await;
    std::fs::create_dir_all(repo_root.path().join(".git"))?;
    std::fs::create_dir_all(repo_root.path().join(".agents/plugins"))?;
    std::fs::create_dir_all(repo_root.path().join("demo-plugin/.agere-plugin"))?;
    write_plugins_enabled_config_with_base_url(
        agere_home.path(),
        &format!("{}/backend-api/", server.uri()),
    )?;
  ...
  "plugins": [
    ...
    std::fs::write(
        repo_root
            .path()
            .join("demo-plugin/.agere-plugin/plugin.json"),
        r#"{"name":"demo-plugin"}"#,
    )?;
        ...
        vec![PluginMarketplaceEntry {
            ...
            plugins: vec![
                PluginSummary {
                    ...
                    install_policy: PluginInstallPolicy::Available,
                    auth_policy: PluginAuthPolicy::OnInstall,
                    interface: Some(agere_app_server_protocol::PluginInterface {
...
async fn plugin_list_accepts_omitted_cwds() -> Result<()> {
    let agere_home = TempDir::new()?;
    std::fs::create_dir_all(agere_home.path().join(".agents/plugins"))?;
    write_plugins_enabled_config(agere_home.path())?;
    std::fs::write(
        agere_home.path().join(".agents/plugins/marketplace.json"),
        r#"{
  "name": "agere-curated",
  "plugins": [
    {
...
async fn plugin_list_includes_install_and_enabled_state_from_config() -> Result<()> {
    ...
    std::fs::create_dir_all(repo_root.path().join(".git"))?;
    std::fs::create_dir_all(repo_root.path().join(".agents/plugins"))?;
    write_installed_plugin(&agere_home, "agere-curated", "enabled-plugin")?;
    write_installed_plugin(&agere_home, "agere-curated", "disabled-plugin")?;
    std::fs::write(
        repo_root.path().join(".agents/plugins/marketplace.json"),
        r#"{
  "name": "agere-curated",
...
[plugins."enabled-plugin@agere-curated"]
enabled = true

[plugins."disabled-plugin@agere-curated"]
enabled = false
"#,
    )?;
    ...
    assert_eq!(marketplace.name, "agere-curated");
    assert_eq!(
        marketplace
            .interface
            .as_ref()
    ...
    assert_eq!(marketplace.plugins[0].id, "enabled-plugin@agere-curated");
    ...
    assert_eq!(marketplace.plugins[1].id, "disabled-plugin@agere-curated");
    ...
    assert_eq!(
        ...
        "uninstalled-plugin@agere-curated"
    );
...
async fn plugin_list_uses_home_config_for_enabled_state() -> Result<()> {
    let agere_home = TempDir::new()?;
    std::fs::create_dir_all(agere_home.path().join(".agents/plugins"))?;
    write_installed_plugin(&agere_home, "agere-curated", "shared-plugin")?;
    std::fs::write(
        agere_home.path().join(".agents/plugins/marketplace.json"),
        r#"{
  "name": "agere-curated",
...
[plugins."shared-plugin@agere-curated"]
enabled = true
"#,
    )?;
  ...
  "plugins": [
    ...
    std::fs::create_dir_all(workspace_enabled.path().join(".openagere"))?;
    std::fs::write(
        workspace_enabled.path().join(".openagere/config.toml"),
        r#"[plugins."shared-plugin@agere-curated"]
enabled = false
"#,
    )?;
    ...
    assert_eq!(shared_plugin.id, "shared-plugin@agere-curated");
...
async fn plugin_list_returns_plugin_interface_with_absolute_asset_paths() -> Result<()> {
  ...
  "interface": {
    ...
    assert_eq!(plugin.id, "demo-plugin@agere-curated");
...
async fn app_server_startup_remote_plugin_sync_runs_once() -> Result<()> {
    let agere_home = TempDir::new()?;
    let server = MockServer::start().await;
    write_plugin_sync_config(agere_home.path(), &format!("{}/backend-api/", server.uri()))?;
    write_chatgpt_auth(
        agere_home.path(),
        ChatGptAuthFixture::new("chatgpt-token")
            .account_id("account-123")
    ...
    write_openai_curated_marketplace(agere_home.path(), &["linear"])?;
  ...
  {"id":"1","name":"linear","marketplace_name":"openagere-curated","version":"1.0.0","enabled":true}
    ...
    Mock::given(method("GET"))
        ...
        .and(header("authorization", "Bearer chatgpt-token"))
        .and(header("chatgpt-account-id", "account-123"))
        .respond_with(ResponseTemplate::new(200).set_body_string(r#"["linear@openagere-curated"]"#))
        .mount(&server)
        .await;

    let marker_path = agere_home
        .path()
        .join(STARTUP_REMOTE_PLUGIN_SYNC_MARKER_FILE);

    {
        let mut mcp = McpProcess::new_with_plugin_startup_tasks(agere_home.path()).await?;
        timeout(DEFAULT_TIMEOUT, mcp.initialize()).await??;
        ...
        let curated_marketplace = response
            ...
            .expect("expected openagere-curated marketplace entry");
        assert_eq!(
            curated_marketplace
                .plugins
                .into_iter()
                ...
                .collect::<Vec<_>>(),
            vec![("linear@openagere-curated".to_string(), true, true)]
        );
        wait_for_remote_plugin_request_count(&server, "/plugins/list", /*expected_count*/ 1)
            .await?;
    }
...
async fn plugin_list_remote_marketplace_replaces_local_marketplace_with_same_name() -> Result<()> {
    ...
    let local_plugin_root = agere_home
        .path()
        .join(".agents/plugins/plugins/local-linear/.agere-plugin");
...
async fn plugin_list_fetches_featured_plugin_ids_without_chatgpt_auth() -> Result<()> {
    ...
    let server = MockServer::start().await;
    write_plugin_sync_config(agere_home.path(), &format!("{}/backend-api/", server.uri()))?;
    write_openai_curated_marketplace(agere_home.path(), &["linear", "gmail"])?;
    ...
    assert_eq!(
        ...
        vec!["linear@openagere-curated".to_string()]
    );
    Ok(())
}
...
fn write_plugin_sync_config(agere_home: &std::path::Path, base_url: &str) -> std::io::Result<()> {
    std::fs::write(
        agere_home.join("config.toml"),
        format!(
            r#"
...
fn write_openai_curated_marketplace(
    ...
    let curated_root = agere_home.join(".tmp/plugins");
...
{plugins}
    ...
    for plugin_name in plugin_names {
        let plugin_root = curated_root.join(format!("plugins/{plugin_name}/.agere-plugin"));
        std::fs::create_dir_all(&plugin_root)?;
        std::fs::write(
            plugin_root.join("plugin.json"),
            format!(r#"{{"name":"{plugin_name}"}}"#),
    ...
    std::fs::create_dir_all(agere_home.join(".tmp"))?;
    std::fs::write(
        agere_home.join(".tmp/plugins.sha"),
