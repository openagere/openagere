...
fn listen_unix_socket_accepts_absolute_custom_path() {
    assert_eq!(
        AppServerTransport::from_listen_url("unix:///tmp/agere.sock"),
        Ok(AppServerTransport::UnixSocket {
            socket_path: absolute_path("/tmp/agere.sock")
        })
    );
}
...
fn listen_unix_socket_accepts_relative_custom_path() {
    assert_eq!(
        AppServerTransport::from_listen_url("unix://agere.sock"),
        Ok(AppServerTransport::UnixSocket {
            socket_path: AbsolutePathBuf::relative_to_current_dir("agere.sock")
...
fn default_control_socket_path() -> AbsolutePathBuf {
    ...
    app_server_control_socket_path(&agere_home).expect("default control socket path")
}
