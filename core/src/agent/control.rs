...
pub(crate) struct AgentControl {
    ...
    /// `ThreadManagerState -> AgereThread -> Session -> SessionServices -> ThreadManagerState`.
...
impl AgentControl {
    ...
    async fn spawn_agent_internal(
        ...
        )) = notification_source.as_ref()
        {
            ...
            let thread_config = new_thread.thread.agere.thread_config_snapshot().await;
    ...
    async fn spawn_forked_thread(
    ...
    ) -> AgereResult<crate::thread_manager::NewThread> {
        if options.fork_parent_spawn_call_id.is_none() {
            return Err(AgereErr::Fatal(
        ...
        if let Some(parent_thread) = parent_thread.as_ref() {
            ...
            parent_thread
                ...
                .await;
            parent_thread.agere.session.flush_rollout().await?;
        }
        ...
        let rollout_path = parent_thread
            ...
            .or(find_thread_path_by_id_str(
                config.agere_home.as_path(),
                &parent_thread_id.to_string(),
            )
            .await?)
            .ok_or_else(|| {
                AgereErr::Fatal(format!(
    ...
    async fn resume_single_agent_from_rollout(
        ...
        let inherited_exec_policy = self
            ...
            .await;
        let rollout_path =
            match find_thread_path_by_id_str(config.agere_home.as_path(), &thread_id.to_string())
                .await?
            {
                ...
                .await?
                .ok_or_else(|| AgereErr::ThreadNotFound(thread_id))?,
            };
    ...
    pub(crate) async fn interrupt_agent(&self, agent_id: ThreadId) -> AgereResult<String> {
        let state = self.upgrade()?;
    ...
    async fn handle_thread_request_result(
        ...
        agent_id: ThreadId,
        state: &Arc<ThreadManagerState>,
        result: AgereResult<String>,
    ) -> AgereResult<String> {
        if matches!(result, Err(AgereErr::InternalAgentDied)) {
    ...
    pub(crate) async fn shutdown_live_agent(&self, agent_id: ThreadId) -> AgereResult<String> {
        let state = self.upgrade()?;
        let result = if let Ok(thread) = state.get_thread(agent_id).await {
            thread.agere.session.ensure_rollout_materialized().await;
            thread.agere.session.flush_rollout().await?;
    ...
    pub(crate) async fn close_agent(&self, agent_id: ThreadId) -> AgereResult<String> {
        let state = self.upgrade()?;
    ...
    async fn shutdown_agent_tree(&self, agent_id: ThreadId) -> AgereResult<String> {
        let descendant_ids = self.live_thread_spawn_descendants(agent_id).await?;
        let result = self.shutdown_live_agent(agent_id).await;
        for descendant_id in descendant_ids {
            match self.shutdown_live_agent(descendant_id).await {
                Ok(_) | Err(AgereErr::ThreadNotFound(_)) | Err(AgereErr::InternalAgentDied) => {}
    ...
    pub(crate) async fn resolve_agent_reference(
        ...
        let agent_path = current_agent_path
            ...
            .map_err(AgereErr::UnsupportedOperation)?;
        if let Some(thread_id) = self.state.agent_id_for_path(&agent_path) {
            return Ok(thread_id);
        }
        Err(AgereErr::UnsupportedOperation(format!(
    ...
    pub(crate) async fn subscribe_status(
        &self,
        agent_id: ThreadId,
    ) -> AgereResult<watch::Receiver<AgentStatus>> {
        let state = self.upgrade()?;
    ...
    ) -> AgereResult<(SessionSource, AgentMetadata)> {
        if depth == 1 {
    ...
    fn upgrade(&self) -> AgereResult<Arc<ThreadManagerState>> {
        self.manager
            .upgrade()
            .ok_or_else(|| AgereErr::UnsupportedOperation("thread manager dropped".to_string()))
    }
    ...
    async fn inherited_shell_snapshot_for_source(
        ...
        let parent_thread = state.get_thread(*parent_thread_id).await.ok()?;
        parent_thread.agere.session.user_shell().shell_snapshot()
    }
    ...
    async fn inherited_exec_policy_for_source(
        ...
        let parent_thread = state.get_thread(*parent_thread_id).await.ok()?;
        let parent_config = parent_thread.agere.session.get_config().await;
        if !crate::exec_policy::child_uses_parent_exec_policy(&parent_config, child_config) {
            return None;
        }

        Some(Arc::clone(
            &parent_thread.agere.session.services.exec_policy,
        ))
    }
    ...
    async fn open_thread_spawn_children(
        &self,
        parent_thread_id: ThreadId,
    ) -> AgereResult<Vec<(ThreadId, AgentMetadata)>> {
    ...
    async fn live_thread_spawn_children(
        &self,
    ) -> AgereResult<HashMap<ThreadId, Vec<(ThreadId, AgentMetadata)>>> {
        let state = self.upgrade()?;
