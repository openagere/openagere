...
`agere-network-proxy` is Agere's local network policy enforcement proxy. It runs:
...
`agere-network-proxy` reads from Agere's merged `config.toml` (via `agere-core` config loading).
...
mitm = false
# CA cert/key are managed internally under $AGERE_HOME/proxy/ (ca.pem + ca.key).
...
```bash
cargo run -p agere-network-proxy --
```
...
`agere-network-proxy` can be embedded as a library with a thin API:

```rust
use agere_network_proxy::{NetworkProxy, NetworkDecision, NetworkPolicyRequest};
...
When `agere-network-proxy` is embedded in managed Agere runtime, policy decisions emit structured
OTEL-compatible events with `target=agere_otel.network_proxy`.

Event name:

- `agere.network_proxy.policy_decision`
...
This section documents the protections implemented by `agere-network-proxy`, and the boundaries of
