...
async fn request_body_is_zstd_compressed_for_agere_backend_when_enabled() -> anyhow::Result<()> {
    ...
    let base_url = format!("{}/backend-api/agere/v1", server.uri());
    ...
    let agere = builder.build(&server).await?.agere;
