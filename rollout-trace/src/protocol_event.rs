//! Mapping from Agere protocol events into raw rollout-trace events.
//!
...
//! `agere-core`: this module translates the protocol surface into the smaller
//! trace vocabulary and keeps the mapping isolated inside `agere-rollout-trace`.
//!
...
use agere_protocol::protocol::PatchApplyStatus;
use agere_protocol::protocol::TurnAbortReason;
use serde::Serialize;

use crate::AgentThreadId;
use crate::AgereTurnId;
use crate::ExecutionStatus;
use crate::RawTraceEventPayload;

pub(crate) struct AgereTurnTraceEvent {
    pub context_turn_id: AgereTurnId,
    pub payload: RawTraceEventPayload,
}

pub(crate) fn agere_turn_trace_event(
    thread_id: AgentThreadId,
    default_turn_id: &str,
    event: &EventMsg,
) -> Option<AgereTurnTraceEvent> {
    match event {
        EventMsg::TurnStarted(event) => {
            let agere_turn_id = event.turn_id.clone();
            Some(AgereTurnTraceEvent {
                context_turn_id: agere_turn_id.clone(),
                payload: RawTraceEventPayload::AgereTurnStarted {
                    agere_turn_id,
                    thread_id,
                },
            })
        ...
        EventMsg::TurnComplete(event) => {
            ...
            Some(AgereTurnTraceEvent {
                context_turn_id: agere_turn_id.clone(),
                payload: RawTraceEventPayload::AgereTurnEnded {
                    agere_turn_id,
...
pub(crate) enum ToolRuntimePayload<'a> {
    ...
    CollabAgentSpawnBegin(&'a agere_protocol::protocol::CollabAgentSpawnBeginEvent),
    CollabAgentSpawnEnd(&'a agere_protocol::protocol::CollabAgentSpawnEndEvent),
    CollabAgentInteractionBegin(&'a agere_protocol::protocol::CollabAgentInteractionBeginEvent),
    CollabAgentInteractionEnd(&'a agere_protocol::protocol::CollabAgentInteractionEndEvent),
    CollabWaitingBegin(&'a agere_protocol::protocol::CollabWaitingBeginEvent),
    CollabWaitingEnd(&'a agere_protocol::protocol::CollabWaitingEndEvent),
    CollabCloseBegin(&'a agere_protocol::protocol::CollabCloseBeginEvent),
    CollabCloseEnd(&'a agere_protocol::protocol::CollabCloseEndEvent),
}
