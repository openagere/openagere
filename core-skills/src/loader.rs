...
use agere_config::default_project_root_markers;
use agere_config::merge_toml_values;
use agere_config::project_root_markers_from_config;
...
use agere_utils_plugins_placeholder::plugin_namespace_for_skill_path;
use dirs::home_dir;
    ...
    for layer in config_layer_stack.get_layers(
        ...
        match &layer.name {
            ...
            ConfigLayerSource::User { .. } => {
                // Deprecated user skills location (`$AGERE_HOME/skills`), kept for backward
                // compatibility.
                roots.push(SkillRoot {
                    path: config_folder.join(SKILLS_DIR_NAME),
                    scope: SkillScope::User,
                ...
                // Embedded system skills are cached under `$AGERE_HOME/skills/.system` and are a
            ...
            ConfigLayerSource::System { .. } => {
                // The system config layer lives under `/etc/agere/` on Unix, so treat
                // `/etc/agere/skills` as admin-scoped skills.
                roots.push(SkillRoot {
                    path: config_folder.join(SKILLS_DIR_NAME),
...
async fn repo_agents_skill_roots(
    ...
    for dir in dirs {
        ...
        match fs.get_metadata(&agents_skills).await {
...
async fn find_project_root(
    ...
    for ancestor in cwd.ancestors() {
        for marker in project_root_markers {
            ...
            match fs.get_metadata(&marker_path).await {
...
async fn discover_skills_under_root(
    ...
    match fs.get_metadata(&root).await {
    ...
    // Follow symlinked directories for user, admin, and repo skills. System skills are written by Agere itself.
    ...
    while let Some((dir, depth)) = queue.pop_front() {
        let entries = match fs.read_directory(&dir).await {
            Ok(entries) => entries,
            Err(e) => {
        ...
        for entry in entries {
            ...
            let metadata = match fs.get_metadata(&path).await {
            ...
            if metadata.is_symlink {
                ...
                match fs.read_directory(&path).await {
                    Ok(_) => {
...
async fn load_skill_metadata(
    ...
    match fs.get_metadata(&metadata_path).await {
    ...
    let contents = match fs.read_file_text(&metadata_path).await {
