...
BIN_PATH="$BIN_DIR/openagere"
CODEX_HOME_DIR="${CODEX_HOME:-$HOME/.openagere}"
...
download_file() {
  ...
  echo "curl or wget is required to install Agere." >&2
  exit 1
}
...
release_url_for_asset() {
  ...
  printf 'https://github.com/openagere/openagere/releases/download/rust-v%s/%s\n' "$resolved_version" "$asset"
}

release_metadata_url() {
  resolved_version="$1"

  printf 'https://api.github.com/repos/openai/agere/releases/tags/rust-v%s\n' "$resolved_version"
}
...
file_sha256() {
  ...
  echo "sha256sum, shasum, or openssl is required to verify the Agere download." >&2
  exit 1
}
...
verify_archive_digest() {
  ...
  if [ "$actual_digest" != "$expected_digest" ]; then
    echo "Downloaded Agere archive checksum did not match release metadata." >&2
...
require_command() {
  if ! command -v "$1" >/dev/null 2>&1; then
    echo "$1 is required to install Agere." >&2
    exit 1
  fi
}
...
resolve_version() {
  ...
  release_json="$(download_text "https://api.github.com/repos/openai/agere/releases/latest")"
  resolved="$(printf '%s\n' "$release_json" | sed -n 's/.*"tag_name":[[:space:]]*"rust-v\([^"]*\)".*/\1/p' | head -n 1)"

  if [ -z "$resolved" ]; then
    echo "Failed to resolve the latest Agere release version." >&2
    exit 1
  fi
...
add_to_path() {
  ...
  begin_marker="# >>> Agere installer >>>"
  end_marker="# <<< Agere installer <<<"
...
cleanup_stale_install_artifacts() {
  ...
  if [ -d "$BIN_DIR" ]; then
    find "$BIN_DIR" -mindepth 1 -maxdepth 1 -name '.openagere.*' -exec rm -f {} +
  fi
}
...
version_from_binary() {
  ...
  if [ ! -x "$agere_path" ]; then
    return 1
  fi

  "$agere_path" --version 2>/dev/null | sed -n 's/.* \([0-9][0-9A-Za-z.+-]*\)$/\1/p' | head -n 1
}

current_installed_version() {
  version="$(version_from_binary "$CURRENT_LINK/bin/openagere" || true)"
  if [ -n "$version" ]; then
    printf '%s\n' "$version"
    return 0
  fi
...
resolve_existing_agere() {
  command -v openagere 2>/dev/null || true
}

classify_existing_agere() {
  existing_path="$1"
...
print_launch_instructions() {
  case "$path_action" in
    added)
      step "Current terminal: export PATH=\"$BIN_DIR:\$PATH\" && openagere"
      step "Future terminals: open a new terminal and run: openagere"
    ...
    configured)
      ...
      ;;
    *)
      step "Current terminal: openagere"
      step "Future terminals: open a new terminal and run: openagere"
      ;;
  esac
}

maybe_launch_agere_now() {
  if prompt_yes_no "Start Agere now?"; then
    step "Launching Agere"
    "$BIN_PATH"
  fi
}

detect_conflicting_install() {
  existing_path="$(resolve_existing_agere)"
  manager="$(classify_existing_agere "$existing_path" || true)"
  ...
  step "Detected existing $manager-managed Agere at $existing_path"
  warn "Multiple managed Agere installs can be ambiguous because PATH order decides which one runs."
}
...
handle_conflicting_install() {
  ...
  case "$conflict_manager" in
    brew)
      uninstall_cmd="brew uninstall --cask agere"
      ;;
    bun)
      uninstall_cmd="bun remove -g openagere"
      ;;
    *)
      uninstall_cmd="npm uninstall -g openagere"
      ;;
  esac

  if prompt_yes_no "Uninstall the existing $conflict_manager-managed Agere now?"; then
    step "Running: $uninstall_cmd"
    if ! sh -c "$uninstall_cmd"; then
      warn "Failed to uninstall the existing $conflict_manager-managed Agere. Continuing with the standalone install."
    fi
  else
    warn "Leaving the existing $conflict_manager-managed Agere installed. PATH order will determine which openagere runs."
  fi
}
...
install_release() {
  ...
  cp "$vendor_root/agere/openagere" "$stage_release/bin/openagere"
  chmod 0755 "$stage_release/bin/openagere"
...
update_visible_command() {
  mkdir -p "$BIN_DIR"
  tmp_link="$BIN_DIR/.openagere.$$"

  replace_path_with_symlink "$BIN_PATH" "$CURRENT_LINK/bin/openagere" "$tmp_link"
}
...
asset="openagere-npm-$npm_tag-$resolved_version.tgz"
...
if [ -n "$current_version" ] && [ "$current_version" != "$resolved_version" ]; then
  step "Updating Agere CLI from $current_version to $resolved_version"
elif [ -n "$current_version" ]; then
  step "Updating Agere CLI"
else
  step "Installing Agere CLI"
fi
...
printf 'Agere CLI %s installed successfully.\n' "$resolved_version"
maybe_launch_agere_now
