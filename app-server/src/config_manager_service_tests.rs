...
async fn write_value_preserves_comments_and_order() -> Result<()> {
    ...
    let original = r#"# Agere user configuration
model = "gpt-5.2"
...
"#;
    ...
    let expected = r#"# Agere user configuration
model = "gpt-5.2"
approval_policy = "on-request"
...
// Note: Cloud requirements validation was removed from ConfigManager.
// These tests are disabled until a replacement validation mechanism is added.

#[tokio::test]
async fn write_value_rejects_feature_requirement_conflict_disabled() {}

#[tokio::test]
async fn write_value_rejects_profile_feature_requirement_conflict_disabled() {}
