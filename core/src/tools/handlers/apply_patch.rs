...
use crate::access_policy_transforms::merge_permission_profiles;
use crate::access_policy_transforms::normalize_additional_permissions;
...
use agere_apply_patch::ApplyPatchArgs;
use agere_apply_patch::ApplyPatchFileChange;
use agere_apply_patch::Hunk;
use agere_apply_patch::parse_patch_streaming;
...
use agere_protocol::protocol::PatchApplyUpdatedEvent;
use agere_tools::ApplyPatchToolArgs;
...
fn format_update_chunks_for_progress(chunks: &[agere_apply_patch::UpdateFileChunk]) -> String {
...
fn write_permissions_for_paths(
    ...
    file_system_access_policy: &agere_protocol::permissions::FileSystemAccessPolicy,
    cwd: &AbsolutePathBuf,
    ...
    let write_paths = file_paths
        ...
        .filter(|path| {
            !file_system_access_policy.can_write_path_with_cwd(path.as_path(), cwd.as_path())
        })
        .collect::<BTreeSet<_>>()
        .into_iter()
...
async fn effective_patch_permissions(
    ...
    agere_protocol::permissions::FileSystemAccessPolicy,
) {
    ...
    let base_file_system_access_policy = turn.file_system_access_policy();
    ...
    let effective_additional_permissions = apply_granted_turn_permissions(
        ...
        write_permissions_for_paths(&file_paths, &file_system_access_policy, &turn.cwd),
    )
    .await;
...
impl ToolHandler for ApplyPatchHandler {
    ...
    async fn handle(&self, invocation: ToolInvocation) -> Result<Self::Output, FunctionCallError> {
        ...
        let fs = turn.local_filesystem();
        match agere_apply_patch::maybe_parse_apply_patch_verified(&command, &cwd, fs.as_ref()).await
        {
            agere_apply_patch::MaybeApplyPatchVerified::Body(changes) => {
                let (file_paths, effective_additional_permissions, file_system_access_policy) =
                    effective_patch_permissions(session.as_ref(), turn.as_ref(), &changes).await;
                match apply_patch::apply_patch(turn.as_ref(), &file_system_access_policy, changes)
                    .await
                {
            ...
            agere_apply_patch::MaybeApplyPatchVerified::CorrectnessError(parse_error) => {
                Err(FunctionCallError::RespondToModel(format!(
                    "apply_patch verification failed: {parse_error}"
                )))
            }
            agere_apply_patch::MaybeApplyPatchVerified::ShellParseError(error) => {
                tracing::trace!("Failed to parse apply_patch input, {error:?}");
                Err(FunctionCallError::RespondToModel(
                    "apply_patch handler received invalid patch input".to_string(),
                ))
            }
            agere_apply_patch::MaybeApplyPatchVerified::NotApplyPatch => {
                Err(FunctionCallError::RespondToModel(
...
pub(crate) async fn intercept_apply_patch(
    ...
    match agere_apply_patch::maybe_parse_apply_patch_verified(command, cwd, fs).await {
        agere_apply_patch::MaybeApplyPatchVerified::Body(changes) => {
            session
                ...
                .await;
            let (approval_keys, effective_additional_permissions, file_system_access_policy) =
                effective_patch_permissions(session.as_ref(), turn.as_ref(), &changes).await;
            match apply_patch::apply_patch(turn.as_ref(), &file_system_access_policy, changes).await
            {
                InternalApplyPatchInvocation::Output(item) => {
                    let content = item?;
        ...
        agere_apply_patch::MaybeApplyPatchVerified::NotApplyPatch => Ok(None),
    }
}
