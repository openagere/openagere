...
impl ChatWidget {
    ...
    /// Clears the terminal title Agere most recently wrote, if any.
    ///
        ...
        self.config
            ...
            .find_map(|layer| match &layer.name {
                ConfigLayerSource::Project { dot_agere_folder } => {
                    dot_agere_folder.as_path().parent().map(Path::to_path_buf)
                }
                _ => None,
            })
    }
    ...
    pub(super) fn status_line_value_for_item(&mut self, item: &StatusLineItem) -> Option<String> {
        match item {
            ...
            StatusLineItem::AgereVersion => Some(AGERE_CLI_VERSION.to_string()),
    ...
    pub(super) fn status_surface_preview_value_for_item(
        ...
        let status_line_item = match item {
            StatusSurfacePreviewItem::AppName => return Some("agere".to_string()),
            ...
            StatusSurfacePreviewItem::AgereVersion => StatusLineItem::AgereVersion,
    ...
    pub(super) fn terminal_title_value_for_item(
        ...
        match item {
            TerminalTitleItem::AppName => Some("agere".to_string()),
            ...
            TerminalTitleItem::WeeklyLimit => self
                ...
                .map(|value| Self::truncate_terminal_title_part(value, /*max_chars*/ 32)),
            TerminalTitleItem::AgereVersion => self
                .status_line_value_for_item(&StatusLineItem::AgereVersion)
                .map(|value| Self::truncate_terminal_title_part(value, /*max_chars*/ 32)),
    ...
    fn model_with_reasoning_display_name(&self) -> String {
        let wire_api = self.config.model_provider.wire_api;
        let label =
            Self::status_line_reasoning_effort_label(self.effective_reasoning_effort(), wire_api);
        let fast_label =
