...
use agere_exec_server::HttpRequestParams;
use agere_login::AgereAuth;
use agere_mcp::ExecutionState;
use agere_mcp::MCP_EXECUTION_STATE_META_CAPABILITY;
...
use agere_utils_cargo_bin::cargo_bin;
...
fn read_only_user_turn(fixture: &TestAgere, text: impl Into<String>) -> Op {
...
fn read_only_user_turn_with_model(
    ...
    let permission_profile = turn_permission_fields(PermissionProfile::read_only(), cwd.as_path());
    Op::UserTurn {
...
fn unique_remote_path(binary_name: &str) -> anyhow::Result<String> {
    ...
    Ok(format!(
        "/tmp/agere-remote-env/{binary_name}-{}-{unique_suffix}",
        std::process::id()
    ))
}
...
async fn wait_for_mcp_tool(fixture: &TestAgere, tool_name: &str) -> anyhow::Result<()> {
    let tools_ready_deadline = Instant::now() + Duration::from_secs(30);
    loop {
        fixture.agere.submit(Op::ListMcpTools).await?;
...
async fn call_cwd_tool(
    ...
    wait_for_event(&fixture.agere, |ev| {
        matches!(ev, EventMsg::McpToolCallBegin(_))
    })
    .await;
    let end_event = wait_for_event(&fixture.agere, |ev| {
    ...
    wait_for_event(&fixture.agere, |ev| matches!(ev, EventMsg::TurnComplete(_))).await;
...
async fn stdio_server_round_trip() -> anyhow::Result<()> {
    ...
    let begin_event = wait_for_event(&fixture.agere, |ev| {
        matches!(ev, EventMsg::McpToolCallBegin(_))
    })
    .await;
...
#[tokio::test(flavor = "multi_thread", worker_threads = 1)]
async fn stdio_mcp_tool_call_includes_execution_state_meta() -> anyhow::Result<()> {
    skip_if_no_network!(Ok(()));

    let server = responses::start_mock_server().await;

    let call_id = "execution-state-meta-call";
    let server_name = "rmcp";
    let namespace = format!("mcp__{server_name}__");
    let tool_name = format!("{namespace}execution_state_meta");
    ...
    let final_mock = mount_sse_once(
        ...
        responses::sse(vec![
            responses::ev_assistant_message(
                "msg-1",
                "rmcp execution_state_meta completed successfully.",
            ),
            responses::ev_completed("resp-2"),
        ]),
    )
    ...
    fixture
        .submit_turn_with_permission_profile(
            "call the rmcp execution_state_meta tool",
    ...
    assert!(
        request
            .tool_by_name(&namespace, "execution_state_meta")
            .is_some(),
        "direct MCP tool should be sent as a namespace child tool: {:?}",
        request.body_json()
    );
    ...
    let wrapped_payload = split_wall_time_wrapped_output(output_text);
    let output_json: Value = serde_json::from_str(wrapped_payload)
        .expect("wrapped MCP output should preserve execution-state metadata JSON envelope");
    let Value::Object(meta) = output_json else {
        panic!("execution_state_meta should return metadata object: {output_json:?}");
    };

    let permission_profile_used = PermissionProfile::read_only();
    let execution_state = ExecutionState {
        permission_profile: turn_permission_fields(
            permission_profile_used.clone(),
            fixture.config.cwd.as_path(),
        ),
        access_policy: permission_profile_used
            .to_legacy_access_policy(fixture.config.cwd.as_path())
            .expect("legacy access policy"),
        agere_linux_exe: fixture.config.agere_linux_exe.clone(),
        access_cwd: fixture.config.cwd.clone().into_path_buf(),
        use_legacy_landlock: fixture.config.features.use_legacy_landlock(),
    };
    let expected_execution_state_value = serde_json::to_value(&execution_state)?;

    assert_eq!(
        meta.get(MCP_EXECUTION_STATE_META_CAPABILITY).cloned(),
        Some(expected_execution_state_value),
        "unexpected execution-state metadata in structured MCP output keys {meta:?}"
    );
...
async fn stdio_mcp_parallel_tool_calls_default_false_runs_serially() -> anyhow::Result<()> {
    ...
    while call_events.len() < 4 {
        let event = wait_for_event(&fixture.agere, |ev| {
            matches!(
                ev,
        ...
        ModelsResponse {
            models: vec![ModelInfo {
                ...
                supported_reasoning_levels: vec![ReasoningEffortPreset {
                    effort: agere_protocol::openai_models::ReasoningEffort::Medium,
...
const REMOTE_EXEC_SERVER_URL_ENV_VAR: &str = "AGERE_TEST_REMOTE_EXEC_SERVER_URL";
...
impl StreamableHttpTestServer {
    /// Returns the MCP endpoint URL that Agere should connect to.
    fn url(&self) -> &str {
        &self.server_url
    }
...
/// What this tests: Agere can discover and call a Streamable HTTP MCP tool in
...
async fn streamable_http_tool_call_round_trip() -> anyhow::Result<()> {
    ...
    // Phase 1: script the model responses so Agere will call the MCP echo tool
    ...
    // Phase 3: configure Agere with the Streamable HTTP MCP server and build a
    ...
    // Phase 5: assert Agere begins the expected tool invocation.
    let begin_event = wait_for_event(&fixture.agere, |ev| {
        matches!(ev, EventMsg::McpToolCallBegin(_))
    })
    .await;
...
/// This test writes to a fallback credentials file in AGERE_HOME.
/// Ideally, we wouldn't need to serialize the test but it's much more cumbersome to wire AGERE_HOME through the code.
#[test]
#[serial(agere_home)]
...
async fn streamable_http_with_oauth_round_trip_impl() -> anyhow::Result<()> {
    ...
    // Phase 1: script the model responses so Agere will call the OAuth-backed
    ...
    // Phase 3: seed an isolated AGERE_HOME with fallback OAuth tokens for this
    // server so the test does not share credentials with other suite cases.
    let temp_home = Arc::new(tempdir()?);
    let _agere_home_guard = EnvVarGuard::set("AGERE_HOME", temp_home.path().as_os_str());
    ...
    // Phase 4: configure Agere with the OAuth-backed Streamable HTTP MCP
    ...
    // Phase 7: assert Agere begins the expected tool invocation.
    let begin_event = wait_for_event(&fixture.agere, |ev| {
        matches!(ev, EventMsg::McpToolCallBegin(_))
    })
    .await;
...
async fn start_remote_streamable_http_test_server(
    ...
    // remote HTTP before handing the URL to the Agere fixture.
