...
use super::*;
use crate::client_common::OutputItemCompletion;
...
use agere_config::NetworkConstraints;
use agere_config::NetworkDomainPermissionToml;
use agere_config::NetworkDomainPermissionsToml;
use agere_config::RequirementSource;
use agere_config::Sourced;
...
use agere_models_manager::model_info;
use agere_models_manager::test_support::construct_model_info_offline_for_tests;
use agere_models_manager::test_support::get_model_offline_for_tests;
...
use agere_protocol::models::FilesystemIsolation;
...
use agere_protocol::request_permissions::PermissionGrantScope;
...
use agere_config::config_toml::ProjectConfig;
use agere_execpolicy::Decision;
use agere_execpolicy::NetworkRuleProtocol;
use agere_execpolicy::Policy;
use agere_network_proxy::NetworkProxyConfig;
use agere_otel::MetricsClient;
use agere_otel::MetricsConfig;
use agere_otel::THREAD_SKILLS_DESCRIPTION_TRUNCATED_CHARS_METRIC;
use agere_otel::THREAD_SKILLS_ENABLED_TOTAL_METRIC;
use agere_otel::THREAD_SKILLS_KEPT_TOTAL_METRIC;
use agere_otel::THREAD_SKILLS_TRUNCATED_METRIC;
...
use agere_protocol::protocol::CompactedItem;
...
use agere_protocol::protocol::InterAgentCommunication;
...
use agere_protocol::protocol::RealtimeAudioFrame;
use agere_protocol::protocol::RealtimeConversationListVoicesResponseEvent;
use agere_protocol::protocol::RealtimeVoice;
...
use agere_protocol::protocol::SkillScope;
use agere_protocol::protocol::Submission;
use agere_protocol::protocol::ThreadGoalStatus;
use agere_protocol::protocol::ThreadRolledBackEvent;
...
use agere_protocol::protocol::TurnStartedEvent;
...
use agere_protocol::mcp::CallToolResult as McpCallToolResult;
...
fn permission_profile_for_access_policy(access_policy: &str) -> PermissionProfile {
    PermissionProfile::from_legacy_access_policy(access_policy)
}

fn windows_restriction_level_from_config(
    config: &crate::config::Config,
) -> WindowsExecutionRestrictionLevel {
    match config.permissions.windows_access_mode {
        Some(WindowsAccessModeToml::Elevated) => WindowsExecutionRestrictionLevel::Elevated,
...
fn test_session_telemetry_without_metadata() -> SessionTelemetry {
    ...
    let metrics = MetricsClient::new(
        MetricsConfig::in_memory("test", "agere-core", env!("CARGO_PKG_VERSION"), exporter)
            .with_runtime_reader(),
    )
...
fn test_model_client_session() -> crate::client::ModelClientSession {
    crate::client::ModelClient::new(
        ...
        agere_protocol::protocol::SessionSource::Exec,
...
fn write_project_hooks(dot_agere: &Path) -> std::io::Result<()> {
    std::fs::create_dir_all(dot_agere)?;
    std::fs::write(
        dot_agere.join("hooks.json"),
        r#"{
  "hooks": {
    "SessionStart": [
      {
...
async fn write_project_trust_config(
    ...
    tokio::fs::write(
        agere_home.join(agere_config::CONFIG_TOML_FILE),
...
async fn preview_session_start_hooks(
    config: &crate::config::Config,
) -> std::io::Result<Vec<agere_protocol::protocol::HookRunSummary>> {
    ...
    Ok(
        hooks.preview_session_start(&agere_hooks::SessionStartRequest {
            session_id: ThreadId::new(),
            cwd: config.cwd.clone(),
            transcript_path: None,
            model: "gpt-5.2".to_string(),
            permission_mode: "default".to_string(),
            source: agere_hooks::SessionStartSource::Startup,
        }),
    )
}
...
async fn start_managed_network_proxy_applies_execpolicy_network_rules() -> anyhow::Result<()> {
    let spec = crate::config::NetworkProxySpec::from_config_and_constraints(
        ...
        /*requirements*/ None,
        &permission_profile_for_access_policy("workspace-write"),
    )?;
...
async fn managed_network_proxy_decider_survives_full_access_start() -> anyhow::Result<()> {
    let spec = crate::config::NetworkProxySpec::from_config_and_constraints(
        ...
        &permission_profile_for_access_policy("danger-full-access"),
    )?;
    let exec_policy = Policy::empty();
    let decider_calls = Arc::new(std::sync::atomic::AtomicUsize::new(0));
    let network_policy_decider: Arc<dyn agere_network_proxy::NetworkPolicyDecider> = Arc::new({
        let decider_calls = Arc::clone(&decider_calls);
        move |_request| {
            decider_calls.fetch_add(1, std::sync::atomic::Ordering::SeqCst);
            async { agere_network_proxy::NetworkDecision::ask("not_allowed") }
        }
    });
    ...
    let spec = spec.recompute_for_permission_profile(&permission_profile_for_access_policy(
        "workspace-write",
    ))?;
...
#[tokio::test]
async fn new_turn_refreshes_managed_network_proxy_for_access_policy_change() -> anyhow::Result<()> {
    let (mut session, _turn_context) = make_session_and_context().await;
    let initial_policy = "workspace-write".to_string();
    ...
    let spec = crate::config::NetworkProxySpec::from_config_and_constraints(
        network_config,
        Some(requirements),
        &permission_profile_for_access_policy(&initial_policy),
    )?;
    let (started_proxy, _) = Session::start_managed_network_proxy(
        &spec,
        &Policy::empty(),
...

    {
        ...
        config
            .permissions
            .set_legacy_access_policy(initial_policy.clone(), cwd.as_path())
            .expect("test setup should allow legacy access policy");
        state.session_configuration.original_config_do_not_use = Arc::new(config);
        state
            .session_configuration
            .permission_profile
            .set(PermissionProfile::from_legacy_access_policy(
                &initial_policy,
            ))
    ...
    session
        .new_turn_with_sub_id(
            "access-policy-change".to_string(),
            SessionSettingsUpdate {
                permission_profile: Some(PermissionProfile::from_legacy_access_policy(
                    "danger-full-access",
                )),
                ..Default::default()
            },
...
async fn danger_full_access_turns_do_not_expose_managed_network_proxy() -> anyhow::Result<()> {
    ...
    let session = make_session_with_config(move |config| {
        ...
        config
            .permissions
            .set_legacy_access_policy("danger-full-access".to_string(), cwd.as_path())
            .expect("test setup should allow legacy access policy");
...
#[tokio::test]
async fn danger_full_access_tool_attempts_run_without_managed_restrictions() -> anyhow::Result<()> {
    #[derive(Default)]
    struct ProbeToolRuntime {
        run_count: usize,
    }

    impl crate::tools::execution::Approvable<()> for ProbeToolRuntime {
        ...
        fn start_approval_async<'a>(
            &'a mut self,
            _req: &'a (),
            _ctx: crate::tools::execution::ApprovalCtx<'a>,
        ) -> futures::future::BoxFuture<'a, ReviewDecision> {
            Box::pin(async { ReviewDecision::Approved })
        }
    }

    impl crate::tools::execution::ToolRuntime<(), ()> for ProbeToolRuntime {
        async fn run(
            &mut self,
            _req: &(),
            _ctx: &crate::tools::execution::ToolCtx,
        ) -> Result<(), crate::tools::execution::ToolError> {
            self.run_count += 1;
            Ok(())
        }
    }
    ...
    let session = make_session_with_config(move |config| {
        ...
        requirements_toml.network = Some(agere_config::NetworkRequirementsToml {
            enabled: Some(true),
            ..Default::default()
        });
    ...
    let tool_ctx = crate::tools::execution::ToolCtx {
...
async fn workspace_write_turns_continue_to_expose_managed_network_proxy() -> anyhow::Result<()> {
    ...
    let network_spec = crate::config::NetworkProxySpec::from_config_and_constraints(
        ...
        &permission_profile_for_access_policy(&access_policy),
    )?;
    ...
    let session = make_session_with_config(move |config| {
        ...
        config
            .permissions
            .set_legacy_access_policy(access_policy, cwd.as_path())
            .expect("test setup should allow legacy access policy");
        config.permissions.network = Some(network_spec);
    })
    .await?;
...
async fn reload_user_config_layer_updates_effective_apps_config() {
    let (session, _turn_context) = make_session_and_context().await;
    let agere_home = session.agere_home().await;
    std::fs::create_dir_all(&agere_home).expect("create agere home");
    let config_toml_path = agere_home.join(CONFIG_TOML_FILE);
    std::fs::write(
        &config_toml_path,
    ...
    let apps_toml = config
        ...
        .cloned()
        .expect("apps table");
    let apps = agere_config::types::AppsConfigToml::deserialize(apps_toml)
...
"#,
    ...
    assert_eq!(
        ...
        vec![
            ...
            ToolSuggestDisabledTool::plugin("slack@openagere-curated"),
        ]
    );
}
...
fn filter_connectors_for_input_skips_plugin_mentions() {
    ...
    let input = vec![user_message(
        "use [@figma](plugin://figma@openagere-curated)",
    )];
    let explicitly_enabled_connectors = HashSet::new();
    let selected = filter_connectors_for_input(
        &connectors,
...
async fn session_configured_reports_permission_profile_for_external_access() -> anyhow::Result<()> {
    let server = start_mock_server().await;
    let access_policy = "external-access".to_string();
    let expected_access_policy = access_policy.clone();
    let mut builder = test_agere().with_config(move |config| {
        config
            .set_legacy_access_policy(access_policy)
            .expect("set legacy access policy");
        config.permissions.permission_profile =
            agere_config::Constrained::allow_any(PermissionProfile::from_runtime_permissions(
    ...
    let expected_permission_profile =
        agere_protocol::models::PermissionProfile::from_legacy_access_policy(
            &expected_access_policy,
        );
    assert_eq!(
        test.session_configured.permission_profile, expected_permission_profile,
        "External is represented explicitly instead of as a lossy root-write profile"
    );
    Ok(())
}
...
async fn fork_startup_context_then_first_turn_diff_snapshot() -> anyhow::Result<()> {
    ...
    let mut builder = test_agere().with_config(|config| {
        config.permissions.approval_policy =
            agere_config::Constrained::allow_any(AskForApproval::OnRequest);
    });
    ...
    initial
        ...
        .await?;
    wait_for_event(&initial.agere, |ev| matches!(ev, EventMsg::TurnComplete(_))).await;
    // Forking reads the persisted rollout JSONL, so force the completed source turn to disk
    // before snapshotting from it.
    initial.agere.ensure_rollout_materialized().await;
    initial
        .agere
        .flush_rollout()
        .await
    ...
    fork_config.permissions.approval_policy =
        agere_config::Constrained::allow_any(AskForApproval::UnlessTrusted);
    let forked = initial
        .thread_manager
        .fork_thread(
            usize::MAX,
    ...
    settings.bind(|| {
        insta::assert_snapshot!(
            "agere_core__agere_tests__fork_startup_context_then_first_turn_diff",
            snapshot
        );
    });
...
async fn record_initial_history_forked_hydrates_previous_turn_settings() {
    ...
    let rollout_items = vec![
        RolloutItem::EventMsg(EventMsg::TurnStarted(
            agere_protocol::protocol::TurnStartedEvent {
        ...
        RolloutItem::EventMsg(EventMsg::UserMessage(
            agere_protocol::protocol::UserMessageEvent {
        ...
        RolloutItem::EventMsg(EventMsg::TurnComplete(
            agere_protocol::protocol::TurnCompleteEvent {
                turn_id,
...
async fn thread_rollback_fails_without_persisted_thread_history() {
    ...
    assert_eq!(
        error_event.agere_error_info,
        Some(AgereErrorInfo::ThreadRollbackFailed)
    );
...
async fn set_rate_limits_retains_previous_credits() {
    let agere_home = tempfile::tempdir().expect("create temp dir");
    let config = build_test_config(agere_home.path()).await;
    ...
    let session_configuration = SessionConfiguration {
        ...
        windows_execution_restriction_level: windows_restriction_level_from_config(&config),
        cwd: config.cwd.clone(),
        agere_home: config.agere_home.clone(),
        thread_name: None,
    ...
    let initial = RateLimitSnapshot {
        ...
        plan_type: Some(agere_protocol::account::PlanType::Plus),
...
async fn set_rate_limits_updates_plan_type_when_present() {
    ...
    let update = RateLimitSnapshot {
        ...
        secondary: None,
        credits: None,
        plan_type: Some(agere_protocol::account::PlanType::Pro),
        rate_limit_reached_type: None,
    };
    state.set_rate_limits(update.clone());
    ...
    loop {
        ...
        match evt.msg {
            EventMsg::Error(payload)
                if payload.agere_error_info == Some(AgereErrorInfo::ThreadRollbackFailed) =>
            {
                return payload;
            }
            _ => continue,
...
async fn build_test_config(agere_home: &Path) -> Config {
    ConfigBuilder::without_managed_config_for_tests()
        .agere_home(agere_home.to_path_buf())
        .build()
        .await
...
#[tokio::test]
async fn session_settings_provider_update_changes_provider_and_snapshot() {
    let session_configuration = make_session_configuration_for_tests().await;
    let mut next_provider = session_configuration.provider.clone();
    next_provider.base_url = Some("https://next-provider.test/v1".to_string());

    let updated = session_configuration
        .apply(&SessionSettingsUpdate {
            provider: Some(("next-provider".to_string(), next_provider.clone())),
            ..Default::default()
        })
        .expect("provider update should apply");

    assert_eq!(updated.provider, next_provider);
    assert_eq!(
...
#[tokio::test]
async fn provider_switch_refreshes_model_scalars_from_fresh_config() {
    // When a provider switch carries the fresh config it was resolved from, the
    // model-scoped scalars consumed by `with_config_overrides` (context window,
    // auto-compact limit, tool-output limit, reasoning-summary support) must be
    // refreshed on `original_config_do_not_use` so the per-turn config reflects the
    // NEW provider rather than the session-start values.
    let session_configuration = make_session_configuration_for_tests().await;
    let mut fresh = (*session_configuration.original_config_do_not_use).clone();
    fresh.model_context_window = Some(123_456);
    fresh.model_auto_compact_token_limit = Some(99_999);
    ...
    let updated = session_configuration
        .apply(&SessionSettingsUpdate {
            ...
            provider_config: Some(std::sync::Arc::new(fresh)),
            ..Default::default()
        })
        .expect("provider switch should apply");

    assert_eq!(
        updated.original_config_do_not_use.model_context_window,
        Some(123_456)
    );
    assert_eq!(
        updated
    ...
    // The refreshed scalar must flow into the per-turn config used to resolve model
    // metadata, so `to_models_manager_config()` carries the new window.
    let per_turn = Session::build_per_turn_config(&updated, updated.cwd.clone());
    assert_eq!(
        per_turn.to_models_manager_config().model_context_window,
        Some(123_456)
    );
}
...
async fn per_turn_config_uses_updated_session_provider() {
    ...
    assert_eq!(per_turn_config.model_provider_id, "next-provider");
    assert_eq!(per_turn_config.model_provider, next_provider);
}

#[tokio::test]
async fn provider_switch_rebuilds_and_swaps_session_models_manager() {
    // The current model slug used by the offline test config.
    let current_slug = get_model_offline_for_tests(None);
    let current_slug_for_config = current_slug.clone();

    // Build a real session whose config advertises a *smaller* context window
    // (256_000) for the current model slug. The session's initial models
    // manager is built without these config models, so it does NOT report this
    // window yet; only a rebuild that reads `config.models` will.
    ...
    // Construct a new provider for the switch. Cloning the existing provider
    // and tweaking it is sufficient; the rebuilt manager reads context-window
    // metadata from `config.models`, which the switch path re-consults.
    let mut small_provider = {
        let state = session.state.lock().await;
        state.session_configuration.provider.clone()
    };
    small_provider.base_url = Some("https://small-provider.test/v1".to_string());

    session
        .update_settings(SessionSettingsUpdate {
            provider: Some(("small".to_string(), small_provider)),
            ..Default::default()
        })
        .await
        .expect("provider update should apply");
    ...
    let model_info = session
        .services
        .models_manager
        .get_model_info(&current_slug, &config.to_models_manager_config())
        .await;

    assert_eq!(model_info.context_window, Some(256_000));
}

#[tokio::test]
async fn provider_switch_rebuilds_models_manager_from_refreshed_config() {
    let current_slug = get_model_offline_for_tests(None);
    let (session, _turn_context, _rx) = make_session_and_context_with_rx().await;

    let mut refreshed_config = (*session.get_config().await).clone();
    let mut refreshed_catalog = bundled_models_response().expect("bundled catalog should parse");
    let catalog_model = refreshed_catalog
        .models
        .iter_mut()
        .find(|model| model.slug == current_slug)
        .expect("test model should exist in bundled catalog");
    catalog_model.context_window = Some(123_456);
    refreshed_config.model_catalog = Some(refreshed_catalog);
    let refreshed_config = Arc::new(refreshed_config);

    let mut provider = session.provider().await;
    provider.base_url = Some("https://refreshed-provider.test/v1".to_string());
    session
        .apply_provider_to_runtime(provider, Some(Arc::clone(&refreshed_config)))
        .await;

    let model_info = session
        .services
        .models_manager
        .get_model_info(&current_slug, &refreshed_config.to_models_manager_config())
        .await;
...
#[tokio::test]
async fn update_settings_provider_switch_rebuilds_models_manager_from_refreshed_config() {
    let current_slug = get_model_offline_for_tests(None);
    let (session, _turn_context, _rx) = make_session_and_context_with_rx().await;
    ...
    session
        .update_settings(SessionSettingsUpdate {
            provider: Some(("refreshed-provider".to_string(), provider)),
            provider_config: Some(Arc::clone(&refreshed_config)),
            ..Default::default()
        })
        .await
        .expect("provider update should apply");
...
#[tokio::test]
async fn provider_switch_turn_context_records_provider_transition() {
    let (session, turn_context, _rx) = make_session_and_context_with_rx().await;
    let mut provider = session.provider().await;
    provider.base_url = Some("https://provider-transition.test/v1".to_string());

    let switched = session
        .new_turn_with_sub_id(
            "sub-provider-switch".to_string(),
            SessionSettingsUpdate {
                provider: Some(("provider-transition".to_string(), provider)),
                ..Default::default()
            },
        )
        .await
        .expect("provider switch turn should be created");
    assert!(switched.provider_changed_for_turn);

    let unchanged = session
        .new_turn_with_sub_id(
            "sub-same-provider".to_string(),
            SessionSettingsUpdate {
        ...
        .await
        .expect("same-provider turn should be created");
    assert!(!unchanged.provider_changed_for_turn);
}

#[tokio::test]
async fn provider_downshift_precompact_does_not_emit_synthetic_turn_lifecycle() {
    let server = start_mock_server().await;
    ...
    let (session, _turn_context, rx) = make_session_and_context_with_auth_and_config_and_rx(
        ...
        Vec::new(),
        move |config| {
            config.model_provider.base_url = Some(server_base_url);
    ...
    let _responses = mount_sse_once(
        &server,
        sse(vec![
            ev_assistant_message("msg-precompact", "summary"),
            ev_completed("resp-precompact"),
        ]),
    )
    .await;
    ...
    let mut switched_provider = session.provider().await;
    switched_provider.base_url = Some(format!("{}/v1", server.uri()));
    let mut switched_config = (*session.get_config().await).clone();
    switched_config.model_provider_id = "small".to_string();
    switched_config.model_provider = switched_provider.clone();
    let switched_config = Arc::new(switched_config);

    let turn_context = session
        .new_turn_with_sub_id(
            "sub-downshift".to_string(),
            SessionSettingsUpdate {
                provider: Some(("small".to_string(), switched_provider)),
                provider_config: Some(switched_config),
                ..Default::default()
            },
        )
        .await
...
fn turn_environments_for_tests(
    environment: &Arc<agere_exec_server::Environment>,
    cwd: &agere_utils_fs::AbsolutePathBuf,
) -> Vec<TurnEnvironment> {
    vec![TurnEnvironment {
        environment_id: agere_exec_server::LOCAL_ENVIRONMENT_ID.to_string(),
...
async fn session_configuration_apply_preserves_profile_file_system_policy_on_cwd_only_update() {
    ...
    let network_access_policy =
        PermissionProfile::from_legacy_access_policy(access_policy.as_str())
            .network_access_policy();
    session_configuration.permission_profile = agere_config::Constrained::allow_any(
        PermissionProfile::from_runtime_permissions_with_filesystem_isolation(
            FilesystemIsolation::from_legacy_access_policy(&access_policy),
    ...
    assert_eq!(
        updated.file_system_access_policy(),
        file_system_access_policy
    );
}
...
async fn session_configuration_apply_permission_profile_preserves_existing_deny_read_entries() {
    ...
    let workspace_policy = "workspace-write".to_string();
    ...
    let mut existing_file_system_policy = FileSystemAccessPolicy::from_legacy_access_policy_for_cwd(
        &workspace_policy,
    ...
    session_configuration.permission_profile = agere_config::Constrained::allow_any(
        PermissionProfile::from_runtime_permissions_with_filesystem_isolation(
            FilesystemIsolation::from_legacy_access_policy(&workspace_policy),
            &existing_file_system_policy,
            NetworkAccessPolicy::Restricted,
        ),
    );

    let requested_file_system_policy = FileSystemAccessPolicy::from_legacy_access_policy_for_cwd(
        &workspace_policy,
        session_configuration.cwd.as_path(),
    );
    let permission_profile = agere_protocol::models::PermissionProfile::from_runtime_permissions(
...
async fn session_configuration_apply_permission_profile_accepts_direct_write_roots() {
    ...
    let external_write_path = AbsolutePathBuf::from_absolute_path(
        agere_utils_fs::canonicalize_preserving_symlinks(external_write_dir.path())
    ...
    assert_eq!(updated.access_policy(), "workspace-write".to_string());
}
...
async fn new_default_turn_uses_config_aware_skills_for_role_overrides() {
    ...
    let agere_home = parent_config.agere_home.clone();
    let skill_dir = agere_home.join("skills").join("demo");
    ...
    let skill_fs = session
        ...
        .unwrap_or_else(|| std::sync::Arc::clone(&agere_exec_server::LOCAL_FS));
    ...
    let role_path = agere_home.join("skills-role.toml");
    std::fs::write(
        &role_path,
        format!(
...
async fn session_configuration_apply_rederives_legacy_file_system_policy_on_cwd_update() {
    ...
    session_configuration.cwd = original_cwd.abs();
    let access_policy = "workspace-write".to_string();
    let file_system_access_policy = FileSystemAccessPolicy::from_legacy_access_policy_for_cwd(
        &access_policy,
    ...
    let expected_file_system_policy = FileSystemAccessPolicy::from_legacy_access_policy_for_cwd(
...
async fn session_new_fails_when_zsh_fork_enabled_without_zsh_path() {
    let agere_home = tempfile::tempdir().expect("create temp dir");
    let mut config = build_test_config(agere_home.path()).await;
    config
        .features
    ...
    let auth_manager = AuthManager::from_auth_for_testing(AgereAuth::from_api_key("Test API Key"));
    ...
    let plugins_manager = Arc::new(PluginsManager::new(config.agere_home.to_path_buf()));
    let mcp_manager = Arc::new(McpManager::new(Arc::clone(&plugins_manager)));
    let skills_manager = Arc::new(SkillsManager::new(
        config.agere_home.clone(),
    ...
    let result = Session::new(
        ...
        Arc::new(agere_thread_store::LocalThreadStore::new(
            agere_rollout::RolloutConfig::from_view(config.as_ref()),
        )),
        agere_rollout_trace::ThreadTraceContext::disabled(),
    )
    .await;
...
pub(crate) async fn make_session_and_context() -> (Session, TurnContext) {
    ...
    let environment = Arc::new(
        agere_exec_server::Environment::create_for_tests(/*exec_server_url*/ None)
            .expect("create environment"),
    );

    let skills_watcher = Arc::new(SkillsWatcher::noop());
    let swappable_models_manager = Arc::new(agere_models_manager::SwappableModelsManager::new(
    ...
    let services = SessionServices {
        ...
        rollout_thread_trace: agere_rollout_trace::ThreadTraceContext::disabled(),
        ...
        models_manager: swappable_models_manager.clone(),
        ...
        state_db: None,
        live_thread: None,
        thread_store: Arc::new(agere_thread_store::LocalThreadStore::new(
            agere_rollout::RolloutConfig::from_view(config.as_ref()),
        )),
        ...
        environment_manager: Arc::new(agere_exec_server::EnvironmentManager::default_for_tests()),
    };
    ...
    let session = Session {
        ...
        pending_model_providers: std::sync::Mutex::new(HashMap::new()),
    };
    ...
    *session.active_turn.lock().await = Some(ActiveTurn::default());
    ...
    session
        .notify_request_permissions_response(
            "missing",
            agere_protocol::request_permissions::RequestPermissionsResponse {
                permissions: RequestPermissionProfile {
                    network: Some(agere_protocol::models::NetworkPermissions {
                        enabled: Some(true),
                    }),
...
async fn request_permissions_emits_event_when_granular_policy_allows_requests() {
    ...
    *session.active_turn.lock().await = Some(ActiveTurn::default());
    ...
    let expected_response = agere_protocol::request_permissions::RequestPermissionsResponse {
        permissions: RequestPermissionProfile {
            network: Some(agere_protocol::models::NetworkPermissions {
                enabled: Some(true),
            }),
        ...
        async move {
            session
                .request_permissions(
                    &turn_context,
                    call_id,
                    agere_protocol::request_permissions::RequestPermissionsArgs {
...
async fn submit_with_id_captures_current_span_trace_context() {
    ...
    let _trace_test_context = install_test_tracing("agere-core-tests");
...
#[tokio::test]
async fn active_user_turn_with_provider_override_does_not_swap_session_provider() {
    let (session, turn_context, _rx) = make_session_and_context_with_rx().await;
    let original_provider = {
        let state = session.state.lock().await;
        state.session_configuration.provider.clone()
    };
    let mut switched_provider = original_provider.clone();
    switched_provider.base_url = Some("https://example.com/switched".to_string());
    let switched_config = Arc::new(session.get_config().await);
    ...
    session
        ...
        .await;
    session.stage_model_provider(
        "sub-1".to_string(),
        "switched".to_string(),
        switched_provider,
        Arc::clone(&switched_config),
    );
    ...
    handlers::user_input_or_turn(
        ...
        Op::UserInputWithTurnContext {
            ...
            model_provider: Some("switched".to_string()),
            effort: None,
            summary: None,
            service_tier: None,
            final_output_json_schema: None,
    ...
    let state = session.state.lock().await;
    assert_eq!(state.session_configuration.provider, original_provider);
}

#[tokio::test]
async fn unstaged_user_turn_provider_override_is_rejected() {
    let (session, turn_context, rx) = make_session_and_context_with_rx().await;

    handlers::user_input_or_turn(
        &session,
        "sub-unstaged".to_string(),
        Op::UserInputWithTurnContext {
            environments: None,
            items: vec![UserInput::Text {
                text: "hello".to_string(),
            ...
            windows_execution_restriction_level: None,
            model: Some(turn_context.model_info.slug.clone()),
            model_provider: Some("missing-staged-provider".to_string()),
            effort: None,
            summary: None,
            service_tier: None,
            final_output_json_schema: None,
    ...
    let EventMsg::Error(ErrorEvent {
        message,
        agere_error_info,
    }) = event.msg
    else {
        panic!("expected error event for unstaged provider override");
    };
    assert_eq!(
        message,
        "model provider override `missing-staged-provider` was not staged for this turn"
    );
    assert_eq!(agere_error_info, Some(AgereErrorInfo::BadRequest));
...
async fn unknown_turn_environment_returns_error() {
    ...
    assert!(matches!(err, AgereErr::InvalidRequest(_)));
...
async fn spawn_task_turn_span_inherits_dispatch_trace_context() {
    ...
    let submission_context =
        agere_otel::context_from_w3c_trace_context(&submission_trace).expect("submission");
    let task_context = agere_otel::context_from_w3c_trace_context(&task_trace).expect("task trace");
...
async fn shutdown_complete_does_not_append_to_thread_store_after_shutdown() {
    let (mut session, _turn_context) = make_session_and_context().await;
    let store = Arc::new(agere_thread_store::InMemoryThreadStore::default());
    let thread_store: Arc<dyn agere_thread_store::ThreadStore> = store.clone();
    let live_thread = LiveThread::create(
    ...
    assert_eq!(
        agere_thread_store::InMemoryThreadStoreCalls {
            create_thread: 1,
            shutdown_thread: 1,
            ..Default::default()
        },
...
async fn shutdown_and_wait_allows_multiple_waiters() {
    ...
    let waiter_1 = {
        let agere = Arc::clone(&agere);
        tokio::spawn(async move { agere.shutdown_and_wait().await })
    };
    let waiter_2 = {
        let agere = Arc::clone(&agere);
        tokio::spawn(async move { agere.shutdown_and_wait().await })
...
async fn shutdown_and_wait_shuts_down_tracked_ephemeral_guardian_review() {
    ...
    parent_session
        .guardian_review_session
        .register_ephemeral_for_test(child_agere)
        .await;
...
async fn upsert_goal_test_thread(session: &Session) {
    ...
    let mut builder = agere_state::ThreadMetadataBuilder::new(
...
async fn handle_output_item_done_records_image_save_history_message() {
    ...
    let expected_saved_path = crate::stream_events_utils::image_generation_artifact_path(
        &turn_context.config.agere_home,
...
fn file_system_policy_with_unreadable_glob(turn_context: &TurnContext) -> FileSystemAccessPolicy {
    let mut policy = FileSystemAccessPolicy::from_legacy_access_policy_for_cwd(
        &turn_context.access_policy(),
        &turn_context.cwd,
    );
    policy.entries.push(FileSystemAccessEntry {
        path: FileSystemPath::GlobPattern {
...
#[tokio::test]
async fn turn_context_item_omits_legacy_equivalent_file_system_access_policy() {
    let (_session, turn_context) = make_session_and_context().await;

    let item = turn_context.to_turn_context_item();

    assert_eq!(item.file_system_access_policy, None);
    assert_eq!(
...
#[tokio::test]
async fn turn_context_item_stores_split_file_system_access_policy_when_different() {
    let (_session, mut turn_context) = make_session_and_context().await;
    let file_system_access_policy = file_system_policy_with_unreadable_glob(&turn_context);
    turn_context.permission_profile =
        PermissionProfile::from_runtime_permissions_with_filesystem_isolation(
            turn_context.permission_profile.filesystem_isolation(),
            &file_system_access_policy,
            turn_context.network_access_policy(),
        );

    let item = turn_context.to_turn_context_item();

    assert_eq!(
        item.file_system_access_policy,
        Some(file_system_access_policy)
    );
    assert_eq!(
        item.permission_profile,
        Some(turn_context.permission_profile())
...
async fn record_context_updates_and_set_reference_context_item_persists_split_file_system_policy_to_rollout()
 {
    ...
    let persisted_file_system_access_policy = resumed.history.iter().find_map(|item| match item {
        RolloutItem::TurnContext(ctx) => ctx.file_system_access_policy.clone(),
        _ => None,
    });
    assert_eq!(
        persisted_file_system_access_policy,
        Some(file_system_access_policy)
    );
}
...
async fn interrupt_accounts_active_goal_before_pausing() -> anyhow::Result<()> {
    ...
    assert_eq!(
        agere_protocol::protocol::ThreadGoalStatus::Paused,
        goal.status
    );
...
async fn active_goal_continuation_runs_to_completion_after_turn() -> anyhow::Result<()> {
    ...
    tokio::time::timeout(std::time::Duration::from_secs(8), async {
        loop {
            let event = test.agere.next_event().await?;
...
async fn goal_test_state_db(sess: &Session) -> anyhow::Result<crate::StateDbHandle> {
    ...
    agere_state::StateRuntime::init(config.sqlite_home.clone(), config.model_provider_id.clone())
        .await
}
...
async fn budget_limited_accounting_steers_active_turn_without_aborting() -> anyhow::Result<()> {
    ...
    assert_eq!(agere_state::ThreadGoalStatus::BudgetLimited, goal.status);
...
async fn completed_goal_accounts_current_turn_tokens_before_tool_response() -> anyhow::Result<()> {
    ...
    let state_db = agere_state::StateRuntime::init(
...
async fn tool_calls_reopen_mailbox_delivery_for_current_turn() {
    ...
    let (output, _completion) = handle_output_item_done(
        &mut ctx,
        item,
        /*previously_active_item*/ None,
        OutputItemCompletion::Final,
...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn review_task_uses_parent_turn_config_after_provider_switch() {
    let (sess, mut parent_turn_context, _rx) = make_session_and_context_with_rx().await;
    let mut stale_provider = parent_turn_context.config.model_provider.clone();
    stale_provider.base_url = Some("https://stale-provider.test".to_string());
    stale_provider.wire_api = WireApi::Anthropic;

    let mut fresh_provider = parent_turn_context.config.model_provider.clone();
    fresh_provider.base_url = Some("https://fresh-provider.test".to_string());
    fresh_provider.wire_api = WireApi::Responses;
    let mut fresh_config = (*parent_turn_context.config).clone();
    fresh_config.model_provider_id = "fresh-provider".to_string();
    fresh_config.model_provider = fresh_provider.clone();
    let fresh_config = Arc::new(fresh_config);
    assert_ne!(fresh_config.model_provider, stale_provider);

    Arc::get_mut(&mut parent_turn_context)
        .expect("parent turn context should be uniquely owned")
        .config = Arc::clone(&fresh_config);
    Arc::get_mut(&mut parent_turn_context)
        .expect("parent turn context should be uniquely owned")
        .provider = agere_model_provider::create_model_provider(
        fresh_provider,
    ...
    spawn_review_thread(
        ...
        "review-turn".to_string(),
        crate::review_prompts::ResolvedReviewRequest {
            target: ReviewTarget::Custom {
                instructions: "review current changes".to_string(),
            },
    ...
    let review_turn_context = {
        let active_turn = sess.active_turn.lock().await;
        active_turn
            .as_ref()
            .and_then(|active_turn| active_turn.tasks.get("review-turn"))
            .map(|task| Arc::clone(&task.turn_context))
            .expect("review task should be active")
    };

    assert_eq!(
        review_turn_context.config.model_provider_id,
        "fresh-provider"
    );
    assert_eq!(
        review_turn_context.config.model_provider,
        fresh_config.model_provider
    );
    assert_eq!(
...
async fn rejects_escalated_permissions_when_policy_not_on_request() {
    ...
    let timeout_ms = 1000;
    let permission_level = ExecutionPermissionLevel::RequireEscalated;
    ...
    let resp = handler
        .handle(ToolInvocation {
            ...
            call_id,
            tool_name: agere_tools::ToolName::plain(tool_name),
    ...
    // policy rather than platform-specific execution isolation.
    ...
    let file_system_access_policy = turn_context.file_system_access_policy();
    let exec_approval_requirement = session
        ...
        .create_exec_approval_requirement_for_command(ExecApprovalRequest {
            ...
            file_system_access_policy: &file_system_access_policy,
            access_cwd: turn_context.cwd.as_path(),
...
async fn unified_exec_rejects_escalated_permissions_when_policy_not_on_request() {
    ...
    let resp = handler
        .handle(ToolInvocation {
            ...
            tool_name: agere_tools::ToolName::plain("exec_command"),
            source: crate::tools::context::ToolCallSource::Direct,
            payload: ToolPayload::Function {
                arguments: serde_json::json!({
                    "cmd": "echo hi",
                    "permission_level": ExecutionPermissionLevel::RequireEscalated,
                    "justification": "need unrestricted execution",
                })
                .to_string(),
            },
        })
...
async fn session_start_hooks_only_load_from_trusted_project_layers() -> std::io::Result<()> {
    ...
    let agere_home = temp.path().join("home");
    let project_root = temp.path().join("project");
    let nested = project_root.join("nested");
    let root_dot_agere = project_root.join(".openagere");
    let nested_dot_agere = nested.join(".openagere");

    std::fs::create_dir_all(&agere_home)?;
    std::fs::create_dir_all(&nested_dot_agere)?;
    std::fs::write(project_root.join(".git"), "gitdir: here")?;
    write_project_hooks(&root_dot_agere)?;
    write_project_hooks(&nested_dot_agere)?;
    write_project_trust_config(&agere_home, &[(&nested, TrustLevel::Trusted)]).await?;

    let config = ConfigBuilder::default()
        .agere_home(agere_home)
    ...
    let expected_source_path =
        agere_utils_fs::AbsolutePathBuf::from_absolute_path(nested_dot_agere.join("hooks.json"))?;
    assert_eq!(
        preview
            .iter()
...
async fn session_start_hooks_require_project_trust_without_config_toml() -> std::io::Result<()> {
    ...
    let project_root = temp.path().join("project");
    let nested = project_root.join("nested");
    let dot_agere = project_root.join(".openagere");
    std::fs::create_dir_all(&nested)?;
    std::fs::write(project_root.join(".git"), "gitdir: here")?;
    write_project_hooks(&dot_agere)?;
    ...
    for (name, trust_entries, expected_hooks) in cases {
        let agere_home = temp.path().join(format!("home_{name}"));
        std::fs::create_dir_all(&agere_home)?;
        write_project_trust_config(&agere_home, &trust_entries).await?;
...
#[tokio::test]
async fn stage_model_provider_round_trips_and_take_clears() {
    let (sess, _tc) = make_session_and_context().await;

    // Initially nothing staged.
    assert!(sess.take_pending_model_provider().is_none());

    let provider =
        ModelProviderInfo::create_openai_provider(Some("https://staged.example/v1".to_string()));
    let config = sess.get_config().await;
    sess.stage_model_provider(
        "sub-1".to_string(),
        "staged-provider".to_string(),
        provider.clone(),
        Arc::clone(&config),
    );

    // First take returns the staged provider...
    let taken = sess.take_pending_model_provider();
    assert!(taken.is_some());
    let (id, info, taken_config) = taken.expect("staged provider present");
    assert_eq!(id, "staged-provider");
    assert_eq!(info.base_url, provider.base_url);
    assert_eq!(taken_config.agere_home, config.agere_home);

    // ...and clears the slot so a second take is empty.
    assert!(sess.take_pending_model_provider().is_none());
}

#[tokio::test]
async fn staged_model_provider_only_matches_triggering_submission_and_provider_id() {
    let (sess, _tc) = make_session_and_context().await;
    ...
    assert!(
        sess.take_pending_model_provider_for("sub-1", "provider-a")
            .is_none()
    );
    assert!(
        sess.take_pending_model_provider_for("sub-2", "provider-b")
            .is_none()
    );

    let taken = sess.take_pending_model_provider_for("sub-1", "provider-b");
    let (id, info, taken_config) = taken.expect("matching provider remains staged");
    assert_eq!(id, "provider-b");
...
#[tokio::test]
async fn staged_model_provider_keeps_concurrent_submissions_separate() {
    let (sess, _tc) = make_session_and_context().await;

    let provider_a =
        ModelProviderInfo::create_openai_provider(Some("https://staged-a.example/v1".to_string()));
    let provider_b =
        ModelProviderInfo::create_openai_provider(Some("https://staged-b.example/v1".to_string()));
    let config = sess.get_config().await;
    sess.stage_model_provider(
        "sub-a".to_string(),
    ...
    let taken_a = sess
        .take_pending_model_provider_for("sub-a", "provider-a")
        .expect("first submission should keep its provider");
    assert_eq!(taken_a.0, "provider-a");
    assert_eq!(taken_a.1, provider_a);

    let taken_b = sess
        .take_pending_model_provider_for("sub-b", "provider-b")
        .expect("second submission should keep its provider");
    assert_eq!(taken_b.0, "provider-b");
    assert_eq!(taken_b.1, provider_b);
    assert!(sess.take_pending_model_provider().is_none());
}

#[tokio::test]
async fn clearing_staged_model_provider_only_removes_matching_submission() {
    let (sess, _tc) = make_session_and_context().await;
    ...
    sess.clear_pending_model_provider_for("sub-a");

    assert!(
        sess.take_pending_model_provider_for("sub-a", "provider-a")
            .is_none()
    );
    let taken_b = sess
        .take_pending_model_provider_for("sub-b", "provider-b")
        .expect("other submissions should remain staged");
    assert_eq!(taken_b.0, "provider-b");
    assert_eq!(taken_b.1, provider_b);
    assert!(sess.take_pending_model_provider().is_none());
}
...
#[tokio::test]
async fn has_active_turn_reflects_session_active_turn_state() {
    let (sess, _tc) = make_session_and_context().await;
...
#[tokio::test]
async fn stage_model_provider_change_guard_only_applies_on_different_id() {
    let (sess, _tc) = make_session_and_context().await;

    // Read the currently-active provider id (what the op-handler compares against).
    let current_id = {
    ...
    // Staging the SAME id then applying the op-handler's guard => no-op (None).
    sess.stage_model_provider(
        "sub-1".to_string(),
        current_id.clone(),
        provider.clone(),
    ...
    let same = sess
        .take_pending_model_provider_for("sub-1", &current_id)
        .and_then(|(id, info, config)| {
            if id != current_id {
                Some((id, info, config))
            } else {
                None
    ...
    assert!(same.is_none(), "same-id provider must not be applied");

    // Staging a DIFFERENT id then applying the guard => applied (Some).
    sess.stage_model_provider(
        "sub-2".to_string(),
        "a-different-provider".to_string(),
        provider.clone(),
        Arc::clone(&config),
    );
    let different = sess
        .take_pending_model_provider_for("sub-2", "a-different-provider")
        .and_then(|(id, info, config)| {
            if id != current_id {
                Some((id, info, config))
            } else {
    ...
    let (applied_id, _info, _config) = different.expect("different-id provider must be applied");
    assert_eq!(applied_id, "a-different-provider");
}
