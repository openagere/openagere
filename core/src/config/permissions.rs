...
#[cfg(test)]
use agere_network_proxy::NetworkUnixSocketPermission as ProxyNetworkUnixSocketPermission;
...
pub(crate) fn builtin_permission_profile(
    profile_name: &str,
    workspace_write: Option<&AccessWorkspaceWrite>,
...
pub(crate) fn network_proxy_config_from_profile_network(
    ...
    // `network.enabled` marks that the profile participates in managed network gating.
    // That bit alone does not require starting a managed proxy; keep the proxy enabled
    // only when the profile also supplies configuration that only the proxy can enforce.
...
pub(crate) fn compile_permission_profile(
...
) -> io::Result<(FileSystemAccessPolicy, NetworkAccessPolicy)> {
    let profile = resolve_permission_profile(permissions, profile_name)?;
            ...
            if cfg!(not(target_os = "macos")) {
                for pattern in unsupported_read_write_glob_paths(filesystem) {
                    push_warning(
                        startup_warnings,
                        format!(
                            "Filesystem glob `{pattern}` uses `read` or `write` access, which this platform cannot enforce through wildcard expansion alone. Use an exact path or trailing `/**` subtree rule instead. `none` deny-read globs are supported."
                        ),
                    );
                }
                for pattern in unbounded_unreadable_globstar_paths(filesystem) {
                    push_warning(
                        startup_warnings,
                        format!(
                            "Filesystem deny-read glob `{pattern}` uses `**`. On non-macOS platforms glob expansion cannot walk unbounded `**` safely; set `glob_scan_max_depth` in this filesystem profile to cap Linux glob expansion and silence this warning, or enumerate explicit depths such as `*.env`, `*/*.env`, and `*/*/*.env`."
                        ),
                    );
                }
            }
    ...
    let network_access_policy = compile_network_access_policy(profile.network.as_ref());
    let mut file_system_access_policy = FileSystemAccessPolicy::restricted(entries);
    file_system_access_policy.glob_scan_max_depth = glob_scan_max_depth;
    Ok((file_system_access_policy, network_access_policy))
}
...
/// for Agere to function. These should always be added to the
/// `FileSystemAccessPolicy` for a thread.
pub(crate) fn get_readable_roots_required_for_agere_runtime(
    agere_home: &Path,
    zsh_path: Option<&PathBuf>,
    main_execve_wrapper_exe: Option<&PathBuf>,
) -> Vec<AbsolutePathBuf> {
    let arg0_root = AbsolutePathBuf::from_absolute_path(agere_home.join("tmp").join("arg0")).ok();
...
fn compile_network_access_policy(network: Option<&NetworkToml>) -> NetworkAccessPolicy {
    let Some(network) = network else {
        return NetworkAccessPolicy::Restricted;
    };

    match network.enabled {
        Some(true) => NetworkAccessPolicy::Enabled,
        _ => NetworkAccessPolicy::Restricted,
    }
}
...
fn compile_filesystem_permission(
    ...
    policy_cwd: &Path,
    startup_warnings: &mut Vec<String>,
) -> io::Result<Vec<FileSystemAccessEntry>> {
    let mut entries = Vec::new();
    match permission {
        ...
        FilesystemPermissionToml::Scoped(scoped_entries) => {
            for (subpath, access) in scoped_entries {
                ...
                if has_glob && *access == FileSystemAccessMode::None && can_compile_as_pattern {
                    ...
                    let entry = FileSystemAccessEntry {
...
fn compile_scoped_filesystem_pattern(
    ...
    // coarse permission profile model does not express yet.
    if access != FileSystemAccessMode::None {
        return Err(io::Error::new(
            io::ErrorKind::InvalidInput,
...
// Adding a new `:special_path` must not make older Agere versions reject the
...
fn missing_filesystem_entries_warning(profile_name: &str) -> String {
    format!(
        "Permissions profile `{profile_name}` does not define any recognized filesystem entries for this version of Agere. Filesystem access will remain restricted. Upgrade Agere if this profile expects filesystem permissions."
    )
}
...
fn maybe_push_unknown_special_path_warning(
    ...
    push_warning(
        ...
        match subpath.as_deref() {
            Some(subpath) => format!(
                "Configured filesystem path `{path}` with nested entry `{}` is not recognized by this version of Agere and will be ignored. Upgrade Agere if this path is required.",
                subpath.display()
            ),
            None => format!(
                "Configured filesystem path `{path}` is not recognized by this version of Agere and will be ignored. Upgrade Agere if this path is required."
            ),
        },
    );
}
