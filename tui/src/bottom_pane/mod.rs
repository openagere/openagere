...
mod provider_wizard;
pub(crate) use provider_wizard::ProviderWizard;
...
impl BottomPane {
    ...
    pub fn set_windows_degraded_restriction_active(&mut self, enabled: bool) {
        self.composer
            .set_windows_degraded_restriction_active(enabled);
        self.request_redraw();
    }
    ...
    /// Return a clone of the list keymap used by selection popups.
    ...
    /// Called when the remote fetch completes while `/provider` is open.
    pub(crate) fn update_provider_wizard_remote_templates(
        &mut self,
        templates: Vec<ProviderTemplate>,
    ) {
        if let Some(view) = self.view_stack.last_mut()
            && view.view_id() == Some("provider_wizard")
            && let Some(wizard) = view
                .as_any_mut()
                .and_then(|a: &mut dyn std::any::Any| a.downcast_mut::<ProviderWizard>())
        {
            wizard.update_remote_templates(templates);
            self.request_redraw();
        }
    }
    ...
    pub fn push_user_input_request(&mut self, request: RequestUserInputEvent) {
        ...
        let modal = RequestUserInputOverlay::new_with_interrupt_keys(
            request,
            self.app_event_tx.clone(),
            self.has_input_focus,
            self.enhanced_keys_supported,
            self.disable_paste_burst,
            self.keymap.chat.interrupt_turn.clone(),
        );
        self.pause_status_timer_for_modal();
    ...
    pub(crate) fn on_grep_search_result(&mut self, query: String, matches: Vec<RgMatch>) {
        self.composer.on_grep_search_result(query, matches);
        self.request_redraw();
    }
    ...
    fn as_renderable(&'_ self) -> RenderableItem<'_> {
        if let Some(view) = self.active_view() {
            ...
            flex2.push(
                /*flex*/ 1,
                RenderableItem::Owned(self.build_top_flex().into()),
            );
...
impl BottomPane {
    /// Build the flex layout containing all items above the composer.
    /// Used by both render() and cursor_pos() to ensure consistent layout.
    fn build_top_flex(&self) -> FlexRenderable<'_> {
...
mod tests {
    ...
    fn exec_request() -> ApprovalRequest {
        ApprovalRequest::Exec {
            thread_id: agere_protocol::ThreadId::new(),
            thread_label: None,
            id: "1".to_string(),
