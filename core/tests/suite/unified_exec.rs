...
use core_test_support::skip_if_seatbelt_execution_marker;
...
async fn wait_for_raw_unified_exec_output(
    ...
    let content = wait_for_event_match(&test.agere, |event| match event {
...
async fn unified_exec_intercepts_apply_patch_exec_command() -> Result<()> {
    ...
    skip_if_seatbelt_execution_marker!(Ok(()));
    skip_if_windows!(Ok(()));

    let builder = test_agere().with_config(|config| {
    ...
    let harness = TestAgereHarness::with_builder(builder).await?;
...
async fn unified_exec_emits_exec_command_begin_event() -> Result<()> {
    ...
    let mut builder = test_agere().with_model("gpt-5.2").with_config(|config| {
        config.use_experimental_unified_exec_tool = true;
        config
            .features
            .enable(Feature::UnifiedExec)
    ...
    submit_unified_exec_turn(&test, "emit begin event", "danger-full-access".to_string()).await?;

    let begin_event = wait_for_event_match(&test.agere, |msg| match msg {
...
async fn unified_exec_respects_workdir_override() -> Result<()> {
    ...
    submit_unified_exec_turn(&test, "run workdir test", "danger-full-access".to_string()).await?;
...
async fn unified_exec_emits_exec_command_end_event() -> Result<()> {
    ...
    submit_unified_exec_turn(&test, "emit end event", "danger-full-access".to_string()).await?;

    let end_event = wait_for_event_match(&test.agere, |msg| match msg {
...
async fn unified_exec_emits_output_delta_for_exec_command() -> Result<()> {
    ...
    submit_unified_exec_turn(&test, "emit delta", "danger-full-access".to_string()).await?;

    let event = wait_for_event_match(&test.agere, |msg| match msg {
        EventMsg::ExecCommandEnd(ev) if ev.call_id == call_id => Some(ev.clone()),
        _ => None,
    })
    .await;
...
async fn unified_exec_full_lifecycle_with_background_end_event() -> Result<()> {
    ...
    loop {
        let msg = wait_for_event(&test.agere, |_| true).await;
        match msg {
...
async fn unified_exec_emits_terminal_interaction_for_write_stdin() -> Result<()> {
    ...
    submit_unified_exec_turn(&test, "stdin delta", "danger-full-access".to_string()).await?;
...
async fn unified_exec_emits_one_begin_and_one_end_event() -> Result<()> {
    ...
    let mut task_completed = false;
    loop {
        let event_msg = wait_for_event(&test.agere, |_| true).await;
        match event_msg {
...
async fn exec_command_reports_chunk_and_exit_metadata() -> Result<()> {
    ...
    submit_unified_exec_turn(&test, "run metadata test", "danger-full-access".to_string()).await?;
...
async fn exec_command_clamps_model_requested_max_output_tokens_to_policy() -> Result<()> {
    ...
    let mut builder = test_agere().with_model("gpt-5.4").with_config(|config| {
        config.use_experimental_unified_exec_tool = true;
...
async fn unified_exec_can_enable_tty() -> Result<()> {
    ...
    submit_unified_exec_turn(&test, "check tty enabled", "danger-full-access".to_string()).await?;
...
async fn unified_exec_emits_end_event_when_session_dies_via_stdin() -> Result<()> {
    ...
    submit_unified_exec_turn(&test, "end on exit", "danger-full-access".to_string()).await?;
...
async fn unified_exec_keeps_long_running_session_after_turn_end() -> Result<()> {
    ...
    let begin_event = wait_for_event_match(&agere, |msg| match msg {
    ...
    wait_for_event(&agere, |event| matches!(event, EventMsg::ShutdownComplete)).await;
...
async fn unified_exec_interrupt_preserves_long_running_session() -> Result<()> {
    ...
    let _begin_event = wait_for_event_match(&agere, |msg| match msg {
        EventMsg::ExecCommandBegin(ev) if ev.call_id == call_id => Some(ev.clone()),
        _ => None,
    })
    .await;
    ...
    agere.submit(Op::Interrupt).await?;
    wait_for_event(&agere, |event| matches!(event, EventMsg::TurnAborted(_))).await;
    ...
    agere.submit(Op::CleanBackgroundTerminals).await?;
    wait_for_process_exit(&pid).await?;
...
async fn unified_exec_reuses_session_via_stdin() -> Result<()> {
    ...
    submit_unified_exec_turn(&test, "run unified exec", "danger-full-access".to_string()).await?;
...
async fn unified_exec_timeout_and_followup_poll() -> Result<()> {
    ...
    submit_unified_exec_turn(&test, "check timeout", "danger-full-access".to_string()).await?;

    loop {
        let event = test.agere.next_event().await.expect("event");
...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn unified_exec_runs_under_managed_restrictions() -> Result<()> {
    skip_if_no_network!(Ok(()));
    skip_if_seatbelt_execution_marker!(Ok(()));
    skip_if_windows!(Ok(()));
    ...
    agere
        .submit(Op::UserTurn {
            ...
            approvals_reviewer: None,
            // Important!
            permission_profile: Some(PermissionProfile::from_legacy_access_policy("read-only")),
            model: session_model,
            effort: None,
            summary: None,
            service_tier: None,
...
async fn unified_exec_enforces_glob_deny_read_policy() -> Result<()> {
    ...
    let server = start_mock_server().await;
    let read_only_policy = "read-only".to_string();
    ...
    let mut builder = test_agere().with_config(move |config| {
        config
            ...
            .expect("test config should allow feature update");
        config
            .set_legacy_access_policy(read_only_policy_for_config)
            .expect("set legacy access policy");
        let mut file_system_access_policy = FileSystemAccessPolicy::default();
    ...
    assert!(
        has_denial,
        "expected access denial details in unified exec output: {output:?}"
    );
