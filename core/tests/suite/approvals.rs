...
use agere_protocol::protocol::ExecPolicyAmendment;
...
impl TargetPath {
    fn resolve_for_patch(self, test: &TestAgere) -> (PathBuf, String) {
        match self {
...
const DEFAULT_UNIFIED_EXEC_JUSTIFICATION: &str =
    "Requires escalated permissions to bypass filesystem restrictions in tests.";
    ...
    async fn prepare(
        ...
        match self {
            ...
            ActionKind::ApplyPatchShell { target, content } => {
                ...
                // Bazel may need to launch the configured Agere helper binary
                // to apply the verified patch, which can exceed the normal
                // short command timeout on slower CI runners.
                let timeout_ms = 30_000;
                let event = shell_event(call_id, &command, timeout_ms, permission_level)?;
                Ok((event, Some(command)))
            }
        }
    }
...
fn shell_event_with_prefix_rule(
    ...
    if permission_level.requests_escalated_permissions() {
        args["permission_level"] = json!(permission_level);
    }
...
impl Expectation {
    fn verify(&self, test: &TestAgere, result: &CommandResult) -> Result<()> {
        match self {
...
async fn expect_exec_approval(
    ...
    let event = wait_for_event(&test.agere, |event| {
        matches!(
            event,
...
async fn wait_for_completion_without_approval(test: &TestAgere) {
    let event = wait_for_event(&test.agere, |event| {
        matches!(
            event,
            EventMsg::ExecApprovalRequest(_) | EventMsg::TurnComplete(_)
...
async fn wait_for_completion(test: &TestAgere) {
...
async fn wait_for_spawned_thread(test: &TestAgere) -> Result<Arc<AgereThread>> {
...
fn scenarios() -> Vec<ScenarioSpec> {
    ...
    let workspace_write = |network_access| "workspace-write".to_string();
    ...
    vec![
        ...
        ScenarioSpec {
            ...
            approval_policy: UnlessTrusted,
            access_policy: workspace_write(false),
            action: ActionKind::RunCommand {
        ...
        ScenarioSpec {
            name: "read_only_on_failure_escalates_after_access_denial",
            approval_policy: OnFailure,
            access_policy: "read-only".to_string(),
            action: ActionKind::WriteFile {
            ...
            outcome: Outcome::ExecApproval {
                decision: ReviewDecision::Approved,
                expected_reason: Some("command failed; retry with escalated access?"),
            },
            expectation: Expectation::FileCreated {
                target: TargetPath::Workspace("ro_on_failure.txt"),
                content: "read-only-on-failure",
        ...
        ScenarioSpec {
            name: "read_only_on_failure_escalates_after_access_denial_gpt_5_1_no_exit",
            approval_policy: OnFailure,
            access_policy: "read-only".to_string(),
            action: ActionKind::WriteFile {
        ...
        ScenarioSpec {
            name: "read_only_never_reports_access_denial_failure",
            approval_policy: Never,
            access_policy: "read-only".to_string(),
            action: ActionKind::WriteFile {
        ...
        ScenarioSpec {
            ...
            approval_policy: OnRequest,
            access_policy: workspace_write(true),
            action: ActionKind::FetchUrl {
        ...
        // Linux arg0 wrapper test workaround doesn't work on ARM
        ScenarioSpec {
...
fn scenario_group(scenario: &ScenarioSpec) -> ScenarioGroup {
    match &scenario.action {
        ...
        | ActionKind::FetchUrlNoProxy { .. }
        | ActionKind::FetchUrl { .. }
        | ActionKind::RunCommand { .. } => match scenario.access_policy.as_str() {
            "danger-full-access" => ScenarioGroup::DangerFullAccess,
            "read-only" => ScenarioGroup::ReadOnly,
            "workspace-write" => ScenarioGroup::WorkspaceWrite,
            "external-access" => ScenarioGroup::WorkspaceWrite,
            other => panic!("unexpected access policy for scenario: {other:?}"),
        },
    }
}
...
async fn run_scenario(scenario: &ScenarioSpec) -> Result<()> {
    ...
    let access_policy = scenario.access_policy.clone();
    let features = scenario.features.clone();
    let model_override = scenario.model_override;
    let model = model_override.unwrap_or("gpt-5.4");

    let mut builder = test_agere().with_model(model).with_config(move |config| {
        config.permissions.approval_policy = Constrained::allow_any(approval_policy);
        config
            .set_legacy_access_policy(access_policy.clone())
    ...
    let (event, expected_command) = scenario
        .action
        .prepare(&test, &server, call_id, scenario.permission_level)
        .await?;
...
async fn approving_apply_patch_for_session_skips_future_prompts_for_same_file() -> Result<()> {
    ...
    let access_policy_for_config = access_policy.clone();
    ...
    let mut builder = test_agere()
        ...
        .with_config(move |config| {
            config.permissions.approval_policy = Constrained::allow_any(approval_policy);
            config
                .set_legacy_access_policy(access_policy_for_config)
                .expect("set legacy access policy");
...
async fn approving_execpolicy_amendment_persists_policy_and_skips_future_prompts() -> Result<()> {
    ...
    .prepare(
        ...
        &server,
        call_id_first,
        ExecutionPermissionLevel::UseDefault,
    )
    .await?;
...
async fn spawned_subagent_execpolicy_amendment_propagates_to_parent_session() -> Result<()> {
    ...
    submit_turn(&test, PARENT_PROMPT, approval_policy, access_policy.clone()).await?;
    ...
    submit_turn(&test, "parent reruns child command", approval_policy).await?;
    wait_for_completion_without_approval(&test).await;
...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
#[cfg(unix)]
async fn matched_prefix_rule_runs_with_escalated_exec_under_zsh_fork() -> Result<()> {
    skip_if_no_network!(Ok(()));

    let Some(runtime) = zsh_fork_runtime("zsh-fork prefix rule escalated exec test")? else {
        return Ok(());
    };
    ...
    let outside_path = outside_dir
        .path()
        .join("zsh-fork-prefix-rule-escalated.txt");
    ...
    let call_id = "zsh-fork-prefix-rule-escalated";
    let event = shell_event(
        call_id,
        &command,
        /*timeout_ms*/ 1_000,
    ...
    let session_model = test.session_configured.model.clone();
    let permission_profile = turn_permission_fields(permission_profile, test.cwd.path());
    test.agere
        .submit(Op::UserTurn {
            environments: None,
            items: vec![UserInput::Text {
    ...
    assert!(
        outside_path.exists(),
        "expected matched prefix_rule to rerun touch with escalated exec; output: {}",
        result.stdout
    );
...
async fn invalid_requested_prefix_rule_falls_back_for_compound_command() -> Result<()> {
    ...
    let command =
        "touch /tmp/agere-fallback-rule-test.txt && echo hello > /tmp/agere-fallback-rule-test.txt";
...
"#,
    ...
    let fetch_command = r#"python3 -c "import urllib.request; opener = urllib.request.build_opener(urllib.request.ProxyHandler()); print('OK:' + opener.open('http://agere-network-test.invalid', timeout=30).read().decode(errors='replace'))""#
        .to_string();
...
"#,
    ...
    let approval_policy = AskForApproval::OnFailure;
    let turn_access_policy = "workspace-write".to_string();
    let mut builder = test_agere()
        .with_home(home)
        .with_cloud_requirements(managed_network_requirements_loader())
        .with_config(move |config| {
