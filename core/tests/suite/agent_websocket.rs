...
async fn websocket_test_agere_shell_chain() -> Result<()> {
    ...
    let mut builder = test_agere().with_windows_cmd_shell();

    let test = builder.build_with_websocket_server(&server).await?;
    test.submit_turn_with_policy("run the echo command", test.config.legacy_access_policy())
        .await?;
...
async fn websocket_first_turn_uses_startup_prewarm_and_create() -> Result<()> {
    ...
    let test = builder.build_with_websocket_server(&server).await?;
    test.submit_turn_with_policy("hello", test.config.legacy_access_policy())
        .await?;
...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn websocket_v2_test_agere_shell_chain() -> Result<()> {
    skip_if_no_network!(Ok(()));
    ...
    let mut builder = test_agere().with_windows_cmd_shell().with_config(|config| {
        config
            .features
