...
use agere_app_server_protocol::DynamicToolSpec;
...
use agere_app_server_protocol::InitializeCapabilities;
use agere_app_server_protocol::InitializeParams;
use agere_app_server_protocol::InitializeResponse;
use agere_app_server_protocol::JSONRPCMessage;
use agere_app_server_protocol::JSONRPCNotification;
use agere_app_server_protocol::JSONRPCRequest;
...
const OTEL_SERVICE_NAME: &str = "agere-app-server-test-client";
const TRACE_DISABLED_MESSAGE: &str =
    "Not enabled - enable tracing in $AGERE_HOME/config.toml to get a trace URL!";

/// Minimal launcher that initializes the Agere app-server and logs the handshake.
#[derive(Parser)]
#[command(author = "Agere", version, about = "Bootstrap Agere app-server", long_about = None)]
struct Cli {
    /// Path to the `agere` CLI binary. When set, requests use stdio by
    /// spawning `agere app-server` as a child process.
    #[arg(long, env = "AGERE_BIN", global = true)]
    agere_bin: Option<PathBuf>,

    /// Existing websocket server URL to connect to.
    ///
    /// If neither `--agere-bin` nor `--url` is provided, defaults to
    /// `ws://127.0.0.1:4222`.
    #[arg(long, env = "AGERE_APP_SERVER_URL", global = true)]
    url: Option<String>,

    /// Forwarded to the `agere` CLI as `--config key=value`. Repeatable.
    ///
    /// Example:
...
#[derive(Subcommand)]
enum CliCommand {
    /// Start `agere app-server` on a websocket endpoint in the background.
    ///
    /// Logs are written to:
    ///   `/tmp/agere-app-server-test-client/`
    Serve {
        /// WebSocket listen URL passed to `agere app-server --listen`.
    ...
    /// Send a user message through the Agere app-server.
    SendMessage {
        /// User message to send to Agere.
        user_message: String,
    },
    ...
    /// Fetch the current account rate limits from the Agere app-server.
    GetAccountRateLimits,
    /// List the available models from the Agere app-server.
    #[command(name = "model-list")]
    ModelList,
    /// List stored threads from the Agere app-server.
    ...
    LiveElicitationTimeoutPause {
        ...
        #[arg(long, env = "AGERE_E2E_MODEL", default_value = "gpt-5")]
        model: String,
...
pub async fn run() -> Result<()> {
    ...
    match command {
        CliCommand::Serve { listen, kill } => {
            ...
            let agere_bin = agere_bin.unwrap_or_else(|| PathBuf::from("agere"));
            serve(&agere_bin, &config_overrides, &listen, kill)
        }
        CliCommand::SendMessage { user_message } => {
            ensure_dynamic_tools_unused(&dynamic_tools, "send-message")?;
            let endpoint = resolve_endpoint(agere_bin, url)?;
        ...
        CliCommand::ThreadIncrementElicitation { thread_id } => {
            ...
            let url = resolve_shared_websocket_url(agere_bin, url, "thread-increment-elicitation")?;
        ...
        CliCommand::ThreadDecrementElicitation { thread_id } => {
            ...
            let url = resolve_shared_websocket_url(agere_bin, url, "thread-decrement-elicitation")?;
...
fn resolve_endpoint(agere_bin: Option<PathBuf>, url: Option<String>) -> Result<Endpoint> {
    if agere_bin.is_some() && url.is_some() {
        bail!("--agere-bin and --url are mutually exclusive");
    }
    if let Some(agere_bin) = agere_bin {
        return Ok(Endpoint::SpawnAgere(agere_bin));
    }
    if let Some(url) = url {
...
fn resolve_shared_websocket_url(
    ...
    if agere_bin.is_some() {
        bail!(
            "{command} requires --url or an already-running websocket app-server; --agere-bin would spawn a private stdio app-server instead"
        );
    }
...
impl BackgroundAppServer {
    fn spawn(agere_bin: &Path, config_overrides: &[String]) -> Result<Self> {
        ...
        let mut cmd = Command::new(agere_bin);
        if let Some(agere_bin_parent) = agere_bin.parent() {
            let mut path = OsString::from(agere_bin_parent.as_os_str());
        ...
        let process = cmd
            ...
            .spawn()
            .with_context(|| format!("failed to start `{}` app-server", agere_bin.display()))?;
...
fn serve(agere_bin: &Path, config_overrides: &[String], listen: &str, kill: bool) -> Result<()> {
    let runtime_dir = PathBuf::from("/tmp/agere-app-server-test-client");
    ...
    let mut cmdline = format!(
        "tail -f /dev/null | RUST_BACKTRACE=full RUST_LOG=warn,agere_=trace {}",
        shell_quote(&agere_bin.display().to_string())
    );
    for override_kv in config_overrides {
    ...
    println!("started agere app-server");
...
pub async fn send_message_v2(
    ...
    let endpoint = Endpoint::SpawnAgere(agere_bin.to_path_buf());
    send_message_v2_endpoint(
        &endpoint,
        config_overrides,
        user_message,
...
async fn trigger_zsh_fork_multi_cmd_approval(
    ...
    with_client(
        ...
        |client| {
            ...
            turn_params.access_policy = Some(AccessPolicy::ReadOnly {
...
async fn trigger_cmd_approval(
    ...
    send_message_v2_with_policies(
        ...
        SendMessagePolicies {
            ...
            access_policy: Some(AccessPolicy::ReadOnly {
                network_access: false,
            }),
            dynamic_tools,
        },
...
async fn send_message_v2_with_policies(
    ...
    with_client(
        ...
        |client| {
            ...
            turn_params.access_policy = policies.access_policy;
...
async fn with_client<T>(
    ...
    endpoint: &Endpoint,
    config_overrides: &[String],
    f: impl FnOnce(&mut AgereClient) -> Result<T>,
) -> Result<T> {
    ...
    let result = command_span.in_scope(|| {
        let mut client = AgereClient::connect(endpoint, config_overrides)?;
        f(&mut client)
    });
...
fn thread_increment_elicitation(url: &str, thread_id: String) -> Result<()> {
    ...
    let mut client = AgereClient::connect(&endpoint, &[])?;
...
fn live_elicitation_timeout_pause(
    ...
    let websocket_url = match (agere_bin, url) {
        (Some(_), Some(_)) => bail!("--agere-bin and --url are mutually exclusive"),
        (Some(agere_bin), None) => {
            let server = BackgroundAppServer::spawn(&agere_bin, config_overrides)?;
    ...
    let app_server_test_client_bin = std::env::current_exe()
        .context("failed to resolve agere-app-server-test-client binary path")?;
...
impl AgereClient {
    fn connect(endpoint: &Endpoint, config_overrides: &[String]) -> Result<Self> {
        match endpoint {
            Endpoint::SpawnAgere(agere_bin) => Self::spawn_stdio(agere_bin, config_overrides),
            Endpoint::ConnectWs(url) => Self::connect_websocket(url),
        }
    }

    fn spawn_stdio(agere_bin: &Path, config_overrides: &[String]) -> Result<Self> {
        let agere_bin_display = agere_bin.display();
        let mut cmd = Command::new(agere_bin);
        if let Some(agere_bin_parent) = agere_bin.parent() {
            let mut path = OsString::from(agere_bin_parent.as_os_str());
            if let Some(existing_path) = std::env::var_os("PATH") {
        ...
        let mut agere_app_server = cmd
            .arg("app-server")
            .stdin(Stdio::piped())
            .stdout(Stdio::piped())
            .stderr(Stdio::inherit())
            .spawn()
            .with_context(|| format!("failed to start `{agere_bin_display}` app-server"))?;

        let stdin = agere_app_server
            .stdin
            .take()
            .context("agere app-server stdin unavailable")?;
        let stdout = agere_app_server
            .stdout
            .take()
            .context("agere app-server stdout unavailable")?;

        Ok(Self {
            transport: ClientTransport::Stdio {
                child: agere_app_server,
                stdin: Some(stdin),
                ...
                Err(err) => {
                    if Instant::now() >= deadline {
                        return Err(err).with_context(|| {
                            format!(
                                "failed to connect to websocket app-server at `{url}`; if no server is running, start one with `agere-app-server-test-client serve --listen {url}`"
                            )
                        });
                    }
        ...
        let request = ClientRequest::Initialize {
            ...
            params: InitializeParams {
                client_info: ClientInfo {
                    name: "agere-toy-app-server".to_string(),
                    title: Some("Agere Toy App Server".to_string()),
    ...
    fn login_account_chatgpt(&mut self) -> Result<LoginAccountResponse> {
        ...
        let request = ClientRequest::LoginAccount {
            request_id: request_id.clone(),
            params: agere_app_server_protocol::LoginAccountParams::Chatgpt,
        };
    ...
    fn login_account_chatgpt_device_code(&mut self) -> Result<LoginAccountResponse> {
        ...
        let request = ClientRequest::LoginAccount {
            request_id: request_id.clone(),
            params: agere_app_server_protocol::LoginAccountParams::ChatgptDeviceCode,
        };
        ...
        match &mut self.transport {
            ClientTransport::Stdio { stdin, .. } => {
                if let Some(stdin) = stdin.as_mut() {
                    ...
                    stdin
                        .flush()
                        .context("failed to flush payload to agere app-server")?;
                    return Ok(());
                }
                bail!("agere app-server stdin closed")
            }
        ...
        match &mut self.transport {
            ClientTransport::Stdio { stdout, .. } => {
                ...
                let bytes = stdout
                    ...
                    .context("failed to read from agere app-server")?;
                if bytes == 0 {
                    bail!("agere app-server closed stdout");
                }
                Ok(response_line)
            }
...
impl TestClientTracing {
    async fn initialize(config_overrides: &[String]) -> Result<Self> {
        ...
        let otel_provider = agere_core::otel_init::build_provider(
            &config,
...
impl Drop for AgereClient {
    fn drop(&mut self) {
        ...
        if let Ok(Some(status)) = child.try_wait() {
            println!("[agere app-server exited: {status}]");
            return;
        }
