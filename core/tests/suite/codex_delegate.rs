...
async fn agere_delegate_forwards_exec_approval_and_proceeds_on_approval() {
    skip_if_no_network!();
    ...
    let approval_event = wait_for_event(&test.agere, |ev| {
...
#[ignore = "TODO once we have a delegate that can ask for approvals"]
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn agere_delegate_forwards_patch_approval_and_proceeds_on_decision() {
    skip_if_no_network!();
    ...
    let mut builder = test_agere().with_model("gpt-5.4").with_config(|config| {
        config.permissions.approval_policy = Constrained::allow_any(AskForApproval::OnRequest);
        // Use a restricted access profile so patch approval is required
        config
            .permissions
            .set_permission_profile(PermissionProfile::read_only())
            .expect("set permission profile");
...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn agere_delegate_ignores_legacy_deltas() {
    skip_if_no_network!();
    ...
    loop {
        let ev = wait_for_event(&test.agere, |_| true).await;
        match ev {
