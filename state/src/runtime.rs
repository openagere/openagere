...
impl StateRuntime {
    /// Initialize the state runtime using the provided Agere home and default provider.
    ///
    /// This opens (and migrates) the SQLite databases under `agere_home`,
    /// keeping logs in a dedicated file to reduce lock contention with the
    /// rest of the state store.
    pub async fn init(agere_home: PathBuf, default_provider: String) -> anyhow::Result<Arc<Self>> {
        ...
        .await;
        let state_path = state_db_path(agere_home.as_path());
        let logs_path = logs_db_path(agere_home.as_path());
    ...
    /// Return the configured Agere home directory for this runtime.
    pub fn agere_home(&self) -> &Path {
...
pub fn state_db_path(agere_home: &Path) -> PathBuf {
    agere_home.join(state_db_filename())
}
...
pub fn logs_db_path(agere_home: &Path) -> PathBuf {
    agere_home.join(logs_db_filename())
}
...
async fn remove_legacy_db_files(
    ...
    let mut entries = match tokio::fs::read_dir(agere_home).await {
        Ok(entries) => entries,
        Err(err) => {
            warn!(
                "failed to read agere_home for {db_label} db cleanup {}: {err}",
                agere_home.display(),
            );
            return;
        }
...
mod tests {
    ...
    async fn open_state_sqlite_tolerates_newer_applied_migrations() {
        ...
        tokio::fs::create_dir_all(&agere_home)
            .await
            .expect("create agere home");
        let state_path = state_db_path(agere_home.as_path());
