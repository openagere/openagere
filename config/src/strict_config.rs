...
pub(crate) fn config_error_from_ignored_toml_fields<T: DeserializeOwned>(
    path: impl AsRef<Path>,
    contents: &str,
) -> Option<ConfigError> {
    match toml::from_str::<TomlValue>(contents) {
        Ok(value) => config_error_from_ignored_toml_value_fields::<T>(path, contents, value),
...
pub(crate) fn config_error_from_ignored_toml_value_fields<T: DeserializeOwned>(
    ...
    match result {
        Ok(_) => unknown_field_error_from_paths(path.as_ref(), contents, ignored_paths)
            .or_else(|| unknown_field_error_from_paths(path, contents, unknown_feature_paths)),
        Err(err) => {
