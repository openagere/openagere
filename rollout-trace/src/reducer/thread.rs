...
use crate::model::AgereTurn;
use crate::model::AgereTurnId;
...
impl TraceReducer {
    ...
    /// Starts a Agere turn inside an existing thread.
    pub(super) fn start_agere_turn(
        &mut self,
        seq: RawEventSeq,
        wall_time_unix_ms: i64,
        ...
        if self.rollout.agere_turns.contains_key(&agere_turn_id) {
            bail!("duplicate agere turn start for {agere_turn_id}");
        }

        self.thread_mut(&thread_id)?;

        self.rollout.agere_turns.insert(
            agere_turn_id.clone(),
            AgereTurn {
                agere_turn_id,
                thread_id,
    ...
    /// Marks a Agere turn terminal and validates any thread id carried by the raw event.
    pub(super) fn end_agere_turn(
        &mut self,
        seq: RawEventSeq,
        wall_time_unix_ms: i64,
        ...
        if let Some(event_thread_id) = thread_id.as_deref()
            && let Some(turn) = self.rollout.agere_turns.get(&agere_turn_id)
            && turn.thread_id != event_thread_id
        {
            bail!(
                "agere turn end for {agere_turn_id} used thread {event_thread_id}, \
                 but the turn belongs to {}",
                turn.thread_id
            );
        }

        let Some(turn) = self.rollout.agere_turns.get_mut(&agere_turn_id) else {
            bail!("agere turn end referenced unknown turn {agere_turn_id}");
        };
