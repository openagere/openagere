...
use agere_utils_fs::canonicalize_preserving_symlinks;
...
const PROTECTED_METADATA_AGERE_PATH_NAME: &str = ".openagere";
...
pub const PROTECTED_METADATA_PATH_NAMES: &[&str] = &[
    ...
    PROTECTED_METADATA_AGERE_PATH_NAME,
];
...
pub fn is_protected_metadata_directory_name(name: &OsStr) -> bool {
    name == OsStr::new(PROTECTED_METADATA_AGENTS_PATH_NAME)
        || name == OsStr::new(PROTECTED_METADATA_AGERE_PATH_NAME)
}
...
pub fn forbidden_agent_metadata_write(
    ...
    let (protected_metadata_path, metadata_name) =
        metadata_child_of_writable_root(file_system_access_policy, target.as_path(), cwd)?;
    ...
    if !file_system_access_policy.can_write_path_with_cwd(target.as_path(), cwd) {
...
impl NetworkAccessPolicy {
    pub fn is_enabled(self) -> bool {
        matches!(self, NetworkAccessPolicy::Enabled)
    }
}
...
pub enum FileSystemSpecialPath {
    ...
    /// representable so config from a newer Agere degrades to warn-and-ignore
    /// instead of failing to load. Agere 0.112.0 rejected unknown values here,
...
pub struct FileSystemAccessPolicy {
    pub kind: FileSystemAccessLevel,
    #[serde(default, skip_serializing_if = "Option::is_none")]
    #[ts(optional)]
...
/// Runtime matcher for read-deny entries in a split filesystem access policy.
...
impl ReadDenyMatcher {
    ...
    pub fn new(file_system_access_policy: &FileSystemAccessPolicy, cwd: &Path) -> Option<Self> {
        if !file_system_access_policy.has_denied_read_restrictions() {
            return None;
        }
        ...
        let denied_candidates = file_system_access_policy
        ...
        let deny_read_matchers = file_system_access_policy
...
impl FileSystemAccessPolicy {
    pub fn unrestricted() -> Self {
        Self {
            kind: FileSystemAccessLevel::Unrestricted,
            glob_scan_max_depth: None,
            entries: Vec::new(),
        }
    }

    pub fn external_filesystem_policy() -> Self {
        Self {
            kind: FileSystemAccessLevel::External,
            glob_scan_max_depth: None,
            entries: Vec::new(),
        }
    }

    pub fn restricted(entries: Vec<FileSystemAccessEntry>) -> Self {
        Self {
            kind: FileSystemAccessLevel::Restricted,
            glob_scan_max_depth: None,
            entries,
    ...
    /// Convert a legacy coarse access-policy name to a `FileSystemAccessPolicy`
    /// resolved against the provided cwd.
    pub fn from_legacy_access_policy_for_cwd(name: &str, _cwd: &Path) -> Self {
        match name {
            "read-only" => Self::restricted(vec![FileSystemAccessEntry {
                path: FileSystemPath::Special {
                    value: FileSystemSpecialPath::Root,
                },
                access: FileSystemAccessMode::Read,
            }]),
            // Match legacy workspace-write: writable under `:project_roots` (the active cwd at
            // check time), without implicitly granting platform temp dirs when excluded here.
            "workspace-write" => Self::workspace_write(
                &[],
                /*exclude_tmpdir_env_var*/ true,
                /*exclude_slash_tmp*/ true,
            ),
            "danger-full-access" => Self::unrestricted(),
            "external-access" => Self::external_filesystem_policy(),
            _ => Self::unrestricted(),
        }
    }

    fn has_root_access(&self, predicate: impl Fn(FileSystemAccessMode) -> bool) -> bool {
        matches!(self.kind, FileSystemAccessLevel::Restricted)
    ...
    pub fn preserve_deny_read_restrictions_from(&mut self, existing: &Self) {
        let has_deny_read_entries = existing
            ...
            .iter()
            .any(|entry| entry.access == FileSystemAccessMode::None);
        if matches!(self.kind, FileSystemAccessLevel::Unrestricted) && has_deny_read_entries {
            *self = Self::restricted(vec![FileSystemAccessEntry {
                path: FileSystemPath::Special {
                    value: FileSystemSpecialPath::Root,
                },
        ...
        if !matches!(self.kind, FileSystemAccessLevel::Restricted) {
            return;
        }
    ...
    fn has_same_target_write_override(&self, entry: &FileSystemAccessEntry) -> bool {
    ...
    /// Filesystem policy matching `WorkspaceWrite` semantics.
    pub fn workspace_write(
        writable_roots: &[AbsolutePathBuf],
        exclude_tmpdir_env_var: bool,
        exclude_slash_tmp: bool,
    ) -> Self {
        let mut entries = vec![FileSystemAccessEntry {
            path: FileSystemPath::Special {
                value: FileSystemSpecialPath::Root,
            },
            access: FileSystemAccessMode::Read,
        ...
        entries.push(FileSystemAccessEntry {
            path: FileSystemPath::Special {
        ...
        entries.extend(
            writable_roots
                .iter()
                .cloned()
                .map(|path| FileSystemAccessEntry {
        ...
        for writable_root in writable_roots {
            for protected_path in default_read_only_subpaths_for_writable_root(
                writable_root,
                /*protect_missing_dot_agere*/ false,
            ) {
                append_default_read_only_path_if_no_explicit_rule(&mut entries, protected_path);
            }
        }

        FileSystemAccessPolicy::restricted(entries)
    }

    /// Returns true when filesystem reads are unrestricted.
    pub fn has_full_disk_read_access(&self) -> bool {
        match self.kind {
            FileSystemAccessLevel::Unrestricted | FileSystemAccessLevel::External => true,
            FileSystemAccessLevel::Restricted => {
    ...
    pub fn with_additional_legacy_workspace_writable_roots(
        ...
        for path in additional_writable_roots {
            ...
            for protected_path in default_read_only_subpaths_for_writable_root(
                path, /*protect_missing_dot_agere*/ false,
            ) {
    ...
    pub fn get_writable_roots_with_cwd(&self, cwd: &Path) -> Vec<WritableRoot> {
        ...
        .map(|root| {
            ...
            // downstream isolation backends can still bind the real target while
            ...
            let protect_missing_dot_agere = AbsolutePathBuf::from_absolute_path(cwd)
                .ok()
                .is_some_and(|cwd| normalize_effective_absolute_path(cwd) == root);
            let mut read_only_subpaths: Vec<AbsolutePathBuf> =
                default_read_only_subpaths_for_writable_root(&root, protect_missing_dot_agere)
                    .into_iter()
            ...
            // so downstream isolation backends can still mask the symlink inode itself.
            // Example: if `<root>/.agere -> <root>/decoy`, bwrap must still see
            // `<root>/.agere`, not only the resolved `<root>/decoy`.
            read_only_subpaths.extend(
                resolved_entries
                    ...
                    .filter_map(|entry| {
                        ...
                        // or escape outside it. Isolation backends need that
            ...
            WritableRoot {
                ...
                // `.agere` so isolation backends can still detect and mask
    ...
    pub fn get_unreadable_roots_with_cwd(&self, cwd: &Path) -> Vec<AbsolutePathBuf> {
        ...
        dedup_absolute_paths(
            self.resolved_entries_with_cwd(cwd)
                ...
                // readable carveouts when isolation helpers apply deny masks last.
    ...
    /// Returns whether the policy requires direct runtime enforcement rather than
    /// being fully expressible as coarse legacy labels like `read-only`. Managed
    /// enforcement always consults split policies directly, so this returns `true`.
    pub fn needs_direct_runtime_enforcement(
        &self,
        _network_policy: NetworkAccessPolicy,
        _cwd: &Path,
    ) -> bool {
        true
    }
...
pub(crate) fn default_read_only_subpaths_for_writable_root(
    ...
    protect_missing_dot_agere: bool,
) -> Vec<AbsolutePathBuf> {
    ...
    // Keep top-level project metadata under .agere read-only to the agent by
    ...
    let top_level_agere = writable_root.join(PROTECTED_METADATA_AGERE_PATH_NAME);
    if protect_missing_dot_agere || top_level_agere.as_path().is_dir() {
        subpaths.push(top_level_agere);
    }
...
fn append_default_read_only_project_root_subpath_if_no_explicit_rule(
    entries: &mut Vec<FileSystemAccessEntry>,
...
mod tests {
    ...
    #[cfg(unix)]
    const SYMLINKED_TMPDIR_TEST_ENV: &str = "AGERE_PROTOCOL_TEST_SYMLINKED_TMPDIR";
    ...
    #[cfg(unix)]
    #[test]
    fn writable_roots_proactively_protect_missing_dot_agere() {
        let cwd = TempDir::new().expect("tempdir");
        let expected_root = AbsolutePathBuf::from_absolute_path(
            cwd.path().canonicalize().expect("canonicalize cwd"),
        )
        .expect("absolute canonical root");
        let expected_dot_agere = expected_root.join(".openagere");

        let policy = FileSystemAccessPolicy::restricted(vec![FileSystemAccessEntry {
            path: FileSystemPath::Special {
                value: FileSystemSpecialPath::project_roots(/*subpath*/ None),
            },
            access: FileSystemAccessMode::Write,
    ...
    #[cfg(unix)]
    #[test]
    fn writable_roots_skip_default_dot_agere_when_explicit_user_rule_exists() {
        let cwd = TempDir::new().expect("tempdir");
        let expected_root = AbsolutePathBuf::from_absolute_path(
            cwd.path().canonicalize().expect("canonicalize cwd"),
        )
        .expect("absolute canonical root");
        let explicit_dot_agere = expected_root.join(".openagere");

        let policy = FileSystemAccessPolicy::restricted(vec![
        ...
        assert!(
            !workspace_root
                .protected_metadata_names
                .contains(&".openagere".to_string()),
            "explicit .agere rule should remove the metadata-name protection"
        );
        assert!(
            !workspace_root
                .read_only_subpaths
                .contains(&explicit_dot_agere),
            "explicit .agere rule should win over the default protected carveout"
        );
        assert!(
            policy.can_write_path_with_cwd(
                explicit_dot_agere.join("config.toml").as_path(),
                cwd.path()
            )
        );
    }
    ...
    fn filesystem_policy_blocks_protected_metadata_path_writes_by_default() {
        ...
        let dot_agere_config = cwd.path().join(".openagere").join("config.toml");
        let root = AbsolutePathBuf::from_absolute_path(cwd.path()).expect("absolute cwd");
        let file_system_policy = FileSystemAccessPolicy::restricted(vec![FileSystemAccessEntry {
        ...
        assert!(!file_system_policy.can_write_path_with_cwd(&dot_agere_config, cwd.path()));
        ...
        assert!(!writable_roots[0].is_path_writable(&dot_agere_config));
    }
    ...
    fn effective_runtime_roots_preserve_symlinked_paths() {
        ...
        let agere_dir = real_root.join(".openagere");

        fs::create_dir_all(&blocked).expect("create blocked");
        fs::create_dir_all(&agere_dir).expect("create .agere");
        ...
        let expected_agere = link_root.join(".openagere");
    ...
    fn project_roots_special_path_preserves_symlinked_root() {
        ...
        let expected_agere = expected_root.join(".openagere");
    ...
    fn writable_roots_preserve_symlinked_protected_subpaths() {
        ...
        let decoy = root.join("decoy-agere");
        let dot_agere = root.join(".openagere");
        fs::create_dir_all(&decoy).expect("create decoy");
        symlink_dir(&decoy, &dot_agere).expect("create .agere symlink");

        let root = AbsolutePathBuf::from_absolute_path(&root).expect("absolute root");
        let expected_dot_agere = AbsolutePathBuf::from_absolute_path(
            root.as_path()
                .canonicalize()
                .expect("canonicalize root")
                .join(".openagere"),
        )
        .expect("absolute .agere symlink");
    ...
    fn tmpdir_special_path_preserves_symlinked_tmpdir() {
        ...
        let agere_dir = real_tmpdir.join(".openagere");
    ...
    fn preserving_deny_entries_keeps_unrestricted_policy_enforceable() {
        ...
        let mut existing = FileSystemAccessPolicy::restricted(vec![deny_entry.clone()]);
        existing.glob_scan_max_depth = Some(2);
        let mut replacement = FileSystemAccessPolicy::unrestricted();

        replacement.preserve_deny_read_restrictions_from(&existing);

        let mut expected = FileSystemAccessPolicy::restricted(vec![
            FileSystemAccessEntry {
                path: FileSystemPath::Special {
                    value: FileSystemSpecialPath::Root,
                },
    ...
    fn deny_policy(path: &Path) -> FileSystemAccessPolicy {
    ...
    fn unreadable_glob_entry(pattern: String) -> FileSystemAccessEntry {
        FileSystemAccessEntry {
    ...
    fn default_policy_with_unreadable_glob(pattern: String) -> FileSystemAccessPolicy {
        let mut policy = FileSystemAccessPolicy::default();
    ...
    fn is_read_denied(
        ...
        ReadDenyMatcher::new(file_system_access_policy, cwd)
