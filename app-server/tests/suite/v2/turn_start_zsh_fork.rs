...
async fn turn_start_shell_zsh_fork_executes_command_v2() -> Result<()> {
    ...
    let mut mcp = create_zsh_test_mcp_process(&agere_home, &workspace).await?;
...
async fn turn_start_shell_zsh_fork_subcommand_decline_marks_parent_declined_v2() -> Result<()> {
    ...
    let turn_id = mcp
        .send_turn_start_request(TurnStartParams {
            ...
            cwd: Some(workspace.clone()),
            approval_policy: Some(agere_app_server_protocol::AskForApproval::UnlessTrusted),
    ...
    match parent_completed_command_execution {
            ...
            if let Some(output) = aggregated_output.as_deref() {
                assert!(
                    output == "exec command rejected by user"
                        || output.contains("restricted execution"),
        ...
        Err(_) => {
            ...
            // exec failures can also complete the turn before the parent
...
async fn create_zsh_test_mcp_process(agere_home: &Path, zdotdir: &Path) -> Result<McpProcess> {
    let zdotdir = zdotdir.to_string_lossy().into_owned();
    McpProcess::new_with_env(agere_home, &[("ZDOTDIR", Some(zdotdir.as_str()))]).await
}
...
fn find_test_zsh_path() -> Result<Option<std::path::PathBuf>> {
    ...
    let dotslash_zsh = repo_root.join("app-server/tests/suite/zsh");
