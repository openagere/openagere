...
impl App {
    pub(super) async fn handle_event(
        ...
        match event {
            ...
            AppEvent::SwitchProvider { name } => {
                self.handle_switch_provider(app_server, name).await;
            }
            ...
            AppEvent::StartGrepSearch(query) => {
                let query_clone = query;
                let tx = self.app_event_tx.clone();
                std::thread::spawn(move || {
                    let matches = crate::external_rg::run_external_rg(&query_clone, ".");
                    let matches = matches.unwrap_or_default();
                    tx.send(AppEvent::GrepSearchResult {
                        query: query_clone,
                        matches,
                    });
                });
            }
            AppEvent::GrepSearchResult { query, matches } => {
                self.chat_widget.apply_grep_search_result(&query, matches);
            }
            ...
            AppEvent::OpenWindowsRestrictionEnablePrompt { preset } => {
                self.chat_widget
                    .open_windows_restriction_enable_prompt(preset);
            }
            AppEvent::OpenWindowsRestrictionFallbackPrompt { preset } => {
                self.session_telemetry.counter(
                    "agere.windows_restriction.fallback_prompt_shown",
                    /*inc*/ 1,
                    &[],
                );
                self.chat_widget.clear_windows_restriction_setup_status();
                if let Some(started_at) = self.windows_restriction.setup_started_at.take() {
                    self.session_telemetry.record_duration(
                        "agere.windows_restriction.elevated_setup_duration_ms",
                        started_at.elapsed(),
                        &[("result", "failure")],
                    );
                }
                self.chat_widget
                    .open_windows_restriction_fallback_prompt(preset);
            }
            AppEvent::BeginWindowsRestrictionElevatedSetup { preset } => {
                let _ = preset;
            }
            AppEvent::BeginWindowsRestrictionLegacySetup { preset } => {
                let _ = preset;
            }
            AppEvent::BeginWindowsRestrictionGrantReadRoot { path } => {
                let _ = path;
            }
            AppEvent::WindowsRestrictionGrantReadRootCompleted { path, error } => match error {
                Some(err) => {
                    self.chat_widget
                ...
                None => {
                    self.chat_widget
                        .add_to_history(history_cell::new_info_event(
                            format!("Read access granted for {}", path.display()),
                            /*hint*/ None,
                        ));
                }
            },
            AppEvent::EnableWindowsRestrictionForAgentMode { preset, mode } => {
                #[cfg(target_os = "windows")]
                {
                    ...
                    let elevated_enabled = matches!(mode, WindowsRestrictionEnableMode::Elevated);
                    let builder = ConfigEditsBuilder::new(&self.config.agere_home)
                        .with_profile(profile)
                        .set_windows_access_mode(if elevated_enabled {
                            "elevated"
                        } else {
                            "unelevated"
                        });
                    match builder.apply().await {
                        Ok(()) => {
                            if elevated_enabled {
                                self.config
                                    .set_windows_unelevated_access_mode(/*enabled*/ false);
                                self.config
                                    .set_windows_elevated_access_mode(/*enabled*/ true);
                            } else {
                                self.config
                                    .set_windows_unelevated_access_mode(/*enabled*/ true);
                                self.config
                                    .set_windows_elevated_access_mode(/*enabled*/ false);
                            }
                            self.chat_widget.set_windows_access_mode(
                                ...
                                self.chat_widget.world_writable_warning_details()
                            {
                                ...
                                self.chat_widget.add_plain_history_lines(vec![
                                    Line::from(vec!["• ".dim(), "Windows access ready".into()]),
                                    Line::from(vec![
                                        "  ".into(),
                                        "Agere can now safely edit files and execute commands in your computer"
                                            .dark_gray(),
                                    ]),
                                ]);
                            }
                        ...
                        Err(err) => {
                            tracing::error!(
                                error = %err,
                                "failed to enable Windows access layer"
                            );
                            self.chat_widget.add_error_message(format!(
                                "Failed to enable Windows access restrictions: {err}"
                            ));
                        }
                    }
                }
            ...
            AppEvent::PersistModelSelection { model, effort } => {
                ...
                match ConfigEditsBuilder::new(&self.config.agere_home)
                    .with_profile(profile)
                    .set_model(Some(model.as_str()), effort, context_window)
                    .apply()
                    .await
                {
                    Ok(()) => {
            ...
            AppEvent::PersistServiceTierSelection { service_tier } => {
                ...
                let mut edits = ConfigEditsBuilder::new(&self.config.agere_home)
                    .with_profile(profile)
            ...
            AppEvent::UpdatePermissionProfile(permission_profile) => {
                ...
                let permission_profile_is_managed_restricted =
                    managed_filesystem_access_is_restricted(&permission_profile);
                ...
                // If managed filesystem access is restricted on Windows, run the
                // world-writable scan.
                #[cfg(target_os = "windows")]
                {
                    // One-shot suppression if the user just confirmed continue.
                    if self.windows_restriction.skip_world_writable_scan_once {
                        self.windows_restriction.skip_world_writable_scan_once = false;
                        return Ok(AppRunControl::Continue);
                    }

                    let should_check = self.config.permissions.windows_access_mode.is_some()
                    ...
                    if should_check {
                        ...
                        let tx = self.app_event_tx.clone();
                        let logs_base_dir = self.config.agere_home.clone();
            ...
            AppEvent::SkipNextWorldWritableScan => {
                self.windows_restriction.skip_world_writable_scan_once = true;
            }
            ...
            AppEvent::StatusLineSetup { items } => {
                ...
                let apply_result = ConfigEditsBuilder::new(&self.config.agere_home)
                    .with_edits([edit])
                    .apply()
                    .await;
                match apply_result {
            ...
            AppEvent::ProviderTemplatesLoaded { templates } => {
                self.chat_widget
