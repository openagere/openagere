...
pub(crate) const PACKAGE_URL: &str = "https://registry.npmjs.org/openagere";
...
impl NpmPackageInfo {
    /// Returns the version for a given dist-tag (e.g., "latest").
    pub(crate) fn dist_tag(&self, name: &str) -> Option<&str> {
        self.dist_tags.get(name).map(String::as_str)
    }
}
    ...
    fn version_json(version: &str) -> serde_json::Value {
        serde_json::json!({
            "dist": {
                ...
                "tarball": format!("https://registry.npmjs.org/openagere/-/openagere-{version}.tgz"),
            }
        })
    }
