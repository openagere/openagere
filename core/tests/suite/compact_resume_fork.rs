...
//! request payload that Agere would send to the model and assert that the
...
use agere_core::spawn::AGERE_NETWORK_DISABLED_ENV_VAR;
...
fn network_disabled() -> bool {
    std::env::var(AGERE_NETWORK_DISABLED_ENV_VAR).is_ok()
}
...
async fn compact_resume_and_fork_preserve_model_history_view() {
    if network_disabled() {
        println!("Skipping test because network is disabled in this execution environment");
        return;
    }
...
async fn start_test_conversation(
    server: &MockServer,
    model: Option<&str>,
) -> (Arc<TempDir>, Config, Arc<ThreadManager>, Arc<AgereThread>) {
    ...
    (test.home, test.config, test.thread_manager, test.agere)
}

async fn user_turn(conversation: &Arc<AgereThread>, text: &str) {
    conversation
        .submit(Op::UserInput {
            environments: None,
...
async fn compact_conversation(conversation: &Arc<AgereThread>) {
    conversation
        .submit(Op::Compact)
        .await
...
fn fetch_conversation_path(conversation: &Arc<AgereThread>) -> std::path::PathBuf {
...
async fn resume_conversation(
    ...
    let auth_manager = agere_core::test_support::auth_manager_from_auth(
        agere_login::AgereAuth::from_api_key("dummy"),
    );
