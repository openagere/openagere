...
fn create_config_toml_custom_provider(
    ...
    let requires_line = if requires_provider_auth {
...
fn create_config_toml_forced_login(agere_home: &Path, forced_method: &str) -> std::io::Result<()> {
    let config_toml = agere_home.join("config.toml");
    let contents = format!(
        r#"
model = "mock-model"
...
async fn get_auth_status_with_api_key_when_auth_not_required() -> Result<()> {
    let agere_home = TempDir::new()?;
    create_config_toml_custom_provider(agere_home.path(), /*requires_provider_auth*/ false)?;
    ...
    assert_eq!(
        ...
        Some(false),
        "requires_provider_auth should be false",
    );
    Ok(())
}
...
async fn login_api_key_rejected_when_forced_chatgpt() -> Result<()> {
    let agere_home = TempDir::new()?;
    create_config_toml_forced_login(agere_home.path(), "chatgpt")?;
