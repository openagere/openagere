//! The main Agere TUI chat surface.
//!
...
use crate::terminal_info::Multiplexer;
use crate::terminal_info::TerminalInfo;
use crate::terminal_info::TerminalName;
use crate::terminal_info::terminal_info;
use crate::terminal_title::SetTerminalTitleResult;
use crate::terminal_title::clear_terminal_title;
use crate::terminal_title::set_terminal_title;
use crate::text_formatting::proper_join;
use crate::version::AGERE_CLI_VERSION;
use agere_app_server_protocol::AddCreditsNudgeCreditType;
use agere_app_server_protocol::AddCreditsNudgeEmailStatus;
use agere_app_server_protocol::AgereErrorInfo as AppServerAgereErrorInfo;
use agere_app_server_protocol::AppInfo;
use agere_app_server_protocol::AppSummary;
use agere_app_server_protocol::CollabAgentState as AppServerCollabAgentState;
use agere_app_server_protocol::CollabAgentStatus as AppServerCollabAgentStatus;
use agere_app_server_protocol::CollabAgentTool;
use agere_app_server_protocol::CollabAgentToolCallStatus;
use agere_app_server_protocol::CommandExecutionRequestApprovalParams;
use agere_app_server_protocol::ConfigLayerSource;
use agere_app_server_protocol::ErrorNotification;
use agere_app_server_protocol::FileChangeRequestApprovalParams;
use agere_app_server_protocol::GuardianApprovalReviewAction;
use agere_app_server_protocol::ItemCompletedNotification;
use agere_app_server_protocol::ItemStartedNotification;
use agere_app_server_protocol::McpServerStartupState;
use agere_app_server_protocol::McpServerStatusDetail;
use agere_app_server_protocol::McpServerStatusUpdatedNotification;
use agere_app_server_protocol::ModelVerification as AppServerModelVerification;
use agere_app_server_protocol::ServerNotification;
use agere_app_server_protocol::ServerRequest;
use agere_app_server_protocol::ThreadGoal as AppThreadGoal;
use agere_app_server_protocol::ThreadGoalStatus as AppThreadGoalStatus;
use agere_app_server_protocol::ThreadItem;
use agere_app_server_protocol::ThreadTokenUsage;
...
use agere_app_server_protocol::TurnPlanStepStatus;
use agere_app_server_protocol::TurnStatus;
use agere_config::ConfigLayerStackOrdering;
use agere_config::types::ApprovalsReviewer;
use agere_config::types::Notifications;
use agere_config::types::WindowsAccessModeToml;
use agere_core_skills::model::SkillMetadata;
use agere_features::FEATURES;
use agere_features::Feature;
#[cfg(test)]
use agere_git_utils::CommitLogEntry;
use agere_git_utils::current_branch_name;
use agere_git_utils::get_git_repo_root;
use agere_git_utils::local_git_branches;
use agere_git_utils::recent_commits;
use agere_model_provider_info::WireApi;
use agere_otel::RuntimeMetricsSummary;
use agere_otel::SessionTelemetry;
use agere_plugin::PluginCapabilitySummary;
use agere_protocol::ThreadId;
use agere_protocol::account::PlanType;
use agere_protocol::approvals::ElicitationRequestEvent;
...
use agere_protocol::items::AgentMessageContent;
use agere_protocol::items::AgentMessageItem;
use agere_protocol::items::UserMessageItem;
use agere_protocol::models::MessagePhase;
use agere_protocol::models::local_image_label_text;
use agere_protocol::parse_command::ParsedCommand;
use agere_protocol::plan_tool::PlanItemArg as UpdatePlanItemArg;
use agere_protocol::plan_tool::StepStatus as UpdatePlanItemStatus;
#[cfg(test)]
use agere_protocol::protocol::AgentMessageDeltaEvent;
#[cfg(test)]
use agere_protocol::protocol::AgentMessageEvent;
#[cfg(test)]
use agere_protocol::protocol::AgentReasoningDeltaEvent;
#[cfg(test)]
use agere_protocol::protocol::AgentReasoningEvent;
#[cfg(test)]
use agere_protocol::protocol::AgentReasoningRawContentDeltaEvent;
#[cfg(test)]
use agere_protocol::protocol::AgentReasoningRawContentEvent;
use agere_protocol::protocol::AgentStatus;
#[cfg(test)]
use agere_protocol::protocol::AgereErrorInfo as CoreAgereErrorInfo;
use agere_protocol::protocol::ApplyPatchApprovalRequestEvent;
#[cfg(test)]
use agere_protocol::protocol::BackgroundEventEvent;
use agere_protocol::protocol::CollabAgentRef;
#[cfg(test)]
use agere_protocol::protocol::CollabAgentSpawnBeginEvent;
use agere_protocol::protocol::CollabAgentStatusEntry;
use agere_protocol::protocol::CreditsSnapshot;
use agere_protocol::protocol::DeprecationNoticeEvent;
#[cfg(test)]
use agere_protocol::protocol::ErrorEvent;
#[cfg(test)]
use agere_protocol::protocol::Event;
use agere_protocol::protocol::EventMsg;
use agere_protocol::protocol::ExecApprovalRequestEvent;
use agere_protocol::protocol::ExecCommandBeginEvent;
use agere_protocol::protocol::ExecCommandEndEvent;
use agere_protocol::protocol::ExecCommandOutputDeltaEvent;
use agere_protocol::protocol::ExecCommandSource;
#[cfg(test)]
use agere_protocol::protocol::ExitedReviewModeEvent;
use agere_protocol::protocol::GuardianAssessmentAction;
use agere_protocol::protocol::GuardianAssessmentDecisionSource;
use agere_protocol::protocol::GuardianAssessmentEvent;
use agere_protocol::protocol::GuardianAssessmentStatus;
use agere_protocol::protocol::ImageGenerationBeginEvent;
use agere_protocol::protocol::ImageGenerationEndEvent;
use agere_protocol::protocol::ListSkillsResponseEvent;
#[cfg(test)]
use agere_protocol::protocol::McpListToolsResponseEvent;
#[cfg(test)]
use agere_protocol::protocol::McpStartupCompleteEvent;
use agere_protocol::protocol::McpStartupStatus;
#[cfg(test)]
use agere_protocol::protocol::McpStartupUpdateEvent;
use agere_protocol::protocol::McpToolCallBeginEvent;
use agere_protocol::protocol::McpToolCallEndEvent;
#[cfg(test)]
use agere_protocol::protocol::ModelVerification as CoreModelVerification;
use agere_protocol::protocol::Op;
use agere_protocol::protocol::PatchApplyBeginEvent;
use agere_protocol::protocol::RateLimitReachedType;
...
use agere_protocol::protocol::SkillMetadata as ProtocolSkillMetadata;
#[cfg(test)]
use agere_protocol::protocol::StreamErrorEvent;
use agere_protocol::protocol::TerminalInteractionEvent;
#[cfg(test)]
use agere_protocol::protocol::ThreadGoalStatus as ProtocolThreadGoalStatus;
...
#[cfg(test)]
use agere_protocol::protocol::TurnCompleteEvent;
#[cfg(test)]
use agere_protocol::protocol::TurnDiffEvent;
#[cfg(test)]
use agere_protocol::protocol::UndoCompletedEvent;
#[cfg(test)]
use agere_protocol::protocol::UndoStartedEvent;
use agere_protocol::protocol::UserMessageEvent;
use agere_protocol::protocol::ViewImageToolCallEvent;
#[cfg(test)]
use agere_protocol::protocol::WarningEvent;
use agere_protocol::protocol::WebSearchBeginEvent;
use agere_protocol::protocol::WebSearchEndEvent;
...
use agere_utils_sleep_inhibitor::SleepInhibitor;
...
const OPENAI_TRUSTED_ACCESS_FOR_CYBER_VERIFICATION_WARNING: &str = "Your conversations have multiple flags for possible cybersecurity risk. Responses may take longer because extra safety checks are on. To get authorized for security work, join the Trusted Access for Cyber program: https://chatgpt.com/cyber";
const GENERIC_PROVIDER_CYBER_SAFETY_VERIFICATION_WARNING: &str = "The model provider reported repeated cybersecurity-risk safety checks. Responses may take longer while additional checks run. If this seems wrong, check this provider's safety, access, or routing policy.";
const MEMORIES_DOC_URL: &str = "https://developers.openagere.com/agere/memories";
...
use crate::app_event::WindowsRestrictionEnableMode;
...
use agere_protocol::plan_tool::StepStatus;
use agere_protocol::plan_tool::UpdatePlanArgs;
use agere_protocol::protocol::AskForApproval;
use agere_utils_approval_presets::ApprovalPreset;
use agere_utils_approval_presets::builtin_approval_presets;
use chrono::Local;
...
pub(crate) struct ChatWidgetInit {
    ...
    pub(crate) feedback: agere_feedback::AgereFeedback,
...
#[cfg(test)]
fn core_rate_limit_error_kind(info: &CoreAgereErrorInfo) -> Option<RateLimitErrorKind> {
    match info {
        CoreAgereErrorInfo::ServerOverloaded => Some(RateLimitErrorKind::ServerOverloaded),
        CoreAgereErrorInfo::UsageLimitExceeded => Some(RateLimitErrorKind::UsageLimit),
        CoreAgereErrorInfo::ResponseTooManyFailedAttempts {
...
fn app_server_rate_limit_error_kind(info: &AppServerAgereErrorInfo) -> Option<RateLimitErrorKind> {
    match info {
        AppServerAgereErrorInfo::ServerOverloaded => Some(RateLimitErrorKind::ServerOverloaded),
        AppServerAgereErrorInfo::UsageLimitExceeded => Some(RateLimitErrorKind::UsageLimit),
        AppServerAgereErrorInfo::ResponseTooManyFailedAttempts {
            http_status_code: Some(429),
        } => Some(RateLimitErrorKind::Generic),
        _ => None,
    }
...
#[cfg(test)]
fn is_core_cyber_policy_error(info: &CoreAgereErrorInfo) -> bool {
    matches!(info, CoreAgereErrorInfo::CyberPolicy)
}

fn is_app_server_cyber_policy_error(info: &AppServerAgereErrorInfo) -> bool {
    matches!(info, AppServerAgereErrorInfo::CyberPolicy)
}
...
/// and by sending requests back to agere-core.
///
...
pub(crate) struct ChatWidget {
    ...
    /// Tracks whether agere-core currently considers an agent turn to be in progress.
    ///
    ...
    session_network_proxy: Option<agere_protocol::protocol::SessionNetworkProxyRuntime>,
    // Shared latch so we only warn once about invalid status-line item IDs.
...
) -> agere_protocol::protocol::SessionConfiguredEvent {
    agere_protocol::protocol::SessionConfiguredEvent {
...
fn hook_output_entry_from_notification(
    entry: agere_app_server_protocol::HookOutputEntry,
) -> agere_protocol::protocol::HookOutputEntry {
    agere_protocol::protocol::HookOutputEntry {
...
fn hook_run_summary_from_notification(
    run: agere_app_server_protocol::HookRunSummary,
) -> agere_protocol::protocol::HookRunSummary {
    agere_protocol::protocol::HookRunSummary {
        id: run.id,
...
fn hook_started_event_from_notification(
    notification: agere_app_server_protocol::HookStartedNotification,
) -> agere_protocol::protocol::HookStartedEvent {
    agere_protocol::protocol::HookStartedEvent {
...
fn hook_completed_event_from_notification(
    notification: agere_app_server_protocol::HookCompletedNotification,
) -> agere_protocol::protocol::HookCompletedEvent {
    agere_protocol::protocol::HookCompletedEvent {
        turn_id: notification.turn_id,
        run: hook_run_summary_from_notification(notification.run),
    }
...
fn app_server_request_id_to_mcp_request_id(
    request_id: &agere_app_server_protocol::RequestId,
) -> agere_protocol::mcp::RequestId {
    match request_id {
        agere_app_server_protocol::RequestId::String(value) => {
            agere_protocol::mcp::RequestId::String(value.clone())
        }
        agere_app_server_protocol::RequestId::Integer(value) => {
            agere_protocol::mcp::RequestId::Integer(*value)
        }
    }
}
...
fn exec_approval_request_from_params(
    ...
    ExecApprovalRequestEvent {
        ...
        proposed_execpolicy_amendment: params
            ...
            .map(agere_app_server_protocol::ExecPolicyAmendment::into_core),
        proposed_network_policy_amendments: params.proposed_network_policy_amendments.map(
            |amendments| {
                amendments
                    .into_iter()
                    .map(agere_app_server_protocol::NetworkPolicyAmendment::into_core)
                    .collect()
            },
        ),
        available_decisions: params.available_decisions.map(|decisions| {
            decisions
                ...
                .map(|decision| match decision {
                    agere_app_server_protocol::CommandExecutionApprovalDecision::Accept => {
                        agere_protocol::protocol::ReviewDecision::Approved
                    }
                    agere_app_server_protocol::CommandExecutionApprovalDecision::AcceptForSession => {
                        agere_protocol::protocol::ReviewDecision::ApprovedForSession
                    }
                    agere_app_server_protocol::CommandExecutionApprovalDecision::AcceptWithExecpolicyAmendment {
                        execpolicy_amendment,
                    } => agere_protocol::protocol::ReviewDecision::ApprovedExecpolicyAmendment {
                        proposed_execpolicy_amendment: execpolicy_amendment.into_core(),
                    },
                    agere_app_server_protocol::CommandExecutionApprovalDecision::ApplyNetworkPolicyAmendment {
                        network_policy_amendment,
                    } => agere_protocol::protocol::ReviewDecision::NetworkPolicyAmendment {
                        network_policy_amendment: network_policy_amendment.into_core(),
                    },
                    agere_app_server_protocol::CommandExecutionApprovalDecision::Decline => {
                        agere_protocol::protocol::ReviewDecision::Denied
                    }
                    agere_app_server_protocol::CommandExecutionApprovalDecision::Cancel => {
                        agere_protocol::protocol::ReviewDecision::Abort
                    }
                })
                .collect()
        }),
        parsed_cmd: params
            ...
            .unwrap_or_default()
            .into_iter()
            .map(agere_app_server_protocol::CommandAction::into_core)
            .collect(),
    }
}
...
fn request_permissions_from_params(
    params: agere_app_server_protocol::PermissionsRequestApprovalParams,
...
fn request_user_input_from_params(params: ToolRequestUserInputParams) -> RequestUserInputEvent {
    RequestUserInputEvent {
        ...
        questions: params
            ...
            .into_iter()
            .map(
                |question| agere_protocol::request_user_input::RequestUserInputQuestion {
                    id: question.id,
...
fn web_search_action_to_core(
    action: agere_app_server_protocol::WebSearchAction,
) -> agere_protocol::models::WebSearchAction {
    match action {
        agere_app_server_protocol::WebSearchAction::Search { query, queries } => {
            agere_protocol::models::WebSearchAction::Search { query, queries }
        }
        agere_app_server_protocol::WebSearchAction::OpenPage { url } => {
            agere_protocol::models::WebSearchAction::OpenPage { url }
        }
        agere_app_server_protocol::WebSearchAction::FindInPage { url, pattern } => {
            agere_protocol::models::WebSearchAction::FindInPage { url, pattern }
        }
        agere_app_server_protocol::WebSearchAction::Other => {
            agere_protocol::models::WebSearchAction::Other
        }
    }
}
...
impl ChatWidget {
    ...
    #[cfg(test)]
    fn on_session_configured(&mut self, event: agere_protocol::protocol::SessionConfiguredEvent) {
    ...
    fn on_session_configured_with_display_and_fork_parent_title(
        &mut self,
        event: agere_protocol::protocol::SessionConfiguredEvent,
        ...
        if let Some(provider) = self.config.model_providers.get(&event.model_provider_id) {
            ...
            self.session_header.set_provider(&event.model_provider_id);
        } else {
            tracing::warn!(
                model_provider_id = %event.model_provider_id,
                "session provider is missing from TUI config; keeping existing provider config"
            );
        }
    ...
    fn on_thread_name_updated(&mut self, event: agere_protocol::protocol::ThreadNameUpdatedEvent) {
    ...
    #[cfg(test)]
    fn handle_steer_rejected_error(&mut self, agere_error_info: &CoreAgereErrorInfo) -> bool {
        matches!(
            agere_error_info,
            CoreAgereErrorInfo::ActiveTurnNotSteerable { .. }
        ) && self.enqueue_rejected_steer()
    }

    fn handle_app_server_steer_rejected_error(
        &mut self,
        agere_error_info: &AppServerAgereErrorInfo,
    ) -> bool {
        matches!(
            agere_error_info,
            AppServerAgereErrorInfo::ActiveTurnNotSteerable { .. }
        ) && self.enqueue_rejected_steer()
    }
    ...
    fn apply_token_info(&mut self, mut info: TokenUsageInfo) {
        if info.model_context_window.is_none()
            && let Some(cw) = self.config.model_context_window
        {
            info.model_context_window = Some(cw);
        }
        let percent = self.context_remaining_percent(&info);
        let used_tokens = self.context_used_tokens(&info, percent.is_some());
    ...
    pub(crate) fn on_rate_limit_snapshot(&mut self, snapshot: Option<RateLimitSnapshot>) {
        if let Some(mut snapshot) = snapshot {
            let limit_id = snapshot
                .limit_id
                .clone()
                .unwrap_or_else(|| "agere".to_string());
            ...
            let is_agere_limit = limit_id.eq_ignore_ascii_case("agere");
            if is_agere_limit
                && let Some(rate_limit_reached_type) = snapshot.rate_limit_reached_type
            {
                self.agere_rate_limit_reached_type = Some(rate_limit_reached_type);
            }
            let warnings = if is_agere_limit {
            ...
            self.agere_rate_limit_reached_type = None;
        }
        self.refresh_status_line();
    }
    ...
    fn on_server_overloaded_error(&mut self, message: String) {
        ...
        let message = if message.trim().is_empty() {
            "Agere is currently experiencing high load.".to_string()
        } else {
            message
        };
    ...
    fn on_rate_limit_error(&mut self, error_kind: RateLimitErrorKind, message: String) {
        ...
        let rate_limit_reached_type = self.agere_rate_limit_reached_type.map(|kind| {
        ...
        self.agere_rate_limit_reached_type = rate_limit_reached_type;

        match rate_limit_reached_type {
            Some(RateLimitReachedType::WorkspaceOwnerCreditsDepleted) => {
                self.on_error(
                    "You're out of credits. Your workspace is out of credits. Add credits to continue using Agere."
                        .to_string(),
                );
            }
            Some(RateLimitReachedType::WorkspaceOwnerUsageLimitReached) => {
                self.on_error(
                    "Usage limit reached. You've reached your usage limit. Increase your limits to continue using agere."
                        .to_string(),
                );
            }
    ...
    fn uses_openai_provider(&self) -> bool {
        self.config.model_provider_id == "openai" || self.config.model_provider.is_openai()
    }

    fn model_verification_warning(&self) -> &'static str {
        if self.uses_openai_provider() {
            OPENAI_TRUSTED_ACCESS_FOR_CYBER_VERIFICATION_WARNING
        } else {
            GENERIC_PROVIDER_CYBER_SAFETY_VERIFICATION_WARNING
        }
    }
    ...
    fn on_core_model_verification(&mut self, verifications: &[CoreModelVerification]) {
        if verifications.contains(&CoreModelVerification::TrustedAccessForCyber) {
            self.on_warning(self.model_verification_warning());
        }
    }
    ...
    fn on_guardian_assessment(&mut self, ev: GuardianAssessmentEvent) {
        ...
        if ev.status == GuardianAssessmentStatus::TimedOut {
            let cell = if let Some(command) = guardian_command(&ev.action) {
                history_cell::new_approval_decision_cell(
                    command,
                    agere_protocol::protocol::ReviewDecision::TimedOut,
                    history_cell::ApprovalDecisionActor::Guardian,
                )
            } else {
                match &ev.action {
                    ...
                    GuardianAssessmentAction::McpToolCall {
                        ...
                        "agere could call MCP tool {server}.{tool_name}"
                    )),
                    GuardianAssessmentAction::NetworkAccess { target, .. } => {
                        history_cell::new_guardian_timed_out_action_request(format!(
                            "agere could access {target}"
                        ))
                    }
                    GuardianAssessmentAction::RequestPermissions { reason, .. } => {
                        history_cell::new_guardian_timed_out_action_request(
                            permission_request_summary("agere could request permissions", reason),
                        )
                    }
        ...
        let cell = if let Some(command) = guardian_command(&ev.action) {
            ...
            match &ev.action {
                ...
                GuardianAssessmentAction::McpToolCall {
                    ...
                    "agere to call MCP tool {server}.{tool_name}"
                )),
                GuardianAssessmentAction::NetworkAccess { target, .. } => {
                    history_cell::new_guardian_denied_action_request(format!(
                        "agere to access {target}"
                    ))
                }
                GuardianAssessmentAction::RequestPermissions { reason, .. } => {
                    history_cell::new_guardian_denied_action_request(permission_request_summary(
                        "agere to request permissions",
                        reason,
                    ))
                }
                GuardianAssessmentAction::Command { .. } => unreachable!(),
    ...
    fn on_patch_apply_end(&mut self, event: agere_protocol::protocol::PatchApplyEndEvent) {
        let ev2 = event.clone();
        self.defer_or_handle(
            ...
            CollabAgentTool::SpawnAgent => {
                ...
                if !matches!(status, CollabAgentToolCallStatus::InProgress) {
                    ...
                    self.on_collab_event(multi_agents::spawn_end(
                        agere_protocol::protocol::CollabAgentSpawnEndEvent {
                            call_id: id,
                            sender_thread_id,
            ...
            CollabAgentTool::SendInput => {
                    ...
                    && !matches!(status, CollabAgentToolCallStatus::InProgress)
                {
                    self.on_collab_event(multi_agents::interaction_end(
                        agere_protocol::protocol::CollabAgentInteractionEndEvent {
                            call_id: id,
                            sender_thread_id,
                            receiver_thread_id,
                ...
                if let Some(receiver_thread_id) = first_receiver {
                    if matches!(status, CollabAgentToolCallStatus::InProgress) {
                        self.on_collab_event(multi_agents::resume_begin(
                            agere_protocol::protocol::CollabResumeBeginEvent {
                                call_id: id,
                                sender_thread_id,
                                receiver_thread_id,
                                receiver_agent_nickname: first_receiver_metadata
                        ...
                        self.on_collab_event(multi_agents::resume_end(
                            agere_protocol::protocol::CollabResumeEndEvent {
                                call_id: id,
                                sender_thread_id,
                                receiver_thread_id,
                                receiver_agent_nickname: first_receiver_metadata
            ...
            CollabAgentTool::Wait => {
                if matches!(status, CollabAgentToolCallStatus::InProgress) {
                    self.on_collab_event(multi_agents::waiting_begin(
                        agere_protocol::protocol::CollabWaitingBeginEvent {
                            sender_thread_id,
                    ...
                    self.on_collab_event(multi_agents::waiting_end(
                        agere_protocol::protocol::CollabWaitingEndEvent {
                            sender_thread_id,
                            call_id: id,
                            agent_statuses,
                            statuses,
            ...
            CollabAgentTool::CloseAgent => {
                    ...
                    && !matches!(status, CollabAgentToolCallStatus::InProgress)
                {
                    self.on_collab_event(multi_agents::close_end(
                        agere_protocol::protocol::CollabCloseEndEvent {
                            call_id: id,
                            sender_thread_id,
                            receiver_thread_id,
                            receiver_agent_nickname: first_receiver_metadata
    ...
    pub(crate) fn handle_history_entry_response(
        &mut self,
        event: agere_protocol::protocol::GetHistoryEntryResponseEvent,
    ) {
        let agere_protocol::protocol::GetHistoryEntryResponseEvent {
            offset,
            log_id,
            entry,
        } = event;
    ...
    fn on_hook_started(&mut self, event: agere_protocol::protocol::HookStartedEvent) {
        self.flush_answer_stream_with_separator();
    ...
    fn on_hook_completed(&mut self, event: agere_protocol::protocol::HookCompletedEvent) {
    ...
    /// When `is_interim` is true (tool calls are still queued), streaming
    /// animation is kept alive rather than finalized.
    fn on_agent_message_item_completed(&mut self, item: AgentMessageItem, is_interim: bool) {
        ...
        if !is_interim {
            ...
            // Interim message: consolidate stream content but keep animation running
    ...
    pub(crate) fn handle_patch_apply_end_now(
        &mut self,
        event: agere_protocol::protocol::PatchApplyEndEvent,
    ) {
    ...
    pub(crate) fn new_with_app_event(common: ChatWidgetInit) -> Self {
        Self::new_with_op_target(common, AgereOpTarget::AppEvent)
    }

    fn new_with_op_target(common: ChatWidgetInit, agere_op_target: AgereOpTarget) -> Self {
        let ChatWidgetInit {
            config,
            frame_requester,
            app_event_tx,
        ...
        let initial_provider_id = config.model_provider_id.clone();
        ...
        let mut widget = Self {
            ...
            model_catalog,
            session_telemetry,
            session_header: SessionHeader::new(initial_provider_id, header_model),
            initial_user_message,
            status_account_display,
            token_info: None,
        ...
        #[cfg(target_os = "windows")]
        widget.bottom_pane.set_windows_degraded_restriction_active(
            false
                && matches!(
                    widget.config.permissions.windows_access_mode,
    ...
    fn submit_user_message_with_history_and_shell_escape_policy(
        ...
        if let Some(apps) = self.connectors_for_mentions() {
            ...
            for app in app_mentions {
                let slug = agere_connectors::metadata::connector_mention_slug(&app);
    ...
    pub(crate) fn replay_thread_item(
        ...
        // Replay renders historical content; streaming animations are never needed.
        ...
        match item {
            ThreadItem::UserMessage { id, content } => {
                let user_message = UserMessageItem {
                    ...
                    content: content
                        .into_iter()
                        .map(agere_app_server_protocol::UserInput::into_core)
                        .collect(),
                };
                ...
                self.on_agent_message_item_completed(
                    AgentMessageItem {
                        ...
                        memory_citation: memory_citation.map(|citation| {
                            agere_protocol::memory_citation::MemoryCitation {
                                entries: citation
                                    .entries
                                    .into_iter()
                                    .map(|entry| {
                                        agere_protocol::memory_citation::MemoryCitationEntry {
                                            path: entry.path,
            ...
            ThreadItem::CommandExecution {
                ...
                if matches!(
                    status,
                    agere_app_server_protocol::CommandExecutionStatus::InProgress
                ) {
                    ...
                    self.on_exec_command_end(ExecCommandEndEvent {
                        ...
                        status: match status {
                            agere_app_server_protocol::CommandExecutionStatus::Completed => {
                                agere_protocol::protocol::ExecCommandStatus::Completed
                            }
                            agere_app_server_protocol::CommandExecutionStatus::Failed => {
                                agere_protocol::protocol::ExecCommandStatus::Failed
                            }
                            agere_app_server_protocol::CommandExecutionStatus::Declined => {
                                agere_protocol::protocol::ExecCommandStatus::Declined
                            }
                            agere_app_server_protocol::CommandExecutionStatus::InProgress => {
                                agere_protocol::protocol::ExecCommandStatus::Failed
                            }
            ...
            ThreadItem::FileChange {
                ...
                if !matches!(
                    status,
                    agere_app_server_protocol::PatchApplyStatus::InProgress
                ) {
                    self.on_patch_apply_end(agere_protocol::protocol::PatchApplyEndEvent {
                        call_id: id,
                        turn_id: turn_id.clone(),
                        stdout: String::new(),
                        stderr: String::new(),
                        success: !matches!(
                            status,
                            agere_app_server_protocol::PatchApplyStatus::Failed
                        ),
                        changes: file_update_changes_to_core(changes),
                        status: match status {
                            agere_app_server_protocol::PatchApplyStatus::Completed => {
                                agere_protocol::protocol::PatchApplyStatus::Completed
                            }
                            agere_app_server_protocol::PatchApplyStatus::Failed => {
                                agere_protocol::protocol::PatchApplyStatus::Failed
                            }
                            agere_app_server_protocol::PatchApplyStatus::Declined => {
                                agere_protocol::protocol::PatchApplyStatus::Declined
                            }
                            agere_app_server_protocol::PatchApplyStatus::InProgress => {
                                agere_protocol::protocol::PatchApplyStatus::Failed
                            }
            ...
            ThreadItem::McpToolCall {
                ...
                self.on_mcp_tool_call_end(agere_protocol::protocol::McpToolCallEndEvent {
                    call_id: id,
                    invocation: agere_protocol::protocol::McpInvocation {
                        server,
                        tool,
                    ...
                    result: match (result, error) {
                        ...
                        (Some(result), None) => {
                            let result = *result;
                            Ok(agere_protocol::mcp::CallToolResult {
            ...
            ThreadItem::WebSearch { id, query, action } => {
                ...
                self.on_web_search_end(WebSearchEndEvent {
                    ...
                    action: action
                        ...
                        .unwrap_or(agere_protocol::models::WebSearchAction::Other),
                });
            }
        ...
        match notification {
            ...
            ServerNotification::ThreadNameUpdated(notification) => {
                match ThreadId::from_string(&notification.thread_id) {
                    Ok(thread_id) => self.on_thread_name_updated(
                        agere_protocol::protocol::ThreadNameUpdatedEvent {
                            thread_id,
            ...
            ServerNotification::CommandExecutionOutputDelta(notification) => {
                self.on_exec_command_output_delta(ExecCommandOutputDeltaEvent {
                    call_id: notification.item_id,
                    stream: agere_protocol::protocol::ExecOutputStream::Stdout,
            ...
            ServerNotification::ThreadRealtimeStarted(notification) => {
                if !from_replay {
                    self.on_realtime_conversation_started(
                        agere_protocol::protocol::RealtimeConversationStartedEvent {
            ...
            ServerNotification::ThreadRealtimeItemAdded(notification) => {
                if !from_replay {
                    self.on_realtime_conversation_realtime(
                        agere_protocol::protocol::RealtimeConversationRealtimeEvent {
                            payload: agere_protocol::protocol::RealtimeEvent::ConversationItemAdded(
                                notification.item,
                            ),
                        },
                    );
            ...
            ServerNotification::ThreadRealtimeOutputAudioDelta(notification) => {
                if !from_replay {
                    self.on_realtime_conversation_realtime(
                        agere_protocol::protocol::RealtimeConversationRealtimeEvent {
                            payload: agere_protocol::protocol::RealtimeEvent::AudioOut(
            ...
            ServerNotification::ThreadRealtimeError(notification) => {
                if !from_replay {
                    self.on_realtime_conversation_realtime(
                        agere_protocol::protocol::RealtimeConversationRealtimeEvent {
                            payload: agere_protocol::protocol::RealtimeEvent::Error(
            ...
            ServerNotification::ThreadRealtimeClosed(notification) => {
                if !from_replay {
                    self.on_realtime_conversation_closed(
                        agere_protocol::protocol::RealtimeConversationClosedEvent {
                            reason: notification.reason,
                        },
                    );
                }
    ...
    fn on_mcp_server_elicitation_request(
        &mut self,
        request_id: agere_protocol::mcp::RequestId,
        params: agere_app_server_protocol::McpServerElicitationRequestParams,
    ) {
        let request = agere_protocol::approvals::ElicitationRequestEvent {
            turn_id: params.turn_id,
            server_name: params.server_name,
            id: request_id,
            request: match params.request {
                agere_app_server_protocol::McpServerElicitationRequest::Form {
                    meta,
                    message,
                    requested_schema,
                } => agere_protocol::approvals::ElicitationRequest::Form {
                    meta,
                    message,
                    requested_schema: serde_json::to_value(requested_schema)
                        .unwrap_or(serde_json::Value::Null),
                },
                agere_app_server_protocol::McpServerElicitationRequest::Url {
                    meta,
                    message,
                    url,
                    elicitation_id,
                } => agere_protocol::approvals::ElicitationRequest::Url {
                    meta,
                    message,
                    url,
                    elicitation_id,
            ...
            TurnStatus::Failed => {
                if let Some(error) = notification.turn.error {
                        ...
                        == Some(&(notification.turn.id.clone(), error.message.clone()))
                    {
                        ...
                        self.handle_non_retry_error(error.message, error.agere_error_info);
                    }
                } else {
                    self.last_non_retry_error = None;
                    self.finalize_turn();
    ...
    fn on_guardian_review_notification(
        ...
        id: String,
        turn_id: String,
        review: agere_app_server_protocol::GuardianApprovalReview,
        decision_source: Option<agere_app_server_protocol::AutoReviewDecisionSource>,
        ...
        self.on_guardian_assessment(GuardianAssessmentEvent {
            ...
            status: match review.status {
                agere_app_server_protocol::GuardianApprovalReviewStatus::InProgress => {
                    GuardianAssessmentStatus::InProgress
                }
                agere_app_server_protocol::GuardianApprovalReviewStatus::Approved => {
                    GuardianAssessmentStatus::Approved
                }
                agere_app_server_protocol::GuardianApprovalReviewStatus::Denied => {
                    GuardianAssessmentStatus::Denied
                }
                agere_app_server_protocol::GuardianApprovalReviewStatus::TimedOut => {
                    GuardianAssessmentStatus::TimedOut
                }
                agere_app_server_protocol::GuardianApprovalReviewStatus::Aborted => {
                    GuardianAssessmentStatus::Aborted
                }
            },
            risk_level: review.risk_level.map(|risk_level| match risk_level {
                agere_app_server_protocol::GuardianRiskLevel::Low => {
                    agere_protocol::protocol::GuardianRiskLevel::Low
                }
                agere_app_server_protocol::GuardianRiskLevel::Medium => {
                    agere_protocol::protocol::GuardianRiskLevel::Medium
                }
                agere_app_server_protocol::GuardianRiskLevel::High => {
                    agere_protocol::protocol::GuardianRiskLevel::High
                }
                agere_app_server_protocol::GuardianRiskLevel::Critical => {
                    agere_protocol::protocol::GuardianRiskLevel::Critical
                }
            }),
            user_authorization: review.user_authorization.map(|user_authorization| {
                match user_authorization {
                    agere_app_server_protocol::GuardianUserAuthorization::Unknown => {
                        agere_protocol::protocol::GuardianUserAuthorization::Unknown
                    }
                    agere_app_server_protocol::GuardianUserAuthorization::Low => {
                        agere_protocol::protocol::GuardianUserAuthorization::Low
                    }
                    agere_app_server_protocol::GuardianUserAuthorization::Medium => {
                        agere_protocol::protocol::GuardianUserAuthorization::Medium
                    }
                    agere_app_server_protocol::GuardianUserAuthorization::High => {
                        agere_protocol::protocol::GuardianUserAuthorization::High
                    }
                }
            }),
            rationale: review.rationale,
            decision_source: decision_source.map(|source| match source {
                agere_app_server_protocol::AutoReviewDecisionSource::Agent => {
    ...
    #[cfg(test)]
    pub(crate) fn handle_agere_event(&mut self, event: Event) {
        let Event { id, msg } = event;
        self.dispatch_event_msg(Some(id), msg, /*replay_kind*/ None);
    }

    #[cfg(test)]
    pub(crate) fn handle_agere_event_replay(&mut self, event: Event) {
        let Event { msg, .. } = event;
    ...
    fn dispatch_event_msg(
        ...
        match msg {
            ...
            _ => {
                tracing::trace!("handle_agere_event: {:?}", msg);
            }
        }
        ...
        match msg {
            ...
            EventMsg::Error(ErrorEvent {
                ...
                {
                    self.on_cyber_policy_error();
                } else if let Some(kind) = agere_error_info
                    .as_ref()
            ...
            EventMsg::ItemCompleted(event) => {
                let item = event.item;
                if !from_replay && let agere_protocol::items::TurnItem::UserMessage(item) = &item {
                    self.on_committed_user_message(item, from_replay);
                }
                if let agere_protocol::items::TurnItem::Plan(plan_item) = &item {
                    self.on_plan_item_completed(plan_item.text.clone());
                }
                if let agere_protocol::items::TurnItem::AgentMessage(item) = item {
                    self.on_agent_message_item_completed(item, event.is_interim);
                }
            }
        }
    ...
    fn open_theme_picker(&mut self) {
        let agere_home = crate::legacy_core::config::find_agere_home().ok();
    ...
    fn status_line_reasoning_effort_label(
        ...
        match effort {
            ...
            None | Some(ReasoningEffortConfig::None) => {
                // Default based on wire_api type.
                match wire_api {
                    WireApi::Responses => "none",
                    WireApi::Anthropic | WireApi::Chat => "medium",
                }
            }
        }
    }
    ...
    fn prefetch_connectors_with_options(&mut self, force_refetch: bool) {
        ...
        tokio::spawn(async move {
            // Remote connectors removed; send empty accessible result
            let accessible_connectors = Vec::new();
    ...
    fn should_prefetch_rate_limits(&self) -> bool {
        self.config.model_provider.requires_provider_auth && self.has_chatgpt_account
    }
    ...
    fn open_workspace_owner_nudge_prompt(&mut self, credit_type: AddCreditsNudgeCreditType) {
        ...
        let (title, prompt) = match credit_type {
            ...
            AddCreditsNudgeCreditType::UsageLimit => (
                "Usage limit reached",
                "Request a limit increase from your owner to continue using agere. Request increase?",
            ),
        };
    ...
    /// Open a multi-step provider configuration wizard.
    pub(crate) fn open_provider_popup(&mut self) {
        let agere_home = self.config.agere_home.clone();
        let active = self.config.model_provider_id.clone();
        let archive = crate::onboarding::provider_toml::load(&agere_home);
        let templates = crate::onboarding::provider_templates::builtin_templates();
        let wizard = crate::bottom_pane::ProviderWizard::new(
            archive,
            templates,
            Some(active),
            self.app_event_tx.clone(),
        ...
        self.bottom_pane.push_view(Box::new(wizard));

        // Use the global cache — already fetched at startup and refreshed every
        // 5 minutes. No spawn, no flicker.
        if let Some(cached) = crate::onboarding::provider_templates::get_cached_templates()
            && !cached.is_empty()
        {
            self.update_provider_wizard_remote_templates(cached);
        }
    }

    /// Update remote templates in the active provider wizard view.
    pub(crate) fn update_provider_wizard_remote_templates(
        &mut self,
        templates: Vec<crate::onboarding::provider_templates::ProviderTemplate>,
    ) {
        self.bottom_pane
            .update_provider_wizard_remote_templates(templates);
    }
    ...
    fn open_personality_popup_for_current_model(&mut self) {
        ...
        header.push(Line::from("Choose a communication style for Agere.".dim()));
    ...
    pub(crate) fn open_realtime_audio_popup(&mut self) {
        ...
        self.bottom_pane.show_selection_view(SelectionViewParams {
            ...
            subtitle: Some("Configure settings for Agere.".to_string()),
            footer_hint: Some(standard_popup_hint_line()),
            items,
            ..Default::default()
        });
    ...
    fn model_menu_warning_line(&self) -> Option<Line<'static>> {
        ...
        let warning = format!(
            "Warning: provider base URL is overridden to {base_url}. Selecting models may not be supported or work properly."
        );
    ...
    fn is_auto_model(model: &str) -> bool {
        model.starts_with("agere-auto-")
    }

    fn auto_model_order(model: &str) -> usize {
        match model {
            "agere-auto-fast" => 0,
            "agere-auto-balanced" => 1,
            "agere-auto-thorough" => 2,
            _ => 3,
        }
    }
    ...
    pub(crate) fn open_all_models_popup(&mut self, presets: Vec<ModelPreset>) {
        ...
        let header = self.model_menu_header(
            ...
            "Access legacy models by running agere -m <model_name> or in your config.toml",
        );
        self.bottom_pane.show_selection_view(SelectionViewParams {
    ...
    pub(crate) fn open_reasoning_popup(&mut self, preset: ModelPreset) {
        ...
        let warn_for_model =
            preset.model.starts_with("gpt-5.2") || preset.model.starts_with("gpt-5.3");
    ...
    pub(crate) fn open_permissions_popup(&mut self) {
        ...
        #[cfg(target_os = "windows")]
        let windows_degraded_restriction_enabled = matches!(
            self.config.permissions.windows_access_mode,
            Some(WindowsAccessModeToml::Unelevated)
        );
        #[cfg(not(target_os = "windows"))]
        let windows_degraded_restriction_enabled = false;

        let show_elevate_restriction_hint = false
            && windows_degraded_restriction_enabled
        ...
        for preset in presets.into_iter() {
            ...
            let base_name = if preset.id == "auto" && windows_degraded_restriction_enabled {
                "Default (non-admin restriction)".to_string()
            } else {
            ...
            let default_actions: Vec<SelectionAction> = if requires_confirmation {
                ...
                #[cfg(target_os = "windows")]
                {
                    if self.config.permissions.windows_access_mode.is_none() {
                        let preset_clone = preset.clone();
                        vec![Box::new(move |tx| {
                            tx.send(AppEvent::OpenWindowsRestrictionEnablePrompt {
                                preset: preset_clone.clone(),
                            });
                        })]
        ...
        let footer_note = show_elevate_restriction_hint.then(|| {
            vec![
                "The non-admin restriction protects your files and prevents network access under most circumstances. However, it carries greater risk if prompt injected. To upgrade to the default restriction profile, run ".dim(),
                "/setup-default-restriction".cyan(),
                ".".dim(),
            ]
            .into()
        });
    ...
    fn preset_matches_current(
        ...
        match preset.id {
            ...
            "read-only" => {
                let file_system_policy = current_permission_profile.file_system_access_policy();
                matches!(
                    current_permission_profile,
                    ...
                    && file_system_policy
                        ...
                        .is_empty()
                    && current_permission_profile.network_access_policy()
                        == preset.permission_profile.network_access_policy()
            }
            "auto" => {
                let file_system_policy = current_permission_profile.file_system_access_policy();
                matches!(
    ...
    pub(crate) fn world_writable_warning_details(&self) -> Option<(Vec<String>, usize, bool)> {
        // Windows native isolation scan removed; skip world-writable scan.
        None
    }
    ...
    pub(crate) fn open_full_access_confirmation(
        ...
        let info_line = Line::from(vec![
            "When Agere runs with full access, it can edit any file on your computer and run commands with network, without your approval. "
                .into(),
        ...
        let info_line = if failed_scan {
            Line::from(vec![
                "We couldn't complete the world-writable scan, so protections cannot be verified. "
                    .into(),
                format!("Windows access restrictions cannot guarantee protection in {mode_label}.")
                    .fg(Color::Red),
            ])
        } else {
            Line::from(vec![
                "Windows access restrictions cannot protect writes to folders that are writable by Everyone.".into(),
    ...
    #[cfg(target_os = "windows")]
    pub(crate) fn open_windows_restriction_enable_prompt(&mut self, preset: ApprovalPreset) {
        use ratatui_macros::line;

        if !false {
            // Legacy flow (pre-NUX): explain execution restrictions and let the user enable them
            // directly (no elevation prompts).
            let mut header = ColumnRenderable::new();
            header.push(*Box::new(
                Paragraph::new(vec![
                    line!["Agent mode on Windows can use execution restrictions to limit network and filesystem access.".bold()],
                    line!["Learn more: https://developers.openagere.com/agere/windows"],
                ])
            ...
            let items = vec![
                SelectionItem {
                    name: "Enable restricted agent mode".to_string(),
                    description: None,
                    actions: vec![Box::new(move |tx| {
                        tx.send(AppEvent::EnableWindowsRestrictionForAgentMode {
                            preset: preset_clone.clone(),
                            mode: WindowsRestrictionEnableMode::Legacy,
                        });
                    })],
                    dismiss_on_select: true,
                    ..Default::default()
        ...
        self.session_telemetry.counter(
            "agere.windows_access.elevated_prompt_shown",
            /*inc*/ 1,
            &[],
        );
        ...
        header.push(*Box::new(
            Paragraph::new(vec![
                line!["Set up Agere execution restrictions on Windows to protect your files and control network access. Learn more <https://developers.openagere.com/agere/windows>"],
            ])
            .wrap(Wrap { trim: false }),
        ));
        ...
        let items = vec![
            SelectionItem {
                name: "Set up default restriction (requires Administrator permissions)".to_string(),
                description: None,
                actions: vec![Box::new(move |tx| {
                    accept_otel.counter(
                        "agere.windows_access.elevated_prompt_accept",
                        /*inc*/ 1,
                        &[],
                    );
                    tx.send(AppEvent::BeginWindowsRestrictionElevatedSetup {
                        preset: preset.clone(),
                    });
                })],
                dismiss_on_select: true,
            ...
            SelectionItem {
                name: "Use non-admin restriction (higher risk if prompt injected)".to_string(),
                description: None,
                actions: vec![Box::new(move |tx| {
                    legacy_otel.counter(
                        "agere.windows_access.elevated_prompt_use_legacy",
                        /*inc*/ 1,
                        &[],
                    );
                    tx.send(AppEvent::BeginWindowsRestrictionLegacySetup {
            ...
            SelectionItem {
                ...
                actions: vec![Box::new(move |tx| {
                    quit_otel.counter(
                        "agere.windows_access.elevated_prompt_quit",
                        /*inc*/ 1,
                        &[],
                    );
    ...
    #[cfg(not(target_os = "windows"))]
    pub(crate) fn open_windows_restriction_enable_prompt(&mut self, _preset: ApprovalPreset) {}

    #[cfg(target_os = "windows")]
    pub(crate) fn open_windows_restriction_fallback_prompt(&mut self, preset: ApprovalPreset) {
        use ratatui_macros::line;

        let mut lines = Vec::new();
        lines.push(line![
            "Couldn't set up execution restrictions with Administrator permissions".bold()
        ]);
        lines.push(line![""]);
        lines.push(line![
            "You can still use Agere with a non-admin restriction. It carries greater risk if prompt injected."
        ]);
        lines.push(line![
            "Learn more <https://developers.openagere.com/agere/windows>"
        ]);
        ...
        let items = vec![
            SelectionItem {
                name: "Try setting up admin restriction again".to_string(),
                description: None,
                actions: vec![Box::new({
                    ...
                    move |tx| {
                        otel.counter(
                            "agere.windows_access.fallback_retry_elevated",
                            /*inc*/ 1,
                            &[],
                        );
                        tx.send(AppEvent::BeginWindowsRestrictionElevatedSetup {
            ...
            SelectionItem {
                name: "Use Agere with non-admin restriction".to_string(),
                description: None,
                actions: vec![Box::new({
                    let otel = self.session_telemetry.clone();
                    ...
                    move |tx| {
                        otel.counter(
                            "agere.windows_access.fallback_use_legacy",
                            /*inc*/ 1,
                            &[],
                        );
                        tx.send(AppEvent::BeginWindowsRestrictionLegacySetup {
            ...
            SelectionItem {
                ...
                actions: vec![Box::new(move |tx| {
                    quit_otel.counter(
                        "agere.windows_access.fallback_prompt_quit",
                        /*inc*/ 1,
                        &[],
                    );
                    tx.send(AppEvent::Exit(ExitMode::ShutdownFirst));
    ...
    #[cfg(not(target_os = "windows"))]
    pub(crate) fn open_windows_restriction_fallback_prompt(&mut self, _preset: ApprovalPreset) {}

    #[cfg(target_os = "windows")]
    pub(crate) fn maybe_prompt_windows_restriction_enable(&mut self, show_now: bool) {
        if show_now
            && self.config.permissions.windows_access_mode.is_none()
            && let Some(preset) = builtin_approval_presets()
                .into_iter()
                .find(|preset| preset.id == "auto")
        {
            self.open_windows_restriction_enable_prompt(preset);
        }
    }

    #[cfg(not(target_os = "windows"))]
    pub(crate) fn maybe_prompt_windows_restriction_enable(&mut self, _show_now: bool) {}

    #[cfg(target_os = "windows")]
    #[allow(dead_code)] // Invoked when elevated Windows restriction setup runs (future wiring).
    pub(crate) fn show_windows_access_setup_status(&mut self) {
        // While elevated restriction setup runs, prevent typing so the user doesn't
        ...
        self.bottom_pane
            .set_interrupt_hint_visible(/*visible*/ false);
        self.set_status(
            "Setting up Windows access restrictions...".to_string(),
    ...
    #[cfg(target_os = "windows")]
    pub(crate) fn clear_windows_access_setup_status(&mut self) {
        self.bottom_pane
    ...
    pub(crate) fn clear_windows_restriction_setup_status(&mut self) {
        self.clear_windows_access_setup_status();
    }
    ...
    #[cfg_attr(not(target_os = "windows"), allow(dead_code))]
    pub(crate) fn set_windows_access_mode(&mut self, mode: Option<WindowsAccessModeToml>) {
        self.config.permissions.windows_access_mode = mode;
        #[cfg(target_os = "windows")]
        self.bottom_pane.set_windows_degraded_restriction_active(
            false
                && matches!(
                    self.config.permissions.windows_access_mode,
                    Some(WindowsAccessModeToml::Unelevated)
    ...
    /// Update the session header to reflect a newly-active provider.
    pub(crate) fn set_session_provider(&mut self, provider: &str) {
        self.config.model_provider_id = provider.to_string();
        self.session_header.set_provider(provider);
        self.refresh_terminal_title();
    }
    ...
    pub(crate) fn set_collaboration_mask(&mut self, mut mask: CollaborationModeMask) {
            ...
            && (previous_model != next_model || previous_effort != next_effort)
        {
            ...
            if !next_model.starts_with("agere-auto-") {
    ...
    fn rename_confirmation_cell(name: &str, thread_id: Option<ThreadId>) -> PlainHistoryCell {
        let resume_cmd = crate::legacy_core::util::resume_command(Some(name), thread_id)
            .unwrap_or_else(|| format!("openagere resume {name}"));
    ...
    fn connectors_popup_params(
        ...
        for connector in connectors {
            let connector_label = agere_connectors::metadata::connector_display_label(connector);
            ...
            let instructions = if connector.is_accessible {
                ...
                "Install this app in your browser, then reload Agere."
            };
    ...
    pub(crate) fn apply_grep_search_result(&mut self, query: &str, matches: Vec<RgMatch>) {
        self.bottom_pane
            .on_grep_search_result(query.to_string(), matches);
    }
    ...
    /// Forward a command directly to agere.
        ...
        T: Into<AppCommand>,
    {
        ...
        match &self.agere_op_target {
            AgereOpTarget::Direct(agere_op_tx) => {
                crate::session_log::log_outbound_op(&op);
                if let Err(e) = agere_op_tx.send(op.into_core()) {
    ...
    pub(crate) fn sync_provider_config(&mut self, config: &Config) {
        ...
        self.config.models = config.models.clone();
        self.config.model_reasoning_effort = config.model_reasoning_effort;
        self.config.model_reasoning_summary = config.model_reasoning_summary;
        self.config.model_supports_reasoning_summaries = config.model_supports_reasoning_summaries;
        self.config.model_verbosity = config.model_verbosity;
    }
...
impl Notification {
    ...
    fn user_input_request_summary(
        questions: &[agere_protocol::request_user_input::RequestUserInputQuestion],
    ) -> Option<String> {
