...
pub enum Feature {
    ...
    /// Allow exec tools to request additional permissions while staying within default restrictions.
    ...
    SearchTool,
    /// Use the legacy Landlock Linux isolation fallback instead of the default
    /// bubblewrap pipeline.
    UseLegacyLandlock,
    ...
    ChildAgentsMd,
    /// Compress request bodies (zstd) when sending streaming requests to agere-backend.
    ...
    /// Requirements-only gate: this should be set from requirements, not user config.
    BrowserUse,
    /// Allow Agere Computer Use.
    ///
    /// Requirements-only gate: this should be set from requirements, not user config.
    ComputerUse,
    /// Temporary internal-only flag for PS-backed remote plugin catalog development.
    RemotePlugin,
    /// Show the startup prompt for migrating external agent config into Agere.
    ExternalMigration,
...
fn legacy_usage_notice(alias: &str, feature: Feature) -> (String, Option<String>) {
    ...
    match feature {
        ...
        Feature::UseLegacyLandlock => {
            ...
            let details =
                "Remove this setting to stop opting into the legacy Linux Landlock isolation path."
                    .to_string();
        ...
        _ => {
            ...
            let details = if alias == canonical {
                ...
                Some(format!(
                    "Enable it with `--enable {canonical}` or `[features].{canonical}` in config.toml. See https://developers.openagere.com/agere/config-basic#feature-flags for details."
                ))
            };
            (summary, details)
        }
...
pub const FEATURES: &[FeatureSpec] = &[
    ...
    FeatureSpec {
        ...
        stage: Stage::Experimental {
            ...
            menu_description: "Rebuild Agere-owned transcript scrollback when the terminal width changes.",
            announcement: "",
        },
        default_enabled: true,
    },
    ...
    FeatureSpec {
        id: Feature::AgereGitCommit,
        key: "agere_git_commit",
        stage: Stage::UnderDevelopment,
        default_enabled: false,
    },
    FeatureSpec {
    ...
    FeatureSpec {
        ...
        stage: Stage::Experimental {
            name: "Memories",
            menu_description: "Allow Agere to create new memories from conversations and bring relevant memories into new conversations.",
            announcement: "NEW: Agere can now generate and uses memories. Try is now with `/memories`",
        },
        default_enabled: false,
    },
    FeatureSpec {
    ...
    FeatureSpec {
        ...
        stage: Stage::Experimental {
            ...
            menu_description: "Show a startup prompt when Agere detects migratable external agent config for this machine or project.",
            announcement: "",
        },
        default_enabled: false,
    },
    ...
    FeatureSpec {
        ...
        stage: if cfg!(any(
            ...
            Stage::Experimental {
                ...
                menu_description: "Keep your computer awake while Agere is running a thread.",
