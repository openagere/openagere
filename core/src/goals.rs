...
use agere_protocol::protocol::validate_thread_goal_objective;
...
impl Session {
        ...
        let goal = match outcome {
            agere_state::ThreadGoalAccountingOutcome::Updated(goal) => {
                let clear_active_goal = match goal.status {
                    agere_state::ThreadGoalStatus::Active => false,
                    agere_state::ThreadGoalStatus::BudgetLimited => {
            ...
            agere_state::ThreadGoalAccountingOutcome::Unchanged(_) => return Ok(()),
        };
        let should_steer_budget_limit =
            matches!(budget_limit_steering, BudgetLimitSteering::Allowed)
                && goal.status == agere_state::ThreadGoalStatus::BudgetLimited
                && !budget_limit_was_already_reported;
        let goal_status = goal.status;
        let goal_id = goal.goal_id.clone();
        if goal_status != agere_state::ThreadGoalStatus::BudgetLimited {
            *self.goal_runtime.budget_limit_reported_goal_id.lock().await = None;
        }
        let goal = protocol_goal_from_state(goal);
        self.send_event(
    ...
    async fn account_thread_goal_wall_clock_usage(
        ...
        mode: agere_state::ThreadGoalAccountingMode,
            ...
            .await?
        {
            ...
            agere_state::ThreadGoalAccountingOutcome::Unchanged(goal) => {
                {
                    let mut accounting = self.goal_runtime.accounting.lock().await;
    ...
    async fn activate_paused_thread_goal_after_resume(&self) -> anyhow::Result<bool> {
        ...
        if goal.status != agere_state::ThreadGoalStatus::Paused {
            let goal_id = goal.goal_id.clone();
            let is_active = goal.status == agere_state::ThreadGoalStatus::Active;
            if is_active {
                self.goal_runtime
                    .accounting
                    .lock()
            ...
            Ok(Some(state_db)) => match state_db.get_thread_goal(self.conversation_id).await {
                Ok(Some(goal))
                    if goal.goal_id == candidate.goal_id
                        && goal.status == agere_state::ThreadGoalStatus::Active =>
                {
                    true
                }
                Ok(Some(_)) | Ok(None) => {
