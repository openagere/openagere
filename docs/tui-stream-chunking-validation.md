...
- `agere_tui::streaming::commit_tick`
...
Run OpenAgere with chunking traces enabled:

```bash
RUST_LOG='agere_tui::streaming::commit_tick=trace,agere_tui=info,agere_core=info,agere_rmcp_client=info' \
  just agere --enable=responses_websockets
```
...
1. Record the current size of `~/.openagere/log/openagere.log` as a start offset.
