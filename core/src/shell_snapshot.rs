...
impl ShellSnapshot {
    ...
    fn spawn_snapshot_task(
        ...
        tokio::spawn(
            async move {
                let timer = session_telemetry.start_timer("agere.shell_snapshot.duration_ms", &[]);
                let snapshot =
                    ShellSnapshot::try_new(&agere_home, session_id, &session_cwd, &snapshot_shell)
                        .await
                        .map(Arc::new);
                ...
                session_telemetry.counter("agere.shell_snapshot", /*inc*/ 1, &counter_tags);
    ...
    async fn try_new(
        ...
        let temp_path = agere_home
            .join(SNAPSHOT_DIR)
            .join(format!("{session_id}.tmp-{nonce}"));

        // Clean the (unlikely) leaked snapshot files.
        let agere_home = agere_home.clone();
        let cleanup_session_id = session_id;
        tokio::spawn(async move {
            if let Err(err) = cleanup_stale_snapshots(&agere_home, cleanup_session_id).await {
...
pub async fn cleanup_stale_snapshots(
    ...
    while let Some(entry) = entries.next_entry().await? {
        ...
        let rollout_path = find_thread_path_by_id_str(agere_home, session_id).await?;
