...
use agere_core::plugins::PluginMarketplaceUpgradeOutcome;
...
use agere_core_plugins::marketplace_remove::MarketplaceRemoveRequest;
...
#[derive(Debug, Parser)]
#[command(bin_name = "agere plugin marketplace")]
...
#[derive(Debug, Parser)]
#[command(bin_name = "agere plugin marketplace add")]
...
#[derive(Debug, Parser)]
#[command(bin_name = "agere plugin marketplace upgrade")]
struct UpgradeMarketplaceArgs {
    marketplace_name: Option<String>,
}

#[derive(Debug, Parser)]
#[command(bin_name = "agere plugin marketplace remove")]
...
async fn run_upgrade(
    ...
    let agere_home = find_agere_home().context("failed to resolve AGERE_HOME")?;
    let manager = PluginsManager::new(agere_home.to_path_buf());
    let outcome = manager
