...
pub(crate) struct ExecServerHarness {
    _agere_home: TempDir,
    _helper_paths: TestAgereHelperPaths,
    child: Child,
...
pub(crate) struct TestAgereHelperPaths {
    pub(crate) agere_exe: PathBuf,
    pub(crate) agere_linux_exe: Option<PathBuf>,
}

pub(crate) fn test_agere_helper_paths() -> anyhow::Result<TestAgereHelperPaths> {
    let (helper_binary, agere_linux_exe) = super::current_test_binary_helper_paths()?;
    Ok(TestAgereHelperPaths {
        agere_exe: helper_binary,
        agere_linux_exe,
    })
}
    ...
    V: AsRef<std::ffi::OsStr>,
{
    ...
    let mut child = Command::new(&helper_paths.agere_exe);
    child.args(["exec-server", "--listen", "ws://127.0.0.1:0"]);
    child.stdin(Stdio::null());
    child.stdout(Stdio::piped());
    child.stderr(Stdio::inherit());
    child.kill_on_drop(true);
    child.env("AGERE_HOME", agere_home.path());
    child.envs(env);
