...
use agere_analytics::GuardianReviewAnalyticsResult;
use agere_analytics::GuardianReviewSessionKind;
...
use crate::agere_delegate::run_agere_thread_interactive;
...
impl GuardianReviewSessionReuseKey {
    fn from_spawn_config(spawn_config: &Config) -> Self {
        Self {
            ...
            agere_linux_exe: spawn_config.agere_linux_exe.clone(),
...
impl GuardianReviewSession {
    async fn shutdown(&self) {
        ...
        let _ = self.agere.shutdown_and_wait().await;
    }
    ...
    async fn refresh_last_committed_fork_snapshot(&self) {
        match load_rollout_items_for_fork(&self.agere.session).await {
...
impl GuardianReviewSessionManager {
    ...
    #[cfg(test)]
    pub(crate) async fn cache_for_test(&self, agere: Agere) {
        let reuse_key = GuardianReviewSessionReuseKey::from_spawn_config(
            agere.session.get_config().await.as_ref(),
        );
    ...
    #[cfg(test)]
    pub(crate) async fn register_ephemeral_for_test(&self, agere: Agere) {
        let reuse_key = GuardianReviewSessionReuseKey::from_spawn_config(
            agere.session.get_config().await.as_ref(),
        );
        self.state
...
async fn spawn_guardian_review_session(
    ...
    let agere = Box::pin(run_agere_thread_interactive(
        spawn_config,
...
async fn run_review_on_session(
    ...
    let mut analytics_result = GuardianReviewAnalyticsResult::from_session(
        review_session.agere.session.conversation_id.to_string(),
        guardian_session_kind,
        params.model.clone(),
        ...
        Box::pin(async {
            params
                ...
                .sync_session_approved_hosts_to(
                    &review_session.agere.session.services.network_approval,
                )
                .await;
    ...
    let submit_result = run_before_review_deadline(
        deadline,
        params.external_cancel.as_ref(),
        Box::pin(review_session.agere.submit(Op::UserTurn {
            environments: None,
            ...
            effort: Some(params.reasoning_effort),
            summary: Some(Some(params.reasoning_summary)),
            service_tier: None,
    ...
    if matches!(outcome.0, GuardianReviewSessionOutcome::Completed(_)) {
        if outcome.2
            && let Some(total_token_usage) = review_session.agere.session.total_token_usage().await
        {
...
async fn append_guardian_followup_reminder(review_session: &GuardianReviewSession) {
    let turn_context = review_session.agere.session.new_default_turn().await;
...
async fn wait_for_guardian_review(
    ...
    loop {
        tokio::select! {
            _ = &mut timeout => {
                let keep_review_session = interrupt_and_drain_turn(&review_session.agere).await.is_ok();
            ...
            event = review_session.agere.next_event() => {
                match event {
...
pub(crate) fn build_guardian_review_session_config(
    ...
    live_network_config: Option<agere_network_proxy::NetworkProxyConfig>,
    active_model: &str,
    ...
    let _access_policy = "read-only".to_string();
...
async fn interrupt_and_drain_turn(agere: &Agere) -> anyhow::Result<()> {
...
mod tests {
    ...
    async fn guardian_review_session_config_disables_hooks() {
        ...
        assert!(!guardian_config.features.enabled(Feature::AgereHooks));
    }
