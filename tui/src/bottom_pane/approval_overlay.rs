...
use agere_protocol::mcp::RequestId;
...
use agere_protocol::protocol::ElicitationAction;
    ...
    available_decisions
        ...
        .filter_map(|decision| match decision {
            ...
            ReviewDecision::Abort => Some(ApprovalOption {
                label: "No, and tell Agere what to do differently".to_string(),
...
fn format_file_system_entry_paths<'a>(
    entries: impl Iterator<Item = &'a FileSystemAccessEntry>,
) -> String {
    entries
...
mod tests {
    ...
    use agere_protocol::protocol::NetworkPolicyAmendment;
    use agere_utils_fs::AbsolutePathBuf;
    use crossterm::event::KeyModifiers;
    ...
    fn network_exec_options_use_expected_labels_and_hide_execpolicy_amendment() {
        ...
        assert_eq!(
            ...
            vec![
                ...
                "Yes, and allow this host for this conversation".to_string(),
                "Yes, and allow this host in the future".to_string(),
                "No, and tell Agere what to do differently".to_string(),
            ]
        );
    }
    ...
    fn exec_history_cell_wraps_with_two_space_indent() {
        ...
        let rendered: Vec<String> = lines
            ...
            .collect();
        let expected = vec![
            "✔ You approved agere to run".to_string(),
