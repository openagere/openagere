...
use agere_core::compact::SUMMARIZATION_PROMPT;
use agere_core::compact::SUMMARY_PREFIX;
...
use core_test_support::test_agere::turn_permission_fields;
...
fn disabled_permission_user_turn(text: impl Into<String>, cwd: PathBuf, model: String) -> Op {
    let permission_profile = turn_permission_fields(PermissionProfile::Disabled, cwd.as_path());
    Op::UserTurn {
        environments: None,
...
async fn assert_compaction_uses_turn_lifecycle_id(agere: &std::sync::Arc<agere_core::AgereThread>) {
    ...
    while turn_completed_id.is_none() {
        let event = agere.next_event().await.expect("next event");
        match event.msg {
...
async fn assert_precompact_uses_separate_turn_lifecycle(
    agere: &std::sync::Arc<agere_core::AgereThread>,
    user_turn_id: &str,
) {
    let mut compact_turn_started_id = None;
    let mut compact_turn_completed_id = None;
    let mut compact_item_started_id = None;
    let mut compact_item_completed_id = None;
    let mut user_turn_started = false;

    while !user_turn_started {
        let event = agere.next_event().await.expect("next event");
        match event.msg {
            EventMsg::TurnStarted(_) if event.id == user_turn_id => {
                user_turn_started = true;
            }
            EventMsg::TurnStarted(_) => {
                compact_turn_started_id = Some(event.id.clone());
            }
            EventMsg::ItemStarted(ItemStartedEvent {
                item: TurnItem::ContextCompaction(_),
                ..
            }) => {
                compact_item_started_id = Some(event.id.clone());
            }
            EventMsg::ItemCompleted(ItemCompletedEvent {
                item: TurnItem::ContextCompaction(_),
                ..
            }) => {
                compact_item_completed_id = Some(event.id.clone());
            }
            EventMsg::TurnComplete(_) if event.id != user_turn_id => {
                compact_turn_completed_id = Some(event.id.clone());
            }
            _ => {}
        }
    }
    ...
    assert_eq!(
        compact_turn_started_id,
        Some(format!("{user_turn_id}-precompact")),
        "pre-switch compaction should use a derived turn id"
    );
    assert_eq!(
        compact_turn_completed_id, compact_turn_started_id,
        "pre-switch compaction should complete its own turn"
    );
    assert_eq!(
        compact_item_started_id, compact_turn_started_id,
        "pre-switch compaction item start should be inside the compaction turn"
    );
    assert_eq!(
        compact_item_completed_id, compact_turn_started_id,
        "pre-switch compaction item completion should be inside the compaction turn"
    );
}
...
async fn summarize_context_three_requests_and_instructions() {
    ...
    // Build config pointing to the mock server and spawn Agere.
    ...
    agere.submit(Op::Compact).await.unwrap();
    let warning_event = wait_for_event(&agere, |ev| matches!(ev, EventMsg::Warning(_))).await;
    ...
    // Shut down Agere to flush rollout entries before inspecting the file.
    agere.submit(Op::Shutdown).await.unwrap();
    wait_for_event(&agere, |ev| matches!(ev, EventMsg::ShutdownComplete)).await;
...
async fn manual_compact_uses_custom_prompt() {
    ...
    agere.submit(Op::Compact).await.expect("trigger compact");
    let warning_event = wait_for_event(&agere, |ev| matches!(ev, EventMsg::Warning(_))).await;
    let EventMsg::Warning(WarningEvent { message }) = warning_event else {
        panic!("expected warning event after compact");
    };
...
async fn manual_compact_emits_api_and_local_token_usage_events() {
    ...
    let agere = builder.build(&server).await.unwrap().agere;
    ...
    let first = wait_for_event_match(&agere, |ev| match ev {
    ...
    let last = wait_for_event_match(&agere, |ev| match ev {
        EventMsg::TokenCount(tc) => tc
            .info
            .as_ref()
            .map(|info| info.last_token_usage.total_tokens),
...
async fn manual_compact_emits_context_compaction_items() {
    ...
    agere
        ...
        .await
        .unwrap();
    wait_for_event(&agere, |event| matches!(event, EventMsg::TurnComplete(_))).await;
    ...
    {
        let event = agere.next_event().await.unwrap();
        match event.msg {
            EventMsg::ItemStarted(ItemStartedEvent {
...
async fn multiple_auto_compact_per_task_runs_after_token_limit_hit() {
    ...
    let _non_openai_provider_name = non_openai_model_provider(&server).wire_api.to_string();
    let agere = test_agere()
        ...
        values
            ...
            .filter_map(|value| {
                ...
                if role == Some("developer")
                    && text.is_some_and(|text| text.contains("`access_mode`"))
                {
                    return None;
                }
                if role == Some("user") {
...
async fn auto_compact_runs_after_resume_when_token_usage_is_over_limit() {
    ...
    let compacted_history = vec![
        agere_protocol::models::ResponseItem::Message {
            id: None,
            role: "assistant".to_string(),
            content: vec![agere_protocol::models::ContentItem::OutputText {
                text: remote_summary.to_string(),
            }],
            phase: None,
        },
        agere_protocol::models::ResponseItem::Compaction {
    ...
    let mut resume_builder = test_agere().with_config(move |config| {
        set_test_compact_prompt(config);
        config.model_auto_compact_token_limit = Some(limit);
    });
    ...
    wait_for_event(&resumed.agere, |event| {
...
async fn pre_sampling_compact_runs_on_switch_to_smaller_context_model() {
    ...
    let mut builder = test_agere()
        .with_auth(AgereAuth::create_dummy_chatgpt_auth_for_testing())
        .with_model(previous_model)
        .with_config(move |config| {
            config.model_provider = model_provider;
            set_test_compact_prompt(config);
        });
    let test = builder.build(&server).await.expect("build test agere");
    ...
    .await
    .expect("provider switch turn should complete");
    ...
    assert_compaction_uses_turn_lifecycle_id(&test.agere).await;
...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn provider_downshift_precompacts_on_previous_provider_before_switch() {
    skip_if_no_network!();
    ...
    .await;
    let new_models_probe_mock = mount_models_once(
        &new_server,
        ModelsResponse {
            models: vec![model_info_with_context_window(
                model, /*context_window*/ 125_000,
            )],
        },
    )
    .await;
    let new_models_turn_mock = mount_models_once(
        &new_server,
        ModelsResponse {
            models: vec![model_info_with_context_window(
                model, /*context_window*/ 125_000,
    ...
    let old_provider_mock = mount_sse_sequence(
        &old_server,
        vec![
            sse(vec![
            ...
            sse(vec![
                ev_assistant_message("m2", "PRE_SWITCH_PROVIDER_SUMMARY"),
                ev_completed_with_tokens("r2", /*total_tokens*/ 10),
            ]),
        ],
    )
    .await;
    let new_provider_mock = mount_sse_once(
        &new_server,
        sse(vec![
    ...
    let old_provider = non_openai_model_provider(&old_server);
    let mut builder = test_agere()
        .with_auth(AgereAuth::create_dummy_chatgpt_auth_for_testing())
        .with_model(model)
        .with_config(move |config| {
    ...
    let test = builder.build(&old_server).await.expect("build test agere");
    ...
    let mut provider_config = (*test.agere.config().await).clone();
    provider_config.model_context_window = Some(125_000);
    let mut new_provider = non_openai_model_provider(&new_server);
    new_provider.base_url = Some(format!("{}/v1", new_server.uri()));
    test.agere
        .stage_model_provider(
            "sub-provider-downshift".to_string(),
            "smaller-provider".to_string(),
            new_provider,
            std::sync::Arc::new(provider_config),
        )
        .await;

    test.agere
        .submit_with_id(Submission {
            id: "sub-provider-downshift".to_string(),
            trace: None,
            op: Op::UserInputWithTurnContext {
                environments: None,
                items: vec![UserInput::Text {
                    text: "after provider switch".to_string(),
                    text_elements: Vec::new(),
                }],
                final_output_json_schema: None,
                responsesapi_client_metadata: None,
                ...
                model_provider: Some("smaller-provider".to_string()),
                effort: None,
                summary: None,
                service_tier: None,
                collaboration_mode: None,
        ...
        .await
        .expect("submit provider switch turn");
    assert_precompact_uses_separate_turn_lifecycle(&test.agere, "sub-provider-downshift").await;
    wait_for_event(&test.agere, |event| {
        matches!(event, EventMsg::TurnComplete(_))
    })
    .await;

    let old_requests = old_provider_mock.requests();
    let new_requests = new_provider_mock.requests();
    // Both providers resolve `gpt-5.3` from the bundled catalog, and the new
    // provider's context window comes from the staged `model_context_window`
    // override, so neither provider's remote `/models` endpoint is exercised.
    // Pin to zero so a future change that introduces an unexpected catalog fetch
    // (or fails to resolve offline) is caught instead of silently tolerated.
    assert_eq!(old_models_mock.requests().len(), 0);
    assert_eq!(new_models_probe_mock.requests().len(), 0);
    assert_eq!(new_models_turn_mock.requests().len(), 0);
    assert_eq!(
        old_requests.len(),
        2,
        "old provider should receive first turn and pre-switch compaction"
    );
    assert_eq!(
        new_requests.len(),
        1,
        "new provider should receive only the follow-up turn"
    );
    assert!(
        body_contains_text(
            &old_requests[1].body_json().to_string(),
            SUMMARIZATION_PROMPT
        ),
        "pre-switch compaction should use the previous provider"
    );
    assert!(
        !body_contains_text(
            &new_requests[0].body_json().to_string(),
            SUMMARIZATION_PROMPT
        ),
        "new provider should not repeat compaction after successful previous-provider compaction"
    );
}
...
async fn pre_sampling_compact_runs_after_resume_and_switch_to_smaller_model() {
    ...
    let model_provider = non_openai_model_provider(&server);
    let mut initial_builder = test_agere()
        .with_auth(AgereAuth::create_dummy_chatgpt_auth_for_testing())
        .with_model(previous_model)
        .with_config(move |config| {
            config.model_provider = model_provider;
    ...
    let initial = initial_builder
        .build(&server)
        .await
        .expect("build initial test agere");
    let home = initial.home.clone();
    let rollout_path = initial
        .session_configured
        .rollout_path
    ...
    wait_for_event(&initial.agere, |event| {
        matches!(event, EventMsg::TurnComplete(_))
    })
    .await;
    ...
    let model_provider = non_openai_model_provider(&server);
    let mut resumed_builder = test_agere()
        .with_auth(AgereAuth::create_dummy_chatgpt_auth_for_testing())
        .with_model(previous_model)
        .with_config(move |config| {
            config.model_provider = model_provider;
    ...
    assert_compaction_uses_turn_lifecycle_id(&resumed.agere).await;
...
async fn manual_compact_retries_after_context_window_error() {
    ...
    let EventMsg::BackgroundEvent(event) =
        wait_for_event(&agere, |ev| matches!(ev, EventMsg::BackgroundEvent(_))).await
    else {
...
async fn manual_compact_non_context_failure_retries_then_emits_task_error() {
    ...
    let reconnect_message = wait_for_event_match(&agere, |event| match event {
    ...
    let task_error_message = wait_for_event_match(&agere, |event| match event {
...
async fn snapshot_request_shape_mid_turn_continuation_compaction() {
    ...
    wait_for_event(&agere, |msg| matches!(msg, EventMsg::TurnComplete(_))).await;
...
async fn auto_compact_clamps_config_limit_to_context_window() {
    ...
    let agere = builder.build(&server).await.unwrap();

    agere.submit_turn("OVER_LIMIT_TURN").await.unwrap();
    agere.submit_turn("FOLLOW_UP_AFTER_CLAMP").await.unwrap();
...
async fn snapshot_request_shape_pre_turn_compaction_context_window_exceeded() {
    ...
    let error_message = wait_for_event_match(&agere, |event| match event {
        EventMsg::Error(err) => Some(err.message.clone()),
        _ => None,
    })
    .await;
...
async fn snapshot_request_shape_manual_compact_without_previous_user_messages() {
    ...
    agere.submit(Op::Compact).await.expect("run /compact");
    wait_for_event(&agere, |ev| matches!(ev, EventMsg::TurnComplete(_))).await;
