...
use agere_config::McpServerConfig;
...
fn write_plugin_with_version(
    ...
    fs::create_dir_all(plugin_root.join(".agere-plugin")).unwrap();
...
fn init_git_repo(repo: &Path) {
    run_git(repo, &["init"]);
    run_git(repo, &["config", "user.email", "agere-test@example.com"]);
    run_git(repo, &["config", "user.name", "Agere Test"]);
    run_git(repo, &["add", "."]);
...
async fn load_plugins_from_config(config_toml: &str, agere_home: &Path) -> PluginLoadOutcome {
    write_file(&agere_home.join(CONFIG_TOML_FILE), config_toml);
    let config = load_config(agere_home, agere_home).await;
    PluginsManager::new(agere_home.to_path_buf())
        .plugins_for_config(&config)
        .await
}

async fn load_config(agere_home: &Path, cwd: &Path) -> crate::config::Config {
...
enabled = true
"#;
    let outcome = load_plugins_from_config(config_toml, agere_home.path()).await;
...
async fn effective_apps_dedupes_connector_ids_across_plugins() {
    let agere_home = TempDir::new().unwrap();
    let plugin_a_root = agere_home
        .path()
        .join("plugins/cache")
        .join("test/plugin-a/local");
    let plugin_b_root = agere_home
        .path()
        .join("plugins/cache")
        .join("test/plugin-b/local");

    write_file(
        &plugin_a_root.join(".agere-plugin/plugin.json"),
        r#"{"name":"plugin-a"}"#,
    );
    write_file(
  ...
  "apps": {
    ...
    write_file(
        &plugin_b_root.join(".agere-plugin/plugin.json"),
        r#"{"name":"plugin-b"}"#,
    );
    write_file(
...
fn capability_index_filters_inactive_and_zero_capability_plugins() {
    ...
    let plugin = |config_name: &str, dir_name: &str, manifest_name: &str| LoadedPlugin {
        ...
        manifest_description: None,
        root: AbsolutePathBuf::try_from(agere_home.path().join(dir_name)).unwrap(),
        enabled: true,
        skill_roots: Vec::new(),
        disabled_skill_paths: HashSet::new(),
        has_enabled_skills: false,
    ...
    let outcome = PluginLoadOutcome::from_plugins(vec![
        LoadedPlugin {
            skill_roots: vec![agere_home.path().join("skills-plugin/skills").abs()],
            has_enabled_skills: true,
        ...
        LoadedPlugin {
            enabled: false,
            skill_roots: vec![agere_home.path().join("disabled-plugin/skills").abs()],
...
async fn load_plugins_returns_empty_when_feature_disabled() {
    ...
    let config = load_config(agere_home.path(), agere_home.path()).await;
    let outcome = PluginsManager::new(agere_home.path().to_path_buf())
        .plugins_for_config(&config)
        .await;
...
async fn install_openai_curated_plugin_uses_short_sha_cache_version() {
    ...
    let installed_path = tmp.path().join(format!(
        "plugins/cache/openagere-curated/slack/{TEST_CURATED_PLUGIN_CACHE_VERSION}"
    ));
    assert_eq!(
        result,
        PluginInstallOutcome {
...
async fn install_plugin_supports_git_subdir_marketplace_sources() {
  ...
  "plugins": [
    ...
    assert!(installed_path.join(".agere-plugin/plugin.json").is_file());
}
...
"#,
    ...
    write_file(
        &repo_root.join(".openagere/config.toml"),
        r#"[[skills.config]]
...
async fn read_plugin_for_config_installed_git_source_reads_from_cache_without_cloning() {
  ...
  "plugins": [
    ...
    write_file(
        &cached_plugin_root.join(".agere-plugin/plugin.json"),
        r#"{
  "name": "toolkit",
...
"#,
  ...
  "plugins": [
    ...
    let curated_marketplace = marketplaces
        .into_iter()
        .find(|marketplace| marketplace.name == "openagere-curated")
    ...
    assert_eq!(
        ...
        ConfiguredMarketplace {
            ...
            plugins: vec![ConfiguredMarketplacePlugin {
                id: "linear@openagere-curated".to_string(),
...
#[tokio::test]
async fn list_marketplaces_uses_official_identity_for_curated_repo_path() {
    let tmp = tempfile::tempdir().unwrap();
    let curated_root = curated_plugins_repo_path(tmp.path());
    let plugin_root = curated_root.join("plugins/linear");
...
async fn sync_plugins_from_remote_reconciles_cache_and_config() {
    ...
    write_curated_plugin_sha(tmp.path(), TEST_CURATED_PLUGIN_SHA);
    write_plugin(
        &tmp.path().join("plugins/cache/openagere-curated"),
        "linear/local",
        "linear",
    );
    write_plugin(
...
[plugins."linear@openagere-curated"]
enabled = false

[plugins."gmail@openagere-curated"]
enabled = false

[plugins."calendar@openagere-curated"]
enabled = true
"#,
    );
    ...
    Mock::given(method("GET"))
        ...
        .respond_with(ResponseTemplate::new(200).set_body_string(
            r#"[
  {"id":"1","name":"linear","marketplace_name":"openagere-curated","version":"1.0.0","enabled":true},
  {"id":"2","name":"gmail","marketplace_name":"openagere-curated","version":"1.0.0","enabled":false}
]"#,
        ))
        .mount(&server)
        .await;
    ...
    let result = manager
        .sync_plugins_from_remote(
            &config,
            Some(&AgereAuth::create_dummy_chatgpt_auth_for_testing()),
            /*additive_only*/ false,
        )
        .await
        .unwrap();
    ...
    assert_eq!(
        ...
        RemotePluginSyncResult {
            ...
            enabled_plugin_ids: vec!["linear@openagere-curated".to_string()],
            disabled_plugin_ids: Vec::new(),
            uninstalled_plugin_ids: vec![
                "gmail@openagere-curated".to_string(),
                "calendar@openagere-curated".to_string(),
            ],
        }
    );
    ...
    assert!(
        tmp.path()
            .join("plugins/cache/openagere-curated/linear/local")
            .is_dir()
    );
    assert!(
        !tmp.path()
            .join("plugins/cache/openagere-curated/gmail")
            .exists()
    );
    assert!(
        !tmp.path()
            .join("plugins/cache/openagere-curated/calendar")
            .exists()
    );

    let config = fs::read_to_string(tmp.path().join(CONFIG_TOML_FILE)).unwrap();
    assert!(config.contains(r#"[plugins."linear@openagere-curated"]"#));
    assert!(config.contains("enabled = true"));
    assert!(!config.contains(r#"[plugins."gmail@openagere-curated"]"#));
    assert!(!config.contains(r#"[plugins."calendar@openagere-curated"]"#));
    ...
    let curated_marketplace = manager
        ...
        .marketplaces
        .into_iter()
        .find(|marketplace| marketplace.name == OPENAGERE_CURATED_MARKETPLACE_NAME)
        .unwrap();
    assert_eq!(
        curated_marketplace
            .plugins
            ...
            .collect::<Vec<_>>(),
        vec![
            ("linear@openagere-curated".to_string(), true, true),
            ("gmail@openagere-curated".to_string(), false, false),
            ("calendar@openagere-curated".to_string(), false, false),
        ]
    );
}
...
"#,
  ...
  {"id":"2","name":"gmail","marketplace_name":"openagere-curated","version":"1.0.0","enabled":false}
    ...
    assert!(
        tmp.path()
            .join("plugins/cache/openagere-curated/gmail/local")
            .is_dir()
    );
    assert!(
        tmp.path()
            .join("plugins/cache/openagere-curated/calendar/local")
            .is_dir()
    );
...
"#,
    ...
    Mock::given(method("GET"))
            ...
            .respond_with(ResponseTemplate::new(200).set_body_string(
                r#"[
  {"id":"1","name":"plugin-one","marketplace_name":"openagere-curated","version":"1.0.0","enabled":true}
]"#,
            ))
            .mount(&server)
            .await;
    ...
    assert_eq!(
        ...
        RemotePluginSyncResult {
            ...
            disabled_plugin_ids: Vec::new(),
            uninstalled_plugin_ids: vec!["linear@openagere-curated".to_string()],
        }
    );
    let config = fs::read_to_string(tmp.path().join(CONFIG_TOML_FILE)).unwrap();
    assert!(!config.contains(r#"[plugins."linear@openagere-curated"]"#));
    assert!(
        !tmp.path()
            .join("plugins/cache/openagere-curated/linear")
            .exists()
    );
}
...
"#,
    ...
    Mock::given(method("GET"))
        ...
        .respond_with(ResponseTemplate::new(200).set_body_string(
            r#"[
  {"id":"1","name":"gmail","marketplace_name":"openagere-curated","version":"1.0.0","enabled":true}
]"#,
        ))
        .mount(&server)
        .await;
...
"#,
  ...
  {"id":"1","name":"gmail","marketplace_name":"openagere-curated","version":"1.0.0","enabled":true}
    ...
    assert_eq!(
        result,
        RemotePluginSyncResult {
            installed_plugin_ids: vec!["gmail@openagere-curated".to_string()],
            enabled_plugin_ids: vec!["gmail@openagere-curated".to_string()],
            disabled_plugin_ids: Vec::new(),
            uninstalled_plugin_ids: Vec::new(),
        }
    );
    assert_eq!(
        fs::read_to_string(tmp.path().join(format!(
            "plugins/cache/openagere-curated/gmail/{TEST_CURATED_PLUGIN_CACHE_VERSION}/marker.txt"
        )))
        .unwrap(),
        "first"
    );
...
"#,
    ...
    Mock::given(method("GET"))
        .and(path("/backend-api/plugins/featured"))
        .and(query_param("platform", "agere"))
        .respond_with(ResponseTemplate::new(200).set_body_string(r#"["agere-plugin"]"#))
        .mount(&server)
        .await;
...
fn refresh_curated_plugin_cache_replaces_existing_local_version_with_short_sha_version() {
    ...
    assert!(
        !tmp.path()
            .join("plugins/cache/openagere-curated/slack/local")
            .exists()
    );
    assert!(
        tmp.path()
...
#[test]
fn curated_plugin_ids_from_config_keys_reads_latest_agere_home_user_config() {
    let tmp = tempfile::tempdir().unwrap();
    write_file(
        &tmp.path().join(CONFIG_TOML_FILE),
        r#"[features]
...
[plugins."slack@openagere-curated"]
enabled = true
...
"#,
    ...
    assert_eq!(
        configured_curated_plugin_ids_from_agere_home(tmp.path())
            .into_iter()
            .map(|plugin_id| plugin_id.as_key())
            .collect::<Vec<_>>(),
        vec!["slack@openagere-curated".to_string()]
    );
...
fn refresh_curated_plugin_cache_migrates_full_sha_cache_version_to_short_version() {
    ...
    assert!(
        !tmp.path()
            .join(format!(
                "plugins/cache/openagere-curated/slack/{TEST_CURATED_PLUGIN_SHA}"
            ))
            .exists()
    );
    assert!(
...
async fn load_plugins_ignores_project_config_files() {
    let agere_home = TempDir::new().unwrap();
    let project_root = agere_home.path().join("project");
    let plugin_root = agere_home
        .path()
        .join("plugins/cache")
        .join("test/sample/local");
    ...
    write_file(
        &project_root.join(".openagere/config.toml"),
        &plugin_config_toml(/*enabled*/ true, /*plugins_feature_enabled*/ true),
    );
    ...
    let stack = ConfigLayerStack::new(
        vec![ConfigLayerEntry::new(
            ConfigLayerSource::Project {
                dot_agere_folder: AbsolutePathBuf::try_from(project_root.join(".openagere"))
                    .unwrap(),
            },
    ...
    let outcome = load_plugins_from_layer_stack(
        &stack,
        &PluginStore::new(agere_home.path().to_path_buf()),
        Some(Product::Agere),
    )
    .await;
