...
use crate::agere_message_processor::AgereMessageProcessor;
use crate::agere_message_processor::AgereMessageProcessorArgs;
...
use agere_analytics::AppServerRpcTransport;
use agere_app_server_protocol::AuthMode as LoginAuthMode;
use agere_app_server_protocol::ChatgptAuthTokensRefreshParams;
use agere_app_server_protocol::ChatgptAuthTokensRefreshReason;
use agere_app_server_protocol::ChatgptAuthTokensRefreshResponse;
...
use agere_app_server_protocol::DeviceKeyCreateParams;
use agere_app_server_protocol::DeviceKeyPublicParams;
use agere_app_server_protocol::DeviceKeySignParams;
use agere_app_server_protocol::ExperimentalApi;
use agere_app_server_protocol::ExperimentalFeatureEnablementSetParams;
use agere_app_server_protocol::ExternalAgentConfigImportCompletedNotification;
...
use agere_app_server_protocol::ModelProviderCapabilitiesReadResponse;
use agere_app_server_protocol::ServerNotification;
use agere_app_server_protocol::ServerRequestPayload;
use agere_app_server_protocol::experimental_required_message;
...
use agere_login::auth::ExternalAuth;
use agere_login::auth::ExternalAuthRefreshContext;
use agere_login::auth::ExternalAuthRefreshReason;
use agere_login::auth::ExternalAuthTokens;
use agere_login::default_client::SetOriginatorError;
use agere_login::default_client::USER_AGENT_SUFFIX;
use agere_login::default_client::get_agere_user_agent;
...
pub(crate) struct MessageProcessor {
    outgoing: Arc<OutgoingMessageSender>,
    agere_message_processor: AgereMessageProcessor,
...
impl MessageProcessor {
    ...
    pub(crate) fn new(args: MessageProcessorArgs) -> Self {
        ...
        let agere_message_processor = AgereMessageProcessor::new(AgereMessageProcessorArgs {
        ...
        let external_agent_config_api =
            ExternalAgentConfigApi::new(config.agere_home.to_path_buf());
        ...
        Self::run_request_with_context(
            ...
            async {
                let result = async {
                    ...
                    let agere_request = serde_json::from_value::<ClientRequest>(request_json)
                        .map_err(|err| invalid_request(format!("Invalid request: {err}")))?;
    ...
    pub(crate) fn thread_created_receiver(&self) -> broadcast::Receiver<ThreadId> {
        self.agere_message_processor.thread_created_receiver()
    }
    ...
    pub(crate) async fn drain_background_tasks(&self) {
        self.agere_message_processor.drain_background_tasks().await;
    }

    pub(crate) async fn cancel_active_login(&self) {
        self.agere_message_processor.cancel_active_login().await;
    }
    ...
    pub(crate) async fn shutdown_threads(&self) {
        self.agere_message_processor.shutdown_threads().await;
    }
    ...
    async fn handle_client_request(
        ...
        if let ClientRequest::Initialize { request_id, params } = agere_request {
            // Handle Initialize internally so AgereMessageProcessor does not have to concern
            ...
            if let Err(error) = set_default_originator(originator.clone()) {
                match error {
                    ...
                    SetOriginatorError::AlreadyInitialized => {
                        ...
                        // TODO(owen): Once we remove support for AGERE_INTERNAL_ORIGINATOR_OVERRIDE,
            ...
            let user_agent = get_agere_user_agent();
    ...
    async fn dispatch_initialized_client_request(
        ...
        if let Some(reason) = agere_request.experimental_reason()
        ...
        | ClientRequest::TurnSteer { request_id, .. } = &agere_request
        {
        ...
        let serialization_scope = agere_request.serialization_scope();
    ...
    async fn handle_initialized_client_request(
        ...
        match agere_request {
            ...
            other => {
                ...
                // inline the full `AgereMessageProcessor::process_request` future, which
    ...
    async fn handle_model_provider_capabilities_read(&self, request_id: ConnectionRequestId) {
        let result = async {
            let config = self
                ...
                .await?;
            let provider =
                create_model_provider(config.model_provider, /*auth_manager*/ None, vec![]);
    ...
    async fn refresh_apps_list_after_experimental_feature_enablement_set(&self) {
        ...
        if !config.features.apps_enabled_for_auth(
            auth.as_ref()
                .is_some_and(agere_login::AgereAuth::uses_agere_backend),
        ) {}

        // ChatGPT connectors removed; skip connector refresh notification
    }
    ...
    async fn handle_config_mutation(&self) {
        self.agere_message_processor.handle_config_mutation();
