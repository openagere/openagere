...
fn write_sample_plugin_manifest_and_config(home: &TempDir) -> std::path::PathBuf {
    ...
    std::fs::create_dir_all(plugin_root.join(".agere-plugin")).expect("create plugin manifest dir");
    std::fs::write(
...
async fn build_plugin_test_agere(
    server: &MockServer,
    agere_home: Arc<TempDir>,
) -> Result<Arc<agere_core::AgereThread>> {
...
async fn build_analytics_plugin_test_agere(
    server: &MockServer,
    agere_home: Arc<TempDir>,
) -> Result<Arc<agere_core::AgereThread>> {
...
async fn build_apps_enabled_plugin_test_agere(
    server: &MockServer,
    agere_home: Arc<TempDir>,
...
async fn wait_for_sample_mcp_ready(agere: &agere_core::AgereThread) -> Result<()> {
...
async fn capability_sections_render_in_developer_message_in_order() -> Result<()> {
    ...
    let agere_home = Arc::new(TempDir::new()?);
    write_plugin_skill_plugin(agere_home.as_ref());
    write_plugin_app_plugin(agere_home.as_ref());
    let agere = build_apps_enabled_plugin_test_agere(
        &server,
        Arc::clone(&agere_home),
    ...
    agere
        .submit(Op::UserInput {
            environments: None,
            items: vec![agere_protocol::user_input::UserInput::Text {
                text: "hello".into(),
...
async fn explicit_plugin_mentions_inject_plugin_guidance() -> Result<()> {
    ...
    write_plugin_skill_plugin(agere_home.as_ref());
    write_plugin_mcp_plugin(agere_home.as_ref(), &rmcp_test_server_bin);
    write_plugin_app_plugin(agere_home.as_ref());

    let agere =
        build_apps_enabled_plugin_test_agere(&server, agere_home, apps_server.chatgpt_base_url)
            .await?;
    wait_for_sample_mcp_ready(&agere).await?;

    agere
        .submit(Op::UserInput {
            environments: None,
            items: vec![agere_protocol::user_input::UserInput::Mention {
                name: "sample".into(),
    ...
    assert!(
        request_tools
            .iter()
            .any(|name| name == "mcp__agere_apps__google_calendar"),
    ...
    let calendar_tool = request
        .tool_by_name("mcp__agere_apps__google_calendar", "_create_event")
...
async fn explicit_plugin_mentions_track_plugin_used_analytics() -> Result<()> {
    ...
    let agere_home = Arc::new(TempDir::new()?);
    write_plugin_skill_plugin(agere_home.as_ref());
    let agere = build_analytics_plugin_test_agere(&server, agere_home).await?;
    ...
    let plugin_event = loop {
        ...
        if let Some(event) = requests
            .into_iter()
            .filter(|request| request.url.path() == "/agere/analytics-events/events")
            .find_map(|request| {
                let payload: serde_json::Value = serde_json::from_slice(&request.body).ok()?;
                payload["events"].as_array().and_then(|events| {
                    events
                        .iter()
                        .find(|event| event["event_type"] == "agere_plugin_used")
                        .cloned()
                })
            })
        {
    ...
    assert_eq!(
        ...
        serde_json::json!(agere_login::default_client::originator().value)
    );
...
async fn plugin_mcp_tools_are_listed() -> Result<()> {
    ...
    write_plugin_mcp_plugin(agere_home.as_ref(), &rmcp_test_server_bin);
    let agere = build_plugin_test_agere(&server, agere_home).await?;
    wait_for_sample_mcp_ready(&agere).await?;

    agere.submit(Op::ListMcpTools).await?;
