...
Runtime orchestration for Phase 1 and Phase 2 still lives in `agere-core` under
`core/src/memories/`.

## Crates

- `memories/read` (`agere-memories-read`) owns the read path:
  memory developer-instruction injection, memory citation parsing, and
  read-usage telemetry classification.
- `memories/write` (`agere-memories-write`) owns the write path:
...
- In `agere`, edit those undated template files in place.
...
- keeps the memories root itself as a git-baseline directory, initialized under
  `~/.openagere/memories/.git` by `agere-git-utils`
