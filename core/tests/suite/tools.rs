...
async fn historical_unavailable_mcp_call_is_exposed_as_placeholder_tool() -> Result<()> {
    ...
    let mut builder = test_agere()
        .with_model("gpt-5.4")
        .with_home(Arc::clone(&agere_home))
    ...
    let rollout_path = test.agere.rollout_path().context("rollout path")?;
    ...
    let mut resume_builder = test_agere().with_model("gpt-5.4").with_config(|config| {
        config
            .features
            .enable(Feature::UnavailableDummyTools)
            .expect("unavailable dummy tools should be enabled for this test");
    ...
    let test = resume_builder
        .resume(&server, agere_home, rollout_path)
        .await?;
...
async fn shell_escalated_permissions_rejected_then_ok() -> Result<()> {
    ...
    let server = start_mock_server().await;
    let mut builder = test_agere().with_model("test-shell-json");
    let test = builder.build(&server).await?;
...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn policy_denied_shell_returns_original_output() -> Result<()> {
    skip_if_no_network!(Ok(()));
    ...
    let call_id = "policy-denied-shell";
    let target_path = fixture.workspace_path("policy-denied.txt");
    let sentinel = "policy-denied sentinel output";
    let command = vec![
        "/bin/sh".to_string(),
        "-c".to_string(),
        format!(
    ...
    fixture
        .submit_turn_with_permission_profile(
            "run a command that should be denied by read-only access restrictions",
    ...
    assert!(
        has_denial,
        "expected access denial details in tool output: {body}"
    );
    assert!(
    ...
    assert!(
        ...
        "expected access denial to mention denied path: {body}"
    );
    assert!(
        !body_lower.contains("failed in restricted execution"),
        "expected original tool output, found fallback message: {body}"
    );
    assert_ne!(
        exit_code, 0,
        "access denial should surface a non-zero exit code"
    );
...
async fn shell_enforces_glob_deny_read_policy() -> Result<()> {
    ...
    assert!(
        has_denial,
        "expected access denial details in shell output: {output_text}"
    );
...
async fn shell_spawn_failure_truncates_exec_error() -> Result<()> {
    ...
    let server = start_mock_server().await;
    let mut builder = test_agere().with_config(|cfg| {
        cfg.permissions
            .set_permission_profile(PermissionProfile::Disabled)
            .expect("set permission profile");
    });
