...
//!
//! # async fn example(state_db: std::sync::Arc<agere_state::StateRuntime>) {
...
mod tests {
    ...
    fn temp_agere_home() -> std::path::PathBuf {
        std::env::temp_dir().join(format!("agere-state-log-db-{}", Uuid::new_v4()))
    }
    ...
    async fn sqlite_feedback_logs_match_feedback_formatter_shape() {
        let agere_home = temp_agere_home();
