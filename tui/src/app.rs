...
use crate::terminal_info::user_agent;
#[cfg(test)]
...
use agere_ansi_escape::ansi_escape_line;
...
use agere_app_server_protocol::ConfigValueWriteParams;
...
use agere_app_server_protocol::MergeStrategy;
...
use agere_models_manager::model_presets::HIDE_GPT_5_1_AGERE_MAX_MIGRATION_PROMPT_CONFIG;
use agere_models_manager::model_presets::HIDE_GPT5_1_MIGRATION_PROMPT_CONFIG;
use agere_otel::SessionTelemetry;
use agere_protocol::ThreadId;
use agere_protocol::approvals::ExecApprovalRequestEvent;
...
#[cfg(target_os = "windows")]
use agere_protocol::protocol::FileSystemAccessLevel;
use agere_protocol::protocol::FinalOutput;
use agere_protocol::protocol::GetHistoryEntryResponseEvent;
use agere_protocol::protocol::ListSkillsResponseEvent;
#[cfg(test)]
use agere_protocol::protocol::McpAuthStatus;
...
fn command_execution_decision_to_review_decision(
    decision: agere_app_server_protocol::CommandExecutionApprovalDecision,
) -> agere_protocol::protocol::ReviewDecision {
    match decision {
...
fn default_exec_approval_decisions(
    network_approval_context: Option<&agere_protocol::protocol::NetworkApprovalContext>,
    proposed_execpolicy_amendment: Option<&agere_protocol::approvals::ExecPolicyAmendment>,
    proposed_network_policy_amendments: Option<
        &[agere_protocol::approvals::NetworkPolicyAmendment],
    >,
    additional_permissions: Option<&agere_protocol::models::AdditionalPermissionProfile>,
) -> Vec<agere_protocol::protocol::ReviewDecision> {
...
#[cfg(target_os = "windows")]
fn managed_filesystem_access_is_restricted(permission_profile: &PermissionProfile) -> bool {
    matches!(
...
fn list_skills_response_to_core(response: SkillsListResponse) -> ListSkillsResponseEvent {
    ListSkillsResponseEvent {
        skills: response
            .data
            .into_iter()
            .map(|entry| agere_protocol::protocol::SkillsListEntry {
                cwd: entry.cwd,
                skills: entry
                    .skills
                    .into_iter()
                    .map(|skill| agere_protocol::protocol::SkillMetadata {
                        name: skill.name,
                        description: skill.description,
                        short_description: skill.short_description,
                        interface: skill.interface.map(|interface| {
                            agere_protocol::protocol::SkillInterface {
                        ...
                        dependencies: skill.dependencies.map(|dependencies| {
                            agere_protocol::protocol::SkillDependencies {
                                tools: dependencies
                                    .tools
                                    .into_iter()
                                    .map(|tool| agere_protocol::protocol::SkillToolDependency {
                                        r#type: tool.r#type,
                                        value: tool.value,
                        ...
                        scope: match skill.scope {
                            agere_app_server_protocol::SkillScope::User => {
                                agere_protocol::protocol::SkillScope::User
                            }
                            agere_app_server_protocol::SkillScope::Repo => {
                                agere_protocol::protocol::SkillScope::Repo
                            }
                            agere_app_server_protocol::SkillScope::System => {
                                agere_protocol::protocol::SkillScope::System
                            }
                            agere_app_server_protocol::SkillScope::Admin => {
                                agere_protocol::protocol::SkillScope::Admin
                            }
                        },
                ...
                errors: entry
                    .errors
                    .into_iter()
                    .map(|error| agere_protocol::protocol::SkillErrorInfo {
                        path: error.path,
...
fn active_turn_not_steerable_turn_error(error: &TypedRequestError) -> Option<AppServerTurnError> {
    ...
    matches!(
        turn_error.agere_error_info,
        Some(AppServerAgereErrorInfo::ActiveTurnNotSteerable { .. })
    )
...
impl App {
    ...
    pub async fn run(
        ...
        should_prompt_windows_restriction_nux_at_startup: bool,
        open_provider_on_startup: bool,
        remote_app_server_url: Option<String>,
        remote_app_server_auth_token: Option<String>,
        environment_manager: Arc<EnvironmentManager>,
    ) -> Result<AppExitInfo> {
        ...
        let requires_provider_auth = bootstrap.requires_provider_auth;
        ...
        let session_telemetry = SessionTelemetry::new(
            ...
            auth_mode,
            agere_login::default_client::originator().value,
        ...
        {
            session_telemetry.counter("agere.status_line", /*inc*/ 1, &[]);
        }
        ...
        let (mut chat_widget, initial_started_thread) = match session_selection {
            ...
            SessionSelection::Resume(target_session) => {
                ...
                let previous_model_provider_id = config.model_provider_id.clone();
                Self::sync_config_to_resumed_session(&mut config, &resumed.session);
                model_catalog = Self::model_catalog_for_config_or_existing(
                    &config,
                    model_catalog.clone(),
                    previous_model_provider_id.as_str(),
                )
                .await;
                let init = crate::chatwidget::ChatWidgetInit {
                    config: config.clone(),
        ...
        chat_widget.maybe_prompt_windows_restriction_enable(
            should_prompt_windows_restriction_nux_at_startup,
        );
...
See the Agere keymap documentation for supported actions and examples."
            )
        })?;
        #[cfg(not(debug_assertions))]
        ...
        // On startup, if managed filesystem access is restricted, warn about
        // world-writable dirs on Windows.
        #[cfg(target_os = "windows")]
        {
            let startup_permission_profile = app.config.permissions.permission_profile();
            let should_check = app.config.permissions.windows_access_mode.is_some()
                && managed_filesystem_access_is_restricted(&startup_permission_profile)
                && !app
                    .config
                    .notices
            ...
            if should_check {
                ...
                let logs_base_dir = app.config.agere_home.clone();
        ...
        if open_provider_on_startup {
            app.chat_widget.open_provider_popup();
        }
        // Kick off a non-blocking rate-limit prefetch so the first `/status`
        // already has data, without delaying the initial frame render.
        if requires_provider_auth && has_chatgpt_account {
