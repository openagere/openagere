...
//! | `AGERE_HOME` | `OnceLock<Option<PathBuf>>` | Root for custom `.tmTheme` discovery |
//!
...
static AGERE_HOME: OnceLock<Option<PathBuf>> = OnceLock::new();
...
/// Set the user-configured syntax theme override and agere home path.
///
/// Call this with the **final resolved config** (after onboarding, resume, and
/// fork reloads complete). The first call persists `name` and `agere_home` in
...
pub(crate) fn set_theme_override(
    ...
    let warning = validate_theme_name(name.as_deref(), agere_home.as_deref());
    let override_set_ok = THEME_OVERRIDE.set(name.clone()).is_ok();
    let agere_home_set_ok = AGERE_HOME.set(agere_home.clone()).is_ok();
    ...
    if !override_set_ok || !agere_home_set_ok {
...
pub(crate) fn validate_theme_name(name: Option<&str>, agere_home: Option<&Path>) -> Option<String> {
    let name = name?;
    let custom_theme_path_display = agere_home
        .map(|home| custom_theme_path(name, home).display().to_string())
        .unwrap_or_else(|| format!("$AGERE_HOME/themes/{name}.tmTheme"));
...
fn custom_theme_path(name: &str, agere_home: &Path) -> PathBuf {
    agere_home.join("themes").join(format!("{name}.tmTheme"))
}

/// Try to load a custom `.tmTheme` file from `{agere_home}/themes/{name}.tmTheme`.
fn load_custom_theme(name: &str, agere_home: &Path) -> Option<Theme> {
    ThemeSet::get_theme(custom_theme_path(name, agere_home)).ok()
}
...
fn resolve_theme_with_override(name: Option<&str>, agere_home: Option<&Path>) -> Theme {
    ...
    if let Some(name) = name {
        ...
        // 2. Try loading {AGERE_HOME}/themes/{name}.tmTheme from disk.
        if let Some(home) = agere_home
...
fn build_default_theme() -> Theme {
    ...
    let agere_home = AGERE_HOME
        .get()
        .and_then(|agere_home| agere_home.as_deref());
    resolve_theme_with_override(name, agere_home)
}
...
pub(crate) fn configured_theme_name() -> String {
    ...
    if let Some(Some(name)) = THEME_OVERRIDE.get() {
        ...
        if let Some(Some(home)) = AGERE_HOME.get()
...
pub(crate) fn resolve_theme_by_name(name: &str, agere_home: Option<&Path>) -> Option<Theme> {
    let ts = two_face::theme::extra();
    // Bundled theme?
...
/// `.tmTheme` file under `{AGERE_HOME}/themes/`.
...
/// found in `{agere_home}/themes/`.
pub(crate) fn list_available_themes(agere_home: Option<&Path>) -> Vec<ThemeEntry> {
...
/// Highlight a single-line command, returning styled spans.
/// Newlines in the input are replaced with spaces to produce a single line.
/// Returns plain text fallback on failure.
pub(crate) fn highlight_bash_to_single_line(script: &str) -> Vec<Span<'static>> {
    let normalized = script.replace('\n', " ").replace('\r', "");
    highlight_to_line_spans(&normalized, "bash")
        .and_then(|lines| {
            lines
                .into_iter()
                .next()
                .map(|spans| spans.into_iter().collect())
        })
        .unwrap_or_else(|| vec![Span::from(normalized)])
}
...
<key>background</key><string>{deleted_background}</string>
    ...
    fn unique_foreground_colors_for_theme(theme_name: &str) -> Vec<String> {
        let theme = resolve_theme_by_name(theme_name, /*agere_home*/ None)
    ...
    fn bundled_theme_can_provide_diff_scope_backgrounds() {
        let theme = resolve_theme_by_name("github", /*agere_home*/ None)
    ...
    fn validate_theme_name_none_for_bundled() {
        ...
        assert!(validate_theme_name(Some("dracula"), /*agere_home*/ None).is_none());
        assert!(validate_theme_name(Some("nord"), Some(Path::new("/nonexistent"))).is_none());
    }

    #[test]
    fn validate_theme_name_none_when_no_override() {
        assert!(validate_theme_name(/*name*/ None, /*agere_home*/ None).is_none());
    }
