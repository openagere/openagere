...
/// keeps control over process spawning, output capture, and filesystem isolation integration.
/// Implementations can capture any execution restriction state they need.
...
mod tests {
    use super::*;
    use agere_protocol::approvals::EscalationPermissions;
    ...
    /// executable paths.
    ///
    /// The `/bin/zsh` and `/tmp/agere-execve-wrapper` values here are
    ...
    async fn start_session_exposes_wrapper_env_overlay() -> anyhow::Result<()> {
        ...
        let execve_wrapper = PathBuf::from("/tmp/agere-execve-wrapper");
    ...
    async fn exec_closes_parent_socket_after_shell_spawn() -> anyhow::Result<()> {
        ...
        let server = EscalateServer::new(
            ...
            PathBuf::from("/tmp/agere-execve-wrapper"),
            DeterministicEscalationPolicy {
                decision: EscalationDecision::run(),
            },
        );
    ...
    #[tokio::test]
    async fn handle_escalate_session_respects_escalation_execution_decision() -> anyhow::Result<()>
    {
        let _guard = ESCALATE_SERVER_TEST_LOCK.acquire().await?;
        ...
        for i in 0..10 {
            ...
            env.insert(format!("AGERE_TEST_VAR{i}"), value);
        }
    ...
    async fn handle_escalate_session_executes_escalated_command() -> anyhow::Result<()> {
        ...
        let server_task = tokio::spawn(handle_escalate_session_with_policy(
            server,
            Arc::new(DeterministicEscalationPolicy {
                decision: EscalationDecision::escalate(EscalationExecution::Unrestricted),
            }),
            Arc::new(ForwardingShellCommandExecutor),
            CancellationToken::new(),
            CancellationToken::new(),
