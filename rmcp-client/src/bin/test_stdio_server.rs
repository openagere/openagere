...
const MEMO_URI: &str = "memo://agere/example-note";
const MEMO_CONTENT: &str = "This is a sample MCP resource served by the rmcp test server.";
const EXECUTION_STATE_META_CAPABILITY: &str = "agere/execution-state-meta";
...
impl TestToolServer {
    fn new() -> Self {
        ...
        let execution_state_meta_schema: JsonObject = serde_json::from_value(serde_json::json!({
            "type": "object",
            "properties": {},
            "additionalProperties": false
        }))
        .expect("execution_state_meta tool schema should deserialize");
        let mut execution_state_meta_tool = Tool::new(
            Cow::Borrowed("execution_state_meta"),
            Cow::Borrowed("Return the MCP request metadata received by this test server."),
            Arc::new(execution_state_meta_schema),
        );
        execution_state_meta_tool.annotations = Some(ToolAnnotations::new().read_only(true));
    ...
    /// Tool intended for manual testing of Agere TUI rendering for MCP image tool results.
    ///
    ...
    ///
    /// Manual testing approach (Agere TUI):
    /// - Build this binary: `cargo build -p agere-rmcp-client --bin test_stdio_server`
    /// - Register it:
    ///   - `agere mcp add mcpimg -- /abs/path/to/test_stdio_server`
    /// - Then in Agere TUI, ask it to call:
    ...
    fn memo_template() -> ResourceTemplate {
        let raw = RawResourceTemplate {
            uri_template: "memo://agere/{slug}".to_string(),
            name: "agere-memo".to_string(),
            title: Some("Agere Memo".to_string()),
            description: Some(
                "Template for memo://agere/{slug} resources used in tests.".to_string(),
            ),
            mime_type: Some("text/plain".to_string()),
            icons: None,
        };
...
/// Scenarios for `image_scenario`, intended to exercise Agere TUI handling of MCP image outputs.
///
...
impl ServerHandler for TestToolServer {
    fn get_info(&self) -> ServerInfo {
        ...
        capabilities.experimental = Some(BTreeMap::from([(
            EXECUTION_STATE_META_CAPABILITY.to_string(),
            JsonObject::new(),
        )]));
    ...
    async fn call_tool(
        ...
        match request.name.as_ref() {
            "execution_state_meta" => Ok(CallToolResult {
                content: Vec::new(),
