        ...
        .record_into_history(
            &[
                ...
                ResponseItem::FunctionCall {
                    ...
                    namespace: None,
                    arguments: "{\"repo\":\"openai/agere\"}".to_string(),
...
fn normalize_guardian_snapshot_paths(text: String) -> String {
    let mut text = text;
    for canonical_path in ["/repo/core", "/repo"] {
...
fn guardian_prompt_text(items: &[agere_protocol::user_input::UserInput]) -> String {
    items
        .iter()
        .map(|item| match item {
            agere_protocol::user_input::UserInput::Text { text, .. } => text.as_str(),
            _ => "",
        })
        .collect::<String>()
}
...
async fn build_guardian_prompt_full_mode_preserves_initial_review_format() -> anyhow::Result<()> {
    ...
    let prompt = build_guardian_prompt_items(
        session.as_ref(),
        Some(r#"Network access to "github.com" is blocked by policy."#.to_string()),
        GuardianApprovalRequest::Shell {
            id: "shell-1".to_string(),
            command: vec!["git".to_string(), "push".to_string()],
            cwd: test_path_buf("/repo/core").abs(),
            permission_level: crate::execution::ExecutionPermissionLevel::UseDefault,
    ...
    assert!(text.contains("The Agere agent has requested the following action:\n"));
...
async fn build_guardian_prompt_delta_mode_preserves_original_numbering() -> anyhow::Result<()> {
    ...
    assert!(text.contains("The Agere agent has requested the following next action:\n"));
...
async fn build_guardian_prompt_items_explains_network_access_review_scope() -> anyhow::Result<()> {
    ...
    settings.bind(|| {
        assert_snapshot!(
            "agere_core__guardian__tests__network_access_guardian_prompt_layout",
...
fn build_guardian_transcript_preserves_recent_tool_context_when_user_history_is_large() {
    ...
    entries.extend([
        ...
        GuardianTranscriptEntry {
            ...
            text: r#"Network access to "example.com" is blocked by policy."#.to_string(),
        },
    ]);
    ...
    assert!(
        transcript.iter().any(|entry| entry.contains(
            r#"tool shell result: Network access to "example.com" is blocked by policy."#,
        ))
    );
    assert_eq!(
        omission,
...
-> anyhow::Result<()> {
    ...
    assert!(matches!(
        ...
        Some(agere_analytics::GuardianReviewSessionKind::TrunkNew)
    ));
    ...
    settings.bind(|| {
        assert_snapshot!(
            "agere_core__guardian__tests__guardian_review_request_layout",
...
async fn build_guardian_prompt_items_includes_parent_session_id() -> anyhow::Result<()> {
    ...
    let prompt_text = prompt
        ...
        .into_iter()
        .map(|item| match item {
            agere_protocol::user_input::UserInput::Text { text, .. } => text,
            agere_protocol::user_input::UserInput::Image { .. } => String::new(),
            _ => String::new(),
        })
        .collect::<String>();
...
async fn guardian_reuses_prompt_cache_key_and_appends_prior_reviews() -> anyhow::Result<()> {
    ...
    assert!(matches!(
        ...
        Some(agere_analytics::GuardianReviewSessionKind::TrunkReused)
    ));
    assert!(matches!(
    ...
    settings.bind(|| {
        assert_snapshot!(
            "agere_core__guardian__tests__guardian_followup_review_request_layout",
            format!(
...
async fn guardian_review_session_config_preserves_parent_network_proxy() {
    ...
    let guardian_config = build_guardian_review_session_config_for_test(
        ...
        "parent-active-model",
        Some(agere_protocol::openai_models::ReasoningEffort::Low),
    )
    ...
    assert_eq!(
        ...
        Constrained::allow_only(PermissionProfile::from_legacy_access_policy("read-only",))
    );
}
    ...
    assert_eq!(
        ...
        Some(
            NetworkProxySpec::from_config_and_constraints(
                live_network,
                /*requirements*/ None,
                &PermissionProfile::from_legacy_access_policy("read-only",),
            )
