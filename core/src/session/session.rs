...
type PendingModelProvider = (
    String,
    agere_model_provider_info::ModelProviderInfo,
    Arc<Config>,
);
type PendingModelProviders = HashMap<String, PendingModelProvider>;
...
pub(crate) struct Session {
    ...
    /// Fresh provider staged by the app-server boundary to be applied atomically
    /// with the next turn's overrides. Carries the full ModelProviderInfo (which the
    /// protocol Op cannot, due to a dependency-cycle constraint).
    ///
    /// The app-server stages the provider immediately before submitting the turn
    /// op, and the core op handler `take`s the entry keyed by the matching
    /// submission id.
    ///
    /// An orphaned stage (for example, `submit_core_op` fails after staging) is
    /// intentionally benign. Ordinary turns and same-id provider echoes submit
    /// `model_provider: None`, so they never `take` this map. A later real
    /// provider switch stages under its own submission id, so concurrent
    /// turn/start calls cannot overwrite each other's provider payloads.
    pub(super) pending_model_providers: std::sync::Mutex<PendingModelProviders>,
}
...
pub(crate) struct SessionConfiguration {
    ...
    pub(super) windows_execution_restriction_level: WindowsExecutionRestrictionLevel,

    /// Absolute working directory that should be treated as the *root* of the
    /// session. All relative paths supplied by the model as well as the
    /// active execution access profile are resolved against this directory **instead** of
    /// the process-wide current working directory.
    pub(super) cwd: AbsolutePathBuf,
    /// Directory containing all Agere state for this session.
    pub(super) agere_home: AbsolutePathBuf,
...
impl SessionConfiguration {
    pub(crate) fn agere_home(&self) -> &AbsolutePathBuf {
        &self.agere_home
    }
    ...
    pub(super) fn file_system_access_policy(&self) -> FileSystemAccessPolicy {
        self.permission_profile.get().file_system_access_policy()
    }

    pub(super) fn network_access_policy(&self) -> NetworkAccessPolicy {
        self.permission_profile.get().network_access_policy()
    }

    pub(super) fn access_policy(&self) -> String {
    ...
    pub(crate) fn apply(&self, updates: &SessionSettingsUpdate) -> ConstraintResult<Self> {
        ...
        let current_file_system_access_policy = self.file_system_access_policy();
        let _current_network_access_policy = self.network_access_policy();
        if let Some((provider_id, provider)) = updates.provider.clone() {
            next_configuration.provider_id = provider_id.clone();
            next_configuration.provider = provider.clone();
            let mut cfg = (*next_configuration.original_config_do_not_use).clone();
            cfg.model_provider_id = provider_id;
            cfg.model_provider = provider;
            next_configuration.original_config_do_not_use = std::sync::Arc::new(cfg);
        }
        // A provider switch carries the fresh config it was resolved from. Refresh the
        // model-scoped scalars that `to_models_manager_config()` feeds into
        // `with_config_overrides` — context window, auto-compact limit, tool-output
        // limit, reasoning-summary support — so this turn's model metadata matches the
        // NEW provider instead of the session-start config. (The provider's model
        // catalog / `models` are refreshed separately via the rebuilt models manager,
        // which `with_config_overrides` does not read, so they are intentionally not
        // copied here.)
        if let Some(fresh) = updates.provider_config.as_ref() {
            let mut cfg = (*next_configuration.original_config_do_not_use).clone();
            cfg.model_context_window = fresh.model_context_window;
            cfg.model_auto_compact_token_limit = fresh.model_auto_compact_token_limit;
            cfg.tool_output_token_limit = fresh.tool_output_token_limit;
            cfg.model_supports_reasoning_summaries = fresh.model_supports_reasoning_summaries;
            next_configuration.original_config_do_not_use = std::sync::Arc::new(cfg);
        }
        ...
        if let Some(windows_execution_restriction_level) =
            updates.windows_execution_restriction_level
        {
            next_configuration.windows_execution_restriction_level =
        ...
        if let Some(permission_profile) = updates.permission_profile.clone() {
            next_configuration.set_permission_profile_projection(
                permission_profile,
                Some(&current_file_system_access_policy),
            )?;
        }
    ...
    fn set_permission_profile_projection(
        ...
        preserve_deny_reads_from: Option<&FileSystemAccessPolicy>,
    ) -> ConstraintResult<()> {
...
pub(crate) struct SessionSettingsUpdate {
    pub(crate) provider: Option<(String, ModelProviderInfo)>,
    pub(crate) provider_config: Option<Arc<Config>>,
...
impl Session {
    /// Stage a fresh provider (resolved against the latest config by the
    /// app-server boundary) to be applied atomically with the next turn.
    pub(crate) fn stage_model_provider(
        &self,
        ...
        self.pending_model_providers
            .lock()
            .unwrap_or_else(std::sync::PoisonError::into_inner)
            .insert(submission_id, (provider_id, provider, config));
    }

    /// Take (and clear) the staged provider only when it matches the triggering op.
    pub(crate) fn take_pending_model_provider_for(
        &self,
        ...
        let mut pending = self
            .pending_model_providers
            .lock()
            .unwrap_or_else(std::sync::PoisonError::into_inner);
        if pending
            .get(expected_submission_id)
            .is_some_and(|(id, _, _)| id == expected_provider_id)
        {
            pending.remove(expected_submission_id)
        } else {
            None
        }
    }

    /// Clear the staged provider for one submission, if any.
    pub(crate) fn clear_pending_model_provider_for(&self, submission_id: &str) {
        self.pending_model_providers
            .lock()
            .unwrap_or_else(std::sync::PoisonError::into_inner)
    ...
    /// Take (and clear) the staged provider, if any.
    pub(crate) fn take_pending_model_provider(
        &self,
    ) -> Option<(
        String,
        agere_model_provider_info::ModelProviderInfo,
    ...
    pub(crate) async fn new(
        ...
        let session_result: anyhow::Result<Arc<Self>> = async {
            ...
            let trace_agent_path = session_configuration
                .session_source
                .get_agent_path()
                .unwrap_or_else(agere_protocol::AgentPath::root);
            let trace_task_name =
            ...
            let config_path = config.agere_home.join(CONFIG_TOML_FILE);
            ...
            let auth_mode = auth.map(AgereAuth::auth_mode).map(TelemetryAuthMode::from);
            let account_id = auth.and_then(AgereAuth::get_account_id);
            let account_email = auth.and_then(AgereAuth::get_account_email);
            let originator = originator().value;
            let terminal_type = user_agent();
            let session_model = session_configuration.collaboration_mode.model().to_string();
            let auth_env_telemetry = collect_auth_env_telemetry(
                &session_configuration.provider,
                auth_manager.agere_api_key_env_enabled(),
            );
            ...
            session_telemetry.conversation_starts(
                ...
                config
                    .permissions
                    .network_access_policy().to_string(),
            ...
            let thread_name =
                thread_title_from_state_db(state_db_ctx.as_ref(), &config.agere_home, conversation_id)
                    .instrument(info_span!(
            ...
            let hooks = Hooks::new(HooksConfig {
                ...
                feature_enabled: config.features.enabled(Feature::AgereHooks),
            ...
            let installation_id = resolve_installation_id(&config.agere_home).await?;
            ...
            let swappable_models_manager = std::sync::Arc::new(
                agere_models_manager::SwappableModelsManager::new(
                    std::sync::Arc::clone(&models_manager),
                ),
            );
            ...
            let (mcp_connection_manager, cancel_token) = McpConnectionManager::new(
                ...
                agere_apps_tools_cache_key(auth),
            ...
            let session_start_source = match &initial_history {
                InitialHistory::Resumed(_) => agere_hooks::SessionStartSource::Resume,
                InitialHistory::New | InitialHistory::Forked(_) => {
                    agere_hooks::SessionStartSource::Startup
                }
                InitialHistory::Cleared => agere_hooks::SessionStartSource::Clear,
            };
    ...
    /// 用给定 provider 与会话现有 config，构建该 provider 自己的 models manager。
    /// 供 provider 切换时热替换使用。
    pub(super) async fn build_provider_models_manager(
        ...
        let (agere_home, model_catalog, models) = if let Some(cfg) = config {
            (
        ...
        let runtime_provider = create_model_provider(
            provider.clone(),
            Some(std::sync::Arc::clone(&self.services.auth_manager)),
            models,
        );
        runtime_provider.models_manager(agere_home, model_catalog, collaboration_modes_config)
    }

    /// Apply a provider switch to the SESSION-level runtime: the wire `model_client`
    /// (which actually issues requests, so its base_url/wire_api must change) and the
    /// per-session models manager. Both the persistent `update_settings` path and the
    /// per-turn `new_turn_with_sub_id` path call this so a switch is never only written
    /// to `session_configuration` while the request keeps using the old provider.
    pub(super) async fn apply_provider_to_runtime(
        &self,
        provider: agere_model_provider_info::ModelProviderInfo,
        config: Option<Arc<Config>>,
    ) {
        self.services.model_client.set_provider(provider.clone());
        let new_manager = self.build_provider_models_manager(&provider, config).await;
