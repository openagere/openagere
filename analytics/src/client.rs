...
impl AnalyticsEventsClient {
    ...
    pub fn track_compaction(&self, event: crate::facts::AgereCompactionEvent) {
...
async fn send_track_events(
    ...
    let url = format!("{base_url}/agere/analytics-events/events");
    let payload = TrackEventsRequest { events };

    let response = create_client()
        .post(&url)
        .timeout(ANALYTICS_EVENTS_TIMEOUT)
        .headers(agere_model_provider::auth_provider_from_auth(&auth).to_auth_headers())
