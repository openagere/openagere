...
use agere_tools::DiscoverablePluginInfo;
...
async fn list_tool_suggest_discoverable_plugins_returns_uninstalled_curated_plugins() {
    ...
    let curated_root = curated_plugins_repo_path(agere_home.path());
    write_openai_curated_marketplace(&curated_root, &["sample", "slack"]);
    write_plugins_feature_config(agere_home.path());

    let config = load_plugins_config(agere_home.path()).await;
    ...
    assert_eq!(
        ...
        vec![DiscoverablePluginInfo {
            id: "slack@openagere-curated".to_string(),
...
async fn list_tool_suggest_discoverable_plugins_deduplicates_allowlisted_configured_plugin() {
        ...
        .find(|plugin_id| {
            plugin_id
                ...
                .is_some_and(|(_plugin_name, marketplace_name)| {
                    marketplace_name == OPENAGERE_BUNDLED_MARKETPLACE_NAME
                })
        })
    ...
    let marketplace_root = agere_home
        .path()
...
"#
    ...
    write_file(
        &agere_home.path().join(crate::config::CONFIG_TOML_FILE),
        &format!(
            r#"[features]
plugins = true
...
"#
    ...
    assert_eq!(discoverable_plugins.len(), 1);
    assert_eq!(discoverable_plugins[0].id, "slack@openagere-curated");
}
...
async fn list_tool_suggest_discoverable_plugins_normalizes_description() {
    ...
    write_plugins_feature_config(agere_home.path());
    write_file(
        &curated_root.join("plugins/slack/.agere-plugin/plugin.json"),
        r#"{
  "name": "slack",
  "description": "  Plugin\n   with   extra   spacing  "
}"#,
...
async fn list_tool_suggest_discoverable_plugins_omits_installed_curated_plugins() {
    ...
    let curated_root = curated_plugins_repo_path(agere_home.path());
    write_openai_curated_marketplace(&curated_root, &["slack"]);
    write_curated_plugin_sha(agere_home.path());
    write_plugins_feature_config(agere_home.path());

    PluginsManager::new(agere_home.path().to_path_buf())
    ...
    let refreshed_config = load_plugins_config(agere_home.path()).await;
...
[tool_suggest]
discoverables = [{ type = "plugin", id = "sample@openagere-curated" }]
"#,
    );
    ...
    assert_eq!(
        discoverable_plugins,
        vec![DiscoverablePluginInfo {
            id: "sample@openagere-curated".to_string(),
...
async fn list_tool_suggest_discoverable_plugins_does_not_reload_marketplace_per_plugin() {
    ...
    for plugin_name in ["build-ios-apps", "life-science-research"] {
        write_file(
            &curated_root.join(format!("plugins/{plugin_name}/.agere-plugin/plugin.json")),
            &format!(
                r#"{{
  "name": "{plugin_name}",
  ...
  "interface": {{
    ...
    assert_eq!(
        normalized_logs
            .matches("build-ios-apps/.agere-plugin/plugin.json")
            .count(),
        1
    );
    assert_eq!(
        normalized_logs
            .matches("life-science-research/.agere-plugin/plugin.json")
            .count(),
        1
    );
}
