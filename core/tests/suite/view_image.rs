...
fn disabled_user_turn(test: &TestAgere, items: Vec<UserInput>, model: String) -> Op {
...
async fn create_workspace_directory(test: &TestAgere, rel_path: &str) -> anyhow::Result<PathBuf> {
...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn view_image_tool_can_preserve_original_resolution_when_requested_on_gpt5_3_agere()
-> anyhow::Result<()> {
    skip_if_no_network!(Ok(()));
