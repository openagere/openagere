...
use agere_state::LogQuery;
use agere_state::LogRow;
...
struct Args {
    ...
    /// Direct path to the logs SQLite database. Overrides --agere-home.
    #[arg(long)]
    db: Option<PathBuf>,
...
async fn main() -> anyhow::Result<()> {
    ...
    let agere_home = db_path
        .parent()
        .map(ToOwned::to_owned)
        .unwrap_or_else(|| PathBuf::from("."));
    let runtime = StateRuntime::init(agere_home, "logs-client".to_string()).await?;
...
fn resolve_db_path(args: &Args) -> anyhow::Result<PathBuf> {
    ...
    let agere_home = args.agere_home.clone().unwrap_or_else(default_agere_home);
    Ok(agere_state::logs_db_path(agere_home.as_path()))
}

fn default_agere_home() -> PathBuf {
    if let Some(home) = home_dir() {
        return home.join(".openagere");
    }
    PathBuf::from(".openagere")
}
...
mod tests {
    ...
    fn log_level_rejects_aliases_and_unknown_values() {
        assert!(Args::try_parse_from(["agere-state-logs", "--level", "warning"]).is_err());
        assert!(Args::try_parse_from(["agere-state-logs", "--level", "err"]).is_err());
        assert!(Args::try_parse_from(["agere-state-logs", "--level", "warn,error"]).is_err());
    }

    #[test]
    fn log_level_accepts_canonical_values_case_insensitively() {
        let args = Args::try_parse_from(["agere-state-logs", "--level", "WARN"])
