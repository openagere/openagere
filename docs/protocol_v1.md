...
These are entities exit on the OpenOpenAgere backend. The intent of this section is to establish vocabulary and construct a shared mental model for the `OpenAgere` core system.

0. `Model`
   - In our case, this is the Responses REST API
1. `OpenAgere`
   - The core engine of OpenAgere
...
2. `Session`
   - The `OpenAgere`'s current configuration and state
   - `OpenAgere` starts with no `Session`, and it is initialized by `Op::ConfigureSession`, which should be the first message sent by the UI.
...
3. `Task`
   - A `Task` is `OpenAgere` executing work in response to user input.
...
4. `Turn`
   - One cycle of iteration in a `Task`, consists of:
     ...
     - `OpenAgere` then executes command(s), applies patch(es), and outputs message(s) returned by the `Model`
...
The term "UI" is used to refer to the application driving `OpenAgere`. This may be the CLI / TUI chat-like interface that users operate, or it may be a GUI interface like a VSCode extension. The UI is external to `OpenAgere`, as `OpenAgere` is intended to be operated by arbitrary UI implementations.
...
Since only 1 `Task` can be run at a time, for parallel tasks it is recommended that a single `OpenAgere` be run for each thread of work.
...
- `Submission`
  - These are messages sent on the `SQ` (UI -> `OpenAgere`)
...
- `Event`
  - These are messages sent on the `EQ` (`OpenAgere` -> UI)
...
- `Op`
  - `Op::UserTurn` – Any input from the user to kick off a `Turn`, including full per-turn context such as cwd, model, permission profile, approval policy, and optional `approvals_reviewer`
...
sequenceDiagram
    ...
    box Daemon
    participant OpenAgere as OpenAgere
    ...
    user->>OpenAgere: Op::ConfigureSession
    OpenAgere-->>session: create session
    OpenAgere->>user: Event::SessionConfigured
