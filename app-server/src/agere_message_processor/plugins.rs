...
impl AgereMessageProcessor {
    ...
    async fn plugin_list_response(
        ...
        let (mut data, marketplace_load_errors) = match tokio::task::spawn_blocking(move || {
            ...
            Ok::<
                (
                    ...
                    Vec<agere_app_server_protocol::MarketplaceLoadErrorInfo>,
                ),
                MarketplaceError,
            >((
                outcome
                ...
                outcome
                    .errors
                    .into_iter()
                    .map(|err| agere_app_server_protocol::MarketplaceLoadErrorInfo {
            ...
            && agere_core_plugins::remote::remote_plugin_backend_supported()
        {
            ...
            match agere_core_plugins::remote::fetch_remote_marketplaces(
        ...
        let featured_plugin_ids = if data
            .iter()
            .any(|marketplace| marketplace.name == OPENAGERE_CURATED_MARKETPLACE_NAME)
        {
    ...
    async fn plugin_read_response(
        ...
        let plugin = match read_source {
            ...
            Err(remote_marketplace_name) => {
                ...
                let remote_detail = agere_core_plugins::remote::fetch_remote_plugin_detail(
                    &remote_plugin_service_config,
                    auth.as_ref(),
                    &remote_marketplace_name,
                    &plugin_name,
                ...
                let plugin_apps = remote_detail
                    ...
                    .iter()
                    .cloned()
                    .map(agere_core::plugins::AppConnectorId)
                    .collect::<Vec<_>>();
                let environment_manager = self.thread_manager.environment_manager();
                let app_summaries = plugin_app_helpers::load_plugin_app_summaries(
                    &config,
    ...
    async fn plugin_install_response(
        ...
        {
            return Err(invalid_request(
                "Agere plugins are disabled for this workspace",
            ));
        }
        ...
        let apps_needing_auth = self
            .plugin_apps_needing_auth_for_install(
                &config,
                auth.as_ref().is_some_and(AgereAuth::is_chatgpt_auth),
    ...
    async fn remote_plugin_install_response(
        ...
        let remote_detail =
            agere_core_plugins::remote::fetch_remote_plugin_detail_with_download_urls(
                &remote_plugin_service_config,
                auth.as_ref(),
                &remote_marketplace_name,
                &plugin_name,
        ...
        let validated_bundle = agere_core_plugins::remote_bundle::validate_remote_plugin_bundle(
            &plugin_name,
            &remote_marketplace_name,
        ...
        let result = agere_core_plugins::remote_bundle::download_and_install_remote_plugin_bundle(
        ...
        agere_core_plugins::remote::install_remote_plugin(
            &remote_plugin_service_config,
            auth.as_ref(),
            &remote_marketplace_name,
            &plugin_name,
    ...
    /// Stub: ChatGPT connectors removed. Returns empty list.
    async fn plugin_apps_needing_auth_for_install(
        &self,
        _config: &Config,
        _is_chatgpt_auth: bool,
        _plugin_id: &str,
        _plugin_apps: &[agere_core::plugins::AppConnectorId],
    ) -> Vec<AppSummary> {
        Vec::new()
    }
    ...
    async fn plugin_uninstall_response(
        ...
        if agere_core::plugins::PluginId::parse(&plugin_id).is_err()
    ...
    async fn remote_plugin_uninstall_response(
        ...
        agere_core_plugins::remote::uninstall_remote_plugin(
            &remote_plugin_service_config,
            auth.as_ref(),
            config.agere_home.to_path_buf(),
            &plugin_id,
...
fn remote_plugin_catalog_error_to_jsonrpc(
    ...
    match err {
        ...
        | RemotePluginCatalogError::Unsupported(_) => JSONRPCErrorError {
...
fn remote_plugin_bundle_install_error_to_jsonrpc(
    err: agere_core_plugins::remote_bundle::RemotePluginBundleInstallError,
) -> JSONRPCErrorError {
