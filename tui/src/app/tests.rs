...
use agere_app_server_protocol::AdditionalFileSystemPermissions;
use agere_app_server_protocol::AdditionalNetworkPermissions;
use agere_app_server_protocol::AdditionalPermissionProfile;
...
use agere_app_server_protocol::ConfigWarningNotification;
...
use agere_app_server_protocol::NetworkApprovalProtocol as AppServerNetworkApprovalProtocol;
use agere_app_server_protocol::NetworkPolicyAmendment as AppServerNetworkPolicyAmendment;
use agere_app_server_protocol::NetworkPolicyRuleAction as AppServerNetworkPolicyRuleAction;
use agere_app_server_protocol::NonSteerableTurnKind as AppServerNonSteerableTurnKind;
use agere_app_server_protocol::PatchChangeKind;
use agere_app_server_protocol::PermissionsRequestApprovalParams;
use agere_app_server_protocol::RequestId as AppServerRequestId;
...
use agere_app_server_protocol::TokenUsageBreakdown;
...
use agere_app_server_protocol::TurnError as AppServerTurnError;
use agere_app_server_protocol::TurnStartedNotification;
use agere_app_server_protocol::TurnStatus;
use agere_app_server_protocol::UserInput as AppServerUserInput;
...
#[tokio::test]
async fn sync_config_to_resumed_session_uses_actual_session_model() {
    let app = make_test_app().await;
    let mut config = app.config.clone();
    config.model = Some("qwen3.6-plus".to_string());
    let current_provider = config.model_provider.clone();
    let resumed_provider = agere_model_provider_info::ModelProviderInfo {
        base_url: Some("https://resumed.example.com/v1".to_string()),
        ..Default::default()
    };
    config
        .model_providers
        .insert("recorded-provider".to_string(), resumed_provider.clone());
    let mut session = test_thread_session(ThreadId::new(), app.config.cwd.to_path_buf());
    session.model = "glm-5".to_string();
    session.model_provider_id = "recorded-provider".to_string();

    App::sync_config_to_resumed_session(&mut config, &session);

    assert_eq!(config.model.as_deref(), Some("glm-5"));
    assert_eq!(config.model_provider_id, "recorded-provider");
    assert_eq!(config.model_provider, resumed_provider);
    assert_ne!(config.model_provider, current_provider);
}

#[tokio::test]
async fn sync_config_to_resumed_session_keeps_existing_model_when_session_model_empty() {
    let app = make_test_app().await;
    let mut config = app.config.clone();
    config.model = Some("qwen3.6-plus".to_string());
    let mut session = test_thread_session(ThreadId::new(), app.config.cwd.to_path_buf());
...
#[tokio::test]
async fn sync_config_to_resumed_session_keeps_existing_model_when_provider_missing() {
    let app = make_test_app().await;
    let mut config = app.config.clone();
...
#[tokio::test]
async fn model_catalog_from_config_uses_current_provider_models() {
    let app = make_test_app().await;
    let mut config = app.config.clone();
    config.model = Some("resumed-model".to_string());
    ...
    assert_eq!(
        catalog.find_model_context_window("resumed-model"),
        Some(123_456)
    );
    assert_eq!(catalog.find_model_context_window("qwen3.6-plus"), None);
}

#[tokio::test]
async fn builtin_model_catalog_refresh_is_skipped_for_same_provider() {
    let app = make_test_app().await;
    let mut config = app.config.clone();
    config.model_provider_id = agere_model_provider_info::OPENAI_PROVIDER_ID.to_string();

    assert!(!App::should_refresh_builtin_model_catalog(
        &config,
...
#[tokio::test]
async fn builtin_model_catalog_refreshes_for_different_provider() {
    let app = make_test_app().await;
    let mut config = app.config.clone();
    config.model_provider_id = agere_model_provider_info::AMAZON_BEDROCK_PROVIDER_ID.to_string();
...
#[tokio::test]
async fn custom_model_catalog_refresh_does_not_use_builtin_model_list() {
    let app = make_test_app().await;
    let mut config = app.config.clone();
    config.model_provider_id = "recorded-provider".to_string();
...
#[tokio::test]
async fn model_catalog_from_config_rebuilds_for_custom_provider() {
    let app = make_test_app().await;
    let mut config = app.config.clone();
    config.model_provider_id = "recorded-provider".to_string();
    config.model = Some("recorded-model".to_string());
    config.models = vec![agere_config::config_toml::ModelConfig {
    ...
    assert_eq!(
        catalog.find_model_context_window("recorded-model"),
        Some(222_333)
    );
...
#[tokio::test]
async fn handle_thread_session_syncs_chat_widget_config_model_and_provider() {
    let mut app = make_test_app().await;
    let mut config = app.chat_widget.config_ref().clone();
    config.model = Some("old-model".to_string());
    let resumed_provider = agere_model_provider_info::ModelProviderInfo {
        base_url: Some("https://resumed.example.com/v1".to_string()),
        ..Default::default()
    };
    ...
    assert_eq!(app.chat_widget.config_ref().model.as_deref(), Some("glm-5"));
    assert_eq!(
...
#[tokio::test]
async fn session_configured_keeps_chat_widget_config_model_when_provider_missing() {
    let mut app = make_test_app().await;
    let original_model = app.chat_widget.config_ref().model.clone();
    let original_provider_id = app.chat_widget.config_ref().model_provider_id.clone();
    let original_provider = app.chat_widget.config_ref().model_provider.clone();
    ...
    app.chat_widget.handle_agere_event(Event {
        ...
        msg: EventMsg::SessionConfigured(SessionConfiguredEvent {
            ...
            model_provider_id: "missing-provider".to_string(),
            service_tier: None,
    ...
    assert_eq!(app.chat_widget.config_ref().model, original_model);
    assert_eq!(
        app.chat_widget.config_ref().model_provider_id,
...
async fn handle_mcp_inventory_result_clears_committed_loading_cell() {
    ...
    app.handle_mcp_inventory_result(
        Ok(vec![McpServerStatus {
            ...
            auth_status: agere_app_server_protocol::McpAuthStatus::Unsupported,
        }]),
...
async fn update_memory_settings_persists_and_updates_widget_config() -> Result<()> {
    ...
    let config = std::fs::read_to_string(agere_home.path().join("config.toml"))?;
...
async fn reset_memories_clears_local_memory_directories() -> Result<()> {
    ...
    let agere_home = tempdir()?;
    app.config.agere_home = agere_home.path().to_path_buf().abs();
    app.config.sqlite_home = agere_home.path().to_path_buf();

    let memory_root = agere_home.path().join("memories");
...
async fn update_feature_flags_enabling_guardian_selects_auto_review() -> Result<()> {
    ...
    assert!(config.contains("access_mode = \"workspace-write\""));
    Ok(())
}
...
-> Result<()> {
    ...
    let agere_home = tempdir()?;
    app.config.agere_home = agere_home.path().to_path_buf().abs();
    let config_toml_path = agere_home.path().join("config.toml").abs();
    ...
    assert_eq!(
        ...
        vec![
            ...
            agere_protocol::protocol::ReviewDecision::NetworkPolicyAmendment {
                network_policy_amendment: agere_protocol::approvals::NetworkPolicyAmendment {
                    host: "example.com".to_string(),
                    action: agere_protocol::approvals::NetworkPolicyRuleAction::Allow,
                },
            },
...
async fn inactive_thread_file_change_approval_recovers_buffered_changes() {
    ...
    app.enqueue_thread_notification(
        ...
        ServerNotification::ItemStarted(ItemStartedNotification {
            ...
            item: ThreadItem::FileChange {
                ...
                status: agere_app_server_protocol::PatchApplyStatus::InProgress,
            },
        }),
    )
    .await
    ...
    let request = ServerRequest::FileChangeRequestApproval {
        ...
        params: FileChangeRequestApprovalParams {
            ...
            reason: Some("command failed; retry with escalated access?".to_string()),
            grant_root: None,
        },
    };
    ...
    assert_eq!(
        reason,
        &Some("command failed; retry with escalated access?".to_string())
    );
...
async fn inactive_thread_permissions_approval_preserves_file_system_permissions() {
    ...
    let request = ServerRequest::PermissionsRequestApproval {
        ...
        params: PermissionsRequestApprovalParams {
            ...
            permissions: agere_app_server_protocol::RequestPermissionProfile {
                network: Some(AdditionalNetworkPermissions {
                    enabled: Some(true),
                }),
                file_system: Some(AdditionalFileSystemPermissions {
...
async fn inactive_thread_started_notification_initializes_replay_session() -> Result<()> {
    ...
    let turn_context = TurnContextItem {
        ...
        network: None,
        file_system_access_policy: None,
    ...
    app.enqueue_thread_notification(
        ...
        ServerNotification::ThreadStarted(ThreadStartedNotification {
            thread: Thread {
                ...
                created_at: 1,
                updated_at: 2,
                status: agere_app_server_protocol::ThreadStatus::Idle,
                path: Some(rollout_path.clone()),
                cwd: test_path_buf("/tmp/agent").abs(),
                cli_version: "0.0.1".to_string(),
                source: agere_app_server_protocol::SessionSource::Unknown,
                agent_nickname: Some("Robie".to_string()),
                agent_role: Some("explorer".to_string()),
                git_info: None,
...
async fn side_fork_config_is_ephemeral_and_appends_developer_guardrails() {
    ...
    let original_access_policy = app.config.legacy_access_policy();
    ...
    assert_eq!(fork_config.legacy_access_policy(), original_access_policy);
...
async fn side_parent_status_prioritizes_input_over_approval() -> Result<()> {
    ...
    app.enqueue_thread_notification(
        ...
        ServerNotification::ServerRequestResolved(
            agere_app_server_protocol::ServerRequestResolvedNotification {
...
async fn side_thread_ignores_global_mcp_startup_notifications() {
    ...
    app.handle_app_server_event(
        &app_server,
        agere_app_server_client::AppServerEvent::ServerNotification(
...
async fn active_non_primary_shutdown_target_returns_none_for_non_shutdown_event() -> Result<()> {
    ...
    assert_eq!(
        app.active_non_primary_shutdown_target(&ServerNotification::SkillsChanged(
            agere_app_server_protocol::SkillsChangedNotification {},
        )),
        None
    );
    Ok(())
...
async fn clear_ui_header_shows_fast_status_for_fast_capable_models() {
    ...
    app.chat_widget
        .set_service_tier(Some(agere_protocol::config_types::ServiceTier::Fast));
...
fn active_turn_not_steerable_turn_error_extracts_structured_server_error() {
    let turn_error = AppServerTurnError {
        ...
        agere_error_info: Some(AppServerAgereErrorInfo::ActiveTurnNotSteerable {
...
async fn fresh_session_config_uses_current_service_tier() {
    ...
    assert_eq!(
        config.service_tier,
        Some(agere_protocol::config_types::ServiceTier::Fast)
    );
}
        ...
        ThreadEventSnapshot {
            ...
            events: vec![ThreadBufferedEvent::Notification(
                ServerNotification::ItemStarted(
                    agere_app_server_protocol::ItemStartedNotification {
                        thread_id: "thread-1".to_string(),
                        turn_id: "turn-1".to_string(),
                        item: ThreadItem::CollabAgentToolCall {
                            id: "wait-1".to_string(),
                            tool: agere_app_server_protocol::CollabAgentTool::Wait,
                            status:
                                agere_app_server_protocol::CollabAgentToolCallStatus::InProgress,
...
async fn session_summary_includes_resume_hint_for_persisted_rollout() {
    ...
    assert_eq!(
        ...
        Some("openagere resume 123e4567-e89b-12d3-a456-426614174000".to_string())
    );
}
