...
const ANNOUNCEMENT_TIP_URL: &str = "https://api.openagere.com/announcement";

const FAST_TOOLTIP: &str =
    "*New* Use **/fast** to enable our fastest inference with increased plan usage.";
const OTHER_TOOLTIP_NON_MAC: &str = "Use **/provider** to manage providers.";
const FREE_GO_TOOLTIP: &str =
    "*New* For a limited time, Agere is included in your plan for free – let’s build together.";
...
/// Pick a random tooltip to show to the user when starting Agere.
...
/// generic random tip pool. Currently only the Fast promo is enabled, and it is
/// suppressed once the user already has Fast mode enabled.
fn pick_paid_tooltip<R: Rng + ?Sized>(
    _rng: &mut R,
    fast_mode_enabled: bool,
) -> Option<&'static str> {
    (!fast_mode_enabled).then_some(FAST_TOOLTIP)
}
...
pub(crate) mod announcement {
    ...
    impl TargetOs {
        const fn current() -> Self {
            if cfg!(target_os = "macos") {
                ...
                // Agere currently publishes CLI builds for macOS, Windows, and Linux.
                Self::Linux
            }
        }
    }
    ...
    pub(crate) fn announcement_tip_url() -> String {
        let region = crate::region::detect_region().query_value();
        format!("{ANNOUNCEMENT_TIP_URL}?region={region}")
    }
    ...
    pub(crate) fn parse_announcement_tip_toml(
        ...
        for raw in announcements {
            ...
            let plan_matches = match tip.target_plan_types.as_ref() {
                None => true,
                Some(target_plans) => plan.is_some_and(|plan| target_plans.contains(&plan)),
            };
            let os_matches = tip
                .target_oses
                .as_ref()
                .is_none_or(|target_oses| target_oses.contains(&CURRENT_OS));
            if tip.version_matches(AGERE_CLI_VERSION)
...
mod tests {
    use super::*;
    use crate::tooltips::announcement::announcement_tip_url;
    ...
    #[test]
    fn startup_tooltips_do_not_reference_openai_or_chatgpt() {
        let mut tips = Vec::new();
        tips.push(FAST_TOOLTIP);
        tips.push(OTHER_TOOLTIP_NON_MAC);
        tips.push(FREE_GO_TOOLTIP);
        tips.extend(
            RAW_TOOLTIPS
                .lines()
                .map(str::trim)
        ...
        for tip in tips {
            let lower_tip = tip.to_lowercase();
            assert!(
                !lower_tip.contains("chatgpt.com"),
                "unexpected chatgpt.com in tip: {tip}"
            );
            assert!(
                !lower_tip.contains("openai"),
                "unexpected openai in tip: {tip}"
            );
        }
    }

    #[test]
    fn startup_tooltips_do_not_advertise_app() {
        let mut tips = vec![FAST_TOOLTIP, OTHER_TOOLTIP_NON_MAC, FREE_GO_TOOLTIP];
        tips.extend(
            RAW_TOOLTIPS
                .lines()
                .map(str::trim)
        ...
        for tip in tips {
            let lower_tip = tip.to_lowercase();
            assert!(
                !lower_tip.contains("agere app"),
                "unexpected app tip: {tip}"
            );
        }
    }
    ...
    fn announcement_tip_url_uses_detected_region() {
        unsafe { std::env::set_var("AGERE_REGION_TEST", "cn") };
        assert_eq!(
            announcement_tip_url(),
            "https://api.openagere.com/announcement?region=1"
        );
        unsafe { std::env::set_var("AGERE_REGION_TEST", "en_US") };
        assert_eq!(
            announcement_tip_url(),
            "https://api.openagere.com/announcement?region=2"
        );
...
from_date = "2000-01-01"
    ...
    fn announcement_tip_toml_parse_comments() {
        let toml = r#"
# Example announcement tips for Agere TUI.
...
[[announcements]]
content = "Welcome to Agere! Check out the new onboarding flow."
...
target_plan_types = ["free"]
    ...
    #[test]
    fn announcement_tip_toml_matches_missing_target_plan_without_plan() {
        let toml = r#"
[[announcements]]
content = "Welcome to OpenAgere"
target_app = "cli"
        "#;

        assert_eq!(
            Some("Welcome to OpenAgere".to_string()),
            parse_announcement_tip_toml(toml, /*plan*/ None)
        );
    }

    #[test]
    fn remote_announcement_tip_accepts_toml_response() {
        let toml = r#"
[[announcements]]
content = "Welcome to Use OpenAgere, Enjoy it ! "
from_date = "2026-06-07"
target_app = "cli"
target_oses = ["macos", "windows", "linux"]
        "#;

        assert_eq!(
            Some("Welcome to Use OpenAgere, Enjoy it !".to_string()),
            parse_announcement_tip_toml(toml, /*plan*/ None)
        );
    }

    #[test]
    fn announcement_tip_toml_requires_plan_for_targeted_plan() {
        let toml = r#"
[[announcements]]
content = "pro announcement"
