//! Aggregates MCP server connections for Agere.
//!
...
use crate::agere_apps::AgereAppsToolsCacheKey;
...
use agere_protocol::protocol::McpStartupFailure;
...
impl McpConnectionManager {
    ...
    pub async fn new(
        ...
        agere_home: PathBuf,
        agere_apps_tools_cache_key: AgereAppsToolsCacheKey,
        ...
        let agere_apps_auth_provider = auth
            .filter(|auth| auth.uses_agere_backend())
            .map(agere_model_provider::auth_provider_from_auth);
        ...
        for (server_name, cfg) in mcp_servers.into_iter().filter(|(_, cfg)| cfg.enabled) {
            ...
            .await;
            let agere_apps_tools_cache_context = if server_name == AGERE_APPS_MCP_SERVER_NAME {
                Some(AgereAppsToolsCacheContext {
                    agere_home: agere_home.clone(),
                    user_key: agere_apps_tools_cache_key.clone(),
                })
            } else {
                None
            };
            ...
            let runtime_auth_provider =
                if server_name == AGERE_APPS_MCP_SERVER_NAME && !uses_env_bearer_token {
                    agere_apps_auth_provider.clone()
                } else {
                    None
                };
    ...
    /// Force-refresh agere apps tools by bypassing the in-process cache.
    ///
    ...
    pub async fn hard_refresh_agere_apps_tools_cache(&self) -> Result<HashMap<String, ToolInfo>> {
        let managed_client = self
            .clients
            .get(AGERE_APPS_MCP_SERVER_NAME)
            .ok_or_else(|| anyhow!("unknown MCP server '{AGERE_APPS_MCP_SERVER_NAME}'"))?
            .client()
            .await
        ...
        .await
        .with_context(|| {
            format!("failed to refresh tools for MCP server '{AGERE_APPS_MCP_SERVER_NAME}'")
        })?;
        emit_duration(
        ...
        write_cached_agere_apps_tools_if_needed(
            AGERE_APPS_MCP_SERVER_NAME,
            managed_client.agere_apps_tools_cache_context.as_ref(),
            &tools,
        );
        emit_duration(
    ...
    pub async fn server_supports_execution_state_meta_capability(
        &self,
        server: &str,
    ) -> Result<bool> {
        Ok(self
