use super::effective_file_system_access_policy;
...
fn intersect_permission_profiles_materializes_relative_deny_globs_for_reuse() {
    ...
    let later_cwd = AbsolutePathBuf::from_absolute_path(temp_dir.path().join("later-cwd"))
        .expect("absolute later cwd");
    let cwd_write = FileSystemAccessEntry {
        path: FileSystemPath::Special {
            value: FileSystemSpecialPath::project_roots(/*subpath*/ None),
        },
        access: FileSystemAccessMode::Write,
    };
    let deny_env_files = FileSystemAccessEntry {
        path: FileSystemPath::GlobPattern {
            pattern: "**/*.env".to_string(),
        },
        access: FileSystemAccessMode::None,
...
fn intersect_permission_profiles_uses_granted_bounded_glob_scan_depth() {
    ...
    let root_write = FileSystemAccessEntry {
        path: FileSystemPath::Special {
            value: FileSystemSpecialPath::Root,
        },
        access: FileSystemAccessMode::Write,
...
fn merge_file_system_policy_with_additional_permissions_preserves_unreadable_roots() {
    ...
    assert_eq!(
        merged_policy.entries.contains(&FileSystemAccessEntry {
...
fn merge_file_system_policy_with_additional_permissions_carries_bounded_glob_scan_depth() {
    ...
    assert_eq!(merged_policy, {
        let mut policy = FileSystemAccessPolicy::restricted(vec![
            FileSystemAccessEntry {
                path: FileSystemPath::Special {
                    value: FileSystemSpecialPath::Root,
                },
...
#[test]
fn effective_file_system_access_policy_returns_base_policy_without_additional_permissions() {
    let temp_dir = TempDir::new().expect("create temp dir");
    let cwd = AbsolutePathBuf::from_absolute_path(
        canonicalize(temp_dir.path()).expect("canonicalize temp dir"),
    )
    .expect("absolute temp dir");
    let denied_path = cwd.join("denied");
    let base_policy = FileSystemAccessPolicy::restricted(vec![
        FileSystemAccessEntry {
            path: FileSystemPath::Special {
                value: FileSystemSpecialPath::Root,
            },
    ...
    let effective_policy =
        effective_file_system_access_policy(&base_policy, /*additional_permissions*/ None);

    assert_eq!(effective_policy, base_policy);
}

#[test]
fn effective_file_system_access_policy_merges_additional_write_roots() {
    let temp_dir = TempDir::new().expect("create temp dir");
    let cwd = AbsolutePathBuf::from_absolute_path(
        canonicalize(temp_dir.path()).expect("canonicalize temp dir"),
    )
    ...
    let effective_policy =
        effective_file_system_access_policy(&base_policy, Some(&additional_permissions));

    assert_eq!(
        effective_policy.entries.contains(&FileSystemAccessEntry {
            path: FileSystemPath::Path { path: denied_path },
            access: FileSystemAccessMode::None,
        }),
        true
