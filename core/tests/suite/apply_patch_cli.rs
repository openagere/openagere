...
use core_test_support::test_agere::ApplyPatchModelOutput;
...
#[cfg(target_os = "linux")]
use agere_exec_server::AGERE_LINUX_HELPER_ARG0;
...
pub async fn apply_patch_harness() -> Result<TestAgereHarness> {
    apply_patch_harness_with(|builder| builder).await
}

async fn apply_patch_harness_with(
    configure: impl FnOnce(TestAgereBuilder) -> TestAgereBuilder,
) -> Result<TestAgereHarness> {
    let builder = configure(test_agere()).with_config(|config| {
    ...
    Box::pin(TestAgereHarness::with_remote_aware_builder(builder)).await
}

async fn submit_without_wait(harness: &TestAgereHarness, prompt: &str) -> Result<()> {
...
async fn apply_patch_cli_uses_agere_self_exe_with_linux_execution_helper_alias() -> Result<()> {
    skip_if_no_network!(Ok(()));

    let harness = apply_patch_harness().await?;
    let agere_linux_exe = harness
        .test()
        .config
        .agere_linux_exe
        .as_ref()
        .expect("linux test config should include agere-linux-helper binary");
    assert_eq!(
        agere_linux_exe.file_name().and_then(|name| name.to_str()),
        Some(AGERE_LINUX_HELPER_ARG0),
    );
...
async fn apply_patch_custom_tool_streaming_emits_updated_changes() -> Result<()> {
    ...
    assert_eq!(
        ...
        Some(&agere_protocol::protocol::FileChange::Add {
            content: "hello\n".to_string(),
        })
    );
    assert_eq!(
