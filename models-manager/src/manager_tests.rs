...
async fn chatgpt_auth_tokens_for_tests(agere_home: &Path) -> AgereAuth {
    let auth_dot_json = agere_login::AuthDotJson {
        openai_api_key: Some("test-chatgpt-auth-tokens-api-key".to_string()),
    };
    std::fs::create_dir_all(agere_home).expect("agere home should be created");
    std::fs::write(
    ...
    AgereAuth::from_auth_storage(
        agere_home,
        agere_config::types::AuthCredentialsStoreMode::File,
...
async fn refresh_available_models_sorts_by_priority() {
    ...
    let manager = openai_manager_for_tests(agere_home.path().to_path_buf(), endpoint.clone());
...
async fn refresh_available_models_drops_removed_remote_models() {
    ...
    let mut manager = openai_manager_for_tests(agere_home.path().to_path_buf(), endpoint.clone());
...
#[tokio::test]
async fn refresh_available_models_with_unresolved_external_api_key() {
    // With simplified auth, external API key auth that cannot resolve means
    // the manager should skip network refresh (since uses_agere_backend is false).
    ...
    // With unresolved external auth and no agere backend, refresh should skip.
    manager
        .refresh_available_models(RefreshStrategy::Online)
        .await
        .expect("refresh should complete without error");

    // No models should have been fetched since refresh was skipped.
    assert_eq!(
        endpoint.fetch_count(),
        0,
        "endpoint should not fetch when external auth is unresolved and no backend is available"
    );
}
...
async fn refresh_available_models_fetches_with_chatgpt_auth_tokens() {
    ...
    let auth = chatgpt_auth_tokens_for_tests(agere_home.path()).await;
    let manager = openai_manager_for_tests_with_auth(
        agere_home.path().to_path_buf(),
        endpoint.clone(),
...
#[tokio::test]
async fn static_manager_filters_unsupported_api_models() {
    let auth_manager =
        AuthManager::from_auth_for_testing(AgereAuth::create_dummy_chatgpt_auth_for_testing());
...
#[test]
fn bundled_models_use_agere_identity_in_base_instructions() {
    let response = crate::bundled_models_response()
        .unwrap_or_else(|err| panic!("bundled models.json should parse: {err}"));

    for model in response.models {
        assert!(
            !model.base_instructions.contains("You are GPT"),
            "{} should not identify the agent as GPT",
            model.slug
        );
        assert!(
            !model
                .base_instructions
                .contains("open source project led by OpenAI"),
            "{} should not describe Agere as OpenAI-led",
            model.slug
        );
        assert!(
            !model.base_instructions.contains("old Agere language model"),
            "{} should not mention legacy OpenAI Agere branding",
            model.slug
        );
    }
}
