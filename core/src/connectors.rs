...
pub use agere_app_server_protocol::AppBranding;
pub use agere_app_server_protocol::AppInfo;
pub use agere_app_server_protocol::AppMetadata;
use agere_connectors::AllConnectorsCacheKey;
use agere_connectors::DirectoryListResponse;
...
use agere_config::AppsRequirementsToml;
...
use agere_login::default_client::create_client;
...
use agere_mcp::ToolPluginProvenance;
...
pub(crate) async fn list_tool_suggest_discoverable_tools_with_auth(
    ...
    let discoverable_connectors =
        agere_connectors::filter::filter_tool_suggest_discoverable_connectors(
...
pub async fn list_cached_accessible_connectors_from_mcp_tools(
    ...
    read_cached_accessible_connectors(&cache_key).map(|connectors| {
        agere_connectors::filter::filter_disallowed_connectors(
            connectors,
            originator().value.as_str(),
        )
    })
...
pub(crate) fn refresh_accessible_connectors_cache_from_mcp_tools(
    ...
    let accessible_connectors = agere_connectors::filter::filter_disallowed_connectors(
...
pub async fn list_accessible_connectors_from_mcp_tools_with_environment_manager(
    ...
    {
        let cached_connectors = agere_connectors::filter::filter_disallowed_connectors(
            cached_connectors,
            originator().value.as_str(),
        );
    ...
    let mcp_servers = with_agere_apps_mcp(HashMap::new(), auth.as_ref(), &mcp_config);
    ...
    let (mut mcp_connection_manager, cancel_token) = McpConnectionManager::new(
        ...
        agere_apps_tools_cache_key(auth.as_ref()),
    ...
    let refreshed_tools = if force_refetch {
        match mcp_connection_manager
            .hard_refresh_agere_apps_tools_cache()
            .await
        {
            Ok(tools) => Some(tools),
            Err(err) => {
                warn!(
                    "failed to force-refresh tools for MCP server '{AGERE_APPS_MCP_SERVER_NAME}', using cached/startup tools: {err:#}"
                );
                None
            }
        }
    ...
    let agere_apps_ready = if refreshed_tools_succeeded {
        true
    } else if let Some(cfg) = mcp_servers.get(AGERE_APPS_MCP_SERVER_NAME) {
        let immediate_ready = mcp_connection_manager
            .wait_for_server_ready(AGERE_APPS_MCP_SERVER_NAME, Duration::ZERO)
            .await;
        if immediate_ready {
            true
            ...
            let ready = mcp_connection_manager
                .wait_for_server_ready(AGERE_APPS_MCP_SERVER_NAME, timeout)
                .await;
    ...
    if agere_apps_ready || !accessible_connectors.is_empty() {
        write_cached_accessible_connectors(cache_key, &accessible_connectors);
    }
...
fn accessible_connectors_cache_key(
    ...
    let chatgpt_user_id = auth.and_then(AgereAuth::get_chatgpt_user_id);
    let is_workspace_account = auth.is_some_and(AgereAuth::is_workspace_account);
    AccessibleConnectorsCacheKey {
...
fn write_cached_accessible_connectors(
    ...
    *cache_guard = Some(CachedAccessibleConnectors {
        key: cache_key,
        expires_at: Instant::now() + agere_connectors::CONNECTORS_CACHE_TTL,
        connectors: connectors.to_vec(),
    });
}

async fn tool_suggest_connector_ids(config: &Config) -> HashSet<String> {
    let mut connector_ids = PluginsManager::new(config.agere_home.to_path_buf())
...
async fn list_directory_connectors_for_tool_suggest_with_auth(
    ...
    let Some(auth) = auth.filter(|auth| auth.uses_agere_backend()) else {
        return Ok(Vec::new());
    };
    ...
    let auth_provider = agere_model_provider::auth_provider_from_auth(auth);
    ...
    agere_connectors::list_all_connectors_with_options(
        cache_key,
        is_workspace_account,
        /*force_refetch*/ false,
        |path| {
...
pub(crate) fn accessible_connectors_from_mcp_tools(
    ...
    let tools = mcp_tools.values().filter_map(|tool| {
        if tool.server_name != AGERE_APPS_MCP_SERVER_NAME {
            return None;
        }
        let connector_id = tool.connector_id.as_deref()?;
        Some(agere_connectors::accessible::AccessibleConnectorTool {
    ...
    agere_connectors::accessible::collect_accessible_connectors(tools)
}
...
pub(crate) fn agere_app_tool_is_enabled(config: &Config, tool_info: &ToolInfo) -> bool {
    if tool_info.server_name != AGERE_APPS_MCP_SERVER_NAME {
        return true;
    }
