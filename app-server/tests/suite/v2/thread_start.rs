...
use agere_app_server_protocol::ThreadStatusChangedNotification;
...
async fn thread_start_creates_thread_and_emits_started() -> Result<()> {
    ...
    create_config_toml_without_approval_policy(agere_home.path(), &server.uri())?;
...
async fn thread_start_response_includes_loaded_instruction_sources() -> Result<()> {
    ...
    let agere_home = TempDir::new()?;
    create_config_toml_without_approval_policy(agere_home.path(), &server.uri())?;
    let global_agents_path = agere_home.path().join("AGENTS.md");
...
async fn thread_start_emits_mcp_server_status_updated_notifications() -> Result<()> {
    ...
    let agere_home = TempDir::new()?;
    create_config_toml_with_optional_broken_mcp(agere_home.path(), &server.uri())?;
...
#[tokio::test]
#[ignore]
async fn thread_start_with_elevated_access_profile_trusts_project_and_followup_loads_project_config()
-> Result<()> {
    let server = create_mock_responses_server_repeating_assistant("Done").await;
...
#[tokio::test]
#[ignore]
async fn thread_start_with_read_only_access_profile_does_not_persist_project_trust() -> Result<()> {
    let server = create_mock_responses_server_repeating_assistant("Done").await;
...
"#,
    ...
    set_project_trust_level(agere_home.path(), workspace.path(), TrustLevel::Trusted)?;
    let config_before = std::fs::read_to_string(agere_home.path().join("config.toml"))?;
    ...
    let config_after = std::fs::read_to_string(agere_home.path().join("config.toml"))?;
    assert_eq!(config_after, config_before);
...
fn create_config_toml_without_approval_policy(
    ...
    create_config_toml_with_optional_approval_policy(
        agere_home, server_uri, /*approval_policy*/ None,
    )
}
...
fn create_config_toml_with_optional_approval_policy(
    ...
    std::fs::write(
        ...
        format!(
            r#"
model = "mock-model"
{approval_policy}access_mode = "read-only"
