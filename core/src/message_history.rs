...
//!
//! The history is stored at `~/.openagere/history.jsonl` with **one JSON object per
...
fn history_filepath(config: &Config) -> AbsolutePathBuf {
    config.agere_home.join(HISTORY_FILENAME)
}
...
pub async fn append_entry(text: &str, conversation_id: &ThreadId, config: &Config) -> Result<()> {
    ...
    // Resolve `~/.openagere/history.jsonl` and ensure the parent directory exists.
