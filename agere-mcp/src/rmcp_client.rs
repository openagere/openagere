...
use crate::agere_apps::CachedAgereAppsToolsLoad;
use crate::agere_apps::filter_disallowed_agere_apps_tools;
use crate::agere_apps::load_cached_agere_apps_tools;
use crate::agere_apps::load_startup_cached_agere_apps_tools_snapshot;
use crate::agere_apps::normalize_agere_apps_callable_name;
use crate::agere_apps::normalize_agere_apps_callable_namespace;
use crate::agere_apps::normalize_agere_apps_tool_title;
use crate::agere_apps::write_cached_agere_apps_tools_if_needed;
...
use agere_async_utils::CancelErr;
use agere_async_utils::OrCancelExt;
use agere_config::McpServerConfig;
use agere_config::McpServerTransportConfig;
use agere_config::types::OAuthCredentialsStoreMode;
use agere_exec_server::HttpClient;
use agere_exec_server::ReqwestHttpClient;
use agere_protocol::protocol::Event;
use agere_rmcp_client::ExecutorStdioServerLauncher;
use agere_rmcp_client::LocalStdioServerLauncher;
use agere_rmcp_client::RmcpClient;
use agere_rmcp_client::StdioServerLauncher;
use anyhow::Result;
use anyhow::anyhow;
...
/// MCP server capability indicating that Agere should include [`ExecutionState`]
/// in tool-call request `_meta` under this key.
pub const MCP_EXECUTION_STATE_META_CAPABILITY: &str = "agere/execution-state-meta";

pub(crate) const MCP_TOOLS_LIST_DURATION_METRIC: &str = "agere.mcp.tools.list.duration_ms";
pub(crate) const MCP_TOOLS_FETCH_UNCACHED_DURATION_METRIC: &str =
    "agere.mcp.tools.fetch_uncached.duration_ms";
...
pub(crate) struct ManagedClient {
    ...
    pub(crate) server_supports_execution_state_meta_capability: bool,
    pub(crate) agere_apps_tools_cache_context: Option<AgereAppsToolsCacheContext>,
}

impl ManagedClient {
    fn listed_tools(&self) -> Vec<ToolInfo> {
        let total_start = Instant::now();
        if let Some(cache_context) = self.agere_apps_tools_cache_context.as_ref()
            && let CachedAgereAppsToolsLoad::Hit(tools) =
                load_cached_agere_apps_tools(cache_context)
        {
            emit_duration(
        ...
        if self.agere_apps_tools_cache_context.is_some() {
            emit_duration(
                MCP_TOOLS_LIST_DURATION_METRIC,
                total_start.elapsed(),
                &[("cache", "miss")],
...
impl AsyncManagedClient {
    ...
    pub(crate) fn new(
        ...
        agere_apps_tools_cache_context: Option<AgereAppsToolsCacheContext>,
        ...
        let startup_snapshot = load_startup_cached_agere_apps_tools_snapshot(
            &server_name,
            agere_apps_tools_cache_context.as_ref(),
        )
            ...
            let outcome = match async {
                ...
                start_server_task(
                    ...
                    StartServerTaskParams {
                        ...
                        agere_apps_tools_cache_context,
                    },
                )
                .await
            }
...
pub(crate) async fn list_tools_for_client_uncached(
    ...
    let tools = resp
        ...
        .map(|tool| {
            ...
            let callable_name = normalize_agere_apps_callable_name(
                server_name,
                &tool_def.name,
            ...
            let callable_namespace =
                normalize_agere_apps_callable_namespace(server_name, connector_name.as_deref());
            if let Some(title) = tool_def.title.as_deref() {
                let normalized_title =
                    normalize_agere_apps_tool_title(server_name, connector_name.as_deref(), title);
    ...
    if server_name == AGERE_APPS_MCP_SERVER_NAME {
        return Ok(filter_disallowed_agere_apps_tools(tools));
    }
    Ok(tools)
}
...
async fn start_server_task(
    ...
    let params = InitializeRequestParams {
        ...
        client_info: Implementation {
            name: "agere-mcp-client".to_owned(),
            version: env!("CARGO_PKG_VERSION").to_owned(),
            title: Some("Agere".into()),
            description: None,
            icons: None,
            website_url: None,
        },
    ...
    let server_supports_execution_state_meta_capability = initialize_result
        .capabilities
        .experimental
        .as_ref()
        .and_then(|exp| exp.get(MCP_EXECUTION_STATE_META_CAPABILITY))
        .is_some();
    ...
    write_cached_agere_apps_tools_if_needed(
        &server_name,
        agere_apps_tools_cache_context.as_ref(),
        &tools,
    );
    ...
    let managed = ManagedClient {
        ...
        server_supports_execution_state_meta_capability,
        agere_apps_tools_cache_context,
    };
...
mod tests {
    ...
    #[test]
    fn agere_apps_connector_metadata_is_preserved() {
        let mut tool = tool_with_connector_meta();
