...
use agere_protocol::protocol::FileSystemAccessPolicy;
...
async fn install_role_with_model_override(turn: &mut TurnContext) -> String {
    ...
    tokio::fs::create_dir_all(&turn.config.agere_home)
        .await
        .expect("agere home should be created");
        ...
        | ResponseInputItem::CustomToolCallOutput { output, .. } => {
            let content = match output.body {
                ...
                FunctionCallOutputBody::ContentItems(items) => {
                    agere_protocol::models::function_call_output_content_items_to_text(&items)
                        .unwrap_or_default()
                }
            };
...
async fn spawn_agent_uses_explorer_role_and_preserves_approval_policy() {
    ...
    turn.approval_policy
        .set(AskForApproval::OnRequest)
        .expect("approval policy should be set");
    turn.provider = create_model_provider(provider_info, turn.auth_manager.clone(), vec![]);
    turn.config = Arc::new(config);
...
async fn multi_agent_v2_list_agents_returns_completed_status_and_last_task_message() {
    ...
    let child_thread = manager
        ...
        .await
        .expect("child thread should exist");
    let child_turn = child_thread.agere.session.new_default_turn().await;
    child_thread
        .agere
        .session
        .send_event(
...
async fn multi_agent_v2_followup_task_interrupts_busy_child_without_losing_message() {
    ...
    let active_turn = thread.agere.session.new_default_turn().await;
...
async fn multi_agent_v2_followup_task_completion_notifies_parent_on_every_turn() {
    ...
    let first_turn = thread.agere.session.new_default_turn().await;
    thread
        .agere
        .session
        .send_event(
    ...
    let second_turn = thread.agere.session.new_default_turn().await;
    thread
        .agere
        .session
        .send_event(
...
async fn multi_agent_v2_interrupted_turn_does_not_notify_parent() {
    ...
    let aborted_turn = thread.agere.session.new_default_turn().await;
    thread
        .agere
        .session
        .send_event(
...
#[tokio::test]
async fn spawn_agent_reapplies_runtime_access_policy_after_role_config() {
    #[derive(Debug, Deserialize)]
    struct SpawnAgentResult {
        agent_id: String,
        nickname: Option<String>,
    ...
    let manager = thread_manager();
    session.services.agent_control = manager.agent_control();
    let expected_legacy_access_label = turn.config.legacy_access_policy();
    let mut expected_file_system_access_policy =
        FileSystemAccessPolicy::from_legacy_access_policy_for_cwd(
            &expected_legacy_access_label,
            &turn.cwd,
        );
    expected_file_system_access_policy
        .entries
        .push(FileSystemAccessEntry {
            path: FileSystemPath::GlobPattern {
                pattern: "**/.env".to_string(),
            },
            access: FileSystemAccessMode::None,
        });
    let expected_network_access_policy =
        PermissionProfile::from_legacy_access_policy(expected_legacy_access_label.as_str())
            .network_access_policy();
    let expected_permission_profile =
        PermissionProfile::from_runtime_permissions_with_filesystem_isolation(
            FilesystemIsolation::from_legacy_access_policy(&expected_legacy_access_label),
            &expected_file_system_access_policy,
            expected_network_access_policy,
        );
    turn.approval_policy
        .set(AskForApproval::OnRequest)
        .expect("approval policy should be set");
    ...
    let snapshot = manager
        ...
        .config_snapshot()
        .await;
    assert_eq!(snapshot.access_policy(), expected_legacy_access_label);
    assert_eq!(snapshot.approval_policy, AskForApproval::OnRequest);
    ...
    let child_turn = child_thread.agere.session.new_default_turn().await;
    assert_eq!(
        child_turn.file_system_access_policy(),
        expected_file_system_access_policy
    );
    assert_eq!(
        child_turn.network_access_policy(),
        expected_network_access_policy
    );
...
async fn resume_agent_restores_closed_agent_and_accepts_send_input() {
    ...
    let thread = manager
        .resume_thread_with_history(
            ...
            AuthManager::from_auth_for_testing(AgereAuth::from_api_key("dummy")),
...
async fn tool_handlers_cascade_close_and_resume_and_keep_explicitly_closed_subtrees_closed() {
    ...
    let parent_session = parent.thread.agere.session.clone();
    ...
    let child_thread = manager
        ...
        .await
        .expect("child thread should exist");
    let child_session = child_thread.agere.session.clone();
    ...
    let operator_session = operator.thread.agere.session.clone();
    let _ = manager
        .agent_control()
    ...
    let parent_resume_output = ResumeAgentHandler
        .handle(invocation(
            operator_session,
            operator.thread.agere.session.new_default_turn().await,
            "resume_agent",
...
async fn build_agent_spawn_config_uses_turn_context_values() {
    fn pick_allowed_access_policy(
        ...
        candidates
            ...
            .find(|candidate| {
                ...
                let base_profile = PermissionProfile::from_legacy_access_policy(candidate);
                let (file_system_access_policy, network_access_policy) =
                    base_profile.to_runtime_permissions();
                let permission_profile =
                    PermissionProfile::from_runtime_permissions_with_filesystem_isolation(
                        FilesystemIsolation::from_legacy_access_policy(candidate),
    ...
    turn.agere_linux_exe = Some(PathBuf::from("/bin/echo"));
    let access_policy = pick_allowed_access_policy(
        &turn.config.permissions.permission_profile,
        turn.config.legacy_access_policy(),
        turn.cwd.as_path(),
    );
    let base_profile = PermissionProfile::from_legacy_access_policy(access_policy.as_str());
    let (file_system_access_policy, network_access_policy) = base_profile.to_runtime_permissions();
    ...
    expected.agere_linux_exe = turn.agere_linux_exe.clone();
