use crate::OPENAGERE_CURATED_MARKETPLACE_NAME;
...
use agere_config::HooksFile;
use agere_config::types::McpServerConfig;
use agere_config::types::PluginConfig;
use agere_core_skills::SkillMetadata;
use agere_core_skills::config_rules::SkillConfigRules;
use agere_core_skills::config_rules::resolve_disabled_skill_paths;
use agere_core_skills::config_rules::skill_config_rules_from_stack;
use agere_core_skills::loader::SkillRoot;
use agere_core_skills::loader::load_skills_from_roots;
use agere_exec_server::LOCAL_FS;
use agere_plugin::AppConnectorId;
use agere_plugin::LoadedPlugin;
use agere_plugin::PluginCapabilitySummary;
use agere_plugin::PluginHookSource;
use agere_plugin::PluginId;
use agere_plugin::PluginIdError;
use agere_plugin::PluginLoadOutcome;
...
use agere_utils_plugins_placeholder::find_plugin_manifest_path;
...
pub fn refresh_curated_plugin_cache(
    ...
    let store = PluginStore::try_new(agere_home.to_path_buf()).map_err(|err| err.to_string())?;
    ...
    for plugin in curated_marketplace.plugins {
        ...
        if plugin_sources.contains_key(&plugin_name) {
            warn!(
                plugin = plugin_name,
                marketplace = OPENAGERE_CURATED_MARKETPLACE_NAME,
...
fn refresh_non_curated_plugin_cache_with_mode(
    ...
    let configured_non_curated_plugin_ids =
        non_curated_plugin_ids_from_config_keys(configured_plugins_from_agere_home(
            agere_home,
    ...
    for marketplace in marketplace_outcome.marketplaces {
        if marketplace.name == OPENAGERE_CURATED_MARKETPLACE_NAME {
            continue;
        }
    ...
    for plugin_id in configured_non_curated_plugin_ids {
        ...
        let materialized =
            materialize_marketplace_plugin_source(agere_home, &source).map_err(|err| {
...
fn configured_plugins_from_agere_home(
    agere_home: &Path,
...
fn curated_plugin_ids_from_config_keys(
    ...
    .into_iter()
    .filter(|plugin_id| plugin_id.marketplace_name == OPENAGERE_CURATED_MARKETPLACE_NAME)
    .collect::<Vec<_>>();
...
pub fn configured_curated_plugin_ids_from_agere_home(agere_home: &Path) -> Vec<PluginId> {
    curated_plugin_ids_from_config_keys(configured_plugins_from_agere_home(
        agere_home,
    ...
    match &manifest_paths.hooks {
        ...
        Some(PluginManifestHooks::Inline(hooks_files)) => {
            let manifest_path = find_plugin_manifest_path(plugin_root.as_path())
                ...
                .unwrap_or_else(|| plugin_root.join(".agere-plugin/plugin.json"));
...
pub async fn installed_plugin_telemetry_metadata(
    ...
    let store = match PluginStore::try_new(agere_home.to_path_buf()) {
        Ok(store) => store,
        Err(err) => {
...
fn normalize_plugin_mcp_server_value(
        ...
        && oauth.contains_key("callbackPort")
    {
        warn!(
            plugin = %plugin_root.display(),
            "plugin MCP server OAuth callbackPort is ignored; Agere uses global MCP OAuth callback settings"
        );
    }
...
pub fn materialize_marketplace_plugin_source(
    ...
    match source {
        ...
        MarketplacePluginSource::Git {
            ...
            let staging_root = agere_home.join("plugins/.marketplace-plugin-source-staging");
