...
#[cfg(test)]
use agere_protocol::items::ContextCompactionItem;
#[cfg(test)]
use agere_protocol::items::ImageGenerationItem;
#[cfg(test)]
...
#[cfg(test)]
use agere_protocol::protocol::ExecCommandStatus;
#[cfg(test)]
...
#[cfg(test)]
use agere_protocol::protocol::RealtimeConversationStartedEvent;
#[cfg(test)]
use agere_protocol::protocol::RealtimeEvent;
#[cfg(test)]
use agere_protocol::protocol::ThreadNameUpdatedEvent;
#[cfg(test)]
        ...
        ServerNotification::Error(notification) => Some((
            ...
            vec![Event {
                ...
                msg: EventMsg::Error(ErrorEvent {
                    ...
                    agere_error_info: notification
                        .error
                        .agere_error_info
                        .and_then(app_server_agere_error_info_to_core),
                }),
            }],
        )),
                ...
                || {
                    Some(vec![Event {
                        ...
                        msg: EventMsg::ItemCompleted(ItemCompletedEvent {
                            ...
                            turn_id: notification.turn_id.clone(),
                            item: thread_item_to_core(&notification.item)?,
                            is_interim: notification.is_interim,
                        }),
                    }])
                },
            )?,
        ...
        ServerNotification::ThreadRealtimeSdp(notification) => Some((
            ...
            vec![Event {
                ...
                msg: EventMsg::RealtimeConversationSdp(
                    agere_protocol::protocol::RealtimeConversationSdpEvent {
...
fn token_usage_from_app_server(
    value: agere_app_server_protocol::TokenUsageBreakdown,
) -> TokenUsage {
    TokenUsage {
            ...
            memory_citation: memory_citation.clone().map(|citation| {
                agere_protocol::memory_citation::MemoryCitation {
                    entries: citation
                        ...
                        .into_iter()
                        .map(
                            |entry| agere_protocol::memory_citation::MemoryCitationEntry {
                                path: entry.path,
...
fn command_execution_completed_event(turn_id: &str, item: &ThreadItem) -> Option<Vec<Event>> {
    ...
    let status = match status {
        agere_app_server_protocol::CommandExecutionStatus::InProgress => return Some(Vec::new()),
        agere_app_server_protocol::CommandExecutionStatus::Completed => {
            ExecCommandStatus::Completed
        }
        agere_app_server_protocol::CommandExecutionStatus::Failed => ExecCommandStatus::Failed,
        agere_app_server_protocol::CommandExecutionStatus::Declined => ExecCommandStatus::Declined,
    };
...
) -> Option<agere_protocol::models::WebSearchAction> {
    match action {
        agere_app_server_protocol::WebSearchAction::Search { query, queries } => {
            Some(agere_protocol::models::WebSearchAction::Search { query, queries })
        }
        agere_app_server_protocol::WebSearchAction::OpenPage { url } => {
            Some(agere_protocol::models::WebSearchAction::OpenPage { url })
        }
        agere_app_server_protocol::WebSearchAction::FindInPage { url, pattern } => {
            Some(agere_protocol::models::WebSearchAction::FindInPage { url, pattern })
        }
        agere_app_server_protocol::WebSearchAction::Other => {
            Some(agere_protocol::models::WebSearchAction::Other)
        }
    }
}

#[cfg(test)]
fn app_server_agere_error_info_to_core(
    value: agere_app_server_protocol::AgereErrorInfo,
) -> Option<agere_protocol::protocol::AgereErrorInfo> {
...
mod tests {
    ...
    use agere_app_server_protocol::CommandAction;
    ...
    fn bridges_thread_snapshot_turns_for_resume_restore() {
        ...
        assert_eq!(
            error.agere_error_info,
            Some(agere_protocol::protocol::AgereErrorInfo::Other)
        );
            ...
            &Turn {
                ...
                items: vec![
                    ...
                    ThreadItem::WebSearch {
                        ...
                        action: Some(agere_app_server_protocol::WebSearchAction::Other),
                    },
                    ThreadItem::ImageGeneration {
