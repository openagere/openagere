...
use crate::agere_apps::agere_apps_tools_cache_key;
use crate::connection_manager::McpConnectionManager;
use crate::runtime::McpRuntimeEnvironment;

pub const AGERE_APPS_MCP_SERVER_NAME: &str = "agere_apps";
const MCP_TOOL_NAME_PREFIX: &str = "mcp";
const MCP_TOOL_NAME_DELIMITER: &str = "__";
const AGERE_CONNECTORS_TOKEN_ENV_VAR: &str = "AGERE_CONNECTORS_TOKEN";
...
pub fn mcp_permission_prompt_is_auto_approved(
    ...
    match permission_profile {
        ...
        PermissionProfile::Managed { file_system, .. } => {
            file_system.to_access_policy().has_full_disk_write_access()
        }
    }
}

/// MCP runtime settings derived from `agere_core::config::Config`.
///
/// This struct should contain only long-lived configuration values that the
/// `agere-mcp` crate needs to construct server transports, enforce MCP
/// approval policies and MCP execution environments, locate OAuth state, and merge plugin-provided MCP
...
/// [`with_agere_apps_mcp`] and snapshot collection helpers so config objects do
/// not go stale when auth changes.
#[derive(Debug, Clone)]
pub struct McpConfig {
    /// Base URL for ChatGPT-hosted app MCP servers, copied from the root config.
    pub chatgpt_base_url: String,
    /// Agere home directory used for MCP OAuth state and app-tool cache files.
    ...
    /// Optional path to the `agere` binary containing the `agere-linux-helper` argv0 re-exec shim.
    pub agere_linux_exe: Option<PathBuf>,
    /// Whether to advertise legacy Linux Landlock behavior in MCP execution-state metadata.
...
pub fn with_agere_apps_mcp(
    mut servers: HashMap<String, McpServerConfig>,
    auth: Option<&AgereAuth>,
    config: &McpConfig,
) -> HashMap<String, McpServerConfig> {
    if config.apps_enabled && auth.is_some_and(AgereAuth::uses_agere_backend) {
        servers.insert(
            AGERE_APPS_MCP_SERVER_NAME.to_string(),
            agere_apps_mcp_server_config(config),
        );
    } else {
        servers.remove(AGERE_APPS_MCP_SERVER_NAME);
    }
    servers
}
...
pub fn effective_mcp_servers(
    ...
    with_agere_apps_mcp(servers, auth, config)
}
...
pub(crate) fn agere_apps_mcp_url(config: &McpConfig) -> String {
    agere_apps_mcp_url_for_base_url(&config.chatgpt_base_url)
}
...
fn agere_apps_mcp_bearer_token_env_var() -> Option<String> {
    match env::var(AGERE_CONNECTORS_TOKEN_ENV_VAR) {
        Ok(value) if !value.trim().is_empty() => Some(AGERE_CONNECTORS_TOKEN_ENV_VAR.to_string()),
        Ok(_) => None,
        Err(env::VarError::NotPresent) => None,
        Err(env::VarError::NotUnicode(_)) => Some(AGERE_CONNECTORS_TOKEN_ENV_VAR.to_string()),
    }
}

fn normalize_agere_apps_base_url(base_url: &str) -> String {
...
fn agere_apps_mcp_url_for_base_url(base_url: &str) -> String {
    let base_url = normalize_agere_apps_base_url(base_url);
    if base_url.contains("/backend-api") {
        format!("{base_url}/wham/apps")
    } else if base_url.contains("/api/agere") {
        format!("{base_url}/apps")
    } else {
        format!("{base_url}/api/agere/apps")
    }
}

fn agere_apps_mcp_server_config(config: &McpConfig) -> McpServerConfig {
    let url = agere_apps_mcp_url(config);

    McpServerConfig {
        transport: McpServerTransportConfig::StreamableHttp {
            url,
            bearer_token_env_var: agere_apps_mcp_bearer_token_env_var(),
            http_headers: None,
