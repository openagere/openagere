...
pub(super) async fn test_config() -> Config {
    ...
    let agere_home = tempfile::Builder::new()
        .prefix("chatwidget-tests-")
        .tempdir()
        .expect("tempdir")
        .keep();
    let mut config =
        Config::load_default_with_cli_overrides_for_agere_home(agere_home.clone(), Vec::new())
            .await
            .expect("config");
    config.agere_home = agere_home.abs();
    config.sqlite_home = agere_home.clone();
    config.log_dir = agere_home.join("log");
...
pub(super) async fn make_chatwidget_manual(
    ...
    let mut bottom = BottomPane::new(BottomPaneParams {
        ...
        placeholder_text: "Ask Agere to do anything".to_string(),
    ...
    let model_provider_id = cfg.model_provider_id.clone();
    let mut widget = ChatWidget {
        app_event_tx,
        agere_op_target: super::AgereOpTarget::Direct(op_tx),
        bottom_pane: bottom,
        active_cell: None,
        ...
        model_catalog,
        session_telemetry,
        session_header: SessionHeader::new(model_provider_id, resolved_model.clone()),
        ...
        normal_placeholder_text: "Ask Agere to do anything".to_string(),
...
fn test_model_info(slug: &str, priority: i32, supports_fast_mode: bool) -> ModelInfo {
    let additional_speed_tiers = if supports_fast_mode {
        vec![agere_protocol::openai_models::SPEED_TIER_FAST]
    } else {
        Vec::new()
    };
...
pub(crate) fn set_fast_mode_test_catalog(chat: &mut ChatWidget) {
    let models: Vec<ModelPreset> = ModelsResponse {
        models: vec![
            ...
            test_model_info(
                "gpt-5.3", /*priority*/ 1, /*supports_fast_mode*/ false,
            ),
        ],
    }
    .models
...
pub(super) fn begin_exec_with_source(
    ...
    let parsed_cmd: Vec<ParsedCommand> =
        agere_shell_command::parse_command::parse_command(&command);
...
pub(super) fn plugins_test_curated_marketplace(
    ...
    PluginMarketplaceEntry {
        name: OPENAGERE_CURATED_MARKETPLACE_NAME.to_string(),
        path: Some(plugins_test_absolute_path("marketplaces/chatgpt")),
        interface: Some(MarketplaceInterface {
            display_name: Some("Codex official".to_string()),
        }),
        plugins,
    }
}
...
pub(super) fn plugins_test_detail(
    ...
    PluginDetail {
        marketplace_name: "OpenAgere Marketplace".to_string(),
        marketplace_path: Some(plugins_test_absolute_path("marketplaces/openagere")),
        summary,
            ...
            run: agere_protocol::protocol::HookRunSummary {
                ...
                entries: vec![
                    agere_protocol::protocol::HookOutputEntry {
                        kind: agere_protocol::protocol::HookOutputEntryKind::Warning,
...
fn hook_event_label(event_name: agere_protocol::protocol::HookEventName) -> &'static str {
    match event_name {
        agere_protocol::protocol::HookEventName::PreToolUse => "PreToolUse",
        agere_protocol::protocol::HookEventName::PermissionRequest => "PermissionRequest",
        agere_protocol::protocol::HookEventName::PostToolUse => "PostToolUse",
        agere_protocol::protocol::HookEventName::SessionStart => "SessionStart",
        agere_protocol::protocol::HookEventName::UserPromptSubmit => "UserPromptSubmit",
        agere_protocol::protocol::HookEventName::Stop => "Stop",
    }
}
