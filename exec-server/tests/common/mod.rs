...
use agere_test_binary_support::TestBinaryDispatchGuard;
use agere_test_binary_support::TestBinaryDispatchMode;
use agere_test_binary_support::configure_test_binary_dispatch;
use ctor::ctor;

pub(crate) mod exec_server;

pub(crate) const DELAYED_OUTPUT_AFTER_EXIT_PARENT_ARG: &str =
    "--agere-test-delayed-output-after-exit-parent";

const DELAYED_OUTPUT_AFTER_EXIT_CHILD_ARG: &str = "--agere-test-delayed-output-after-exit-child";

#[ctor]
pub static TEST_BINARY_DISPATCH_GUARD: Option<TestBinaryDispatchGuard> = {
    let guard = configure_test_binary_dispatch("agere-exec-server-tests", |exe_name, argv1| {
        if argv1 == Some(AGERE_FS_HELPER_ARG1) {
            return TestBinaryDispatchMode::DispatchArg0Only;
        }
        if exe_name == AGERE_LINUX_HELPER_ARG0 {
            return TestBinaryDispatchMode::DispatchArg0Only;
        }
...
pub(crate) fn current_test_binary_helper_paths() -> anyhow::Result<(PathBuf, Option<PathBuf>)> {
    ...
    let agere_linux_exe = if cfg!(target_os = "linux") {
        TEST_BINARY_DISPATCH_GUARD
            .as_ref()
            .and_then(|guard| guard.paths().agere_linux_exe.clone())
            .or_else(|| Some(current_exe.clone()))
    } else {
        None
    };
    Ok((current_exe, agere_linux_exe))
}
...
fn maybe_run_exec_server_from_test_binary(guard: Option<&TestBinaryDispatchGuard>) {
    ...
    let runtime_paths = match ExecServerRuntimePaths::new(
        current_exe.clone(),
        linux_helper_exe(guard, &current_exe),
    ) {
    ...
    let exit_code = match runtime.block_on(agere_exec_server::run_main(&listen_url, runtime_paths))
    {
        Ok(()) => 0,
        Err(error) => {
