...
fn usage_limit_reached_error_formats_plus_plan() {
    ...
    assert_eq!(
        err.to_string(),
        "You've hit your usage limit. Upgrade to Pro (https://chatgpt.com/explore/pro), visit https://chatgpt.com/agere/settings/usage to purchase more credits or try again later."
    );
}

#[test]
fn server_overloaded_maps_to_protocol() {
    let err = AgereErr::ServerOverloaded;
    assert_eq!(
        err.to_agere_protocol_error(),
        AgereErrorInfo::ServerOverloaded
    );
}

#[test]
fn exec_policy_denial_uses_aggregated_output_when_stderr_empty() {
    ...
    let err = AgereErr::Exec(ExecErr::Denied {
...
#[test]
fn exec_policy_denial_reports_both_streams_when_available() {
    let output = ExecToolCallOutput {
        exit_code: 9,
...
#[test]
fn exec_policy_denial_reports_stdout_when_no_stderr() {
    let output = ExecToolCallOutput {
        exit_code: 11,
...
fn to_error_event_handles_response_stream_failed() {
    ...
    let err = AgereErr::ResponseStreamFailed(ResponseStreamFailed {
        source,
    ...
    assert_eq!(
        event.agere_error_info,
        Some(AgereErrorInfo::ResponseStreamConnectionFailed {
...
fn exec_policy_denial_reports_exit_code_when_no_output_available() {
    ...
    assert_eq!(
        ...
        "command failed with restricted execution (exit code 13)"
    );
}
...
fn usage_limit_reached_error_formats_free_plan() {
    ...
    assert_eq!(
        err.to_string(),
        "You've hit your usage limit. Upgrade to Plus to continue using Agere (https://chatgpt.com/explore/plus), or try again later."
    );
}
...
fn usage_limit_reached_error_formats_pro_plan_with_reset() {
    ...
    with_now_override(base, move || {
        ...
        let expected = format!(
            "You've hit your usage limit. Visit https://chatgpt.com/agere/settings/usage to purchase more credits or try again at {expected_time}."
        );
        assert_eq!(err.to_string(), expected);
    });
}
...
#[test]
fn usage_limit_reached_error_hides_upsell_for_non_agere_limit_name() {
    let base = Utc.with_ymd_and_hms(2024, 1, 1, 0, 0, 0).unwrap();
    let resets_at = base + ChronoDuration::hours(1);
    with_now_override(base, move || {
        let expected_time = format_retry_timestamp(&resets_at);
        let err = UsageLimitReachedError {
            ...
            promo_message: Some(
                "Visit https://chatgpt.com/agere/settings/usage to purchase more credits"
                    .to_string(),
            ),
        };
        let expected = format!(
            "You've hit your usage limit for agere_other. Switch to another model now, or try again at {expected_time}."
        );
        assert_eq!(err.to_string(), expected);
    });
}
...
fn unexpected_status_prefers_error_message_when_present() {
    let err = UnexpectedResponseError {
        ...
        body: r#"{"error":{"message":"Workspace is not authorized in this region."},"status":401}"#
            .to_string(),
        url: Some("https://chatgpt.com/backend-api/agere/responses".to_string()),
        cf_ray: None,
        request_id: Some("req-123".to_string()),
        identity_authorization_error: None,
        identity_error_code: None,
    ...
    assert_eq!(
        err.to_string(),
        format!(
            "unexpected status {status}: Workspace is not authorized in this region., url: https://chatgpt.com/backend-api/agere/responses, request id: req-123"
        )
    );
}
...
fn unexpected_status_includes_cf_ray_and_request_id() {
    ...
    assert_eq!(
        err.to_string(),
        format!(
            "unexpected status {status}: plain text error, url: https://chatgpt.com/backend-api/agere/responses, cf-ray: 9c81f9f18f2fa49d-LHR, request id: req-xyz"
        )
    );
}
...
fn unexpected_status_includes_identity_auth_details() {
    ...
    assert_eq!(
        err.to_string(),
        format!(
            "unexpected status {status}: plain text error, url: https://chatgpt.com/backend-api/agere/models, cf-ray: cf-ray-auth-401-test, request id: req-auth, auth error: missing_authorization_header, auth error code: token_expired"
        )
    );
}
...
fn usage_limit_reached_includes_hours_and_minutes() {
    ...
    with_now_override(base, move || {
        ...
        let expected = format!(
            "You've hit your usage limit. Upgrade to Pro (https://chatgpt.com/explore/pro), visit https://chatgpt.com/agere/settings/usage to purchase more credits or try again at {expected_time}."
        );
        assert_eq!(err.to_string(), expected);
    });
}
...
fn usage_limit_reached_with_promo_message() {
    ...
    with_now_override(base, move || {
        ...
        let err = UsageLimitReachedError {
            ...
            rate_limits: Some(Box::new(rate_limit_snapshot())),
            promo_message: Some(
                "To continue using Agere, start a free trial of <PLAN> today".to_string(),
            ),
        };
        let expected = format!(
            "You've hit your usage limit. To continue using Agere, start a free trial of <PLAN> today, or try again at {expected_time}."
        );
        assert_eq!(err.to_string(), expected);
    });
}
