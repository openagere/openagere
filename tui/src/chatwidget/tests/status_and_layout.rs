...
fn use_non_openai_provider(chat: &mut ChatWidget) {
    ...
    chat.config.model_provider.wire_api = WireApi::Anthropic;
...
async fn core_cyber_policy_error_renders_dedicated_notice() {
    ...
    chat.handle_agere_event(Event {
        ...
        msg: EventMsg::Error(ErrorEvent {
            ...
            agere_error_info: Some(AgereErrorInfo::CyberPolicy),
        }),
    });
...
#[tokio::test]
async fn core_cyber_policy_error_uses_generic_notice_for_non_openai_provider() {
    let (mut chat, mut rx, _ops) = make_chatwidget_manual(/*model_override*/ None).await;
    use_non_openai_provider(&mut chat);
    ...
    assert_eq!(cells.len(), 1);
    let rendered = lines_to_single_string(&cells[0]);
    assert!(rendered.contains("This chat was flagged by the model provider"));
    assert!(rendered.contains("provider's"));
    assert!(rendered.contains("safety, access, or routing policy"));
...
#[tokio::test]
async fn core_model_verification_uses_generic_warning_for_non_openai_provider() {
    let (mut chat, mut rx, _ops) = make_chatwidget_manual(/*model_override*/ None).await;
    use_non_openai_provider(&mut chat);
    ...
    assert_eq!(cells.len(), 1);
    let rendered = lines_to_single_string(&cells[0]);
    assert!(rendered.contains("model provider reported repeated cybersecurity-risk safety checks"));
    assert!(rendered.contains("provider's safety, access, or routing policy"));
    assert!(!rendered.contains("Trusted Access"));
    assert!(!rendered.contains("chatgpt.com/cyber"));
}
...
#[tokio::test]
async fn token_count_keeps_runtime_context_window_for_current_turn() {
    let (mut chat, _rx, _ops) = make_chatwidget_manual(/*model_override*/ None).await;
...
#[tokio::test]
async fn provider_switch_sequence_updates_context_window_from_new_catalog() {
    let (mut chat, _rx, _ops) = make_chatwidget_manual(/*model_override*/ None).await;
    ...
    assert_eq!(
        chat.status_line_value_for_item(&crate::bottom_pane::StatusLineItem::ContextWindowSize),
        Some("128K window".to_string())
    );
    assert_eq!(chat.bottom_pane.context_window_percent(), Some(67));
}
...
async fn prefetch_rate_limits_is_gated_on_chatgpt_auth_provider() {
    ...
    chat.config.model_provider.requires_provider_auth = false;
    assert!(!chat.should_prefetch_rate_limits());
...
async fn rate_limit_snapshots_keep_separate_entries_per_limit_id() {
    ...
    chat.on_rate_limit_snapshot(Some(RateLimitSnapshot {
        ...
        limit_name: Some("agere".to_string()),
        primary: Some(RateLimitWindow {
            used_percent: 20.0,
            window_minutes: Some(300),
            resets_at: Some(100),
    ...
    let agere = chat
        .rate_limit_snapshots_by_limit_id
        .get("agere")
        .expect("agere snapshot should exist");
    let other = chat
        .rate_limit_snapshots_by_limit_id
        .get("agere_other")
        .expect("agere_other snapshot should exist");

    assert_eq!(agere.primary.as_ref().map(|w| w.used_percent), Some(20.0));
    assert_eq!(
        agere
            .credits
            .as_ref()
...
#[tokio::test]
async fn rate_limit_switch_prompt_skips_non_agere_limit() {
...
async fn fast_status_indicator_is_hidden_for_models_without_fast_support() {
    ...
    set_fast_mode_test_catalog(&mut chat);
    assert!(!get_available_model(&chat, "gpt-5.3").supports_fast_mode());
    chat.set_service_tier(Some(ServiceTier::Fast));
    set_chatgpt_auth(&mut chat);
    set_fast_mode_test_catalog(&mut chat);
    assert!(!get_available_model(&chat, "gpt-5.3").supports_fast_mode());
...
async fn status_line_branch_refreshes_after_interrupt() {
    ...
    chat.handle_agere_event(Event {
        id: "turn-1".into(),
        msg: EventMsg::TurnAborted(agere_protocol::protocol::TurnAbortedEvent {
...
async fn status_line_model_with_reasoning_includes_fast_for_fast_capable_models() {
    ...
    chat.set_model("gpt-5.3");
    chat.refresh_status_line();

    assert_eq!(
        status_line_text(&chat),
        Some(format!("gpt-5.3 xhigh · Context 0% used · {test_cwd}"))
    );
}
...
async fn terminal_title_model_updates_on_model_change_without_manual_refresh() {
    ...
    assert_eq!(chat.last_terminal_title, Some("gpt-5.3".to_string()));
}
...
async fn status_line_model_with_reasoning_updates_on_mode_switch_without_manual_refresh() {
    ...
    assert_eq!(status_line_text(&chat), Some("gpt-5.3 high".to_string()));

    let plan_mask = collaboration_modes::plan_mask(chat.model_catalog.as_ref())
        .expect("expected plan collaboration mode");
    chat.set_collaboration_mask(plan_mask);

    assert_eq!(status_line_text(&chat), Some("gpt-5.3 medium".to_string()));
...
async fn renamed_thread_footer_title_snapshot() {
    ...
    chat.handle_agere_event(Event {
        id: "rename".to_string(),
        msg: EventMsg::ThreadNameUpdated(agere_protocol::protocol::ThreadNameUpdatedEvent {
            thread_id,
...
async fn status_line_goal_active_token_budget_footer_snapshot() {
    ...
    chat.handle_server_notification(
        ServerNotification::ThreadGoalUpdated(
            agere_app_server_protocol::ThreadGoalUpdatedNotification {
                thread_id: "thread-1".to_string(),
                turn_id: None,
                goal: test_thread_goal(
                    agere_app_server_protocol::ThreadGoalStatus::Active,
    ...
    chat.handle_server_notification(
        ServerNotification::ThreadGoalUpdated(
            agere_app_server_protocol::ThreadGoalUpdatedNotification {
                ...
                turn_id: None,
                goal: test_thread_goal(
                    agere_app_server_protocol::ThreadGoalStatus::Complete,
                    /*token_budget*/ None,
                    /*tokens_used*/ 40_000,
                ),
            },
...
async fn thread_goal_update_for_other_thread_is_ignored() {
    ...
    let mut goal = test_thread_goal(
        agere_app_server_protocol::ThreadGoalStatus::BudgetLimited,
        /*token_budget*/ Some(50_000),
        /*tokens_used*/ 50_000,
    );
...
fn test_thread_goal(
    status: agere_app_server_protocol::ThreadGoalStatus,
    token_budget: Option<i64>,
    tokens_used: i64,
) -> agere_app_server_protocol::ThreadGoal {
    agere_app_server_protocol::ThreadGoal {
        thread_id: "thread-1".to_string(),
    ...
    chat.handle_server_notification(
        ServerNotification::HookStarted(AppServerHookStartedNotification {
            ...
            run: AppServerHookRunSummary {
                ...
                source: agere_app_server_protocol::HookSource::User,
                display_order: 0,
...
async fn pre_tool_use_hook_events_render_snapshot() {
    assert_hook_events_snapshot(
        agere_protocol::protocol::HookEventName::PreToolUse,
...
async fn post_tool_use_hook_events_render_snapshot() {
    assert_hook_events_snapshot(
        agere_protocol::protocol::HookEventName::PostToolUse,
...
async fn completed_hook_with_no_entries_stays_out_of_history() {
    ...
    chat.handle_agere_event(Event {
        id: "hook-1".into(),
        msg: EventMsg::HookStarted(agere_protocol::protocol::HookStartedEvent {
            turn_id: None,
            run: agere_protocol::protocol::HookRunSummary {
                id: "post-tool-use:0:/tmp/hooks.json".to_string(),
                event_name: agere_protocol::protocol::HookEventName::PostToolUse,
                handler_type: agere_protocol::protocol::HookHandlerType::Command,
                execution_mode: agere_protocol::protocol::HookExecutionMode::Sync,
                scope: agere_protocol::protocol::HookScope::Turn,
                source_path: PathBuf::from(test_path_display("/tmp/hooks.json")).abs(),
                source: agere_protocol::protocol::HookSource::User,
                display_order: 0,
                status: agere_protocol::protocol::HookRunStatus::Running,
                status_message: None,
                started_at: 1,
                completed_at: None,
                duration_ms: None,
    ...
    chat.handle_agere_event(Event {
        id: "hook-1".into(),
        msg: EventMsg::HookCompleted(agere_protocol::protocol::HookCompletedEvent {
            turn_id: None,
            run: agere_protocol::protocol::HookRunSummary {
                id: "post-tool-use:0:/tmp/hooks.json".to_string(),
                event_name: agere_protocol::protocol::HookEventName::PostToolUse,
                ...
                source: agere_protocol::protocol::HookSource::User,
                display_order: 0,
                status: agere_protocol::protocol::HookRunStatus::Completed,
                status_message: None,
                started_at: 1,
                completed_at: Some(2),
                duration_ms: Some(1),
...
async fn quiet_hook_linger_starts_when_delayed_redraw_reveals_hook() {
    ...
    chat.handle_agere_event(hook_started_event(
        "post-tool-use:0:/tmp/hooks.json",
        agere_protocol::protocol::HookEventName::PostToolUse,
    ...
    chat.handle_agere_event(hook_completed_event(
        "post-tool-use:0:/tmp/hooks.json",
        agere_protocol::protocol::HookEventName::PostToolUse,
        agere_protocol::protocol::HookRunStatus::Completed,
        Vec::new(),
    ));
...
async fn blocked_and_failed_hooks_render_feedback_and_errors() {
    ...
    chat.handle_agere_event(hook_completed_event(
        "pre-tool-use:0:/tmp/hooks.json",
        agere_protocol::protocol::HookEventName::PreToolUse,
        agere_protocol::protocol::HookRunStatus::Blocked,
        vec![agere_protocol::protocol::HookOutputEntry {
            kind: agere_protocol::protocol::HookOutputEntryKind::Feedback,
    ...
    chat.handle_agere_event(hook_completed_event(
        ...
        agere_protocol::protocol::HookEventName::PostToolUse,
        agere_protocol::protocol::HookRunStatus::Failed,
        vec![agere_protocol::protocol::HookOutputEntry {
            kind: agere_protocol::protocol::HookOutputEntryKind::Error,
...
async fn completed_same_id_hook_output_survives_restart() {
    ...
    chat.handle_agere_event(hook_started_event(
        hook_id,
        agere_protocol::protocol::HookEventName::Stop,
    ...
    chat.handle_agere_event(hook_completed_event(
        hook_id,
        agere_protocol::protocol::HookEventName::Stop,
        agere_protocol::protocol::HookRunStatus::Stopped,
        vec![agere_protocol::protocol::HookOutputEntry {
            kind: agere_protocol::protocol::HookOutputEntryKind::Stop,
...
async fn hook_completed_before_reveal_renders_completed_without_running_flash() {
    ...
    chat.handle_agere_event(hook_started_event(
        ...
        agere_protocol::protocol::HookEventName::SessionStart,
    ...
    chat.handle_agere_event(hook_completed_event(
        ...
        agere_protocol::protocol::HookRunStatus::Completed,
        vec![agere_protocol::protocol::HookOutputEntry {
            kind: agere_protocol::protocol::HookOutputEntryKind::Context,
...
fn hook_started_event(
    id: &str,
    event_name: agere_protocol::protocol::HookEventName,
    ...
    Event {
        ...
        msg: EventMsg::HookStarted(agere_protocol::protocol::HookStartedEvent {
            ...
            run: hook_run_summary(
                id,
                event_name,
                agere_protocol::protocol::HookRunStatus::Running,
                status_message,
                Vec::new(),
            ),
        }),
...
fn hook_completed_event(
    id: &str,
    event_name: agere_protocol::protocol::HookEventName,
    status: agere_protocol::protocol::HookRunStatus,
    entries: Vec<agere_protocol::protocol::HookOutputEntry>,
) -> Event {
    Event {
        id: id.to_string(),
        msg: EventMsg::HookCompleted(agere_protocol::protocol::HookCompletedEvent {
...
fn hook_run_summary(
    ...
    agere_protocol::protocol::HookRunSummary {
        ...
        started_at: 1,
        completed_at: (status != agere_protocol::protocol::HookRunStatus::Running).then_some(2),
        duration_ms: (status != agere_protocol::protocol::HookRunStatus::Running).then_some(1),
        entries,
    }
}
