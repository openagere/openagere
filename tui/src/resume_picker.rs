...
use crate::resolve_session_thread_id;
            ...
            ThreadItem::UserMessage { content, .. } => Some(TranscriptPreviewLine {
                ...
                text: content
                    ...
                    .filter_map(|input| match input {
                        agere_app_server_protocol::UserInput::Text { text, .. } => {
                            Some(text.as_str())
                        }
                        _ => None,
                    })
...
impl PickerState {
    ...
    async fn persist_density(&self) -> Result<()> {
        ...
        ConfigEditsBuilder::new(&persistence.agere_home)
...
"#
    ...
    async fn ctrl_o_keeps_toggled_density_when_persistence_fails() {
        let tmp = tempdir().expect("tmpdir");
        let agere_home_file = tmp.path().join("codex-home-file");
        std::fs::write(&agere_home_file, "not a directory").expect("write codex home file");
        let loader = page_only_loader(|_| {});
        let mut state = PickerState::new(
            FrameRequester::test_dummy(),
            loader,
    ...
    fn app_server_row_keeps_pathless_threads() {
        ...
        let thread = Thread {
            ...
            source: agere_app_server_protocol::SessionSource::Cli,
            agent_nickname: None,
            agent_role: None,
            git_info: None,
    ...
    fn thread_to_transcript_cells_renders_core_message_types() {
        ...
        let thread = Thread {
            ...
            git_info: None,
            name: None,
            turns: vec![agere_app_server_protocol::Turn {
    ...
    #[tokio::test]
    async fn control_characters_are_not_appended_to_query() {
        let loader = page_only_loader(|_| {});
        let mut state = PickerState::new(
            FrameRequester::test_dummy(),
            loader,
        ...
        // Raw C0 control characters sent with no modifiers must not pollute
        // the search query.
        for code in ['\u{0004}', '\u{000c}', '\u{0015}', '\u{0017}'] {
            state
                .handle_key(KeyEvent::new(KeyCode::Char(code), KeyModifiers::NONE))
                .await
                .unwrap();
        }
        assert_eq!(state.query, "");

        // Regular printable input still appends to the query.
        state
