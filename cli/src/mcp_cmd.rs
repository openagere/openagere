...
use agere_core::McpManager;
use agere_core::config::Config;
use agere_core::config::edit::ConfigEditsBuilder;
use agere_core::config::find_agere_home;
use agere_core::config::load_global_mcp_servers;
use agere_core::plugins::PluginsManager;
use agere_mcp::McpOAuthLoginSupport;
use agere_mcp::ResolvedMcpOAuthScopes;
use agere_mcp::compute_auth_statuses;
use agere_mcp::discover_supported_scopes;
use agere_mcp::oauth_login_support;
use agere_mcp::resolve_oauth_scopes;
use agere_mcp::should_retry_without_scopes;
use agere_protocol::protocol::McpAuthStatus;
use agere_rmcp_client::delete_oauth_tokens;
use agere_rmcp_client::perform_oauth_login;
...
/// - `add`    — add a server launcher entry to `~/.openagere/config.toml`
...
#[derive(Debug, clap::Parser)]
#[command(override_usage = "agere mcp add [OPTIONS] <NAME> (--url <URL> | -- <COMMAND>...)")]
pub struct AddArgs {
...
async fn perform_oauth_login_retry_without_scopes(
    name: &str,
    url: &str,
    store_mode: agere_config::types::OAuthCredentialsStoreMode,
...
async fn run_add(config_overrides: &CliConfigOverrides, add_args: AddArgs) -> Result<()> {
    ...
    let agere_home = find_agere_home().context("failed to resolve AGERE_HOME")?;
    let mut servers = load_global_mcp_servers(&agere_home)
        .await
        .with_context(|| format!("failed to load MCP servers from {}", agere_home.display()))?;
    ...
    ConfigEditsBuilder::new(&agere_home)
        ...
        .apply()
        .await
        .with_context(|| format!("failed to write MCP servers to {}", agere_home.display()))?;
    ...
    match oauth_login_support(&transport).await {
        ...
        McpOAuthLoginSupport::Unknown(_) => println!(
            "MCP server may or may not require login. Run `agere mcp login {name}` to login."
        ),
    }
...
async fn run_list(config_overrides: &CliConfigOverrides, list_args: ListArgs) -> Result<()> {
    ...
    if entries.is_empty() {
        println!("No MCP servers configured yet. Try `agere mcp add my-tool -- my-command`.");
        return Ok(());
    }
...
async fn run_get(config_overrides: &CliConfigOverrides, get_args: GetArgs) -> Result<()> {
    ...
    println!("  remove: agere mcp remove {}", get_args.name);
