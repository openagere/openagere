...
use agere_hooks::HookEventAfterToolUse;
use agere_hooks::HookPayload;
use agere_hooks::HookResult;
use agere_hooks::HookToolInput;
use agere_hooks::HookToolInputLocalShell;
use agere_hooks::HookToolKind;
...
use agere_utils_readiness::Readiness;
...
impl ToolRegistry {
    ...
    pub(crate) async fn dispatch_any(
        ...
        let access_mode_tag = permission_profile_access_mode_tag(
            &invocation.turn.permission_profile,
            invocation.turn.windows_execution_restriction_level,
            invocation.turn.network.is_some(),
        );
        let metric_tags: [(&str, &str); 1] = [("access_mode", &access_mode_tag)];
        ...
        .dispatch(HookPayload {
            ...
            hook_event: HookEvent::AfterToolUse {
                event: HookEventAfterToolUse {
                    ...
                    access_mode: permission_profile_access_mode_tag(
                        &turn.permission_profile,
                        turn.windows_execution_restriction_level,
                        turn.network.is_some(),
                    )
                    .to_string(),
                    access_policy: permission_profile_policy_tag(
                        &turn.permission_profile,
                        turn.windows_execution_restriction_level,
                    )
                    .to_string(),
...
// Stub: permission_profile_access_mode_tag lived in a removed tags helper module.
fn permission_profile_access_mode_tag(
    profile: &agere_protocol::models::PermissionProfile,
    _windows_execution_restriction_level: agere_protocol::config_types::WindowsExecutionRestrictionLevel,
    _network_enabled: bool,
) -> String {
    match profile {
        agere_protocol::models::PermissionProfile::Disabled => "disabled".into(),
        agere_protocol::models::PermissionProfile::Managed { .. } => "managed".into(),
        agere_protocol::models::PermissionProfile::External { .. } => "external".into(),
    }
}
...
fn permission_profile_policy_tag(
    ...
    permission_profile_access_mode_tag(profile, _windows_execution_restriction_level, false)
}
