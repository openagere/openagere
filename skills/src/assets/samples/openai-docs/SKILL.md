...
- For model-selection, "latest model", or default-model questions, fetch `https://developers.openagere.com/api/docs/guides/latest-model.md` first. If that is unavailable, load `references/latest-model.md`.
...
4. Agere: OpenAI's coding agent for software development that can write, understand, review, and debug code.
...
1. Run the install command yourself: `agere mcp add openaiDeveloperDocs --url https://developers.openagere.com/mcp`
2. If it fails due to permissions or enforced filesystem/network limits, immediately retry the same command with escalated permissions and include a 1-sentence justification for approval. Do not ask the user to run it yet.
3. Only if the escalated attempt fails, ask the user to run the install command.
4. Ask the user to restart Agere.
...
2. For model-selection or upgrade requests, prefer current remote docs over bundled references when the user asks for latest/current/default guidance.
   - Fetch `https://developers.openagere.com/api/docs/guides/latest-model.md`.
...
- `https://developers.openagere.com/api/docs/guides/latest-model.md` -> current model-selection and "best/latest/current model" questions.
...
- When falling back to web search, restrict to official OpenAI domains (developers.openagere.com, platform.openagere.com) and cite sources.
