...
impl ChatWidget {
        ...
        match cmd {
            ...
            SlashCommand::Rename => {
                self.session_telemetry
                    .counter("agere.thread.rename", /*inc*/ 1, &[]);
            ...
            SlashCommand::SetupDefaultRestriction => {
                #[cfg(target_os = "windows")]
                {
                    ...
                    if !windows_degraded_restriction_enabled || !false {
                    ...
                    self.session_telemetry.counter(
                        "agere.windows_restriction.setup_elevated_restriction_command",
                        /*inc*/ 1,
                        &[],
                    );
                    self.app_event_tx
                        .send(AppEvent::BeginWindowsRestrictionElevatedSetup { preset });
                }
            ...
            SlashCommand::AddReadDir => {
                self.add_error_message(
                    "Usage: /add-read-dir <absolute-directory-path>".to_string(),
                );
            }
            ...
            // SlashCommand::Undo => {
            //     self.app_event_tx.send(AppEvent::AgereOp(Op::Undo));
            // }
            SlashCommand::Copy => {
        ...
        match cmd {
            ...
            SlashCommand::Goal if !trimmed.is_empty() => {
                ...
                let control_command = match trimmed.to_ascii_lowercase().as_str() {
                    ...
                    "resume" | "unpause" => {
                        Some(GoalControlCommand::SetStatus(AppThreadGoalStatus::Active))
                    }
                    _ => None,
                };
            ...
            SlashCommand::AddReadDir if !trimmed.is_empty() => {
                self.app_event_tx
                    .send(AppEvent::BeginWindowsRestrictionGrantReadRoot { path: args });
            }
    ...
    /// Handle `/provider <name>` by trying to switch directly when possible and
    /// falling back to the picker for unknown names.
    fn dispatch_provider_inline_arg(&mut self, name: &str) {
        let name = name.trim();
        if name.is_empty() {
            self.open_provider_popup();
            return;
        ...
        if let Some(entry) = archive.find(name) {
            if !entry.has_api_key() {
                self.add_info_message(
                    format!("Provider '{name}' is missing an API key."),
                    Some("Open the picker with /provider to enter one.".to_string()),
                );
                self.open_provider_popup();
            } else {
                self.app_event_tx
                    .send(crate::app_event::AppEvent::SwitchProvider {
                        name: name.to_string(),
                    });
            }
            return;
        ...
        if templates.iter().any(|t| t.name == name) {
            self.add_info_message(
                format!("Provider '{name}' is a known template."),
                Some("Open the picker with /provider to enter an API key.".to_string()),
            );
        } else {
            self.add_info_message(format!("Unknown provider: '{name}'."), None);
        }
        self.open_provider_popup();
    }
    ...
    fn builtin_command_flags(&self) -> slash_commands::BuiltinCommandFlags {
        ...
        #[cfg(not(target_os = "windows"))]
        let allow_elevate_restriction = false;
