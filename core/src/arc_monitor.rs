...
const AGERE_ARC_MONITOR_ENDPOINT_OVERRIDE: &str = "AGERE_ARC_MONITOR_ENDPOINT_OVERRIDE";
const AGERE_ARC_MONITOR_TOKEN: &str = "AGERE_ARC_MONITOR_TOKEN";
...
struct ArcMonitorMetadata {
    agere_thread_id: String,
    agere_turn_id: String,
    #[serde(skip_serializing_if = "Option::is_none")]
...
pub(crate) async fn monitor_action(
    ...
    let auth = match turn_context.auth_manager.as_ref() {
        Some(auth_manager) => match auth_manager.auth().await {
            Some(auth) if auth.uses_agere_backend() => Some(auth),
            _ => None,
        },
        None => None,
    };
    let env_token = read_non_empty_env_var(AGERE_ARC_MONITOR_TOKEN);
    if env_token.is_none() && auth.is_none() {
        return ArcMonitorOutcome::Ok;
    }

    let url = read_non_empty_env_var(AGERE_ARC_MONITOR_ENDPOINT_OVERRIDE).unwrap_or_else(|| {
        format!(
            "{}/agere/safety/arc",
    ...
    if let Some(token) = env_token {
        ...
        request =
            request.headers(agere_model_provider::auth_provider_from_auth(auth).to_auth_headers());
    }
...
async fn build_arc_monitor_request(
    ...
    ArcMonitorRequest {
        metadata: ArcMonitorMetadata {
            agere_thread_id: conversation_id.clone(),
            agere_turn_id: turn_context.sub_id.clone(),
