...
impl TraceReducer {
    ...
    pub(super) fn start_compaction_request(
        ...
        let Some(turn) = self.rollout.agere_turns.get(&started.agere_turn_id) else {
            bail!(
                "compaction request {} referenced unknown agere turn {}",
                started.compaction_request_id,
                started.agere_turn_id
            );
        };
        if turn.thread_id != started.thread_id {
            bail!(
                "compaction request {} used thread {}, but agere turn {} belongs to {}",
                started.compaction_request_id,
                started.thread_id,
                started.agere_turn_id,
                turn.thread_id
        ...
        self.rollout.compaction_requests.insert(
            ...
            CompactionRequest {
                ...
                agere_turn_id: started.agere_turn_id,
    ...
    pub(super) fn reduce_compaction_installed_event(
        ...
        let Some(turn) = self.rollout.agere_turns.get(&agere_turn_id) else {
            bail!(
                "compaction install {compaction_id} referenced unknown agere turn {agere_turn_id}"
            );
        };
        if turn.thread_id != thread_id {
            bail!(
                "compaction install {compaction_id} used thread {thread_id}, but agere turn {agere_turn_id} belongs to {}",
                turn.thread_id
            );
        }
...
pub(super) struct StartedCompactionRequest {
    ...
    pub(super) agere_turn_id: String,
