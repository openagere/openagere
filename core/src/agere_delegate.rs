...
use agere_analytics::GuardianApprovalRequestSource;
...
/// Start an interactive sub-Agere thread and return IO channels.
///
/// The returned `events_rx` yields non-approval events emitted by the sub-agent.
/// Approval requests are handled via `parent_session` and are not surfaced.
/// The returned `ops_tx` allows the caller to submit additional `Op`s to the sub-agent.
#[allow(clippy::too_many_arguments)]
pub(crate) async fn run_agere_thread_interactive(
    config: Config,
...
) -> Result<Agere, AgereErr> {
    let (tx_sub, rx_sub) = async_channel::bounded(SUBMISSION_CHANNEL_CAPACITY);
    let (tx_ops, rx_ops) = async_channel::bounded(SUBMISSION_CHANNEL_CAPACITY);
    let AgereSpawnOk { agere, .. } = Box::pin(Agere::spawn(AgereSpawnArgs {
        config,
        auth_manager,
        models_manager,
        ...
        parent_rollout_thread_trace: agere_rollout_trace::ThreadTraceContext::disabled(),
        parent_trace: None,
    ...
    let agere_for_events = Arc::clone(&agere);
    ...
    let agere_for_ops = Arc::clone(&agere);
    tokio::spawn(async move {
        forward_ops(agere_for_ops, rx_ops, cancel_token_ops).await;
    });

    Ok(Agere {
        tx_sub: tx_ops,
        rx_event: rx_sub,
        agent_status: agere.agent_status.clone(),
        session: Arc::clone(&agere.session),
        session_loop_termination: agere.session_loop_termination.clone(),
    })
}
...
#[allow(clippy::too_many_arguments)]
pub(crate) async fn run_agere_thread_one_shot(
    config: Config,
    auth_manager: Arc<AuthManager>,
    models_manager: SharedModelsManager,
    input: Vec<UserInput>,
    ...
    let io = Box::pin(run_agere_thread_interactive(
        config,
        auth_manager,
        models_manager,
        parent_session,
        ...
        tokio::select! {
            ...
            event = agere.next_event() => {
                ...
                match event {
                    ...
                    other => {
                        if !forward_event_or_shutdown(&agere, &tx_sub, &cancel_token, other).await
                        {
                            break;
                        }
                    }
...
async fn shutdown_delegate(agere: &Agere) {
    let _ = agere.submit(Op::Interrupt).await;
    let _ = agere.submit(Op::Shutdown {}).await;

    let _ = timeout(Duration::from_millis(500), async {
        while let Ok(event) = agere.next_event().await {
            if matches!(
                event.msg,
...
async fn forward_ops(
    ...
    loop {
        ...
        let _ = agere.submit_with_id(submission).await;
    }
}
    ...
    let decision = if routes_approval_to_guardian(parent_ctx) {
        ...
        let review_rx = spawn_approval_request_review(
            ...
            GuardianApprovalRequest::Shell {
                ...
                command,
                cwd,
                permission_level: if additional_permissions.is_some() {
                    crate::execution::ExecutionPermissionLevel::WithAdditionalPermissions
                } else {
                    crate::execution::ExecutionPermissionLevel::UseDefault
                },
                additional_permissions,
                justification: None,
            },
    ...
    let guardian_decision = if routes_approval_to_guardian(parent_ctx) {
        ...
        let patch = changes
            ...
            .map(|(path, change)| match change {
                agere_protocol::protocol::FileChange::Add { content } => {
                    format!("*** Add File: {}\n{}", path.display(), content)
                }
                agere_protocol::protocol::FileChange::Delete { content } => {
                    format!("*** Delete File: {}\n{}", path.display(), content)
                }
                agere_protocol::protocol::FileChange::Update {
                    unified_diff,
                    move_path,
                } => {
...
async fn handle_request_user_input(
    ...
    if routes_approval_to_guardian(parent_ctx)
        ...
        .await
    {
        let _ = agere.submit(Op::UserInputAnswer { id, response }).await;
        return;
    }
...
async fn maybe_auto_review_mcp_request_user_input(
    ...
    Some(RequestUserInputResponse {
        answers: HashMap::from([(
            question.id.clone(),
            agere_protocol::request_user_input::RequestUserInputAnswer {
...
where
    F: core::future::Future<Output = agere_protocol::protocol::ReviewDecision>,
{
    tokio::select! {
        biased;
        _ = cancel_token.cancelled() => {
            ...
            parent_session
                .notify_approval(approval_id, agere_protocol::protocol::ReviewDecision::Abort)
                .await;
