...
pub struct ConfigRequirementsToml {
    ...
    pub allowed_access_modes: Option<Vec<AccessModeRequirement>>,
    pub remote_access_config: Option<Vec<RemoteAccessConfigToml>>,
...
pub struct RemoteAccessConfigToml {
    pub hostname_patterns: Vec<String>,
    pub allowed_access_modes: Vec<AccessModeRequirement>,
}
...
pub struct ConfigRequirementsWithSources {
    ...
    pub allowed_access_modes: Option<Sourced<Vec<AccessModeRequirement>>>,
...
impl ConfigRequirementsWithSources {
    pub fn merge_unset_fields(&mut self, source: RequirementSource, other: ConfigRequirementsToml) {
        ...
        let ConfigRequirementsToml {
            ...
            allowed_access_modes: _,
            remote_access_config: _,
    ...
    pub fn into_toml(self) -> ConfigRequirementsToml {
        ...
        ConfigRequirementsToml {
            ...
            allowed_access_modes: allowed_access_modes.map(|sourced| sourced.value),
...
/// Currently, `external-access` is not supported in config.toml, but it is
/// supported through programmatic use.
#[derive(Deserialize, Debug, Clone, Copy, PartialEq)]
pub enum AccessModeRequirement {
    ...
    #[serde(rename = "external-access")]
    External,
}

impl From<AccessMode> for AccessModeRequirement {
    fn from(mode: AccessMode) -> Self {
        match mode {
            AccessMode::ReadOnly => AccessModeRequirement::ReadOnly,
            AccessMode::WorkspaceWrite => AccessModeRequirement::WorkspaceWrite,
            AccessMode::DangerFullAccess => AccessModeRequirement::DangerFullAccess,
        }
    }
}
...
impl ConfigRequirementsToml {
    pub fn apply_remote_access_config(&mut self, hostname: Option<&str>) {
        let Some(remote_access_config) = self.remote_access_config.as_ref() else {
            return;
        };
        let Some(hostname) = hostname.and_then(normalize_hostname) else {
            return;
        };
        let Some(matched_config) = remote_access_config
            .iter()
            .find(|config| hostname_matches_any_pattern(&hostname, &config.hostname_patterns))
        else {
            return;
        };
        self.allowed_access_modes = Some(matched_config.allowed_access_modes.clone());
    }

    pub fn is_empty(&self) -> bool {
        self.allowed_approval_policies.is_none()
            && self.allowed_approvals_reviewers.is_none()
            && self.allowed_access_modes.is_none()
            && self.remote_access_config.is_none()
...
impl TryFrom<ConfigRequirementsWithSources> for ConfigRequirements {
    ...
    fn try_from(toml: ConfigRequirementsWithSources) -> Result<Self, Self::Error> {
        ...
        let permission_profile = match allowed_access_modes {
            Some(Sourced {
                value: modes,
                source: requirement_source,
            }) => {
                if !modes.contains(&AccessModeRequirement::ReadOnly) {
                    return Err(ConstraintError::InvalidValue {
                        field_name: "allowed_access_modes",
                ...
                let constrained = Constrained::new(default_permission_profile, move |candidate| {
                    let mode = access_mode_requirement_for_permission_profile(candidate);
...
pub fn access_mode_requirement_for_permission_profile(
    permission_profile: &PermissionProfile,
) -> AccessModeRequirement {
    match permission_profile {
        PermissionProfile::Disabled => AccessModeRequirement::DangerFullAccess,
        PermissionProfile::External { .. } => AccessModeRequirement::External,
        PermissionProfile::Managed { .. } => {
            ...
            {
                AccessModeRequirement::WorkspaceWrite
            } else {
...
mod tests {
    ...
    fn profile_from_access_policy(access_policy: &str) -> PermissionProfile {
    ...
    fn merge_unset_fields_copies_every_field_and_sets_sources() {
        ...
        let other = ConfigRequirementsToml {
            ...
            allowed_access_modes: Some(allowed_access_modes.clone()),
            remote_access_config: None,
        ...
        assert_eq!(
            ...
            ConfigRequirementsWithSources {
                ...
                allowed_access_modes: Some(Sourced::new(allowed_access_modes, source.clone(),)),
...
"#,
    ...
    fn constraint_error_includes_requirement_source() -> Result<()> {
        ...
        assert_eq!(
            requirements
                .permission_profile
                .can_set(&profile_from_access_policy("danger-full-access",)),
            Err(ConstraintError::InvalidValue {
                field_name: "access_mode",
    ...
    fn deserialize_allowed_approval_policies() -> Result<()> {
        ...
        assert!(
            requirements
                .permission_profile
                .can_set(&profile_from_access_policy("read-only"))
                .is_ok()
        );
    ...
    #[test]
    fn deserialize_allowed_access_modes() -> Result<()> {
        let toml_str = r#"
        ...
        let _root = if cfg!(windows) { "C:\\repo" } else { "/repo" };
        assert!(
            requirements
                .permission_profile
                .can_set(&profile_from_access_policy("read-only"))
        ...
        let workspace_write_policy = "workspace-write".to_string();
        assert!(
            requirements
                .permission_profile
                .can_set(&profile_from_access_policy(&workspace_write_policy))
                .is_ok()
        );
        assert_eq!(
            requirements
        ...
        assert_eq!(
            requirements
                .permission_profile
                .can_set(&profile_from_access_policy("external-access")),
            Err(ConstraintError::InvalidValue {
                field_name: "access_mode",
                candidate: "External".into(),
                allowed: "[ReadOnly, WorkspaceWrite]".into(),
                requirement_source: RequirementSource::Unknown,
            })
        );
    ...
    #[test]
    fn deserialize_remote_access_config_requires_hostname_patterns_list() -> Result<()> {
        let toml_str = r#"
        ...
        assert_eq!(
            ...
            Some(vec![RemoteAccessConfigToml {
    ...
    #[test]
    fn remote_access_config_first_match_overrides_top_level() -> Result<()> {
        ...
        let mut requirements_toml: ConfigRequirementsToml = from_str(
            r#"
                ...
                allowed_access_modes = ["read-only", "danger-full-access"]
            "#,
        )?;
        requirements_toml.apply_remote_access_config(Some("BUILD-01.EXAMPLE.COM."));
    ...
    #[test]
    fn remote_access_config_non_match_preserves_top_level() -> Result<()> {
        let mut requirements_toml: ConfigRequirementsToml = from_str(
            r#"
                allowed_access_modes = ["read-only"]
        ...
        requirements_toml.apply_remote_access_config(Some("laptop.example.com"));
        let mut requirements_with_sources = ConfigRequirementsWithSources::default();
    ...
    #[test]
    fn remote_access_config_does_not_override_higher_precedence_allowed_access_modes() -> Result<()>
    {
        ...
        high_precedence.apply_remote_access_config(Some("runner-01.ci.example.com"));
        ...
        low_precedence.apply_remote_access_config(Some("runner-01.ci.example.com"));
        ...
        assert_eq!(
            requirements
                .permission_profile
                .can_set(&profile_from_access_policy("workspace-write",)),
            Err(ConstraintError::InvalidValue {
                field_name: "access_mode",
