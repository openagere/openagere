use agere_utils_cli::ApprovalModeCliArg;
...
pub struct Cli {
    ...
    // Internal controls set by the top-level `openagere resume` subcommand.
    // These are not exposed as user flags on the base `agere` command.
    #[clap(skip)]
    ...
    /// top-level `openagere resume <SESSION_ID>` wrapper; not exposed as a public flag.
    #[clap(skip)]
    ...
    // Internal controls set by the top-level `agere fork` subcommand.
    // These are not exposed as user flags on the base `agere` command.
    #[clap(skip)]
    ...
    /// top-level `agere fork <SESSION_ID>` wrapper; not exposed as a public flag.
    #[clap(skip)]
    ...
    /// Internal: open the provider wizard immediately after startup.
    #[clap(skip)]
    pub open_provider_on_startup: bool,
...
fn mark_tui_args(cmd: clap::Command) -> clap::Command {
    cmd.mut_arg("dangerously_bypass_approvals", |arg| {
