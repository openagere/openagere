...
#[cfg(any(not(debug_assertions), test))]
use agere_install_context::StandalonePlatform;

/// Update action the CLI should perform after the TUI exits.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum UpdateAction {
    /// Update via `npm install -g openagere@latest`.
    NpmGlobalLatest,
    /// Update via `bun install -g openagere@latest`.
    BunGlobalLatest,
    /// Update via `brew upgrade agere`.
    BrewUpgrade,
    /// Update via `curl -fsSL https://openagere.com/install.sh | sh`.
    StandaloneUnix,
    /// Update via `irm https://openagere.com/install.ps1|iex`.
    StandaloneWindows,
}
...
impl UpdateAction {
    ...
    pub fn command_args(self) -> (&'static str, &'static [&'static str]) {
        match self {
            UpdateAction::NpmGlobalLatest => ("npm", &["install", "-g", "openagere"]),
            UpdateAction::BunGlobalLatest => ("bun", &["install", "-g", "openagere"]),
            UpdateAction::BrewUpgrade => ("brew", &["upgrade", "--cask", "agere"]),
            UpdateAction::StandaloneUnix => (
                "sh",
                &["-c", "curl -fsSL https://openagere.com/install.sh | sh"],
            ),
            UpdateAction::StandaloneWindows => (
                "powershell",
                &["-c", "irm https://openagere.com/install.ps1|iex"],
            ),
        }
    }
    ...
    fn maps_install_context_to_update_action() {
        ...
        assert_eq!(
            UpdateAction::from_install_context(&InstallContext::Standalone {
                ...
                resources_dir: Some(native_release_dir.join("agere-resources")),
            }),
