...
pub(crate) struct TrustDirectoryWidget {
    ...
    pub show_windows_access_restriction_hint: bool,
    ...
    fn render_ref(&self, area: Rect, buf: &mut Buffer) {
        ...
        column.push(
            Line::from(vec![
                "Press ".dim(),
                keys::CONFIRM[0].into(),
                if self.show_windows_access_restriction_hint {
                    " to continue and configure access restrictions...".dim()
                } else {
                    " to continue".dim()
                },
            ])
...
impl TrustDirectoryWidget {
    fn handle_trust(&mut self) {
        ...
        if let Err(e) = set_project_trust_level(&self.agere_home, &target, TrustLevel::Trusted) {
...
mod tests {
    ...
    fn release_event_does_not_change_selection() {
        let agere_home = TempDir::new().expect("temp home");
        let mut widget = TrustDirectoryWidget {
            agere_home: agere_home.path().to_path_buf(),
            cwd: PathBuf::from("."),
            trust_target: PathBuf::from("."),
            show_windows_access_restriction_hint: false,
            should_quit: false,
            selection: None,
