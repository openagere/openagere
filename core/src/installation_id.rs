...
pub async fn resolve_installation_id(agere_home: &AbsolutePathBuf) -> Result<String> {
    let path = agere_home.join(INSTALLATION_ID_FILENAME);
    fs::create_dir_all(agere_home).await?;
...
mod tests {
    ...
    async fn resolve_installation_id_generates_and_persists_uuid() {
        ...
        let agere_home_abs = agere_home.path().abs();
        let persisted_path = agere_home.path().join(INSTALLATION_ID_FILENAME);

        let installation_id = resolve_installation_id(&agere_home_abs)
            .await
    ...
    async fn resolve_installation_id_reuses_existing_uuid() {
        ...
        std::fs::write(
            agere_home.path().join(INSTALLATION_ID_FILENAME),
            existing.clone(),
        )
        .expect("write installation id");

        let resolved = resolve_installation_id(&agere_home_abs)
            .await
            .expect("resolve installation id");
    ...
    async fn resolve_installation_id_rewrites_invalid_file_contents() {
        ...
        assert_eq!(
            std::fs::read_to_string(agere_home.path().join(INSTALLATION_ID_FILENAME))
