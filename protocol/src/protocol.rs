//! Defines the protocol for a Agere session between a client and an agent.
//!
...
use crate::config_types::WindowsExecutionRestrictionLevel;
...
pub use crate::permissions::FileSystemAccessEntry;
pub use crate::permissions::FileSystemAccessLevel;
pub use crate::permissions::FileSystemAccessMode;
pub use crate::permissions::FileSystemAccessPolicy;
pub use crate::permissions::FileSystemPath;
pub use crate::permissions::FileSystemSpecialPath;
pub use crate::permissions::NetworkAccessPolicy;
...
pub const USER_MESSAGE_BEGIN: &str = "## My request for Agere:";
...
pub enum Op {
    ...
    /// (cwd/approval/access-mode/model/etc.) for each turn.
    UserInput {
    ...
    UserInputWithTurnContext {
        ...
        /// Updated `cwd` for execution/tool calls.
        #[serde(skip_serializing_if = "Option::is_none")]
        cwd: Option<PathBuf>,
        ...
        /// Updated Windows execution restriction level for tool execution.
        #[serde(skip_serializing_if = "Option::is_none")]
        windows_execution_restriction_level: Option<WindowsExecutionRestrictionLevel>,
        ...
        /// Updated model provider id. Resolved to a `ModelProviderInfo` in core and applied
        /// atomically with `model` in submission order.
        #[serde(default, skip_serializing_if = "Option::is_none")]
    ...
    /// for a turn of a [`crate::agere_thread::AgereThread`].
    UserTurn {
        /// User input items, see `InputItem`
        items: Vec<UserInput>,

        /// `cwd` to use for tool calls such as `local_shell`.
        cwd: PathBuf,
    ...
    /// This is a local-only operation handled by agere-core; it does not
    /// involve the model.
    ...
    /// Request Agere to drop the last N user turns from in-memory context.
    ///
    ...
    /// Request to shut down agere instance.
    Shutdown,
...
/// running the command proposed by Agere.
#[derive(
    Debug,
    Clone,
    Copy,
...
pub enum AskForApproval {
    ...
    /// run under the active managed access policy (network disabled and writes
    /// confined to allowed paths). If the command still fails, it may be
    /// escalated so the user can approve a less restricted attempt.
...
impl GranularApprovalConfig {
    pub const fn allows_access_approval(self) -> bool {
...
/// privileges of the agent (e.g. `.agere`, `.git`, notably `.git/hooks`) under
...
impl FromStr for FileSystemAccessPolicy {
...
/// Agere errors that we expose to clients.
...
impl ErrorEvent {
    /// Whether this error should mark the current turn as failed when replaying history.
    pub fn affects_turn_status(&self) -> bool {
        self.agere_error_info
            .as_ref()
            .is_none_or(AgereErrorInfo::affects_turn_status)
    }
}
...
impl SessionSource {
    ...
    pub fn restriction_product(&self) -> Option<Product> {
        match self {
            ...
            | SessionSource::Exec
            | SessionSource::Mcp
            | SessionSource::Unknown => Some(Product::Agere),
...
pub struct TurnContextItem {
    ...
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub file_system_access_policy: Option<FileSystemAccessPolicy>,
    pub model: String,
    #[serde(skip_serializing_if = "Option::is_none")]
...
impl TurnContextItem {
    pub fn permission_profile(&self) -> PermissionProfile {
        self.permission_profile.clone().unwrap_or_default()
    }
}
    ...
    pub fn token_budget(&self) -> usize {
        match self {
            TruncationPolicy::Bytes(bytes) => {
                usize::try_from(agere_utils_common::approx_tokens_from_byte_count(*bytes))
                    .unwrap_or(usize::MAX)
            }
    ...
    pub fn byte_budget(&self) -> usize {
        match self {
            ...
            TruncationPolicy::Tokens(tokens) => {
                agere_utils_common::approx_bytes_for_tokens(*tokens)
            }
        }
    }
}
...
impl<'de> Deserialize<'de> for SessionConfiguredEvent {
        ...
        D: serde::Deserializer<'de>,
    {
        ...
        struct Wire {
            ...
            // Older rollouts had `access_policy` (now removed). serde ignores unknown fields,
            // so old persisted data with that field is harmless.
            permission_profile: Option<PermissionProfile>,
            cwd: AbsolutePathBuf,
        ...
        let Some(permission_profile) = wire.permission_profile else {
...
mod tests {
    ...
    #[test]
    fn session_source_restriction_product_defaults_non_subagent_sources_to_agere() {
        assert_eq!(
    ...
    fn session_source_restriction_product_maps_custom_sources_to_products() {
        ...
        assert_eq!(
            SessionSource::Custom("agere".to_string()).restriction_product(),
            Some(Product::Agere)
        );
        assert_eq!(
    ...
    #[test]
    fn product_deserializes_codex_as_agere() {
        assert_eq!(
            serde_json::from_str::<Product>("\"CODEX\"").expect("CODEX product alias"),
            Product::Agere
        );
    }
    ...
    fn session_source_matches_product_restriction() {
        ...
        assert!(
            !SessionSource::Custom("chatgpt".to_string())
                .matches_product_restriction(&[Product::Agere])
        );
        assert!(SessionSource::VSCode.matches_product_restriction(&[Product::Agere]));
        assert!(
    ...
    fn restricted_file_system_policy_reports_full_access_from_root_entries() {
        let read_only = FileSystemAccessPolicy::restricted(vec![FileSystemAccessEntry {
        ...
        let writable = FileSystemAccessPolicy::restricted(vec![FileSystemAccessEntry {
            path: FileSystemPath::Special {
                value: FileSystemSpecialPath::Root,
            },
    ...
    fn restricted_file_system_policy_treats_root_with_carveouts_as_scoped_access() {
        ...
        let canonical_cwd =
            agere_utils_fs::canonicalize_preserving_symlinks(cwd.path()).expect("canonicalize cwd");
        ...
        let expected_blocked = AbsolutePathBuf::from_absolute_path(
            agere_utils_fs::canonicalize_preserving_symlinks(cwd.path())
    ...
    fn restricted_file_system_policy_derives_effective_paths() {
        ...
        std::fs::create_dir_all(cwd.path().join(".openagere")).expect("create .agere");
        let canonical_cwd =
            agere_utils_fs::canonicalize_preserving_symlinks(cwd.path()).expect("canonicalize cwd");
        let cwd_absolute =
        ...
        let expected_agere = AbsolutePathBuf::from_absolute_path(canonical_cwd.join(".openagere"))
            .expect("canonical .agere");
        let policy = FileSystemAccessPolicy::restricted(vec![
            FileSystemAccessEntry {
                path: FileSystemPath::Special {
        ...
        assert!(
            writable_roots[0]
                .read_only_subpaths
                .iter()
                .any(|path| path.as_path() == expected_agere.as_path())
        );
    }
    ...
    fn restricted_file_system_policy_treats_read_entries_as_read_only_subpaths() {
        ...
        let expected_dot_agere =
            AbsolutePathBuf::from_absolute_path(canonical_cwd.join(".openagere"))
                .expect("canonical .agere");
        let policy = FileSystemAccessPolicy::restricted(vec![
            FileSystemAccessEntry {
                path: FileSystemPath::Special {
        ...
        assert_eq!(
            ...
            vec![
                (
                    canonical_cwd,
                    vec![
                        expected_dot_agere.to_path_buf(),
    ...
    fn turn_context_item_deserializes_without_network() -> Result<()> {
        let item: TurnContextItem = serde_json::from_value(json!({
            ...
            "access_policy": { "type": "danger-full-access" },
            "model": "gpt-5",
            "summary": "auto",
        }))?;
    ...
    fn turn_context_item_serializes_network_when_present() -> Result<()> {
        let item = TurnContextItem {
            ...
            file_system_access_policy: Some(FileSystemAccessPolicy::restricted(vec![
                FileSystemAccessEntry {
        ...
        assert_eq!(
            value["file_system_access_policy"],
            json!({
                "kind": "restricted",
                "entries": [{
                    "path": {
    ...
    fn serialize_event() -> Result<()> {
        ...
        let event = Event {
            ...
            msg: EventMsg::SessionConfigured(SessionConfiguredEvent {
                ...
                forked_from_id: None,
                thread_name: None,
                model: "agere-mini-latest".to_string(),
    ...
    #[test]
    fn user_input_with_turn_context_round_trips_model_provider() {
        ...
        assert_eq!(value["type"], "user_input_with_turn_context");
        assert_eq!(value["model_provider"], "openai");

        let parsed: Op = serde_json::from_value(value).expect("deserialize op");
        match parsed {
        ...
        // Older serialized ops without the field still deserialize (serde default).
        let without_field: Op = serde_json::from_value(json!({
            "type": "user_input_with_turn_context",
            "items": [],
        }))
        .expect("deserialize op without model_provider");
        match without_field {
            Op::UserInputWithTurnContext { model_provider, .. } => {
