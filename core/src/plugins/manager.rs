...
use agere_config::types::PluginConfig;
use agere_core_plugins::OPENAGERE_CURATED_MARKETPLACE_NAME;
use agere_core_plugins::installed_marketplaces::installed_marketplace_roots_from_layer_stack;
use agere_core_plugins::loader::configured_curated_plugin_ids_from_agere_home;
use agere_core_plugins::loader::curated_plugin_cache_version;
use agere_core_plugins::loader::installed_plugin_telemetry_metadata;
use agere_core_plugins::loader::load_plugin_apps;
use agere_core_plugins::loader::load_plugin_mcp_servers;
use agere_core_plugins::loader::load_plugin_skills;
use agere_core_plugins::loader::load_plugins_from_layer_stack;
use agere_core_plugins::loader::log_plugin_load_errors;
use agere_core_plugins::loader::materialize_marketplace_plugin_source;
use agere_core_plugins::loader::plugin_telemetry_metadata_from_root;
use agere_core_plugins::loader::refresh_curated_plugin_cache;
...
use agere_core_plugins::manifest::load_plugin_manifest;
use agere_core_plugins::marketplace::MarketplaceError;
use agere_core_plugins::marketplace::MarketplaceInterface;
use agere_core_plugins::marketplace::MarketplaceListError;
use agere_core_plugins::marketplace::MarketplacePluginAuthPolicy;
use agere_core_plugins::marketplace::MarketplacePluginPolicy;
use agere_core_plugins::marketplace::MarketplacePluginSource;
use agere_core_plugins::marketplace::ResolvedMarketplacePlugin;
use agere_core_plugins::marketplace::find_installable_marketplace_plugin;
use agere_core_plugins::marketplace::find_marketplace_plugin;
use agere_core_plugins::marketplace::list_marketplaces;
use agere_core_plugins::marketplace::load_marketplace;
use agere_core_plugins::marketplace::plugin_interface_with_marketplace_category;
use agere_core_plugins::marketplace_upgrade::ConfiguredMarketplaceUpgradeError;
use agere_core_plugins::marketplace_upgrade::ConfiguredMarketplaceUpgradeOutcome;
...
use agere_core_plugins::startup_sync::has_local_curated_plugins_snapshot;
use agere_core_plugins::startup_sync::read_curated_plugins_sha;
use agere_core_plugins::startup_sync::sync_openai_plugins_repo;
use agere_core_plugins::store::PluginInstallResult as StorePluginInstallResult;
use agere_core_plugins::store::PluginStore;
use agere_core_plugins::store::PluginStoreError;
...
use agere_plugin::prompt_safe_plugin_description;
...
static CURATED_REPO_SYNC_STATE: AtomicU8 = AtomicU8::new(CURATED_REPO_SYNC_IDLE);
const CURATED_REPO_SYNC_IDLE: u8 = 0;
const CURATED_REPO_SYNC_IN_FLIGHT: u8 = 1;
const CURATED_REPO_SYNC_COMPLETE: u8 = 2;
...
impl From<RemotePluginFetchError> for PluginRemoteSyncError {
    fn from(value: RemotePluginFetchError) -> Self {
        match value {
            ...
            RemotePluginFetchError::Unsupported(msg) => Self::Unsupported(msg),
        }
    }
}
...
impl PluginsManager {
    pub fn new(agere_home: PathBuf) -> Self {
        Self::new_with_restriction_product(agere_home, Some(Product::Agere))
    }
    ...
    pub fn new_with_restriction_product(
        ...
        // Product restrictions are enforced at marketplace admission time for a given AGERE_HOME:
        ...
        // contents of that AGERE_HOME and does not re-filter configured plugins by product, so
        // already-admitted plugins may continue exposing MCP servers/tools from shared local state.
        //
        // This assumes a single AGERE_HOME is only used by one product.
        Self {
            agere_home: agere_home.clone(),
            store: PluginStore::new(agere_home),
    ...
    pub async fn featured_plugin_ids_for_config(
        ...
        let featured_plugin_ids =
            agere_core_plugins::remote_legacy::fetch_remote_featured_plugin_ids(
    ...
    pub async fn install_plugin(
        ...
        let resolved = self.normalize_official_curated_plugin(
    ...
    pub async fn install_plugin_with_remote_sync(
        ...
        agere_core_plugins::remote_legacy::enable_remote_plugin(
            &remote_plugin_service_config(config),
            auth,
            &plugin_id,
        )
    ...
    async fn install_resolved_plugin(
        ...
        let plugin_version =
            if resolved.plugin_id.marketplace_name == OPENAGERE_CURATED_MARKETPLACE_NAME {
                let curated_plugin_version = read_curated_plugins_sha(self.agere_home.as_path())
                    .ok_or_else(|| {
        ...
        let agere_home = self.agere_home.clone();
        let result: StorePluginInstallResult = tokio::task::spawn_blocking(move || {
            let materialized =
                materialize_marketplace_plugin_source(agere_home.as_path(), &resolved.source)
        ...
        ConfigEditsBuilder::new(&self.agere_home)
    ...
    pub async fn uninstall_plugin_with_remote_sync(
        ...
        agere_core_plugins::remote_legacy::uninstall_remote_plugin(
            &remote_plugin_service_config(config),
            auth,
            &plugin_key,
        )
    ...
    async fn uninstall_plugin_id(&self, plugin_id: PluginId) -> Result<(), PluginUninstallError> {
        let plugin_telemetry = if self.store.active_plugin_root(&plugin_id).is_some() {
            Some(installed_plugin_telemetry_metadata(self.agere_home.as_path(), &plugin_id).await)
        } else {
            None
        };
        let store = self.store.clone();
    ...
    pub async fn sync_plugins_from_remote(
        ...
        let remote_plugins = agere_core_plugins::remote_legacy::fetch_remote_plugin_status(
            &remote_plugin_service_config(config),
            auth,
        )
        .await
        ...
        let curated_marketplace_root = curated_plugins_repo_path(self.agere_home.as_path());
    ...
    pub fn list_marketplaces_for_config(
        ...
        let marketplaces = marketplace_outcome
            ...
            .filter_map(|marketplace| {
                let is_official_curated_marketplace =
                    self.is_official_curated_marketplace_path(marketplace.path.as_path());
                let marketplace_name = if is_official_curated_marketplace {
                    OPENAGERE_CURATED_MARKETPLACE_NAME.to_string()
                } else {
                ...
                (!plugins.is_empty()).then_some(ConfiguredMarketplace {
                    ...
                    interface: if is_official_curated_marketplace {
                        None
                    } else {
    ...
    fn is_official_curated_marketplace_path(&self, marketplace_path: &std::path::Path) -> bool {
        marketplace_path
            == curated_plugins_repo_path(self.agere_home.as_path())
                .join(".agents/plugins/marketplace.json")
                .as_path()
    }

    fn normalize_official_curated_plugin(
        &self,
        mut plugin: ResolvedMarketplacePlugin,
        marketplace_path: &AbsolutePathBuf,
    ) -> Result<ResolvedMarketplacePlugin, MarketplaceError> {
        if self.is_official_curated_marketplace_path(marketplace_path.as_path()) {
            plugin.plugin_id = PluginId::new(
                plugin.plugin_id.plugin_name.clone(),
                OPENAGERE_CURATED_MARKETPLACE_NAME.to_string(),
            )
    ...
    pub async fn read_plugin_for_config(
        ...
        let plugin = self.normalize_official_curated_plugin(
            find_marketplace_plugin(&request.marketplace_path, &request.plugin_name)?,
            &request.marketplace_path,
        )?;
        if !self.restriction_product_matches(plugin.policy.products.as_deref()) {
    ...
    pub(crate) async fn read_plugin_detail_for_marketplace_plugin(
        ...
        let source_path =
            if matches!(plugin.source, MarketplacePluginSource::Git { .. }) && plugin.installed {
                ...
                let materialized = tokio::task::spawn_blocking(move || {
                    materialize_marketplace_plugin_source(agere_home.as_path(), &source)
                })
                .await
                .map_err(|err| {
                    MarketplaceError::InvalidPlugin(format!(
        ...
        let resolved_skills = load_plugin_skills(
            ...
            &manifest.paths,
            self.restriction_product,
            &agere_core_skills::config_rules::skill_config_rules_from_stack(
                &config.config_layer_stack,
            ),
        )
        .await;
    ...
    pub fn maybe_start_curated_repo_sync(self: &Arc<Self>) {
        self.start_curated_repo_sync();
    }

    fn start_curated_repo_sync(self: &Arc<Self>) {
        loop {
            let state = CURATED_REPO_SYNC_STATE.load(Ordering::SeqCst);
            if state == CURATED_REPO_SYNC_IN_FLIGHT {
                return;
            }
            if state == CURATED_REPO_SYNC_COMPLETE
                && has_local_curated_plugins_snapshot(self.agere_home.as_path())
            {
                return;
            }
            if CURATED_REPO_SYNC_STATE
                .compare_exchange(
                    state,
                    CURATED_REPO_SYNC_IN_FLIGHT,
                    Ordering::SeqCst,
                    Ordering::SeqCst,
                )
                .is_ok()
        ...
        if let Err(err) = std::thread::Builder::new()
            ...
            .spawn(
                move || match sync_openai_plugins_repo(agere_home.as_path()) {
                    Ok(curated_plugin_version) => {
                        let configured_curated_plugin_ids =
                            configured_curated_plugin_ids_from_agere_home(agere_home.as_path());
                        match refresh_curated_plugin_cache(
                            ...
                            Ok(cache_refreshed) => {
                                ...
                                CURATED_REPO_SYNC_STATE
                                    .store(CURATED_REPO_SYNC_COMPLETE, Ordering::SeqCst);
                            }
                            Err(err) => {
                                manager.clear_cache();
                                CURATED_REPO_SYNC_STATE
                                    .store(CURATED_REPO_SYNC_IDLE, Ordering::SeqCst);
                                warn!("failed to refresh curated plugin cache after sync: {err}");
                            }
                        }
                    }
                    Err(err) => {
                        CURATED_REPO_SYNC_STATE.store(CURATED_REPO_SYNC_IDLE, Ordering::SeqCst);
    ...
    fn run_non_curated_plugin_cache_refresh_loop(self: Arc<Self>) {
        loop {
            ...
            let refresh_result = match request.mode {
                NonCuratedCacheRefreshMode::IfVersionChanged => {
                    refresh_non_curated_plugin_cache(self.agere_home.as_path(), &request.roots)
                }
    ...
    fn marketplace_roots(
        ...
        let curated_repo_root = curated_plugins_repo_path(self.agere_home.as_path());
