...
use agere_core::config::AgentRoleConfig;
...
const INHERITED_MODEL: &str = "gpt-5.3";
...
async fn wait_for_spawned_thread_id(test: &TestAgere) -> Result<String> {
...
async fn setup_turn_one_with_custom_spawned_child(
    ...
    configure_test: impl FnOnce(
        core_test_support::test_agere::TestAgereBuilder,
    ) -> core_test_support::test_agere::TestAgereBuilder,
) -> Result<(TestAgere, String)> {
    ...
    let mut builder = configure_test(test_agere().with_config(|config| {
        config
            .features
    ...
    let child_snapshot = spawn_child_and_capture_snapshot(
        ...
        |builder| {
            builder.with_config(|config| {
                let role_path = config.agere_home.join("custom-role.toml");
                std::fs::write(
                    &role_path,
                    format!(
