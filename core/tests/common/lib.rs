...
use agere_utils_cargo_bin::CargoBinError;
...
use agere_config::NetworkRequirementsToml;
...
pub use agere_utils_fs::test_support::PathBufExt;
pub use agere_utils_fs::test_support::PathExt;
...
fn enable_deterministic_unified_exec_process_ids_for_tests() {
    agere_core::test_support::set_thread_manager_test_mode(/*enabled*/ true);
    agere_core::test_support::set_deterministic_process_ids(/*enabled*/ true);
}

#[ctor]
fn configure_arg0_dispatch_for_test_binaries() {
    let _ = TEST_ARG0_PATH_ENTRY.get_or_init(agere_arg0::arg0_dispatch);
}
...
fn configure_insta_workspace_root_for_snapshot_tests() {
    ...
    let workspace_root = agere_utils_cargo_bin::repo_root()
        .ok()
        .map(|root| root.join("agere-rs"));
...
pub fn test_tmp_path() -> AbsolutePathBuf {
    test_absolute_path_with_windows("/tmp", Some(r"C:\Users\agere\AppData\Local\Temp"))
}
...
/// avoids clobbering a developer's real `~/.openagere`.
pub async fn load_default_config_for_test(agere_home: &TempDir) -> Config {
...
fn default_test_overrides() -> ConfigOverrides {
    ConfigOverrides {
        agere_linux_exe: Some(
            find_agere_linux_exe().expect("should find binary for agere-linux-helper"),
        ),
...
#[cfg(target_os = "linux")]
pub fn find_agere_linux_exe() -> Result<PathBuf, CargoBinError> {
    if let Some(path) = TEST_ARG0_PATH_ENTRY
        .get()
        .and_then(Option::as_ref)
        .and_then(|path_entry| path_entry.paths().agere_linux_exe.clone())
    {
        return Ok(path);
    }
    ...
    agere_utils_cargo_bin::cargo_bin("agere-linux-helper")
}
...
pub async fn wait_for_event<F>(
    agere: &AgereThread,
    predicate: F,
) -> agere_protocol::protocol::EventMsg
where
    F: FnMut(&agere_protocol::protocol::EventMsg) -> bool,
{
    use tokio::time::Duration;
    wait_for_event_with_timeout(agere, predicate, Duration::from_secs(1)).await
}

pub async fn wait_for_event_match<T, F>(agere: &AgereThread, matcher: F) -> T
where
    F: Fn(&agere_protocol::protocol::EventMsg) -> Option<T>,
{
    let ev = wait_for_event(agere, |ev| matcher(ev).is_some()).await;
    ...
    F: FnMut(&agere_protocol::protocol::EventMsg) -> bool,
{
    ...
    loop {
        ...
        let ev = timeout(wait_time.max(Duration::from_secs(10)), agere.next_event())
            .await
...
pub fn test_execution_marker_env_var() -> &'static str {
    agere_core::spawn::AGERE_EXECUTION_ENV_VAR
}

pub fn test_network_disabled_env_var() -> &'static str {
    agere_core::spawn::AGERE_NETWORK_DISABLED_ENV_VAR
}

const REMOTE_ENV_ENV_VAR: &str = "AGERE_TEST_REMOTE_ENV";
...
pub fn format_with_current_shell(command: &str) -> Vec<String> {
    agere_core::shell::default_user_shell().derive_exec_args(command, /*use_login_shell*/ true)
}
...
pub fn format_with_current_shell_non_login(command: &str) -> Vec<String> {
    agere_core::shell::default_user_shell()
...
pub fn stdio_server_bin() -> Result<String, CargoBinError> {
    agere_utils_cargo_bin::cargo_bin("test_stdio_server").map(|p| p.to_string_lossy().to_string())
}
...
#[macro_export]
macro_rules! skip_if_seatbelt_execution_marker {
    () => {{
        if ::std::env::var($crate::test_execution_marker_env_var())
            == ::core::result::Result::Ok("seatbelt".to_string())
        {
            eprintln!(
                "{} is set to 'seatbelt', skipping test.",
                $crate::test_execution_marker_env_var()
            );
            return;
        }
    }};
...
macro_rules! skip_if_no_network {
    () => {{
        if ::std::env::var($crate::test_network_disabled_env_var()).is_ok() {
            println!(
                "Skipping test because network is disabled via {}.",
                $crate::test_network_disabled_env_var()
            );
            return;
        }
    }};
...
#[macro_export]
macro_rules! agere_linux_exe_or_skip {
    () => {{
        #[cfg(target_os = "linux")]
        {
            match $crate::find_agere_linux_exe() {
                Ok(path) => Some(path),
                Err(err) => {
                    eprintln!("agere-linux-helper binary not available, skipping test: {err}");
                    return;
                }
            }
        }
