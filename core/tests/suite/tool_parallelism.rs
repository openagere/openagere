...
async fn run_turn(test: &TestAgere, prompt: &str) -> anyhow::Result<()> {
    let session_model = test.session_configured.model.clone();
    let permission_profile = turn_permission_fields(PermissionProfile::Disabled, test.cwd.path());
...
async fn run_turn_and_measure(test: &TestAgere, prompt: &str) -> anyhow::Result<Duration> {
...
async fn build_agere_with_test_tool(server: &wiremock::MockServer) -> anyhow::Result<TestAgere> {
    let mut builder = test_agere().with_model("test-gpt-5.3");
...
async fn read_file_tools_run_in_parallel() -> anyhow::Result<()> {
    ...
    let test = build_agere_with_test_tool(&server).await?;
