...
const X_AGERE_TURN_STATE_HEADER: &str = "x-agere-turn-state";
...
async fn connect_websocket(
    ...
    // If a Agere-specific CA bundle is configured, build an explicit rustls connector so this
...
mod tests {
    ...
    fn parse_wrapped_websocket_error_event_maps_to_transport_http() {
        ...
        assert_eq!(
            headers
                .get("x-agere-primary-used-percent")
                .and_then(|value| value.to_str().ok()),
            Some("100.0")
        );
        assert_eq!(
            headers
                .get("x-agere-primary-window-minutes")
                .and_then(|value| value.to_str().ok()),
            Some("15")
        );
