...
impl TraceReducer {
    ...
    pub(super) fn start_tool_call(
        ...
        let thread_id = self.tool_thread_id(thread_id, agere_turn_id.as_deref())?;
        self.validate_tool_turn(&thread_id, agere_turn_id.as_deref())?;
        ...
        self.rollout.tool_calls.insert(
            ...
            ToolCall {
                ...
                thread_id,
                started_by_agere_turn_id: agere_turn_id,
    ...
    fn tool_thread_id(
        &self,
        thread_id: Option<String>,
        agere_turn_id: Option<&str>,
    ) -> Result<String> {
        if let Some(thread_id) = thread_id {
            return Ok(thread_id);
        }
        let Some(agere_turn_id) = agere_turn_id else {
            bail!("tool call start did not include thread or Agere turn context");
        };
        self.rollout
            .agere_turns
            .get(agere_turn_id)
            ...
            .with_context(|| {
                format!("tool call start referenced unknown Agere turn {agere_turn_id}")
            })
    }

    fn validate_tool_turn(&self, thread_id: &str, agere_turn_id: Option<&str>) -> Result<()> {
        if !self.rollout.threads.contains_key(thread_id) {
            bail!("tool call start referenced unknown thread {thread_id}");
        }
        if let Some(agere_turn_id) = agere_turn_id {
            let Some(turn) = self.rollout.agere_turns.get(agere_turn_id) else {
                bail!("tool call start referenced unknown Agere turn {agere_turn_id}");
            };
            if turn.thread_id != thread_id {
                bail!(
                    "tool call start used thread {thread_id}, but Agere turn {agere_turn_id} \
                     belongs to {}",
                    turn.thread_id
                );
            }
