...
use agere_config::ConfigLayerSource;
...
fn config_toml_source_path(layer: &ConfigLayerEntry) -> AbsolutePathBuf {
    match &layer.name {
        ...
        ConfigLayerSource::Project { dot_agere_folder } => dot_agere_folder.join(CONFIG_TOML_FILE),
...
mod tests {
    ...
    fn hook_source_for_config_layer_source_discards_source_details() {
        let config_file = test_path_buf("/tmp/.openagere/config.toml").abs();
        let dot_agere_folder = test_path_buf("/tmp/worktree/.agere").abs();
        ...
        assert_eq!(
            super::hook_source_for_config_layer_source(&ConfigLayerSource::Mdm {
                domain: "com.openagere".to_string(),
