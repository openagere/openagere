...
use agere_protocol::config_types::TUI_VISIBLE_COLLABORATION_MODES;
...
fn default_mode_instructions(collaboration_modes_config: CollaborationModesConfig) -> String {
    ...
    format!(
        "Known mode names are {known_mode_names}. {request_user_input_availability} {asking_questions_guidance}"
    )
}
