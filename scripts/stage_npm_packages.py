#!/usr/bin/env python3
"""Stage one or more Openagere npm packages for release."""
...
# Import constants from build_npm_package.py
_SPEC = importlib.util.spec_from_file_location("openagere_build_npm_package", BUILD_SCRIPT)
...
OPENAGERE_PLATFORM_PACKAGES = getattr(_BUILD_MODULE, "OPENAGERE_PLATFORM_PACKAGES", {})
...
def tarball_name_for_package(package: str, version: str) -> str:
    if package in OPENAGERE_PLATFORM_PACKAGES:
        platform = package.removeprefix("openagere-")
        return f"openagere-npm-{platform}-{version}.tgz"
