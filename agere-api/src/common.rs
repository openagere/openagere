...
/// Indicates whether an output item's completion is provisional (turn may
/// produce more content) or final (turn-level completion is confirmed).
#[non_exhaustive]
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum OutputItemCompletion {
    /// The item is complete at the block level, but the turn may continue
    /// and emit additional items. Callers should defer UI/transcript
    /// completion until a flush point confirms the item's final state.
    Provisional,
    /// The item is definitively complete; no further turn-level events
    /// will retroactively change this item's status.
    Final,
}
...
pub enum ResponseEvent {
    ...
    OutputItemDone {
        item: ResponseItem,
        completion: OutputItemCompletion,
    },
...
impl ResponseEvent {
    /// Creates an `OutputItemDone` event with `Final` completion.
    /// Used by Responses API and OpenAI chat providers which do not
    /// have Anthropic's provisional ordering issue.
    pub fn output_item_done(item: ResponseItem) -> Self {
        Self::OutputItemDone {
            item,
            completion: OutputItemCompletion::Final,
        }
    }

    /// Creates an `OutputItemDone` event with `Provisional` completion.
    /// Used only by the Anthropic provider for text block stop events,
    /// where the turn may still produce tool calls after the text.
    pub fn provisional_output_item_done(item: ResponseItem) -> Self {
        Self::OutputItemDone {
            item,
            completion: OutputItemCompletion::Provisional,
        }
    }
}
