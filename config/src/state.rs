...
use agere_app_server_protocol::ConfigLayer;
...
impl LoaderOverrides {
    ...
    pub fn without_managed_config_for_tests() -> Self {
        let base = std::env::temp_dir().join("agere-config-tests");
        Self {
...
impl ConfigLayerEntry {
    ...
    // Get the `.openagere/` folder associated with this config layer, if any.
    pub fn config_folder(&self) -> Option<AbsolutePathBuf> {
        match &self.name {
            ConfigLayerSource::Mdm { .. } => None,
            ConfigLayerSource::System { file } => file.parent(),
            ConfigLayerSource::User { file } => file.parent(),
            ConfigLayerSource::Project { dot_agere_folder } => Some(dot_agere_folder.clone()),
...
fn verify_layer_ordering(layers: &[ConfigLayerEntry]) -> std::io::Result<Option<usize>> {
    ...
    let mut previous_project_dot_agere_folder: Option<&AbsolutePathBuf> = None;
    for (index, layer) in layers.iter().enumerate() {
        ...
        if let ConfigLayerSource::Project {
            dot_agere_folder: current_project_dot_agere_folder,
        } = &layer.name
        {
            if let Some(previous) = previous_project_dot_agere_folder {
                ...
                if previous == current_project_dot_agere_folder
                    || !current_project_dot_agere_folder
                        .as_path()
                        .ancestors()
            ...
            previous_project_dot_agere_folder = Some(current_project_dot_agere_folder);
        }
    }
