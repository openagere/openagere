...
use agere_tools::DiscoverableToolAction;
use agere_tools::DiscoverableToolType;
use agere_tools::TOOL_SUGGEST_PERSIST_ALWAYS_VALUE;
use agere_tools::TOOL_SUGGEST_PERSIST_KEY;
use agere_tools::TOOL_SUGGEST_TOOL_NAME;
use agere_tools::ToolSuggestArgs;
use agere_tools::ToolSuggestResult;
use agere_tools::all_suggested_connectors_picked_up;
use agere_tools::build_tool_suggestion_elicitation_request;
use agere_tools::filter_tool_suggest_discoverable_tools_for_client;
use agere_tools::verified_connector_suggestion_completed;
...
impl ToolHandler for ToolSuggestHandler {
    ...
    async fn handle(&self, invocation: ToolInvocation) -> Result<Self::Output, FunctionCallError> {
        ...
        if args.tool_type == DiscoverableToolType::Plugin
            && turn.app_server_client_name.as_deref() == Some("openagere")
        {
            return Err(FunctionCallError::RespondToModel(
                "plugin tool suggestions are not available in openagere yet".to_string(),
            ));
        }
...
async fn maybe_persist_tool_suggest_disable(
    ...
    if let Err(err) = persist_tool_suggest_disable(&turn.config.agere_home, tool).await {
        warn!(
            error = %err,
            tool_id = tool.id(),
...
async fn verify_tool_suggestion_completed(
    ...
    turn: &crate::session::turn_context::TurnContext,
    tool: &DiscoverableTool,
    auth: Option<&agere_login::AgereAuth>,
) -> bool {
    match tool {
...
async fn refresh_missing_suggested_connectors(
    ...
    match manager.hard_refresh_agere_apps_tools_cache().await {
        Ok(mcp_tools) => {
            let accessible_connectors = connectors::with_app_enabled_state(
                connectors::accessible_connectors_from_mcp_tools(&mcp_tools),
                &turn.config,
        ...
        Err(err) => {
            warn!(
                "failed to refresh agere apps tools cache after tool suggestion for {tool_id}: {err:#}"
            );
            None
        }
    }
