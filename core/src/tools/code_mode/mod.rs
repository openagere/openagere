...
use agere_code_mode::CodeModeNestedToolCall;
use agere_code_mode::CodeModeTurnHost;
use agere_code_mode::RuntimeResponse;
...
use agere_tools::collect_code_mode_tool_definitions;
use agere_utils_output_truncation::TruncationPolicy;
use agere_utils_output_truncation::formatted_truncate_text_content_items_with_policy;
use agere_utils_output_truncation::truncate_function_output_items_with_policy;

pub(crate) use execute_handler::CodeModeExecuteHandler;
use response_adapter::into_function_call_output_content_items;
pub(crate) use wait_handler::CodeModeWaitHandler;

pub(crate) const PUBLIC_TOOL_NAME: &str = agere_code_mode::PUBLIC_TOOL_NAME;
pub(crate) const WAIT_TOOL_NAME: &str = agere_code_mode::WAIT_TOOL_NAME;
pub(crate) const DEFAULT_WAIT_YIELD_TIME_MS: u64 = agere_code_mode::DEFAULT_WAIT_YIELD_TIME_MS;
...
impl CodeModeService {
    ...
    pub(crate) async fn execute(
        &self,
        request: agere_code_mode::ExecuteRequest,
    ...
    pub(crate) async fn wait(
        &self,
        request: agere_code_mode::WaitRequest,
    ) -> Result<agere_code_mode::WaitOutcome, String> {
...
pub(super) async fn build_enabled_tools(
    exec: &ExecContext,
) -> Vec<agere_code_mode::ToolDefinition> {
...
fn tool_kind_for_spec(spec: &ToolSpec) -> agere_code_mode::CodeModeToolKind {
    if matches!(spec, ToolSpec::Freeform(_)) {
        agere_code_mode::CodeModeToolKind::Freeform
    } else {
        agere_code_mode::CodeModeToolKind::Function
    }
}
...
fn tool_kind_for_name(
    ...
    tool_name: &ToolName,
) -> Result<agere_code_mode::CodeModeToolKind, String> {
    spec.as_ref()
