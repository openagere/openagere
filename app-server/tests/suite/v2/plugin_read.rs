...
[plugins."demo-plugin@openagere-curated"]
enabled = true
"#,
    )?;
    write_installed_plugin(&agere_home, "openagere-curated", "demo-plugin")?;
    ...
    assert_eq!(response.plugin.marketplace_name, "openagere-curated");
    assert_eq!(response.plugin.marketplace_path, Some(marketplace_path));
    assert_eq!(response.plugin.summary.id, "demo-plugin@openagere-curated");
...
[plugins."demo-plugin@agere-curated"]
enabled = true
"#,
    )?;
    write_installed_plugin(&agere_home, "agere-curated", "demo-plugin")?;
    ...
    assert_eq!(response.plugin.marketplace_name, "agere-curated");
    assert_eq!(response.plugin.marketplace_path, Some(marketplace_path));
    assert_eq!(response.plugin.summary.id, "demo-plugin@agere-curated");
    assert_eq!(response.plugin.summary.name, "demo-plugin");
    assert_eq!(
        response.plugin.description.as_deref(),
...
async fn plugin_read_accepts_legacy_string_default_prompt() -> Result<()> {
  ...
  "interface": {
    ...
    write_plugins_enabled_config(&agere_home)?;
...
fn write_plugins_enabled_config(agere_home: &TempDir) -> Result<()> {
    std::fs::write(
        agere_home.path().join("config.toml"),
        r#"[features]
plugins = true
