...
/// The easiest way to iterate is to add unit tests and have Agere fix the implementation.
...
/// Tests are at the top to encourage using TDD + Agere to fix the implementation.
mod tests {
    use super::*;
    ...
    fn supports_grep_recursive_current_dir() {
        assert_parsed(
            &vec_str(&["grep", "-R", "AGERE_EXECUTION_ENV_VAR", "-n", "."]),
            vec![ParsedCommand::Search {
                cmd: "grep -R AGERE_EXECUTION_ENV_VAR -n .".to_string(),
                query: Some("AGERE_EXECUTION_ENV_VAR".to_string()),
                path: Some(".".to_string()),
            }],
        );
    }
    ...
    fn supports_grep_recursive_specific_file() {
        assert_parsed(
            &vec_str(&[
                "grep",
                "-R",
                "AGERE_EXECUTION_ENV_VAR",
                "-n",
                "core/src/spawn.rs",
            ]),
            vec![ParsedCommand::Search {
                cmd: "grep -R AGERE_EXECUTION_ENV_VAR -n core/src/spawn.rs".to_string(),
                query: Some("AGERE_EXECUTION_ENV_VAR".to_string()),
    ...
    fn supports_grep_weird_backtick_in_query() {
        assert_parsed(
            &shlex_split_safe("grep -R COD`EX_SCOPE -n"),
            vec![ParsedCommand::Search {
                cmd: "grep -R 'COD`EX_SCOPE' -n".to_string(),
                query: Some("COD`EX_SCOPE".to_string()),
                path: None,
            }],
        );
    }
    ...
    fn supports_cd_and_rg_files() {
        assert_parsed(
            &shlex_split_safe("cd agere-rs && rg --files"),
            vec![ParsedCommand::ListFiles {
                cmd: "rg --files".to_string(),
                path: None,
            }],
    ...
    fn supports_single_string_script_with_cd_and_pipe() {
        let inner = r#"cd /Users/pakrym/code/agere && rg -n "agere_api" agere-rs -S | head -n 50"#;
        assert_parsed(
            &vec_str(&["bash", "-lc", inner]),
            vec![ParsedCommand::Search {
                cmd: "rg -n agere_api agere-rs -S".to_string(),
                query: Some("agere_api".to_string()),
                path: Some("agere-rs".to_string()),
            }],
        );
    }
