...
use agere_apply_patch::AGERE_CORE_APPLY_PATCH_ARG1;
use agere_exec_server::AGERE_FS_HELPER_ARG1;
use agere_utils_fs::find_agere_home;
#[cfg(unix)]
...
#[cfg(unix)]
const EXECVE_WRAPPER_ARG0: &str = "agere-execve-wrapper";
const LOCK_FILENAME: &str = ".lock";
const TOKIO_WORKER_STACK_SIZE_BYTES: usize = 16 * 1024 * 1024;

#[derive(Clone, Debug, Default, Eq, PartialEq)]
pub struct Arg0DispatchPaths {
    /// Stable path to the current Agere executable for child re-execs.
    ///
    ...
    /// instead of the real Agere CLI.
    pub agere_self_exe: Option<PathBuf>,
    /// Always `None` — legacy Linux isolation helper dispatch is unused (no-op).
...
pub fn arg0_dispatch() -> Option<Arg0PathEntryGuard> {
    ...
    if exe_name == EXECVE_WRAPPER_ARG0 {
        ...
        let exit_code = runtime.block_on(
            agere_shell_escalation::run_shell_escalation_execve_wrapper(file, argv),
        );
        match exit_code {
    ...
    if argv1 == AGERE_FS_HELPER_ARG1 {
        agere_exec_server::run_fs_helper_main();
    }
    if argv1 == AGERE_CORE_APPLY_PATCH_ARG1 {
        let patch_arg = args.next().and_then(|s| s.to_str().map(str::to_owned));
        let exit_code = match patch_arg {
            Some(patch_arg) => {
                let mut stdout = std::io::stdout();
                let mut stderr = std::io::stderr();
                let cwd = match agere_utils_fs::AbsolutePathBuf::current_dir() {
                    Ok(cwd) => cwd,
                    Err(_) => std::process::exit(1),
                };
                let runtime = match tokio::runtime::Builder::new_current_thread()
                ...
                match runtime.block_on(agere_apply_patch::apply_patch(
                    &patch_arg,
                    &cwd,
                    &mut stdout,
                    &mut stderr,
                    agere_exec_server::LOCAL_FS.as_ref(),
                )) {
                    Ok(()) => 0,
                    Err(_) => 1,
                }
            ...
            None => {
                eprintln!("Error: {AGERE_CORE_APPLY_PATCH_ARG1} requires a UTF-8 PATCH argument.");
                1
            }
        };
    ...
    match prepend_path_entry_for_agere_aliases() {
        Ok(path_entry) => Some(path_entry),
        Err(err) => {
            // It is possible that Agere will proceed successfully even if
...
/// While we want to deploy the Agere CLI as a single executable for simplicity,
/// we also want to expose some of its functionality as distinct CLIs, so we use
/// the "arg0 trick" to determine which CLI to dispatch. This effectively allows
/// us to simulate deploying multiple executables as a single binary on Mac and
/// Linux (but not Windows).
///
/// 1.  Load `.env` values from `~/.openagere/.env` before creating any threads.
...
///     contains the helper executable paths.
///
    ...
    Fut: Future<Output = anyhow::Result<()>>,
{
    let paths = Arg0DispatchPaths {
        agere_self_exe: current_exe.clone(),
...
const ILLEGAL_ENV_VAR_PREFIX: &str = "AGERE_";

/// Load env vars from ~/.openagere/.env.
///
/// Security: Do not allow `.env` files to create or modify any variables
/// with names starting with `AGERE_`.
fn load_dotenv() {
    if let Ok(agere_home) = find_agere_home()
        && let Ok(iter) = dotenvy::from_path_iter(agere_home.join(".env"))
    {
        set_filtered(iter);
    }
}

/// Helper to set vars from a dotenvy iterator while filtering out `AGERE_` keys.
...
///   with the hidden `--agere-run-as-apply-patch` flag.
///
...
pub fn prepend_path_entry_for_agere_aliases() -> std::io::Result<Arg0PathEntryGuard> {
    let agere_home = find_agere_home()?;
    #[cfg(not(debug_assertions))]
    {
        // Guard against placing helpers in system temp directories outside debug builds.
        let temp_root = std::env::temp_dir();
        if agere_home.starts_with(&temp_root) {
            return Err(std::io::Error::new(
                std::io::ErrorKind::InvalidInput,
                format!(
                    "Refusing to create helper binaries under temporary dir {temp_root:?} (agere_home: {agere_home:?})"
                ),
            ));
        }
    }
    ...
    // Use a AGERE_HOME-scoped temp root to avoid cluttering the top-level directory.
    let temp_root = agere_home.join("tmp").join("arg0");
    ...
    for filename in &[
        ...
        #[cfg(windows)]
        {
            ...
            let exe = exe.display();
            let flag = AGERE_CORE_APPLY_PATCH_ARG1;
            std::fs::write(
                &batch_script,
                format!(
                    r#"@echo off
...
"#,
    ...
    let paths = Arg0DispatchPaths {
        agere_self_exe: std::env::current_exe().ok(),
        agere_linux_exe: None,
...
mod tests {
    ...
    fn run_main_with_arg0_guard_keeps_aliases_alive_until_main_returns() -> anyhow::Result<()> {
        ...
        let alias_path = temp_dir.path().join("agere-helper-alias");
        ...
        let path_entry = Arg0PathEntryGuard::new(
            ...
            lock_file,
            Arg0DispatchPaths {
                agere_self_exe: Some(PathBuf::from("/usr/bin/agere")),
                agere_linux_exe: None,
        ...
        super::build_runtime()?.block_on(run_main_with_arg0_guard(
            ...
            Some(PathBuf::from("/usr/bin/agere")),
            |paths| async move {
