...
use agere_execpolicy::AmendError;
use agere_execpolicy::Decision;
use agere_execpolicy::Error as ExecPolicyRuleError;
use agere_execpolicy::Evaluation;
use agere_execpolicy::MatchOptions;
use agere_execpolicy::NetworkRuleProtocol;
use agere_execpolicy::Policy;
use agere_execpolicy::PolicyParser;
use agere_execpolicy::RuleMatch;
use agere_execpolicy::blocking_append_allow_prefix_rule;
use agere_execpolicy::blocking_append_network_rule;
...
use agere_shell_command::is_dangerous_command::command_might_be_dangerous;
use agere_shell_command::is_safe_command::is_known_safe_command;
use thiserror::Error;
use tokio::fs;
...
use agere_shell_command::bash::parse_shell_lc_plain_commands;
use agere_shell_command::bash::parse_shell_lc_single_command_prefix;
use agere_shell_command::powershell::extract_powershell_command;
...
const REJECT_ESCALATION_APPROVAL_REASON: &str =
    "approval required by policy, but AskForApproval::Granular.access_approval is false";
...
pub(crate) fn child_uses_parent_exec_policy(parent_config: &Config, child_config: &Config) -> bool {
    fn exec_policy_config_folders(config: &Config) -> Vec<AbsolutePathBuf> {
        config
            ...
            .into_iter()
            .filter_map(agere_config::ConfigLayerEntry::config_folder)
            .collect()
    }
...
///
/// `prompt_is_rule` distinguishes policy-rule prompts from access-escalation
/// prompts so granular `rules` and `access_approval` settings are honored
...
pub(crate) fn prompt_is_rejected_by_policy(
    ...
    match approval_policy {
        ...
        AskForApproval::Granular(granular_config) => {
            ...
            } else if !granular_config.allows_access_approval() {
                Some(REJECT_ESCALATION_APPROVAL_REASON)
            } else {
                None
            }
        }
...
pub(crate) struct ExecApprovalRequest<'a> {
    ...
    pub(crate) permission_level: ExecutionPermissionLevel,
...
impl ExecPolicyManager {
    ...
    pub(crate) async fn append_amendment_and_update(
        ...
        let policy_path = default_policy_path(agere_home);
        spawn_blocking({
...
fn exec_policy_message_for_display(source: &agere_execpolicy::Error) -> String {
...
pub fn render_decision_for_unmatched_command(
    ...
    // On Windows, coarse read-only profiles do not tighten path rules the same way,
    // so special-case managed read-only envelopes here.
    let environment_is_read_only_managed = cfg!(windows)
        && profile_is_managed_read_only(permission_profile, file_system_access_policy, access_cwd);

    // If the command is flagged as dangerous or we have no substantive managed
    ...
    // forbid the command.
    if command_might_be_dangerous(command) || environment_is_read_only_managed {
        return match approval_policy {
            AskForApproval::Never => {
                let access_restriction_explicitly_disabled = matches!(
                    permission_profile,
                    PermissionProfile::Disabled | PermissionProfile::External { .. }
                );
                if access_restriction_explicitly_disabled {
                    // If managed restriction is explicitly disabled, allow the command to run.
                    Decision::Allow
                } else {
                    Decision::Forbidden
                }
    ...
    match approval_policy {
        AskForApproval::Never | AskForApproval::OnFailure => {
            // We allow the command to run, relying on enforced access policy for
            // containment.
            Decision::Allow
        }
        AskForApproval::UnlessTrusted => {
        ...
        AskForApproval::OnRequest => {
            match file_system_access_policy.kind {
                ...
                FileSystemAccessLevel::Restricted => {
                    // Under restricted filesystem profiles, do not prompt for non-escalated,
                    // non-dangerous commands; let enforced policy handle limits without extra prompts.
        ...
        AskForApproval::Granular(_) => match file_system_access_policy.kind {
            FileSystemAccessLevel::Unrestricted | FileSystemAccessLevel::External => {
...
fn profile_is_managed_read_only(
    ...
    matches!(permission_profile, PermissionProfile::Managed { .. })
        ...
        && file_system_access_policy
            .get_writable_roots_with_cwd(access_cwd)
            .is_empty()
}

fn default_policy_path(agere_home: &Path) -> PathBuf {
    agere_home.join(RULES_DIR_NAME).join(DEFAULT_POLICY_FILE)
}
...
///
/// Returns only the command name (first token) so future invocations of the
/// same command with different arguments are auto-approved.
    ...
    matched_rules
        ...
        .find_map(|rule_match| match rule_match {
            RuleMatch::HeuristicsRuleMatch {
                ...
                .first()
                .map(|name| ExecPolicyAmendment::new(vec![name.clone()])),
            _ => None,
        })
}

/// Derive an execpolicy amendment when the constrained attempt fails and Agere
/// prompts for a less restricted retry.
///
/// Returns only the command name (first token) as the amendment prefix.
