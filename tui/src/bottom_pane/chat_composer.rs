...
use super::grep_search_popup::GrepSearchPopup;
...
#[cfg(test)]
use agere_core_skills::model::SkillInterface;
...
pub(crate) struct ChatComposer {
    ...
    dismissed_grep_popup_token: Option<String>,
    current_grep_query: Option<String>,
    ...
    /// When false, the composer is temporarily read-only (e.g. during access restriction setup).
    input_enabled: bool,
    ...
    windows_degraded_restriction_active: bool,
...
impl ChatComposer {
    fn builtin_command_flags(&self) -> BuiltinCommandFlags {
        BuiltinCommandFlags {
            ...
            allow_elevate_restriction: self.windows_degraded_restriction_active,
    ...
    pub(crate) fn new_with_config(
        ...
        let mut this = Self {
            ...
            dismissed_grep_popup_token: None,
            current_grep_query: None,
            ...
            windows_degraded_restriction_active: false,
            side_conversation_active: false,
            is_zellij: matches!(
                crate::terminal_info::terminal_info().multiplexer,
    ...
    pub fn set_windows_degraded_restriction_active(&mut self, enabled: bool) {
        self.windows_degraded_restriction_active = enabled;
    }
    fn layout_areas(&self, area: Rect) -> [Rect; 4] {
        ...
        let popup_required_height = match &self.active_popup {
            ActivePopup::Command(popup) => popup.calculate_required_height(area.width),
            ActivePopup::File(popup) => popup.calculate_required_height(),
            ActivePopup::Skill(popup) => popup.calculate_required_height(area.width),
            ActivePopup::Grep(popup) => popup.calculate_required_height(),
            ActivePopup::None => footer_total_height,
        };
        const COLS_WITH_MARGIN: u16 = LIVE_PREFIX_COLS + 1;
        let textarea_width = area.width.saturating_sub(COLS_WITH_MARGIN);
        let remote_images_height = self
            .remote_images_lines(textarea_width)
            .len()
            .try_into()
            .unwrap_or(u16::MAX);
        let remote_images_separator = u16::from(remote_images_height > 0);
        let textarea_height = self.textarea.desired_height(textarea_width);
        let composer_height = textarea_height
            .saturating_add(remote_images_height)
            .saturating_add(remote_images_separator)
            .saturating_add(2)
            .min(area.height);
        let popup_height = popup_required_height.min(area.height.saturating_sub(composer_height));
        let composer_rect = Rect::new(area.x, area.y, area.width, composer_height);
        let popup_rect = Rect::new(
            area.x,
            area.bottom().saturating_sub(popup_height),
            area.width,
            popup_height,
        );
        ...
        let remote_images_height = remote_images_height.min(textarea_rect.height.saturating_sub(1));
        let remote_images_separator = u16::from(remote_images_height > 0);
    ...
    pub(crate) fn on_grep_search_result(&mut self, query: String, matches: Vec<RgMatch>) {
        // Only apply if user is still editing a token starting with `query`.
        ...
        if let ActivePopup::Grep(popup) = &mut self.active_popup {
            popup.set_matches(matches);
        }
    }
    ...
    pub fn handle_key_event(&mut self, key_event: KeyEvent) -> (InputResult, bool) {
        ...
        let result = match &mut self.active_popup {
            ...
            ActivePopup::Grep(_) => self.handle_key_event_with_grep_popup(key_event),
    ...
    fn handle_key_event_with_grep_popup(&mut self, key_event: KeyEvent) -> (InputResult, bool) {
        if self.handle_shortcut_overlay_key(&key_event) {
            return (InputResult::None, true);
        }
        if key_event.code == KeyCode::Esc {
        ...
        let ActivePopup::Grep(popup) = &mut self.active_popup else {
            unreachable!();
        };
        ...
        match key_event {
            ...
            KeyEvent {
                ...
                if let Some(tok) = Self::current_hash_token(&self.textarea) {
                    self.dismissed_grep_popup_token = Some(tok);
                }
                self.active_popup = ActivePopup::None;
                (InputResult::None, true)
            }
            ...
            | KeyEvent {
                ...
                let popup_ref = match &self.active_popup {
                    ActivePopup::Grep(p) => p,
                    _ => unreachable!(),
                };
                let Some(sel) = popup_ref.selected_match() else {
                    self.active_popup = ActivePopup::None;
                    return if key_event.code == KeyCode::Enter {
                        self.handle_key_event_without_popup(key_event)
                    } else {
                ...
                // Insert the grep result as a file:line reference.
                let reference = format!("{}:{}", sel.path, sel.line_number);
                ...
                self.textarea.replace_range(start_idx..end_idx, "");
                self.textarea.insert_str(&reference);
                self.current_grep_query = None;
                self.active_popup = ActivePopup::None;
                (InputResult::None, true)
            }
            input => self.handle_input_basic(input),
    ...
    pub(crate) fn sync_popups(&mut self) {
        ...
        if self.history_search.is_some() {
            ...
            if self.current_grep_query.is_some() {
                self.app_event_tx
                    .send(AppEvent::StartGrepSearch(String::new()));
                self.current_grep_query = None;
            }
            self.active_popup = ActivePopup::None;
            self.dismissed_file_popup_token = None;
            self.dismissed_mention_popup_token = None;
            self.dismissed_grep_popup_token = None;
            return;
        }
        ...
        let hash_token = Self::current_hash_token(&self.textarea);
        ...
        if let Some(token) = hash_token {
            ...
            self.sync_grep_search_popup(token);
            self.dismissed_file_popup_token = None;
            return;
        }
        ...
        if matches!(
            self.active_popup,
            ActivePopup::File(_) | ActivePopup::Skill(_) | ActivePopup::Grep(_)
        ) {
            self.active_popup = ActivePopup::None;
        }
    }
            ...
            _ => {
                if is_editing_slash_command_name {
                    ...
                    let mut command_popup = CommandPopup::new(CommandPopupFlags {
                        ...
                        windows_degraded_restriction_active: self
                            .windows_degraded_restriction_active,
                        side_conversation_active: self.side_conversation_active,
                    });
    ...
    fn sync_grep_search_popup(&mut self, query: String) {
        // If user dismissed popup for this exact query, don't reopen until text changes.
        if self.dismissed_grep_popup_token.as_ref() == Some(&query) {
            return;
        }

        self.app_event_tx
            .send(AppEvent::StartGrepSearch(query.clone()));

        match &mut self.active_popup {
            ActivePopup::Grep(popup) => {
                popup.set_query(query.clone());
            }
            _ => {
                let mut popup = GrepSearchPopup::new();
                popup.set_query(query.clone());
                self.active_popup = ActivePopup::Grep(popup);
            }
        }
        ...
        if query.is_empty() {
            ...
            self.current_grep_query = Some(query);
        }
        self.dismissed_grep_popup_token = None;
    }
    ...
    fn mention_items(&self) -> Vec<MentionItem> {
            ...
            && let Some(snapshot) = self.connectors_snapshot.as_ref()
        {
            for connector in &snapshot.connectors {
                ...
                let display_name = agere_connectors::metadata::connector_display_label(connector);
    ...
    fn desired_height(&self, width: u16) -> u16 {
        ...
        self.textarea.desired_height(inner_width)
            ...
            + match &self.active_popup {
                ...
                ActivePopup::Grep(c) => c.calculate_required_height(),
            }
    }
...
impl ChatComposer {
            ...
            ActivePopup::None => {
                ...
                if let Some(line) = self.history_search_footer_line() {
                    ...
                    let combined_status_line = if status_line_active {
                        passive_footer_status_line(&footer_props).map(green_line)
                    } else {
                        None
                    };
    ...
    fn render_textarea(
        ...
        if composer_rect.width > 0 && composer_rect.height > 0 {
            let separator = "─".repeat(composer_rect.width as usize);
            let separator_style = style.fg(ratatui::style::Color::DarkGray);
            buf.set_string(
                composer_rect.x,
                composer_rect.y,
                &separator,
...
mod tests {
    ...
    fn footer_hint_row_is_separated_from_composer() {
        ...
        let [composer_rect, _, _, _] = composer.layout_areas(area);
        let separator_row = row_to_string(composer_rect.bottom() - 1);
        assert_eq!(
            separator_row,
            "─".repeat(area.width as usize),
            "expected bottom composer separator in textarea shell but saw: {separator_row:?}",
        );
        for y in composer_rect.bottom()..hint_row_idx {
            assert_eq!(
                row_to_string(y),
                " ".repeat(area.width as usize),
                "expected empty spacer row between composer and footer at row {y}",
            );
            assert_eq!(buf[(0, y)].style().bg, Some(Color::Reset), "row {y}");
        }
        assert_eq!(
            composer_rect.height, 3,
            "composer shell should be top border, textarea row, bottom border",
        );
    }

    #[test]
    fn composer_renders_gray_horizontal_separators_without_side_lines() {
        let (tx, _rx) = unbounded_channel::<AppEvent>();
        let sender = AppEventSender::new(tx);
        let composer = ChatComposer::new(
            /*has_input_focus*/ true,
        ...
        let [composer_rect, _, _, _] = composer.layout_areas(area);
        assert_eq!(
            row_to_string(composer_rect.y),
            "─".repeat(area.width as usize)
        );
        assert_eq!(
            row_to_string(composer_rect.bottom() - 1),
            "─".repeat(area.width as usize)
        );
        for y in composer_rect.y + 1..composer_rect.bottom() - 1 {
            let row = row_to_string(y);
            assert_ne!(row.chars().next(), Some('│'));
            assert_ne!(row.chars().last(), Some('│'));
        }
        for x in 0..area.width {
            assert_eq!(buf[(x, composer_rect.y)].style().fg, Some(Color::DarkGray));
            assert_eq!(
                buf[(x, composer_rect.bottom() - 1)].style().fg,
                Some(Color::DarkGray)
            );
        }
    }
    ...
    #[test]
    fn composer_does_not_add_background() {
        let (tx, _rx) = unbounded_channel::<AppEvent>();
        let sender = AppEventSender::new(tx);
        let composer = ChatComposer::new(
            /*has_input_focus*/ true,
        ...
        let mut buf = Buffer::empty(area);
        let [composer_rect, _, _, _] = composer.layout_areas(area);
        buf.set_style(composer_rect, Style::default().bg(Color::Red));
        composer.render(area, &mut buf);

        let [composer_rect, _, textarea_rect, popup_rect] = composer.layout_areas(area);
        assert_eq!(textarea_rect.height, 1);
        assert_eq!(popup_rect.height, 1);
        assert_eq!(composer_rect.height, 3);

        for y in composer_rect.y..popup_rect.y {
            assert_eq!(buf[(0, y)].style().bg, Some(Color::Reset), "row {y}");
        }
    }
    ...
    #[test]
    fn textarea_borrows_composer_padding_before_scrolling_wrapped_text() {
        let (tx, _rx) = unbounded_channel::<AppEvent>();
        let sender = AppEventSender::new(tx);
        let mut composer = ChatComposer::new(
            /*has_input_focus*/ true,
        ...
        snapshot_composer_state(
            ...
            |composer| {
                ...
                composer.set_status_line(Some(Line::from(
                    "gpt-5.4 high fast · ~/code/agere-1 · Context 0% used",
                )));
    ...
    fn set_skill_mentions_refreshes_open_mention_popup() {
        ...
        composer.set_skill_mentions(Some(vec![SkillMetadata {
            name: "agere".to_string(),
            description: "Primary personal Agere repo skill.".to_string(),
            short_description: None,
            interface: None,
            dependencies: None,
            policy: None,
            path_to_skills_md: skill_path.clone(),
            scope: agere_protocol::protocol::SkillScope::User,
        }]));
        ...
        assert_eq!(mention.insert_text, "$agere".to_string());
    ...
    fn mention_items_show_plugin_owned_skill_and_app_duplicates() {
        ...
        composer.set_skill_mentions(Some(vec![SkillMetadata {
            ...
            policy: None,
            path_to_skills_md: skill_path.clone(),
            scope: agere_protocol::protocol::SkillScope::Repo,
        }]));
        composer.set_plugin_mentions(Some(vec![PluginCapabilitySummary {
...
程序员 与 Unicode 同行";
    ...
    fn file_completion_preserves_large_paste_placeholder_elements() {
        ...
        composer.on_file_search_result(
            ...
            vec![FileMatch {
                ...
                match_type: agere_file_search::MatchType::File,
    ...
    fn pasting_filepath_attaches_image() {
        ...
        let tmp_path: PathBuf = tmp.path().join("agere_tui_test_paste_image.png");
