...
use agere_protocol::permissions::ReadDenyMatcher;
    ...
    let file_system = match additional_permissions.file_system {
        Some(file_system) => {
            ...
            for entry in file_system.entries {
                ...
                let normalized_entry = FileSystemAccessEntry {
                    path,
                    access: entry.access,
                };
    ...
    let file_system = requested
        ...
        .map(|requested_file_system| {
            ...
            let requested_policy =
                FileSystemAccessPolicy::restricted(requested_file_system.entries.clone());
...
fn merge_glob_scan_max_depth(
    left_entries: &[FileSystemAccessEntry],
    left_depth: Option<usize>,
    right_entries: &[FileSystemAccessEntry],
...
fn granted_file_system_entry_within_request(
    ...
    requested_policy: &FileSystemAccessPolicy,
    requested_read_deny_matcher: Option<&ReadDenyMatcher>,
    granted_entry: &FileSystemAccessEntry,
    cwd: &Path,
) -> bool {
...
fn retain_constraining_deny_entries(
    source_entries: &[FileSystemAccessEntry],
    accepted_entries: &[FileSystemAccessEntry],
    cwd: &Path,
    output_entries: &mut Vec<FileSystemAccessEntry>,
...
fn deny_entry_constrains_accepted_grant(
    deny_entry: &FileSystemAccessEntry,
    accepted_entries: &[FileSystemAccessEntry],
    cwd: &Path,
) -> bool {
    accepted_entries
...
fn materialize_cwd_dependent_entry(
    ...
    match &entry.path {
        ...
        FileSystemPath::GlobPattern { pattern } => FileSystemAccessEntry {
...
fn merge_permission_entries(
    base: &[FileSystemAccessEntry],
    permissions: &[FileSystemAccessEntry],
) -> Vec<FileSystemAccessEntry> {
...
pub fn effective_file_system_access_policy(
    file_system_policy: &FileSystemAccessPolicy,
...
pub fn effective_network_access_policy(
...
pub fn effective_permission_profile(
    ...
    let effective_file_system_policy =
        effective_file_system_access_policy(&file_system_policy, additional_permissions);
    let effective_network_policy =
        effective_network_access_policy(network_policy, additional_permissions);
...
#[cfg(test)]
#[path = "access_policy_transforms_tests.rs"]
mod tests;
