...
pub async fn run(context: Arc<MemoryStartupContext>, config: Arc<Config>) {
    ...
    let root = memory_root(&config.agere_home);
...
mod job {
    ...
    pub(super) async fn claim(
        ...
        let (token, watermark) = match claim {
            agere_state::Phase2JobClaimOutcome::Claimed {
                ownership_token,
                input_watermark,
            } => {
                context.counter(
            ...
            agere_state::Phase2JobClaimOutcome::SkippedRetryUnavailable => {
                return Err("skipped_retry_unavailable");
            }
            agere_state::Phase2JobClaimOutcome::SkippedCooldown => {
                return Err("skipped_cooldown");
            }
            agere_state::Phase2JobClaimOutcome::SkippedRunning => return Err("skipped_running"),
        };
    ...
    pub(super) async fn succeed(
        ...
        selected_outputs: &[agere_state::Stage1Output],
        reason: &'static str,
    ) -> bool {
        context.counter(MEMORY_PHASE_TWO_JOBS, /*inc*/ 1, &[("status", reason)]);
...
mod agent {
    ...
    pub(super) fn get_config(config: &Config) -> Option<Config> {
        ...
        agent_config
            ...
            .set_permission_profile(
                agere_protocol::models::PermissionProfile::workspace_write_with(
                    &[root],
                    agere_protocol::permissions::NetworkAccessPolicy::Restricted,
    ...
    pub(super) fn handle(
        ...
        claim: Claim,
        new_watermark: i64,
        selected_outputs: Vec<agere_state::Stage1Output>,
        memory_root: agere_utils_fs::AbsolutePathBuf,
        agent: SpawnedConsolidationAgent,
        phase_two_e2e_timer: Option<agere_otel::Timer>,
    ) {
        let Some(db) = context.state_db() else {
            return;
        };
...
pub(super) fn get_watermark(
    ...
    latest_memories: &[agere_state::Stage1Output],
) -> i64 {
    latest_memories
        .iter()
