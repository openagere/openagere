...
//!
//! # Relationship to `agere-app-server-client`
//!
//! This module provides the low-level runtime handle ([`InProcessClientHandle`]).
//! Higher-level callers (TUI, exec) should go through `agere-app-server-client`,
...
/// through `agere-app-server-client`, which adds worker-task buffering,
...
fn start_uninitialized(args: InProcessStartArgs) -> InProcessClientHandle {
    ...
    let runtime_handle = tokio::spawn(async move {
        ...
        let auth_manager =
            AuthManager::shared_from_config(args.config.as_ref(), args.enable_agere_api_key_env)
                .await;
