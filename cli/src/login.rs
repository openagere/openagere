...
//! interactive-session layers. Direct `agere login` intentionally does less: it preserves the
...
//! into a one-shot CLI command while still producing a durable `agere-login.log` artifact that
...
use agere_login::ServerOptions;
use agere_login::login_with_agent_identity;
use agere_login::login_with_api_key;
use agere_login::logout_with_revoke;
use agere_login::run_device_code_login;
...
const API_KEY_LOGIN_DISABLED_MESSAGE: &str =
    "API key login is disabled. Use account login instead.";
const AGENT_IDENTITY_LOGIN_DISABLED_MESSAGE: &str =
    "Agent Identity login is disabled. Use API key login instead.";
const LOGIN_SUCCESS_MESSAGE: &str = "Successfully logged in";

/// Installs a small file-backed tracing layer for direct `agere login` flows.
///
...
/// command produce a durable `agere-login.log` artifact without coupling it to the TUI's broader
/// telemetry and feedback initialization.
fn init_login_file_logging(config: &Config) -> Option<WorkerGuard> {
    let log_dir = match agere_core::config::log_dir(config) {
        Ok(log_dir) => log_dir,
        Err(err) => {
    ...
    let log_path = log_dir.join("agere-login.log");
    ...
    let env_filter = EnvFilter::try_from_default_env()
        .unwrap_or_else(|_| EnvFilter::new("agere_cli=info,agere_core=info,agere_login=info"));
    ...
    // Direct `agere login` otherwise relies on ephemeral stderr and browser output.
...
fn print_login_server_start(actual_port: u16, auth_url: &str) {
    eprintln!(
        "Starting local login server on http://localhost:{actual_port}.\nIf your browser did not open, navigate to this URL to authenticate:\n\n{auth_url}\n\nOn a remote or headless machine? Use `agere login --device-auth` instead."
    );
}
...
pub fn read_agent_identity_from_stdin() -> String {
    read_stdin_secret(
        "--with-agent-identity expects the Agent Identity token on stdin. Try piping it, e.g. `printenv AGERE_AGENT_IDENTITY | agere login --with-agent-identity`.",
...
pub async fn run_login_with_device_code(
    ...
    if matches!(config.forced_login_method, Some(ForcedLoginMethod::Api)) {
        eprintln!("Device code login is disabled. Use API key login instead.");
        std::process::exit(1);
    }
...
/// `agere login` working in environments where device-code may be disabled/feature-gated.
...
pub async fn run_login_status(cli_config_overrides: CliConfigOverrides) -> ! {
    ...
    match AgereAuth::from_auth_storage(
        &config.agere_home,
        config.cli_auth_credentials_store_mode,
        Some(&config.chatgpt_base_url),
    )
    .await
    {
        Ok(Some(auth)) => match auth.auth_mode() {
            ...
            AuthMode::Chatgpt | AuthMode::ChatgptAuthTokens => {
                eprintln!("Logged in using account login");
                std::process::exit(0);
            }
...
pub async fn run_logout(cli_config_overrides: CliConfigOverrides) -> ! {
    ...
    match logout_with_revoke(&config.agere_home, config.cli_auth_credentials_store_mode).await {
        Ok(true) => {
