...
use agere_network_proxy::BlockedRequestObserver;
use agere_network_proxy::ConfigReloader;
use agere_network_proxy::ConfigState;
use agere_network_proxy::NetworkDecision;
use agere_network_proxy::NetworkPolicyDecider;
...
use agere_network_proxy::NetworkProxyHandle;
use agere_network_proxy::NetworkProxyState;
use agere_network_proxy::build_config_state;
use agere_network_proxy::host_and_port_from_network_addr;
use agere_network_proxy::normalize_host;
use agere_network_proxy::validate_policy_against_constraints;
...
impl NetworkProxySpec {
    ...
    pub async fn start_proxy(
        ...
        if enable_network_approval_flow && !self.hard_deny_allowlist_misses {
            ...
            } else if Self::managed_filesystem_restriction_active(permission_profile) {
                builder = builder
    ...
    fn allowlist_expansion_enabled(
        ...
        Self::managed_filesystem_restriction_active(permission_profile)
            && !hard_deny_allowlist_misses
    }
    ...
    fn managed_filesystem_restriction_active(permission_profile: &PermissionProfile) -> bool {
    ...
    for host in hosts {
        if incoming.insert(host.clone()) {
            config.network.upsert_domain_permission(
                host,
                if allow {
                    agere_network_proxy::NetworkDomainPermission::Allow
                } else {
                    agere_network_proxy::NetworkDomainPermission::Deny
                },
                normalize_host,
            );
        }
