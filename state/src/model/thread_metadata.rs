...
pub struct ThreadMetadata {
    ...
    pub title: String,
    /// The access policy (SQLite column `access_policy`; stringified enum).
...
pub struct ThreadMetadataBuilder {
    ...
    /// The access policy (see `ThreadMetadata::access_policy`).
    pub access_policy: String,
    /// The approval mode.
...
impl ThreadMetadata {
    ...
    pub fn diff_fields(&self, other: &Self) -> Vec<&'static str> {
        ...
        if self.access_policy != other.access_policy {
            diffs.push("access_policy");
        }
