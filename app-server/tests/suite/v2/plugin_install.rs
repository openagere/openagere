...
const TEST_ALLOW_HTTP_REMOTE_PLUGIN_BUNDLE_DOWNLOADS: &str =
    "AGERE_TEST_ALLOW_HTTP_REMOTE_PLUGIN_BUNDLE_DOWNLOADS";
    ...
    let request_id = mcp
        .send_plugin_install_request(PluginInstallParams {
            marketplace_path: Some(AbsolutePathBuf::try_from(
                agere_home.path().join("marketplace.json"),
            )?),
...
async fn plugin_install_writes_remote_plugin_to_cloud_and_cache() -> Result<()> {
    ...
    .await;
    configure_remote_plugin_test(agere_home.path(), &server)?;
    ...
    mount_remote_plugin_install_after_cache_write(
        &server,
        REMOTE_PLUGIN_ID,
        installed_path.join(".agere-plugin/plugin.json"),
    )
    .await;
...
async fn plugin_install_rejects_invalid_remote_plugin_name() -> Result<()> {
    let agere_home = TempDir::new()?;
    write_remote_plugin_catalog_config(agere_home.path(), "https://example.invalid/backend-api/")?;
    let mut mcp = McpProcess::new(agere_home.path()).await?;
    timeout(DEFAULT_TIMEOUT, mcp.initialize()).await??;
...
#[tokio::test]
#[ignore]
async fn plugin_install_rejects_when_workspace_agere_plugins_disabled() -> Result<()> {
    let agere_home = TempDir::new()?;
    ...
    assert!(
        err.error
            .message
            .contains("Agere plugins are disabled for this workspace")
    );
    Ok(())
}
...
async fn plugin_install_returns_invalid_request_for_missing_marketplace_file() -> Result<()> {
    ...
    let request_id = mcp
        .send_plugin_install_request(PluginInstallParams {
            marketplace_path: Some(AbsolutePathBuf::try_from(
                agere_home.path().join("missing-marketplace.json"),
            )?),
            remote_marketplace_name: None,
...
async fn plugin_install_returns_invalid_request_for_disallowed_product_plugin() -> Result<()> {
  ...
  "plugins": [
    ...
    let mut mcp =
        McpProcess::new_with_args(agere_home.path(), &["--session-source", "atlas"]).await?;
    timeout(DEFAULT_TIMEOUT, mcp.initialize()).await??;
...
async fn plugin_install_tracks_analytics_event() -> Result<()> {
    ...
    let agere_home = TempDir::new()?;
    write_analytics_config(agere_home.path(), &analytics_server.uri())?;
    write_chatgpt_auth(
        agere_home.path(),
        ChatGptAuthFixture::new("chatgpt-token")
            .account_id("account-123")
    ...
    let payload = timeout(DEFAULT_TIMEOUT, async {
        loop {
            ...
            if let Some(request) = requests.iter().find(|request| {
                request.method == "POST" && request.url.path() == "/agere/analytics-events/events"
            }) {
...
async fn plugin_install_returns_apps_needing_auth() -> Result<()> {
    ...
    let agere_home = TempDir::new()?;
    write_connectors_config(agere_home.path(), &server_url)?;
    write_chatgpt_auth(
        agere_home.path(),
        ChatGptAuthFixture::new("chatgpt-token")
            .account_id("account-123")
...
async fn start_apps_server(
    ...
    let router = Router::new()
        ...
        .with_state(state)
        .nest_service("/api/agere/apps", mcp_service);
...
fn write_connectors_config(agere_home: &std::path::Path, base_url: &str) -> std::io::Result<()> {
    std::fs::write(
...
fn write_analytics_config(agere_home: &std::path::Path, base_url: &str) -> std::io::Result<()> {
    std::fs::write(
        agere_home.join("config.toml"),
...
fn configure_remote_plugin_test(agere_home: &std::path::Path, server: &MockServer) -> Result<()> {
    write_remote_plugin_catalog_config(agere_home, &format!("{}/backend-api/", server.uri()))?;
    write_chatgpt_auth(
        agere_home,
        ChatGptAuthFixture::new("chatgpt-token")
            .account_id("account-123")
