...
use core_test_support::test_agere::TestAgereBuilder;
...
fn remote_realtime_test_agere_builder(
    realtime_server: &responses::WebSocketTestServer,
) -> TestAgereBuilder {
    let realtime_base_url = realtime_server.uri().to_string();
    test_agere()
        .with_auth(AgereAuth::from_api_key("dummy"))
...
async fn start_realtime_conversation(agere: &agere_core::AgereThread) -> Result<()> {
    agere
    ...
    wait_for_event_match(agere, |msg| match msg {
...
async fn close_realtime_conversation(agere: &agere_core::AgereThread) -> Result<()> {
    agere.submit(Op::RealtimeConversationClose).await?;
    wait_for_event_match(agere, |msg| match msg {
...
async fn wait_for_turn_complete(agere: &agere_core::AgereThread) {
...
async fn remote_compact_replaces_history_for_followups() -> Result<()> {
    ...
    let harness = TestAgereHarness::with_builder(
        test_agere().with_auth(AgereAuth::create_dummy_chatgpt_auth_for_testing()),
    )
    .await?;
    let agere = harness.test().agere.clone();
    ...
    agere
        ...
        .await?;
    wait_for_turn_complete(&agere).await;

    agere.submit(Op::Compact).await?;
    wait_for_turn_complete(&agere).await;
...
async fn remote_compact_filters_deferred_dynamic_tools() -> Result<()> {
    ...
    let mut builder = test_agere().with_auth(AgereAuth::create_dummy_chatgpt_auth_for_testing());
    ...
    assert_eq!(
        namespace_child_tool_names(&first_response_body, "agere_app"),
        vec![visible_tool.to_string()]
    );
    assert_eq!(
        namespace_child_tool_names(&compact_body, "agere_app"),
        vec![visible_tool.to_string()]
    );
...
async fn remote_compact_runs_automatically() -> Result<()> {
    ...
    let message = wait_for_event_match(&agere, |event| match event {
...
async fn remote_compact_trim_estimate_uses_session_base_instructions() -> Result<()> {
    ...
    let baseline_harness = TestAgereHarness::with_builder(
        test_agere()
            .with_auth(AgereAuth::create_dummy_chatgpt_auth_for_testing())
            .with_config(|config| {
    ...
    .await?;
    let baseline_agere = baseline_harness.test().agere.clone();
    ...
    baseline_agere
        ...
        .await?;
    wait_for_event(&baseline_agere, |event| {
    ...
    baseline_agere.submit(Op::Compact).await?;
    wait_for_event(&baseline_agere, |event| {
        matches!(event, EventMsg::TurnComplete(_))
    })
    .await;
    ...
    let override_harness = TestAgereHarness::with_builder(
        test_agere()
            .with_auth(AgereAuth::create_dummy_chatgpt_auth_for_testing())
            .with_config({
    ...
    .await?;
    let override_agere = override_harness.test().agere.clone();
    ...
    override_agere
        ...
        .await?;
    wait_for_event(&override_agere, |event| {
        matches!(event, EventMsg::TurnComplete(_))
    })
    .await;
    ...
    override_agere.submit(Op::Compact).await?;
    wait_for_event(&override_agere, |event| {
        matches!(event, EventMsg::TurnComplete(_))
    })
    .await;
...
async fn remote_compact_persists_replacement_history_in_rollout() -> Result<()> {
    ...
    agere.submit(Op::Shutdown).await?;
...
async fn remote_compact_and_resume_refresh_stale_developer_instructions() -> Result<()> {
    ...
    initial.agere.submit(Op::Compact).await?;
    wait_for_event(&initial.agere, |ev| matches!(ev, EventMsg::TurnComplete(_))).await;
    ...
    initial.agere.submit(Op::Shutdown).await?;
    wait_for_event(&initial.agere, |ev| {
    ...
    resumed
        ...
        .await?;
    wait_for_event(&resumed.agere, |ev| matches!(ev, EventMsg::TurnComplete(_))).await;
...
async fn remote_compact_refreshes_stale_developer_instructions_without_resume() -> Result<()> {
    ...
    test.agere
        ...
        .await?;
    wait_for_event(&test.agere, |ev| matches!(ev, EventMsg::TurnComplete(_))).await;

    test.agere.submit(Op::Compact).await?;
    wait_for_event(&test.agere, |ev| matches!(ev, EventMsg::TurnComplete(_))).await;
...
async fn snapshot_request_shape_remote_pre_turn_compaction_restates_realtime_start() -> Result<()> {
    ...
    let mut builder = remote_realtime_test_agere_builder(&realtime_server).with_config(|config| {
    ...
    start_realtime_conversation(test.agere.as_ref()).await?;
    ...
    close_realtime_conversation(test.agere.as_ref()).await?;
...
async fn remote_request_uses_custom_experimental_realtime_start_instructions() -> Result<()> {
    ...
    let mut builder = remote_realtime_test_agere_builder(&realtime_server).with_config({
...
async fn snapshot_request_shape_remote_manual_compact_restates_realtime_start() -> Result<()> {
    ...
    let server = wiremock::MockServer::start().await;
    let realtime_server = start_remote_realtime_server().await;
    let mut builder = remote_realtime_test_agere_builder(&realtime_server);
    let test = builder.build(&server).await?;
...
async fn snapshot_request_shape_remote_compact_resume_restates_realtime_end() -> Result<()> {
    ...
    start_realtime_conversation(initial.agere.as_ref()).await?;
    ...
    close_realtime_conversation(initial.agere.as_ref()).await?;
