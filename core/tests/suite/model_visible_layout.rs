...
async fn snapshot_model_visible_layout_turn_overrides() -> Result<()> {
    ...
    let second_permission_profile = turn_permission_fields(
...
async fn snapshot_model_visible_layout_resume_with_personality_change() -> Result<()> {
    ...
    let server = start_mock_server().await;
    let mut initial_builder = test_agere().with_config(|config| {
        config.model = Some("gpt-5.2".to_string());
    });
    let initial = initial_builder.build(&server).await?;
    let agere = Arc::clone(&initial.agere);
    ...
    let mut resume_builder = test_agere().with_config(|config| {
        config.model = Some("gpt-5.3".to_string());
        config
            .features
            .enable(Feature::Personality)
            .expect("test config should allow feature update");
    ...
    let permission_profile = turn_permission_fields(
        PermissionProfile::read_only(),
    ...
    resumed
        ...
        .submit(Op::UserTurn {
            ...
            effort: Some(resumed.config.model_reasoning_effort),
            summary: None,
            service_tier: None,
            collaboration_mode: None,
            personality: Some(Personality::Friendly),
