use agere_app_server::INPUT_TOO_LARGE_ERROR_CODE;
use agere_app_server::INVALID_PARAMS_ERROR_CODE;
use agere_app_server_protocol::ByteRange;
use agere_app_server_protocol::ClientInfo;
use agere_app_server_protocol::CollabAgentStatus;
...
use agere_app_server_protocol::FileSystemAccessEntry;
use agere_app_server_protocol::FileSystemAccessMode;
use agere_app_server_protocol::FileSystemPath;
...
use agere_app_server_protocol::PermissionProfile;
use agere_app_server_protocol::PermissionProfileFileSystemPermissions;
use agere_app_server_protocol::PermissionProfileNetworkPermissions;
...
use agere_app_server_protocol::TurnEnvironmentParams;
...
use agere_core::personality_migration::PERSONALITY_MIGRATION_FILENAME;
...
const TEST_ORIGINATOR: &str = "agere_vscode";
...
async fn turn_start_sends_originator_header() -> Result<()> {
    ...
    timeout(
        ...
        mcp.initialize_with_client_info(ClientInfo {
            ...
            title: Some("Agere VS Code Extension".to_string()),
...
async fn turn_start_emits_thread_scoped_warning_notification_for_trimmed_skills() -> Result<()> {
    ...
    write_models_cache(agere_home.path())?;
    let cache_path = agere_home.path().join("models_cache.json");
    ...
    write_test_skill(agere_home.path(), "alpha-skill")?;
    write_test_skill(agere_home.path(), "beta-skill")?;
...
async fn turn_start_tracks_turn_event_analytics() -> Result<()> {
    ...
    mount_analytics_capture(&server, agere_home.path()).await?;
    ...
    let event = wait_for_analytics_event(&server, DEFAULT_READ_TIMEOUT, "agere_turn_event").await?;
    ...
    assert_eq!(event["event_params"]["access_policy"], "read_only");
...
async fn turn_start_rejects_invalid_permission_profile_before_starting_turn() -> Result<()> {
    ...
    std::fs::write(
        agere_home.path().join("managed_config.toml"),
    ...
    let turn_req = mcp
        .send_turn_start_request(TurnStartParams {
            ...
            environments: Some(vec![TurnEnvironmentParams {
                ...
                cwd: agere_home.path().to_path_buf().try_into()?,
            }]),
            ..Default::default()
        })
        .await?;
...
async fn turn_start_emits_notifications_and_accepts_model_override() -> Result<()> {
    ...
    // Three Agere turns hit the mock model (session start + two turn/start calls).
    let responses = vec![
    ...
    assert_eq!(
        started.turn.status,
        agere_app_server_protocol::TurnStatus::InProgress
    );
...
async fn turn_start_accepts_personality_override_v2() -> Result<()> {
    ...
    let thread_req = mcp
        .send_thread_start_request(ThreadStartParams {
            model: Some("exp-agere-personality".to_string()),
            ..Default::default()
        })
        .await?;
    let thread_resp: JSONRPCResponse = timeout(
...
async fn turn_start_accepts_local_image_input() -> Result<()> {
    // Two Agere turns hit the mock model (session start + turn/start).
    let responses = vec![
        create_final_assistant_message_sse_response("Done")?,
        create_final_assistant_message_sse_response("Done")?,
    ];
    ...
    let image_path = agere_home.path().join("image.png");
...
async fn turn_start_exec_approval_toggle_v2() -> Result<()> {
    ...
    let mut mcp = McpProcess::new(agere_home.as_path()).await?;
    timeout(DEFAULT_READ_TIMEOUT, mcp.initialize()).await??;
...
#[tokio::test]
#[ignore]
async fn turn_start_updates_filesystem_access_envelope_and_cwd_between_turns_v2() -> Result<()> {
    skip_if_no_network!(Ok(()));

    let tmp = TempDir::new()?;
    let agere_home = tmp.path().join("agere_home");
    std::fs::create_dir(&agere_home)?;
    ...
    let mut mcp = McpProcess::new(&agere_home).await?;
    timeout(DEFAULT_READ_TIMEOUT, mcp.initialize()).await??;
    ...
    // first turn with workspace-write access profile and first_cwd
    let first_turn = mcp
        .send_turn_start_request(TurnStartParams {
            environments: None,
            thread_id: thread.id.clone(),
            ...
            access_policy: Some(agere_app_server_protocol::AccessPolicy::WorkspaceWrite {
...
async fn turn_start_resolves_sticky_thread_environments_and_turn_overrides() -> Result<()> {
    ...
    let server = create_mock_responses_server_repeating_assistant("done").await;
    create_config_toml(&agere_home, &server.uri(), "never", &BTreeMap::default())?;

    let mut mcp = McpProcess::new_with_env(
        &agere_home,
        &[("AGERE_EXEC_SERVER_URL", Some("http://127.0.0.1:1"))],
    )
    .await?;
    timeout(DEFAULT_READ_TIMEOUT, mcp.initialize()).await??;
...
"#;
    ...
    pretty_assertions::assert_eq!(
        started_changes,
        vec![agere_app_server_protocol::FileUpdateChange {
...
async fn turn_start_emits_spawn_agent_item_with_effective_role_model_metadata_v2() -> Result<()> {
    ...
    std::fs::write(
        agere_home.path().join("custom-role.toml"),
...
async fn command_execution_notifications_include_process_id() -> Result<()> {
    ...
    let server = create_mock_responses_server_sequence(responses).await;
    let agere_home = TempDir::new()?;
    create_config_toml_with_access_mode(
        agere_home.path(),
        &server.uri(),
        "never",
...
#[tokio::test]
#[ignore]
async fn turn_start_validates_model_override_against_selected_model_provider() -> Result<()> {
...
"#,
    ...
    write_models_cache_with_models(
        agere_home.path(),
        vec![test_model_info("old-provider-model")],
    )?;
    ...
    let turn_request = mcp
        .send_turn_start_request(TurnStartParams {
            ...
            model: Some("new-provider-model".to_string()),
            input: vec![V2UserInput::Text {
                text: "Hello".to_string(),
                text_elements: Vec::new(),
            }],
...
async fn turn_start_with_elevated_override_does_not_persist_project_trust() -> Result<()> {
    ...
    let config_toml = std::fs::read_to_string(agere_home.path().join("config.toml"))?;
...
fn create_config_toml_with_access_mode(
    agere_home: &Path,
    server_uri: &str,
    approval_policy: &str,
    feature_flags: &BTreeMap<Feature, bool>,
...
fn write_test_skill(agere_home: &Path, name: &str) -> std::io::Result<()> {
    let skill_dir = agere_home.join("skills").join(name);
