        ...
        "response.output_item.done" => {
            if let Some(item_val) = event.item {
                if let Ok(item) = serde_json::from_value::<ResponseItem>(item_val) {
                    return Ok(Some(ResponseEvent::output_item_done(item)));
                }
...
mod tests {
    ...
    async fn parses_items_and_completed() {
        ...
        assert_matches!(
            &events[0],
            Ok(ResponseEvent::OutputItemDone { item: ResponseItem::Message {
                role,
        ...
        assert_matches!(
            &events[1],
            Ok(ResponseEvent::OutputItemDone { item: ResponseItem::Message { role, .. }, .. }) if role == "assistant"
        );
    ...
    async fn parses_tool_search_call_items() {
        ...
        assert_matches!(
            &events[0],
            ResponseEvent::OutputItemDone { item: ResponseItem::ToolSearchCall {
                call_id,
                execution,
                arguments,
                ..
