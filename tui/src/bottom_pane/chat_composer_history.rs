...
mod tests {
    ...
    fn navigation_with_async_fetch() {
        ...
        let AppEvent::AgereOp(op) = event2 else {
            panic!("unexpected event variant");
        };
        assert_eq!(
            Op::GetHistoryEntryRequest {
    ...
    fn search_matches_local_history_and_stops_at_boundaries() {
        ...
        history.record_local_submission(HistoryEntry::new("cargo test -p agere-tui".to_string()));
    ...
    fn search_fetches_persistent_history_until_match() {
        ...
        let AppEvent::AgereOp(op) = rx.try_recv().expect("expected latest lookup") else {
            panic!("unexpected event variant");
        };
        assert_eq!(
            Op::GetHistoryEntryRequest {
        ...
        let AppEvent::AgereOp(op) = rx.try_recv().expect("expected next lookup") else {
            panic!("unexpected event variant");
        };
        assert_eq!(
            Op::GetHistoryEntryRequest {
