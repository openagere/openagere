...
use agere_utils_common::strip_proposed_plan_blocks;
...
fn strip_hidden_assistant_markup_and_parse_memory_citation(
    ...
    String,
    Option<agere_protocol::memory_citation::MemoryCitation>,
) {
        ...
        && role == "assistant"
    {
        let combined = content
            ...
            .filter_map(|ci| match ci {
                agere_protocol::models::ContentItem::OutputText { text } => Some(text.as_str()),
                _ => None,
            })
            .collect::<String>();
...
async fn save_image_generation_result(
    ...
    let bytes = BASE64_STANDARD
        ...
        .map_err(|err| {
            AgereErr::InvalidRequest(format!("invalid image generation payload: {err}"))
        })?;
    let path = image_generation_artifact_path(agere_home, session_id, call_id);
...
pub(crate) struct OutputItemResult {
    ...
    /// A non-tool response item that should be emitted as ItemCompleted.
    pub completed_item: Option<TurnItem>,
}
...
) -> Result<(OutputItemResult, OutputItemCompletion)> {
    ...
    match ctx.tool_runtime.build_tool_call(item.clone()).await {
        ...
        Ok(None) => {
            ...
            .await
            {
                ...
                output.completed_item = Some(turn_item);
            }
        ...
        Err(FunctionCallError::Fatal(message)) => {
            return Err(AgereErr::Fatal(message));
        }
    }
        ...
        | ResponseItem::ImageGenerationCall { .. } => {
            ...
            if let TurnItem::AgentMessage(agent_message) = &mut turn_item {
                ...
                agent_message.content =
                    vec![agere_protocol::items::AgentMessageContent::Text { text: stripped }];
                ...
                .await
                {
                    Ok(path) => {
                        ...
                        let image_output_dir = image_output_path
                            .parent()
                            .unwrap_or_else(|| turn_context.config.agere_home.clone());
