...
) -> agere_exec_server::ExecEnvPolicy {
    agere_exec_server::ExecEnvPolicy {
...
fn exec_server_env_for_request(
    ...
    Option<agere_exec_server::ExecEnvPolicy>,
    HashMap<String, String>,
) {
...
impl UnifiedExecProcessManager {
    ...
    pub(crate) async fn exec_command(
        ...
        let process = self
            .open_managed_exec_session(&request, cwd.clone(), context)
            .await;
        ...
        let (response_process_id, exit_code) = if process_started_alive {
            match self.refresh_process_state(process_id).await {
                ...
                ProcessStatus::Exited { exit_code, .. } => {
                    process.check_for_policy_denial_with_text(&text).await?;
                    (None, exit_code)
                }
    ...
    pub(crate) async fn open_session_with_exec_env(
        ...
        let spawn_result = if tty {
            agere_utils_pty::pty::spawn_process_with_inherited_fds(
                program,
                args,
                request.cwd.as_path(),
                &request.env,
            ...
            agere_utils_pty::pipe::spawn_process_no_stdin_with_inherited_fds(
                program,
                args,
                request.cwd.as_path(),
                &request.env,
    ...
    pub(super) async fn open_managed_exec_session(
        &self,
        request: &ExecCommandRequest,
        cwd: AbsolutePathBuf,
        context: &UnifiedExecContext,
        ...
        env.insert(
            AGERE_THREAD_ID_ENV_VAR.to_string(),
        ...
        let file_system_access_policy = context.turn.file_system_access_policy();
        let exec_approval_requirement = context
            ...
            .create_exec_approval_requirement_for_command(ExecApprovalRequest {
                ...
                access_cwd: context.turn.cwd.as_path(),
                permission_level: if request.additional_permissions_preapproved {
        ...
        orchestrator
            ...
            .map_err(|err| match err {
                ToolError::Agere(AgereErr::Exec(ExecErr::Denied { output, .. })) => {
                    let output = *output;
                    ...
                    UnifiedExecError::policy_denied(message, output)
                }
