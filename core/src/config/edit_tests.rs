...
fn blocking_set_model_top_level() {
    ...
    let contents = std::fs::read_to_string(agere_home.join(CONFIG_TOML_FILE)).expect("read config");
...
fn keymap_bindings_edit_writes_multiple_bindings_as_array() {
    ...
    let raw = std::fs::read_to_string(agere_home.join(CONFIG_TOML_FILE)).expect("read config");
...
profiles = { fast = { model = "gpt-4o", access_mode = "strict" } }
"#,
    )
    .expect("seed");
    ...
    // Ensure access_mode is preserved under profiles.fast and model updated.
    ...
    let fast_tbl = profiles_tbl
        ...
        .expect("fast table");
    assert_eq!(
        fast_tbl.get("access_mode").and_then(|v| v.as_str()),
        Some("strict")
    );
    assert_eq!(
...
fn blocking_set_model_writes_through_symlink_chain() {
    ...
    let link_path = agere_home.join("config-link.toml");
...
fn blocking_set_model_replaces_symlink_on_cycle() {
    let tmp = tempdir().expect("tmpdir");
    let agere_home = tmp.path();
    let link_a = agere_home.join("a.toml");
    let link_b = agere_home.join("b.toml");
    let config_path = agere_home.join(CONFIG_TOML_FILE);
...
"#;
    std::fs::write(agere_home.join(CONFIG_TOML_FILE), original).expect("seed config");
    ...
    apply_blocking(
        ...
        &[
            ...
            ConfigEdit::SetPath {
                segments: vec![
                    "access_workspace_write".to_string(),
    ...
    let updated = std::fs::read_to_string(agere_home.join(CONFIG_TOML_FILE)).expect("read config");
...
#[test]
fn blocking_set_hide_gpt_5_1_agere_max_migration_prompt_preserves_table() {
    let tmp = tempdir().expect("tmpdir");
    let agere_home = tmp.path();
    std::fs::write(
        agere_home.join(CONFIG_TOML_FILE),
...
"#,
    ...
    apply_blocking(
        ...
        /*profile*/ None,
        &[ConfigEdit::SetNoticeHideModelMigrationPrompt(
            "hide_gpt-5.1-agere-max_migration_prompt".to_string(),
            true,
        )],
    )
    .expect("persist");
    ...
    let expected = r#"[notice]
existing = "value"
"hide_gpt-5.1-agere-max_migration_prompt" = true
"#;
    assert_eq!(contents, expected);
}
...
fn blocking_clear_path_noop_when_missing() {
    ...
    assert!(
        !agere_home.join(CONFIG_TOML_FILE).exists(),
...
"#;
    ...
    let mut contents =
        std::fs::read_to_string(agere_home.join(CONFIG_TOML_FILE)).expect("read config");
...
"#;
    ...
    contents = std::fs::read_to_string(agere_home.join(CONFIG_TOML_FILE)).expect("read config");
...
#[test]
fn replace_models_table_blocking_clears_table_when_empty() {
    let tmp = tempdir().expect("tmpdir");
    let agere_home = tmp.path();
    std::fs::write(
        agere_home.join(CONFIG_TOML_FILE),
    ...
    apply_blocking(
        agere_home,
        /*profile*/ None,
        &[ConfigEdit::ReplaceModelsTable(Vec::new())],
    )
    .expect("persist");
...
#[test]
fn replace_models_table_blocking_writes_full_table() {
    let tmp = tempdir().expect("tmpdir");
    let agere_home = tmp.path();
    ...
    apply_blocking(
        agere_home,
        /*profile*/ None,
        &[ConfigEdit::ReplaceModelsTable(vec![
            ModelConfig {
...
#[test]
fn replace_models_table_combines_with_set_path_in_one_batch() {
    let tmp = tempdir().expect("tmpdir");
    let agere_home = tmp.path();
    ...
    let contents = std::fs::read_to_string(agere_home.join(CONFIG_TOML_FILE)).expect("read config");
    let parsed: TomlValue = toml::from_str(&contents).expect("parse");
    assert_eq!(
        parsed.get("model_provider").and_then(TomlValue::as_str),
        Some("deepseek")
    );
    assert_eq!(
        parsed.get("model").and_then(TomlValue::as_str),
        Some("deepseek-chat")
    );
    let models = parsed
        .get("models")
    ...
    assert_eq!(
        models[0].get("name").and_then(TomlValue::as_str),
        Some("deepseek-chat")
    );
}
