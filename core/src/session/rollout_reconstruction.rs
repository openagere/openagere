                ...
                RolloutItem::TurnContext(ctx) => {
                    ...
                    if turn_ids_are_compatible(
                        ...
                        active_segment.previous_turn_settings = Some(PreviousTurnSettings {
                            ...
                            // `TurnContextItem` 是已序列化的 rollout 类型，未携带 provider/窗口信息。
                            // 为保持向后兼容（避免给旧 rollout 加字段），此处使用最小回填：
                            // provider_id 留空、context_window 为 None。
                            // context_window 缺失时 Task 5 回退到用旧 model 在当前 manager 上解析；
                            // provider_id 为空表示未知，Task 5 不将其视为 provider 变化。
