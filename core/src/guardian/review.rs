...
use agere_analytics::GuardianReviewDecision;
use agere_analytics::GuardianReviewFailureReason;
use agere_analytics::GuardianReviewTerminalStatus;
use agere_analytics::GuardianReviewTrackContext;
...
pub(crate) fn is_guardian_reviewer_source(
    session_source: &agere_protocol::protocol::SessionSource,
) -> bool {
    matches!(
        session_source,
        agere_protocol::protocol::SessionSource::SubAgent(SubAgentSource::Other(name))
...
/// it is pinned to read-only access with `approval_policy = never` and
...
pub(super) async fn run_guardian_review_session(
    ...
    let available_models = session
        .services
        .models_manager
        .list_models(agere_models_manager::manager::RefreshStrategy::Offline)
        .await;
    ...
    let (guardian_model, guardian_reasoning_effort) = if let Some(preset) = preferred_model {
        let reasoning_effort = preferred_reasoning_effort(
            preset
                ...
                .iter()
                .any(|effort| effort.effort == agere_protocol::openai_models::ReasoningEffort::Low),
        ...
        let reasoning_effort = preferred_reasoning_effort(
            turn.model_info
                ...
                .iter()
                .any(|preset| preset.effort == agere_protocol::openai_models::ReasoningEffort::Low),
