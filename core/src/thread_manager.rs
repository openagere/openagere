...
use crate::agere_thread::AgereThread;
...
use crate::session::AgereSpawnArgs;
use crate::session::AgereSpawnOk;
...
use agere_app_server_protocol::ThreadHistoryBuilder;
...
struct TempAgereHomeGuard {
    path: PathBuf,
}

impl Drop for TempAgereHomeGuard {
    fn drop(&mut self) {
...
/// Represents a newly created Agere thread (formerly called a conversation), including the first event
/// (which is [`EventMsg::SessionConfigured`]).
pub struct NewThread {
    pub thread_id: ThreadId,
    pub thread: Arc<AgereThread>,
...
pub struct ThreadManager {
    ...
    _test_agere_home_guard: Option<TempAgereHomeGuard>,
}
...
pub struct StartThreadOptions {
    ...
    pub dynamic_tools: Vec<agere_protocol::dynamic_tools::DynamicToolSpec>,
...
pub(crate) struct ThreadManagerState {
    threads: Arc<RwLock<HashMap<ThreadId, Arc<AgereThread>>>>,
...
impl ThreadManager {
    pub fn new(
        ...
        Self {
            ...
            _test_agere_home_guard: None,
        }
    }

    /// Construct with a dummy AuthManager containing the provided AgereAuth.
    ...
    pub(crate) fn with_models_provider_for_tests(
        ...
        let agere_home = std::env::temp_dir().join(format!(
            "agere-thread-manager-test-{}",
            uuid::Uuid::new_v4()
        ));
        std::fs::create_dir_all(&agere_home)
            .unwrap_or_else(|err| panic!("temp agere home dir create failed: {err}"));
        ...
        manager._test_agere_home_guard = Some(TempAgereHomeGuard { path: agere_home });
        manager
    }

    /// Construct with a dummy AuthManager containing the provided AgereAuth and agere home.
    /// Used for integration tests: should not be used by ordinary business logic.
    pub(crate) fn with_models_provider_and_home_for_tests(
        ...
        let skills_agere_home = match AbsolutePathBuf::from_absolute_path_checked(&agere_home) {
            ...
            Err(err) => panic!("test agere_home should be absolute: {err}"),
        };
        let (thread_created_tx, _) = broadcast::channel(THREAD_CREATED_CHANNEL_CAPACITY);
        ...
        Self {
            state: Arc::new(ThreadManagerState {
                threads: Arc::new(RwLock::new(HashMap::new())),
                thread_created_tx,
                models_manager: create_model_provider(provider, Some(auth_manager.clone()), vec![])
                    .models_manager(
                        agere_home,
    ...
    pub async fn get_thread(&self, thread_id: ThreadId) -> AgereResult<Arc<AgereThread>> {
        ...
        if let Some(state_db_ctx) = thread.state_db() {
            for status in [
                ...
                for descendant_id in state_db_ctx
                    ...
                    .await
                    .map_err(|err| {
                        AgereErr::Fatal(format!("failed to load thread-spawn descendants: {err}"))
                    })?
                {
    ...
    pub async fn start_thread(&self, config: Config) -> AgereResult<NewThread> {
    ...
    pub async fn start_thread_with_tools(
        &self,
        config: Config,
        dynamic_tools: Vec<agere_protocol::dynamic_tools::DynamicToolSpec>,
    ...
    /// as `Arc<AgereThread>`, it is possible that other references to it exist elsewhere.
    /// Returns the thread if the thread was found and removed.
    pub async fn remove_thread(&self, thread_id: &ThreadId) -> Option<Arc<AgereThread>> {
...
impl ThreadManagerState {
    ...
    pub(crate) async fn get_thread(&self, thread_id: ThreadId) -> AgereResult<Arc<AgereThread>> {
        let threads = self.threads.read().await;
        match threads.get(&thread_id) {
            Some(thread) if !thread.session_source.is_internal() => Ok(thread.clone()),
            Some(_) | None => Err(AgereErr::ThreadNotFound(thread_id)),
        }
    }

    /// Send an operation to a thread by ID.
    pub(crate) async fn send_op(&self, thread_id: ThreadId, op: Op) -> AgereResult<String> {
    ...
    pub(crate) async fn remove_thread(&self, thread_id: &ThreadId) -> Option<Arc<AgereThread>> {
    ...
    pub(crate) async fn spawn_thread_with_source(
        ...
        .await?;
        let new_thread = self
            .finalize_thread_spawn(agere, thread_id, tracked_session_source, watch_registration)
            .await?;
    ...
    async fn finalize_thread_spawn(
        ...
        let event = agere.next_event().await?;
        let session_configured = match event {
            ...
            _ => {
                return Err(AgereErr::SessionConfiguredNotFirstEvent);
            }
        };

        let thread = Arc::new(AgereThread::new(
            agere,
    ...
    async fn parent_rollout_thread_trace_for_source(
        ...
        session_source: &SessionSource,
        initial_history: &InitialHistory,
    ) -> agere_rollout_trace::ThreadTraceContext {
        ...
        let SessionSource::SubAgent(SubAgentSource::ThreadSpawn {
            ...
            return agere_rollout_trace::ThreadTraceContext::disabled();
        };
        ...
        self.get_thread(*parent_thread_id)
            .await
            .ok()
            .map(|thread| thread.agere.session.services.rollout_thread_trace.clone())
            .unwrap_or_else(agere_rollout_trace::ThreadTraceContext::disabled)
    }
}
