...
async fn persist_tool_suggest_disable_writes_connector_config() {
    ...
    persist_tool_suggest_disable(&agere_home.path().abs(), &tool)
        .await
        .expect("persist connector disable");

    let contents =
        std::fs::read_to_string(agere_home.path().join(CONFIG_TOML_FILE)).expect("read config");
...
async fn persist_tool_suggest_disable_writes_plugin_config() {
    ...
    assert_eq!(
        ...
        Some(ToolSuggestConfig {
            discoverables: Vec::new(),
            disabled_tools: vec![ToolSuggestDisabledTool::plugin("slack@openagere-curated")],
        })
    );
}
...
[tool_suggest]
discoverables = [
  { type = "plugin", id = "sample@openagere-curated" }
]
