...
impl SessionTelemetry {
    ...
    pub fn record_responses(&self, handle_responses_span: &Span, event: &ResponseEvent) {
        ...
        match event {
            ...
            ResponseEvent::Completed {
                ...
                handle_responses_span.record(
                    "agere.usage.reasoning_output_tokens",
                    token_usage.reasoning_output_tokens,
                );
                handle_responses_span.record("agere.usage.total_tokens", token_usage.total_tokens);
            }
            _ => {}
        }
    }
    ...
    pub fn conversation_starts(
        ...
        log_and_trace_event!(
            self,
            common: {
                event.name = "agere.conversation_starts",
                provider_name = %provider_name,
                auth.env_openai_api_key_present = self.metadata.auth_env.openai_api_key_env_present,
                auth.env_agere_api_key_present = self.metadata.auth_env.agere_api_key_env_present,
                auth.env_agere_api_key_enabled = self.metadata.auth_env.agere_api_key_env_enabled,
    ...
    pub fn record_websocket_connect(
        ...
        log_and_trace_event!(
            self,
            common: {
                event.name = "agere.websocket_connect",
                duration_ms = %duration.as_millis(),
                http.response.status_code = status,
                success = success_str,
                error.message = error,
    ...
    pub fn record_websocket_request(
        ...
        log_and_trace_event!(
            self,
            common: {
                event.name = "agere.websocket_request",
                duration_ms = %duration.as_millis(),
                success = success_str,
                error.message = error,
                auth.env_openai_api_key_present = self.metadata.auth_env.openai_api_key_env_present,
    ...
    pub fn record_auth_recovery(
        ...
        log_and_trace_event!(
            self,
            common: {
                event.name = "agere.auth_recovery",
                auth.mode = mode,
                auth.step = step,
    ...
    pub fn record_websocket_event(
        ...
        log_and_trace_event!(
            self,
            common: {
                event.name = "agere.websocket_event",
                event.kind = %kind_str,
                duration_ms = %duration.as_millis(),
                success = success_str,
    ...
    pub fn tool_decision(
        ...
        log_event!(
            self,
            event.name = "agere.tool_decision",
            tool_name = %tool_name,
            call_id = %call_id,
