...
use crate::test_agere::TestAgere;
use crate::test_agere::test_agere;
...
pub fn zsh_fork_runtime(test_name: &str) -> Result<Option<ZshForkRuntime>> {
    ...
    let Ok(main_execve_wrapper_exe) = agere_utils_cargo_bin::cargo_bin("agere-execve-wrapper")
    else {
        eprintln!("skipping {test_name}: unable to resolve `agere-execve-wrapper` binary");
        return Ok(None);
    };
