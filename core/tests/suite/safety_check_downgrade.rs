...
fn disabled_text_turn(test: &TestAgere, text: &str) -> Op {
    let permission_profile = turn_permission_fields(PermissionProfile::Disabled, test.cwd_path());
    Op::UserTurn {
        environments: None,
...
async fn openai_model_header_mismatch_emits_warning_event_and_warning_item() -> Result<()> {
    ...
    let mut builder = test_agere().with_model(REQUESTED_MODEL);
    ...
    let reroute = wait_for_event(&test.agere, |event| {
    ...
    let warning = wait_for_event(&test.agere, |event| matches!(event, EventMsg::Warning(_))).await;
    let EventMsg::Warning(warning) = warning else {
        panic!("expected warning event");
    };
    assert!(warning.message.contains(REQUESTED_MODEL));
    assert!(warning.message.contains(SERVER_MODEL));
    assert!(warning.message.contains("OpenAI routed this request"));
    assert!(warning.message.contains("Trusted Access for Cyber"));
    assert!(warning.message.contains("chatgpt.com/cyber"));
    assert!(!warning.message.contains("-agere"));

    let warning_item = wait_for_event(&test.agere, |event| {
        matches!(
            event,
    ...
    assert!(warning_text.contains("OpenAI routed this request"));
    assert!(warning_text.contains("Trusted Access for Cyber"));
    assert!(warning_text.contains("chatgpt.com/cyber"));
    assert!(!warning_text.contains("-agere"));

    let _ = wait_for_event(&test.agere, |event| {
...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn non_openai_model_header_mismatch_uses_generic_safety_warning() -> Result<()> {
    skip_if_no_network!(Ok(()));
    ...
    let mut builder = test_agere()
        ...
        .with_config(|config| {
            config.model_provider_id = "openai-compatible-test".to_string();
        });
    let test = builder.build(&server).await?;

    test.agere
        .submit(disabled_text_turn(&test, "trigger non-openai safety check"))
        .await?;
    ...
    assert_eq!(reroute.from_model, REQUESTED_MODEL);
    assert_eq!(reroute.to_model, SERVER_MODEL);
    assert_eq!(reroute.reason, ModelRerouteReason::ProviderSafetyRouting);
    ...
    assert!(
        warning
            .message
            .contains("A provider-side safety check routed this request")
    );
    assert!(!warning.message.contains("ChatGPT"));
    assert!(!warning.message.contains("Trusted Access"));
    assert!(!warning.message.contains("chatgpt.com/cyber"));
    assert!(!warning.message.contains("-agere"));
...
async fn cyber_policy_response_emits_typed_error_without_retry() -> Result<()> {
    ...
    let error = wait_for_event(&test.agere, |event| matches!(event, EventMsg::Error(_))).await;
    let EventMsg::Error(error) = error else {
        panic!("expected error event");
    };
    assert_eq!(error.message, CYBER_POLICY_MESSAGE);
    assert_eq!(error.agere_error_info, Some(AgereErrorInfo::CyberPolicy));
...
async fn response_model_field_mismatch_emits_warning_when_header_matches_requested() -> Result<()> {
    ...
    let warning = wait_for_event(&test.agere, |event| {
        matches!(
            event,
            EventMsg::Warning(warning)
                if warning
                    .message
                    .contains("OpenAI routed this request")
        )
    })
    .await;
    let EventMsg::Warning(warning) = warning else {
...
async fn openai_model_header_mismatch_only_emits_one_warning_per_turn() -> Result<()> {
    ...
    loop {
        let event = wait_for_event(&test.agere, |_| true).await;
        match event {
    ...
    loop {
        ...
        match event {
            ...
            EventMsg::Warning(warning)
                if warning
                    .message
                    .contains("provider-side safety check routed this request") =>
            {
                warning_count += 1;
            }
            EventMsg::TurnComplete(_) => break,
