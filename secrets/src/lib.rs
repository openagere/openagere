...
const KEYRING_SERVICE: &str = "agere";
...
impl SecretsManager {
    pub fn new(agere_home: PathBuf, backend_kind: SecretsBackendKind) -> Self {
        let backend: Arc<dyn SecretsBackend> = match backend_kind {
            SecretsBackendKind::Local => {
                let keyring_store: Arc<dyn KeyringStore> = Arc::new(DefaultKeyringStore);
                Arc::new(LocalSecretsBackend::new(agere_home, keyring_store))
            }
        };
        Self { backend }
    }
...
pub(crate) fn compute_keyring_account(agere_home: &Path) -> String {
    let canonical = agere_home
        .canonicalize()
        .unwrap_or_else(|_| agere_home.to_path_buf())
        .to_string_lossy()
        .into_owned();
    let mut hasher = Sha256::new();
    hasher.update(canonical.as_bytes());
