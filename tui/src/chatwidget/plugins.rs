...
const OPENAGERE_MARKETPLACE_TAB_ID: &str = "marketplace:openagere-curated";
const OPENAGERE_MARKETPLACE_DISPLAY_NAME: &str = "OpenAgere Marketplace";
const OPENAGERE_MARKETPLACE_SYNCING_TITLE: &str = "Syncing OpenAgere Marketplace...";
const OPENAGERE_MARKETPLACE_SYNCING_DESCRIPTION: &str = "Plugins will appear here soon.";
...
impl ChatWidget {
    ...
    fn plugin_install_auth_popup_params(&self) -> Option<SelectionViewParams> {
        ...
        let status_label = if is_installed {
            ...
            "Complete the required setup to continue:"
        };
        ...
        if let Some(install_url) = app.install_url.clone() {
            ...
            items.push(SelectionItem {
                ...
                description: Some("Open the setup page".to_string()),
            ...
            items.push(SelectionItem {
                name: "Setup link unavailable".to_string(),
    ...
    fn plugin_detail_error_popup_params(
        ...
        let mut header = ColumnRenderable::new();
        header.push(Line::from("Plugins".bold()));
        header.push(Line::from("Plugin action failed.".dim()));

        let mut items = vec![SelectionItem {
            name: "Plugin action failed".to_string(),
            description: Some(err.to_string()),
            is_disabled: true,
            ..Default::default()
        }];
    ...
    fn plugins_popup_params(
        ...
        let installed_entries = all_entries
            ...
            .cloned()
            .collect();
        let all_plugins_empty_description = if marketplaces.is_empty() {
            OPENAGERE_MARKETPLACE_SYNCING_DESCRIPTION
        } else {
            "No plugins are available in the discovered marketplaces."
        };
        let all_plugins_empty_name = if marketplaces.is_empty() {
            OPENAGERE_MARKETPLACE_SYNCING_TITLE
        } else {
        ...
        tabs.push(SelectionTab {
            ...
            items: self.plugin_selection_items(
                ...
                all_plugins_empty_name,
                all_plugins_empty_description,
            ),
        });
        ...
        let curated_marketplace = marketplaces
            .iter()
            .find(|marketplace| is_default_curated_marketplace(marketplace.name.as_str()))
            .copied();
        ...
        let curated_installed = curated_entries
            ...
            .filter(|(_, plugin, _)| plugin.installed)
            .count();
        let curated_empty_description = if curated_marketplace.is_none() {
            OPENAGERE_MARKETPLACE_SYNCING_DESCRIPTION
        } else {
            "No OpenAgere Marketplace plugins available."
        };
        let curated_empty_name = if curated_marketplace.is_none() {
            OPENAGERE_MARKETPLACE_SYNCING_TITLE
        } else {
            "No OpenAgere Marketplace plugins available"
        };
        tabs.push(SelectionTab {
            id: OPENAGERE_MARKETPLACE_TAB_ID.to_string(),
            label: OPENAGERE_MARKETPLACE_DISPLAY_NAME.to_string(),
            header: plugins_header(
                format!("{OPENAGERE_MARKETPLACE_DISPLAY_NAME}."),
                format!(
                    "Installed {curated_installed} of {curated_total} OpenAgere Marketplace plugins."
                ),
            ),
            items: self.plugin_selection_items(
                curated_entries,
        ...
        let mut additional_marketplaces: Vec<&PluginMarketplaceEntry> = marketplaces
            .iter()
            .copied()
            .filter(|marketplace| !is_default_curated_marketplace(marketplace.name.as_str()))
            .collect();
    ...
    fn plugin_detail_popup_params(
        ...
        let marketplace_label = marketplace_name_display_label(plugin.marketplace_name.as_str());
            ...
            let actions: Vec<SelectionAction> = if let Some(marketplace_path) = marketplace_path {
                vec![Box::new(move |tx| {
                    ...
                    tx.send(AppEvent::FetchPluginDetail {
                        cwd: cwd.clone(),
                        params: agere_app_server_protocol::PluginReadParams {
...
fn marketplace_display_name(marketplace: &PluginMarketplaceEntry) -> String {
    if is_default_curated_marketplace(marketplace.name.as_str()) {
        return OPENAGERE_MARKETPLACE_DISPLAY_NAME.to_string();
    }
    marketplace
        .interface
        .as_ref()
...
fn is_default_curated_marketplace(marketplace_name: &str) -> bool {
    marketplace_name == OPENAGERE_CURATED_MARKETPLACE_NAME
}

fn marketplace_name_display_label(marketplace_name: &str) -> String {
    if is_default_curated_marketplace(marketplace_name) {
        OPENAGERE_MARKETPLACE_DISPLAY_NAME.to_string()
    } else {
