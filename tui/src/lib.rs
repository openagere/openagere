...
// The standalone `agere-tui` binary prints a short help message before the
...
use crate::legacy_core::config::find_agere_home;
...
use agere_app_server_client::DEFAULT_IN_PROCESS_CHANNEL_CAPACITY;
use agere_app_server_client::InProcessAppServerClient;
use agere_app_server_client::InProcessClientStartArgs;
use agere_app_server_client::RemoteAppServerClient;
use agere_app_server_client::RemoteAppServerConnectArgs;
use agere_app_server_protocol::Account as AppServerAccount;
use agere_app_server_protocol::AuthMode as AppServerAuthMode;
use agere_app_server_protocol::ConfigWarningNotification;
use agere_app_server_protocol::Thread as AppServerThread;
use agere_app_server_protocol::ThreadListCwdFilter;
use agere_app_server_protocol::ThreadListParams;
use agere_app_server_protocol::ThreadSortKey as AppServerThreadSortKey;
...
use agere_utils_fs::canonicalize_existing_preserving_symlinks;
use agere_utils_fs::path_utils;
use app::App;
...
pub(crate) use agere_app_server_client::legacy_core;
...
mod model_catalog;
mod model_migration;
pub(crate) mod model_preset_builder;
mod multi_agents;
mod neon_frame;
mod notifications;
...
pub async fn run_main(
    ...
    let (access_mode, approval_policy) = if cli.full_auto {
        (
            Some(AccessMode::WorkspaceWrite),
            Some(AskForApproval::OnRequest),
        )
    } else if cli.dangerously_bypass_approvals {
        (
            Some(AccessMode::DangerFullAccess),
            Some(AskForApproval::Never),
        )
    } else {
        (cli.access_mode, cli.approval_policy.map(Into::into))
    };
    ...
    // `oss` model provider.
    let overrides_cli = agere_utils_cli::CliConfigOverrides { raw_overrides };
    ...
    #[allow(clippy::print_stderr)]
    let agere_home = match find_agere_home() {
        Ok(agere_home) => agere_home.to_path_buf(),
        Err(err) => {
            eprintln!("Error finding agere home: {err}");
            std::process::exit(1);
        }
    };
    ...
    let overrides = ConfigOverrides {
        ...
        agere_self_exe: arg0_paths.agere_self_exe.clone(),
        agere_linux_exe: arg0_paths.agere_linux_exe.clone(),
    ...
    // Agere CLI is officially supported on Windows.
    #[cfg(unix)]
    {
    ...
    let log_file = log_file_opts.open(log_dir.join("openagere.log"))?;

    // Wrap file in non‑blocking writer.
    let (non_blocking, _guard) = non_blocking(log_file);

    // use RUST_LOG env var, default to info for agere crates.
    let env_filter = || {
        EnvFilter::try_from_default_env().unwrap_or_else(|_| {
            EnvFilter::new("agere_core=info,agere_tui=info,agere_rmcp_client=info,agere_openai_chat_client=info")
        })
    };
    ...
    let feedback = agere_feedback::AgereFeedback::new();
...
async fn run_ratatui_app(
    ...
    config_toml: agere_config::config_toml::ConfigToml,
            ...
            match update_prompt::run_update_prompt_if_needed(&mut tui, &initial_config).await? {
                ...
                UpdatePromptOutcome::RunUpdate(action) => {
                    ...
                    return Ok(AppExitInfo {
                        token_usage: agere_protocol::protocol::TokenUsage::default(),
                        thread_id: None,
                        thread_name: None,
    ...
    let login_status = if initial_config.model_provider.requires_provider_auth {
    ...
    let should_show_onboarding = should_show_onboarding(&config_toml, &initial_config);

    // Start the global remote-template cache refresh. Fetches at TUI startup,
    // then every 5 minutes. Onboarding and /provider wizard read from this
    // cache instead of spawning their own fetches — eliminates UI flicker.
    crate::onboarding::provider_templates::spawn_background_refresh_task(None);

    let _ = login_status; // login is no longer driving onboarding directly.
    let config = if should_show_onboarding {
        let show_provider_screen =
            config_toml.model_provider.is_none() || config_toml.model.is_none();
        ...
        // If onboarding ran (provider or trust step), refresh cloud
        // requirements and rebuild config so the current process state
        // reflects persisted provider/trust changes.
        if show_provider_screen && !remote_mode {
            cloud_requirements = CloudRequirementsLoader::default();
        }

        if onboarding_result.directory_trust_decision.is_some() || show_provider_screen {
    ...
    let mut missing_session_exit = |id_str: &str, action: &str| {
        ...
        Ok(AppExitInfo {
            ...
            exit_reason: ExitReason::Fatal(format!(
                "No saved session found with ID {id_str}. Run `agere {action}` without an ID to choose from existing sessions."
            )),
        })
    };
    ...
    if let Some(w) = crate::render::highlight::set_theme_override(
        ...
        find_agere_home().ok().map(AbsolutePathBuf::into_path_buf),
    ) {
    ...
    let should_prompt_windows_restriction_nux_at_startup = cfg!(target_os = "windows")
        && trust_decision_was_made
        && config.permissions.windows_access_mode.is_none();

    let Cli {
        prompt,
        shared,
        no_alt_screen,
        open_provider_on_startup,
        ..
    } = cli;
        ...
        match tui_alternate_screen {
            ...
            AltScreenMode::Auto => {
                ...
                !matches!(
                    ...
                    Some(crate::terminal_info::Multiplexer::Zellij {})
                )
            }
        }
    }
...
async fn get_login_status(
    ...
    if !config.model_provider.requires_provider_auth {
...
fn should_show_onboarding(
    raw_toml: &agere_config::config_toml::ConfigToml,
    config: &Config,
) -> bool {
    // If there's already a configured provider with an API key, skip onboarding.
    let has_configured_provider = raw_toml.model_provider.as_ref().is_some_and(|id| {
        let archive = crate::onboarding::provider_toml::load(&config.agere_home);
        archive
            .find(id)
            .is_some_and(onboarding::provider_toml::ProviderEntry::has_api_key)
    });
    if has_configured_provider && config.active_project.trust_level.is_some() {
        return false;
    }
    let needs_provider = raw_toml.model_provider.is_none() || raw_toml.model.is_none();
    let needs_trust = config.active_project.trust_level.is_none();
    needs_provider || needs_trust
}
...
mod tests {
    ...
    use agere_protocol::protocol::RolloutItem;
    use agere_protocol::protocol::RolloutLine;
    use agere_protocol::protocol::SessionMeta;
    ...
    -> std::io::Result<()> {
        ...
        let environment_manager =
            EnvironmentManager::new(agere_exec_server::EnvironmentManagerArgs {
    ...
    #[tokio::test]
    #[serial]
    async fn windows_shows_trust_prompt_without_unelevated_access() -> std::io::Result<()> {
        let temp_dir = TempDir::new()?;
        let mut config = build_config(&temp_dir).await?;
        config.active_project = ProjectConfig { trust_level: None };
        config.set_windows_unelevated_access_mode(/*enabled*/ false);
    ...
    async fn lookup_session_target_by_name_uses_backend_title_search() -> color_eyre::Result<()> {
        ...
        let state_runtime = agere_state::StateRuntime::init(
        ...
        let mut app_server =
            AppServerSession::new(agere_app_server_client::AppServerClient::InProcess(
    ...
    #[tokio::test]
    #[serial]
    async fn windows_shows_trust_prompt_with_unelevated_access() -> std::io::Result<()> {
        let temp_dir = TempDir::new()?;
        let mut config = build_config(&temp_dir).await?;
        config.active_project = ProjectConfig { trust_level: None };
        config.set_windows_unelevated_access_mode(/*enabled*/ true);

        let should_show = should_show_trust_screen(&config);
        if cfg!(target_os = "windows") {
            assert!(
                should_show,
                "Windows trust prompt should be shown on native Windows with unelevated access restrictions enabled"
            );
        } else {
            assert!(
                should_show,
    ...
    async fn config_rebuild_changes_trust_defaults_with_cwd() -> std::io::Result<()> {
        let temp_dir = TempDir::new()?;
        let agere_home = temp_dir.path().to_path_buf();
...
"#
    ...
    async fn read_session_cwd_prefers_sqlite_when_thread_id_present() -> std::io::Result<()> {
        ...
        let runtime = agere_state::StateRuntime::init(
            config.agere_home.to_path_buf(),
            config.model_provider_id.clone(),
        )
        .await
