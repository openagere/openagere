...
async fn user_shell_cmd_ls_and_cat_in_temp_dir() {
    ...
    agere
        ...
        .await
        .unwrap();
    let msg = wait_for_event(&agere, |ev| matches!(ev, EventMsg::ExecCommandEnd(_))).await;
...
async fn user_shell_cmd_can_be_interrupted() {
    ...
    let _begin = wait_for_event_match(agere, |ev| match ev {
    ...
    .await;
    agere.submit(Op::Interrupt).await.unwrap();
...
async fn user_shell_command_does_not_replace_active_turn() -> anyhow::Result<()> {
    ...
    let _ = wait_for_event_match(&fixture.agere, |ev| match ev {
    ...
    for _ in 0..200 {
        let event = timeout(Duration::from_secs(20), fixture.agere.next_event())
            .await
...
async fn user_shell_command_history_is_persisted_and_shared_with_model() -> anyhow::Result<()> {
    ...
    let mut builder = core_test_support::test_agere::test_agere().with_config(move |config| {
        config
            .features
    ...
    #[cfg(windows)]
    let command = r#"$val = $env:AGERE_EXECUTION; if ([string]::IsNullOrEmpty($val)) { $val = 'not-set' } ; [System.Console]::Write($val)"#.to_string();
    #[cfg(not(windows))]
    let command = r#"sh -c "printf '%s' \"${AGERE_EXECUTION:-not-set}\"""#.to_string();
    ...
    let begin_event = wait_for_event_match(&test.agere, |ev| match ev {
    ...
    let delta_event = wait_for_event_match(&test.agere, |ev| match ev {
    ...
    let end_event = wait_for_event_match(&test.agere, |ev| match ev {
    ...
    let _ = wait_for_event(&test.agere, |ev| matches!(ev, EventMsg::TurnComplete(_))).await;
...
#[tokio::test]
async fn user_shell_command_does_not_set_network_disabled_env_var() -> anyhow::Result<()> {
    let server = responses::start_mock_server().await;
    let mut builder = core_test_support::test_agere::test_agere().with_config(|config| {
        let file_system_access_policy = config.permissions.file_system_access_policy();
    ...
    #[cfg(windows)]
    let command = r#"$val = $env:AGERE_NETWORK_DISABLED; if ([string]::IsNullOrEmpty($val)) { $val = 'not-set' } ; [System.Console]::Write($val)"#.to_string();
    #[cfg(not(windows))]
    let command = r#"sh -c "printf '%s' \"${AGERE_NETWORK_DISABLED:-not-set}\"""#.to_string();
    ...
    let ExecCommandEndEvent {
        ...
        stderr,
        ..
    } = wait_for_event_match(&test.agere, |ev| match ev {
        EventMsg::ExecCommandEnd(event) => Some(event.clone()),
        _ => None,
    })
    .await;
...
async fn user_shell_command_output_is_truncated_in_history() -> anyhow::Result<()> {
    let server = responses::start_mock_server().await;
    let builder = core_test_support::test_agere::test_agere();
    let test = builder
        .with_config(|config| {
