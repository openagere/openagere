...
use crate::protocol_event::agere_turn_trace_event;
...
pub const AGERE_ROLLOUT_TRACE_ROOT_ENV: &str = "AGERE_ROLLOUT_TRACE_ROOT";
...
impl ThreadTraceContext {
    ...
    /// Starts a root thread trace from `AGERE_ROLLOUT_TRACE_ROOT`, or disables tracing.
    ///
    /// Trace startup is best-effort. A tracing failure must not make the Agere
    ...
    pub fn start_root_or_disabled(metadata: ThreadStartedTraceMetadata) -> Self {
        let Some(root) = std::env::var_os(AGERE_ROLLOUT_TRACE_ROOT_ENV) else {
    ...
    /// Emits typed Agere turn lifecycle events from protocol lifecycle events.
    pub fn record_agere_turn_event(&self, default_turn_id: &str, event: &EventMsg) {
        let ThreadTraceContextState::Enabled(context) = &self.state else {
            return;
        };
        let Some(trace_event) =
            agere_turn_trace_event(context.thread_id.clone(), default_turn_id, event)
        else {
            return;
        };
    ...
    /// what Agere did while executing that boundary.
    pub fn record_tool_call_event(&self, agere_turn_id: impl Into<AgereTurnId>, event: &EventMsg) {
        let ThreadTraceContextState::Enabled(context) = &self.state else {
            return;
        };
        ...
        context.append_with_context_best_effort(agere_turn_id.into(), payload);
    }
    ...
    pub fn record_agent_result_interaction(
        &self,
        child_agere_turn_id: impl Into<AgereTurnId>,
    ...
    pub fn record_agere_turn_started(&self, agere_turn_id: impl Into<AgereTurnId>) {
        let ThreadTraceContextState::Enabled(context) = &self.state else {
            return;
        };
        let agere_turn_id = agere_turn_id.into();
        context.append_with_context_best_effort(
            agere_turn_id.clone(),
            RawTraceEventPayload::AgereTurnStarted {
                agere_turn_id,
                thread_id: context.thread_id.clone(),
            },
        );
    ...
    /// Builds reusable inference trace context for one Agere turn.
    ///
...
impl EnabledThreadTraceContext {
    ...
    fn append_with_context_best_effort(
        &self,
        agere_turn_id: AgereTurnId,
        payload: RawTraceEventPayload,
    ) {
        let event_context = RawTraceEventContext {
            thread_id: Some(self.thread_id.clone()),
            agere_turn_id: Some(agere_turn_id),
        };
