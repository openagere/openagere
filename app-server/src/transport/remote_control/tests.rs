...
use agere_app_server_protocol::RemoteControlConnectionStatus;
use agere_app_server_protocol::RemoteControlStatusChangedNotification;
...
use agere_core::test_support::auth_manager_from_auth_with_home;
...
fn remote_control_auth_manager() -> Arc<AuthManager> {
    auth_manager_from_auth(AgereAuth::create_dummy_chatgpt_auth_for_testing())
}

fn remote_control_auth_manager_with_home(agere_home: &TempDir) -> Arc<AuthManager> {
...
fn remote_control_auth_dot_json(_account_id: Option<&str>) -> AuthDotJson {
    AuthDotJson {
        openai_api_key: Some("test-remote-control-api-key".to_string()),
    }
}

async fn remote_control_state_runtime(agere_home: &TempDir) -> Arc<StateRuntime> {
    StateRuntime::init(agere_home.path().to_path_buf(), "test-provider".to_string())
        .await
...
async fn remote_control_transport_manages_virtual_clients_and_routes_messages() {
    ...
    let agere_home = TempDir::new().expect("temp dir should create");
    let (transport_event_tx, mut transport_event_rx) =
        mpsc::channel::<TransportEvent>(CHANNEL_CAPACITY);
    let shutdown_token = CancellationToken::new();
    let (remote_task, remote_handle) = start_remote_control(
        remote_control_url,
        Some(remote_control_state_runtime(&agere_home).await),
        ...
        ClientEnvelope {
            event: ClientEvent::ClientMessage {
                message: JSONRPCMessage::Notification(
                    agere_app_server_protocol::JSONRPCNotification {
    ...
    let initialize_message = JSONRPCMessage::Request(agere_app_server_protocol::JSONRPCRequest {
        id: agere_app_server_protocol::RequestId::Integer(1),
    ...
    let followup_message =
        JSONRPCMessage::Notification(agere_app_server_protocol::JSONRPCNotification {
            method: "initialized".to_string(),
            params: None,
        });
    send_client_event(
...
async fn remote_control_transport_reconnects_after_disconnect() {
    ...
    send_client_event(
        ...
        ClientEnvelope {
            event: ClientEvent::ClientMessage {
                message: JSONRPCMessage::Request(agere_app_server_protocol::JSONRPCRequest {
                    id: agere_app_server_protocol::RequestId::Integer(2),
                    method: "initialize".to_string(),
                    params: Some(json!({
                        "clientInfo": {
                            "name": "remote-test-client",
...
async fn remote_control_http_mode_enrolls_before_connecting() {
    ...
    assert_eq!(
        handshake_request.headers.get("x-agere-server-id"),
        Some(&"srv_e_test".to_string())
    );
    assert_eq!(
        handshake_request.headers.get("x-agere-name"),
        Some(&base64::engine::general_purpose::STANDARD.encode(&expected_server_name))
    );
    assert_eq!(
        handshake_request.headers.get("x-agere-protocol-version"),
    ...
    let writer = {
        let initialize_message =
            JSONRPCMessage::Request(agere_app_server_protocol::JSONRPCRequest {
                id: agere_app_server_protocol::RequestId::Integer(11),
                method: "initialize".to_string(),
                params: Some(json!({
                    "clientInfo": {
...
async fn remote_control_http_mode_reuses_persisted_enrollment_before_reenrolling() {
    ...
    let remote_control_url = remote_control_url_for_listener(&listener);
    let agere_home = TempDir::new().expect("temp dir should create");
    let state_db = remote_control_state_runtime(&agere_home).await;
    ...
    let (remote_task, _remote_handle) = start_remote_control(
        remote_control_url,
        Some(state_db.clone()),
        remote_control_auth_manager_with_home(&agere_home),
        transport_event_tx,
        shutdown_token.clone(),
        /*app_server_client_name_rx*/ None,
        /*initial_enabled*/ true,
...
// Note: This test requires the full ChatGPT auth flow which was simplified to API key only.
#[tokio::test]
#[ignore]
async fn disabled_remote_control_waits_for_account_id_before_enrolling() {
    let listener = TcpListener::bind("127.0.0.1:0")
        .await
        .expect("listener should bind");
    let remote_control_url = remote_control_url_for_listener(&listener);
...
async fn remote_control_http_mode_clears_stale_persisted_enrollment_after_404() {
    ...
    assert_eq!(
        websocket_request.headers.get("x-agere-server-id"),
