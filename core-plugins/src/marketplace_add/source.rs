...
pub(crate) fn parse_marketplace_source(
    ...
    if let Some(npm_source) = source.strip_prefix("npm:") {
        if explicit_ref.is_some() {
            return Err(MarketplaceAddError::InvalidRequest(
                "--ref is only supported for git marketplace sources".to_string(),
            ));
        }
        let (package, version) = parse_npm_package_spec(npm_source)?;
    ...
    F: Fn(&str, Option<&str>, &[String], &Path) -> Result<(), MarketplaceAddError>,
{
    ...
    match source {
        ...
        MarketplaceSource::Npm { .. } => Err(MarketplaceAddError::InvalidRequest(
            "npm marketplace source staging is not implemented".to_string(),
        )),
    }
}
...
fn parse_npm_package_spec(source: &str) -> Result<(String, Option<String>), MarketplaceAddError> {
    let source = source.trim();
    if source.is_empty() {
        return Err(MarketplaceAddError::InvalidRequest(
            "npm marketplace package must not be empty".to_string(),
        ));
    }

    let (package, version) = if source.starts_with('@') {
        let Some(scope_separator) = source.find('/') else {
            return Err(MarketplaceAddError::InvalidRequest(
                "scoped npm marketplace package must include a package name".to_string(),
            ));
        };
        let version_separator = source[scope_separator + 1..]
            .find('@')
            .map(|index| scope_separator + 1 + index);
        match version_separator {
            Some(index) => (&source[..index], non_empty_ref(&source[index + 1..])),
            None => (source, None),
        }
    } else if let Some((package, version)) = source.rsplit_once('@') {
        (package, non_empty_ref(version))
    } else {
        (source, None)
    };
    ...
    if let Some(version) = &version {
        validate_npm_version_selector(version)?;
    }
    Ok((package.to_string(), version))
}

fn validate_npm_package_name(package: &str) -> Result<(), MarketplaceAddError> {
    let valid = if let Some(rest) = package.strip_prefix('@') {
        let mut segments = rest.split('/');
        let scope = segments.next();
        let name = segments.next();
        let extra = segments.next();
        scope.is_some_and(is_npm_package_segment)
            && name.is_some_and(is_npm_package_segment)
            && extra.is_none()
    } else {
        is_npm_package_segment(package)
    };
    if valid {
        Ok(())
    } else {
        Err(MarketplaceAddError::InvalidRequest(format!(
            "invalid npm marketplace package name: {package}"
        )))
    }
}

fn validate_npm_version_selector(version: &str) -> Result<(), MarketplaceAddError> {
    if version
        .chars()
        .all(|ch| ch.is_ascii_alphanumeric() || matches!(ch, '-' | '_' | '.' | '+'))
    {
...
mod tests {
    ...
    #[test]
    fn npm_source_parses_package_and_version() {
        assert_eq!(
            parse_marketplace_source("npm:@openagere/plugins@0.0.1", /*explicit_ref*/ None)
                .unwrap(),
            MarketplaceSource::Npm {
                package: "@openagere/plugins".to_string(),
    ...
    #[test]
    fn npm_source_defaults_to_latest_without_version() {
        assert_eq!(
            parse_marketplace_source("npm:@openagere/plugins", /*explicit_ref*/ None).unwrap(),
            MarketplaceSource::Npm {
                package: "@openagere/plugins".to_string(),
                version: None,
                registry: None,
    ...
    #[test]
    fn npm_source_rejects_ref_override() {
        let err = parse_marketplace_source("npm:@openagere/plugins", Some("main".to_string()))
            .unwrap_err();
