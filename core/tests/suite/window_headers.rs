...
async fn window_id_advances_after_compact_persists_on_resume_and_resets_on_fork() -> Result<()> {
    ...
    let initial_thread = Arc::clone(&initial.agere);
    ...
    let resumed = builder
        ...
        .await?;
    submit_user_turn(&resumed.agere, "after resume").await?;
    shutdown_thread(&resumed.agere).await?;
...
async fn submit_user_turn(agere: &Arc<AgereThread>, text: &str) -> Result<()> {
    agere
        .submit(Op::UserInput {
            environments: None,
...
async fn submit_compact_turn(agere: &Arc<AgereThread>) -> Result<()> {
    agere.submit(Op::Compact).await?;
    let warning_event = wait_for_event(agere, |event| matches!(event, EventMsg::Warning(_))).await;
...
async fn shutdown_thread(agere: &Arc<AgereThread>) -> Result<()> {
...
fn window_id_parts(request: &ResponsesRequest) -> (String, u64) {
    let window_id = request
        .header("x-agere-window-id")
        .expect("missing x-agere-window-id header");
