...
use agere_mcp::collect_mcp_snapshot_from_manager;
...
use agere_protocol::protocol::SkillsListEntry;
    ...
    let (items, updates, responsesapi_client_metadata) = match op {
        ...
        Op::UserInputWithTurnContext {
            ...
            if model_provider.is_some() {
                // Match `/model` semantics: model/provider runtime switches are not
                // queued behind an active turn. Rejecting keeps the user's mental
                // model simple (a switch either applies to the new turn being started
                // now, or it does not happen) and prevents delayed provider mutation
                // from being mistaken for an immediate switch.
                let has_active_turn = sess.active_turn.lock().await.is_some();
                if has_active_turn {
                    sess.clear_pending_model_provider_for(&sub_id);
                    sess.send_event_raw(Event {
                        id: sub_id,
                        msg: EventMsg::Error(ErrorEvent {
                            message: "model provider overrides are only supported when starting a new turn"
                                .to_string(),
                            agere_error_info: Some(AgereErrorInfo::BadRequest),
                        }),
                    })
            ...
            // Take the provider staged by the app-server boundary (it carries the
            // FRESH ModelProviderInfo, resolved against the latest config, so it
            // sees runtime-added providers). The Op's `model_provider` is only a
            // trigger/marker. Apply only when the staged id actually differs from
            // the currently-active provider, to avoid needless re-swaps (and
            // websocket resets) on turns that merely echo the current id.
            let (provider, provider_config) = if let Some(requested_provider_id) = model_provider {
                match sess.take_pending_model_provider_for(&sub_id, &requested_provider_id) {
                    Some((id, info, config)) => {
                        let current_id = {
                            let state = sess.state.lock().await;
                            state.session_configuration.provider_id.clone()
                        };
                        let applied = id != current_id;
                        tracing::info!(
                            staged_id = %id,
                            current_id = %current_id,
                            applied,
                            base_url = ?info.base_url,
                            "PROVIDER_SWITCH core consuming staged provider"
                        );
                        if applied {
                            (Some((id, info)), Some(config))
                        } else {
                            (None, None)
                        }
                    }
                    None => {
                        tracing::warn!(
                            requested_id = %requested_provider_id,
                            "model_provider override present but no matching staged provider; rejecting turn"
                        );
                        sess.send_event_raw(Event {
                            id: sub_id,
                            msg: EventMsg::Error(ErrorEvent {
                                message: format!(
                                    "model provider override `{requested_provider_id}` was not staged for this turn"
                                ),
                                agere_error_info: Some(AgereErrorInfo::BadRequest),
                            }),
                        })
...
pub async fn resolve_elicitation(
    ...
    decision: agere_protocol::approvals::ElicitationAction,
    content: Option<Value>,
    meta: Option<Value>,
) {
    let action = match decision {
        agere_protocol::approvals::ElicitationAction::Accept => ElicitationAction::Accept,
        agere_protocol::approvals::ElicitationAction::Decline => ElicitationAction::Decline,
        agere_protocol::approvals::ElicitationAction::Cancel => ElicitationAction::Cancel,
    };
    ...
    tokio::spawn(async move {
        ...
        let event = Event {
            ...
            msg: EventMsg::GetHistoryEntryResponse(
                agere_protocol::protocol::GetHistoryEntryResponseEvent {
                    offset,
                    log_id,
                    entry: entry_opt.map(|e| agere_protocol::message_history::HistoryEntry {
...
pub async fn list_skills(sess: &Session, sub_id: String, cwds: Vec<PathBuf>, force_reload: bool) {
    ...
    let agere_home = sess.agere_home().await;
    ...
    if let Err(err) = sess.flush_rollout().await {
        sess.send_event(
            ...
            EventMsg::Warning(WarningEvent {
                message: format!(
                    "Rolled the thread back, but failed to save the rollback marker. Agere will continue retrying. Error: {err}"
                ),
            }),
        )
        .await;
...
pub async fn set_thread_name(sess: &Arc<Session>, sub_id: String, name: String) {
    ...
    let agere_home = sess.agere_home().await;
    if let Err(err) =
        crate::rollout::append_thread_name(&agere_home, sess.conversation_id, &name).await
    {
...
pub async fn shutdown(sess: &Arc<Session>, sub_id: String) -> bool {
    ...
    info!("Shutting down Agere instance");
    ...
    sess.services
        .rollout_thread_trace
        .record_ended(agere_rollout_trace::RolloutStatus::Completed);
    true
}
