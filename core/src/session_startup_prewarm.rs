...
use agere_otel::STARTUP_PREWARM_AGE_AT_FIRST_TURN_METRIC;
use agere_otel::STARTUP_PREWARM_DURATION_METRIC;
...
pub(crate) struct SessionStartupPrewarmHandle {
    task: JoinHandle<AgereResult<ModelClientSession>>,
    started_at: Instant,
    timeout: Duration,
}
...
impl SessionStartupPrewarmHandle {
    ...
    fn resolution_from_join_result(
        result: std::result::Result<AgereResult<ModelClientSession>, tokio::task::JoinError>,
        started_at: Instant,
    ) -> SessionStartupPrewarmResolution {
        match result {
...
) -> AgereResult<ModelClientSession> {
