...
- recursive grep: ["powershell.exe", "-Command", "rg 'TODO'"]
...
fn create_approval_parameters(
    ...
    let mut properties = BTreeMap::from([
        (
            ...
            JsonSchema::string(Some(
                if exec_permission_approvals_enabled {
                    "Permission level for the command. Use \"with_additional_permissions\" to request extra filesystem or network access within managed limits (preferred), or \"require_escalated\" to ask for less restricted execution; defaults to \"use_default\"."
                } else {
                    "Permission level for the command. Set to \"require_escalated\" to ask for less restricted execution; defaults to \"use_default\"."
                }
                .to_string(),
            )),
        ),
        (
            ...
            JsonSchema::string(Some(
                r#"Only set if permission_level is \"require_escalated\".
                    Request approval from the user to run this command with escalated access.
        ...
        (
            ...
            JsonSchema::array(JsonSchema::string(/*description*/ None), Some(
                    r#"Only specify when permission_level is `require_escalated`.
