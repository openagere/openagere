//! Unified Exec: interactive process execution orchestrated with approvals and policy.
//!
//! Responsibilities
//! - Manages interactive processes (create, reuse, buffer output with caps).
//! - Uses the shared ToolOrchestrator to handle approval, retries, and policy in
//!   a single, descriptive flow.
//! - Spawns the PTY from an `ExecRequest`; on policy denial, may retry with
//!   adjusted permissions when allowed (with caching to avoid redundant prompts).
//! - Uses the shared `is_likely_policy_denial` heuristic to keep denial messages
//!   consistent with other exec paths.
//!
//! Flow at a glance (open process)
//! 1) Build a small request `{ command, cwd }`.
//! 2) Orchestrator: approval (bypass/cache/prompt) → run.
//! 3) Runtime: `ExecRequest` → spawn PTY.
//! 4) If denial, orchestrator may retry with relaxed restrictions.
...
//! - `process_manager.rs`: orchestration (approvals, reuse) and request handling.
