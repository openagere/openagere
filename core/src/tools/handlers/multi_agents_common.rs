...
pub(crate) fn collab_spawn_error(err: AgereErr) -> FunctionCallError {
    match err {
        AgereErr::UnsupportedOperation(message) if message == "thread manager dropped" => {
            FunctionCallError::RespondToModel("collab manager unavailable".to_string())
        }
        AgereErr::UnsupportedOperation(message) => FunctionCallError::RespondToModel(message),
        err => FunctionCallError::RespondToModel(format!("collab spawn failed: {err}")),
    }
}

pub(crate) fn collab_agent_error(agent_id: ThreadId, err: AgereErr) -> FunctionCallError {
    match err {
        AgereErr::ThreadNotFound(id) => {
...
/// approval policy, permission profile, and cwd. Role-specific overrides are layered after this step;
...
/// can make a child agent disagree with its parent about approval policy, cwd, or access policy.
pub(crate) fn apply_spawn_agent_runtime_overrides(
    ...
    config.agere_linux_exe = turn.agere_linux_exe.clone();
...
fn find_spawn_agent_model_name(
    available_models: &[agere_protocol::openai_models::ModelPreset],
    requested_model: &str,
