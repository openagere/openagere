...
use agere_app_server_protocol::McpResourceContent;
...
const TEST_RESOURCE_URI: &str = "test://agere/resource";
const TEST_BLOB_RESOURCE_URI: &str = "test://agere/resource.bin";
...
async fn start_resource_apps_mcp_server() -> Result<(String, JoinHandle<()>)> {
    ...
    let router = Router::new().nest_service("/api/agere/apps", mcp_service);
