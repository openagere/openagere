...
/// directory. If you need Agere to work from such a checkout simply pass the
...
pub async fn resolve_root_git_project_for_trust(
    ...
    if fs.get_metadata(&dot_git).await.ok()?.is_directory {
        return Some(repo_root);
    }

    let git_dir_s = fs.read_file_text(&dot_git).await.ok()?;
