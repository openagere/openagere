...
fn workspace_write_excluding_tmp() -> String {
...
async fn with_additional_permissions_requires_approval_under_on_request() -> Result<()> {
    ...
    submit_turn(&test, call_id, approval_policy, access_policy.clone()).await?;
...
async fn workspace_write_with_additional_permissions_can_write_outside_cwd() -> Result<()> {
    ...
    let server = start_mock_server().await;
    let approval_policy = AskForApproval::OnRequest;
    let access_policy = workspace_write_excluding_tmp();
    let access_policy_for_config = access_policy.clone();
...
async fn request_permissions_grants_apply_to_later_exec_command_calls() -> Result<()> {
    ...
    submit_turn(&test, "write outside the workspace", approval_policy).await?;
...
async fn request_permissions_session_grants_carry_across_turns() -> Result<()> {
    ...
    let completion_event = wait_for_event(&test.agere, |event| {
        matches!(
            event,
            EventMsg::ExecApprovalRequest(_) | EventMsg::TurnComplete(_)
        )
