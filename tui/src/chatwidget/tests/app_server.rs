    ...
    chat.handle_server_notification(
        ServerNotification::Error(ErrorNotification {
            error: AppServerTurnError {
                ...
                agere_error_info: Some(AgereErrorInfo::Other.into()),
                additional_details: None,
            },
            will_retry: true,
            thread_id: "thread-1".to_string(),
    ...
    chat.handle_server_notification(
        ServerNotification::Error(ErrorNotification {
            error: AppServerTurnError {
                ...
                agere_error_info: Some(AgereErrorInfo::ServerOverloaded.into()),
                additional_details: None,
            },
            will_retry: false,
            thread_id: "thread-1".to_string(),
    ...
    chat.handle_server_notification(
        ServerNotification::Error(ErrorNotification {
            error: AppServerTurnError {
                ...
                agere_error_info: Some(AgereErrorInfo::CyberPolicy.into()),
                additional_details: None,
            },
            will_retry: false,
            thread_id: "thread-1".to_string(),
...
#[tokio::test]
async fn live_app_server_cyber_policy_error_uses_generic_notice_for_non_openai_provider() {
    let (mut chat, mut rx, _op_rx) = make_chatwidget_manual(/*model_override*/ None).await;
...
#[tokio::test]
async fn live_app_server_model_verification_uses_generic_warning_for_non_openai_provider() {
    let (mut chat, mut rx, _op_rx) = make_chatwidget_manual(/*model_override*/ None).await;
    use_non_openai_provider(&mut chat);
...
async fn live_app_server_invalid_thread_name_update_is_ignored() {
    ...
    chat.handle_server_notification(
        ServerNotification::ThreadNameUpdated(
            agere_app_server_protocol::ThreadNameUpdatedNotification {
...
async fn live_app_server_thread_name_update_shows_resume_hint() {
    ...
    assert!(rendered.contains("openagere resume review-fix"));
}
