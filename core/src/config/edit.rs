...
pub enum ConfigEdit {
    ...
    /// Toggle the opt-out marker for Agere-managed fast defaults.
    ...
    /// Replace the entire `[[models]]` array of tables. An empty vec clears it.
    ReplaceModelsTable(Vec<ModelConfig>),
...
impl ConfigDocument {
    ...
    fn apply(&mut self, edit: &ConfigEdit) -> anyhow::Result<bool> {
        match edit {
            ConfigEdit::SetModel {
                ...
                mutated |=
                    self.write_profile_value(&["model_context_window"], context_window.map(value));
                mutated
            }),
            ...
            ConfigEdit::ReplaceModelsTable(models) => Ok(self.replace_models_table(models)),
    ...
    fn replace_models_table(&mut self, models: &[ModelConfig]) -> bool {
        if models.is_empty() {
            return self.clear(Scope::Global, &["models"]);
        }
        ...
        for model in models {
            let mut table = TomlTable::new();
            table.set_implicit(false);
            table["name"] = value(model.name.clone());
            if let Some(context_window) = model.context_window {
                table["context_window"] = value(context_window);
            }
            array.push(table);
        }
        root.insert("models", TomlItem::ArrayOfTables(array));
        true
    }
...
pub async fn apply(
    ...
    let agere_home = agere_home.to_path_buf();
    let profile = profile.map(ToOwned::to_owned);
    task::spawn_blocking(move || apply_blocking(&agere_home, profile.as_deref(), &edits))
        .await
...
impl ConfigEditsBuilder {
    pub fn new(agere_home: &Path) -> Self {
        Self {
    ...
    pub fn replace_models_table(mut self, models: Vec<ModelConfig>) -> Self {
        self.edits.push(ConfigEdit::ReplaceModelsTable(models));
        self
    }
    ...
    pub fn set_windows_access_mode(mut self, mode: &str) -> Self {
        let segments = if let Some(profile) = self.profile.as_ref() {
            vec![
                "profiles".to_string(),
                profile.clone(),
            ...
            vec!["windows".to_string(), "access_mode".to_string()]
        };
        self.edits.push(ConfigEdit::SetPath {
            segments,
            value: value(mode),
    ...
    pub fn apply_blocking(self) -> anyhow::Result<()> {
        apply_blocking(&self.agere_home, self.profile.as_deref(), &self.edits)
    }
