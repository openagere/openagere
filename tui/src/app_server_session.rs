...
use agere_app_server_client::AppServerClient;
use agere_app_server_client::AppServerEvent;
use agere_app_server_client::AppServerRequestHandle;
use agere_app_server_client::TypedRequestError;
use agere_app_server_protocol::Account;
use agere_app_server_protocol::AuthMode;
use agere_app_server_protocol::ClientRequest;
use agere_app_server_protocol::ConfigBatchWriteParams;
use agere_app_server_protocol::ConfigWriteResponse;
...
use agere_app_server_protocol::Model as ApiModel;
...
use agere_app_server_protocol::ReviewDelivery;
...
use agere_app_server_protocol::ThreadStartSource;
...
use agere_protocol::openai_models::ModelAvailabilityNux;
use agere_protocol::openai_models::ModelPreset;
use agere_protocol::openai_models::ModelUpgrade;
...
pub(crate) struct AppServerBootstrap {
    ...
    pub(crate) plan_type: Option<agere_protocol::account::PlanType>,
    /// Whether the configured model provider needs account-style auth. Combined
    /// with `has_chatgpt_account` to decide if a startup rate-limit prefetch
    /// should be fired.
    pub(crate) requires_provider_auth: bool,
...
pub(crate) struct ThreadSessionState {
    ...
    pub(crate) service_tier: Option<agere_protocol::config_types::ServiceTier>,
    pub(crate) approval_policy: AskForApproval,
    pub(crate) approvals_reviewer: agere_protocol::config_types::ApprovalsReviewer,
    ...
    pub(crate) reasoning_effort: Option<agere_protocol::openai_models::ReasoningEffort>,
...
impl AppServerSession {
    ...
    pub(crate) async fn bootstrap(&mut self, config: &Config) -> Result<AppServerBootstrap> {
        ...
        Ok(AppServerBootstrap {
            ...
            status_account_display,
            plan_type,
            requires_provider_auth: account.requires_provider_auth,
            default_model,
            feedback_audience,
            has_chatgpt_account,
            available_models,
    ...
    /// Fetches the available models list from the app server.
    ...
    pub(crate) async fn turn_start(
        &mut self,
        thread_id: ThreadId,
        items: Vec<agere_protocol::user_input::UserInput>,
        cwd: PathBuf,
        ...
        let request_id = self.next_request_id();
        let access_policy = if matches!(self.thread_params_mode(), ThreadParamsMode::Remote) {
            let isolation = permission_profile.filesystem_isolation();
            let policy = match isolation {
                agere_protocol::models::FilesystemIsolation::Disabled => {
                    agere_app_server_protocol::AccessPolicy::DangerFullAccess
                }
                agere_protocol::models::FilesystemIsolation::External => {
                    agere_app_server_protocol::AccessPolicy::External {
                        network_access: agere_app_server_protocol::NetworkAccess::Restricted,
                    }
                }
                agere_protocol::models::FilesystemIsolation::Managed => {
                    agere_app_server_protocol::AccessPolicy::WorkspaceWrite {
                        writable_roots: vec![],
...
pub(crate) fn status_account_display_from_auth_mode(
    ...
    plan_type: Option<agere_protocol::account::PlanType>,
...
fn approvals_reviewer_override_from_config(
    config: &Config,
) -> Option<agere_app_server_protocol::ApprovalsReviewer> {
...
fn access_mode_from_permission_profile(
    permission_profile: &PermissionProfile,
    cwd: &std::path::Path,
) -> Option<agere_app_server_protocol::AccessMode> {
    match permission_profile {
        PermissionProfile::Disabled => {
            Some(agere_app_server_protocol::AccessMode::DangerFullAccess)
        }
        ...
        PermissionProfile::Managed { .. } => {
            ...
            if file_system_policy.has_full_disk_write_access() {
                permission_profile
                    ...
                    .is_enabled()
                    .then_some(agere_app_server_protocol::AccessMode::DangerFullAccess)
            } else if file_system_policy.can_write_path_with_cwd(cwd, cwd) {
                Some(agere_app_server_protocol::AccessMode::WorkspaceWrite)
            } else {
                Some(agere_app_server_protocol::AccessMode::ReadOnly)
            }
        }
    }
}
...
fn thread_start_params_from_config(
    ...
    let access_mode = permission_profile
        .is_none()
        .then(|| {
            access_mode_from_permission_profile(
...
async fn thread_session_state_from_thread_start_response(
    ...
    thread_session_state_from_thread_response(
        ...
        response
            ...
            .map(Into::into)
            .unwrap_or_else(|| {
                PermissionProfile::from_legacy_access_policy_for_cwd(
                    &response.access_policy.to_core(),
...
) -> agere_app_server_protocol::ReviewTarget {
    match target {
        CoreReviewTarget::UncommittedChanges => {
            agere_app_server_protocol::ReviewTarget::UncommittedChanges
        }
        CoreReviewTarget::BaseBranch { branch } => {
            agere_app_server_protocol::ReviewTarget::BaseBranch { branch }
        }
        CoreReviewTarget::Commit { sha, title } => {
            agere_app_server_protocol::ReviewTarget::Commit { sha, title }
        }
        CoreReviewTarget::Custom { instructions } => {
            agere_app_server_protocol::ReviewTarget::Custom { instructions }
        }
    }
}
...
async fn thread_session_state_from_thread_response(
    ...
    service_tier: Option<agere_protocol::config_types::ServiceTier>,
    approval_policy: AskForApproval,
    approvals_reviewer: agere_protocol::config_types::ApprovalsReviewer,
    permission_profile: PermissionProfile,
    cwd: AbsolutePathBuf,
    instruction_source_paths: Vec<AbsolutePathBuf>,
    reasoning_effort: Option<agere_protocol::openai_models::ReasoningEffort>,
    config: &Config,
) -> Result<ThreadSessionState, String> {
...
pub(crate) fn app_server_rate_limit_snapshot_to_core(
    snapshot: agere_app_server_protocol::RateLimitSnapshot,
) -> RateLimitSnapshot {
    RateLimitSnapshot {
...
fn app_server_rate_limit_window_to_core(
    window: agere_app_server_protocol::RateLimitWindow,
) -> RateLimitWindow {
    RateLimitWindow {
...
fn app_server_credits_snapshot_to_core(
    snapshot: agere_app_server_protocol::CreditsSnapshot,
) -> CreditsSnapshot {
    CreditsSnapshot {
...
mod tests {
    ...
    async fn build_config(temp_dir: &TempDir) -> Config {
        ConfigBuilder::default()
            .agere_home(temp_dir.path().to_path_buf())
            .build()
            .await
    ...
    async fn thread_start_params_include_cwd_for_embedded_sessions() {
        ...
        assert_eq!(params.access_mode, None);
        assert_eq!(
    ...
    #[tokio::test]
    async fn thread_resume_params_forward_model_overrides_for_embedded_sessions() {
        let temp_dir = tempfile::tempdir().expect("tempdir");
    ...
    async fn thread_lifecycle_params_omit_cwd_without_remote_override_for_remote_sessions() {
        ...
        let _expected_access_mode = access_mode_from_permission_profile(
            &config.permissions.permission_profile(),
            config.cwd.as_path(),
        );
    ...
    #[tokio::test]
    async fn turn_params_omit_model_provider_for_remote_sessions() {
        let temp_dir = tempfile::tempdir().expect("tempdir");
        let config = build_config(&temp_dir).await;

        assert_eq!(
            ThreadParamsMode::Embedded.model_provider_from_config(&config),
            Some(config.model_provider_id.clone())
        );
        assert_eq!(
            ThreadParamsMode::Remote.model_provider_from_config(&config),
            None
        );
        assert_eq!(
    ...
    #[test]
    fn access_mode_does_not_project_non_cwd_write_roots_for_remote_sessions() {
        ...
        assert_eq!(
            access_mode_from_permission_profile(&permission_profile, cwd.as_path()),
            Some(agere_app_server_protocol::AccessMode::ReadOnly)
        );
    }

    #[test]
    fn access_mode_projects_cwd_write_for_remote_sessions() {
        let cwd = test_path_buf("/workspace/project").abs();
        let permission_profile = PermissionProfile::Managed {
            file_system: ManagedFileSystemPermissions::Restricted {
                entries: vec![
        ...
        let response = ThreadResumeResponse {
            thread: agere_app_server_protocol::Thread {
                ...
                turns: vec![Turn {
                    id: "turn-1".to_string(),
                    items: vec![
                        agere_app_server_protocol::ThreadItem::UserMessage {
                            id: "user-1".to_string(),
                            content: vec![agere_app_server_protocol::UserInput::Text {
                                text: "hello from history".to_string(),
                                text_elements: Vec::new(),
                            }],
                        },
                        agere_app_server_protocol::ThreadItem::AgentMessage {
    ...
    async fn session_configured_populates_history_metadata() {
        ...
        let session = thread_session_state_from_thread_response(
            ...
            agere_protocol::config_types::ApprovalsReviewer::User,
    ...
    fn status_account_display_from_auth_mode_uses_remapped_plan_labels() {
        let business = status_account_display_from_auth_mode(
            Some(AuthMode::Chatgpt),
            Some(agere_protocol::account::PlanType::EnterpriseCbpUsageBased),
        );
        assert!(matches!(
            business,
            Some(StatusAccountDisplay::ChatGpt {
        ...
        let team = status_account_display_from_auth_mode(
            Some(AuthMode::Chatgpt),
            Some(agere_protocol::account::PlanType::SelfServeBusinessUsageBased),
        );
        assert!(matches!(
            team,
            Some(StatusAccountDisplay::ChatGpt {
