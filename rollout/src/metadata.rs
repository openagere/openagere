...
use agere_state::BackfillState;
use agere_state::BackfillStats;
use agere_state::BackfillStatus;
use agere_state::DB_ERROR_METRIC;
use agere_state::DB_METRIC_BACKFILL;
use agere_state::DB_METRIC_BACKFILL_DURATION_MS;
use agere_state::ExtractionOutcome;
use agere_state::ThreadMetadataBuilder;
use agere_state::apply_rollout_item;
use chrono::DateTime;
...
pub(crate) fn builder_from_session_meta(
    ...
    builder.access_policy = "read-only".to_string();
...
pub(crate) async fn backfill_sessions(
    runtime: &agere_state::StateRuntime,
    config: &impl RolloutConfigView,
) {
    let metric_client = agere_otel::global();
    ...
    let sessions_root = config.agere_home().join(SESSIONS_SUBDIR);
    let archived_root = config.agere_home().join(ARCHIVED_SESSIONS_SUBDIR);
        ...
        match collect_rollout_paths(&root).await {
            Ok(paths) => {
                rollout_paths.extend(paths.into_iter().map(|path| BackfillRolloutPath {
                    watermark: backfill_watermark_for_path(config.agere_home(), &path),
                    path,
                    archived,
                }));
            }
...
fn backfill_watermark_for_path(agere_home: &Path, path: &Path) -> String {
    path.strip_prefix(agere_home)
        .unwrap_or(path)
        .to_string_lossy()
        .replace('\\', "/")
}
