...
use crate::config_requirements::AccessModeRequirement;
...
use agere_file_system::ExecutorFileSystem;
...
#[cfg(unix)]
const SYSTEM_CONFIG_TOML_FILE_UNIX: &str = "/etc/agere/config.toml";
...
/// - system    `/etc/agere/requirements.toml` (Unix) or
///   `%ProgramData%\OpenAI\Agere\requirements.toml` (Windows)
///
...
///
/// - admin:    managed preferences (*)
/// - system    `/etc/agere/config.toml` (Unix) or
///   `%ProgramData%\OpenAI\Agere\config.toml` (Windows)
/// - user      `${AGERE_HOME}/config.toml`
/// - cwd       `${PWD}/config.toml` (loaded but disabled when the directory is untrusted)
/// - tree      parent directories up to root looking for `./.openagere/config.toml` (loaded but disabled when untrusted)
/// - repo      `$(git rev-parse --show-toplevel)/.openagere/config.toml` (loaded but disabled when untrusted)
/// - runtime   e.g., --config flags, model selector in UI
///
/// (*) Only available on macOS via managed device profiles.
///
/// See https://developers.openagere.com/agere/security for details.
///
...
pub async fn load_config_layers_state(
    ...
    if let Some(requirements) = cloud_requirements.get().await.map_err(io::Error::other)? {
        merge_requirements_with_remote_access_config(
    ...
    let loaded_config_layers =
        layer_io::load_config_layers_internal(fs, agere_home, overrides.clone()).await?;
    ...
    // Add a layer for $AGERE_HOME/config.toml so folder-derived resources such
    // as rules/ can still be discovered. When user config is ignored, preserve
    // the layer metadata without reading config.toml.
    let user_file = AbsolutePathBuf::resolve_path_against_base(CONFIG_TOML_FILE, agere_home);
    ...
    if let Some(config) = managed_config_from_mdm {
        ...
        // value for base_dir, so agere_home is as good a value as any.
        let managed_config =
            resolve_relative_paths_in_config_toml(config.managed_config, agere_home)?;
...
pub async fn load_requirements_toml(
    ...
    match fs.read_file_text(requirements_toml_file).await {
        Ok(contents) => {
...
fn system_requirements_toml_file() -> io::Result<AbsolutePathBuf> {
    AbsolutePathBuf::from_absolute_path(Path::new("/etc/agere/requirements.toml"))
}
...
#[cfg(windows)]
fn windows_agere_system_dir() -> PathBuf {
...
pub(super) fn merge_requirements_with_remote_access_config(
    target: &mut ConfigRequirementsWithSources,
    source: RequirementSource,
    mut requirements: ConfigRequirementsToml,
) {
    if requirements.remote_access_config.is_some() {
        let host_name = crate::host_name();
        requirements.apply_remote_access_config(host_name.as_deref());
    }
...
fn project_layer_entry(
    ...
    let source = ConfigLayerSource::Project {
        dot_agere_folder: dot_agere_folder.clone(),
    };
...
async fn find_project_root(
    ...
    for ancestor in cwd.ancestors() {
        for marker in project_root_markers {
            ...
            if fs.get_metadata(&marker_path).await.is_ok() {
                return Ok(ancestor);
            }
        }
    }
...
async fn load_project_layers(
    ...
    let agere_home_abs = AbsolutePathBuf::from_absolute_path(agere_home)?;
    let agere_home_normalized =
        normalize_path(agere_home_abs.as_path()).unwrap_or_else(|_| agere_home_abs.to_path_buf());
    let mut dirs = cwd
        .ancestors()
        .scan(false, |done, a| {
            if *done {
    ...
    for dir in dirs {
        let dot_agere_abs = dir.join(".openagere");
        if !fs
            .get_metadata(&dot_agere_abs)
            .await
        ...
        let dot_agere_normalized =
            normalize_path(dot_agere_abs.as_path()).unwrap_or_else(|_| dot_agere_abs.to_path_buf());
        if dot_agere_abs == agere_home_abs || dot_agere_normalized == agere_home_normalized {
            continue;
        }
        let config_file = dot_agere_abs.join(CONFIG_TOML_FILE);
        match fs.read_file_text(&config_file).await {
            Ok(contents) => {
                ...
                let config =
                    resolve_relative_paths_in_config_toml(config, dot_agere_abs.as_path())?;
                let entry = project_layer_entry(&dot_agere_abs, config, disabled_reason.clone());
                layers.push(entry);
            }
            Err(err) => {
...
/// from a file like `/etc/agere/managed_config.toml` that has the same
...
impl From<LegacyManagedConfigToml> for ConfigRequirementsToml {
    fn from(legacy: LegacyManagedConfigToml) -> Self {
        ...
        if let Some(required_access_mode) = access_mode {
            let required_mode: AccessModeRequirement = required_access_mode.into();
            // Allowing read-only is a requirement for Agere to function correctly.
            // So in this backfill path, we append read-only if it's not already specified.
            let mut allowed_modes = vec![AccessModeRequirement::ReadOnly];
            if required_mode != AccessModeRequirement::ReadOnly {
                allowed_modes.push(required_mode);
            }
            config_requirements_toml.allowed_access_modes = Some(allowed_modes);
        }
        config_requirements_toml
    }
}
...
"#;
    ...
    #[test]
    fn legacy_managed_config_backfill_includes_read_only_access_mode() {
        ...
        assert_eq!(
            requirements.allowed_access_modes,
            Some(vec![
    ...
    fn windows_system_requirements_toml_file_uses_expected_suffix() {
        ...
        assert!(
            windows_system_requirements_toml_file()
                .expect("requirements.toml path")
                .as_path()
                .ends_with(Path::new("OpenAI").join("Agere").join("requirements.toml"))
        );
    }
    ...
    fn windows_system_config_toml_file_uses_expected_suffix() {
        ...
        assert!(
            windows_system_config_toml_file()
                .expect("config.toml path")
                .as_path()
                .ends_with(Path::new("OpenAI").join("Agere").join("config.toml"))
        );
    }
}
