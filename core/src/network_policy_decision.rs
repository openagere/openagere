use agere_execpolicy::Decision as ExecPolicyDecision;
use agere_execpolicy::NetworkRuleProtocol as ExecPolicyNetworkRuleProtocol;
use agere_network_proxy::BlockedRequest;
use agere_network_proxy::NetworkPolicyDecision;
...
use agere_protocol::network_policy::NetworkPolicyDecisionPayload;
...
pub(crate) fn denied_network_policy_message(blocked: &BlockedRequest) -> Option<String> {
    ...
    let detail = match blocked.reason.as_str() {
        ...
        "not_allowed" => "domain is not on the allowlist for the current access mode",
        "not_allowed_local" => "local/private network addresses are blocked by the access policy",
