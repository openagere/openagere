...
const LOGIN_ISSUER_ENV_VAR: &str = "AGERE_APP_SERVER_LOGIN_ISSUER";
...
struct CreateConfigTomlParams {
    ...
    requires_provider_auth: Option<bool>,
    base_url: Option<String>,
    model_provider_id: Option<String>,
    extra_provider_config: Option<String>,
}

fn create_config_toml(agere_home: &Path, params: CreateConfigTomlParams) -> std::io::Result<()> {
    ...
    let requires_line = match params.requires_provider_auth {
        Some(true) => "requires_provider_auth = true\n".to_string(),
...
async fn logout_account_removes_auth_and_notifies() -> Result<()> {
    ...
    create_config_toml(agere_home.path(), CreateConfigTomlParams::default())?;
    ...
    assert!(agere_home.path().join("auth.json").exists());
    ...
    assert!(
        !agere_home.path().join("auth.json").exists(),
...
async fn external_auth_refreshes_on_unauthorized() -> Result<()> {
    ...
    let thread_req = mcp
        .send_thread_start_request(agere_app_server_protocol::ThreadStartParams {
    ...
    .await??;
    let thread = to_response::<agere_app_server_protocol::ThreadStartResponse>(thread_resp)?;

    let turn_req = mcp
        .send_turn_start_request(agere_app_server_protocol::TurnStartParams {
            thread_id: thread.thread.id,
            input: vec![agere_app_server_protocol::UserInput::Text {
...
async fn login_account_chatgpt_device_code_succeeds_and_notifies() -> Result<()> {
    ...
    assert_eq!(verification_url, format!("{issuer}/agere/device"));
...
async fn get_account_no_auth() -> Result<()> {
    ...
    assert_eq!(account.requires_provider_auth, true);
    Ok(())
}
