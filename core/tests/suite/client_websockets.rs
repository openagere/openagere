...
use agere_api::WS_REQUEST_HEADER_TRACEPARENT_CLIENT_METADATA_KEY;
use agere_api::WS_REQUEST_HEADER_TRACESTATE_CLIENT_METADATA_KEY;
use agere_core::ModelClient;
use agere_core::ModelClientSession;
use agere_core::Prompt;
use agere_core::ResponseEvent;
use agere_core::X_RESPONSESAPI_INCLUDE_TIMING_METRICS_HEADER;
...
async fn responses_websocket_streams_request() {
    ...
    assert_eq!(
        ...
        Some(agere_login::default_client::get_agere_user_agent())
    );
    assert_eq!(
        body["client_metadata"]["x-agere-installation-id"].as_str(),
...
async fn responses_websocket_usage_limit_error_emits_rate_limit_event() {
    ...
    let usage_limit_error = json!({
        ...
        "headers": {
            "x-agere-primary-used-percent": "100.0",
            "x-agere-secondary-used-percent": "87.5",
            "x-agere-primary-over-secondary-limit-percent": "95.0",
            "x-agere-primary-window-minutes": "15",
            "x-agere-secondary-window-minutes": "60"
        }
    });
    ...
    let token_event =
        wait_for_event(&test.agere, |msg| matches!(msg, EventMsg::TokenCount(_))).await;
    ...
    let error_event = wait_for_event(&test.agere, |msg| matches!(msg, EventMsg::Error(_))).await;
...
async fn responses_websocket_forwards_turn_metadata_on_initial_and_incremental_create() {
    ...
    let first_turn_metadata =
        r#"{"turn_id":"turn-123","thread_source":"user","access_mode":"workspace-write"}"#;
    let enriched_turn_metadata = r#"{"turn_id":"turn-123","thread_source":"user","access_mode":"workspace-write","workspaces":[{"root_path":"/tmp/repo","latest_git_commit_hash":"abc123","associated_remote_urls":["git@github.com:openai/agere.git"],"has_changes":true}]}"#;
    let prompt_one = prompt_with_input(vec![message_item("hello")]);
    let prompt_two = prompt_with_input(vec![
        message_item("hello"),
        assistant_message_item("msg-1", "assistant output"),
    ...
    assert_eq!(first["type"].as_str(), Some("response.create"));
    assert_eq!(
        first["client_metadata"]["x-agere-turn-metadata"].as_str(),
        Some(first_turn_metadata)
    );
    assert_eq!(second["type"].as_str(), Some("response.create"));
    assert_eq!(second["previous_response_id"].as_str(), Some("resp-1"));
    assert_eq!(
        second["client_metadata"]["x-agere-turn-metadata"].as_str(),
        Some(enriched_turn_metadata)
    );
...
async fn responses_websocket_preserves_custom_turn_metadata_fields() {
    ...
    assert_eq!(body["type"].as_str(), Some("response.create"));
    assert_eq!(
        body["client_metadata"]["x-agere-turn-metadata"]
            .as_str()
...
async fn websocket_harness_with_provider_options(
    ...
    let model_info = agere_core::test_support::construct_model_info_offline(MODEL, &config);
