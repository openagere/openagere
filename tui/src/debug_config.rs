...
use agere_config::NetworkUnixSocketPermissionToml;
use agere_config::RequirementSource;
use agere_config::ResidencyRequirement;
use agere_config::WebSearchModeRequirement;
...
fn render_debug_config_lines(stack: &ConfigLayerStack) -> Vec<Line<'static>> {
    ...
    if let Some(modes) = requirements_toml.allowed_access_modes.as_ref() {
        let value = join_or_empty(
            modes
                .iter()
                .copied()
                .map(format_access_mode_requirement)
                .collect::<Vec<_>>(),
        );
        requirement_lines.push(requirement_line(
...
fn format_config_layer_source(source: &ConfigLayerSource) -> String {
    match source {
        ...
        ConfigLayerSource::Project { dot_agere_folder } => {
            format!(
                "project ({}/config.toml)",
                dot_agere_folder.as_path().display()
            )
        }
...
fn format_access_mode_requirement(mode: AccessModeRequirement) -> String {
    match mode {
        AccessModeRequirement::ReadOnly => "read-only".to_string(),
        AccessModeRequirement::WorkspaceWrite => "workspace-write".to_string(),
        AccessModeRequirement::DangerFullAccess => "danger-full-access".to_string(),
        AccessModeRequirement::External => "external-access".to_string(),
    }
}
...
mod tests {
    ...
    use agere_config::FilesystemConstraints;
    use agere_config::HookEventsToml;
    use agere_config::HookHandlerConfig;
    use agere_config::ManagedHooksRequirementsToml;
    use agere_config::MatcherGroup;
    ...
    use agere_config::NetworkUnixSocketPermissionToml;
    use agere_config::NetworkUnixSocketPermissionsToml;
    use agere_config::RequirementSource;
    use agere_config::ResidencyRequirement;
    ...
    fn debug_config_output_lists_all_layers_including_disabled() {
        let system_file = if cfg!(windows) {
            absolute_path("C:\\etc\\agere\\config.toml")
        } else {
            absolute_path("/etc/agere/config.toml")
        };
        let project_folder = if cfg!(windows) {
            absolute_path("C:\\repo\\.agere")
        } else {
            absolute_path("/repo/.agere")
        };
        ...
        let layers = vec![
            ...
            ConfigLayerEntry::new_disabled(
                ConfigLayerSource::Project {
                    dot_agere_folder: project_folder,
                },
                empty_toml_table(),
    ...
    fn debug_config_output_lists_requirement_sources() {
        let requirements_file = if cfg!(windows) {
            absolute_path("C:\\ProgramData\\OpenAI\\Agere\\requirements.toml")
        } else {
            absolute_path("/etc/agere/requirements.toml")
        };
        ...
        let requirements_toml = ConfigRequirementsToml {
            ...
            allowed_access_modes: Some(vec![AccessModeRequirement::ReadOnly]),
        ...
        let user_file = if cfg!(windows) {
            absolute_path("C:\\users\\alice\\.agere\\config.toml")
        } else {
            absolute_path("/home/alice/.openagere/config.toml")
        };
        let stack = ConfigLayerStack::new(
        ...
        assert!(
            rendered.contains(
                format!(
                    "allowed_access_modes: read-only (source: {})",
    ...
    fn debug_config_output_formats_unix_socket_permissions() {
        ...
        let rendered = render_to_text(&render_debug_config_lines(&stack));
        assert!(rendered.contains(
            "experimental_network: unix_sockets={/tmp/agere.sock=allow, /tmp/blocked.sock=none} (source: cloud requirements)"
        ));
    }
...
"#,
        ...
        assert!(rendered.contains("     - access_workspace_write.network_access = true"));
        assert!(rendered.contains("access_workspace_write.writable_roots"));
