...
use agere_protocol::protocol::NetworkAccessPolicy;
...
#[tokio::test]
#[serial]
async fn approvals_selection_popup_snapshot_windows_degraded_restriction() {
    let (mut chat, _rx, _op_rx) = make_chatwidget_manual(/*model_override*/ None).await;

    chat.config.notices.hide_full_access_warning = None;
    chat.set_windows_access_mode(Some(WindowsAccessModeToml::Unelevated));

    chat.open_approvals_popup();

    let popup = render_bottom_popup(&chat, /*width*/ 80);
    assert!(
        popup.contains("Default (non-admin restriction)"),
        "expected degraded restriction label in approvals popup: {popup}"
    );
}
...
#[cfg(target_os = "windows")]
#[tokio::test]
async fn windows_auto_mode_prompt_shows_restriction_enable_options() {
    let (mut chat, _rx, _op_rx) = make_chatwidget_manual(/*model_override*/ None).await;
    ...
    let preset = builtin_approval_presets()
        ...
        .expect("auto preset");
    chat.open_windows_restriction_enable_prompt(preset);

    let popup = render_bottom_popup(&chat, /*width*/ 120);
    assert!(
        popup.contains("Agent mode on Windows"),
        "expected auto mode prompt to describe Windows execution restrictions, popup: {popup}"
    );
    assert!(
        popup.contains("Enable restricted agent mode"),
        "expected auto mode prompt to offer enabling restricted agent mode, popup: {popup}"
    );
}

#[cfg(target_os = "windows")]
#[tokio::test]
async fn startup_prompts_for_windows_access_restriction_when_agent_requested() {
    let (mut chat, _rx, _op_rx) = make_chatwidget_manual(/*model_override*/ None).await;

    chat.maybe_prompt_windows_restriction_enable(/*show_now*/ true);

    let popup = render_bottom_popup(&chat, /*width*/ 120);
    assert!(
        popup.contains("Agent mode on Windows"),
        "expected startup prompt to describe execution restrictions: {popup}"
    );
    assert!(
        popup.contains("Enable restricted agent mode"),
        "expected startup prompt to offer restricted agent mode: {popup}"
    );
    assert!(
        popup.contains("Go back"),
        "expected startup prompt to offer go back action: {popup}"
    );
}

#[cfg(target_os = "windows")]
#[tokio::test]
async fn startup_does_not_prompt_for_windows_access_when_not_requested() {
    let (mut chat, _rx, _op_rx) = make_chatwidget_manual(/*model_override*/ None).await;

    chat.maybe_prompt_windows_restriction_enable(/*show_now*/ false);

    assert!(
        chat.bottom_pane.no_modal_or_popup_active(),
        "expected no startup restriction NUX popup when startup trigger is false"
    );
}
