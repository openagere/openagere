...
use agere_config::AppRequirementToml;
...
use agere_config::types::AppsDefaultConfig;
use agere_connectors::filter::filter_disallowed_connectors;
use agere_connectors::filter::filter_tool_suggest_discoverable_connectors;
use agere_connectors::merge::merge_connectors;
use agere_connectors::merge::plugin_connector_to_app_info;
use agere_connectors::metadata::connector_install_url;
use agere_connectors::metadata::connector_mention_slug;
use agere_connectors::metadata::sanitize_name;
...
fn agere_app_tool(
    ...
    let tool_namespace = connector_name
        .map(sanitize_name)
        .map(|connector_name| format!("mcp__{AGERE_APPS_MCP_SERVER_NAME}__{connector_name}"))
        .unwrap_or_else(|| AGERE_APPS_MCP_SERVER_NAME.to_string());
...
async fn refresh_accessible_connectors_cache_from_mcp_tools_writes_latest_installed_apps() {
    let agere_home = tempdir().expect("tempdir should succeed");
    ...
    let tools = HashMap::from([
        ...
        (
            "mcp__agere_apps__openai_hidden".to_string(),
            agere_app_tool(
                "openai_hidden",
...
async fn cloud_requirements_disable_connector_applies_without_user_apps_table() {
    let agere_home = tempdir().expect("tempdir should succeed");
    std::fs::write(agere_home.path().join(CONFIG_TOML_FILE), "").expect("write config");
...
async fn local_requirements_disable_connector_overrides_user_apps_config() {
    ...
    let config_toml_path =
        AbsolutePathBuf::try_from(agere_home.path().join(CONFIG_TOML_FILE)).expect("abs path");
    let mut config = ConfigBuilder::default()
        .agere_home(agere_home.path().to_path_buf())
        .fallback_cwd(Some(agere_home.path().to_path_buf()))
        .build()
...
fn filter_disallowed_connectors_allows_non_disallowed_connectors() {
    let filtered =
        filter_disallowed_connectors(vec![app("asdk_app_hidden"), app("alpha")], "agere_cli");
