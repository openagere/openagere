...
use agere_core::REVIEW_PROMPT;
use agere_core::config::Config;
use agere_core::review_format::render_review_output_text;
...
use agere_protocol::protocol::ReviewCodeLocation;
use agere_protocol::protocol::ReviewFinding;
use agere_protocol::protocol::ReviewLineRange;
...
async fn review_op_emits_lifecycle_and_review_output() {
    // Skip when Agere network-disabled CI restrictions apply.
    skip_if_no_network!();
...
async fn review_input_isolated_from_parent_history() {
    ...
    let session_file = agere_home.path().join("resume.jsonl");
    {
        ...
        let user = agere_protocol::models::ResponseItem::Message {
            id: None,
            role: "user".to_string(),
        ...
        let assistant = agere_protocol::models::ResponseItem::Message {
            id: None,
    ...
    let agere =
        resume_conversation_for_server(&server, agere_home.clone(), session_file.clone(), |_| {})
            .await;
