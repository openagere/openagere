...
mod tests {
    ...
    fn win_to_wsl_basic() {
        assert_eq!(
            win_path_to_wsl(r"C:\Temp\agere.zip").as_deref(),
            Some("/mnt/c/Temp/agere.zip")
        );
        assert_eq!(
            win_path_to_wsl("D:/Work/agere.tgz").as_deref(),
            Some("/mnt/d/Work/agere.tgz")
        );
        assert!(win_path_to_wsl("/home/user/agere").is_none());
    }
