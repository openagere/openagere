...
use agere_app_server_protocol::McpElicitationObjectType;
use agere_app_server_protocol::McpElicitationSchema;
...
use agere_mcp::AGERE_APPS_MCP_SERVER_NAME;
use agere_mcp::ExecutionState;
use agere_mcp::declared_openai_file_input_param_names;
...
use agere_rmcp_client::ElicitationAction;
...
const MCP_CALL_COUNT_METRIC: &str = "agere.mcp.call";
const MCP_CALL_DURATION_METRIC: &str = "agere.mcp.call.duration_ms";
const MCP_RESULT_TELEMETRY_META_KEY: &str = "agere/telemetry";
...
const MCP_RESULT_TELEMETRY_TARGET_ID_SPAN_ATTR: &str = "agere.mcp.target.id";
const MCP_RESULT_TELEMETRY_SERVER_USER_FLOW_SPAN_ATTR: &str =
    "agere.mcp.server_user_flow.triggered";
...
pub(crate) async fn handle_mcp_tool_call(
    ...
    let app_tool_policy = if server == AGERE_APPS_MCP_SERVER_NAME {
    ...
    let approval_mode = if server == AGERE_APPS_MCP_SERVER_NAME {
        app_tool_policy.approval
    } else {
        custom_mcp_tool_approval_mode(turn_context.as_ref(), &server, &tool_name)
    };

    if server == AGERE_APPS_MCP_SERVER_NAME && !app_tool_policy.enabled {
...
async fn handle_approved_mcp_tool_call(
    ...
    maybe_track_agere_app_used(sess, turn_context, &server, &tool_name).await;
...
fn mcp_tool_call_span(
    ...
    let span = tracing::info_span!(
        ...
        server.port = Empty,
        agere.mcp.target.id = Empty,
        agere.mcp.server_user_flow.triggered = Empty,
    );
...
async fn execute_mcp_tool_call(
    ...
    let request_meta = augment_mcp_tool_request_meta_with_execution_state(
        sess,
        turn_context,
        server,
        request_meta,
...
#[expect(
    ...
    reason = "MCP execution-state metadata reads through the session-owned manager guard"
)]
async fn augment_mcp_tool_request_meta_with_execution_state(
    sess: &Session,
    turn_context: &TurnContext,
    server: &str,
    mut meta: Option<serde_json::Value>,
) -> anyhow::Result<Option<serde_json::Value>> {
    let supports_execution_state_meta = sess
        .services
        .mcp_connection_manager
        .read()
        .await
        .server_supports_execution_state_meta_capability(server)
        .await
        .unwrap_or(false);
    if !supports_execution_state_meta {
        return Ok(meta);
    }

    let execution_state = serde_json::to_value(ExecutionState {
        permission_profile: Some(turn_context.permission_profile()),
        access_policy: turn_context.access_policy(),
        agere_linux_exe: turn_context.agere_linux_exe.clone(),
        access_cwd: turn_context.cwd.to_path_buf(),
    ...
    match meta.as_mut() {
        Some(serde_json::Value::Object(map)) => {
            map.insert(
                agere_mcp::MCP_EXECUTION_STATE_META_CAPABILITY.to_string(),
                execution_state,
            );
        }
        Some(_) => {}
...
async fn maybe_track_agere_app_used(
    sess: &Session,
    turn_context: &TurnContext,
    server: &str,
    tool_name: &str,
) {
    if server != AGERE_APPS_MCP_SERVER_NAME {
        return;
    }
...
pub(crate) struct McpToolApprovalMetadata {
    ...
    mcp_app_resource_uri: Option<String>,
    agere_apps_meta: Option<serde_json::Map<String, serde_json::Value>>,
    openai_file_input_params: Option<Vec<String>>,
}

const MCP_TOOL_AGERE_APPS_META_KEY: &str = "_agere_apps";
...
fn custom_mcp_tool_approval_mode(
    ...
    turn_context
        ...
        .cloned()
        .and_then(|value| {
            HashMap::<String, agere_config::types::McpServerConfig>::deserialize(value).ok()
        })
        .and_then(|servers| {
...
fn build_mcp_tool_call_request_meta(
    ...
    if let Some(turn_metadata) = turn_context.turn_metadata_state.current_meta_value() {
        request_meta.insert(
            crate::X_AGERE_TURN_METADATA_HEADER.to_string(),
            turn_metadata,
        );
    }

    if server == AGERE_APPS_MCP_SERVER_NAME {
        let mut agere_apps_meta = metadata
            .and_then(|metadata| metadata.agere_apps_meta.clone())
            .unwrap_or_default();
        agere_apps_meta.insert(
            "call_id".to_string(),
            serde_json::Value::String(call_id.to_string()),
        );
        request_meta.insert(
            MCP_TOOL_AGERE_APPS_META_KEY.to_string(),
            serde_json::Value::Object(agere_apps_meta),
        );
    }
...
pub(crate) const MCP_TOOL_APPROVAL_DECLINE_SYNTHETIC: &str = "__agere_mcp_decline__";
const MCP_TOOL_APPROVAL_ACCEPT_AND_REMEMBER: &str = "Allow and don't ask me again";
const MCP_TOOL_APPROVAL_CANCEL: &str = "Cancel";
const MCP_TOOL_APPROVAL_KIND_KEY: &str = "agere_approval_kind";
...
fn session_mcp_tool_approval_key(
    ...
    if invocation.server == AGERE_APPS_MCP_SERVER_NAME && connector_id.is_none() {
        return None;
    }
...
pub(crate) async fn lookup_mcp_tool_metadata(
    ...
    let connector_description = if server == AGERE_APPS_MCP_SERVER_NAME {
    ...
    Some(McpToolApprovalMetadata {
        ...
        agere_apps_meta: tool_info
            .tool
            .meta
            .as_ref()
            .and_then(|meta| meta.get(MCP_TOOL_AGERE_APPS_META_KEY))
...
fn openai_file_input_params_for_server(
    ...
    (server == AGERE_APPS_MCP_SERVER_NAME)
...
async fn maybe_persist_mcp_tool_approval(
    ...
    let persist_result = if key.server == AGERE_APPS_MCP_SERVER_NAME {
        let Some(connector_id) = key.connector_id.clone() else {
            remember_mcp_tool_approval(sess, key).await;
            return;
        };
        persist_agere_app_tool_approval(&turn_context.config.agere_home, &connector_id, &tool_name)
            .await
    } else {
...
async fn persist_agere_app_tool_approval(
...
async fn persist_custom_mcp_tool_approval(
        ...
        project_mcp_tool_approval_config_folder(config, server)
    {
        ...
        let servers = load_global_mcp_servers(&config.agere_home).await?;
