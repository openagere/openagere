...
use agere_exec_server::ExecProcess;
use agere_exec_server::ReadResponse as ExecReadResponse;
use agere_exec_server::StartedExecProcess;
use agere_exec_server::WriteStatus;
use agere_protocol::exec_output::ExecToolCallOutput;
use agere_protocol::exec_output::StreamOutput;
use agere_protocol::protocol::TruncationPolicy;
use agere_utils_output_truncation::formatted_truncate_text;
use agere_utils_pty::ExecCommandSession;
use agere_utils_pty::SpawnedPty;
...
impl UnifiedExecProcess {
    ...
    pub(super) async fn check_for_policy_denial(&self) -> Result<(), UnifiedExecError> {
        let _ =
        ...
        self.check_for_policy_denial_with_text(&aggregated_text)
            .await?;

        Ok(())
    }

    pub(super) async fn check_for_policy_denial_with_text(
        &self,
        text: &str,
        ...
        if is_likely_policy_denial(&exec_output) {
            ...
            return Err(UnifiedExecError::policy_denied(message, exec_output));
        }
        Ok(())
    }
    ...
    pub(super) async fn from_spawned(
        ...
        let output_rx = agere_utils_pty::combine_output_receivers(stdout_rx, stderr_rx);
        ...
        match exit_rx.try_recv() {
            Ok(exit_code) => {
                ...
                managed.check_for_policy_denial().await?;
                return Ok(managed);
            }
