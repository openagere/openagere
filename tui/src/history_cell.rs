//! Transcript/history cells for the Agere TUI.
//!
...
use agere_protocol::request_user_input::RequestUserInputAnswer;
use agere_protocol::request_user_input::RequestUserInputQuestion;
use agere_protocol::user_input::TextElement;
use agere_utils_cli::format_env_display;
...
impl Renderable for Box<dyn HistoryCell> {
    fn render(&self, area: Rect, buf: &mut Buffer) {
        ...
        let lines = self.display_lines(content_width);
...
impl HistoryCell for UpdateAvailableHistoryCell {
    fn display_lines(&self, width: u16) -> Vec<Line<'static>> {
        ...
        let update_instruction = if let Some(update_action) = self.update_action {
            ...
            line![
                "See ",
                "https://github.com/openagere/openagere".cyan().underlined(),
        ...
        let content = text![
            line![
                ...
                " ",
                format!("{AGERE_CLI_VERSION} -> {}", self.latest_version).bold(),
            ],
            update_instruction,
            "",
            "See full release notes:",
            "https://github.com/openagere/openagere/releases/latest"
                .cyan()
                .underlined(),
        ];
...
pub fn new_approval_decision_cell(
    command: Vec<String>,
    decision: agere_protocol::protocol::ReviewDecision,
        ...
        ApprovedExecpolicyAmendment {
            ...
            (
                ...
                vec![
                    actor.subject().into(),
                    "approved".bold(),
                    " agere to always run commands that start with ".into(),
                    snippet,
                ],
            )
        }
...
pub(crate) const SESSION_HEADER_MAX_INNER_WIDTH: usize = 44;
...
pub(crate) fn with_border_with_inner_width(
    ...
    with_border_internal(lines, Some(inner_width), None)
}

pub(crate) fn with_brand_border_with_inner_width(
    lines: Vec<Line<'static>>,
    inner_width: usize,
) -> Vec<Line<'static>> {
    with_border_internal(
        lines,
        Some(inner_width),
        Some(crate::brand_card::titled_top_border),
    )
}
...
fn with_border_internal(
    ...
    top_border: Option<fn(usize) -> Line<'static>>,
) -> Vec<Line<'static>> {
    ...
    out.push(
        top_border
            .map(|build_top_border| build_top_border(border_inner_width))
            .unwrap_or_else(|| {
                Line::from(format!("╭{}╮", "─".repeat(border_inner_width)))
    ...
    for line in lines.into_iter() {
        ...
        spans.push(Span::from("│ ").fg(crate::brand_card::BORDER_COLOR));
        spans.extend(line);
    ...
    out.push(
        vec![
            Span::from("╰"),
            Span::from("─".repeat(border_inner_width)),
            Span::from("╯"),
        ]
        .into_iter()
        .map(|span| span.fg(crate::brand_card::BORDER_COLOR))
        .collect::<Vec<_>>()
        .into(),
    );
    ...
    if is_first_event {
        ...
        let help_lines: Vec<Line<'static>> = vec![
            ...
            Line::from(vec![
                "  ".into(),
                "/init".into(),
                " - create an AGENTS.md file with instructions for Agere".dim(),
            ]),
            Line::from(vec![
                "  ".into(),
                "/status".into(),
            ...
            Line::from(vec![
                "  ".into(),
                "/permissions".into(),
                " - choose what Agere is allowed to do".dim(),
            ]),
            Line::from(vec![
                "  ".into(),
                "/model".into(),
        ...
        && matches!(
            ...
            PermissionProfile::Disabled
                | PermissionProfile::Managed {
                    ...
                    network: agere_protocol::protocol::NetworkAccessPolicy::Enabled,
                }
        )
}
...
impl HistoryCell for SessionHeaderHistoryCell {
    fn display_lines(&self, width: u16) -> Vec<Line<'static>> {
        ...
        const PERMISSION_LABEL: &str = "Permission";
        ...
        .max()
        .unwrap_or(0);
        let row_prefix_width = label_width + UnicodeWidthStr::width(" : ");
        let value_width = inner_width.saturating_sub(row_prefix_width).max(1);
        let model_style = Style::default().green().patch(self.model_style);

        let mut model_value = self.model.clone();
        if let Some(reasoning) = self.reasoning_label() {
            model_value.push(' ');
            model_value.push_str(reasoning);
        }
        if self.show_fast_status {
            model_value.push_str(" / FAST");
        }

        let make_row = |label: &str, value: Span<'static>| {
            Line::from(vec![
        ...
        let mut lines = crate::brand_card::brand_body_lines(inner_width);

        lines.push(make_row(
            PROVIDER_LABEL,
            Span::from(truncate_text(&self.provider, value_width)).green(),
        ));
        lines.push(make_row(
            MODEL_LABEL,
            Span::styled(truncate_text(&model_value, value_width), model_style),
        ));
        lines.push(make_row(
            DIRECTORY_LABEL,
            Span::from(self.format_directory(Some(value_width))).green(),
        ));
        lines.push(make_row(
            PERMISSION_LABEL,
            if self.yolo_mode {
                Span::from(truncate_text("YOLO", value_width))
                    .magenta()
                    .bold()
            } else {
                Span::from(truncate_text("standard", value_width)).green()
            },
        ));
        lines.push(make_row(
            VERSION_LABEL,
            Span::from(truncate_text(&format!("v{}", self.version), value_width))
                .green()
                .dim(),
        ));

        with_brand_border_with_inner_width(lines, inner_width)
    }
}
...
pub(crate) struct McpToolCallCell {
    ...
    result: Option<Result<agere_protocol::mcp::CallToolResult, String>>,
...
impl McpToolCallCell {
    ...
    pub(crate) fn complete(
        &mut self,
        duration: Duration,
        result: Result<agere_protocol::mcp::CallToolResult, String>,
...
impl HistoryCell for McpToolCallCell {
    fn display_lines(&self, width: u16) -> Vec<Line<'static>> {
        ...
        if let Some(result) = &self.result {
            match result {
                Ok(agere_protocol::mcp::CallToolResult { content, .. }) => {
                    if !content.is_empty() {
...
///
/// Manual testing tip:
/// - Run the rmcp stdio test server (`rmcp-client/src/bin/test_stdio_server.rs`) and
///   register it as an MCP server via `agere mcp add`.
...
pub(crate) fn new_cyber_policy_error_event(is_openai_provider: bool) -> CyberPolicyNoticeCell {
    CyberPolicyNoticeCell { is_openai_provider }
}

impl HistoryCell for CyberPolicyNoticeCell {
    fn display_lines(&self, width: u16) -> Vec<Line<'static>> {
        let mut lines: Vec<Line<'static>> = Vec::new();
        let title = if self.is_openai_provider {
            "This chat was flagged for possible cybersecurity risk"
        } else {
            "This chat was flagged by the model provider for possible cybersecurity risk"
        };
        lines.push(vec!["ⓘ ".cyan(), title.bold()].into());

        let wrap_width = width.saturating_sub(2).max(1) as usize;
        let body = if self.is_openai_provider {
            Line::from(vec![
            ...
            "  If this seems wrong, try rephrasing your request or check this provider's safety, access, or routing policy."
                .dim()
                .into()
        };
        let wrapped = adaptive_wrap_line(
...
pub(crate) fn empty_mcp_output() -> PlainHistoryCell {
    let lines: Vec<Line<'static>> = vec![
        ...
        Line::from(vec![
            "    See the ".into(),
            "\u{1b}]8;;https://developers.openagere.com/agere/mcp\u{7}MCP docs\u{1b}]8;;\u{7}"
                .underlined(),
...
pub(crate) fn new_mcp_tools_output(
    config: &Config,
    tools: HashMap<String, agere_protocol::mcp::Tool>,
    ...
    for server in server_names {
        ...
        let auth_status = status
            .map(|status| match status.auth_status {
                agere_app_server_protocol::McpAuthStatus::Unsupported => McpAuthStatus::Unsupported,
                agere_app_server_protocol::McpAuthStatus::NotLoggedIn => McpAuthStatus::NotLoggedIn,
                agere_app_server_protocol::McpAuthStatus::BearerToken => McpAuthStatus::BearerToken,
                agere_app_server_protocol::McpAuthStatus::OAuth => McpAuthStatus::OAuth,
            })
            .unwrap_or(McpAuthStatus::Unsupported);
        lines.push(vec!["    • Auth: ".into(), auth_status.to_string().into()].into());
...
mod tests {
    ...
    use agere_otel::RuntimeMetricTotals;
    use agere_otel::RuntimeMetricsSummary;
    ...
    fn cyber_policy_error_event_snapshot() {
        let cell = new_cyber_policy_error_event(true);
    ...
    #[test]
    fn cyber_policy_error_event_generic_provider_snapshot() {
        let cell = new_cyber_policy_error_event(false);
        let rendered = render_lines(&cell.display_lines(/*width*/ 80)).join("\n");
        insta::assert_snapshot!(rendered);
    }
    ...
    fn prefixed_wrapped_history_cell_indents_wrapped_lines() {
        ...
        assert_eq!(
            rendered,
            vec![
                "✔ You approved agere to".to_string(),
    ...
    #[test]
    fn session_header_uses_title_case_labels() {
        let cell = SessionHeaderHistoryCell::new(
            "openai".to_string(),
            "gpt-4o".to_string(),
            /*reasoning_effort*/ None,
    ...
    #[test]
    fn session_header_border_uses_full_intensity_glyphs() {
        let cell = SessionHeaderHistoryCell::new(
            "openai".to_string(),
            "gpt-4o".to_string(),
            /*reasoning_effort*/ None,
        ...
        let top = lines.first().expect("session header has top border");
        let body = &lines[1];
        let bottom = lines.last().expect("session header has bottom border");

        assert_eq!(
            top.spans.first().map(|span| span.content.as_ref()),
            Some("╭─ ")
        );
        assert_eq!(
            body.spans.first().map(|span| span.content.as_ref()),
            Some("│ ")
        );
        assert_eq!(
            body.spans.last().map(|span| span.content.as_ref()),
            Some(" │")
        );
        assert_eq!(
            bottom.spans.first().map(|span| span.content.as_ref()),
            Some("╰")
        );
        assert_eq!(
            bottom.spans.last().map(|span| span.content.as_ref()),
            Some("╯")
        );
        assert!(
            top.spans
                ...
                .is_some_and(|span| !span.style.add_modifier.contains(Modifier::DIM)
                    && span.style.fg == Some(crate::brand_card::BORDER_COLOR))
        );
        assert!(
            body.spans
                .first()
        ...
        assert!(
            bottom.spans.first().is_some_and(|span| !span
                .style
                .add_modifier
        ...
        assert!(
            bottom.spans.last().is_some_and(|span| !span
                .style
                .add_modifier
                .contains(Modifier::DIM)
                && span.style.fg == Some(crate::brand_card::BORDER_COLOR))
    ...
    #[test]
    fn yolo_mode_excludes_external_permission_profiles() {
    ...
    fn multiline_command_wraps_with_extra_indent_on_subsequent_lines() {
        ...
        let cmd = "set -o pipefail\ncargo test -p agere-tui --quiet".to_string();
        let call_id = "c1".to_string();
        let mut cell = ExecCell::new(
            ExecCall {
                call_id: call_id.clone(),
        ...
        // Mark call complete so it renders with tool name header
        cell.complete_call(&call_id, CommandOutput::default(), Duration::from_millis(1));
