...
use agere_core::ModelClient;
use agere_core::NewThread;
use agere_core::Prompt;
use agere_core::ResponseEvent;
...
use agere_protocol::models::DEFAULT_IMAGE_DETAIL;
...
use agere_protocol::models::LocalShellExecAction;
use agere_protocol::models::LocalShellStatus;
...
/// Writes an `auth.json` into the provided `agere_home` with the specified parameters.
...
fn write_auth_json(
    ...
    std::fs::write(
        agere_home.path().join("auth.json"),
...
"#,
    ...
    fn auth(&self) -> ModelProviderAuthInfo {
        ModelProviderAuthInfo {
            ...
            cwd: match agere_utils_fs::AbsolutePathBuf::try_from(self.tempdir.path()) {
                Ok(cwd) => cwd,
...
async fn resume_includes_initial_messages_and_sends_prior_items() {
    ...
    let prior_user = agere_protocol::models::ResponseItem::Message {
        id: None,
        role: "user".to_string(),
    ...
    let prior_system = agere_protocol::models::ResponseItem::Message {
        id: None,
    ...
    let prior_item = agere_protocol::models::ResponseItem::Message {
        id: None,
    ...
    // Configure Agere to resume from our file
    ...
    let test = builder
        .resume(&server, agere_home, session_path.clone())
        .await
...
async fn resume_replays_legacy_js_repl_image_rollout_shapes() {
    ...
    let agere_home = Arc::new(TempDir::new().unwrap());
    let mut builder = test_agere().with_model("gpt-5.4");
    let test = builder
        .resume(&server, agere_home, session_path.clone())
        .await
        .expect("resume conversation");
...
async fn includes_conversation_id_and_model_headers_in_request() {
    ...
    let mut builder = test_agere().with_auth(AgereAuth::from_api_key("Test API Key"));
    let test = builder
        .build(&server)
        .await
    ...
    let request_body = request.body_json();
    let installation_id =
        std::fs::read_to_string(test.agere_home_path().join(INSTALLATION_ID_FILENAME))
    ...
    assert_eq!(
        request_body["client_metadata"]["x-agere-installation-id"].as_str(),
...
async fn send_provider_auth_request(server: &MockServer, auth: ModelProviderAuthInfo) {
    ...
    let agere_home = TempDir::new().unwrap();
    let mut config = load_default_config_for_test(&agere_home).await;
    config.model_provider_id = format!("{}", provider.wire_api);
    config.model_provider = provider.clone();
    let effort = config.model_reasoning_effort;
    let summary = config.model_reasoning_summary;
    let model = agere_core::test_support::get_model_offline(config.model.as_deref());
    config.model = Some(model.clone());
    let config = Arc::new(config);
    let model_info =
        agere_core::test_support::construct_model_info_offline(model.as_str(), &config);
    ...
    let mut stream = client_session
        .stream(
            ...
            &agere_rollout_trace::InferenceTraceContext::disabled(),
        )
        .await
...
async fn includes_base_instructions_override_in_request() {
    ...
    let mut builder = test_agere()
        .with_auth(AgereAuth::from_api_key("Test API Key"))
        .with_config(|config| {
...
async fn chatgpt_auth_sends_correct_request() {
    ...
    model_provider.base_url = Some(format!("{}/api/agere", server.uri()));
    model_provider.supports_websockets = false;
    let mut builder = test_agere()
        .with_auth(create_dummy_agere_auth())
    ...
    let request = resp_mock.single_request();
    assert_eq!(request.path(), "/api/agere/responses");
    let request_authorization = request
        .header("authorization")
        .expect("authorization header");
    let request_originator = request.header("originator").expect("originator header");
...
async fn prefers_apikey_when_config_prefers_apikey_even_with_chatgpt_tokens() {
    ...
    let auth_manager = match AgereAuth::from_auth_storage(
        agere_home.path(),
    ...
    .await
    {
        Ok(Some(auth)) => agere_core::test_support::auth_manager_from_auth(auth),
        Ok(None) => panic!("No AgereAuth found in agere_home"),
        Err(e) => panic!("Failed to load AgereAuth: {e}"),
    };
    ...
    let NewThread { thread: agere, .. } = thread_manager
        .start_thread(config)
        .await
        .expect("create new conversation");
...
async fn includes_user_instructions_message_in_request() {
    ...
    assert!(
        permissions_text.contains("`access_mode`"),
        "expected permissions message to mention access_mode, got {permissions_text:?}"
    );
...
async fn skills_append_to_developer_message() {
    ...
    let agere_home = Arc::new(TempDir::new().unwrap());
    let skill_dir = agere_home.path().join("skills/demo");
    ...
    let agere_home_path = agere_home.path().to_path_buf();
    let mut builder = test_agere()
        .with_home(agere_home.clone())
        .with_auth(AgereAuth::from_api_key("Test API Key"))
        .with_config(move |config| {
            config.cwd = agere_home_path.abs();
        });
    let agere = builder
        .build(&server)
        .await
...
async fn skills_use_aliases_in_developer_message_under_budget_pressure() {
    ...
    let agere_home_parent = TempDir::new().unwrap();
    let long_home_parent = agere_home_parent
        .path()
        .join("agere-home-with-long-shared-prefix-for-skill-alias-budget-test");
    std::fs::create_dir_all(&long_home_parent).expect("create long home parent");
    let agere_home = Arc::new(TempDir::new_in(long_home_parent).unwrap());
    let skill_root = agere_home.path().join("skills");
    for index in 0..12 {
    ...
    let mut builder = test_agere()
        ...
        .with_config(move |config| {
            config.cwd = agere_home_path.abs();
            let user_config_path = agere_home_path.join("config.toml").abs();
    ...
    let _agere_home_guard = agere_home;
    let _agere_home_parent_guard = agere_home_parent;
}
...
async fn includes_no_effort_in_request() -> anyhow::Result<()> {
    ...
    .await;
    let TestAgere { agere, .. } = test_agere().with_model("gpt-5.4").build(&server).await?;
...
async fn user_turn_collaboration_mode_overrides_model_and_effort() -> anyhow::Result<()> {
    ...
    let TestAgere {
        ...
        session_configured,
        ..
    } = test_agere().with_model("gpt-5.4").build(&server).await?;
...
async fn user_turn_explicit_reasoning_summary_overrides_model_catalog_default() -> anyhow::Result<()>
{
    ...
    agere
        .submit(Op::UserTurn {
            ...
            effort: None,
            summary: Some(Some(ReasoningSummary::Concise)),
            service_tier: None,
...
async fn azure_responses_request_includes_store_and_reasoning_ids() {
    ...
    let conversation_id = ThreadId::new();
    let auth_manager =
        agere_core::test_support::auth_manager_from_auth(AgereAuth::from_api_key("Test API Key"));
    let session_telemetry = SessionTelemetry::new(
        conversation_id,
        model.as_str(),
        model_info.slug.as_str(),
...
async fn token_count_includes_rate_limits_snapshot() {
    ...
    let response = ResponseTemplate::new(200)
        .insert_header("content-type", "text/event-stream")
        .insert_header("x-agere-primary-used-percent", "12.5")
        .insert_header("x-agere-secondary-used-percent", "40.0")
        .insert_header("x-agere-primary-window-minutes", "10")
        .insert_header("x-agere-secondary-window-minutes", "60")
        .insert_header("x-agere-primary-reset-at", "1704069000")
        .insert_header("x-agere-secondary-reset-at", "1704074400")
    ...
    let mut builder = test_agere()
        .with_auth(AgereAuth::from_api_key("test"))
        .with_config(move |config| {
    ...
    let first_token_event =
        wait_for_event(&agere, |msg| matches!(msg, EventMsg::TokenCount(_))).await;
...
async fn usage_limit_error_emits_rate_limit_event() -> anyhow::Result<()> {
    ...
    let response = ResponseTemplate::new(429)
        .insert_header("x-agere-primary-used-percent", "100.0")
        .insert_header("x-agere-secondary-used-percent", "87.5")
        .insert_header("x-agere-primary-over-secondary-limit-percent", "95.0")
        .insert_header("x-agere-primary-window-minutes", "15")
        .insert_header("x-agere-secondary-window-minutes", "60")
        .set_body_json(json!({
            "error": {
    ...
    let mut builder = test_agere();
    let agere_fixture = builder.build(&server).await?;
    let agere = agere_fixture.agere.clone();
    ...
    let token_event = wait_for_event(&agere, |msg| matches!(msg, EventMsg::TokenCount(_))).await;
    ...
    let error_event = wait_for_event(&agere, |msg| matches!(msg, EventMsg::Error(_))).await;
...
async fn context_window_error_sets_total_tokens_to_model_window() -> anyhow::Result<()> {
    ...
    let token_event = wait_for_event(&agere, |event| {
        matches!(
            event,
    ...
    let error_event = wait_for_event(&agere, |ev| matches!(ev, EventMsg::Error(_))).await;
    let expected_context_window_message = AgereErr::ContextWindowExceeded.to_string();
    assert!(
        matches!(
            error_event,
...
fn create_dummy_agere_auth() -> AgereAuth {
...
async fn history_dedupes_streamed_and_final_messages_across_turns() {
    // Mirrors other tests that bail when `skip_if_no_network` applies.
    skip_if_no_network!();
