...
use agere_config::AccessModeRequirement;
use agere_config::CloudRequirementsLoader;
use agere_config::ConfigLayerStack;
use agere_config::ConfigLayerStackOrdering;
use agere_config::ConfigRequirements;
use agere_config::ConfigRequirementsToml;
use agere_config::ConstrainedWithSource;
use agere_config::FeatureRequirementsToml;
use agere_config::LoaderOverrides;
use agere_config::McpServerIdentity;
use agere_config::McpServerRequirement;
use agere_config::ResidencyRequirement;
use agere_config::Sourced;
use agere_config::ThreadConfigLoader;
use agere_config::access_mode_requirement_for_permission_profile;
use agere_config::config_toml::ConfigToml;
use agere_config::config_toml::ProjectConfig;
use agere_config::config_toml::RealtimeAudioConfig;
use agere_config::config_toml::RealtimeConfig;
use agere_config::config_toml::ThreadStoreToml;
use agere_config::config_toml::validate_model_providers;
use agere_config::loader::load_config_layers_state;
use agere_config::loader::project_trust_key;
use agere_config::profile_toml::ConfigProfile;
use agere_config::types::ApprovalsReviewer;
use agere_config::types::AuthCredentialsStoreMode;
use agere_config::types::DEFAULT_OTEL_ENVIRONMENT;
use agere_config::types::History;
use agere_config::types::McpServerConfig;
use agere_config::types::McpServerDisabledReason;
use agere_config::types::McpServerTransportConfig;
use agere_config::types::MemoriesConfig;
use agere_config::types::ModelAvailabilityNuxConfig;
use agere_config::types::Notice;
use agere_config::types::OAuthCredentialsStoreMode;
...
use agere_config::types::TuiKeymap;
use agere_config::types::TuiNotificationSettings;
use agere_config::types::UriBasedFileOpener;
...
use agere_features::FeatureConfigSource;
use agere_features::FeatureOverrides;
use agere_features::FeatureToml;
use agere_features::Features;
use agere_features::FeaturesToml;
use agere_features::MultiAgentV2ConfigToml;
use agere_git_utils::resolve_root_git_project_for_trust;
use agere_login::AuthManagerConfig;
use agere_mcp::McpConfig;
use agere_memories_read::memory_root;
use agere_model_provider_info::LEGACY_OLLAMA_CHAT_PROVIDER_ID;
use agere_model_provider_info::ModelProviderInfo;
use agere_model_provider_info::OLLAMA_CHAT_PROVIDER_REMOVED_ERROR;
use agere_model_provider_info::built_in_model_providers;
use agere_model_provider_info::merge_configured_model_providers;
use agere_models_manager::ModelsManagerConfig;
use agere_protocol::config_types::AccessMode;
use agere_protocol::config_types::AltScreenMode;
...
use agere_protocol::config_types::ShellEnvironmentPolicy;
use agere_protocol::config_types::TrustLevel;
use agere_protocol::config_types::Verbosity;
use agere_protocol::config_types::WebSearchConfig;
...
use crate::config::permissions::get_readable_roots_required_for_agere_runtime;
...
#[cfg(test)]
mod schema;
pub use agere_config::Constrained;
pub use agere_config::ConstraintError;
pub use agere_config::ConstraintResult;
pub use agere_network_proxy::NetworkProxyAuditMetadata;
...
const LOCAL_DEV_BUILD_VERSION: &str = "0.0.1";

pub const CONFIG_TOML_FILE: &str = "config.toml";

fn resolve_sqlite_home_env(resolved_cwd: &Path) -> Option<PathBuf> {
    let raw = std::env::var(agere_state::SQLITE_HOME_ENV).ok()?;
...
pub(crate) async fn test_config() -> Config {
    ...
    Config::load_from_base_config_with_overrides(
        ...
        AbsolutePathBuf::from_absolute_path(agere_home.path()).expect("temp dir should resolve"),
    )
    .await
...
pub struct Permissions {
    ...
    /// Effective Windows access mode derived from `[windows].access_mode` or
    /// legacy feature keys.
    pub windows_access_mode: Option<WindowsAccessModeToml>,
    /// Whether restricted Windows child processes should use a private desktop (legacy flag).
...
impl Permissions {
    ...
    /// Effective filesystem access policy derived from the canonical permission profile.
    pub fn file_system_access_policy(&self) -> FileSystemAccessPolicy {
        self.permission_profile.get().file_system_access_policy()
    }

    /// Effective network access policy derived from the canonical permission profile.
    pub fn network_access_policy(&self) -> NetworkAccessPolicy {
        self.permission_profile.get().network_access_policy()
    }

    /// Legacy access-policy label derived from the permission profile (wire compat).
    pub fn legacy_access_policy(&self, cwd: &std::path::Path) -> Result<String, String> {
        self.permission_profile.get().to_legacy_access_policy(cwd)
    }
    ...
    pub fn set_legacy_access_policy(
        &mut self,
        legacy: String,
        cwd: &std::path::Path,
    ) -> ConstraintResult<()> {
        let permission_profile = PermissionProfile::from_legacy_access_policy_for_cwd(&legacy, cwd);
...
// when Agere is still responsible for the network policy. `Disabled` means no
// managed network envelope from Agere, so starting the managed proxy would narrow the override.
...
pub struct Config {
    ...
    ///
    /// - `None`: use default attribution (`Agere <noreply@openagere.com>`)
    /// - `Some("")` or whitespace-only: disable commit attribution
    /// - `Some("...")`: use the provided attribution text verbatim
    pub commit_attribution: Option<String>,

    /// Optional external notifier command. When set, Agere will spawn this
    /// program after each completed *turn* (i.e. when the agent finishes
    /// processing a user submission). The value must be the full command
    /// broken into argv tokens **without** the trailing JSON argument - Agere
    /// appends one extra argument containing a JSON payload describing the
    /// event.
    ///
    /// Example `~/.openagere/config.toml` snippet:
    ///
    /// ```toml
    /// notify = ["notify-send", "Agere"]
    /// ```
    ///
    /// which will be invoked as:
    ///
    /// ```shell
    /// notify-send Agere '{"type":"agent-turn-complete","turn-id":"12345"}'
    /// ```
    ///
    ...
    /// file (default): Use a file in the Agere home directory.
    /// keyring: Use an OS-specific keyring service.
    /// auto: Use the OS-specific keyring service if available, otherwise use a file.
    pub cli_auth_credentials_store_mode: AuthCredentialsStoreMode,

    /// Definition for MCP servers that Agere can reach out to for tool calls.
    pub mcp_servers: Constrained<HashMap<String, McpServerConfig>>,

    /// Preferred store for MCP OAuth credentials.
    /// keyring: Use an OS-specific keyring service.
    ///          Credentials stored in the keyring will only be readable by Agere unless the user explicitly grants access via OS-level keyring access.
    /// file: AGERE_HOME/.credentials.json
    ///       This file will be readable to Agere and other applications running as the same user.
    /// auto (default): keyring if available, otherwise file.
    pub mcp_oauth_credentials_store_mode: OAuthCredentialsStoreMode,

    /// Optional fixed port to use for the local HTTP callback server used during MCP OAuth login.
    ///
    /// When unset, Agere will bind to an ephemeral port chosen by the OS.
    ...
    /// User-defined model entries from [[models]] in config.toml.
    pub models: Vec<agere_config::config_toml::ModelConfig>,
    ...
    /// Directory where Agere stores the SQLite state DB.
    pub sqlite_home: PathBuf,

    /// Directory where Agere writes log files (defaults to `$AGERE_HOME/log`).
    pub log_dir: PathBuf,

    /// Settings that govern if and what will be written to `~/.openagere/history.jsonl`.
    pub history: History,
    ...
    /// Path to the current Agere executable. This cannot be set in the config
    /// file: it must be set in code via [`ConfigOverrides`].
    pub agere_self_exe: Option<PathBuf>,

    /// Path to the `agere-linux-helper` executable used for Linux managed exec plumbing.
    /// This cannot be set in the config file: it must be set in code via
    /// [`ConfigOverrides`].
    ///
    /// When this program is invoked as the helper, argv0 is expected to be `agere-linux-helper`.
    pub agere_linux_exe: Option<PathBuf>,

    /// Path to the `agere-execve-wrapper` executable used for shell
    ...
    /// When `true`, checks for Agere updates on startup and surfaces update prompts.
    /// Set to `false` only if your Agere updates are centrally managed.
    /// Defaults to `true`.
    ...
    /// When `false`, disables analytics across Agere product surfaces in this machine.
    /// Voluntarily left as Optional because the default value might depend on the client.
    pub analytics_enabled: Option<bool>,

    /// When `false`, disables feedback collection across Agere product surfaces.
    /// Defaults to `true`.
    ...
    pub otel: agere_config::types::OtelConfig,
}
...
impl ConfigBuilder {
    pub fn agere_home(mut self, agere_home: PathBuf) -> Self {
        self.agere_home = Some(agere_home);
        self
    }
    ...
    pub async fn build(self) -> std::io::Result<Config> {
        ...
        let agere_home = match agere_home {
            Some(agere_home) => AbsolutePathBuf::from_absolute_path(agere_home)?,
            None => find_agere_home()?,
        };
        ...
        let config_layer_stack = load_config_layers_state(
            ...
            thread_config_loader
                .as_deref()
                .unwrap_or(&agere_config::NoopThreadConfigLoader),
        )
        .await?;
        ...
        let config_toml: ConfigToml = match merged_toml.try_into() {
            ...
            Err(err) => {
                if let Some(config_error) = agere_config::first_layer_config_error::<ConfigToml>(
                    &config_layer_stack,
                    agere_config::CONFIG_TOML_FILE,
                )
                .await
                {
                    return Err(agere_config::io_error_from_config_error(
...
impl Config {
    ...
    pub async fn to_mcp_config(
        ...
        McpConfig {
            ...
            agere_home: self.agere_home.to_path_buf(),
    ...
    pub async fn load_default_with_cli_overrides(
        ...
        let agere_home = find_agere_home()?;
        Self::load_default_with_cli_overrides_for_agere_home(
    ...
    /// Load a default configuration for a specific Agere home without reading
    /// user, project, or system config layers.
    pub async fn load_default_with_cli_overrides_for_agere_home(
        agere_home: PathBuf,
        cli_overrides: Vec<(String, TomlValue)>,
    ) -> std::io::Result<Self> {
        ...
        let cli_layer = agere_config::build_cli_overrides_layer(&cli_overrides);
        agere_config::merge_toml_values(&mut merged, &cli_layer);
        let agere_home = AbsolutePathBuf::from_absolute_path_checked(agere_home)?;
        let config_toml = deserialize_config_toml_with_base(merged, &agere_home)?;
    ...
    /// ignores user settings. For example, the `agere exec` subcommand is
    /// designed to use [AskForApproval::Never] exclusively.
    ///
    /// Further, [ConfigOverrides] contains some options that are not supported
    /// in [ConfigToml], such as `cwd`, `agere_self_exe`, `agere_linux_exe`, and
    ...
    /// Legacy access policy label for [`Self::cwd`] (panics if projection fails).
    pub fn legacy_access_policy(&self) -> String {
        self.permissions
            .legacy_access_policy(self.cwd.as_path())
            .unwrap_or_else(|err| panic!("legacy access policy: {err}"))
    }

    pub fn set_legacy_access_policy(&mut self, legacy: String) -> ConstraintResult<()> {
        self.permissions
            .set_legacy_access_policy(legacy, self.cwd.as_path())
    }
}
...
pub async fn load_config_as_toml_with_cli_and_loader_overrides(
    ...
    let merged_toml = config_layer_stack.effective_config();
    let cfg = deserialize_config_toml_with_base(merged_toml, agere_home).map_err(|e| {
...
pub async fn load_global_mcp_servers(
    ...
    // MCP servers defined in in-repo .openagere/ folders.
...
/// Patch `AGERE_HOME/config.toml` project state to set trust level.
/// Use with caution.
pub fn set_project_trust_level(
    ...
    ConfigEditsBuilder::new(agere_home)
        .set_project_trust_level(project_path, trust_level)
        .apply_blocking()
}

/// Save the default OSS provider preference to config.toml
pub fn set_default_oss_provider(agere_home: &Path, provider: &str) -> std::io::Result<()> {
    agere_config::config_toml::validate_oss_provider(provider)?;
    use toml_edit::value;
...
fn resolve_permission_config_syntax(
    config_layer_stack: &ConfigLayerStack,
    cfg: &ConfigToml,
    access_mode_override: Option<AccessMode>,
    profile_access_mode: Option<AccessMode>,
) -> Option<PermissionConfigSyntax> {
    if access_mode_override.is_some() || profile_access_mode.is_some() {
    ...
    for layer in config_layer_stack.get_layers(
        ...
        if layer_selection.access_mode.is_some() {
    ...
    selection.or_else(|| {
        ...
        } else if cfg.access_mode.is_some() {
...
fn apply_managed_filesystem_constraints(
    ...
    filesystem_constraints: &agere_config::FilesystemConstraints,
) {
    for deny_read in &filesystem_constraints.deny_read {
        let deny_entry = if deny_read.contains_glob() {
            agere_protocol::permissions::FileSystemAccessEntry {
                path: agere_protocol::permissions::FileSystemPath::GlobPattern {
                    pattern: deny_read.as_str().to_string(),
                },
                access: agere_protocol::permissions::FileSystemAccessMode::None,
            }
        } else {
            ...
            agere_protocol::permissions::FileSystemAccessEntry {
                path: agere_protocol::permissions::FileSystemPath::Path { path },
                access: agere_protocol::permissions::FileSystemAccessMode::None,
            }
        };
        if !file_system_access_policy
            .entries
            .iter()
            .any(|existing| existing == &deny_entry)
        {
            file_system_access_policy.entries.push(deny_entry);
        }
    }
}
...
impl Config {
    ...
    pub(crate) async fn load_config_with_layer_stack(
        ...
        let user_instructions = AgentsMdManager::load_global_instructions(Some(&agere_home))
        ...
        if access_mode.is_some() && permission_profile.is_some() {
            return Err(std::io::Error::new(
                std::io::ErrorKind::InvalidInput,
                "`access_mode` and `permission_profile` overrides cannot both be set",
            ));
        }
        ...
        let windows_access_mode = resolve_windows_access_mode(&cfg, &config_profile);
        let windows_execution_private_desktop =
            resolve_windows_execution_private_desktop(&cfg, &config_profile);
        ...
        let windows_execution_restriction_level = match windows_access_mode {
            Some(WindowsAccessModeToml::Elevated) => WindowsExecutionRestrictionLevel::Elevated,
            Some(WindowsAccessModeToml::Unelevated) => WindowsExecutionRestrictionLevel::RestrictedToken,
            None => WindowsExecutionRestrictionLevel::Disabled,
        };
        let memories_root = memory_root(&agere_home);
        ...
        let (
            ...
            let configured_network_proxy_config =
                if profile_allows_configured_network_proxy(&permission_profile)
                    && profiles_are_active
                {
                    // PermissionProfile carries the active network-access posture, not the configured
            ...
            let is_workspace_write =
                file_system_access_policy.can_write_path_with_cwd(resolved_cwd.as_path(), resolved_cwd.as_path());
            if is_workspace_write {
                ...
                permission_profile = PermissionProfile::from_runtime_permissions_with_filesystem_isolation(
            ...
            let default_permissions = cfg.default_permissions.as_deref().unwrap_or_else(|| {
                default_builtin_permission_profile_name(&active_project, windows_execution_restriction_level)
            });
            ...
            let (mut file_system_access_policy, network_access_policy) =
                compile_permission_profile_selection(
                    cfg.permissions.as_ref(),
                    default_permissions,
                    cfg.access_workspace_write.as_ref(),
                    resolved_cwd.as_path(),
                    &mut startup_warnings,
                )?;
            let mut permission_profile = if let Some(permission_profile) =
                builtin_permission_profile(default_permissions, cfg.access_workspace_write.as_ref())
            {
                permission_profile
            } else {
            ...
            let is_workspace_write =
                file_system_access_policy.can_write_path_with_cwd(resolved_cwd.as_path(), resolved_cwd.as_path());
            if is_workspace_write {
                file_system_access_policy = if using_implicit_builtin_profile {
                    file_system_access_policy
                        .with_additional_legacy_workspace_writable_roots(
                            &additional_writable_roots,
                        )
                } else {
                    file_system_access_policy.with_additional_writable_roots(
                        resolved_cwd.as_path(),
                        &additional_writable_roots,
                    )
                };
            ...
            // write access.
            if matches!(permission_profile.filesystem_isolation(), FilesystemIsolation::Managed)
                && file_system_access_policy.can_write_path_with_cwd(
                    resolved_cwd.as_path(),
                    resolved_cwd.as_path(),
                )
                && !file_system_access_policy.has_full_disk_write_access()
            {
        ...
        let log_dir = cfg
            ...
            .unwrap_or_else(|| agere_home.join("log").to_path_buf());
        let sqlite_home = cfg
            .sqlite_home
            .as_ref()
            .map(AbsolutePathBuf::to_path_buf)
            .or_else(|| resolve_sqlite_home_env(&resolved_cwd))
            .unwrap_or_else(|| agere_home.to_path_buf());
            ...
            && !filesystem_requirements.deny_read.is_empty()
        {
            ...
            constrained_permission_profile
                ...
                .add_validator(move |permission_profile| {
                    let mode =
                        access_mode_requirement_for_permission_profile(permission_profile);
                    match mode {
                        AccessModeRequirement::ReadOnly
                        | AccessModeRequirement::WorkspaceWrite => Ok(()),
                        AccessModeRequirement::DangerFullAccess
                        | AccessModeRequirement::External => {
            ...
            if cfg!(target_os = "windows") {
                startup_warnings.push(format!(
                    "managed filesystem deny_read from {filesystem_requirements_source} is only enforced for direct file tools on Windows; shell subprocess reads are not covered by the same enforcement"
                ));
            }
        }
        apply_requirement_constrained_value(
        ...
        let helper_readable_roots = get_readable_roots_required_for_agere_runtime(
            &agere_home,
            zsh_path.as_ref(),
            main_execve_wrapper_exe.as_ref(),
        );
        let effective_permission_profile = constrained_permission_profile.value.get().clone();
        let (mut effective_file_system_access_policy, effective_network_access_policy) =
            effective_permission_profile.to_runtime_permissions();
        if effective_permission_profile != original_permission_profile {
            effective_file_system_access_policy
                .preserve_deny_read_restrictions_from(&file_system_access_policy);
        }
        if let Some(Sourced {
            value: filesystem_requirements,
            ..
        }) = filesystem_requirements.as_ref()
        {
            apply_managed_filesystem_constraints(
                &mut effective_file_system_access_policy,
                filesystem_requirements,
            );
        }
        let effective_file_system_access_policy = effective_file_system_access_policy
        ...
        let effective_permission_profile =
            PermissionProfile::from_runtime_permissions_with_filesystem_isolation(
                effective_permission_profile.filesystem_isolation(),
                &effective_file_system_access_policy,
                effective_network_access_policy,
            );
        constrained_permission_profile
            .value
    ...
    async fn try_read_non_empty_file(
        ...
        let contents = fs.read_file_text(path).await.map_err(|e| {
            std::io::Error::new(
                e.kind(),
    ...
    pub fn set_windows_unelevated_access_mode(&mut self, value: bool) {
        self.permissions.windows_access_mode = if value {
            Some(WindowsAccessModeToml::Unelevated)
        } else if matches!(
            self.permissions.windows_access_mode,
            Some(WindowsAccessModeToml::Unelevated)
        ) {
            None
        } else {
    ...
    pub fn set_windows_elevated_access_mode(&mut self, value: bool) {
        self.permissions.windows_access_mode = if value {
...
/// - If `AGERE_HOME` is not set, this function does not verify that the
///   directory exists.
pub fn find_agere_home() -> std::io::Result<AbsolutePathBuf> {
    agere_utils_fs::find_agere_home()
}

/// Returns the path to the folder where Agere logs are stored. Does not verify
/// that the directory exists.
pub fn log_dir(cfg: &Config) -> std::io::Result<PathBuf> {
    Ok(cfg.log_dir.clone())
}

/// Resolve Windows access mode from config and profile. Returns None (legacy Windows execution restrictions removed).
pub fn resolve_windows_access_mode(
    _cfg: &agere_config::config_toml::ConfigToml,
    _profile: &agere_config::profile_toml::ConfigProfile,
) -> Option<agere_config::types::WindowsAccessModeToml> {
    None
}

/// Resolve Windows private-desktop flag for restricted execution. Always false (legacy path removed).
pub fn resolve_windows_execution_private_desktop(
    _cfg: &agere_config::config_toml::ConfigToml,
    _profile: &agere_config::profile_toml::ConfigProfile,
) -> bool {
    false
