...
async fn exec_end_without_begin_uses_event_command() {
    ...
    let parsed_cmd = agere_shell_command::parse_command::parse_command(&command);
    ...
    assert!(
        blob.contains("• Bash(echo orphaned)"),
...
async fn exec_end_without_begin_does_not_flush_unrelated_running_exploring_cell() {
    ...
    assert!(
        orphan_blob.contains("• Bash(echo repro-marker)"),
...
async fn exec_end_without_begin_flushes_completed_unrelated_exploring_cell() {
    ...
    assert!(
        second.contains("• Bash(echo after)"),
...
async fn exec_history_shows_unified_exec_startup_commands() {
    ...
    let blob = lines_to_single_string(&cells[0]);
    assert!(
        blob.contains("• Bash(echo unified exec startup)"),
...
async fn unified_exec_wait_after_final_agent_message_snapshot() {
    ...
    begin_unified_exec_startup(&mut chat, "call-wait", "proc-1", "cargo test -p agere-core");
...
async fn unified_exec_wait_status_renders_command_in_single_details_row_snapshot() {
    ...
    begin_unified_exec_startup(
        ...
        "call-wait-ui",
        "proc-ui",
        "cargo test -p agere-core -- --exact some::very::long::test::name",
    );
...
//
// Synthesizes a Agere ExecApprovalRequest event to trigger the approval modal
...
async fn apply_patch_full_flow_integration_like() {
    ...
    // 3) App forwards to widget.submit_op, which pushes onto agere_op_tx
    chat.submit_op(op);
    let forwarded = op_rx
        .try_recv()
        .expect("expected op forwarded to agere channel");
    match forwarded {
