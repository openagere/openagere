...
async fn responses_stream_includes_subagent_header_on_review() {
    ...
    let agere_home = TempDir::new().expect("failed to create TempDir");
    ...
    assert_eq!(
        request.header("x-agere-window-id").as_deref(),
        Some(expected_window_id.as_str())
    );
    assert_eq!(request.header("x-agere-parent-thread-id"), None);
    assert_eq!(
        request.body_json()["client_metadata"]["x-agere-installation-id"].as_str(),
...
async fn responses_stream_includes_turn_metadata_header_for_git_workspace_e2e() {
    ...
    let test = test_agere().build(&server).await.expect("build test agere");
    ...
    let initial_header = first_request
        ...
        .expect("x-agere-turn-metadata header should be present");
    let initial_parsed: serde_json::Value =
        serde_json::from_str(&initial_header).expect("x-agere-turn-metadata should be valid JSON");
    ...
    assert!(
        ...
        "turn_id should not be empty in x-agere-turn-metadata"
    );
    ...
    run_git(&[
        ...
        "add",
        "origin",
        "https://github.com/openagere/openagere.git",
    ]);
