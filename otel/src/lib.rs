...
pub use agere_utils_common::sanitize_metric_tag_value;
...
/// Maps to API/auth `AuthMode` to avoid a circular dependency on agere-core.
...
impl From<agere_app_server_protocol::AuthMode> for TelemetryAuthMode {
    fn from(mode: agere_app_server_protocol::AuthMode) -> Self {
        match mode {
            agere_app_server_protocol::AuthMode::ApiKey => Self::ApiKey,
            agere_app_server_protocol::AuthMode::Chatgpt
            | agere_app_server_protocol::AuthMode::ChatgptAuthTokens
            | agere_app_server_protocol::AuthMode::AgentIdentity => Self::Chatgpt,
        }
    }
}
