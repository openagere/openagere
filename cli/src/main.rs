...
use agere_arg0::arg0_dispatch_or_else;
use agere_cli::read_agent_identity_from_stdin;
use agere_cli::read_api_key_from_stdin;
use agere_cli::run_login_status;
use agere_cli::run_login_with_agent_identity;
use agere_cli::run_login_with_api_key;
use clap::Args;
use clap::CommandFactory;
use clap::Parser;
use clap_complete::Shell;
use clap_complete::generate;
// run_login_with_chatgpt was removed during codebase simplification
use agere_cli::run_login_with_device_code;
use agere_cli::run_login_with_device_code_fallback_to_browser;
use agere_cli::run_logout;
use agere_exec::Cli as ExecCli;
use agere_exec::Command as ExecCommand;
use agere_exec::ReviewArgs;
use agere_execpolicy::ExecPolicyCheckCommand;
use agere_rollout_trace::REDUCED_STATE_FILE_NAME;
use agere_rollout_trace::replay_bundle;
use agere_state::StateRuntime;
use agere_state::state_db_path;
use agere_tui::AppExitInfo;
use agere_tui::Cli as TuiCli;
use agere_tui::ExitReason;
use agere_tui::UpdateAction;
use agere_utils_cli::CliConfigOverrides;
...
/// Returns whether the current terminal is detected as "dumb".
...
#[clap(
    ...
    // `agere-x86_64-unknown-linux-musl`, but the help output should always use
    // the generic `agere` command name that users run.
    bin_name = "agere",
    override_usage = "agere [OPTIONS] [PROMPT]\n       agere [OPTIONS] <COMMAND> [ARGS]"
)]
struct MultitoolCli {
    #[clap(flatten)]
...
enum Subcommand {
    ...
    /// Manage external MCP servers for Agere.
    Mcp(McpCli),

    /// Manage Agere plugins.
    Plugin(PluginCli),

    /// Open the provider management UI.
    Provider(TuiCli),

    /// [experimental] Run the app server or related tooling.
    AppServer(AppServerCommand),

    /// Launch the Agere desktop app (opens the app installer if missing).
    #[cfg(any(target_os = "macos", target_os = "windows"))]
    ...
    /// Update Agere to the latest version.
    Update,
...
enum PluginSubcommand {
    /// Manage plugin marketplaces for Agere.
...
struct LoginCommand {
    ...
    #[arg(
        ...
        help = "Read the experimental Agent Identity token from stdin (e.g. `printenv AGERE_AGENT_IDENTITY | agere login --with-agent-identity`)"
    )]
...
struct AppServerCommand {
    ...
    #[arg(
        long = "listen",
        value_name = "URL",
        default_value = agere_app_server::AppServerTransport::DEFAULT_LISTEN_URL
    )]
    listen: agere_app_server::AppServerTransport,
    ...
    /// ```
    ///
    /// See https://developers.openagere.com/agere/config-advanced/#metrics for more details.
...
enum AppServerSubcommand {
    ...
    /// [internal] Generate internal JSON Schema artifacts for Agere tooling.
    #[clap(hide = true)]
...
fn format_exit_messages(exit_info: AppExitInfo, color_enabled: bool) -> Vec<String> {
    ...
    if !token_usage.is_zero() {
        lines.push(format!(
            "{}",
            agere_protocol::protocol::FinalOutput::from(token_usage)
        ));
    }

    if let Some(resume_cmd) =
        agere_core::util::resume_command(/*thread_name*/ None, conversation_id)
    {
...
fn run_update_action(action: UpdateAction) -> anyhow::Result<()> {
    ...
    println!("Updating Agere via `{cmd_str}`...");
    ...
    println!("\n🎉 Update ran successfully! Please restart Agere.");
    Ok(())
}
...
fn run_update_command() -> anyhow::Result<()> {
    ...
    {
        anyhow::bail!(
            "`agere update` is not available in debug builds. Install a release build of Agere to use this command."
        );
    }

    #[cfg(not(debug_assertions))]
    {
        let Some(action) = agere_tui::get_update_action() else {
            anyhow::bail!(
                "Could not detect the Agere installation method. Please update manually: https://developers.openagere.com/agere/cli/"
            );
        };
        run_update_action(action)
    }
...
async fn run_debug_app_server_command(cmd: DebugAppServerCommand) -> anyhow::Result<()> {
    match cmd.subcommand {
        DebugAppServerSubcommand::SendMessageV2(cmd) => {
            let agere_bin = std::env::current_exe()?;
            agere_app_server_test_client::send_message_v2(&agere_bin, &[], cmd.user_message, &None)
                .await
        }
    }
}
...
async fn cli_main(arg0_paths: Arg0DispatchPaths) -> anyhow::Result<()> {
    ...
    match subcommand {
        ...
        Some(Subcommand::Exec(mut exec_cli)) => {
            ...
            agere_exec::run_main(exec_cli, arg0_paths.clone()).await?;
        }
        Some(Subcommand::Review(review_args)) => {
            ...
            let mut exec_cli = ExecCli::try_parse_from(["agere", "exec"])?;
        ...
        Some(Subcommand::Provider(provider_cli)) => {
            interactive = finalize_provider_interactive(
                interactive,
                root_config_overrides.clone(),
                provider_cli,
            );
        ...
        Some(Subcommand::AppServer(app_server_cli)) => {
            ...
            match subcommand {
                None => {
                    ...
                    agere_app_server::run_main_with_transport(
                        arg0_paths.clone(),
                ...
                Some(AppServerSubcommand::Proxy(proxy_cli)) => {
                    let socket_path = match proxy_cli.socket_path {
                        ...
                        None => {
                            ...
                            agere_app_server::app_server_control_socket_path(&agere_home)?
                        }
                    };
                    agere_stdio_to_uds::run(socket_path.as_path()).await?;
                }
                Some(AppServerSubcommand::GenerateTs(gen_cli)) => {
                    let options = agere_app_server_protocol::GenerateTsOptions {
                        experimental_api: gen_cli.experimental,
                        ..Default::default()
                    };
                    agere_app_server_protocol::generate_ts_with_options(
                        &gen_cli.out_dir,
                ...
                Some(AppServerSubcommand::GenerateJsonSchema(gen_cli)) => {
                    agere_app_server_protocol::generate_json_with_experimental(
                        &gen_cli.out_dir,
                        gen_cli.experimental,
                    )?;
                }
                Some(AppServerSubcommand::GenerateInternalJsonSchema(gen_cli)) => {
                    agere_app_server_protocol::generate_internal_json_schema(&gen_cli.out_dir)?;
                }
            }
        }
        #[cfg(any(target_os = "macos", target_os = "windows"))]
...
async fn run_exec_server_command(
    ...
    let agere_self_exe = arg0_paths
        .agere_self_exe
        .clone()
        .ok_or_else(|| anyhow::anyhow!("Agere executable path is not configured"))?;
    let runtime_paths = agere_exec_server::ExecServerRuntimePaths::new(
        agere_self_exe,
        arg0_paths.agere_linux_exe.clone(),
    )?;
    agere_exec_server::run_main(&cmd.listen, runtime_paths)
        .await
...
async fn enable_feature_in_config(interactive: &TuiCli, feature: &str) -> anyhow::Result<()> {
    ...
    maybe_print_under_development_feature_warning(&agere_home, interactive, feature);
    Ok(())
}
...
fn maybe_print_under_development_feature_warning(
    ...
    let config_path = agere_home.join(agere_config::CONFIG_TOML_FILE);
    eprintln!(
...
async fn run_debug_prompt_input_command(
    ...
    } else if shared.dangerously_bypass_approvals {
        Some(AskForApproval::Never)
    } else {
        interactive.approval_policy.map(Into::into)
    };
    let access_mode = if shared.full_auto {
        Some(agere_protocol::config_types::AccessMode::WorkspaceWrite)
    } else if shared.dangerously_bypass_approvals {
        Some(agere_protocol::config_types::AccessMode::DangerFullAccess)
    } else {
        shared.access_mode
    };
    let overrides = ConfigOverrides {
        ...
        access_mode,
        cwd: shared.cwd,
        agere_self_exe: arg0_paths.agere_self_exe,
        agere_linux_exe: arg0_paths.agere_linux_exe,
    ...
    let prompt_input = agere_core::build_prompt_input(config, input).await?;
...
async fn run_debug_models_command(
    ...
    let catalog = if cmd.bundled {
        ...
        let auth_manager =
            AuthManager::shared_from_config(&config, /*enable_agere_api_key_env*/ true).await;
        let models_manager =
...
async fn run_debug_clear_memories_command(
    ...
    clear_memory_roots_contents(&config.agere_home).await?;
...
fn reject_remote_mode_for_subcommand(
    ...
    if let Some(remote) = remote {
        anyhow::bail!(
            "`--remote {remote}` is only supported for interactive TUI commands, not `agere {subcommand}`"
        );
    }
    if remote_auth_token_env.is_some() {
        anyhow::bail!(
            "`--remote-auth-token-env` is only supported for interactive TUI commands, not `agere {subcommand}`"
        );
    }
    Ok(())
}
...
async fn run_interactive_tui(
    ...
    if is_dumb_terminal() {
        ...
        eprintln!(
            "WARNING: TERM is set to \"dumb\". Agere's interactive TUI may not work in this terminal."
        );
    ...
    let normalized_remote = remote
        .as_deref()
        .map(agere_tui::normalize_remote_addr)
        .transpose()
...
/// Build the final `TuiCli` for a `openagere resume` invocation.
fn finalize_resume_interactive(
    ...
    // configuration surface area as `agere` without additional flags.
...
/// Build the final `TuiCli` for a `agere fork` invocation.
...
/// Build the final `TuiCli` for a `agere provider` invocation.
fn finalize_provider_interactive(
    mut interactive: TuiCli,
    root_config_overrides: CliConfigOverrides,
    provider_cli: TuiCli,
) -> TuiCli {
    merge_interactive_cli_flags(&mut interactive, provider_cli);
    prepend_config_flags(&mut interactive.config_overrides, root_config_overrides);
    interactive.open_provider_on_startup = true;

    interactive
}

/// Merge flags provided to `openagere resume`/`agere fork` so they take precedence over any
...
mod tests {
    ...
    fn exec_resume_last_accepts_prompt_positional() {
        let cli =
            MultitoolCli::try_parse_from(["agere", "exec", "--json", "resume", "--last", "2+2"])
                .expect("parse should succeed");

        let Some(Subcommand::Exec(exec)) = cli.subcommand else {
            panic!("expected exec subcommand");
        };
        let Some(agere_exec::Command::Resume(args)) = exec.command else {
    ...
    fn default_app_server_socket_path() -> AbsolutePathBuf {
        let agere_home = find_agere_home().expect("agere home");
        agere_app_server::app_server_control_socket_path(&agere_home)
    ...
    fn debug_models_parses_bundled_flag() {
        let cli =
            MultitoolCli::try_parse_from(["agere", "debug", "models", "--bundled"]).expect("parse");
    ...
    fn plugin_marketplace_help_uses_plugin_namespace() {
        let help = help_from_args(&["agere", "plugin", "marketplace", "--help"]);
        assert!(
            help.contains("Usage: agere plugin marketplace [OPTIONS] <COMMAND>"),
            "{help}"
        );

        for (subcommand, usage) in [
            ("add", "Usage: agere plugin marketplace add"),
            ("upgrade", "Usage: agere plugin marketplace upgrade"),
            ("remove", "Usage: agere plugin marketplace remove"),
        ] {
            let help = help_from_args(&["agere", "plugin", "marketplace", subcommand, "--help"]);
    ...
    fn plugin_marketplace_add_parses_under_plugin() {
        let cli =
            MultitoolCli::try_parse_from(["agere", "plugin", "marketplace", "add", "owner/repo"])
                .expect("parse");
    ...
    fn plugin_marketplace_upgrade_parses_under_plugin() {
        let cli =
            MultitoolCli::try_parse_from(["agere", "plugin", "marketplace", "upgrade", "debug"])
                .expect("parse");
    ...
    fn update_parses_as_update_subcommand() {
        let cli = MultitoolCli::try_parse_from(["agere", "update"]).expect("parse");
        assert!(matches!(cli.subcommand, Some(Subcommand::Update)));
    }

    #[test]
    fn provider_parses_as_interactive_provider_subcommand() {
        let cli = MultitoolCli::try_parse_from(["agere", "provider"]).expect("parse");
        assert!(matches!(cli.subcommand, Some(Subcommand::Provider(_))));
    }

    #[test]
    fn plugin_marketplace_remove_parses_under_plugin() {
        let cli =
            MultitoolCli::try_parse_from(["agere", "plugin", "marketplace", "remove", "debug"])
                .expect("parse");
    ...
    fn marketplace_no_longer_parses_at_top_level() {
        let add_result =
            MultitoolCli::try_parse_from(["agere", "marketplace", "add", "owner/repo"]);
        assert!(add_result.is_err());

        let upgrade_result =
            MultitoolCli::try_parse_from(["agere", "marketplace", "upgrade", "debug"]);
        assert!(upgrade_result.is_err());

        let remove_result =
            MultitoolCli::try_parse_from(["agere", "marketplace", "remove", "debug"]);
    ...
    fn format_exit_messages_includes_resume_hint_without_color() {
        ...
        assert_eq!(
            ...
            vec![
                ...
                "To continue this session, run openagere resume 123e4567-e89b-12d3-a456-426614174000"
                    .to_string(),
            ]
        );
    }
    ...
    fn resume_model_flag_applies_when_no_root_flags() {
        let interactive =
            finalize_resume_from_args(["agere", "resume", "-m", "gpt-5.1-test"].as_ref());
    ...
    fn resume_picker_logic_none_and_not_last() {
        let interactive = finalize_resume_from_args(["agere", "resume"].as_ref());
        assert!(interactive.resume_picker);
        assert!(!interactive.resume_last);
        assert_eq!(interactive.resume_session_id, None);
    ...
    fn resume_picker_logic_last() {
        let interactive = finalize_resume_from_args(["agere", "resume", "--last"].as_ref());
        assert!(!interactive.resume_picker);
        assert!(interactive.resume_last);
        assert_eq!(interactive.resume_session_id, None);
        assert!(!interactive.resume_show_all);
    ...
    fn resume_last_accepts_prompt_positional() {
        let interactive = finalize_resume_from_args(
            ["agere", "resume", "--last", "/compact focus on auth"].as_ref(),
        );
    ...
    fn resume_last_rejects_explicit_session_and_prompt() {
        let err =
            MultitoolCli::try_parse_from(["agere", "resume", "--last", "1234", "continue here"])
    ...
    fn resume_picker_logic_with_session_id() {
        let interactive = finalize_resume_from_args(["agere", "resume", "1234"].as_ref());
        assert!(!interactive.resume_picker);
        assert!(!interactive.resume_last);
    ...
    fn resume_with_session_id_accepts_prompt_positional() {
        let interactive =
            finalize_resume_from_args(["agere", "resume", "1234", "continue here"].as_ref());
    ...
    fn resume_all_flag_sets_show_all() {
        let interactive = finalize_resume_from_args(["agere", "resume", "--all"].as_ref());
        assert!(interactive.resume_picker);
        assert!(interactive.resume_show_all);
    }
    ...
    fn resume_include_non_interactive_flag_sets_source_filter_override() {
        let interactive =
            finalize_resume_from_args(["agere", "resume", "--include-non-interactive"].as_ref());
    ...
    fn resume_merges_option_flags_and_full_auto() {
        ...
        assert_matches!(
            interactive.access_mode,
            Some(agere_protocol::config_types::AccessMode::WorkspaceWrite)
        );
        assert_matches!(
            interactive.approval_policy,
            Some(agere_utils_cli::ApprovalModeCliArg::OnRequest)
        );
    ...
    fn resume_merges_dangerously_bypass_flag() {
        let interactive = finalize_resume_from_args(
            ["agere", "resume", "--dangerously-bypass-approvals"].as_ref(),
        );
        assert!(interactive.dangerously_bypass_approvals);
        assert!(interactive.resume_picker);
        assert!(!interactive.resume_last);
        assert_eq!(interactive.resume_session_id, None);
    }
    ...
    fn fork_picker_logic_none_and_not_last() {
        let interactive = finalize_fork_from_args(["agere", "fork"].as_ref());
    ...
    fn fork_picker_logic_last() {
        let interactive = finalize_fork_from_args(["agere", "fork", "--last"].as_ref());
        assert!(!interactive.fork_picker);
        assert!(interactive.fork_last);
        assert_eq!(interactive.fork_session_id, None);
        assert!(!interactive.fork_show_all);
    ...
    fn fork_picker_logic_with_session_id() {
        let interactive = finalize_fork_from_args(["agere", "fork", "1234"].as_ref());
        assert!(!interactive.fork_picker);
        assert!(!interactive.fork_last);
    ...
    fn fork_all_flag_sets_show_all() {
        let interactive = finalize_fork_from_args(["agere", "fork", "--all"].as_ref());
        assert!(interactive.fork_picker);
        assert!(interactive.fork_show_all);
    }
    ...
    fn app_server_analytics_default_disabled_without_flag() {
        let app_server = app_server_from_args(["agere", "app-server"].as_ref());
        assert!(!app_server.analytics_default_enabled);
        assert_eq!(
            app_server.listen,
            agere_app_server::AppServerTransport::Stdio
        );
    }
    ...
    fn app_server_analytics_default_enabled_with_flag() {
        let app_server =
            app_server_from_args(["agere", "app-server", "--analytics-default-enabled"].as_ref());
        assert!(app_server.analytics_default_enabled);
    }

    #[test]
    fn remote_flag_parses_for_interactive_root() {
        let cli = MultitoolCli::try_parse_from(["agere", "--remote", "ws://127.0.0.1:4500"])
            .expect("parse");
    ...
    fn remote_flag_parses_for_resume_subcommand() {
        let cli =
            MultitoolCli::try_parse_from(["agere", "resume", "--remote", "ws://127.0.0.1:4500"])
                .expect("parse");
    ...
    fn read_remote_auth_token_from_env_var_reports_missing_values() {
        let err = read_remote_auth_token_from_env_var_with("AGERE_REMOTE_AUTH_TOKEN", |_| {
    ...
    fn read_remote_auth_token_from_env_var_trims_values() {
        let auth_token =
            read_remote_auth_token_from_env_var_with("AGERE_REMOTE_AUTH_TOKEN", |_| {
    ...
    fn app_server_listen_websocket_url_parses() {
        let app_server = app_server_from_args(
            ["agere", "app-server", "--listen", "ws://127.0.0.1:4500"].as_ref(),
        );
        assert_eq!(
            app_server.listen,
            agere_app_server::AppServerTransport::WebSocket {
    ...
    fn app_server_listen_stdio_url_parses() {
        let app_server =
            app_server_from_args(["agere", "app-server", "--listen", "stdio://"].as_ref());
        assert_eq!(
            app_server.listen,
            agere_app_server::AppServerTransport::Stdio
        );
    ...
    fn app_server_listen_unix_socket_url_parses() {
        let app_server =
            app_server_from_args(["agere", "app-server", "--listen", "unix://"].as_ref());
        assert_eq!(
            app_server.listen,
            agere_app_server::AppServerTransport::UnixSocket {
    ...
    fn app_server_listen_unix_socket_path_parses() {
        let app_server = app_server_from_args(
            ["agere", "app-server", "--listen", "unix:///tmp/agere.sock"].as_ref(),
        );
        assert_eq!(
            app_server.listen,
            agere_app_server::AppServerTransport::UnixSocket {
                socket_path: AbsolutePathBuf::from_absolute_path("/tmp/agere.sock")
    ...
    fn app_server_listen_off_parses() {
        let app_server = app_server_from_args(["agere", "app-server", "--listen", "off"].as_ref());
        assert_eq!(app_server.listen, agere_app_server::AppServerTransport::Off);
    }

    #[test]
    fn app_server_listen_invalid_url_fails_to_parse() {
        let parse_result =
            MultitoolCli::try_parse_from(["agere", "app-server", "--listen", "http://foo"]);
        assert!(parse_result.is_err());
    }

    #[test]
    fn app_server_proxy_subcommand_parses() {
        let app_server = app_server_from_args(["agere", "app-server", "proxy"].as_ref());
        assert!(matches!(
    ...
    fn app_server_proxy_sock_path_parses() {
        let app_server =
            app_server_from_args(["agere", "app-server", "proxy", "--sock", "agere.sock"].as_ref());
        ...
        assert_eq!(
            proxy.socket_path,
            Some(
                AbsolutePathBuf::relative_to_current_dir("agere.sock")
    ...
    fn app_server_capability_token_flags_parse() {
        ...
        assert_eq!(
            ...
            Some(agere_app_server::WebsocketAuthCliMode::CapabilityToken)
        );
        assert_eq!(
            app_server.auth.ws_token_file,
            Some(PathBuf::from("/tmp/agere-token"))
        );
    }
    ...
    fn app_server_signed_bearer_flags_parse() {
        ...
        assert_eq!(
            app_server.auth.ws_auth,
            Some(agere_app_server::WebsocketAuthCliMode::SignedBearerToken)
        );
        assert_eq!(
            app_server.auth.ws_shared_secret_file,
            Some(PathBuf::from("/tmp/agere-secret"))
        );
    ...
    fn features_enable_parses_feature_name() {
        let cli = MultitoolCli::try_parse_from(["agere", "features", "enable", "unified_exec"])
            .expect("parse should succeed");
    ...
    fn features_disable_parses_feature_name() {
        let cli = MultitoolCli::try_parse_from(["agere", "features", "disable", "shell_tool"])
            .expect("parse should succeed");
        let Some(Subcommand::Features(FeaturesCli { sub })) = cli.subcommand else {
            panic!("expected features subcommand");
        };
    ...
    #[test]
    fn feature_toggles_accept_legacy_landlock_flag() {
        let toggles = FeatureToggles {
            enable: vec!["use_legacy_landlock".to_string()],
            disable: Vec::new(),
        };
        let overrides = toggles.to_overrides().expect("valid features");
        assert_eq!(
            overrides,
            vec!["features.use_legacy_landlock=true".to_string(),]
        );
    }
