...
function Get-ReleaseAssetMetadata {
    ...
    $release = Invoke-RestMethod -Uri "https://api.github.com/repos/openai/agere/releases/tags/rust-v$ResolvedVersion"
    $asset = $release.assets | Where-Object { $_.name -eq $AssetName } | Select-Object -First 1
    if ($null -eq $asset) {
        throw "Could not find release asset $AssetName for Agere $ResolvedVersion."
    }
...
function Test-ArchiveDigest {
    ...
    if ($actualDigest -ne $ExpectedDigest) {
        throw "Downloaded Agere archive checksum did not match release metadata. Expected $ExpectedDigest but got $actualDigest."
    }
}
...
function Resolve-Version {
    ...
    $release = Invoke-RestMethod -Uri "https://api.github.com/repos/openai/agere/releases/latest"
    if (-not $release.tag_name) {
        Write-Error "Failed to resolve the latest Agere release version."
        exit 1
    }
...
function Get-VersionFromBinary {
    ...
    if (-not (Test-Path -LiteralPath $AgerePath -PathType Leaf)) {
        return $null
    }

    try {
        $versionOutput = & $AgerePath --version 2>$null
    } catch {
        return $null
    }
...
function Get-CurrentInstalledVersion {
    ...
    $standaloneVersion = Get-VersionFromBinary -AgerePath (Join-Path $StandaloneCurrentDir "openagere.exe")
...
function Move-OldStandaloneBinIfApproved {
    ...
    Write-Step "We found an older Agere install at $VisibleBinDir"
    Write-WarningStep "To continue, Agere needs to update the install at this path."
    if (-not (Prompt-YesNo "Replace it with the current Agere setup now?")) {
...
function Add-JunctionSupportType {
    if (([System.Management.Automation.PSTypeName]'AgereInstaller.Junction').Type) {
        return
    }
...
function Set-JunctionTarget {
    ...
    [AgereInstaller.Junction]::SetTarget($LinkPath, $TargetPath)
}
...
function Get-ExistingAgereCommand {
    $existing = Get-Command agere -ErrorAction SilentlyContinue
    if ($null -eq $existing) {
        return $null
    }
...
function Get-ExistingAgereManager {
    param(
        [string]$ExistingPath,
        [string]$VisibleBinDir
    )
...
function Get-ConflictingInstall {
    ...
    $existingPath = Get-ExistingAgereCommand
    $manager = Get-ExistingAgereManager -ExistingPath $existingPath -VisibleBinDir $VisibleBinDir
    if ($null -eq $manager) {
        return $null
    }

    Write-Step "Detected existing $manager-managed Agere at $existingPath"
    Write-WarningStep "Multiple managed Agere installs can be ambiguous because PATH order decides which one runs."
...
function Maybe-HandleConflictingInstall {
    ...
    $uninstallArgs = if ($manager -eq "bun") {
        ...
        @("uninstall", "-g", "openagere")
    }
    $uninstallCommand = if ($manager -eq "bun") { "bun" } else { "npm" }

    if (Prompt-YesNo "Uninstall the existing $manager-managed Agere now?") {
        Write-Step "Running: $uninstallCommand $($uninstallArgs -join ' ')"
        try {
            & $uninstallCommand @uninstallArgs
        } catch {
            Write-WarningStep "Failed to uninstall the existing $manager-managed Agere. Continuing with the standalone install."
        }
    } else {
        Write-WarningStep "Leaving the existing $manager-managed Agere installed. PATH order will determine which agere runs."
    }
}

function Test-VisibleAgereCommand {
    param(
        [string]$VisibleBinDir
    )

    $agereCommand = Join-Path $VisibleBinDir "openagere.exe"
    & $agereCommand --version *> $null
    if ($LASTEXITCODE -ne 0) {
        throw "Installed Agere command failed verification: $agereCommand --version"
    }
}
...
if (-not [Environment]::Is64BitOperatingSystem) {
    Write-Error "Agere requires a 64-bit version of Windows."
    exit 1
}
...
$agereHome = if ([string]::IsNullOrWhiteSpace($env:AGERE_HOME)) {
    Join-Path $env:USERPROFILE ".agere"
} else {
    $env:AGERE_HOME
}
$standaloneRoot = Join-Path $agereHome "packages\standalone"
$releasesDir = Join-Path $standaloneRoot "releases"
$currentDir = Join-Path $standaloneRoot "current"
$lockPath = Join-Path $standaloneRoot "install.lock"

$defaultVisibleBinDir = Join-Path $env:LOCALAPPDATA "Programs\OpenAI\Agere\bin"
if ([string]::IsNullOrWhiteSpace($env:AGERE_INSTALL_DIR)) {
    $visibleBinDir = $defaultVisibleBinDir
} else {
    $visibleBinDir = $env:AGERE_INSTALL_DIR
}
...
if (-not [string]::IsNullOrWhiteSpace($currentVersion) -and $currentVersion -ne $resolvedVersion) {
    Write-Step "Updating Agere CLI from $currentVersion to $resolvedVersion"
} elseif (-not [string]::IsNullOrWhiteSpace($currentVersion)) {
    Write-Step "Updating Agere CLI"
} else {
    Write-Step "Installing Agere CLI"
}
...
$packageAsset = "openagere-npm-$npmTag-$resolvedVersion.tgz"
$tempDir = Join-Path ([System.IO.Path]::GetTempPath()) ("agere-install-" + [System.Guid]::NewGuid().ToString("N"))
...
try {
    Invoke-WithInstallLock -LockPath $lockPath -Script {
        ...
        if (-not (Test-ReleaseIsComplete -ReleaseDir $releaseDir -ExpectedVersion $resolvedVersion -ExpectedTarget $target)) {
            ...
            Write-Step "Downloading Agere CLI"
            ...
            $copyMap = @{
                "agere/openagere.exe" = "bin\openagere.exe"
            }
        ...
        try {
            ...
            Test-VisibleAgereCommand -VisibleBinDir $visibleBinDir
        } catch {
...
Write-Step "Current PowerShell session: agere"
Write-Step "Future PowerShell windows: open a new PowerShell window and run: agere"
Write-Host "Agere CLI $resolvedVersion installed successfully."

$agereCommand = Join-Path $visibleBinDir "openagere.exe"
if (Prompt-YesNo "Start Agere now?") {
    Write-Step "Launching Agere"
    & $agereCommand
}
