...
fn test_mcp_config(agere_home: PathBuf) -> McpConfig {
    McpConfig {
...
#[test]
fn agere_apps_mcp_url_for_base_url_keeps_existing_paths() {
    assert_eq!(
        agere_apps_mcp_url_for_base_url("https://chatgpt.com/backend-api"),
        "https://chatgpt.com/backend-api/wham/apps"
    );
    assert_eq!(
        agere_apps_mcp_url_for_base_url("https://chat.openai.com"),
        "https://chat.openai.com/backend-api/wham/apps"
    );
    assert_eq!(
        agere_apps_mcp_url_for_base_url("http://localhost:8080/api/agere"),
        "http://localhost:8080/api/agere/apps"
    );
    assert_eq!(
        agere_apps_mcp_url_for_base_url("http://localhost:8080"),
        "http://localhost:8080/api/agere/apps"
    );
}

#[test]
fn agere_apps_mcp_url_uses_legacy_agere_apps_path() {
    let config = test_mcp_config(PathBuf::from("/tmp"));

    assert_eq!(
        agere_apps_mcp_url(&config),
        "https://chatgpt.com/backend-api/wham/apps"
    );
}

#[test]
fn agere_apps_server_config_uses_legacy_agere_apps_path() {
    let mut config = test_mcp_config(PathBuf::from("/tmp"));
    let auth = AgereAuth::create_dummy_chatgpt_auth_for_testing();

    let mut servers = with_agere_apps_mcp(HashMap::new(), /*auth*/ None, &config);
    assert!(!servers.contains_key(AGERE_APPS_MCP_SERVER_NAME));

    config.apps_enabled = true;

    servers = with_agere_apps_mcp(servers, Some(&auth), &config);
    let server = servers
        .get(AGERE_APPS_MCP_SERVER_NAME)
        .expect("agere apps should be present when apps is enabled");
    let url = match &server.transport {
        McpServerTransportConfig::StreamableHttp { url, .. } => url,
        _ => panic!("expected streamable http transport for agere apps"),
    };
...
#[tokio::test]
async fn effective_mcp_servers_preserve_user_servers_and_add_agere_apps() {
    let agere_home = tempfile::tempdir().expect("tempdir");
    let mut config = test_mcp_config(agere_home.path().to_path_buf());
    config.apps_enabled = true;
    let auth = AgereAuth::create_dummy_chatgpt_auth_for_testing();
    ...
    let agere_apps = effective
        .get(AGERE_APPS_MCP_SERVER_NAME)
        .expect("agere apps server should exist");
