...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
#[ignore]
async fn get_conversation_summary_by_relative_rollout_path_resolves_from_agere_home() -> Result<()>
{
    let agere_home = TempDir::new()?;
    let conversation_id = create_fake_rollout(
        agere_home.path(),
    ...
    let relative_path = rollout_path.strip_prefix(agere_home.path())?.to_path_buf();
