...
impl ToolsConfig {
    pub fn new(params: &ToolsConfigParams<'_>) -> Self {
        ...
        // API-key auth bypasses Agere backend entitlement/tool normalization, so
        ...
        let shell_type = if !features.enabled(Feature::ShellTool) {
            ...
            if agere_utils_pty::conpty_supported() {
...
fn unified_exec_allowed_in_environment(
    ...
    let managed_policy_constraints = match permission_profile {
        PermissionProfile::Managed {
            file_system,
            network,
        } => !file_system.to_access_policy().has_full_disk_write_access() || !network.is_enabled(),
