...
async fn build_agere(server: &StreamingSseServer) -> Arc<AgereThread> {
    test_agere()
        .with_model("gpt-5.4")
        .build_with_streaming_server(server)
        .await
        .unwrap_or_else(|err| panic!("build streaming Agere test session: {err}"))
        .agere
}

async fn submit_user_input(agere: &AgereThread, text: &str) {
    agere
        .submit(Op::UserInput {
            environments: None,
...
async fn submit_danger_full_access_user_turn(test: &TestAgere, text: &str) {
...
async fn steer_user_input(agere: &AgereThread, text: &str) {
    agere
        .steer_input(
            vec![UserInput::Text {
                text: text.to_string(),
...
async fn submit_queue_only_agent_mail(agere: &AgereThread, text: &str) {
    agere
    ...
    wait_for_event(agere, |event| {
        matches!(event, EventMsg::McpListToolsResponse(_))
    })
    .await;
}

async fn wait_for_reasoning_item_started(agere: &AgereThread) {
    wait_for_event(agere, |event| {
        matches!(
            event,
...
async fn wait_for_agent_message(agere: &AgereThread, text: &str) {
...
async fn wait_for_turn_complete(agere: &AgereThread) {
...
async fn queued_inter_agent_mail_triggers_follow_up_after_reasoning_item() {
    ...
    let agere = build_agere(&server).await;

    submit_user_input(&agere, "first prompt").await;

    wait_for_reasoning_item_started(&agere).await;

    submit_queue_only_agent_mail(&agere, "queued child update").await;
...
async fn queued_inter_agent_mail_triggers_follow_up_after_commentary_message_item() {
    ...
    wait_for_agent_message(&agere, "first answer").await;
...
async fn user_input_does_not_preempt_after_reasoning_item() {
    ...
    steer_user_input(&agere, "second prompt").await;
...
async fn steered_user_input_waits_for_model_continuation_after_mid_turn_compact() {
    ...
    submit_user_input(&agere, "first prompt").await;
    submit_user_input(&agere, "second prompt").await;

    wait_for_agent_message(&agere, "resumed old task").await;
    wait_for_turn_complete(&agere).await;
...
async fn steered_user_input_follows_compact_when_only_the_steer_needs_follow_up() {
    ...
    wait_for_agent_message(&agere, "processed steered prompt").await;
    wait_for_turn_complete(&agere).await;
...
async fn steered_user_input_waits_when_tool_output_triggers_compact_before_next_request() {
    ...
    wait_for_event(&agere, |event| matches!(event, EventMsg::TurnStarted(_))).await;
    steer_user_input(&agere, "second prompt").await;
    let _ = gate_first_completed_tx.send(());
