...
use crate::render::highlight::highlight_bash_to_single_line;
...
use agere_shell_command::bash::extract_bash_command;
use agere_utils_common::format_duration;
...
/// Derive a Claude Code–style header for an exec call: (tool_name, parenthesized_arg).
/// The tool_name reflects the semantic kind (Bash, Read, Search),
/// and display_arg provides a concise human-readable summary.
fn command_display_name_and_arg(call: &ExecCall) -> (&'static str, String) {
    match call.parsed.first() {
        Some(ParsedCommand::Read { name, .. }) => ("Read", name.clone()),
        Some(ParsedCommand::Search { query, .. }) => ("Search", query.clone().unwrap_or_default()),
        Some(ParsedCommand::ListFiles { path, cmd, .. }) => {
            ("Bash", path.clone().unwrap_or_else(|| cmd.clone()))
        }
        Some(ParsedCommand::Unknown { cmd }) => ("Bash", cmd.clone()),
        None => ("Bash", extract_command_display(&call.command)),
    }
}

/// Extract a human-readable command string, stripping `bash -lc` / `zsh -lc` wrappers.
/// Multi-line scripts are normalized (newlines → spaces) by the caller.
fn extract_command_display(command: &[String]) -> String {
...
/// Indent for wrapped command lines in the header.
const COMMAND_WRAP_INDENT: &str = "        ";
...
impl ExecCell {
                ...
                for parsed in &call.parsed {
                    match parsed {
                        ...
                        ParsedCommand::Unknown { cmd } => {
                            lines.push(("Bash", vec![cmd.clone().into()]));
                        }
                    }
                }
                lines
    ...
    fn command_display_lines(&self, width: u16) -> Vec<Line<'static>> {
        ...
        let (tool_name, display_arg) = command_display_name_and_arg(call);

        // Build header: ● ToolName(display_arg)
        // If the command is too long for one line, it wraps with continuation indent.
        let header_line = if is_interaction {
            Line::from(vec![bullet.clone(), " ".into()])
        } else {
            let highlighted = highlight_bash_to_single_line(&display_arg);
            let mut spans = Vec::with_capacity(5 + highlighted.len());
            spans.push(bullet.clone());
            spans.push(" ".into());
            spans.push(tool_name.bold());
            spans.push("(".into());
        ...
        // Wrap header at full width, with indentation for continuation lines.
        let header_lines = adaptive_wrap_line(
            &header_line,
            RtOptions::new(width as usize).subsequent_indent(COMMAND_WRAP_INDENT.into()),
        );
        let mut lines: Vec<Line<'static>> = Vec::new();
        push_owned_lines(&header_lines, &mut lines);
