...
impl AgereMessageProcessor {
    ...
    async fn state_db_for_materialized_thread(
        ...
        if let Ok(thread) = self.thread_manager.get_thread(thread_id).await {
            ...
            match find_thread_path_by_id_str(&self.config.agere_home, &thread_id.to_string()).await
            {
                Ok(Some(_)) => {}
                Ok(None) => {
...
fn thread_goal_status_to_state(status: ThreadGoalStatus) -> agere_state::ThreadGoalStatus {
    match status {
...
fn thread_goal_status_from_state(status: agere_state::ThreadGoalStatus) -> ThreadGoalStatus {
    match status {
...
pub(super) fn api_thread_goal_from_state(goal: agere_state::ThreadGoal) -> ThreadGoal {
    ThreadGoal {
