...
use crate::events::AgereTurnEventParams;
use crate::events::AgereTurnEventRequest;
use crate::events::AgereTurnSteerEventParams;
use crate::events::AgereTurnSteerEventRequest;
...
use crate::events::agere_compaction_event_params;
...
use agere_git_utils::collect_git_info;
...
struct CompletedTurnState {
    ...
    turn_error: Option<AgereErrorInfo>,
    completed_at: u64,
...
impl AnalyticsReducer {
    ...
    fn ingest_guardian_review(
        ...
        out.push(TrackEventRequest::GuardianReview(Box::new(
            GuardianReviewEventRequest {
                event_type: "agere_guardian_review",
    ...
    fn ingest_app_mentioned(&mut self, input: AppMentionedInput, out: &mut Vec<TrackEventRequest>) {
        ...
        out.extend(mentions.into_iter().map(|mention| {
            let event_params = agere_app_metadata(&tracking, mention);
            TrackEventRequest::AppMentioned(AgereAppMentionedEventRequest {
    ...
    fn ingest_app_used(&mut self, input: AppUsedInput, out: &mut Vec<TrackEventRequest>) {
        ...
        let event_params = agere_app_metadata(&tracking, app);
        out.push(TrackEventRequest::AppUsed(AgereAppUsedEventRequest {
    ...
    fn ingest_hook_run(&mut self, input: HookRunInput, out: &mut Vec<TrackEventRequest>) {
        ...
        out.push(TrackEventRequest::HookRun(AgereHookRunEventRequest {
            event_type: "agere_hook_run",
            event_params: agere_hook_run_metadata(&tracking, hook),
        }));
    }
    ...
    fn ingest_plugin_used(&mut self, input: PluginUsedInput, out: &mut Vec<TrackEventRequest>) {
        ...
        out.push(TrackEventRequest::PluginUsed(AgerePluginUsedEventRequest {
            event_type: "agere_plugin_used",
            event_params: agere_plugin_used_metadata(&tracking, plugin),
        }));
    }
    ...
    fn ingest_plugin_state_changed(
        ...
        let event = AgerePluginEventRequest {
            event_type: plugin_state_event_type(state),
            event_params: agere_plugin_metadata(plugin),
        };
            ...
            ServerNotification::TurnCompleted(notification) => {
                ...
                turn_state.completed = Some(CompletedTurnState {
                    ...
                    turn_error: notification
                        .turn
                        .error
                        .and_then(|error| error.agere_error_info),
    ...
    fn ingest_compaction(&mut self, input: AgereCompactionEvent, out: &mut Vec<TrackEventRequest>) {
        let Some(connection_id) = self.thread_connections.get(&input.thread_id) else {
            tracing::warn!(
                thread_id = %input.thread_id,
                turn_id = %input.turn_id,
        ...
        out.push(TrackEventRequest::Compaction(Box::new(
            AgereCompactionEventRequest {
                event_type: "agere_compaction_event",
                event_params: agere_compaction_event_params(
                    input,
    ...
    fn emit_turn_steer_event(
        ...
        out.push(TrackEventRequest::TurnSteer(AgereTurnSteerEventRequest {
            event_type: "agere_turn_steer_event",
            event_params: AgereTurnSteerEventParams {
                thread_id: pending_request.thread_id,
    ...
    fn maybe_emit_turn_event(&mut self, turn_id: &str, out: &mut Vec<TrackEventRequest>) {
        ...
        out.push(TrackEventRequest::TurnEvent(Box::new(
            AgereTurnEventRequest {
                event_type: "agere_turn_event",
                event_params: agere_turn_event_params(
                    app_server_client,
                    runtime,
                    turn_id.to_string(),
                    turn_state,
...
fn agere_turn_event_params(
    app_server_client: AgereAppServerClientMetadata,
    runtime: AgereRuntimeMetadata,
    turn_id: String,
    turn_state: &TurnState,
    thread_metadata: &ThreadMetadataState,
) -> AgereTurnEventParams {
...
fn analytics_turn_status(status: agere_app_server_protocol::TurnStatus) -> Option<TurnStatus> {
    match status {
        agere_app_server_protocol::TurnStatus::Completed => Some(TurnStatus::Completed),
        agere_app_server_protocol::TurnStatus::Failed => Some(TurnStatus::Failed),
        agere_app_server_protocol::TurnStatus::Interrupted => Some(TurnStatus::Interrupted),
        agere_app_server_protocol::TurnStatus::InProgress => None,
    }
}
