...
use agere_network_proxy::BlockedRequestArgs;
...
#[test]
fn network_approval_flow_is_limited_to_restricted_access_profiles() {
...
async fn record_blocked_request_sets_policy_outcome_for_owner_call() {
    ...
    assert_eq!(
            ...
            Some(NetworkApprovalOutcome::DeniedByPolicy(
                "Network access to \"example.com\" was blocked: domain is not on the allowlist for the current access mode.".to_string()
            ))
        );
}
