...
//!
//! This module owns the auth-step state machine (account login/device-code/API
...
fn onboarding_request_id() -> agere_app_server_protocol::RequestId {
    agere_app_server_protocol::RequestId::String(Uuid::new_v4().to_string())
}
...
pub(super) async fn cancel_login_attempt(
    ...
    let _ = request_handle
        .request_typed::<agere_app_server_protocol::CancelLoginAccountResponse>(
...
impl AuthModeWidget {
    ...
    fn render_pick_mode(&self, area: Rect, buf: &mut Buffer) {
        let mut lines: Vec<Line> = vec![
            Line::from(vec!["  ".into(), "Sign in with your Agere account".into()]),
            Line::from(vec![
                "  ".into(),
        ...
        let chatgpt_description = if !self.is_chatgpt_login_allowed() {
            ...
            "Use the plan and rate limits associated with your account"
        };
        ...
        if !self.is_api_login_allowed() {
            lines.push(
                "  API key login is disabled by this workspace. Sign in with your account to continue."
                    .dim()
                    .into(),
            );
            lines.push("".into());
    ...
    fn render_chatgpt_success_message(&self, area: Rect, buf: &mut Buffer) {
        let lines = vec![
            "✓ Signed in with your account".fg(Color::Green).into(),
            "".into(),
            "  Before you start:".into(),
            "".into(),
            "  Decide how much autonomy you want to grant Agere".into(),
            Line::from(vec![
                "  For more details see the ".into(),
                "\u{1b}]8;;https://developers.openagere.com/agere/security\u{7}Agere docs\u{1b}]8;;\u{7}".underlined(),
            ])
            .dim(),
            "".into(),
            "  Agere can make mistakes".into(),
            "  Review the code it writes and commands it runs".dim().into(),
            "".into(),
            "  Powered by your account".into(),
            Line::from(vec![
                "  Uses your plan's rate limits and account settings".into(),
            ])
            .dim(),
            "".into(),
            Line::from(vec![
    ...
    fn render_chatgpt_success(&self, area: Rect, buf: &mut Buffer) {
        let lines = vec!["✓ Signed in with your account".fg(Color::Green).into()];
    ...
    fn render_api_key_configured(&self, area: Rect, buf: &mut Buffer) {
        let lines = vec![
            ...
            "".into(),
            "  Agere will use usage-based billing with your API key.".into(),
        ];
    ...
    fn render_api_key_entry(&self, area: Rect, buf: &mut Buffer, state: &ApiKeyInputState) {
        ...
        if state.prepopulated_from_env {
            intro_lines.push("  Detected API key from the environment.".into());
            intro_lines.push(
    ...
    /// Kicks off the account auth flow and keeps the UI state consistent with the attempt.
    fn start_chatgpt_login(&mut self) {
        // If we're already authenticated with an account, don't start a new login –
...
mod tests {
    ...
    async fn widget_forced_chatgpt() -> (AuthModeWidget, TempDir) {
        ...
        let config = ConfigBuilder::default()
            .agere_home(agere_home_path.clone())
            .build()
            .await
            .unwrap();
