...
async fn submit_turn_with_policies(
    ...
    let permission_profile = turn_permission_fields(permission_profile, test.cwd_path());
    test.agere
        .submit(Op::UserTurn {
            environments: None,
...
fn skill_script_command(test: &TestAgere, script_name: &str) -> Result<String> {
...
async fn wait_for_exec_approval_request(test: &TestAgere) -> Option<ExecApprovalRequestEvent> {
    wait_for_event_match(test.agere.as_ref(), |event| match event {
...
async fn wait_for_turn_complete(test: &TestAgere) {
    wait_for_event(test.agere.as_ref(), |event| {
        matches!(event, EventMsg::TurnComplete(_))
    })
    .await;
}

fn output_shows_access_denial(output: &str) -> bool {
...
async fn shell_zsh_fork_skill_scripts_ignore_declared_permissions() -> Result<()> {
    ...
    let command = skill_script_command(&test, "restricted_skill.sh")?;
    ...
    assert!(
        ...
        "skill script should now be governed by the turn access restrictions, not the removed skill approval gate: {output:?}"
    );
    assert!(
        output_shows_access_denial(output) || !output.contains("allowed"),
        "expected the turn access restrictions to block the out-of-workspace write, got output: {output:?}"
    );
    assert!(
        !allowed_path.exists(),
        "declared skill permissions should not widen script execution beyond the turn access restrictions"
    );
...
#[cfg(unix)]
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn shell_zsh_fork_still_enforces_workspace_write_restrictions() -> Result<()> {
    skip_if_no_network!(Ok(()));

    let Some(runtime) = zsh_fork_runtime("zsh-fork workspace write restrictions test")? else {
        return Ok(());
    };

    let server = start_mock_server().await;
    let tool_call_id = "zsh-fork-workspace-write-deny";
    let outside_path = "/tmp/agere-zsh-fork-workspace-write-deny.txt";
    let workspace_write_profile = restrictive_workspace_write_profile();
    ...
    let output = call_output["output"].as_str().unwrap_or_default();
    assert!(
        output_shows_access_denial(output),
        "expected filesystem access denial, got output: {output:?}"
    );
    assert!(
