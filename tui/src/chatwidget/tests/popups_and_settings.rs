...
#[tokio::test]
async fn plugins_popup_empty_marketplace_state_mentions_background_sync() {
    let (mut chat, _rx, _op_rx) = make_chatwidget_manual(/*model_override*/ None).await;
    chat.set_feature_enabled(Feature::Plugins, /*enabled*/ true);

    let popup = render_loaded_plugins_popup(&mut chat, plugins_test_response(Vec::new()));

    assert!(
        popup.contains("Syncing OpenAgere Marketplace...")
            && popup.contains("Plugins will appear here soon."),
        "expected empty /plugins state to mention background marketplace sync, got:\n{popup}"
    );
}

#[tokio::test]
async fn plugins_popup_empty_openagere_tab_mentions_background_sync() {
    let (mut chat, _rx, _op_rx) = make_chatwidget_manual(/*model_override*/ None).await;
    chat.set_feature_enabled(Feature::Plugins, /*enabled*/ true);

    render_loaded_plugins_popup(&mut chat, plugins_test_response(Vec::new()));
    ...
    assert!(
        popup.contains("Syncing OpenAgere Marketplace...")
            && popup.contains("Plugins will appear here soon."),
        "expected empty OpenAgere Marketplace tab to mention background sync, got:\n{popup}"
    );
}
...
async fn plugins_popup_truncates_long_descriptions_in_list_rows() {
    ...
    insta::assert_snapshot!(
        verbose_row,
        @"  [-] Verbose Plugin  Available · OpenAgere Marketplace · This descri…"
    );
    assert!(
        !popup
...
async fn plugins_popup_openai_curated_tab_omits_marketplace_in_rows() {
    ...
    let popup = render_bottom_popup(&chat, /*width*/ 100);
    assert!(
        popup.contains("OpenAgere Marketplace."),
        "expected OpenAgere Marketplace tab header, got:\n{popup}"
    );
    assert!(
        !popup.contains("Codex official"),
        "expected default marketplace to ignore source displayName, got:\n{popup}"
    );
    assert!(
        popup.contains("Calendar") && !popup.contains("Repo Plugin"),
        "expected OpenAgere Marketplace tab to show only official marketplace plugins, got:\n{popup}"
    );
    assert!(
        !popup.contains("OpenAgere Marketplace ·"),
...
async fn realtime_audio_picker_emits_persist_event() {
    let (mut chat, mut rx, _op_rx) = make_chatwidget_manual(Some("gpt-5.3")).await;
    chat.open_realtime_audio_device_selection_with_names(
...
async fn server_overloaded_error_does_not_switch_models() {
    let (mut chat, mut rx, mut op_rx) = make_chatwidget_manual(Some("gpt-5.3")).await;
    ...
    chat.handle_agere_event(Event {
        ...
        msg: EventMsg::Error(ErrorEvent {
            ...
            agere_error_info: Some(AgereErrorInfo::ServerOverloaded),
        }),
    });
...
async fn feedback_upload_consent_popup_snapshot() {
    ...
    chat.show_selection_view(crate::bottom_pane::feedback_upload_consent_params(
        ...
        &agere_feedback::FeedbackDiagnostics::new(vec![agere_feedback::FeedbackDiagnostic {
