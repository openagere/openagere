...
use agere_api::RealtimeAudioFrame;
use agere_api::RealtimeEvent;
use agere_api::RealtimeEventParser;
use agere_api::RealtimeSessionConfig;
use agere_api::RealtimeSessionMode;
use agere_api::RealtimeWebsocketClient;
use agere_api::RealtimeWebsocketEvents;
use agere_api::RealtimeWebsocketWriter;
...
use agere_login::default_client::default_headers;
use agere_login::read_openai_api_key_from_env;
...
use agere_protocol::protocol::RealtimeConversationSdpEvent;
use agere_protocol::protocol::RealtimeConversationStartedEvent;
use agere_protocol::protocol::RealtimeHandoffRequested;
...
impl RealtimeConversationManager {
    ...
    async fn start(&self, start: RealtimeStart) -> AgereResult<RealtimeStartOutput> {
        let previous_state = {
    ...
    async fn start_inner(&self, start: RealtimeStart) -> AgereResult<RealtimeStartOutput> {
        let RealtimeStart {
            api_provider,
            extra_headers,
            session_config,
    ...
    pub(crate) async fn audio_in(&self, frame: RealtimeAudioFrame) -> AgereResult<()> {
        let sender = {
        ...
        match sender.try_send(frame) {
            ...
            Err(TrySendError::Closed(_)) => Err(AgereErr::InvalidRequest(
                "conversation is not running".to_string(),
            )),
        }
    }

    pub(crate) async fn text_in(&self, text: String) -> AgereResult<()> {
        let sender = {
            let guard = self.state.lock().await;
            guard
                .as_ref()
        ...
        sender
            .send(text)
            .await
            .map_err(|_| AgereErr::InvalidRequest("conversation is not running".to_string()))?;
        Ok(())
    }

    pub(crate) async fn handoff_out(&self, output_text: String) -> AgereResult<()> {
        let handoff = {
            let guard = self.state.lock().await;
    ...
    pub(crate) async fn handoff_complete(&self) -> AgereResult<()> {
        let handoff = {
            let guard = self.state.lock().await;
    ...
    pub(crate) async fn shutdown(&self) -> AgereResult<()> {
        let state = {
            let mut guard = self.state.lock().await;
            guard.take()
        };
...
async fn prepare_realtime_start(
    sess: &Arc<Session>,
    params: ConversationStartParams,
) -> AgereResult<PreparedRealtimeConversationStart> {
...
) -> AgereResult<RealtimeSessionConfig> {
    let config = sess.get_config().await;
...
fn validate_realtime_voice(version: RealtimeWsVersion, voice: RealtimeVoice) -> AgereResult<()> {
    let voices = RealtimeVoicesList::builtin();
    ...
    let allowed = allowed
        ...
        .collect::<Vec<_>>()
        .join(", ");
    Err(AgereErr::InvalidRequest(format!(
...
pub(crate) async fn handle_audio(
    ...
    if let Err(err) = sess.conversation.audio_in(params.frame).await {
        ...
        if sess.conversation.running_state().await.is_some() {
            ...
            send_conversation_error(sess, sub_id, err.to_string(), AgereErrorInfo::BadRequest)
                .await;
        }
    }
}
...
fn realtime_api_key(auth: Option<&AgereAuth>, provider: &ModelProviderInfo) -> AgereResult<String> {
    ...
    if let Some(api_key) = auth.and_then(AgereAuth::api_key) {
...
fn realtime_request_headers(
    ...
    if let Some(api_key) = api_key {
        let auth_value = HeaderValue::from_str(&format!("Bearer {api_key}")).map_err(|err| {
            AgereErr::InvalidRequest(format!("invalid realtime api key header: {err}"))
        })?;
...
async fn send_conversation_error(
    ...
    sub_id: String,
    message: String,
    agere_error_info: AgereErrorInfo,
) {
    sess.send_event_raw(Event {
        id: sub_id,
