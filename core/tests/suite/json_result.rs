...
async fn agere_returns_json_result_for_gpt5() -> anyhow::Result<()> {
    agere_returns_json_result("gpt-5.4".to_string()).await
}

#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn agere_returns_json_result_for_gpt54() -> anyhow::Result<()> {
    agere_returns_json_result("gpt-5.4".to_string()).await
}

async fn agere_returns_json_result(model: String) -> anyhow::Result<()> {
    ...
    let match_json_text_param = move |req: &wiremock::Request| {
        ...
        format.get("name") == Some(&serde_json::Value::String("agere_output_schema".into()))
    ...
    let TestAgere { agere, cwd, .. } = test_agere().build(&server).await?;
    ...
    let message = wait_for_event(&agere, |ev| matches!(ev, EventMsg::AgentMessage(_))).await;
