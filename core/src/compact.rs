...
use agere_analytics::AgereCompactionEvent;
use agere_analytics::CompactionImplementation;
use agere_analytics::CompactionPhase;
use agere_analytics::CompactionReason;
use agere_analytics::CompactionStatus;
use agere_analytics::CompactionStrategy;
use agere_analytics::CompactionTrigger;
use agere_analytics::now_unix_seconds;
...
use agere_utils_output_truncation::truncate_text;
use futures::prelude::*;
use tracing::error;
...
async fn run_compact_task_inner_impl(
    ...
    loop {
        ...
        match attempt_result {
            ...
            Err(AgereErr::Interrupted) => {
                return Err(AgereErr::Interrupted);
            }
            Err(e @ AgereErr::ContextWindowExceeded) => {
                if turn_input_len > 1 {
    ...
    sess.emit_turn_item_completed(&turn_context, compaction_item, false)
        .await;
...
impl CompactionAnalyticsAttempt {
    ...
    pub(crate) async fn track(
        ...
        sess.services
            ...
            .track_compaction(AgereCompactionEvent {
...
pub(crate) fn compaction_status_from_result<T>(result: &AgereResult<T>) -> CompactionStatus {
    match result {
        Ok(_) => CompactionStatus::Completed,
        Err(AgereErr::Interrupted | AgereErr::TurnAborted) => CompactionStatus::Interrupted,
