...
pub async fn init(config: &impl RolloutConfigView) -> Option<StateDbHandle> {
    ...
    let runtime = match agere_state::StateRuntime::init(
...
pub async fn get_state_db(config: &impl RolloutConfigView) -> Option<StateDbHandle> {
    let state_path = agere_state::state_db_path(config.sqlite_home());
...
pub async fn open_if_present(agere_home: &Path, default_provider: &str) -> Option<StateDbHandle> {
    let db_path = agere_state::state_db_path(agere_home);
    if !tokio::fs::try_exists(&db_path).await.unwrap_or(false) {
        return None;
    }
    let runtime =
        agere_state::StateRuntime::init(agere_home.to_path_buf(), default_provider.to_string())
            .await
            .ok()?;
    require_backfill_complete(runtime, agere_home).await
}
...
async fn require_backfill_complete(
    ...
    match runtime.get_backfill_state().await {
        Ok(state) if state.status == agere_state::BackfillStatus::Complete => Some(runtime),
        Ok(state) => {
            warn!(
...
fn cursor_to_anchor(cursor: Option<&Cursor>) -> Option<agere_state::Anchor> {
    let cursor = cursor?;
...
pub async fn list_thread_ids_db(
    context: Option<&agere_state::StateRuntime>,
    agere_home: &Path,
    page_size: usize,
    ...
    let ctx = context?;
    if ctx.agere_home() != agere_home {
        warn!(
            "state db agere_home mismatch: expected {}, got {}",
            ctx.agere_home().display(),
            agere_home.display()
        );
    }
    ...
    match ctx
        .list_thread_ids(
            ...
            anchor.as_ref(),
            match sort_key {
                ThreadSortKey::CreatedAt => agere_state::SortKey::CreatedAt,
                ThreadSortKey::UpdatedAt => agere_state::SortKey::UpdatedAt,
            },
...
pub async fn list_threads_db(
...
) -> Option<agere_state::ThreadsPage> {
    let ctx = context?;
    if ctx.agere_home() != agere_home {
        warn!(
            "state db agere_home mismatch: expected {}, got {}",
    ...
    match ctx
        .list_threads(
            page_size,
            agere_state::ThreadFilterOptions {
                ...
                sort_direction: match sort_direction {
                    SortDirection::Asc => agere_state::SortDirection::Asc,
                    SortDirection::Desc => agere_state::SortDirection::Desc,
                },
                search_term,
            },
        )
